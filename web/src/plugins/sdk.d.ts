/**
 * Athena Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__ATHENA_PLUGIN_SDK__`` and
 * ``window.__ATHENA_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__ATHENA_PLUGIN_SDK__`` because:
 ...
 *
 * Versioning: bump ``AthenaPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@athena/dashboard-plugin-sdk`` types package, or
 ...
 * of calling ``fetch`` with a hand-read ``window.__ATHENA_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__ATHENA_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__ATHENA_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface AthenaPluginSDK {
...
declare global {
  interface Window {
    __ATHENA_PLUGIN_SDK__?: AthenaPluginSDK;
    __ATHENA_PLUGINS__?: PluginRegistry;
  }
}
