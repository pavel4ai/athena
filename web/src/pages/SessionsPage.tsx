...
const SOURCE_CONFIG: Record<string, { icon: typeof Terminal; color: string }> =
  {
    ...
    athena_flow: { icon: Play, color: "text-warning" },
...
const NO_MATCHING_SESSION_SOURCE = "__athena_dashboard_no_matching_source__";
        ...
        const res = await fetch(api.exportSessionUrl(id, rowProfile(id)), {
          ...
          headers: {
            "X-Athena-Session-Token":
              (window as unknown as { __ATHENA_SESSION_TOKEN__?: string })
                .__ATHENA_SESSION_TOKEN__ ?? "",
          },
        });
