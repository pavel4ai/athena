...
interface AthenaConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function AthenaConsoleModal({ open, onClose }: AthenaConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("athena> ");
  const inputPromptRef = useRef("athena> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "athena> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Athena Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="athena-console-title"
    >
      <div
        className={cn(
          themedBody,
