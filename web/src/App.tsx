          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="athena-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="athena-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateAthena = status?.can_update_athena === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'athena update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "athena update";
    return (
      t.status.updateAthenaConfirmMessage ??
      `This will run 'athena update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateAthenaConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateAthena) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateAthena,
      runningLabel: t.status.updatingAthena,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Athena gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateAthenaConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateAthenaConfirmTitle ?? `${t.status.updateAthena}?`}
    />
    </>
  );
}
