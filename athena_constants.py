...
_UNSET = object()
_ATHENA_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_ATHENA_HOME_OVERRIDE", default=_UNSET
)
...
def get_athena_home() -> Path:
    ...
    Reads ATHENA_HOME env var, falls back to the platform-native default.
    This is the single source of truth — all other copies should import this.

    When ``ATHENA_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``ATHENA_HOME`` explicitly (see the systemd
    template in ``athena_cli/gateway.py`` and the kanban dispatcher in
    ``athena_cli/kanban_db.py``).  See https://github.com/pavel4ai/athena/issues/18594.
    """
    ...
    return _get_platform_default_athena_home()
...
def get_default_athena_root() -> Path:
    ...
    (``~/.athena`` on POSIX, ``%LOCALAPPDATA%\\athena`` on native Windows).

    In Docker or custom deployments where ``ATHENA_HOME`` points outside
    ``~/.athena`` (e.g. ``/opt/data``), returns ``ATHENA_HOME`` directly
    — that IS the root.

    In profile mode where ``ATHENA_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.athena/profiles/coder``) and Docker
    ...
    try:
        ...
        # ATHENA_HOME is under ~/.athena (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — ATHENA_HOME itself is the root
    return env_path
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``ATHENA_OPTIONAL_SKILLS``.
    """
    override = os.getenv("ATHENA_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_athena_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``ATHENA_OPTIONAL_MCPS``.
    """
    override = os.getenv("ATHENA_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_athena_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``ATHENA_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Wheel-installed ``<sysconfig data>/skills`` (pip install path)
        3. Caller-supplied ``default`` (typically the source-checkout path)
        4. ``<ATHENA_HOME>/skills`` last-resort
    """
    override = os.getenv("ATHENA_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
...
def get_athena_dir(new_subpath: str, old_name: str) -> Path:
    ...
    Args:
        new_subpath: Preferred path relative to ATHENA_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to ATHENA_HOME (e.g. ``"image_cache"``).
...
def iter_athena_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Athena-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\athena\\node``. POSIX installs use
    ``$ATHENA_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_athena_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with athenaManagedNodePathEntries() in
...
def find_athena_node_executable(command: str) -> str | None:
    """Return a Athena-managed Node/npm executable path, if installed."""
    names = _candidate_node_command_names(command)
    for directory in iter_athena_node_dirs():
        for name in names:
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Athena-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring Athena-managed installs.

    This is for Athena-owned subprocesses that should not be broken by a bad,
    missing, or elevation-triggering system Node/npm on PATH.
    """
    return find_athena_node_executable(command) or find_node_executable_on_path(command)
...
def with_athena_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    ...
    managed = [str(path) for path in iter_athena_node_dirs() if path.is_dir()]
...
def display_athena_home() -> str:
    """Return a user-friendly display string for the current ATHENA_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.athena``
        profile:  ``~/.athena/profiles/coder``
        custom:   ``/opt/athena-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.athena``.  For code that needs a real ``Path``, use
    :func:`get_athena_home` instead.
    """
    home = get_athena_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``ATHENA_HOME`` or other path
    env vars resolve to an unexpected location.

    See https://github.com/pavel4ai/athena/issues/25821.
    """
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("ATHENA_REAL_HOME") or os.getenv("ATHENA_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Athena profile HOME.

    ``ATHENA_HOME`` scopes Athena state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={ATHENA_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{ATHENA_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{ATHENA_HOME}/home`` when it exists, preserving the
...
def get_config_path() -> Path:
    ...
    Replaces the ``get_athena_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, athena_logging.py, athena_time.py, etc.).
    """
