...
_UNSET = object()
_ATHENA_HOME_OVERRIDE: ContextVar[str | object] = ContextVar("_ATHENA_HOME_OVERRIDE", default=_UNSET)
...
def set_athena_home_override(path: str | Path | None) -> Token:
    ...
    return _ATHENA_HOME_OVERRIDE.set(value)


def reset_athena_home_override(token: Token) -> None:
    """Restore the previous context-local Athena home override."""
    _ATHENA_HOME_OVERRIDE.reset(token)


def get_athena_home_override() -> str | None:
    """Return the active context-local Athena home override, if any."""
    override = _ATHENA_HOME_OVERRIDE.get()
    return str(override) if override is not _UNSET and override else None


def _get_platform_default_athena_home() -> Path:
...
def _warn_profile_fallback_once() -> None:
    """Warn once when ATHENA_HOME is unset but a non-default profile is sticky-active (wrong fallback)."""
    global _profile_fallback_warned
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_athena_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[ATHENA_HOME fallback] ATHENA_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass ATHENA_HOME explicitly "
            f"(see issue #18594)."
        )
...
def get_athena_home() -> Path:
    """Athena home: context-local override → ``ATHENA_HOME`` env var → platform default."""
    override = get_athena_home_override()
    if override:
        return Path(override)
    if not os.environ.get("ATHENA_HOME", "").strip():
        _warn_profile_fallback_once()
    return get_process_athena_home()
...
def athena_home_key(path: str | Path | None = None) -> str:
    """Stable registry key for a Athena home/profile dir.
    ...
    """
    candidate = Path(path) if path is not None else get_athena_home()
    raw = str(candidate)
...
def reset_athena_home_key_cache() -> None:
    """Forget every remembered home key (for tests that move a home dir on disk)."""
    _HOME_KEY_CACHE.clear()


def get_process_athena_home() -> Path:
    """Athena home of the running process, ignoring task overrides.
...
# get_default_athena_root() memo keyed on (native home, ATHENA_HOME) so it stays
# fresh when a test or plugin mutates ATHENA_HOME; saves ~80us/call at 31+ sites.
_default_athena_root_memo: "tuple[str, str, Path] | None" = None


def get_default_athena_root() -> Path:
    """Root Athena dir for profile-level ops: ``<root>`` when ``ATHENA_HOME=<root>/profiles/<name>``."""
    global _default_athena_root_memo
    native_home = _get_platform_default_athena_home()
    env_home = os.environ.get("ATHENA_HOME", "")
    memo = _default_athena_root_memo
    ...
    if env_home:
        ...
        try:
            env_path.resolve().relative_to(native_home.resolve())  # under ~/.athena (normal or profile mode)
        except ValueError:  # Docker/custom root: <root>/profiles/<name> -> <root>, else ATHENA_HOME itself
            result = env_path.parent.parent if env_path.parent.name == "profiles" else env_path
    _default_athena_root_memo = (str(native_home), env_home, result)
    return result
...
# Files marking a real Athena home; arbitrary dirs with a ``profiles`` segment lack them.
_ATHENA_HOME_MARKERS = ("config.yaml", ".env", "state.db")


def _is_athena_profiles_root(profiles_dir: Path) -> bool:
    """True when *profiles_dir* is provably ``<athena-home>/profiles``.

    Accepts the classic ``~/.athena`` layout, a root carrying Athena-home marker files, a
    ...
    try:
        if (profiles_dir / _DELETED_PROFILES_DIR).is_dir() or any(
            (root / marker).exists() for marker in _ATHENA_HOME_MARKERS
        ):
            return True
    except OSError:
        pass
    try:
        return root.resolve(strict=False) == get_default_athena_root().resolve(strict=False)
    except OSError:
        return False
...
def named_profile_home(path: str | Path) -> Path | None:
    ...
    Requires ``<name>`` not to start with ``.`` and the ``profiles`` parent to be a real Athena home;
    ...
    for candidate in (current, *current.parents):
        if (candidate.parent.name == "profiles" and not candidate.name.startswith(".")
                and _is_athena_profiles_root(candidate.parent)):
            return candidate
        if candidate.name == ".athena":  # default home: a coincidental profiles/ ancestor is not a root
            return None
    return None
...
def profile_name_for_home(path: str | Path | None) -> str | None:
    ...
    The default home is the Athena root itself, so its basename is an installation detail (``.athena``
    on POSIX and commonly ``athena`` on Windows), not the profile id ``default``.
    """
...
def mkdir_under_athena_home(path: str | Path) -> Path:
...
def _packaged_dir(env_var: str, default: Path | None, subdir: str) -> Path:
    ...
    return Path(override) if override else default if default is not None else get_athena_home() / subdir


def get_optional_skills_dir(default: Path | None = None) -> Path:
    """Return the optional-skills directory, honoring package-manager wrappers."""
    return _packaged_dir("ATHENA_OPTIONAL_SKILLS", default, "optional-skills")


def get_optional_mcps_dir(default: Path | None = None) -> Path:
    """Return the optional-mcps directory, honoring package-manager wrappers (``ATHENA_OPTIONAL_MCPS``)."""
    return _packaged_dir("ATHENA_OPTIONAL_MCPS", default, "optional-mcps")


def get_bundled_skills_dir(default: Path | None = None) -> Path:
    """Return the bundled skills directory, honoring package-manager wrappers (``ATHENA_BUNDLED_SKILLS``)."""
    return _packaged_dir("ATHENA_BUNDLED_SKILLS", default, "skills")


def get_athena_dir(new_subpath: str, old_name: str, *, home: Path | None = None) -> Path:
    """Resolve a Athena subdirectory, honouring a populated legacy ``<old_name>/`` (no migration).
    ...
    """
    home = home or get_athena_home()
    old_path = home / old_name
    return old_path if _legacy_path_has_content(old_path) else home / new_subpath


def iter_athena_node_dirs(home: Path | None = None) -> list[Path]:
    """Athena-managed Node dirs in lookup order; both Windows and POSIX shapes so migrated installs work.

    Keep in sync with athenaManagedNodePathEntries() in apps/desktop/electron/backend-env.ts.
    """
    node_dir = (home or get_athena_home()) / "node"
...
def _iter_managed_node_candidates(names: list[str], home: Path | None = None):
    ...
    for directory in iter_athena_node_dirs(home):
        for name in names:
...
def _version_probe_ok(path: str) -> bool:
    """True when ``<path> --version`` exits 0 under the Athena-managed Node PATH."""
    result = _run_version_probe([path, "--version"], env=with_athena_node_path())
    return result is not None and result.returncode == 0


_ATHENA_NODE_TARGET_MAJOR = int(os.environ.get("ATHENA_NODE_TARGET_MAJOR", "22"))
_managed_node_heal_attempted = False
_NODE_BOOTSTRAP_SCRIPT = Path(__file__).resolve().parent / "scripts" / "lib" / "node-bootstrap.sh"

# Install tree root (this file lives at <install_root>/athena_constants.py). Used by secure_parent_dir() to
# skip chmod on the install dir — chmodding it 0700 breaks athena-user traversal in Docker (UID 10000). See
# #25821, #93050.
...
def athena_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Athena-managed node/npm/npx shim exists on disk."""
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        "→ Athena-managed Node.js is in use by a running app; deferring its "
        "upgrade until the app is closed (re-run `athena update` afterwards).", flush=True,
    )
...
def _stage_windows_node_zip(home: Path, node_arch: str) -> Path | None:
    ...
    index_url = f"https://nodejs.org/dist/latest-v{_ATHENA_NODE_TARGET_MAJOR}.x/"
    index_bytes = _fetch_url(index_url, 60)
    if index_bytes is None:
        return None
    pattern = rf"node-v{_ATHENA_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip"
...
def _heal_managed_node_windows(home: Path | None = None) -> bool | None:
    """Redownload the portable Node zip into ``%ATHENA_HOME%\\node`` on Windows.
    ...
    (images are mapped with ``FILE_SHARE_DELETE`` — the same mechanism as the athena.exe quarantine); when
...
def _run_node_bootstrap(func: str, *, timeout: int, **extra_env: str) -> bool:
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ATHENA_HOME": str(get_athena_home()), **extra_env},
...
def bootstrap_athena_managed_node() -> str | None:
    """Install a Athena-managed Node tree under ``$ATHENA_HOME/node`` and return its npm path.

    Athena never modifies a user-owned toolchain (system, nvm, brew, Nix) that fails ``engines``.
    """
    existing = find_athena_node_executable("npm")
    if existing:
        return existing
    if sys.platform == "win32":
        ok = _heal_managed_node_windows()
    else:
        # ATHENA_NODE_SKIP_LINKS=1 keeps node/npm/npx out of ~/.local/bin: never shadow the user toolchain.
        ok = _run_node_bootstrap("_nb_install_bundled_node", timeout=600, ATHENA_NODE_SKIP_LINKS="1")
    if not ok:
        return None
    return _first_runnable_managed(_candidate_node_command_names("npm"))[0]


def heal_athena_managed_node() -> bool:
    """Redownload Athena-managed Node when the tree exists but is broken; at most once per process.
    ...
    if _managed_node_heal_attempted or not athena_managed_node_tree_present():
        return False
    if sys.platform == "win32":
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    for candidate in _iter_managed_node_candidates(_candidate_node_command_names("node"), home):
        ...
        if "-" in version:
            return True
        return major < _ATHENA_NODE_TARGET_MAJOR
    return False


def find_athena_node_executable(command: str) -> str | None:
    """Athena-managed Node/npm path, healing broken/outdated trees; heal failure still returns old Node."""
    ...
    if needs_heal and heal_athena_managed_node():
...
def find_node_executable(command: str) -> str | None:
    ...
    """
    managed = find_athena_node_executable(command)
    if managed:
        return managed
    if athena_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_athena_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Athena-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    parts = [p for p in merged.get("PATH", "").split(os.pathsep) if p]
    for entry in reversed([str(path) for path in iter_athena_node_dirs() if path.is_dir()]):
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    local ``node_modules/agent-browser/bin/...`` binary, which then disappears on the next ``athena update``
...
def display_athena_home() -> str:
    """User-facing ``~/`` display string for ATHENA_HOME (``~/.athena/profiles/coder``)."""
    home = get_athena_home()
    try:  # as_posix(): str() on Windows yields chimeras like ~/AppData\Local\athena/skills/
...
def profile_cli_selector() -> str:
    """``-p <name> `` (trailing space) pinning copy-pasteable ``athena ...`` guidance to the
    active NAMED profile, else ``""``: a bare ``athena`` follows the sticky ``active_profile``
    file, which can name a different database than the one that failed (#105887). A custom
    home outside the profile tree has no selector (only ATHENA_HOME names it)."""
    name = profile_name_for_home(get_athena_home())
    return f"-p {name} " if name and name != "default" else ""


def secure_parent_dir(path: Path) -> None:
    """Chmod ``0o700`` on *path*'s parent, refusing ``/`` and top-level dirs (misresolved ATHENA_HOME)."""
    parent = path.parent.resolve()
    if parent == Path("/") or len(parent.parts) < 3:
        return
    # Refuse the install tree: chmod 0700 breaks athena-user traversal in Docker (UID 10000).
    # A credential file here means ATHENA_HOME misresolved; surface it (caused production lockouts).
    # See #25821, #93050.
    if parent == _INSTALL_ROOT or _INSTALL_ROOT in parent.parents:
        ...
        logging.getLogger(__name__).warning(
            ...
            "athena-agent install directory (%s). Credential files are "
            "normally stored under the athena home directory instead.", parent, _INSTALL_ROOT,
        )
        return
    with contextlib.suppress(OSError):
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{ATHENA_HOME}/home`` when the profile-home directory exists."""
    athena_home = get_athena_home_override() or (env or {}).get("ATHENA_HOME") or os.getenv("ATHENA_HOME")
    if not athena_home:
        return None
    profile_home = os.path.join(athena_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates = [_env_get(env, "ATHENA_REAL_HOME"), _env_get(env, "HOME")]
    with contextlib.suppress(Exception):
        import pwd
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """The OS user's real home, avoiding the Athena profile HOME.

    ``HOME`` belongs to the OS account and external CLIs keeping credentials under ``~``; a parent
    already running with ``HOME={ATHENA_HOME}/home`` is repaired back when possible.
    """
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    ``{ATHENA_HOME}/home``; ``real``: always real HOME; ``profile``: always the profile home.
    """
    env = env or {}
    profile_home = _profile_home_path(env)
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Map a Windows-host cwd (drive path or ``\\\\wsl.localhost\\`` UNC) to POSIX when Athena runs in WSL.
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under ATHENA_HOME."""
    return get_athena_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under ATHENA_HOME."""
    return get_athena_home() / "skills"


def get_env_path() -> Path:
    """Return the path to the ``.env`` file under ATHENA_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_athena_ipv4_patched", False):
        return
    ...
    def _ipv4_getaddrinfo(host, port, family=0, type=0, proto=0, flags=0):
        ...
        return _original_getaddrinfo(host, port, family, type, proto, flags)
    _ipv4_getaddrinfo._athena_ipv4_patched = True  # type: ignore[attr-defined]
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    Canonical helper for venv layout. This was open-coded in seven places across four ``athena_cli`` modules
    ...
    lived 2400 lines away in another function. A few sites outside ``athena_cli``
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* ships with Athena (exact first segment; ``startswith`` would claim ``agentops``)."""
    root = str(name).split(".")[0] if name else ""
    return bool(root) and (root in FIRST_PARTY_MODULE_ROOTS or root.startswith("athena_"))
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    return [
        ...
        "    athena update",
        "If that also fails, reinstall: https://athena-agent.nousresearch.com",
    ]
