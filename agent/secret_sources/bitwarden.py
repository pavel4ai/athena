...
so they don't have to live in plaintext in ``~/.athena/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<athena_home>/bin/bws`` on
...
* The access token is stored in ``~/.athena/.env`` as
...
# In-process cache so repeated load_athena_dotenv() calls (CLI startup,
...
# <athena_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.athena/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under athena_home/cache/.

    `home_path` is what `load_athena_dotenv()` already resolved; falling back
    to `$ATHENA_HOME` / `~/.athena` keeps direct callers working too.
    """
    if home_path is None:
        home_path = Path(os.getenv("ATHENA_HOME", Path.home() / ".athena"))
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<athena_home>/bin/bws``  (our managed copy — preferred)
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<athena_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
...
# ---------------------------------------------------------------------------
# Public entry point — called from athena_cli.env_loader
# ---------------------------------------------------------------------------
