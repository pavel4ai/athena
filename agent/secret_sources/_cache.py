...
a cache problem must never block Athena startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """``home_path`` as resolved by ``load_athena_dotenv()``, else ``$ATHENA_HOME``/``~/.athena``."""
    if home_path is None:
        from athena_constants import get_athena_home

        home_path = get_athena_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend at ``<athena_home>/cache/<basename>``::
