...
# Matches ${ATHENA_SKILL_DIR} / ${ATHENA_SESSION_ID} tokens in SKILL.md.
# Tokens that don't resolve (e.g. ${ATHENA_SESSION_ID} with no session) are
...
def substitute_template_vars(
    ...
    """Replace ${ATHENA_SKILL_DIR} / ${ATHENA_SESSION_ID} in skill content.
    ...
    def _replace(match: re.Match) -> str:
        token = match.group(1)
        if token == "ATHENA_SKILL_DIR" and skill_dir_str:
            return skill_dir_str
        if token == "ATHENA_SESSION_ID" and session_id:
