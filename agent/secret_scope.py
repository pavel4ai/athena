...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the ATHENA_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    ...
    "ATHENA_MAX_ITERATIONS", "ATHENA_MAX_TOKENS", "ATHENA_API_TIMEOUT",
...
def build_profile_secret_scope(athena_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(athena_home) / ".env")
