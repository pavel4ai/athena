...
``ANTHROPIC_API_KEY``, Athena-owned OAuth grants in the ``auth.json`` credential
pool, then ``~/.claude/.credentials.json`` / macOS Keychain as a borrowed fallback.
``~/.athena/.anthropic_oauth.json`` (Athena PKCE) and
...
_SPENT_ROTATION_SIDECAR_COMMENT = (
    ...
    "consumed server-side but never durably committed. Written by Athena so sibling "
...
def _spent_rotation_sidecar_path(source_path: Path) -> Path:
    return source_path.with_name(source_path.name + ".athena-spent-rotations.json")
...
_SINGLETON_SOURCE_PATHS = {
    "claude_code": lambda: claude_code_credentials_path(), "athena_pkce": lambda: _get_athena_oauth_file()
}
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    try:
        from athena_cli.auth import AUTH_LOCK_TIMEOUT_SECONDS, _auth_store_lock, env_float
        refresh_timeout_seconds = env_float("ATHENA_ANTHROPIC_REFRESH_TIMEOUT_SECONDS", 20)
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    """Prefer refreshable Claude Code creds over a static env OAuth token: Athena historically persisted setup tokens
...
def _resolve_anthropic_pool_token(*, skip_borrowed: bool = False) -> Optional[str]:
    ...
    refresh=False`` (never ``select()``) so diagnostic call sites (account_usage, ``athena models``) never mutate
...
# ── Athena-native PKCE OAuth flow (~/.athena/.anthropic_oauth.json); mirrors Claude Code / pi-ai / OpenCode ──


def _get_athena_oauth_file() -> Path:
    return get_athena_home() / ".anthropic_oauth.json"


def _root_athena_oauth_file() -> Optional[Path]:
    ...
    try:
        ...
        return None if root.resolve(strict=False) == get_athena_home().resolve(strict=False) else root / ".anthropic_oauth.json"
    except Exception:
        return None
...
def run_athena_oauth_login_pure() -> Optional[Dict[str, Any]]:
    ...
    print("\n".join([
        "", "Authorize Athena with your Claude Pro/Max subscription.", "",
    ...
    try:
        from athena_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_athena_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Athena-managed OAuth credentials from ~/.athena/.anthropic_oauth.json."""
    data = _load_json_if_exists(_get_athena_oauth_file(), "Athena OAuth credentials")
    return data if data is not None and data.get("accessToken") else None


def _write_athena_oauth_credentials(
    access_token: str, refresh_token: Optional[str], expires_at_ms: Optional[int], *, target: Optional[Path] = None
) -> None:
    """Commit refreshed athena_pkce tokens to ~/.athena/.anthropic_oauth.json (``CredentialPersistError`` on failure).
    ``target`` lets a named profile commit a grant it BORROWED from the global root back to the ROOT singleton
    instead of forking a copy under its own ATHENA_HOME; without this write-through the next ``load_pool()``
    re-seeds the stale (consumed) pair from the file over the rotated pool entry."""
    _commit_private_json(
        target if target is not None else _get_athena_oauth_file(),
        {"accessToken": access_token, "refreshToken": refresh_token, "expiresAt": expires_at_ms},
        "Athena OAuth credentials",
    )
