...
def _record_codex_app_server_usage(agent, turn, messages=None) -> dict[str, Any]:
    """Translate Codex app-server token usage into Athena accounting. Prompt bucket = uncached + cached
    ...
    estimate grows monotonically and athena-mode fires thread compaction on tiny threads (#100381)."""
...
def _record_codex_app_server_compaction(agent, turn, *, approx_tokens: int | None = None, force: bool = False) -> bool:
    ...
    if compressor is not None:
        ...
        # Codex owns this summary: a prior Athena deterministic-fallback flag must not leak into it.
...
# --- Codex app-server → Athena UI bridge -------------------------------------
# The app-server bypasses the Athena tool loop, so the bridge translates JSON-RPC notifications
# into the callbacks the standard runtime fires (tool_progress_callback, _fire_stream_delta, ...).

# Item types that project to a Athena tool_call (keep in sync with agent/transports/codex_event_projector.py
...
# Internal MCP server wrapping Athena' native tools: its inner dispatch has no tool_progress_callback, so the
# codex-level mcpToolCall IS the display event and the mcp.athena-tools.* prefix is stripped (users see Athena tools).
_INTERNAL_MCP_SERVER = "athena-tools"
...
def _codex_item_to_tool_name(item: dict) -> str:
    """Synthetic Athena tool name for a codex item (mirrors CodexEventProjector)."""
...
def _ensure_codex_session(agent) -> None:
    ...
    # Approval callback: Athena' standard prompt flow when a CLI thread installed one.
    ...
    # explicit approval bypass (approvals.mode: off, /yolo, --yolo, ATHENA_YOLO_MODE) hands policy to codex's sandbox.
    ...
    # Athena' gateway UI callbacks (tool_progress_callback, _fire_stream_delta,
...
def _bypass_sdk_request_transform(stream_kwargs: dict) -> dict:
    ...
    request without the walk. ATHENA_CODEX_SDK_TRANSFORM=1 disables."""
    if os.environ.get("ATHENA_CODEX_SDK_TRANSFORM", "").strip().lower() in {"1", "true", "yes", "on"}:
