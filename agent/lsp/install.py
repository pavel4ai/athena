...
``<ATHENA_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
# ``<ATHENA_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def athena_lsp_bin_dir() -> Path:
    ...
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".athena")
    p = Path(home) / "lsp" / "bin"
...
def _install_npm(
    ...
    for c in _native_binary_candidates(nm_bin):
        if c.exists():
            ...
            link = athena_lsp_bin_dir() / c.name
            if not link.exists():
                try:
                    link.symlink_to(c)
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a athena-owned target dir.
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = athena_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
