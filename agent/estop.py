...
``athena pause`` writes a sentinel at ``$ATHENA_HOME/ESTOP``; ``athena resume``
...
(fail safe, e.g. ``touch ~/.athena/ESTOP``). Ported from gastownhall/gastown estop.go (MIT).
"""
...
# Same profile-aware / fleet-root resolvers the file-safety guards use (fail-open to ~/.athena).
from agent.file_safety import _athena_home_path as _athena_home, _athena_root_path as _canonical_root
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel this process would write on `athena pause`."""
    return _athena_home() / SENTINEL_NAME


def _candidate_sentinel_paths() -> list:
    """Profile home first, then the fleet root if it is a different directory: a profile
    gateway (ATHENA_HOME=~/.athena/profiles/<n>) must still honor an operator's ~/.athena/ESTOP."""
...
def paused_reply() -> Optional[str]:
    ...
    return f"⏸️ Athena is paused{tag}. New work is on hold; run `athena resume` to pick things back up."
...
def check_paused(component: str, logger: logging.Logger) -> bool:
    ...
    if first:
        ...
        logger.info(
            "%s dispatch paused by global emergency stop%s — remove with `athena resume` (%s)",
