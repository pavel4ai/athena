...
try:
    import athena_cli as _athena_cli

    _ATHENA_VERSION = str(_athena_cli.__version__)
except Exception:
    _ATHENA_VERSION = "0.0.0"
_API_CLIENT = f"athena-agent/{_ATHENA_VERSION}"  # client context per Gemini's partner-integration guidance
...
_FREE_TIER_GUIDANCE = (
    ...
    "Athena typically makes 3-10 API calls per user turn, so the free tier is exhausted in a handful of "
...
_STANDARD_KEY_GUIDANCE = (
    ...
    "generativelanguage.googleapis.com). Then update GEMINI_API_KEY / GOOGLE_API_KEY in ~/.athena/.env and "
...
_MISSING_KEY_ERROR = (
    ...
    "environment / ~/.athena/.env (get one at https://aistudio.google.com/app/apikey), or run `athena setup` to "
...
class GeminiNativeClient:
    ...
    # (No ATHENA_SKIP_ASYNC_WRAP — the async path has a real conversion, AsyncGeminiNativeClient.)
