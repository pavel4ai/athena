...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``ATHENA_KANBAN_TASK`` / ``ATHENA_KANBAN_BOARD`` in the
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``athena_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``ATHENA_PLATFORM`` or
            ``ATHENA_SESSION_PLATFORM`` env vars.  Returns the global
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.athena/skills/`` are silently skipped.
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against ATHENA_HOME, not cwd
        if not p.is_absolute():
            p = (athena_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.athena/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    athena = metadata.get("athena") or {}
    if not isinstance(athena, dict):
        athena = {}
    return {
        "fallback_for_toolsets": athena.get("fallback_for_toolsets", []),
        "requires_toolsets": athena.get("requires_toolsets", []),
        "fallback_for_tools": athena.get("fallback_for_tools", []),
        "requires_tools": athena.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    athena = metadata.get("athena")
    if not isinstance(athena, dict):
        return []
    raw = athena.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Athena metadata, VCS, virtualenv/dependency, cache, and skill
