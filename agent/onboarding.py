...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Athena starts and finds ``~/.openclaw/``.

    Points users at ``athena claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``athena claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Athena, run "
        "`athena claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `athena claw cleanup` (renames it to ~/.openclaw.pre-migration — "
