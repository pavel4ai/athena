...
``athena auth remove <provider> <N>`` must make the entry stay gone across
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    """env:<VAR> — clear from ~/.athena/.env; hint when the shell exports it."""
    from athena_cli.config import get_env_path, remove_env_value
    ...
    # .env as utf-8-sig like athena_cli/config.py: a BOM-sensitive read would
...
def _remove_athena_pkce(provider: str, removed) -> RemovalResult:
    """~/.athena/.anthropic_oauth.json is ours — delete it outright."""
    from athena_constants import get_athena_home

    result = RemovalResult()
    oauth_file = get_athena_home() / ".anthropic_oauth.json"
...
_REGISTRY: List[RemovalStep] = [
    ...
    RemovalStep(
        provider="anthropic", source_id="athena_pkce",
        remove_fn=_remove_athena_pkce,
        description="~/.athena/.anthropic_oauth.json",
    ),
    RemovalStep(
