...
Athena seeds its credential pool from many places:
    ...
    env:<VAR>     — os.environ / ~/.athena/.env
    claude_code   — ~/.claude/.credentials.json
    athena_pkce   — ~/.athena/.anthropic_oauth.json
...
qwen-cli, nous device_code (partial), athena_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.athena/.env  → clear it
