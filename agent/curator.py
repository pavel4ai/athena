...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.athena/config.yaml. Tolerates missing file."""
    try:
...
def get_consolidate() -> bool:
    ...
    The explicit ``athena curator run --consolidate`` flag overrides this for
...
CURATOR_REVIEW_PROMPT = (
    ...
    "~/.athena/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.athena/logs/curator/``)
    ...
    authored skill data in ``~/.athena/skills/``.

    ``ensure_athena_home()`` pre-creates this dir on every CLI launch and
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every athena install already has it
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.athena/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.athena/skills/.archive/` for "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.athena/skills/.archive/`. "
    ...
    lines.append("- All archives live under `~/.athena/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    *consolidate* gates the LLM umbrella-building pass. ``None`` (the default)
    ...
    ``athena curator run --consolidate`` flag. When consolidation is off, only
