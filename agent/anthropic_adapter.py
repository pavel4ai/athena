...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``athena models``) never mutates ``~/.athena/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      ...
      4. Anthropic credential_pool OAuth entry (~/.athena/auth.json)
    ...
    # 4. Athena credential_pool OAuth entry.
...
# Stores credentials in ~/.athena/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_ATHENA_OAUTH_FILE = get_athena_home() / ".anthropic_oauth.json"
...
def read_athena_oauth_credentials() -> Optional[Dict[str, Any]]:
    ...
    if _ATHENA_OAUTH_FILE.exists():
        try:
            data = json.loads(_ATHENA_OAUTH_FILE.read_text(encoding="utf-8"))
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                ...
                text = text.replace("Athena agent", "Claude Code")
                text = text.replace("athena-agent", "claude-code")
                text = text.replace("Futurebound Corp.", "Anthropic")
                block["text"] = text
        ...
        #      a) bare Athena-native tools (``read_file``)  -> ``mcp__read_file``
