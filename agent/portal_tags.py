...
Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):
    ...
    [
        ...
        "client=athena-client-v<__version__>",
    ]

The version is sourced live from ``athena_cli.__version__`` so it auto-aligns
...
``athena_cli.__version__`` is the canonical source of truth.
"""
...
def _athena_version() -> str:
    ...
    Falls back to ``"unknown"`` if ``athena_cli`` cannot be imported (should
...
def athena_client_tag() -> str:
    ...
    Format: ``client=athena-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=athena-client-v{_athena_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=athena-agent", athena_client_tag()]
