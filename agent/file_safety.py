...
def _athena_home_path() -> Path:
    """Resolve the active ATHENA_HOME (profile-aware) without circular imports."""
    try:
        from athena_constants import get_athena_home  # local import to avoid cycles
...
def _athena_root_path() -> Path:
    ...
    try:
        from athena_constants import get_default_athena_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    athena_home = _athena_home_path()
    athena_root = _athena_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(athena_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(athena_root / ".anthropic_oauth.json"),
...
def get_safe_write_root() -> Optional[str]:
    """Return the resolved ATHENA_WRITE_SAFE_ROOT path, or None if unset."""
    root = os.getenv("ATHENA_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    athena_dirs = []
    for base in (_athena_home_path(), _athena_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in athena_dirs:
                athena_dirs.append(real)
        except Exception:
            continue

    for base_real in athena_dirs:
        try:
...
def get_read_block_error(path: str) -> Optional[str]:
    ...
    Three categories are blocked:
      ...
      * Internal Athena cache files under ``ATHENA_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under ATHENA_HOME and the global Athena
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.athena/.env`` and exfiltrate the file. The read-deny exists
...
#
# Athena profiles are separate ATHENA_HOME dirs under
...
#
# Reference: May 2026 incident where a athena-security profile session
# edited skills under both ``~/.athena/profiles/athena-security/skills/``
# AND ``~/.athena/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under ATHENA_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from ATHENA_HOME.
    ...
    """
    try:
        home_real = _athena_home_path().resolve()
...
# ``…/sandboxes/<backend>/<task>/home/.athena/…`` segment and warns
...
# (the agent's view inside the container is plain ``/root/.athena/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.athena`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.athena/…`` and returns the
    ...
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .athena / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".athena":
            return i + 4
    return None
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    ...
    Otherwise returns a dict with:
      ...
      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.athena``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.athena`` (what the
    ...
    ATHENA_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"under the real ATHENA_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.athena/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.athena/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host ATHENA_HOME)
    """
...
def get_container_mirror_warning(
    ...
    return (
        ...
        f"{info['inner_path']!r} under the real ATHENA_HOME. Use the "
