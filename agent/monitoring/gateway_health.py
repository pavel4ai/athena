...
def build_gateway_health_snapshot(
    ...
    base = {
        ...
        "athena.supervision_mode": mode if mode in _SUPERVISION_MODES else "unknown",
    }
    ...
    metrics: list[GatewayMetric] = [
        metric("athena.gateway.up", int(bool(gateway_running))),
        metric("athena.gateway.active_agents", active_agents),
        metric("athena.gateway.busy", int(bool(busy))),
        metric("athena.gateway.drainable", int(bool(drainable))),
        metric("athena.gateway.restart_requested", int(bool(runtime.get("restart_requested")))),
        metric("athena.gateway.state", 1, **{"athena.gateway.state": gateway_state}),
    ]
    fatal_count = 0
    events: list[GatewayHealthEvent | GatewayDiagnosticEvent] = []
    ...
    for platform, pdata in platforms.items():
        ...
        pattrs = {"athena.platform": str(platform), "athena.platform.state": state}
        metrics.append(metric("athena.platform.up", int(state in _RUNNING_PLATFORM_STATES), **pattrs))
        metrics.append(metric("athena.platform.degraded", int(is_degraded), **pattrs, **{"athena.error_code": error_code}))
        if is_degraded:
