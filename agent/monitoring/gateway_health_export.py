...
_OBSERVABLE_METRIC_NAMES = (
    "athena.gateway.up", "athena.gateway.state", "athena.gateway.active_agents", "athena.gateway.busy",
    "athena.gateway.drainable", "athena.gateway.restart_requested", "athena.gateway.background_work",
    "athena.gateway.background_delegations", "athena.platform.up", "athena.platform.degraded",
    "athena.cron.scheduler.heartbeat_age_seconds", "athena.cron.scheduler.last_success_age_seconds",
    "athena.cron.scheduler.catch_up_occurrences", "athena.cron.jobs.enabled", "athena.cron.jobs.running",
    "athena.cron.jobs.overdue",
)
...
class GatewayHealthExportRuntime:
    ...
    def shutdown(self) -> None:
        ...
        if closeables:
            worker = threading.Thread(target=_close, name="athena-gateway-health-export-shutdown", daemon=True)
            worker.start()
...
def _read_runtime_snapshot(config: Dict[str, Any]):
    ...
    try:
        ...
        for name, read in (
            ("athena.gateway.background_work", _read_background_work_count),
            ("athena.gateway.background_delegations", _read_background_delegations_count),
        ):
...
def _start_metric_provider(config: Dict[str, Any], sdk: Dict[str, Any]) -> Any:
    ...
    meter = provider.get_meter("athena.gateway.health")
...
class GatewayDiagnosticLogStreamer(EmitterStreamer):
    ...
    def __init__(self, config: Dict[str, Any], sdk: Dict[str, Any]):
        ...
        self._logger = self._provider.get_logger("athena.gateway.diagnostics")
        self._sdk = sdk
        self.exported = 0
...
def start_gateway_health_export(config: Dict[str, Any]) -> GatewayHealthExportRuntime:
    ...
    if metrics_on or diagnostics_on:
        try:
            ...
            logger.warning("monitoring.gateway_health_export.enabled but OTLP SDK is unavailable; install 'athena-agent[otlp]'", exc_info=True)
    ...
    if diagnostics_on:
        try:
            ...
            thread = threading.Thread(target=_run, name="athena-gateway-health-export", daemon=True)
            thread.start()
