...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `athena backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - athena_home (str): The active ATHENA_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.athena``.
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'athena memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], athena_home: str) -> None:
        ...
        Called by 'athena memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``athena_home`` is the active ATHENA_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE ATHENA_HOME.

        ``athena backup`` only walks ATHENA_HOME, so any provider state kept
        ...
        of the archive, and ``athena import`` restores them to their original
