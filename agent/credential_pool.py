        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("athena_pkce"),
                )
                updated = replace(
                    entry,
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("athena_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    try:
        from athena_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
    ...
    if provider == "anthropic":
        ...
        # Claude Code CLI, athena_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.athena/.env over os.environ for the
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"athena_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_athena_oauth_credentials
        ...
        # MiniMax OAuth tokens live in ~/.athena/auth.json providers.minimax-oauth.
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.athena/.env over os.environ — the user's config file is the
    ...
    # won't be re-seeded from the user's shell environment or ~/.athena/.env.
    ...
    try:
        from athena_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    if provider == "openrouter":
        # Prefer ~/.athena/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `athena auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Athena
