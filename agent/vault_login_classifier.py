...
INSPECTION_STAMP_ATTR = "data-athena-vault-slot"
...
_LOGIN_CONTROL_INSPECTION_JS_TEMPLATE = """(() => {
  ...
  elements.forEach((element, index) => element.setAttribute("data-athena-vault-slot", nonce + ":" + index));
...
_FILL_JS_TEMPLATE = """(() => {
  ...
  for (const f of fills) {
    const el = document.querySelector('[data-athena-vault-slot="' + nonce + ':' + f.index + '"]');
  ...
  document.querySelectorAll("[data-athena-vault-slot]").forEach((n) => n.removeAttribute("data-athena-vault-slot"));
