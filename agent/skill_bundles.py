...
Storage
-------
Bundles live in ``~/.athena/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``ATHENA_HOME``). Each file looks like::
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under ATHENA_HOME.

    Honors ``ATHENA_BUNDLES_DIR`` for tests; falls back to
    ``<ATHENA_HOME>/skill-bundles``.
    """
    override = os.environ.get("ATHENA_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_athena_home() / "skill-bundles"
