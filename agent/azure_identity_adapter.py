...
class EntraIdentityConfig:
    ...
    ``athena_cli/runtime_provider.py:1310-1377`` for the analogous
...
def _build_default_credential(config: EntraIdentityConfig) -> Any:
    ...
    ``~/.athena/.env`` or the deployment environment.
    """
...
def materialize_bearer_for_http(value: Any) -> str:
    ...
    header outside the OpenAI SDK (e.g. ``athena_cli/azure_detect.py``).
