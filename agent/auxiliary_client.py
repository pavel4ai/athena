...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.athena/auth.json active provider)
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``ATHENA_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``ATHENA_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
...
# in lockstep with athena_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    ...
    Computed at call time so a hot-reloaded ``athena_cli.__version__`` is
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``athena_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.athena/auth.json for an active Nous provider.
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    Falls back to ``athena_cli.auth``'s singleton runtime resolver for older
...
def _try_azure_foundry(
    *,
    ...
    :func:`athena_cli.runtime_provider._resolve_azure_foundry_runtime` —
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second athena
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Athena' built-in discovery
...
def _resolve_auto(
    ...
    #    old OPENAI_BASE_URL lingers in ~/.athena/.env. ──
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`athena_cli.plugins.PluginContext.register_auxiliary_task`) the
