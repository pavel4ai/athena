"""OpenAI-shape bridge shared by Athena' ACP clients.

ACP has no OpenAI-style ``tools``/``tool_calls`` channel, so Athena' tool schemas travel INTO the
...
read/edit/execute tools forwards only Athena' agent-level tools, since re-offering overlapping ones
makes Athena redo finished work.
"""
...
class StreamChunks(list):
    """Chunk list that also carries response-level attributes (e.g. ``athena_projected_messages``)
    Athena reads off the ``create`` result; a plain list would drop them on the stream path."""
