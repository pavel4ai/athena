...
def compress_context(
    ...
    *,
    ...
    # long-lived ``athena_state.SessionDB`` class still bound to the
    ...
    if agent._session_db:
        try:
            ...
            if in_place:
                ...
                # from ``athena_logging._session_context`` (set once per turn in
                ...
                try:
                    agent._session_db.create_session(
                        ...
                        source=agent.platform or os.environ.get("ATHENA_SESSION_SOURCE", "cli"),
                        model=agent.model,
    ...
    # engines (e.g. athena-lcm) use boundary_reason="compression" to preserve
    # DAG lineage / checkpoint per-session state across the boundary instead of
    # re-initializing fresh. See athena-lcm#68. Built-in ContextCompressor
    ...
    def _shrink_data_url(url: str) -> tuple:
        ...
        try:
            ...
            tmp = tempfile.NamedTemporaryFile(
                prefix="athena_shrink_", suffix=suffix, delete=False,
            )
            try:
                tmp.write(raw)
                tmp.close()
