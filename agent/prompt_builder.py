...
from athena_constants import get_athena_home, get_skills_dir, is_wsl
...
_ATHENA_MD_NAMES = (".athena.md", "ATHENA.md")


def _find_athena_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.athena.md`` or ``ATHENA.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Athena Agent, an intelligent AI assistant created by Futurebound Corp.. "
...
ATHENA_AGENT_HELP_GUIDANCE = (
    "You run on Athena Agent (by Futurebound Corp.). When the user needs help with "
    "Athena itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://github.com/pavel4ai/athena/docs is your "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.athena/kanban.db`. Your task id is in "
    "`$ATHENA_KANBAN_TASK`; your workspace is `$ATHENA_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $ATHENA_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $ATHENA_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${ATHENA_KANBAN_BRANCH:-wt/$ATHENA_KANBAN_TASK}` from the main repo, then "
    "cd there.\n"
    ...
    "every assignee in a real profile (`athena profile list`, or ask the user), "
...
#
# The athena-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to athena-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `athena computer-use doctor` and share the output. That command "
...
def build_environment_hints() -> str:
    ...
    # it. ATHENA_DESKTOP marks the backend powering the chat; ATHENA_DESKTOP_TERMINAL
    # marks a athena launched in the embedded terminal pane. Both set by main.cjs.
    _truthy = ("1", "true", "yes")
    _in_desktop = (os.getenv("ATHENA_DESKTOP") or "").strip().lower() in _truthy
    _in_desktop_term = (os.getenv("ATHENA_DESKTOP_TERMINAL") or "").strip().lower() in _truthy
    ...
    extra = (os.getenv("ATHENA_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.athena/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("ATHENA_PLATFORM")
        or get_session_env("ATHENA_SESSION_PLATFORM")
        or ""
    )
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: athena setup, athena setup tools, athena setup terminal, athena status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from ATHENA_HOME and return its content, or None.
    ...
    soul_path = get_athena_home() / "SOUL.md"
...
def _load_athena_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.athena.md / ATHENA.md — walk to git root."""
    athena_md_path = _find_athena_md(cwd_path)
    if not athena_md_path:
        return ""
    try:
        content = athena_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = athena_md_path.name
        try:
            rel = str(athena_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".athena.md", context_length=context_length,
            read_path=str(athena_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", athena_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .athena.md / ATHENA.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from ATHENA_HOME is independent and always included when present.
    ...
    project_context = (
        _load_athena_md(cwd_path, context_length)
    ...
    # SOUL.md from ATHENA_HOME only — skip when already loaded as identity
    if not skip_soul:
