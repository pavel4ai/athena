...
Language resolution order:
    ...
    2. ``ATHENA_LANGUAGE`` environment variable (for tests / quick override)
...
def _locales_dir() -> Path:
    ...
    1. ``ATHENA_BUNDLED_LOCALES`` env var -- set by the Nix wrapper (or any
...
def _load_catalog(lang: str) -> dict[str, str]:
    ...
    try:
        import yaml  # PyYAML is already a athena dependency
...
def reset_language_cache() -> None:
    ...
    Call after :func:`athena_cli.config.save_config` if a running process
