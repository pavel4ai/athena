...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .athena.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / ATHENA_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.athena/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from athena_cli.config import load_config as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind ATHENA_DEV_CREDITS.
    ...
    # both live under ~/.athena/logs/.  Idempotent, so gateway mode
    ...
    if agent.verbose_logging:
        ...
        # on the fact that athena_logging.setup_logging() does not
        ...
        # handler level inside athena_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        try:
            from athena_cli.config import load_config as _load_br_cfg
    ...
    # worker (kanban_show tool is present iff ATHENA_KANBAN_TASK is set).
    ...
    try:
        ...
        os.environ["ATHENA_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.athena/sessions/ alongside gateway sessions
    athena_home = get_athena_home()
    agent.logs_dir = athena_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.athena/sessions/session_{sid}.json)
    ...
    try:
        from athena_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from athena_cli.config import load_config as _load_agent_config
    ...
    # Lets an enterprise admin append to or replace Athena' built-in
