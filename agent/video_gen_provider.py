...
as ``kind: backend``) or ``~/.athena/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _videos_cache_dir() -> Path:
    """Return ``$ATHENA_HOME/cache/videos/``, creating parents as needed."""
    from athena_constants import get_athena_home

    path = get_athena_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$ATHENA_HOME/cache/videos/``.
