...
RUN set -eu; \
    ...
    # 'Athena WebUI' catalog). The image has moved to s6-overlay /init\
...
# Keep /opt/athena immutable for the runtime athena user. Hosted/container
...
# supervised Athena processes drop to the non-root athena user.
USER root
...
ARG ATHENA_GIT_SHA=
RUN if [ -n "${ATHENA_GIT_SHA}" ]; then \
        chmod u+w /opt/athena && \
        printf '%s\n' "${ATHENA_GIT_SHA}" > /opt/athena/.athena_build_sha && \
        chmod a-w /opt/athena /opt/athena/.athena_build_sha; \
    fi
