...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``athena gateway enroll`` writes them to
    ``~/.athena/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_endpoint() -> Optional[str]:
    ...
    ``ATHENA_DASHBOARD_PUBLIC_URL``); a hosted/NAS container has the same var
...
def self_provision_relay() -> bool:
    ...
    up. The creds live ONLY in process memory — never written to ``~/.athena/.env``.
    ...
    sets neither ``ATHENA_MANAGED`` nor a ``.managed`` marker), so gating on it
    ...
    self-guarding:
      ...
      - A self-hosted operator who ran ``athena gateway enroll``: has a PINNED
