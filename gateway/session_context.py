...
(``ATHENA_SESSION_PLATFORM``, ``ATHENA_SESSION_CHAT_ID``, etc.) with
...
messages arrive at the same time the old code did:
    ...
    os.environ["ATHENA_SESSION_THREAD_ID"] = str(context.source.thread_id)
...
``os.getenv("ATHENA_SESSION_*", ...)`` calls.  Existing tool code only
needs to replace the import + call site:

    # before
    import os
    platform = os.getenv("ATHENA_SESSION_PLATFORM", "")
...
_SESSION_PLATFORM: ContextVar = ContextVar("ATHENA_SESSION_PLATFORM", default=_UNSET)
_SESSION_SOURCE: ContextVar = ContextVar("ATHENA_SESSION_SOURCE", default=_UNSET)
_SESSION_CHAT_ID: ContextVar = ContextVar("ATHENA_SESSION_CHAT_ID", default=_UNSET)
_SESSION_CHAT_NAME: ContextVar = ContextVar("ATHENA_SESSION_CHAT_NAME", default=_UNSET)
_SESSION_THREAD_ID: ContextVar = ContextVar("ATHENA_SESSION_THREAD_ID", default=_UNSET)
_SESSION_USER_ID: ContextVar = ContextVar("ATHENA_SESSION_USER_ID", default=_UNSET)
_SESSION_USER_NAME: ContextVar = ContextVar("ATHENA_SESSION_USER_NAME", default=_UNSET)
_SESSION_KEY: ContextVar = ContextVar("ATHENA_SESSION_KEY", default=_UNSET)
_SESSION_ID: ContextVar = ContextVar("ATHENA_SESSION_ID", default=_UNSET)
# ID of the message that triggered the current turn. Used as a reply anchor
# so background-process notifications stay inside the originating Telegram
# private-chat topic (those lanes route only with thread id + reply anchor).
_SESSION_MESSAGE_ID: ContextVar = ContextVar("ATHENA_SESSION_MESSAGE_ID", default=_UNSET)
...
_SESSION_ASYNC_DELIVERY: ContextVar = ContextVar("ATHENA_SESSION_ASYNC_DELIVERY", default=_UNSET)

# Cron auto-delivery vars — set per-job in run_job() so concurrent jobs
# don't clobber each other's delivery targets.
_CRON_AUTO_DELIVER_PLATFORM: ContextVar = ContextVar("ATHENA_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID: ContextVar = ContextVar("ATHENA_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID: ContextVar = ContextVar("ATHENA_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)

_VAR_MAP = {
    "ATHENA_SESSION_PLATFORM": _SESSION_PLATFORM,
    "ATHENA_SESSION_SOURCE": _SESSION_SOURCE,
    "ATHENA_SESSION_CHAT_ID": _SESSION_CHAT_ID,
    "ATHENA_SESSION_CHAT_NAME": _SESSION_CHAT_NAME,
    "ATHENA_SESSION_THREAD_ID": _SESSION_THREAD_ID,
    "ATHENA_SESSION_USER_ID": _SESSION_USER_ID,
    "ATHENA_SESSION_USER_NAME": _SESSION_USER_NAME,
    "ATHENA_SESSION_KEY": _SESSION_KEY,
    "ATHENA_SESSION_ID": _SESSION_ID,
    "ATHENA_SESSION_MESSAGE_ID": _SESSION_MESSAGE_ID,
    "ATHENA_CRON_AUTO_DELIVER_PLATFORM": _CRON_AUTO_DELIVER_PLATFORM,
    "ATHENA_CRON_AUTO_DELIVER_CHAT_ID": _CRON_AUTO_DELIVER_CHAT_ID,
    "ATHENA_CRON_AUTO_DELIVER_THREAD_ID": _CRON_AUTO_DELIVER_THREAD_ID,
}


def set_current_session_id(session_id: str) -> None:
    """Synchronize ``ATHENA_SESSION_ID`` across ContextVar and ``os.environ``.
    ...
    ``get_session_env("ATHENA_SESSION_ID")`` with an ``os.environ`` fallback,
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session context variable by its legacy ``ATHENA_SESSION_*`` name.

    Drop-in replacement for ``os.getenv("ATHENA_SESSION_*", default)``.
