...
Replaces the old ``os.environ``-based ``ATHENA_SESSION_*`` state with task-local ``ContextVar``s
...
_SESSION_VARS = (
    ...
    "ATHENA_SESSION_PLATFORM", "ATHENA_SESSION_SOURCE", "ATHENA_SESSION_CHAT_ID",
    "ATHENA_SESSION_CHAT_TYPE", "ATHENA_SESSION_CHAT_NAME", "ATHENA_SESSION_THREAD_ID",
    "ATHENA_SESSION_USER_ID", "ATHENA_SESSION_USER_ID_ALT", "ATHENA_SESSION_USER_NAME",
    "ATHENA_SESSION_SCOPE_ID", "ATHENA_SESSION_KEY", "ATHENA_SESSION_ID",
    "ATHENA_UI_SESSION_ID", "ATHENA_SESSION_MESSAGE_ID", "ATHENA_SESSION_PROFILE",
    "ATHENA_BROWSER_CONTROL_PRINCIPAL", "ATHENA_BROWSER_CONTROL_TRANSPORT_FAMILY",
    "ATHENA_CRON_SESSION", "ATHENA_SESSION_PARENT_CHAT_ID",
))
...
_SESSION_ASYNC_DELIVERY = ContextVar("ATHENA_SESSION_ASYNC_DELIVERY", default=_UNSET)
...
_SESSION_HISTORY_DELIVERY = ContextVar("ATHENA_SESSION_HISTORY_DELIVERY", default=_UNSET)

# Cron auto-delivery vars, set per-job in run_job() so concurrent jobs don't clobber.
_CRON_AUTO_DELIVER_PLATFORM = ContextVar("ATHENA_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID = ContextVar("ATHENA_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID = ContextVar("ATHENA_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)
...
def set_current_session_id(session_id: str) -> None:
    """Synchronize ``ATHENA_SESSION_ID`` across ContextVar and ``os.environ`` (tools read it
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session var by legacy ``ATHENA_SESSION_*`` name; drop-in for os.getenv.  The
...
# Surfaces that are not a human chat channel (gateway binds ATHENA_SESSION_PLATFORM, CLI/TUI/
# desktop bind ATHENA_SESSION_SOURCE, so both are consulted).  Default-deny: an unrecognized
...
def session_is_messaging_surface() -> bool:
    ...
    ``ATHENA_PLATFORM``, then the session platform, then the session source)."""
    platform = os.getenv("ATHENA_PLATFORM") or get_session_env("ATHENA_SESSION_PLATFORM", "")
    idents = (platform, get_session_env("ATHENA_SESSION_SOURCE", ""))
...
def declare_stateless_channel() -> None:
    ...
    See pavel4ai/athena#53027 and #63142.
    """
...
def async_delivery_supported() -> bool:
    ...
    (``ATHENA_KANBAN_TASK``: one-shot subprocesses whose parent disappears after the turn)."""
