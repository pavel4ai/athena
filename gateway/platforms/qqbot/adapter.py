...
class QQAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # to ~/.athena/.update_response. Set here so the cross-adapter gateway
    ...
    async def _default_interaction_dispatch(
        ...
        - ``update_prompt:<answer>`` →
          writes the answer to ``~/.athena/.update_response`` for the
    ...
    async def send_update_prompt(
        ...
        ``~/.athena/.update_response`` so the detached update process
        can read it.
        """
