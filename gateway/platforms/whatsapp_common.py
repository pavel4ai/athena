...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to ATHENA_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/athena), this function
    mirrors the bridge source to a writable ATHENA_HOME location and returns that
    ...
    # Try ATHENA_HOME location first
    athena_home = get_athena_home()
    athena_home_bridge = athena_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to ATHENA_HOME if needed
    if athena_home_bridge.exists():
        return athena_home_bridge

    # Mirror the bridge source to ATHENA_HOME
    try:
        athena_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            athena_home_bridge,
            dirs_exist_ok=False,
