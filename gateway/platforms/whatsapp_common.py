...
def resolve_whatsapp_bridge_dir() -> Path:
    ...
    /opt/athena) is mirrored to ATHENA_HOME so npm install works."""
    import shutil
    from athena_constants import get_athena_home
    install_bridge = Path(__file__).resolve().parents[2] / "scripts" / "whatsapp-bridge"
    athena_home_bridge = get_athena_home() / "scripts" / "whatsapp-bridge"
    try:
    ...
    try:
        ...
        shutil.copytree(install_bridge, athena_home_bridge, dirs_exist_ok=False)
        return athena_home_bridge
    except Exception:
        return install_bridge
