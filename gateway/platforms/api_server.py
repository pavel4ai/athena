...
def _athena_version() -> str:
    ...
    install), then the in-tree ``athena_cli.__version__`` (covers editable /
...
class ResponseStore:
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        # gracefully on NFS/SMB/FUSE-mounted ATHENA_HOME (same filesystem
        # issue addressed for state.db/kanban.db — see
        # athena_state._WAL_INCOMPAT_MARKERS).
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the athena-api-server default.
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"status": "ok", "platform": "athena-agent", "version": _athena_version()}
        )
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``athena.tool.progress`` with ``status: running``.
    ...
    async def _write_sse_chat_completion(
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: athena.tool.progress`` SSE event so
    ...
    async def _handle_run_approval(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "athena.run.approval_response",
            "run_id": run_id,
            "choice": choice,
            "resolved": resolved,
        })
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # the June 2026 athena-0day hardening (an 8-char key dispatching
            ...
            # that is the exact surface the athena-0day campaign abused to write
            # ~/.athena/config.yaml and plant persistence. Sandboxing (Docker /
