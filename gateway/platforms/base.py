...
# Default location: {ATHENA_HOME}/cache/images/ (legacy: image_cache/)
...
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _ATHENA_HOME / "image_cache",
    _ATHENA_HOME / "audio_cache",
    _ATHENA_HOME / "video_cache",
    _ATHENA_HOME / "document_cache",
    _ATHENA_HOME / "browser_screenshots",
    ...
    _ATHENA_HOME / "cache" / "images",
    _ATHENA_HOME / "cache" / "audio",
    _ATHENA_HOME / "cache" / "videos",
    _ATHENA_HOME / "cache" / "documents",
    _ATHENA_HOME / "cache" / "screenshots",
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # ATHENA_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.athena
    ...
    for athena_root in (_ATHENA_HOME, _ATHENA_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(athena_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    secrets (``~/.athena/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``ATHENA_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Athena-managed cache, under an operator-allowlisted root
    (``ATHENA_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the Athena root (~/.athena/.env,
    ...
    # ``MEDIA:~/.athena/google_token.json``) remain rejected.
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``ATHENA_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("ATHENA_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "ATHENA_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "ATHENA_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Athena sends
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.athena/media/generated/stale.png"}
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          ATHENA_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          ATHENA_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          ATHENA_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        ...
        try:
            min_ms = int(os.getenv("ATHENA_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("ATHENA_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _athena_run_generation is set on
