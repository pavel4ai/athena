...
def _account_file(athena_home: str, account_id: str) -> Path:
    return _account_dir(athena_home) / f"{account_id}.json"
...
def save_weixin_account(
    ...
    *,
    ...
    path = _account_file(athena_home, account_id)
...
def _sync_buf_path(athena_home: str, account_id: str) -> Path:
    return _account_dir(athena_home) / f"{account_id}.sync.json"


def _load_sync_buf(athena_home: str, account_id: str) -> str:
    path = _sync_buf_path(athena_home, account_id)
    if not path.exists():
        return ""
    try:
...
class WeixinAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        if self._account_id and not self._token:
            persisted = load_weixin_account(athena_home, self._account_id)
            if persisted:
