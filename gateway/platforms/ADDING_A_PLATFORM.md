...
Create a plugin directory in `~/.athena/plugins/` (or under `plugins/platforms/`
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `athena gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `athena_cli/config.py` so the setup
...
And add it to the `athena-gateway` composite:

```python
"athena-gateway": {
    "includes": [..., "athena-your-platform"]
}
```
...
## 12. Status Display (`athena_cli/status.py`)
...
## 13. Gateway Setup Wizard (`athena_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ athena_cli/ toolsets.py \
  --include="*.py" -l | sort -u
