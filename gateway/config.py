...
class GatewayConfig:
    ...
    # gateway behaves exactly as before — single ATHENA_HOME, no profile stamping.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.athena/config.yaml (primary user-facing config)
    3. ~/.athena/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through athena_cli.config.load_config, so without this a managed
