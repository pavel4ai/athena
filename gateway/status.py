...
The PID file lives at ``{ATHENA_HOME}/gateway.pid``.  ATHENA_HOME defaults to
``~/.athena`` but can be overridden via the environment variable.  This means
separate ATHENA_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting ATHENA_HOME."""
...
def looks_like_gateway_command_line(command: str | None) -> bool:
    ...
    test also matched ``athena_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\athena-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Athena's
    ...
    for token in tokens:
        ...
        if basename in ("athena-gateway", "athena-gateway.exe"):
            return True

    joined = " ".join(tokens)
    has_gateway_entry = (
        "athena_cli.main" in joined
        or "athena_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("athena", "athena.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return True  # bare `athena gateway` defaults to `run`
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `athena status` show per-profile
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different ATHENA_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``athena.service`` + ``athena-
# gateway.service``).
#
