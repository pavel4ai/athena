...
class GatewayInboundMixin:
    ...
    async def _hm_offer_pairing_code(self, source: SessionSource) -> None:
        ...
        if code:
            ...
            reply = (
                ...
                f"`athena {profile_arg}pairing approve "
    ...
    async def _hm_admit_event(
        ...
        # spawning context, which may carry ANOTHER message's ATHENA_SESSION_* ContextVars; until
    ...
    def _hm_estop_turn_allowed(self, event: "MessageEvent", source: SessionSource) -> bool:
        ...
        with suppress(Exception):
            ...
            if _estop_cmd:
                from athena_cli.commands import resolve_command as _resolve_estop_cmd
    ...
    def _hm_estop_gate(
        ...
        """Global emergency-stop (`athena pause`) notice when this turn must be blocked, else None.
    ...
    def _hm_write_update_response(response_text: str) -> Optional[str]:
        ...
        try:
            ...
            (_athena_home / ".update_prompt.json").unlink(missing_ok=True)
        except OSError as e:
            return str(e)
        return None
    ...
    async def _hm_busy_slash_or_photo(
        ...
        from athena_cli.commands import resolve_command as _resolve_cmd_inner
    ...
    def _hm_busy_telegram_grace_queue(
        ...
        _grace = float(os.getenv("ATHENA_TELEGRAM_FOLLOWUP_GRACE_SECONDS", "3.0"))
    ...
    async def _hm_resolve_command(
        ...
        from athena_cli.commands import is_gateway_known_command, resolve_command as _resolve_cmd
    ...
    async def _hm_cmd_moa(self, event, source, _quick_key):
        ...
        from athena_cli.moa_config import moa_usage, normalize_moa_config
            ...
            if _plat and (_skill_name or extra_keys):
                ...
                if _disabled_extra:
                    return (
                        ...
                        f"Enable them with: `athena skills config`"
                    )
    ...
    async def _dispatch_plugin_message_injection(
        ...
        await adapter.handle_message(MessageEvent(
            ...
            metadata={
                "athena_plugin_id": plugin_id, "athena_plugin_injection": True,
