...
Rules:
  ...
  - empty dirs    → always delete (under ATHENA_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ATHENA_HOME and /tmp/athena-*
Never touches: ~/.athena/logs/ or any system directory.
"""
...
try:
    ...
    def get_athena_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ATHENA_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".athena").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ATHENA_HOME/logs/``."""
    return get_athena_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ATHENA_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ATHENA_HOME or ``/tmp/athena-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    athena_home = get_athena_home()
    try:
        path.resolve().relative_to(athena_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/athena-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("athena-"):
        return True
    return False
...
# Paths under $ATHENA_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    """
    # Lazily build the set once per process so ATHENA_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        athena_home = get_athena_home()
        for parent in ("cron", "cronjobs"):
            base = athena_home / parent
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ATHENA_HOME, but never recurse into known
    # durable state trees.  Some installs place the Athena checkout, venv,
    # and desktop build under ATHENA_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    athena_home = get_athena_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in athena_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    athena_home = get_athena_home()
    try:
        rel = path.resolve().relative_to(athena_home)
        ...
        # Path isn't under ATHENA_HOME (e.g. /tmp/athena-*) — fall through.
        pass
