...
Scoped strictly to `$ATHENA_HOME` and `/tmp/athena-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ATHENA_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `ATHENA_HOME` or `/tmp/athena-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ATHENA_HOME/disk-cleanup/` is itself excluded
- `$ATHENA_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
