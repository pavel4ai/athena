...
One active meeting at a time, recorded in ``$ATHENA_HOME/workspace/meetings/.active.json``
...
def _root() -> Path:
    return Path(get_athena_home()) / "workspace" / "meetings"
...
def start(url: str, *, out_dir: Optional[Path] = None, headed: bool = False,
          auth_state: Optional[str] = None, guest_name: str = "Athena Agent", duration: Optional[str] = None,
    ...
    for name in ("transcript.txt", "status.json"):
        with contextlib.suppress(OSError):
            (out / name).unlink()
    env = {**os.environ, "ATHENA_MEET_URL": url, "ATHENA_MEET_OUT_DIR": str(out),
           "ATHENA_MEET_GUEST_NAME": guest_name}
    for value, var in (
        (headed and "1", "ATHENA_MEET_HEADED"),
        (auth_state, "ATHENA_MEET_AUTH_STATE"),
        (duration, "ATHENA_MEET_DURATION"),
        (mode, "ATHENA_MEET_MODE"),  # bot defaults to transcribe when unset (v1 behavior)
        (realtime_model, "ATHENA_MEET_REALTIME_MODEL"),
        (realtime_voice, "ATHENA_MEET_REALTIME_VOICE"),
        (realtime_instructions, "ATHENA_MEET_REALTIME_INSTRUCTIONS")):
        if value:
            env[var] = value
    ...
    if not realtime_api_key:
        ...
        realtime_api_key = get_secret("ATHENA_MEET_REALTIME_KEY") or get_secret("OPENAI_API_KEY")
    if realtime_api_key:
        env["ATHENA_MEET_REALTIME_KEY"] = realtime_api_key
