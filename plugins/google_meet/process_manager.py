...
session-scoped state file under ``$ATHENA_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $ATHENA_HOME):
#
#   workspace/meetings/
...
def start(
    ...
    *,
    ...
    If a bot is already running for this athena install, leave it first —
    ...
    env = os.environ.copy()
    env["ATHENA_MEET_URL"] = url
    env["ATHENA_MEET_OUT_DIR"] = str(out)
    env["ATHENA_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["ATHENA_MEET_HEADED"] = "1"
    if auth_state:
        env["ATHENA_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["ATHENA_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if ATHENA_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["ATHENA_MEET_MODE"] = mode
    if realtime_model:
        env["ATHENA_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["ATHENA_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["ATHENA_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
