...
athena meet install                 # pip deps + Chromium (transcribe only)
athena meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
athena meet auth                    # optional; skips guest-lobby wait
athena meet setup                   # preflight checks
```

`athena meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or ATHENA_MEET_REALTIME_KEY in ~/.athena/.env
```
...
pip install playwright websockets && python -m playwright install chromium
athena plugins enable google_meet
athena meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
athena meet node approve my-mac ws://<mac-ip>:18789 <token>
athena meet node ping my-mac                   # confirm reachable
```

Run `athena meet setup` to preflight local prereqs.
...
- Guest mode sits in the lobby until a host admits. Warn the user; `athena meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `ATHENA_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `athena_meet_src`). |
...
Local:
```
$ATHENA_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$ATHENA_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `athena meet node approve`.
