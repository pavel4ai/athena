...
```
┌─ gateway (Linux box, where athena runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `athena meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `athena meet` CLI |
...
| `cli.py` | `athena meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$ATHENA_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `athena meet node {run,list,approve,remove,status,ping}` |
...
athena meet install                                      # pip + Chromium
athena meet setup                                        # preflight
athena meet auth                                         # optional
athena meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
athena meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.athena/.env
athena meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
athena meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
athena meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.athena/.env
athena meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, athena will **not** switch your system audio input automatically — the
...
athena plugins enable google_meet
athena meet node run --display-name my-mac --host 0.0.0.0 --port 18789
