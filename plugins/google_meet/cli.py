...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    try:
        ...
        def _node_unavailable(args):
            print(f"athena meet node: module unavailable ({e})")
            return 1
...
def meet_command(args: argparse.Namespace) -> int:
    ...
    if sub == "node":
        ...
        if fn is None or fn is meet_command:
            print("usage: athena meet node {run,list,approve,remove,status,ping}")
            return 2
        return fn(args)
...
def _cmd_setup() -> int:
    ...
    if all_ok:
        print(
            ...
            "athena meet join https://meet.google.com/abc-defg-hij"
        )
    else:
...
def _cmd_stop() -> int:
    res = pm.stop(reason="athena meet stop")
    print(json.dumps(res, indent=2))
    return 0 if res.get("ok") else 1
