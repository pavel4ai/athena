...
Owner: athena-achievements plugin
...
### `GET /api/plugins/athena-achievements/achievements`
...
### `POST /api/plugins/athena-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/athena-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/athena-achievements/overview`
...
- Persist plugin data under:
  - `~/.athena/plugins/athena-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.athena/plugins/athena-achievements/scan_checkpoint.json`
...
- `~/.athena/plugins/athena-achievements/`
