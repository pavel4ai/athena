...
Mounted at /api/plugins/athena-achievements/ by Athena dashboard.
"""
...
try:
    ...
    def get_athena_home() -> Path:  # type: ignore[misc]
        val = (_os.environ.get("ATHENA_HOME") or "").strip()
...
def state_path() -> Path:
    return get_athena_home() / "plugins" / "athena-achievements" / "state.json"


def snapshot_path() -> Path:
    return get_athena_home() / "plugins" / "athena-achievements" / "scan_snapshot.json"


def checkpoint_path() -> Path:
    return get_athena_home() / "plugins" / "athena-achievements" / "scan_checkpoint.json"
...
def analyze_messages(session_id: str, title: str, messages: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "plugin_events": len(re.findall(r"plugin|dashboard-plugins|__ATHENA_PLUGIN|manifest\.json", full_text, re.I)),
