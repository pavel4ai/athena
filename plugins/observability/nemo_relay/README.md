...
```bash
athena plugins enable observability/nemo_relay
```

The `ATHENA_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `ATHENA_HOME` that the
agent run will use:

```bash
env ATHENA_HOME=/tmp/athena-nemo-relay-test \
  athena plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`ATHENA_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`ATHENA_HOME` is the Athena profile/config home used by both
`athena plugins enable ...` and the later `athena chat ...` run. If unset,
Athena uses the user's default home, usually `~/.athena`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export ATHENA_HOME=/tmp/athena-nemo-relay-test
athena plugins enable observability/nemo_relay
athena chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `athena` command you run is built from the
...
uv run athena plugins enable observability/nemo_relay
uv run athena chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/athena_agent-*.whl
...
The plugin can configure exporters directly from `ATHENA_NEMO_RELAY_*`
...
```bash
export ATHENA_NEMO_RELAY_ATOF_ENABLED=1
export ATHENA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export ATHENA_NEMO_RELAY_ATIF_ENABLED=1
export ATHENA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `ATHENA_NEMO_RELAY_ATOF_FILENAME`
- `ATHENA_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
...
- `ATHENA_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)
...
```bash
export ATHENA_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `ATHENA_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`ATHENA_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`ATHENA_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Athena does not
...
export ATHENA_HOME=/tmp/athena-nemo-relay-docs/athena-home
mkdir -p "$ATHENA_HOME"

cat > "$ATHENA_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
```bash
export ATHENA_NEMO_RELAY_ATOF_ENABLED=1
export ATHENA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/athena-nemo-relay-docs/subagent/atof
export ATHENA_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export ATHENA_NEMO_RELAY_ATOF_MODE=overwrite
export ATHENA_NEMO_RELAY_ATIF_ENABLED=1
export ATHENA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/athena-nemo-relay-docs/subagent/atif
export ATHENA_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export ATHENA_NEMO_RELAY_ATIF_AGENT_NAME='Athena Agent E2E'
export ATHENA_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export ATHENA_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"athena.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```bash
mkdir -p /tmp/athena-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/athena-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/athena-nemo-relay-docs/workdir/beta.txt
cd /tmp/athena-nemo-relay-docs/workdir

export ATHENA_NEMO_RELAY_ATOF_ENABLED=1
export ATHENA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/athena-nemo-relay-docs/parallel/atof
export ATHENA_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export ATHENA_NEMO_RELAY_ATOF_MODE=overwrite
export ATHENA_NEMO_RELAY_ATIF_ENABLED=1
export ATHENA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/athena-nemo-relay-docs/parallel/atif
export ATHENA_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export ATHENA_NEMO_RELAY_ATIF_AGENT_NAME='Athena Agent E2E'
export ATHENA_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
```bash
export ATHENA_NEMO_RELAY_PLUGINS_TOML=/tmp/athena-middleware-test/plugins.toml
```
...
```bash
export ATHENA_HOME=/tmp/athena-middleware-test/athena-home
mkdir -p "$ATHENA_HOME" /tmp/athena-middleware-test/nemo-relay
...
cat > /tmp/athena-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/athena-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/athena-middleware-test/atif"
...
export ATHENA_NEMO_RELAY_PLUGINS_TOML=/tmp/athena-middleware-test/nemo-relay/plugins.toml
