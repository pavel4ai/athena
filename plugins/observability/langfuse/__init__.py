...
Required env vars (set via ``athena tools`` or ~/.athena/.env):
  ATHENA_LANGFUSE_PUBLIC_KEY  - Langfuse project public key (pk-lf-...)
  ATHENA_LANGFUSE_SECRET_KEY  - Langfuse project secret key (sk-lf-...)
  ATHENA_LANGFUSE_BASE_URL    - Langfuse server URL (default: https://cloud.langfuse.com)

Optional env vars:
  ATHENA_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  ATHENA_LANGFUSE_RELEASE     - release/version tag
  ATHENA_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  ATHENA_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  ATHENA_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
def _debug_enabled() -> bool:
    return _env_bool("ATHENA_LANGFUSE_DEBUG")
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    public_key = _env("ATHENA_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
    secret_key = _env("ATHENA_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
    ...
    base_url = _env("ATHENA_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
    environment = _env("ATHENA_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
    release = _env("ATHENA_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
...
def _trace_key(
    ...
    *,
    ...
    Older Athena paths only expose ``task_id``/``session_id``. Newer paths
