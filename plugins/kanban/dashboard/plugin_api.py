...
``athena_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
own pages inject it via ``window.__ATHENA_SESSION_TOKEN__`` so logged-in
...
the in-browser PTY bridge in ``athena_cli/web_server.py``.
...
def _ws_upgrade_authorized(ws: "WebSocket") -> bool:
    ...
    WS auth gate (``athena_cli.web_server._ws_auth_ok``).
    ...
    """
    try:
        from athena_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``athena_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
    from athena_cli import kanban_diagnostics as kd
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from athena_cli.kanban_diagnostics import SEVERITY_ORDER
...
def get_board(
    ...
    through to the active board (``ATHENA_KANBAN_BOARD`` env → on-disk
...
# spawn failures, stuck-blocked). See athena_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def specify_task_endpoint(
    ...
    # ATHENA_KANBAN_BOARD env var — this endpoint runs in FastAPI's
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        ...
        from athena_cli import kanban_specify  # noqa: WPS433 (intentional)
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.athena/config.yaml.
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.athena/profiles/*`` on disk and every
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    try:
        ...
        if canon == "default":
            ...
            profile_dir = _Path(get_athena_home())
        else:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    """
    try:
        from athena_cli import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    # ATHENA_KANBAN_BOARD env var would let concurrent requests for
    # different boards race and cross-write (issue #38323).
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        from athena_cli import kanban_decompose  # noqa: WPS433 (intentional)
...
def set_orchestration_settings(payload: OrchestrationSettingsBody):
    """Update the kanban orchestration knobs in ~/.athena/config.yaml.
