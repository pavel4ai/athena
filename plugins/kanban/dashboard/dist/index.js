/**
 * Athena Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.athena/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__ATHENA_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__ATHENA_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "athena-kanban-dot-triage",
    todo: "athena-kanban-dot-todo",
    ready: "athena-kanban-dot-ready",
    running: "athena-kanban-dot-running",
    blocked: "athena-kanban-dot-blocked",
    review: "athena-kanban-dot-review",
    done: "athena-kanban-dot-done",
    archived: "athena-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-athena-task";
  ...
  const LS_BOARD_KEY = "athena.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="athena-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])athena-kanban-md-code\1/i.test(attrs)) {
      return ' class="athena-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "athena-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "athena-kanban-md",
  ...
  // standard `drop` event and our `athena-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("athena-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("athena-kanban-column--drop");
          if (target) target.classList.add("athena-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("athena-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("athena-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("athena-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__ATHENA_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "athena-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "athena-kanban-attention",
        "athena-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "athena-kanban-attention-bar" },
        h("span", { className: "athena-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "athena-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "athena-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "athena-kanban-attention-row",
                  "athena-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "athena-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "athena-kanban-attention-row-id" }, task.id),
                h("span", { className: "athena-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "athena-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "athena-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "athena-kanban-diag-action-btn",
      action.suggested ? "athena-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " athena-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".athena-kanban-drawer-comment-row input, .athena-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "athena-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("athena-kanban-diag", sevClass) },
      h("div", { className: "athena-kanban-diag-header" },
        h("span", { className: "athena-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "athena-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "athena-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "athena-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "athena-kanban-diag-data-row" },
                  h("span", { className: "athena-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "athena-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "athena-kanban-diag-data-row" },
                h("span", { className: "athena-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "athena-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "athena-kanban-diag-reassign-row" },
            h("span", { className: "athena-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "athena-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "athena-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "athena-kanban-diag-msg",
              msg.ok ? "athena-kanban-diag-msg--ok" : "athena-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "athena-kanban-section" },
      h("div", { className: "athena-kanban-section-head-row" },
        h("span", { className: "athena-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "athena-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "athena-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "athena-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "athena-kanban-docs-link",
      title: "Open Athena Kanban docs in a new tab",
      "aria-label": "Athena Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `athena kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "athena-kanban-msg-ok" : "athena-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "athena-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "athena-kanban-boardswitcher" },
      h("div", { className: "athena-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "athena-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "athena-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "athena-kanban-dialog-title" },
        ...
        h("div", { className: "athena-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Athena profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "athena-kanban-bulk" },
      h("span", { className: "athena-kanban-bulk-count" },
      ...
      h("div", { className: "athena-kanban-bulk-priority",
      ...
      h("div", { className: "athena-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Athena profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "athena-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("athena-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("athena-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "athena-kanban-trash",
        dragOver ? "athena-kanban-trash--drop" : "",
        props.draggingTaskId ? "athena-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "athena-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "athena-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".athena-kanban-card")) return true;
      if (target.closest && target.closest(".athena-kanban-column-add")) return true;
      if (target.closest && target.closest(".athena-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("athena-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("athena-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".athena-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "athena-kanban-columns",
        isScrollable ? "athena-kanban-columns--scrollable" : "",
        isPanning ? "athena-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("athena-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("athena-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        ...
        dragOver ? "athena-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "athena-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "athena-kanban-col-check",
        ...
        h("span", { className: cn("athena-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "athena-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "athena-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "athena-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "athena-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "athena-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "athena-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "athena-kanban-lane" },
                  h("div", { className: "athena-kanban-lane-head" },
                    h("span", { className: "athena-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "athena-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "athena-kanban-card--stale-red";
    if (age >= tier.amber) return "athena-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".athena-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "athena-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "athena-kanban-card",
        props.selected ? "athena-kanban-card--selected" : "",
        props.failed ? "athena-kanban-card--failed" : "",
        props.draggingSource ? "athena-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "athena-kanban-card-content" },
          h("div", { className: "athena-kanban-card-row" },
            h("label", {
              className: "athena-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "athena-kanban-card-check",
            ...
            h("span", { className: "athena-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or athena kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "athena-kanban-warning-badge",
                    "athena-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "athena-kanban-needs-assignee",
          ...
          h("div", { className: "athena-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "athena-kanban-card-row athena-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "athena-kanban-assignee",
                            title: `Assigned to Athena profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "athena-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "athena-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "athena-kanban-ago",
  ...
  // centered modal reusing the athena-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "athena-kanban-dialog athena-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Athena profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Athena profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "athena-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "athena-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "athena-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "athena-kanban-drawer-close",
        ...
        data ? h("div", { className: "athena-kanban-drawer-comment-foot" },
          h("div", {
            className: "athena-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "athena-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "athena-kanban-section" },
      h("div", { className: "athena-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "athena-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "athena-kanban-drawer-body" },
      h("div", { className: "athena-kanban-drawer-title" },
        h("span", { className: cn("athena-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "athena-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "athena-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "athena-kanban-section" },
            h("div", { className: "athena-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "athena-kanban-section" },
            h("div", { className: "athena-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "athena-kanban-done-no-result athena-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "athena-kanban-section" },
            h("div", { className: "athena-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "athena-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "athena-kanban-section" },
        h("div", { className: "athena-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "athena-kanban-comment" },
            h("div", { className: "athena-kanban-comment-head" },
              h("span", { className: "athena-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "athena-kanban-diag-action-btn",
      ...
      h("div", { className: "athena-kanban-section" },
        h("div", { className: "athena-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "athena-kanban-comment" },
                h("div", { className: "athena-kanban-comment-head" },
                  h("span", { className: "athena-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "athena-kanban-comment-ago" },
      ...
      h("div", { className: "athena-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "athena-kanban-event",
              isDiag ? "athena-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "athena-kanban-event-header" },
                  h("span", { className: "athena-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "athena-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "athena-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "athena-kanban-event-header-plain" },
                  h("span", { className: "athena-kanban-event-kind" }, e.kind),
                  h("span", { className: "athena-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "athena-kanban-event-phantom-row" },
                  h("span", { className: "athena-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "athena-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "athena-kanban-event-payload" },
    ...
    return h("div", { className: "athena-kanban-section" },
      h("div", { className: "athena-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "athena-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `athena-kanban-run--${r.outcome || r.status || "ended"}`
          : "athena-kanban-run--active";
        return h("div", { key: r.id, className: cn("athena-kanban-run", outcomeClass) },
          h("div", { className: "athena-kanban-run-head" },
            h("span", { className: "athena-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "athena-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "athena-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "athena-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "athena-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "athena-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "athena-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "athena-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "athena-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "athena-kanban-pre athena-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "athena-kanban-meta-row" },
      h("span", { className: "athena-kanban-meta-label" }, props.label),
      h("span", { className: "athena-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "athena-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "athena-kanban-meta-row" },
        h("span", { className: "athena-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "athena-kanban-meta-value athena-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "athena-kanban-meta-row" },
        h("span", { className: "athena-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "athena-kanban-meta-value athena-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "athena-kanban-meta-row" },
        h("span", { className: "athena-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "athena-kanban-meta-value athena-kanban-editable",
    ...
    return h("div", { className: "athena-kanban-meta-row" },
      h("span", { className: "athena-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "athena-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "athena-kanban-section" },
      h("div", { className: "athena-kanban-section-head-row" },
        h("span", { className: "athena-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "athena-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "athena-kanban-section" },
      h("div", { className: "athena-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "athena-kanban-deps-row" },
        h("span", { className: "athena-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "athena-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "athena-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "athena-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "athena-kanban-dep-chip-x",
      ...
      h("div", { className: "athena-kanban-deps-row" },
        h("span", { className: "athena-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "athena-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "athena-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "athena-kanban-msg-ok"
          : "athena-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "athena-kanban-section" },
      ...
      h("div", { className: "athena-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "athena-kanban-home-sub athena-kanban-home-sub--on"
              : "athena-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__ATHENA_PLUGINS__ && typeof window.__ATHENA_PLUGINS__.register === "function") {
    window.__ATHENA_PLUGINS__.register("kanban", KanbanPage);
  }
})();
