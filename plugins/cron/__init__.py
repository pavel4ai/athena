...
1. Bundled providers: ``plugins/cron/<name>/`` (shipped with athena-agent)
2. User-installed providers: ``$ATHENA_HOME/plugins/<name>/``
...
_USER_NAMESPACE = "_athena_user_cron"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_athena_user_cron.<name>``, a dotted
    ...
    ``ModuleNotFoundError: No module named '_athena_user_cron'`` — the same
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$ATHENA_HOME/plugins/`` or None if unavailable."""
    try:
...
def load_cron_scheduler(name: str) -> Optional["CronScheduler"]:  # noqa: F821
    ...
    (``$ATHENA_HOME/plugins/<name>/``) directories. Bundled takes precedence
    on name collisions.
