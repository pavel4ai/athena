...
:class:`~athena_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``athena_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~athena_cli.dashboard_auth.Session`.
...
same precedence convention as the ``nous`` plugin)::
    ...
    ATHENA_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``ATHENA_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "ATHENA_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "ATHENA_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("ATHENA_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(ATHENA_DASHBOARD_OIDC_ISSUER + ATHENA_DASHBOARD_OIDC_CLIENT_ID) "
