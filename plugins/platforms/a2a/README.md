# A2A — Agent-to-Agent protocol for Athena
...
peer (another Athena, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
athena gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Athena serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.athena/a2a_audit.jsonl`.
- Conversations persist to `~/.athena/a2a_conversations/` — they survive context
