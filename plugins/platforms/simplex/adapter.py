...
Optional environment variables:
    ...
    ATHENA_SIMPLEX_TEXT_BATCH_DELAY
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("ATHENA_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    ...
    auto-accept / home channel. Writes to ``~/.athena/.env`` via
    ``athena_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "athena_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.athena/.env"
        )
        return
