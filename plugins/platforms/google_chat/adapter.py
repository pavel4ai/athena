...
# ``athena_plugins.platforms__google_chat.adapter`` once the plugin
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a athena MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${ATHENA_HOME}/google_chat_thread_counts.json so
        ...
        self._thread_count_store = _ThreadCountStore(
            _athena_home / "google_chat_thread_counts.json"
        )
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("ATHENA_HOME", str(_Path.home() / ".athena"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${ATHENA_HOME}/google_chat_user_tokens/<sanitized>.json
...
def interactive_setup() -> None:
    ...
    The setup wizard at ``athena_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.athena/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. athena-chat-events) and a Pull subscription.")
