...
- Per-user tokens (keyed by sender email):
    ``${ATHENA_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${ATHENA_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${ATHENA_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${ATHENA_HOME}/google_chat_user_oauth_pending.json``
- OAuth client secret (profile-scoped — each profile registers its own):
    ``${ATHENA_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's ATHENA_HOME helper so the token follows the user's
# profile (e.g. tests can override via ATHENA_HOME=/tmp/...).
try:
    from athena_constants import display_athena_home, get_athena_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where athena_constants isn't importable
    # (mirrors the same fallback used by the google-workspace skill's
    # _athena_home.py shim).
...
def _athena_home() -> Path:
    """Resolve ATHENA_HOME at call time (NOT module import).

    Tests and ``ATHENA_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.athena/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _athena_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _athena_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _athena_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _athena_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _athena_home() / "google_chat_user_client_secret.json"
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_athena_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_athena_home()}/google_chat_user_token.json"
    )
