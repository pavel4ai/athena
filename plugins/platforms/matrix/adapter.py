...
# Uses get_athena_home() so each profile gets its own Matrix store.
from athena_constants import get_athena_dir as _get_athena_dir

_STORE_DIR = _get_athena_dir("platforms/matrix/store", "matrix/store")
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("ATHENA_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("ATHENA_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
...
# + _PLATFORMS["matrix"] static dict in athena_cli/{setup,gateway}.py, and the
...
def interactive_setup() -> None:
    """Configure Matrix credentials. Replaces athena_cli/setup.py::_setup_matrix
...
def _is_connected(config) -> bool:
    ...
    configured. Read via athena_cli.gateway.get_env_value so setup-status
