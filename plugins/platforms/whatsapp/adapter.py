...
def check_whatsapp_requirements() -> bool:
    ...
    """
    # Prefer Athena-managed Node/npm so Windows installs are not broken by a
...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._session_path: Path = Path(config.extra.get(
            "session_path",
            get_athena_dir("platforms/whatsapp/session", "whatsapp/session")
        ))
    ...
    async def connect(self) -> bool:
        ...
        # clear "run athena whatsapp" message instead of the watcher
        ...
        try:
            ...
            _dep_stamp = bridge_dir / "node_modules" / ".athena-pkg-hash"
            ...
            if not _deps_fresh:
                ...
                # Athena-managed portable Node before falling back to PATH.
            ...
            # hardcodes ~/.athena/{image,audio,document}_cache, which diverges
            # under ATHENA_HOME overrides, profiles, and the new cache/ layout.
            from gateway.platforms.base import (
                get_audio_cache_dir as _get_audio_dir,
                get_document_cache_dir as _get_doc_dir,
                get_image_cache_dir as _get_img_dir,
            )
            bridge_env["ATHENA_IMAGE_CACHE_DIR"] = str(_get_img_dir())
            bridge_env["ATHENA_AUDIO_CACHE_DIR"] = str(_get_audio_dir())
            bridge_env["ATHENA_DOCUMENT_CACHE_DIR"] = str(_get_doc_dir())
...
# dict in athena_cli/gateway.py, and the _send_whatsapp dispatch in
...
def interactive_setup() -> None:
    ...
    Replaces the central _setup_whatsapp in athena_cli/gateway.py and the
...
def _is_connected(config) -> bool:
    ...
    would make WhatsApp always show as "configured" in ``athena setup`` even
    ...
    if config is not None and getattr(config, "enabled", False) and extra:
        ...
        return True
    # Read via athena_cli.gateway.get_env_value (not os.getenv) so setup-status
