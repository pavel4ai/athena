...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``ATHENA_HOME``. PR #8398.
        """
        from aiohttp import web
...
def interactive_setup() -> None:
    ...
    one optional public URL. Writes to ``~/.athena/.env`` via ``athena_cli.config``.
    """
    print()
    ...
    try:
        from athena_cli.config import get_env_var, set_env_var
    except ImportError:
