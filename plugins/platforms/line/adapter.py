...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        path is rechecked against allowed roots (tempdir, ``/tmp``→``/private/tmp`` on macOS, ATHENA_HOME).
        ...
        try:
            ...
            athena_home = Path.home().joinpath(".athena").resolve()
        resolved = path.resolve()
        if not any(resolved.is_relative_to(r) for r in (Path(tempfile.gettempdir()).resolve(), Path("/tmp").resolve(), athena_home)):
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    """Seed PlatformConfig.extra from env-only setups so ``athena status`` sees them."""
    if not _env_credentials_present():
        return None
    seeded: Dict[str, Any] = {}
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``athena setup line`` (writes ``~/.athena/.env``)."""
    print("\nLINE Messaging API setup\n------------------------\n"
          "Create a Messaging API channel at https://developers.line.biz/console/\nthen copy the values below.\n")
    try:
        from athena_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("athena_cli.config not available; set LINE_* vars manually in ~/.athena/.env")
        return
