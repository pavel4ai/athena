...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # /model work directly without /athena prefix). A single regex
            ...
            from athena_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/athena$")
            ...
            for _action_id in (
                "athena_approve_once",
                "athena_approve_session",
                "athena_approve_always",
                "athena_deny",
            ):
            ...
            for _action_id in (
                "athena_confirm_once",
                "athena_confirm_always",
                "athena_confirm_cancel",
            ):
    ...
    async def _handle_slack_message(self, event: dict) -> None:
        ...
        if original_text.startswith("!"):
            try:
                ...
                # forms like ``!stop@athena`` still resolve.
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "athena_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "athena_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "athena_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "athena_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "athena_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "athena_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "athena_confirm_once": "once",
            "athena_confirm_always": "always",
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "athena_approve_once": "once",
            "athena_approve_session": "session",
            "athena_approve_always": "always",
            "athena_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/athena <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/athena
        ...
        if slash_name in {"athena", ""}:
            # Legacy /athena <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from athena_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/athena   ``).
        ...
        # "Running /cmd…" ack shown by handle_athena_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/athena <question>" must produce public replies so
...
# ``athena_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``athena_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under ATHENA_HOME, and print
        ...
        try:
            ...
            target = Path(get_athena_home()) / "slack-manifest.json"
    ...
    print()
    print_info("   Full guide: https://github.com/pavel4ai/athena/docs/user-guide/messaging/slack/")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``athena_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'athena-agent[slack]'",
        # Interactive setup wizard — replaces athena_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in athena_cli/gateway.py.
