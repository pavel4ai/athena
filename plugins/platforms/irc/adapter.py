...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "athena-bot")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: athena_, athena_1, athena_2...
...
def interactive_setup() -> None:
    ...
    Lazy-imports ``athena_cli.setup`` helpers so the plugin stays importable
    ...
    channel = prompt(
        "Channel to join (e.g. #athena — comma-separate for multiple)",
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "athena-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "athena-bot"
