// Athena Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Athena): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Athena -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Athena-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `athena photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Athena can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "athena-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Athena'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Athena allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Athena, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-athena-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Athena supplied them so a known-good
