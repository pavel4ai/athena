...
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Athena can receive inbound messages
...
The Athena plugin's `athena photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Athena is Python, so replies go through this sidecar
