...
Credential storage mirrors every other Athena channel:
    ...
    * runtime SDK creds  -> ``~/.athena/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.athena/auth.json`` under
...
def _auth_json_path() -> Path:
    """Resolve ``~/.athena/auth.json`` honouring the active Athena profile."""
    try:
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.athena/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.athena/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.athena/.env`` (canonical runtime store).
