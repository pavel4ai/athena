...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def _on_card_action(
        ...
        choice = choice_map.get(athena_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="athena_approve",
                    data={**btn_data_base, "athena_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="athena_approve",
                    data={**btn_data_base, "athena_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="athena_approve",
                    data={**btn_data_base, "athena_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="athena_approve",
                    data={**btn_data_base, "athena_action": "deny"},
                    style="destructive",
                ),
            ])
        )
