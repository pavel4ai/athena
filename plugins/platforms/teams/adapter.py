...
``<athena-venv>/bin/pip install microsoft-teams-apps aiohttp``) and credentials via env
...
async def _standalone_send(
    ...
    (``athena cron``). ``TEAMS_SERVICE_URL`` is allowlisted and ``chat_id`` charset-checked
...
def _suppress_third_party_dotenv() -> Iterator[None]:
    ...
    cwd-discovered ``.env`` at import, mutating process-global ``os.environ``. Athena owns dotenv loading.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def _on_card_action(
        ...
        athena_action = data.get("athena_action", "")
        session_key = data.get("session_key", "")
        if not athena_action or not session_key:
        ...
        choice = _APPROVAL_CHOICES.get(athena_action)
        if not choice:
            return self._invoke_message("Unknown action.")
    ...
    async def send_exec_approval(
        ...
        def _action(title: str, athena_action: str, **kw) -> "ExecuteAction":
            return ExecuteAction(
                title=title, verb="athena_approve", data={**btn_data_base, "athena_action": athena_action}, **kw)
...
_SETUP_INTRO = (  # "" → blank line
    ...
    '  teams app create --name "Athena" --endpoint "https://<tunnel>/api/messages"', "",
...
def interactive_setup() -> None:
    ...
    from athena_cli.cli_output import prompt, prompt_yes_no, print_info, print_success, print_warning
    ...
    print()
    print_success("Teams configuration saved to ~/.athena/.env")
...
def _install_hint() -> str:
    ...
    ``venv_pip=True`` targets the real Athena venv, sidestepping PEP 668 on Ubuntu 24.04."""
    try:
