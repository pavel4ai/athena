...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "athena-agent" "version" "{_athena_version}" '
            '"vendor" "FutureboundCorp" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `athena config show` already read. Without the
    ...
    async def connect(self) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `athena gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        msg_id = f"<athena-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
...
# athena_cli/gateway.py, and the _send_email dispatch in
