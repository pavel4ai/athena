...
Resolution order for config file:
  1. $ATHENA_HOME/honcho.json  (instance-local, enables isolated Athena instances)
...
def resolve_active_host() -> str:
    ...
    Resolution order:
      1. ATHENA_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``athena.<profile>``
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $ATHENA_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.athena/honcho.json          (default profile — shared host blocks live here)
...
class HonchoClientConfig:
    ...
    # True when Honcho was explicitly configured for this host (hosts.athena
    ...
    def from_global_config(
        ...
        Resolution: $ATHENA_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        ...
        # A hosts.athena block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — Athena session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. Athena session title (from /title command; non-per-session)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    def _build() -> "Honcho":
        ...
        # the "go run athena honcho setup" hint they used to.
        try:
        ...
        if not resolved_base_url or resolved_timeout is None:
            try:
                ...
                athena_cfg = load_config()
                honcho_cfg = athena_cfg.get("honcho", {})
