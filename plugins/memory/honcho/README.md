...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/athena>
...
```bash
athena memory setup honcho   # configure Honcho directly (works on a fresh install)
athena memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
athena config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.athena/.env
```

> `athena honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `athena memory setup honcho`.
...
Set `"recallSync": true` in `$ATHENA_HOME/honcho.json` (at the root or under
`hosts.athena`), or enable **Current-query recall** in the memory settings or
`athena honcho setup`. An explicit host-level `false` overrides root-level `true`.
...
cycle (not per pass). Select a fast, inexpensive model under `athena model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ATHENA_HOME/honcho.json` | Profile-local (isolated Athena instances) |
| 2 | `~/.athena/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Athena profile: `athena` (default) or `athena_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `athena honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
- **`per-session`** — Athena session ID (timestamp + hex). Every `athena` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
...
Both profiles see the same user (`yourname`) in the same shared environment (`athena`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Athena profile: `athena` (default) or `athena_<profile>` (e.g. `athena -p coder` -> host key `athena_coder`). Older `athena.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `ATHENA_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `athena-agent`) |
...
|---------|-------------|
| `athena memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `athena honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `athena memory setup`) |
| `athena honcho status` | Show resolved config for active profile |
| `athena honcho enable` / `disable` | Toggle Honcho for active profile |
| `athena honcho mode <mode>` | Change recall or observation mode |
| `athena honcho peer --user <name>` | Update user peer name |
| `athena honcho peer --ai <name>` | Update AI peer name |
| `athena honcho tokens --context <N>` | Set context token budget |
| `athena honcho tokens --dialectic <N>` | Set dialectic max chars |
| `athena honcho map <name>` | Map current directory to a session name |
| `athena honcho sync` | Create host blocks for all Athena profiles |
