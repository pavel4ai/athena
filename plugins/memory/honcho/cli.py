...
Handles: athena honcho setup | status | sessions | map | peer
"""
...
def cmd_sync(args) -> None:
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'athena honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'athena honcho setup' first.\n")
        return
...
def _local_config_path() -> Path:
    ...
    Always returns $ATHENA_HOME/honcho.json so each profile/instance gets
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        athena_host.get("pinUserPeer"),
        athena_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in athena_host:
        aliases_src = athena_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in athena_host:
        prefix_src = athena_host.get("runtimePeerPrefix")
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom athena_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    athena_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
...
def _echo_identity_mapping(athena_host: dict) -> None:
    ...
    aliases = athena_host.get("userPeerAliases")
    prefix = athena_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(athena_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _apply_runtime_prefix(
        athena_host, current_prefix, prefix_from_root,
...
def cmd_setup(args) -> None:
    ...
    athena_host = hosts.setdefault(_host_key(), {})
    ...
    if not is_local:
        ...
        if use_oauth:
            ...
            try:
                ...
                print("  Re-run 'athena honcho setup' to retry, or choose an API key instead.\n")
                return
            ...
            if not cfg.get("apiKey"):
                ...
                print("  Run 'athena honcho setup' again once you have a key.\n")
                return
    ...
    if new_peer:
        athena_host["peerName"] = new_peer

    current_ai = athena_host.get("aiPeer") or cfg.get("aiPeer", "athena")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        athena_host["aiPeer"] = new_ai

    current_workspace = athena_host.get("workspace") or cfg.get("workspace", "athena")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        athena_host["workspace"] = new_workspace

    # --- 3b. Gateway identity mapping ---
    # These keys only affect the Athena GATEWAY (Telegram/Discord/Slack/...),
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the Athena gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
    ...
    if run_mapping:
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(athena_host)
            athena_host["pinUserPeer"] = False
            if prior_aliases:
                athena_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                athena_host, current_prefix, prefix_from_root,
            ...
            if merged:
                athena_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                athena_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                athena_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = athena_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        athena_host["observationMode"] = new_obs
    else:
        athena_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(athena_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        athena_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        athena_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = athena_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        athena_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = athena_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    current_dialectic = str(athena_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    current_reasoning = (
        athena_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        athena_host["dialecticReasoningLevel"] = new_reasoning
    else:
        athena_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = athena_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        athena_host["sessionStrategy"] = new_strat
    ...
    print("\n  Other commands:")
    print("    athena honcho status     -- show full config")
    print("    athena honcho mode       -- change recall/observation mode")
    print("    athena honcho tokens     -- tune context and dialectic budgets")
    print("    athena honcho peer       -- update peer names")
    print("    athena honcho map <name> -- map this directory to a session name\n")
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'athena honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'athena honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: athena honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        athena = hosts.get(_host_key(), {})
        user = athena.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = athena.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = athena.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = athena.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Athena' identity in Honcho. Seed with 'athena honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: athena honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: athena honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = athena.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = athena.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = athena.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'athena honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'athena honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    athena honcho identity --show        — show both peer representations")
        print("    athena honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    ...
    if has_key:
        ...
        print("  2. Run:  athena honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'athena honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'athena honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  athena honcho identity <path/to/file>")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            for f in agent_files:
                print(f"    athena honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  athena honcho identity <path/to/SOUL.md>")
    ...
    print("              Map a custom name:  athena honcho map <session-name>")
    ...
    if not has_key:
        ...
        print("  2. athena honcho migrate            — re-run this walkthrough")
    else:
        print("  1. athena honcho status             — verify Honcho connection")
        print("  2. athena                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. athena honcho identity --show    — verify AI peer representation")
        print("  4. athena honcho tokens             — tune context and dialectic budgets")
        print("  5. athena honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'athena memory setup'...\n")
...
def register_cli(subparser) -> None:
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to athena memory setup)",
    )
