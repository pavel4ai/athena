...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: athena)
...
Or via $ATHENA_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $ATHENA_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_athena_home() / "hindsight" / "config.json"
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    collapsed — e.g. ``athena-{user}`` with no user becomes ``athena``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, athena_home):
        """Write config to $ATHENA_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(athena_home) / "hindsight"
    ...
    def post_setup(self, athena_home: str, config: dict) -> None:
        ...
        self.save_config(provider_config, athena_home)
        ...
        if mode == "local_embedded":
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(athena_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                ...
                # Athena get sluggish. (issue #13125)
                try:
