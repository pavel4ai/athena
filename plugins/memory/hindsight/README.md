...
Daemon startup logs: `~/.athena/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p athena ui start
```
...
Config file: `~/.athena/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `athena` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `athena-{profile}` isolates memory per active Athena profile. Empty placeholders collapse cleanly (e.g. `athena-{user}` with no user becomes `athena`). |
...
| `retain_source` | — | Opt-in `metadata.source` attached to retained memories (identifies the storing client, e.g. `athena`). Empty by default — no attribution tag ships unless you set it. |
...
The LLM API key is stored in `~/.athena/.env` as `HINDSIGHT_LLM_API_KEY`.
