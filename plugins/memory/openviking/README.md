...
- OpenViking server running and reachable from Athena

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Athena can identify older servers that expose the legacy status-only health
...
connection values into Athena, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.athena/.env`; for a named profile use
...
OpenViking's server config is separate from Athena:
...
Athena-side provider config is read from environment variables in the active
profile's `.env`:
...
When `OPENVIKING_API_KEY` is set, Athena lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Athena sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
Athena also sends `User-Agent: openviking-memory-athena/<version>` on
OpenViking requests. This standard harness identifier contains the Athena
...
does not ask for a peer ID. Without a configured peer, Athena sends neither
...
order remains environment, linked OpenViking config, then Athena YAML. To use
...
on the old implicit `athena` peer now use user memory for new writes. Without
...
Set `agent: athena` to restore peer-scoped writes. Memories written at user
...
`viking_remember` creates a one-shot `athena-remember-<random>` OpenViking
...
not advertised or used. The one-shot session is separate from the live Athena
...
Use the same OpenViking profile and credentials as Athena for manual recovery.
...
Athena built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Athena action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under user memory, or the configured peer memory directory |

Built-in `replace` and `remove` operations are not mirrored because Athena
...
file URIs, such as
`viking://user/default/peers/athena/memories/preferences/mem_abc123.md` (any
