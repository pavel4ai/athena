...
- Hosted: API key from [app.supermemory.ai/integrations?connect=athena](http://app.supermemory.ai/integrations?connect=athena)
...
Before running `athena memory setup`, add the local endpoint to
...
Then run `athena memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$ATHENA_HOME/supermemory.json`
...
| `container_tag` | `athena` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `athena-{identity}` → `athena-coder`). |
...
`https://api.supermemory.ai`. Athena resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: athena`, and document writes stamp
`metadata.sm_source: athena`. This is a **functional routing key, not telemetry**:
...
```json
{
  "container_tag": "athena-{identity}"
}
```

For a profile named `coder`, this resolves to `athena-coder`. The default profile resolves to `athena-default`. Without `{identity}`, all profiles share the same container.
