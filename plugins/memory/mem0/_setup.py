...
def _save_mem0_json(athena_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(athena_home) / "mem0.json"
    existing = {}
...
def _setup_platform(athena_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "athena-user"},
...
def _setup_oss(athena_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "athena-user")
    ...
    if env_writes:
        _write_env(Path(athena_home) / ".env", env_writes)
    _save_mem0_json(athena_home, {"mode": "oss", "user_id": user_id, "agent_id": "athena", "oss": oss_config})
...
_PGVECTOR_CONTAINER = "athena-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "athena"
...
def _setup_oss_interactive(athena_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], athena_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], athena_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'athena-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "athena-user")

    agent_id = input("  Agent ID [athena]: ").strip()
    agent_id = agent_id or "athena"
    ...
    if env_writes:
        _write_env(Path(athena_home) / ".env", env_writes)
    _save_mem0_json(athena_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(athena_home: str, config: dict) -> None:
    """Entry point called by athena memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(athena_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(athena_home, config, flags)
        return
