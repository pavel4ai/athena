...
Configuration
-------------
Secret (lives in $ATHENA_HOME/.env or the environment):
  MEM0_API_KEY       — Mem0 Platform API key (required for platform mode)

Behavioral settings (live in $ATHENA_HOME/mem0.json, set via `athena memory
setup`):
  ...
  agent_id           — Agent identifier (default: athena)
...
def _load_config() -> dict:
    """Load config from env vars, with $ATHENA_HOME/mem0.json overrides.
    ...
    config_path = get_athena_home() / "mem0.json"
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, athena_home):
        """Write config to $ATHENA_HOME/mem0.json."""
        import json
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $ATHENA_HOME/mem0.json) —
