...
def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    ...
    User-installed providers import as ``_athena_user_memory.<name>``, a
    ...
    ``ModuleNotFoundError: No module named '_athena_user_memory'`` — the
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$ATHENA_HOME/plugins/<name>/``) directories.  Bundled takes
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    try:
        ...
        if module_name in sys.modules:
            ...
            if not _is_bundled:
                # cli.py imports as _athena_user_memory.<name>.cli, usually
