...
``$ATHENA_HOME/plugins/<name>/``, project ``./.athena/plugins/<name>/`` (opt-in via
ATHENA_ENABLE_PROJECT_PLUGINS), then ``athena_agent.memory_providers`` entry points.
...
ENTRY_POINTS_GROUP = "athena_agent.memory_providers"
...
_USER_NAMESPACE = "_athena_user_memory"
...
def _get_project_plugins_dir() -> Optional[Path]:
    """``./.athena/plugins/`` or None. Gated on ATHENA_ENABLE_PROJECT_PLUGINS like the
    PluginManager scan: a repo you merely ``cd`` into must not offer a memory backend."""
    try:
        from athena_cli.plugins import _env_enabled

        if not _env_enabled("ATHENA_ENABLE_PROJECT_PLUGINS"):
            return None
        d = Path.cwd() / ".athena" / "plugins"
...
def find_provider_dir(name: str) -> Optional[Path]:
    ...
    provider silently loses its dashboard panel and ``athena <provider>`` commands."""
...
def _entry_point_package_dir(entry_point) -> Optional[Path]:
    ...
    if entry_point is None:
        return None
    try:
        from athena_cli.plugins import resolve_module_origin
...
class _ProviderCollector:
    ...
    def _plugin_context(self):
        ...
        if self._context is None:
            from athena_cli.plugins import PluginContext, PluginManifest, get_plugin_manager
    ...
    try:
        ...
        if cli_mod is None:
            if not _is_bundled(plugin_dir):
                # cli.py imports as _athena_user_memory.<name>.cli, usually before the
