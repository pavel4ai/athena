...
Bundled Athena Desktop plugin, **off by default**. Enable **Radio** in
...
`plugin.js` imports only `@athena/plugin-sdk`, `react`, and
...
`$ATHENA_HOME/desktop-plugins/radio/plugin.js` for development. Do not install
that duplicate alongside the bundled version. Layout styles use Athena theme
...
Stream availability, regional restrictions, and metadata vary. Athena is not
