...
import { host, LruCache } from '@athena/plugin-sdk'
...
// to (local, remote URL, SSH, Athena Cloud, docker) must be able to find
...
// athena_cli/config_defaults.py and tools/bot_relay.py and fails if the
  ...
  try {
    ...
    return profiles
      .map(profile => ({
        ...
        title: String(profile?.ui_meta?.['athena-bots']?.title || profile?.display_name || ''),
