...
import { atom, host } from '@athena/plugin-sdk'
...
const GROUP_CHAT_SYNC_META_KEY = 'athena-bots-groups'
...
/** The v3 envelope stored under the default profile's `athena-bots-groups`
 ...
 *  clients (Athena Go, headless backends) see rooms regardless of which
 ...
 *  renamed bot never shows up as its raw profile id or a stale "Athena"
 *  (community report, Aug 21 2026: renamed default still read "Athena is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as Athena (matching displayName and the
 *  @athena handle) so the main agent never loses its name in rooms. */
export function groupSpeakerLabel(name?: null | string) {
  ...
  // Core profile display_name (`athena profile rename …` / dashboard) from
  ...
  return trimmed.toLowerCase() === 'default' ? 'Athena' : trimmed
}
