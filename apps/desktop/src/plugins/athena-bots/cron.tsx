/**
 * Routines: the Athena cron jobs scoped to the bot you're chatting with — the
...
// athena -p <bot> chat delegation wrapper so the run reaches that bot's
...
export function routinePrompt(
  ...
  return (
    ...
    `athena -p ${shellQuote(bot)} chat -c ${shellQuote(`Routine: ${title}`)} -q ${shellQuote(`[Scheduled routine] ${instruction}`)}\n\n` +
...
// Athena-native schedule string; Advanced exposes it raw.
...
/** Compose the Athena schedule string from picker state. */
