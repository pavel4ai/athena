...
vi.mock('@athena/plugin-sdk', async () => {
...
vi.mock('./shared', () => ({ getPluginCtx: () => null, ID: 'athena-bots' }))
...
describe('the @handle a bot answers to', () => {
  it('presents the primary profile as @athena — "default" never surfaces in the UI', () => {
    expect(botHandle('default')).toBe('athena')
...
describe('renamed bots stay taggable', () => {
  // Discord report, Aug 2026: renaming a bot — Bot Mode title or `athena
  ...
  it('drops reserved tokens so a rename cannot hijack a built-in tag', () => {
    expect(mentionNameForms('Athena')).toEqual([])
  ...
  it('inserts the renamed slug, falling back to the profile handle', () => {
    ...
    expect(botMentionTag(row({ name: 'default' }))).toBe('athena')
    // display_name (`athena profile rename`) drives the tag too.
...
describe('resolving @mentions against the roster', () => {
  ...
  it('never treats @athena in your own chat as a handoff to yourself', () => {
    expect(resolveRosterMentions('@athena do it', roster, { connectionId: 'local', name: 'default' })).toEqual([])
    // From ANOTHER bot's chat the same tag is a real handoff.
    expect(
      resolveRosterMentions('@athena do it', roster, { connectionId: 'mac-mini', name: 'dixie' }).map(bot => bot.name)
    ).toEqual(['default'])
  })
...
describe('roster search narrows without re-ranking', () => {
  ...
  it('matches profile handles and preserves roster order', () => {
    ...
    expect(filterBots(roster, meta, '@athena').map(bot => bot.name)).toEqual(['default'])
...
describe('needs-attention badge (#93091 item 3)', () => {
  ...
  it('classifies the raw gateway error strings current backends emit', () => {
    ...
    expect(reasonFor('No LLM provider configured. Run athena model to pick one.')).toBe('missing_config')
