...
describe('routing', () => {
  ...
  it('resolves @athena to the default member', async () => {
    const { rounds } = await loadRoom()
    ...
    const parsed = rounds.parseGroupChatMentions('@athena take a look', members)
...
describe('turn prompt', () => {
  it('addresses the default profile as @athena', async () => {
    const { rounds } = await loadRoom()
    ...
    expect(own).toMatch(/You are @athena,/)
    ...
    expect(peer).toMatch(/group chat with @athena/)
  })
