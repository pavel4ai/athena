...
import * as sdk from '@athena/plugin-sdk'
import { host } from '@athena/plugin-sdk'
...
// grandfather, or recover. Stored-id pins (ui_meta['athena-bots'].chat) were
 ...
 *  click path's tile-staleness probe (athena-agent#90102), which must
...
async function openStoredBotChat(
  ...
  if (!storedId || typeof host.openSession !== 'function') {
    throw new Error('This Athena Desktop version cannot open stored sessions')
  }
  ...
  // even though the session is fine (athena-agent#89617) — clicking Retry
  ...
  // until an app restart (athena-agent#93604). A resume is cheap and
  ...
  if (route && typeof host.requestProfile !== 'function') {
    throw new Error(
      getPluginCtx()?.i18n?.t('bot.remoteConnectionsUnsupported') ??
        'Update Athena Desktop to chat with bots on other connections.'
    )
  }
...
export async function ensureBotMetadata(bot: RosterRow): Promise<BotMeta> {
  ...
  const server = row?.ui_meta?.['athena-bots']
