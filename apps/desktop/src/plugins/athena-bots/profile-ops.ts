...
import { forgetSessionUnread, host, queryClient } from '@athena/plugin-sdk'
...
export function mergeServerMeta(roster: RosterRow[], fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['athena-bots']
...
/** Permanently delete a bot's Athena profile, then remove plugin-local state
 ...
 * directory (athena-agent#52279). That is the "can't delete a bot" error. */
