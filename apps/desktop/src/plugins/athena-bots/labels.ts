...
export function displayName(bot: Partial<RosterRow>, meta?: BotMeta | null): string {
  ...
  // "Athena". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `athena profile rename
  ...
  // never generic "Athena" or a hostname-derived label.
  if (alias) {
  ...
  // that reads like nobody bothered. Present it as Athena (the agent it is)
