...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'athena:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.athenaDesktop?.saveImageFromUrl) {
        if (await window.athenaDesktop.saveImageFromUrl(src)) {
