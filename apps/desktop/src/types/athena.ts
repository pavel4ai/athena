...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Athena doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in athena_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `athena model` uses — instead of
...
export type AthenaConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `athena model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// non-desktop) Athena simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (athena.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors athena_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Athena, 'hub' = installed. */
  provenance?: 'agent' | 'bundled' | 'hub'
}

/** One entry of the built-in optional-skills catalog (optional-skills/ in the
 *  repo) — official skills that ship with Athena but install on demand. */
...
export interface ToolProvider {
   ...
   *  CLI `athena tools` active-provider detection). */
  is_active: boolean
 ...
 *  not Athena); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** athena-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** athena-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/athena/update/check` — the backend's own update state.
