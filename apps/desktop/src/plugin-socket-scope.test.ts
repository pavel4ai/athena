...
// (#73044). Exercises the REAL athena + store/gateway + store/profile chain:
...
const { pluginSocket, setApiRequestConnection, setApiRequestProfile } = await import('@/athena')
...
const conn = (over: Partial<AthenaConnection> = {}): AthenaConnection =>
  ({
    authMode: 'oauth',
