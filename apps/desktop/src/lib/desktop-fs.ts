import { athenaApi } from '@/api/client'
import type {
  AthenaConnection,
  AthenaReadDirResult,
  AthenaReadFileTextResult,
  AthenaSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: AthenaConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: AthenaConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<AthenaReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<AthenaReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<AthenaReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<AthenaReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.athenaDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: AthenaSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
