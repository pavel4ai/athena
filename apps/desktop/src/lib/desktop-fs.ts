...
export async function readDesktopDir(path: string): Promise<AthenaReadDirResult> {
  ...
  return desktop.api<AthenaReadDirResult>({ path: fsPath('list', path) })
}
...
export async function readDesktopFileText(path: string): Promise<AthenaReadFileTextResult> {
  ...
  return desktop.api<AthenaReadFileTextResult>({ path: fsPath('read-text', path) })
}
...
export async function desktopWorktrees(cwds: string[]): Promise<Record<string, AthenaWorktreeInfo | null>> {
  if (isDesktopFsRemoteMode()) {
    return {}
  }
