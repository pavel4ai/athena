 ...
 * `athena://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `athena://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `athena://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type AthenaOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }
...
export function normalizeAthenaOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('athena://') || trimmed.startsWith(`${ATHENA_PROTOCOL}//`)) {
    try {
      const url = new URL(trimmed)
      const host = url.hostname || ''
      const rest = decodeURIComponent((url.pathname || '').replace(/^\//, ''))

      // athena://open/<path>?… → /<path>?…
      if (host === 'open') {
        if (!rest) {
          return null
        }
      ...
      // athena://index-network/intent/1 → /index-network/intent/1
...
export function resolveAthenaOpenPath(target: AthenaOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizeAthenaOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizeAthenaOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizeAthenaOpenString(target.path)
 ...
 * (`athena://<kind>/<name>?…` → kind/name/params).
 */
export function pathFromAthenaDeepLink(kind: string, name: string, params: Record<string, string> = {}): string | null {
  if (!kind || !name) {
    return null
  }

  if (kind === 'open') {
    return resolveAthenaOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolveAthenaOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `athena://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromAthenaDeepLink('open', name, params)
}
