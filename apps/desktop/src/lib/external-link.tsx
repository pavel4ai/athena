...
const SKIP_PROTO_RE = /^(?:file|data|mailto|javascript|blob|chrome|about|athena):/i
...
export function fetchLinkTitle(url: string): Promise<string> {
  ...
  const bridge = typeof window === 'undefined' ? undefined : window.athenaDesktop?.fetchLinkTitle
...
export function openExternalLink(href: string): void {
  if (href) {
    void window.athenaDesktop?.openExternal?.(href)
  }
}
 ...
 * doesn't cost a context switch out of Athena, and it is the surface the agent
