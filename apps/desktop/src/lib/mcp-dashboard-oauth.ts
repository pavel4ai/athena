...
const UPDATE_BACKEND = 'Update the Athena backend to support Desktop MCP OAuth callbacks.'
...
export async function completeMcpDesktopOAuth({
  ...
  const bridge = window.athenaDesktop.mcpOauth

  // A legacy null connection can resolve to a remote registry primary.
  if (!bridge && scope.connectionId !== 'local') {
    throw new Error('Update Athena Desktop to support MCP OAuth callbacks.')
  }
  ...
  try {
    ...
    await window.athenaDesktop.openExternal(authUrl)
    let pollFailures = 0
