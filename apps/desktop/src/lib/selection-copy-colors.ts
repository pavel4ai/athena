...
// Athena is rendering, and when the two are opposite schemes it takes over
...
function renderedMode(doc: Document): RenderedMode {
  const attr = doc.documentElement.dataset.athenaMode
 ...
 * the only carriers of Athena' palette — are dropped so the paste target's
