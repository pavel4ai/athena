...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __athenaWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__athenaWatch || (win.__athenaWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('athena-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('athena-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __athenaWatchTag?: number }).__athenaWatchTag
