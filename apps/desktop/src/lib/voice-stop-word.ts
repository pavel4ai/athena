...
//
// When someone is in a hands-free "Hey Athena" voice chat, the natural way to
...
// phrase (optionally addressed to Athena), so a real turn that merely contains
...
// Optional address prefixes so "athena stop" / "ok stop" / "hey athena, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey athena', 'hey athena,', 'athena', 'athena,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("athena") is not a stop command on its own.
      continue
    }
 ...
 * to Athena). Returns false for anything that merely contains "stop" as part of
