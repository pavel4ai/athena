...
function sessionsBotsTree() {
  ...
  registerPane('athena-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'athena-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    ...
    expect(setStripTabHidden('athena-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('athena-bots:pane')
    expect($hiddenStripTabs.get()).toContain('athena-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('athena-bots:pane')

    expect(setStripTabHidden('athena-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('athena-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('athena-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('athena-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('athena.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('athena-bots:pane')
    ...
    revealTreePane('athena-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('athena-bots:pane')
    expect(window.localStorage.getItem('athena.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'athena-bots:pane', title: 'athena-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'athena-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
