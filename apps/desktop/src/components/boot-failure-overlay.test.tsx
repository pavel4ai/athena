...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Athena gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>, overrides: Record<string, unknown> = {}) {
  const original = window.athenaDesktop
  Object.defineProperty(window, 'athenaDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config, ...overrides }
  })

  return () => Object.defineProperty(window, 'athenaDesktop', { configurable: true, value: original })
}
