...
const ATHENA_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type AthenaRefType = (typeof ATHENA_REF_TYPES)[number]
...
const ICON_PATHS: Record<AthenaRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as AthenaRefType] ?? ICON_FALLBACK
...
const ATHENA_DIRECTIVE_RE = new RegExp(
...
export const athenaDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(ATHENA_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as AthenaRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: AthenaRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...athenaDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Athena directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.athenaDesktop && isRemoteGateway()
        ? gatewayMediaDataUrl(id)
        : window.athenaDesktop?.readFileDataUrl(id)
