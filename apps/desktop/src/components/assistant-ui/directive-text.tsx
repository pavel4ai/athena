...
const ATHENA_REF_TYPES = WIRE_REFERENCE_KINDS
...
const ATHENA_DIRECTIVE_RE = referenceRe()
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.athenaDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.athenaDesktop?.readFileDataUrl(id)
