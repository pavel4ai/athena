...
const ErrorRecoveryActions: FC = () => {
  ...
  // Open Logs reveals the LOCAL Electron profile's ATHENA_HOME/logs. On a
  ...
  const openLogs = useCallback(async () => {
    try {
      const root = await window.athenaDesktop?.logsRoot?.()
      ...
      const result = await window.athenaDesktop?.openDir?.(root)
