/**
 * @athena/plugin-sdk — THE plugin language. The vscode-module model: plugin
 ...
 *    object as `window.__ATHENA_PLUGIN_SDK__` and maps the import to it, so a
...
import type { PaginatedSessions, UsageStats } from '@/types/athena'
...
export interface PluginProfileRoute {
  ...
  profile: string
  /** Backend Athena profile served by that route. */
...
async function requestPluginProfile<T>(
  ...
  const getAgentRoster = window.athenaDesktop?.getAgentRoster
...
async function pluginRouteStillRegistered(route: PluginProfileRoute): Promise<boolean> {
  const getProfileRoutes = window.athenaDesktop?.getProfileRoutes
...
//
// ensureGatewayProfile awaits the store's dial, and AthenaGateway.connect() has
...
export const host = {
  ...
  deleteProfile: async (profile: string | PluginProfileRoute): Promise<void> => {
    ...
    // restores on relaunch and re-creates the deleted profile (athena-agent#94235).
    dropTilesForProfile(
  ...
  connections: async () => {
    const bridge = window.athenaDesktop?.connections

    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update Athena Desktop.')
    }
  ...
  agents: async () => {
    const roster = window.athenaDesktop?.getAgentRoster

    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update Athena Desktop.')
    }
  ...
  newChat: (profile?: null | string | PluginProfileRoute, options: PluginNewChatOptions = {}): void => {
    if (options.workspaceMode === 'bots') {
      ...
      if (!openTab) {
        notify({ kind: 'error', message: 'Update Athena Desktop to open another Bot chat.' })
   ...
   *
   *  `isStaleTile` (athena-agent#90102): the caller's reconciliation probe
  ...
  profileRoutes: async () => {
    ...
    if (!getProfileRoutes) {
      throw new Error('Athena Desktop connection routing unavailable')
    }
  ...
  listPersistedSessions: async (
    ...
    return athenaApi<PaginatedSessions>({
  ...
  setPersistedSessionHidden: async (
    ...
    return athenaApi<{ ok: boolean; hidden: boolean }>({
      ...(route ? { connectionId: route.connectionId } : {}),
...
/** Athena' reasoning levels + their compact labels, so a plugin surfacing a
...
export type { RpcEvent, StatusResponse } from '@/types/athena'
