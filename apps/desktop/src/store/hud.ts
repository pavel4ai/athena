 ...
 * composer with the reply scrolling above it, so Athena can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.athenaDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.athenaDesktop?.hud
...
export function resetHudLayout(): void {
  void window.athenaDesktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.athenaDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.athenaDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
