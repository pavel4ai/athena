...
import type { AthenaGitWorktree, AthenaRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: AthenaGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, AthenaRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, AthenaGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<AthenaRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<AthenaGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<AthenaRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<AthenaGitWorktree[]>>()
const $noRepoStatus = atom<AthenaRepoStatus | null>(null)
const $noWorktrees = atom<AthenaGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<AthenaRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<AthenaGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: AthenaRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: AthenaGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<AthenaRepoStatus | null>
  seq: number
  target: string
}
