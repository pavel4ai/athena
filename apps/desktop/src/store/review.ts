...
import type { AthenaReviewFile, AthenaReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Athena' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'athena.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'athena.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'athena.desktop.reviewTreeMode'
const SELECTED_KEY = 'athena.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<AthenaReviewFile[]>([])
...
export const $reviewShipInfo = atom<AthenaReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['athenaDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: AthenaReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly AthenaReviewFile[], path: string): AthenaReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.athenaDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.athenaDesktop?.openExternal?.(url)
    }
