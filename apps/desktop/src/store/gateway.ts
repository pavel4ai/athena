import { type ConnectionState, type GatewayEvent, registryBackendScopeKey, resolveGatewayWsUrl } from '@athena/shared'
import { atom } from 'nanostores'

import type { AthenaConnection } from '@/global'
import { AthenaGateway, setApiRequestConnection } from '@/athena'
...
function dialProfile(
  desktop: NonNullable<typeof window.athenaDesktop>,
  profile: string,
...
const isOpen = (gateway: AthenaGateway | null): boolean => gateway?.connectionState === 'open'
...
interface RegistryConfig {
  ...
  onActiveConnectionChanged?: (connection: AthenaConnection) => void
  /**
...
interface GatewayRegistryState {
  ...
  primaryGateway: AthenaGateway | null
  ...
  $gateway: ReturnType<typeof atom<AthenaGateway | null>>
  $activeProfile: ReturnType<typeof atom<string>>
}

const STATE_KEY = Symbol.for('athena.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<AthenaGateway | null>(null),
...
export function setPrimaryGateway(gateway: AthenaGateway | null, profile = 'default'): void {
...
export function setPrimaryGatewayConnection(connection: Pick<AthenaConnection, 'connectionId'> | null): void {
...
async function requestOnPrimaryGateway<T>(
  ...
  if (!gateway || !isOpen(gateway)) {
    throw new Error('Athena gateway unavailable')
  }
...
export function activeGateway(): AthenaGateway | null {
  if (g.activeKey === g.primaryProfile) {
...
function applyActive(profile: string, activationEpoch: number): boolean {
  ...
  // Push the active scope's registry connection into the athena module (null
...
function publishActiveConnection(connection: AthenaConnection): void {
...
function createSecondary(profile: string, connectionId: null | string = null): Secondary {
  const gateway = new AthenaGateway()
...
async function gatewayForProfile(
  ...
  leaseRequest = false,
  spawnPriority: SpawnPriority = 'background'
): Promise<{ gateway: AthenaGateway | null; key: string; release: () => void; scopeProfile: boolean }> {
  const key = normKey(profile)
...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`Athena gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.athenaDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update Athena Desktop.')
  }
...
export async function ensureActiveGatewayOpen(): Promise<AthenaGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "Athena gateway is not connected" on the Sessions `+` action (#88880).
