...
import type { ProjectInfo, ProjectsPayload } from '@/types/athena'
...
const PROJECT_SCOPE_KEY = 'athena.desktop.projectScope'
...
async function activeProjectsContext(profile = projectProfile()): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== normalizeProfileKey($activeGatewayProfile.get())) {
    throw new Error('Active Athena profile changed while connecting')
  }
...
async function refreshProjectTreeAcrossProfiles(): Promise<void> {
  ...
  try {
    const res = await athenaApi<ProjectTreePayload>({
...
const repoScanStates = new WeakMap<AthenaGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<AthenaGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  if (isDesktopFsRemoteMode()) {
    ...
    // with `scan: true` — added in #81723) so repos with zero Athena
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getAthenaConfig(context.profile))
...
// repo, returning where Athena should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<AthenaGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<AthenaGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.athenaDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.athenaDesktop?.writeClipboard?.(path)
  }
}
