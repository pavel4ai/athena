 ...
 *
 * Local models ship on main behind `--local` (either `athena desktop --local`
 * or the flag on Athena.exe itself). The flag is strict: without it the GUI
...
export const $localModelsEnabled = atom<boolean>(
  typeof window !== 'undefined' && window.athenaDesktop?.localModelsEnabled === true
)
