...
/**
 * Feature store for backend (agent) plugins — the native Athena plugins plus
 ...
 * primitives `athena plugins` and the dashboard use, so all surfaces agree on
...
export interface AgentPluginRow {
  ...
  /** Agent Plugins v1 package (portable skills/MCP format) vs native Athena. */
  portable?: boolean
 ...
 *  ATHENA_HOME. Always refetches (it's a cheap local disk scan on the
...
export async function installAgentPlugin(
  ...
  opts: {
    ...
    ref?: string
    /** Target profile's ATHENA_HOME (null/undefined = backend launch profile). */
    profile?: string | null
  }
