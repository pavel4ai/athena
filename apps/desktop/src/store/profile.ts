...
// /api/profiles/active `current`). "default" is the root ~/.athena. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'athena.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'athena.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await athenaApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new ATHENA_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.athenaDesktop.profile.set(name)
}
...
async function resolveConnectionForProfile(profile: string): Promise<AthenaConnection | null> {
...
export async function ensureGatewayProfile(profile: string | null | undefined): Promise<void> {
  ...
  // #89206 split-brain — observed live as atom 'default' over a athena-setup
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<AthenaConnection | null> {
  const getConnectionFor = window.athenaDesktop?.getConnectionFor
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'athena.desktop.showAllProfiles'
...
export function selectProfile(name: string): void {
  ...
  void Promise.all([activateOnCurrentSource(target), shouldRememberStartupProfile])
    .then(([, shouldRemember]) => {
      if (shouldRemember) {
        return window.athenaDesktop?.profile?.remember(target)
      }
...
async function isLocalDesktopProfile(target: string): Promise<boolean> {
  const getConnectionConfig = window.athenaDesktop?.getConnectionConfig
...
// Switch to the default (root ~/.athena) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.athenaDesktop?.touchBackend?.(target).catch(() => undefined)
}
