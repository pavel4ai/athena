...
export const $pullRequestsByBranch = atom<Record<string, AthenaBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'athena.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('athena.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: AthenaBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
