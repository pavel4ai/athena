 ...
 * app's bounds and stays visible while Athena is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'athena.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'athena.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.athenaDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.athenaDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.athenaDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.athenaDesktop?.petOverlay?.close()
}
