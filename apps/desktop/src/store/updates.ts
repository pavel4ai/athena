...
import { checkAthenaUpdate, getActionStatus, updateAthena } from '@/athena'
...
import type { BackendUpdateCheckResponse } from '@/types/athena'
...
const UPDATE_TOAST_SNOOZE_KEY = 'athena:update-toast-snooze-until'
...
const SKEW_TOAST_SNOOZE_KEY = 'athena:backend-skew-toast-snooze-until'
...
const INSTALL_METHOD_TOAST_SNOOZE_KEY = 'athena:install-method-toast-snooze-until'
...
export function reportBackendContract(contract: number | undefined): void {
  ...
  notify({
    action: {
      label: translateNow('notifications.updateAthena'),
      onClick: () => {
        snoozeSkewToast()
...
export async function refreshDesktopVersion(): Promise<DesktopVersionInfo | null> {
  ...
  try {
    const next = await window.athenaDesktop?.getVersion?.()
...
export async function checkBackendUpdates({
  ...
  try {
    const status = mapBackendCheck(await checkAthenaUpdate(force))
...
export async function checkUpdates({ force = false }: UpdateCheckOptions = {}): Promise<DesktopUpdateStatus | null> {
  const bridge = window.athenaDesktop?.updates
...
export async function applyUpdates(opts: DesktopUpdateApplyOptions = {}): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    // `athena update` themselves. Land on a dedicated manual state so the
    ...
    if (result?.manual) {
      $updateApply.set({
        ...
        applying: false,
        stage: 'manual',
        message: result.command ?? 'athena update',
        command: result.command ?? 'athena update'
      })
...
function completedAfterRestart(
  ...
  return !!actionId && status.lines.some(line => line === `=== athena-update completed ${actionId} ===`)
}
...
async function runBackendUpdate(): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    const started = await updateAthena()
    ...
    if (!started.ok) {
      ...
      const command = (started as { update_command?: string }).update_command || 'athena update'
    ...
    while (Date.now() < deadline) {
      ...
      if (!started.action_id && last.exit_code === null) {
        try {
          const status = await checkAthenaUpdate(true)
...
const FOCUS_RECHECK_KEY = 'athena.updates.last-passive-check'
