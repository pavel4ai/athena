 ...
 *  Working in `~/www/athena-agent`, the draft says "athena-agent" constantly
 *  — a "Use skill: athena-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`athena-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/athena-agent-dev`, `plugin:skill`). */
