 ...
 * `@athena/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-athena-glass]`
 * block in styles.css).
 */
...
} from '@athena/shared/translucency'
 ...
 * sets `athenaDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.athenaDesktop?.glassSupported === 'boolean'
    ? window.athenaDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.athenaDesktop?.translucencySupported === 'boolean'
    ? window.athenaDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-athena-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-athena-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-athena-glass-on', glassLive)
  root.toggleAttribute('data-athena-glass', glassOn)
  root.toggleAttribute('data-athena-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-athena-glass-scope', scope)
  } else {
    root.removeAttribute('data-athena-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.athenaDesktop?.setTranslucency?.(state)
  })
