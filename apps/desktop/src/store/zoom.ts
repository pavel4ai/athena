...
export function setZoomPercent(percent: number): void {
  window.athenaDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.athenaDesktop?.zoom) {
  void window.athenaDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.athenaDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
