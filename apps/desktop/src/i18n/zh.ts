...
export const zh: Translations = {
  ...
  settings: {
    ...
    gateway: {
      ...
      envOverride: '环境变量覆盖',
      intro:
        'Athena Desktop 默认会启动自己的本地网关。当你希望此应用控制另一台机器上或可信代理后的现有 Athena 后端时，可以使用远程网关。下面可按 profile 指定各自的远程主机。',
      appliesTo: '应用于',
      ...
      remoteTitle: '远程网关',
      remoteDesc:
        '将此桌面外壳连接到远程 Athena 后端。托管网关使用 OAuth 或用户名密码；自托管网关也可能使用会话 token。',
      ...
      savedTitle: '网关设置已保存',
      restartingMessage: 'Athena Desktop 将使用已保存设置重新连接。',
  ...
  updates: {
    ...
    guiSkewBody: '后端已更新，但此桌面应用包未更改。请更新或重新安装 Athena 桌面应用（你的 AppImage / .deb / .rpm）以保持一致。',
    copy: '复制',
    copied: '已复制',
    done: '完成',
    applyingBody: 'Athena 更新器会在自己的窗口中接管，并在完成后自动重新打开 Athena。更新期间请不要自行重新打开 Athena。',
