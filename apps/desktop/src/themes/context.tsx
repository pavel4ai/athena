...
const SKIN_KEY = 'athena-desktop-theme-v2'
const MODE_KEY = 'athena-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'athena-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'athena-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'athena-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.athenaTheme = skinName
  root.dataset.athenaMode = rendered
  ...
  window.athenaDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('athena-boot-background', chromeBg)
    window.localStorage.setItem('athena-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.athenaThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.athenaDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Athena authoring/activating a skin from a
