/**
 * Live skin sync from the Athena backend.
 *
 * The backend resolves the active skin (built-in or `$ATHENA_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Athena
...
const BACKEND_THEMES_KEY = 'athena-desktop-backend-themes-v1'
 ...
 */
export function ingestBackendSkin(skin: AthenaSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as AthenaSkin)
