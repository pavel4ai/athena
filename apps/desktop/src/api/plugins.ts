 ...
 *
 *  Exported for tests. */
export async function activeConnection(): Promise<AthenaConnection> {
  ...
  return withTimeout(
    window.athenaDesktop.getConnection(profile),
    RECONNECT_ATTEMPT_TIMEOUT_MS,
    `Timed out connecting to profile "${profile}"`
  )
}

/** Options for a plugin REST call — mirrors the app's own `athenaDesktop.api`
 *  shape, minus the path (which is namespace-derived). */
export interface PluginRestOptions {
  method?: string
  body?: unknown
  /** Single-file multipart upload (see AthenaApiRequest.upload). */
...
export async function pluginRest<T>(pluginId: string, path: string, opts: PluginRestOptions = {}): Promise<T> {
  if (!window.athenaDesktop?.api) {
    throw new Error('Athena desktop bridge unavailable')
  }
