import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@athena/shared'
...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as AthenaGateway | null
  const gatewayRef = useRef<AthenaGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.athenaDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<AthenaGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as AthenaGateway | null
      }),
    []
  )
