...
vi.mock('@/athena', async importOriginal => {
  const actual = await importOriginal<typeof AthenaModule>()
  ...
  return { ...actual, AthenaGateway: FakeAthenaGateway }
})
...
const fakeGateway = { connectionState: 'open' } as unknown as AthenaGateway
...
async function activateRemoteGateway() {
  ...
  setPrimaryGateway(primary as unknown as AthenaGateway, 'default')
  $gateway.set(primary as unknown as AthenaGateway)
...
describe('useGatewayRequest', () => {
  ...
  it('surfaces a missing optional scoped mint bridge without falling back to a stale ticket or local lookup', async () => {
    const { desktop, gateway } = await activateRemoteGateway()
    Reflect.deleteProperty(window.athenaDesktop, 'getGatewayWsUrlFor')
  ...
  it('rejects instead of hanging forever when the reconnect getConnection() wedges (#93454)', async () => {
    ...
    } as unknown as AthenaGateway

    const getConnection = vi.fn(() => new Promise(() => undefined))

    ;(window as unknown as { athenaDesktop: unknown }).athenaDesktop = { getConnection }
    $gateway.set(dropped)
