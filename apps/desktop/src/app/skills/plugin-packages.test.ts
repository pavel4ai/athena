...
describe('mergePluginPackages', () => {
  it('shows a unified package as ONE row with both halves, never two rows', () => {
    const rows = mergePluginPackages(
      [desktop({ id: 'media', name: 'Media Studio', packageName: 'athena-media-studio' })],
      [agent({ name: 'athena-media-studio', has_desktop_half: true, description: 'Generate media.' })]
    )
    ...
    expect(rows[0].agent?.name).toBe('athena-media-studio')
  })

  it('a desktop half whose agent half is absent from THIS profile offers the install-here affordance', () => {
    const rows = mergePluginPackages([desktop({ id: 'media', packageName: 'athena-media-studio' })], [])
