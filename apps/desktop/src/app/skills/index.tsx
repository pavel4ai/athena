...
import type { OfficialSkillInfo, SkillInfo, ToolsetInfo } from '@/types/athena'
...
export function SkillsView({
  ...
  const registryBridge = window.athenaDesktop?.connections
  const rosterBridge = window.athenaDesktop?.getAgentRoster
  ...
  const scopeOptions: { key: string; label: string; value: string }[] = useMemo(() => {
    ...
    return (profilesData?.profiles ?? []).map(p => ({
      key: p.name,
      label: p.is_default ? 'Athena (default)' : p.name,
      value: p.name
    }))
                ...
                <MasterDetail pane={skillEditorPane} resizeId="capabilities-split" split="wide">
                  ...
                  >
                    ...
                    {/* The built-in optional-skills catalog, below the
                        installed list: every official skill Athena ships but
