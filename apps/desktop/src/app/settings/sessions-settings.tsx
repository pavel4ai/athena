...
import type { AthenaConfigRecord, SessionInfo } from '@/types/athena'
...
function AutoArchiveSetting() {
  ...
  const persist = useCallback(
    async (autoArchive: boolean, archiveDays: number) => {
      ...
      try {
        await saveAthenaConfig(updated)
      } catch (err) {
...
// / Program Files), which buried any files Athena wrote there.
function DefaultProjectDirSetting() {
  ...
  useEffect(() => {
    ...
    // / Storybook / non-Electron context `window.athenaDesktop` is
    ...
    const settings = window.athenaDesktop?.settings
