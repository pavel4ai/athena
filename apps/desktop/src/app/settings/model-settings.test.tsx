...
beforeEach(() => {
  getGlobalModelInfo.mockResolvedValue({ provider: 'nous', model: 'athena-4' })
  getGlobalModelOptions.mockResolvedValue({
    providers: [
      {
        name: 'Nous',
        slug: 'nous',
        models: ['athena-4', 'athena-4-mini'],
        authenticated: true,
        capabilities: { 'athena-4': { reasoning: true, fast: true } }
      },
  ...
  getAuxiliaryModels.mockResolvedValue({
    main: { provider: 'nous', model: 'athena-4' },
    tasks: [{ task: 'vision', provider: 'auto', model: '', base_url: '' }]
  })
  setModelAssignment.mockResolvedValue({ provider: 'nous', model: 'athena-4', gateway_tools: [] })
...
describe('ModelSettings', () => {
  ...
  it('hides the reasoning/speed defaults when the main model reports no capabilities', async () => {
    getGlobalModelOptions.mockResolvedValueOnce({
      providers: [{ name: 'Nous', slug: 'nous', models: ['athena-4'], authenticated: true, capabilities: { 'athena-4': { reasoning: false, fast: false } } }]
    })
  ...
  it('warns when a main switch leaves auxiliary tasks pinned to another provider', async () => {
    setModelAssignment.mockResolvedValueOnce({
      ...
      gateway_tools: [],
      stale_aux: [{ task: 'compression', provider: 'nous', model: 'athena-4' }]
    })
