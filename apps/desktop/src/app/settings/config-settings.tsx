...
function ConfigSettingsInner({
  ...
  const [config, setConfig] = useState<AthenaConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useAthenaConfigRecord(scopeProfile)
  ...
  const writeConfigCache = useMemo(() => athenaConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const {
    ...
    queryKey:
      scopeProfile == null ? ['athena-config-schema'] : ['athena-config-schema', normalizeProfileKey(scopeProfile)],
    queryFn: () => getAthenaConfigSchema(scopeProfile),
    staleTime: 5 * 60 * 1000
  })
  ...
  // never touched — possibly changed out-of-band by `athena config set`
  // while this page sat open — is never resent with its stale value.
  const configBaselineRef = useRef<AthenaConfigRecord | null>(null)
    ...
    const t = window.setTimeout(() => {
      ...
      saveQueueRef.current = saveQueueRef.current.then(async () => {
        try {
          ...
          const result = await saveAthenaConfig(patch, scopeProfile)
  ...
  const applyConfig = (next: AthenaConfigRecord) => {
    saveVersionRef.current += 1
    setConfig(next)
    setSaveVersion(saveVersionRef.current)
  }

  const updateConfig = (next: AthenaConfigRecord) => {
