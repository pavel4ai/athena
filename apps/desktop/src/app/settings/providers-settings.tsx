...
import type { EnvVarInfo, OAuthProvider } from '@/types/athena'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.athenaDesktop?.terminal)
...
//      in athena_cli/provider_catalog.py) — the SAME provider identity
//      `athena model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Athena can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Athena can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // Athena never deletes creds another tool owns behind a silent API call.
