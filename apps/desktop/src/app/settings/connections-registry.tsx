 ...
 * agent sources (local runtime + any number of remote gateways / Athena Cloud
...
export function ConnectionsRegistrySection() {
  ...
  useEffect(() => {
    if (!editorWantsOauth || !editorUrl || !window.athenaDesktop?.probeConnectionConfig) {
      setAuthProbe(null)
  ...
  const signInOauth = useCallback(async () => {
    ...
    try {
      const result = await window.athenaDesktop.oauthLoginConnectionConfig(editorUrl)
  ...
  // Fan out `athena update` to every eligible source; per-connection results
