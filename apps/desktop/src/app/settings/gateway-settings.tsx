...
export function GatewaySettings() {
  ...
  const save = async (apply: boolean) => {
    ...
    try {
      const next = apply
        ? await window.athenaDesktop.applyConnectionConfig(payload())
        : await window.athenaDesktop.saveConnectionConfig(payload())
  ...
  const signIn = async () => {
    ...
    try {
      ...
      if (result.connected) {
        const refreshed = await window.athenaDesktop.getConnectionConfig(scope)
        setState(refreshed)
  ...
  const signOut = async () => {
    ...
    try {
      await window.athenaDesktop.oauthLogoutConnectionConfig(trimmedUrl || undefined)
      const refreshed = await window.athenaDesktop.getConnectionConfig(scope)
      setState(refreshed)
