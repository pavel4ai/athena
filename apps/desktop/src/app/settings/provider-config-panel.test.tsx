...
import type { MemoryProviderConfig } from '@/types/athena'
...
function hindsightSchema(overrides: Partial<MemoryProviderConfig['fields'][number]>[] = []): MemoryProviderConfig {
  const fields: MemoryProviderConfig['fields'] = [
    ...
    { key: 'bank_id', label: 'Bank ID', kind: 'text', value: 'athena', description: '', placeholder: '', is_set: true, options: [] },
    {
      key: 'recall_budget',
      label: 'Recall budget',
      kind: 'select',
...
describe('ProviderConfigPanel', () => {
  it('renders the declared provider fields generically', async () => {
    ...
    expect(screen.getByDisplayValue('athena')).toBeTruthy()
