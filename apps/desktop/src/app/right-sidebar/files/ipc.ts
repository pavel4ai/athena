...
import type { AthenaReadDirEntry, AthenaReadDirResult } from '@/global'

export type ProjectTreeEntry = AthenaReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: AthenaReadDirEntry) {
...
async function filterIgnored(entries: AthenaReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<AthenaReadDirResult> {
  if (!window.athenaDesktop) {
