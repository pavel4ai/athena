...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { athenaDesktop: { readDir: typeof readDir } }).athenaDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): AthenaReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { athenaDesktop: unknown }).athenaDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: AthenaReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<AthenaReadDirResult>(resolve => {
  ...
  it('discards a stale live refresh after the active registered connection changes', async () => {
    let resolveRefreshFromA: ((result: AthenaReadDirResult) => void) | undefined
  ...
  it('discards a stale child read after the active registered connection changes', async () => {
    let resolveChildFromA: ((result: AthenaReadDirResult) => void) | undefined
  ...
  it('discards a stale root read after the active registered connection changes', async () => {
    let resolveFirst: ((result: AthenaReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<AthenaReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.athenaDesktop is missing', async () => {
    delete (window as unknown as { athenaDesktop?: unknown }).athenaDesktop
  ...
  it('keeps an unreadable root on screen while it re-probes', async () => {
    ...
    let releaseProbe: ((value: AthenaReadDirResult) => void) | undefined
  ...
  it('keeps loaded rows on screen while the root refreshes', async () => {
    ...
    let releaseRefresh: ((value: AthenaReadDirResult) => void) | undefined
  ...
  it('clears the rows when the same path is re-read from another backend', async () => {
    ...
    let releaseFromB: ((value: AthenaReadDirResult) => void) | undefined
