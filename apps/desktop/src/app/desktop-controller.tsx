...
import { getCronJobs, getSessionMessages, listAllProfileSessions, type SessionInfo, triggerCronJob } from '../athena'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.athenaDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.athenaDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  // athena:// deep links (e.g. a docs "Send to App" button for an automation blueprint).
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.athenaDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshAthenaConfig, sttEnabled, voiceMaxRecordingSeconds } = useAthenaConfig({
    activeSessionIdRef,
  ...
  }, [activeSessionId, freshDraftReady, gatewayState, refreshCurrentModel, refreshAthenaConfig])
