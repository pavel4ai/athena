...
import { setAthenaConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `athena://mcp/install` deep links. The payload is
 ...
 * confirming lets Athena spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getAthenaConfigRecord())
      ...
      setAthenaConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
