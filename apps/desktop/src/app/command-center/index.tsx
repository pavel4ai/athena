...
import { getActionStatus, getLogs, getStatus, getUsageAnalytics, restartGateway, updateAthena } from '@/athena'
import type { ActionStatusResponse, AnalyticsResponse, SessionInfo, StatusResponse } from '@/athena'
...
export function CommandCenterView({ initialSection, onClose, onDeleteSession, onOpenSession }: CommandCenterViewProps) {
  ...
  // Client-side substring filter over the fetched tail (matches `athena logs --search`).
  ...
  const runSystemAction = useCallback(
    async (kind: 'restart' | 'update') => {
      ...
      try {
        const started = kind === 'restart' ? await restartGateway() : await updateAthena()
                    ...
                    <div className="flex items-start justify-between gap-3 max-[47.5rem]:flex-col max-[47.5rem]:gap-2">
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 text-[length:var(--conversation-caption-font-size)] text-(--ui-text-tertiary)">
                          {cc.athenaActiveSessions(status.version, status.active_sessions)}
                        </div>
                      </div>
