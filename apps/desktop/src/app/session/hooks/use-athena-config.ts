...
import { getAthenaConfig, getAthenaConfigDefaults } from '@/athena'
...
interface AthenaConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useAthenaConfig({ activeSessionIdRef }: AthenaConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshAthenaConfig = useCallback(
    async (force = false, shouldPublish: () => boolean = () => true) => {
      ...
      try {
        const [config, defaults] = await Promise.all([getAthenaConfig(), getAthenaConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Athena' built-in medium over the user's config.
        if (!canPublish()) {
          return
        }
  ...
  return { refreshAthenaConfig, sttEnabled, voiceMaxRecordingSeconds }
}
