import type { BillingBlock } from '@athena/shared'
 ...
 * billing-specific toast — never the generic "Athena error" — with a smart CTA
  ...
  if (event.type === 'message.complete') {
    ...
    const failure =
      payload?.status === 'error'
        ? {
            error: coerceGatewayText(payload.error).trim() || finalText || 'Athena reported an error',
