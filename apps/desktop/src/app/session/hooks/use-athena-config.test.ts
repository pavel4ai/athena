...
import { getAthenaConfig } from '@/athena'
...
import { useAthenaConfig } from './use-athena-config'

vi.mock('@/athena', () => ({
  getAthenaConfig: vi.fn(),
  getAthenaConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'athena.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getAthenaConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getAthenaConfig>>)

describe('useAthenaConfig refreshAthenaConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to Athena' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => useAthenaConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshAthenaConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useAthenaConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getAthenaConfig>>>()
    vi.mocked(getAthenaConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useAthenaConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshAthenaConfig(true)
    })
    expect(getAthenaConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getAthenaConfig>>)
  ...
  it('does not publish config after its switch loses ownership', async () => {
    const staleConfig = deferred<Awaited<ReturnType<typeof getAthenaConfig>>>()
    vi.mocked(getAthenaConfig).mockReturnValueOnce(staleConfig.promise)
    const { result } = renderHook(() => useAthenaConfig({ activeSessionIdRef: { current: null } }))
    let ownsSwitch = true

    let refresh!: Promise<void>
    act(() => {
      refresh = result.current.refreshAthenaConfig(false, () => ownsSwitch)
    })
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getAthenaConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getAthenaConfig>>>()
    vi.mocked(getAthenaConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshAthenaConfig(true)
      refreshC = result.current.refreshAthenaConfig(true)
    })
