...
function buildTourScript(action: TourAction): string {
  return `(function () {
  var w = window;
  if (!w.__athenaTourEngine) {
    ${driverIife}
    w.__athenaTourHolder = {};
    w.__athenaTourCollect = (${collectTourTargets.toString()});
    w.__athenaTourEngine = (${runTourEngine.toString()});
  }
  if (!document.getElementById('__athena-tour-style')) {
    var style = document.createElement('style');
    style.id = '__athena-tour-style';
    style.textContent = ${JSON.stringify(driverCss)};
    (document.head || document.documentElement).appendChild(style);
  }
  return JSON.stringify(w.__athenaTourEngine(
    w.driver.js.driver,
    w.__athenaTourHolder,
    ${JSON.stringify(action)},
    w.__athenaTourCollect,
    document
  ));
})()`
}
