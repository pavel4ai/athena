        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.athenaDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const annotateGuest = useCallback((): null | PreviewAnnotateGuest => {
    ...
    return {
      capture: async rect => {
        ...
        const dataUrl = await window.athenaDesktop.capturePreview?.({ rect, viewport, webContentsId })
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Athena to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.athenaDesktop?.watchPreviewFile ||
      !window.athenaDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.athenaDesktop.onPreviewFileChanged(payload => {
    ...
    void window.athenaDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.athenaDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.athenaDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:athena-preview')
    ...
    const onGuestContextMenu = (event: Event) => {
      ...
      const zoom = window.athenaDesktop?.zoom?.factor?.() || 1
      ...
      openGuestContextMenu(
        ...
        },
        {
          addToDictionary: (word: string) => {
            ...
            if (typeof webContentsId === 'number') {
              void window.athenaDesktop?.contextMenuGuestAddWord?.({ webContentsId, word })
            }
          },
          copyImage: () => void window.athenaDesktop?.contextMenuCopyImage?.(),
          ...
          <PreviewBrowserBar
            ...
            onOpenExternal={
              !isBrowserWindow() && !canOpenBrowserWindow()
                ? () => void window.athenaDesktop?.openExternal(currentUrl)
                : undefined
            }
