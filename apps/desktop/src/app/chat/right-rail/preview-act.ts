...
const preamble = () => `  var w = window;
  ...
  w.__athenaActHolder = w.__athenaActHolder || {};
  w.__athenaAct = ${actEngineSource()};
  w.__athenaWatch_fn = (${watchInPage.toString()});
  w.__athenaWatchTag = ${WATCH_TAG};
  var holder = w.__athenaActHolder;
  var act = function (a) { return w.__athenaAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__athenaWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  document.addEventListener('pointerdown', function (e) {
    w.__athenaHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'ATHENA-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}
