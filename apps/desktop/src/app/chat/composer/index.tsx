...
import { athenaDirectiveFormatter, type SlashChipKind } from '@/components/assistant-ui/directive-text'
...
import { extractDroppedFiles, ATHENA_PATHS_MIME, partitionDroppedFiles } from '../hooks/use-composer-actions'
...
export function ChatBar({
  ...
  // Distinguish a cold start ("Starting Athena...") from a dropped connection
  ...
  const replaceTriggerWithChip = (item: Unstable_TriggerItem) => {
    ...
    const serialized = athenaDirectiveFormatter.serialize(item)
  ...
  const handleDragEnter = (event: ReactDragEvent<HTMLFormElement>) => {
    if (!onAttachDroppedItems || !dragHasAttachments(event.dataTransfer, ATHENA_PATHS_MIME)) {
      return
    }
  ...
  const handleInputDragOver = (event: ReactDragEvent<HTMLDivElement>) => {
    if (!dragHasAttachments(event.dataTransfer, ATHENA_PATHS_MIME)) {
      return
    }
