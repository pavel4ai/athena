 ...
 *    a `AthenaPlugin` registers automatically (vite glob — drop a folder in).
 *    `athena-bots` (Bot Mode) ships in-tree and is ON by default; other
 *    reference/demo plugins live in the companion `athena-example-plugins`
 ...
 *  - RUNTIME: the on-disk doors (`<athena home>/desktop-plugins/<name>/plugin.js`
 *    and the unified-package half `<athena home>/plugins/<name>/desktop/plugin.js`)
...
const modules = import.meta.glob<{ default: AthenaPlugin }>('../plugins/*/plugin.{js,ts,tsx}', { eager: true })
...
export function discoverBundledPlugins(): void {
  ...
  for (const [path, mod] of Object.entries(modules)) {
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      console.warn(`[plugins] ${path} has no valid default AthenaPlugin export — skipped`)
