...
test('compare payload maps to the behind count and a newest-first commit list; malformed = null', () => {
  ...
  assert.equal(githubRepoSlug('git@github.com:Someone/athena-agent.git'), 'someone/athena-agent')
  assert.equal(githubRepoSlug('https://gitlab.example/x/y.git'), null)
  assert.equal(
    branchTipApiUrl('nousresearch/athena-agent', 'bb/gui'),
    'https://api.github.com/repos/nousresearch/athena-agent/commits/bb%2Fgui'
  )
})
