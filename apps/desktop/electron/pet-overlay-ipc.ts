...
export function registerPetOverlayIpc({
  ...
  ipcMain.handle('athena:pet-overlay:open', async (_event, request) => {
  ...
  ipcMain.handle('athena:pet-overlay:close', async () => {
    closePetOverlay()
  ...
  ipcMain.on('athena:pet-overlay:set-bounds', (_event, bounds) => {
  ...
  ipcMain.on('athena:pet-overlay:ignore-mouse', (_event, ignore) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('athena:pet-overlay:set-focusable', (_event, focusable) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('athena:pet-overlay:state', (_event, payload) => {
    const petOverlayWindow = getPetOverlayWindow()

    if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
      petOverlayWindow.webContents.send('athena:pet-overlay:state', payload)
    }
  })
  // Overlay → main renderer: control messages (pop back in, composer submit).
  ipcMain.on('athena:pet-overlay:control', (_event, payload) => {
    const mainWindow = getMainWindow()
    ...
    mainWindow.webContents.send('athena:pet-overlay:control', payload)
  })
}
