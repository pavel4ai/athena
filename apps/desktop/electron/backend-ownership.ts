  ...
  return {
    ...
    async reapOrphans(): Promise<number[]> {
      ...
      for (let i = 0; i < entries.length; i += 1) {
        ...
        // if it reaches reapOrphans (see main.ts startAthena + #87295).
...
export function backendCommandMatches(command: unknown): boolean {
  return /(?:^|[\s/\\"])(?:athena(?:\.exe)?|athena_cli\.main|athena_cli[/\\]main\.py)"?(?:\s+(?:--profile|-p)\s+\S+)?\s+(?:serve|dashboard)(?:\s|$)/i.test(
    String(command ?? '')
  )
}
