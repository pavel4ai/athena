...
export function markerPath(athenaHome) {
  return path.join(athenaHome, '.athena-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(athenaHome)
  let raw
 ...
 *
 * The Tauri-based athena-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(athenaHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(athenaHome, { kill, maxAgeMs, now: () => nowMs })
 ...
 * Athena to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(athenaHome, opts)
