 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Athena dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Athena install on the remote (login-shell probe),
 ...
 *     `ATHENA_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForAthena in.
 *
 * The minted ATHENA_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const REMOTE_LOCK_DIR = '~/.athena/desktop-ssh'
...
// Resolve the remote athena executable. An EXPLICIT path is honored strictly
...
async function locateAthena(ssh, remoteAthenaPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The athena binary or wrapper script
  ...
  if (remoteAthenaPath) {
    if (await isExecutable(remoteAthenaPath)) {
      return resolveLauncher(remoteAthenaPath)
    }

    const err: any = new Error(
      `The Athena path you set is not an executable on the remote host: "${remoteAthenaPath}". ` +
        'Check the path (it must be the full path to the `athena` binary on the remote, e.g. ' +
        '~/athena-agent/.venv/bin/athena), or clear it to auto-detect.'
    )

    err.kind = 'athena-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v athena')}`)).trim()
  ...
  candidates.push('~/.local/bin/athena')
  candidates.push('/usr/local/bin/athena')
  candidates.push('~/.athena/athena-agent/venv/bin/athena')
  ...
  const err: any = new Error(
    'Athena is not installed on the remote host (could not find a `athena` executable). ' +
      'Install it on the remote with:  curl -fsSL https://athena-agent.nousresearch.com/install.sh | sh  ' +
      '— or set the Athena path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<athena> --version`,
// e.g. "Athena Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH athena a
// connection uses, so a stale/unexpected install is visible.
async function probeAthenaVersion(ssh, athenaPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(athenaPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Athena Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The ATHENA_HOME the remote dashboard will use (explicit env wins, else
// ~/.athena). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteAthenaHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${ATHENA_HOME:-$HOME/.athena}"')).trim().split('\n').pop()

    return out || '~/.athena'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Athena home.')
...
marker=home/'.athena-update-in-progress'
try:
 ...
 * bytes; it never imports or executes code from the changing Athena checkout.
 ...
 */
async function assertRemoteInstallUpdateClear(ssh, athenaHome) {
  const home = assertSafeRemoteHome(athenaHome)
  let observation = ''
  ...
  try {
    ...
    const error: any = new Error('Could not prove that the remote Athena install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ? `Remote Athena update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote Athena update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemoteAthenaProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteAthenaHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Athena profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Athena home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'athenaPath', 'athenaHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !athenaPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(athenaPath)})\n` +
      // The installer-facing launcher is intentionally preserved for invocation
      // (#74411), but it may `exec python <install-dir>/athena`, leaving neither
      // launcher nor ATHENA_HOME-derived entrypoint in argv. The ownership-scoped
      // token path + random nonce + exact profile below are the alternative proof.
      `athena_home=os.path.expanduser(${shq(athenaHome)}) if ${shq(athenaHome)} else ""\n` +
      'expected_entries={expected}\n' +
      'if athena_home:\n' +
      ' expected_entries.add(os.path.join(athena_home,"athena-agent","venv","bin","athena"))\n' +
...
function buildOwnedStaleTerminationCommand(lock, ownershipId) {
  ...
  const expectedPath = expandRemotePath(lock.athenaPath)
  const expectedHome = lock.athenaHome ? expandRemotePath(lock.athenaHome) : "''"
  ...
  const executableMatch = lock.athenaHome
...
function lockMatchesManagedUpdateScope(lock, expected) {
  return Boolean(
    ...
    lock.athenaPath === expected.athenaPath &&
    lock.athenaHome === expected.athenaHome
  )
}
...
expected_path=os.path.expanduser(${py(lock.athenaPath)})
athena_home=os.path.expanduser(${py(lock.athenaHome)})
expected_entries={expected_path,os.path.join(athena_home,"athena-agent","venv","bin","athena")}
...
// execing Athena. mutexPath is expandRemotePath() output — a complete shell
...
result=None
try:
 result=subprocess.run(["sh","-c",payload,"athena-update-mutex",str(fd)],pass_fds=(fd,),check=False)
finally:
 os.close(fd)
...
function buildSpawnCommand(athenaPath, profile, opts: any = {}) {
  const athena = expandRemotePath(athenaPath)
  ...
  const marker = expandRemotePath(`${remoteInstallRoot(opts.athenaHome || '~/.athena')}/.athena-update-in-progress`)

  const updateMutex = expandRemotePath(
    `${remoteInstallRoot(opts.athenaHome || '~/.athena')}/.athena-update-in-progress.mutex`
  )
  ...
  const dashCmd =
    ...
    `exec env ATHENA_DESKTOP=1${opts.guestOnboarding === true ? ' ATHENA_GUEST_ONBOARDING=1' : ''} ${athena} ${profileArgs}${subCmd}`

  const detachedShell = `eval "exec $1>&-"; ${dashCmd} </dev/null >> ${logPath} 2>&1 & echo $!`
  const detachedSpawn = `child=$("$(command -v setsid || echo nohup)" sh -c ${shq(detachedShell)} athena-update-child "$1" & echo $!)`
...
async function remoteSupportsSshOwnership(ssh, athenaPath) {
  const athena = expandRemotePath(athenaPath)

  const out = await ssh.exec(
    `help="$(${athena} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(
  ...
  if (!(await remoteSupportsSshOwnership(ssh, athenaPath))) {
    const err: any = new Error(
      'The remote Athena install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Athena on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    ...
    out = await ssh.exec(
      buildSpawnCommand(athenaPath, profile, {
        spawnNonce,
        tokenFilePath,
        logPath,
        athenaHome,
 ...
 * opened SshConnection, forward/pickLocalPort/waitForAthena, a token-gated
...
async function connect(deps) {
  ...
  const athenaHome = await probeRemoteAthenaHome(ssh)
  await assertRemoteInstallUpdateClear(ssh, athenaHome)
  const athenaPath = await locateAthena(ssh, remoteAthenaPath)
  log(`located athena at ${athenaPath}`)
  const athenaVersion = await probeAthenaVersion(ssh, athenaPath)

  if (athenaVersion) {
    log(`remote athena version: ${athenaVersion}`)
  }
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this Athena Desktop build (${lock.reason}). ` +
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.athenaPath === athenaPath &&
      lock.athenaHome === athenaHome
  ...
  const spawned = await spawnRemoteDashboard(ssh, {
    ...
    athenaHome,
    guestOnboarding,
    assertInstallClear: () => assertRemoteInstallUpdateClear(ssh, athenaHome)
  })
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForAthena(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)
