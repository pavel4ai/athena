// Unit tests for the pure Windows `athena` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvAthenaCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `athena` shim shadows
//      the real athena.cmd/athena.exe.
...
//   3. resolveVenvAthenaCommand() — must probe the venv python via
//      canImportAthenaCli() before trusting it, or a broken venv gets
...
test('chooseUpdaterArgs: gentle --update when both updater runtime files exist', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvAthena: true, hasVenvPython: true }, 'main'), [
    '--update',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: marker-only install uses --repair when the venv is gone', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvAthena: false, hasVenvPython: false }, 'main'),
    ['--repair', '--branch', 'main']
  )
})

test('chooseUpdaterArgs: partial updater runtimes use --repair', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvAthena: false, hasVenvPython: true }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvAthena: true, hasVenvPython: false }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: passes the branch through unchanged in both modes', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvAthena: true, hasVenvPython: true }, 'release/1.2'),
    ['--update', '--branch', 'release/1.2']
  )
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvAthena: false, hasVenvPython: false }, 'release/1.2'),
    ['--repair', '--branch', 'release/1.2']
  )
})

function makeDeps(overrides: Partial<Parameters<typeof resolveVenvAthenaCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportAthenaCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    athenaHome: '/fake/athena-home',
...
test('resolveVenvAthenaCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvAthenaCommand('/root/venv/Scripts/athena.exe', [], deps), null)
})

test('resolveVenvAthenaCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvAthenaCommand('/root/venv/Scripts/athena.cmd', [], deps), null)
})

test('resolveVenvAthenaCommand: returns null when the basename is not athena/athena.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvAthenaCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvAthenaCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvAthenaCommand('/root/venv/bin/athena.exe', [], deps), null)
})

test('resolveVenvAthenaCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvAthenaCommand('/root/venv/Scripts/athena.exe', [], deps), null)
})

test('resolveVenvAthenaCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportAthenaCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvAthenaCommand('/root/venv/Scripts/athena.exe', ['serve'], deps)
...
test('resolveVenvAthenaCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvAthenaCommand('/root/venv/Scripts/athena.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'athena_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvAthenaCommand: is case-insensitive on athena.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvAthenaCommand('/root/venv/Scripts/ATHENA.EXE', [], deps))
  assert.ok(resolveVenvAthenaCommand('/root/venv/SCRIPTS/athena.exe', [], deps))
})
