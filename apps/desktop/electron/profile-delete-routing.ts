// Profile-delete routing logic for the `athena:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_athena_home() and
...
/** Parse a `athena:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its ATHENA_HOME.
 *
...
/**
 * Route the next `athena:api` request away from the primary/window backend
 ...
 * ensure_athena_home() recreates the directory the delete just removed.
 */
