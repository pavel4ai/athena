...
export const ZOOM_STORAGE_KEY = 'athena:desktop:zoomLevel'
 ...
 * but forgot to emit 'athena:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('athena:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
 ...
 * only learns of changes through `athena:zoom:changed`, which never fired —
