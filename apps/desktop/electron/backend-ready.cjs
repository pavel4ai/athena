const _READY_RE = /^ATHENA_DASHBOARD_READY port=(\d+)/m
...
/**
 * Watch a child process's stdout for the `ATHENA_DASHBOARD_READY port=<N>`
...
function waitForDashboardPort(child, timeoutMs = resolvePortAnnounceTimeoutMs()) {
  return new Promise((resolve, reject) => {
    ...
    function onExit(code, signal) {
      cleanup()
      reject(new Error(`Athena backend: exited before port announcement (${signal || code})`))
    }
