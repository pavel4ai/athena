...
const installerConfig = JSON.parse(
  fs.readFileSync(path.resolve(desktopRoot, '../bootstrap-installer/src-tauri/tauri.conf.json'), 'utf8')
)
...
describe('Athena desktop distribution identity', () => {
  it('keeps Athena product branding and protocol metadata', () => {
    ...
    expect(packageJson.build.protocols).toEqual([{ name: 'Athena Protocol', schemes: ['athena'] }])
    expect(JSON.stringify(packageJson).toLowerCase()).not.toContain(upstreamName.toLowerCase())
  })

  it('keeps runtime branding aligned with package metadata', () => {
    expect(mainSource).toMatch(/const APP_NAME = process\.env\.ATHENA_DESKTOP_APP_NAME \|\| 'Athena'/)
  ...
  it('preserves upgrade-compatible desktop and setup identifiers', () => {
    ...
    expect(mainSource).toMatch(/app\.setAppUserModelId\('com\.nousresearch\.athena'\)/)
  })
