...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/athena', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/athena/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/athena'), 'https://gw.example.com/athena/auth/native/refresh')
})
