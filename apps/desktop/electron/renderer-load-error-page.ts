 ...
 *
 * 1. TORN BUNDLE after an update (#95575): `athena update` replaces the app
...
export interface RendererLoadErrorDetails {
  ...
  /** Repair command hint, e.g. `athena desktop --force-build`. */
  repairHint?: string
  /**
...
export function buildRendererLoadErrorPage(details: RendererLoadErrorDetails = {}): string {
  ...
  const title = 'Athena couldn\u2019t start the desktop UI'
  ...
  const repair = details.repairHint ? `<p>Repair with: <code>athena desktop --force-build</code></p>` : ''
...
<main>
  ...
  <code>athena desktop --force-build</code>, then restart the app.</p>
