 ...
 * runtime, remote gateways, Athena Cloud instances, SSH hosts) that are all
 ...
 * apps/shared/src/backend-scope.ts (`@athena/shared`) — tsconfig project
...
export async function reuseMatchingPrimarySshBackend({
  ...
  if (
    ...
    String(source.remoteAthenaPath || '').trim() !== String(activeSsh.remoteAthenaPath || '').trim() ||
...
/** Turn `ls ~/.athena/profiles` output into roster names. Always includes
...
/**
 * Whether "Update all instances" may drive this connection. Athena Cloud
 * instances are platform-managed — we never run `athena update` against them.
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remoteAthenaPath: input.remoteAthenaPath,
  ...
  if (kind === 'remote' || kind === 'cloud') {
    ...
    // transport-/Athena-managed names; an empty result stores nothing.
 ...
 * `remoteAthenaPath`/`remoteProfile`. Only fields the payload explicitly
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Athena Cloud' : 'Remote gateway'),
...
export function reconcileAppliedGlobalConnection(
  ...
  const hostLabel = hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Athena Cloud' : 'Remote gateway')
