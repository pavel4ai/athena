 ...
 *
 * The staged Tauri `athena-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `athena update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `athena update`, swaps/relaunches
 * the app, and writes .athena-update-result.json. With the app gone before
 * the update starts, the ATHENA_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `athena update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into ATHENA_HOME on *every* platform
 * (`athena-setup.exe` on Windows, `athena-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_athena_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `athena update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(athenaHome, 'athena-setup.exe')
 ...
 *
 * `copy_self_to_athena_home` deliberately no-ops during `--update`
 ...
 * "Another Athena update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `athena update`, which
 ...
 * the installer is written to ATHENA_HOME at install/repair time, making mtime
