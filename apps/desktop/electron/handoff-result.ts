 ...
 * console. It writes ATHENA_HOME/.athena-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen Athena within 30 minutes.
...
export function handoffResultPath(athenaHome: string): string {
  return path.join(athenaHome, '.athena-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(athenaHome)
  let raw: string
