export type ParentWatchdogEnv = {
  ATHENA_PARENT_PID: string
  ATHENA_PARENT_START_MARKER?: string
  ATHENA_PARENT_NONCE?: string
  /** Spawn tag consumed by athena_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { ATHENA_PARENT_PID: String(pid) }
  env.ATHENA_SPAWN = spawnTag(pid, startMarker)
  ...
  env.ATHENA_PARENT_START_MARKER = startMarker
  env.ATHENA_PARENT_NONCE = nonce

  return env
}

/** Build the `ATHENA_SPAWN` tag mirrored by athena_cli/process_identity.py.
