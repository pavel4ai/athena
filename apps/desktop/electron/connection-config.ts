 ...
 *     `X-Athena-Session-Token` header; WS uses `?token=`.
...
// athena_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - athena_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - athena_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / athena #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-athena_session_at', '__Secure-athena_session_at', 'athena_session_at']
const RT_COOKIE_VARIANTS = ['__Host-athena_session_rt', '__Secure-athena_session_rt', 'athena_session_rt']

// The Nous portal (NAS) does NOT use Athena gateway session cookies — it is a
...
// Keep this aligned with athena_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['athena', 'test', 'tmp', 'root', 'sudo'])
...
function gatewayTicketFailure(error, authMessage, transportMessage) {
  ...
  if (needsOauthLogin) {
    ...
    // change until the user signs in, so tag it the way startAthena latches
...
/** Which Athena profile the remote SSH dashboard should actually run as.
...
// Athena Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteAthenaPath = String(entry.remoteAthenaPath || '').trim()

  if (remoteAthenaPath) {
    out.remoteAthenaPath = remoteAthenaPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Athena installation. Preserve an explicit mapping
  // when it is a valid Athena profile identifier; otherwise fall back to the
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_athena_action` registers the (often dynamic, e.g.
 ...
 *     `--profile`, so its `ATHENA_HOME` scopes it.
 *
...
function resolveProfileBackendRoute(profile, opts: ProfileRouteOptions = {}): ProfileBackendRoute {
  ...
  if (scopedProfile === primaryProfile) {
    ...
    // the wire: the dashboard's process ATHENA_HOME can belong to a different
 ...
 * `athena:api` handler — both routes reach a backend whose namespace is the
...
function translateSelfProfileQuery(path, profile, backendProfile) {
  ...
  try {
    parsed = new URL(rawPath, 'http://athena.local')
  } catch {
    return path
  }
...
/**
 * Resolve the two decisions made by the `athena:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a athena session ACCESS-token cookie
 ...
 * the Athena gateway session cookies, so cloud sign-in / discovery liveness
