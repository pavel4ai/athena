...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'athena-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/athena-runner-test',
    sourceRepoRoot: null,
    athenaHome: '/tmp/athena-runner-test',
    logRoot: '/tmp/athena-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'athena-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'athena-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/athena-agent',
      athenaHome: '/tmp/athena'
    }),
    ['--dir', '/tmp/athena-agent', '--athena-home', '/tmp/athena', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/athena-agent', '--athena-home', '/tmp/athena', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/athena',
      athenaHome: '/tmp/home'
    }),
    ['--dir', '/tmp/athena', '--athena-home', '/tmp/home', '--branch', 'main']
  )
})
