// Backend subcommand routing for the desktop-managed Athena process.
//
// The desktop app launches its own headless backend via `athena serve` — it
...
// app hasn't updated yet, or an older `athena` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Athena profile to pin via `--profile`.
 */
 ...
 * `-m athena_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `athena_cli/subcommands/dashboard.py` source registers
