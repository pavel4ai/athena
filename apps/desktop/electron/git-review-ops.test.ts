...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'athena-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'athena-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Athena Test'], { cwd: dir })
