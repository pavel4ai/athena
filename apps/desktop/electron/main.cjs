...
const { canImportAthenaCli, verifyAthenaCli } = require('./backend-probes.cjs')
...
const { buildDesktopBackendEnv, normalizeAthenaHomeRoot } = require('./backend-env.cjs')
...
function resolveAthenaHome() {
  if (process.env.ATHENA_HOME) return normalizeAthenaHomeRoot(process.env.ATHENA_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'athena-home')
  if (IS_WINDOWS) {
    ...
    if (fromRegistry) return normalizeAthenaHomeRoot(fromRegistry)
  }
...
function athenaManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_athena_node_dirs() in
  // athena_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(ATHENA_HOME, 'node')
...
function pathWithAthenaManagedNode(...entries) {
  return [...athenaManagedNodePathEntries(), ...entries, process.env.PATH]
    .filter(Boolean)
    .join(path.delimiter)
}
...
const MEDIA_PROTOCOL = 'athena-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(athena-media://…)`.
...
let mainWindow = null
let athenaProcess = null
let connectionPromise = null
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by athenaProcess +
// connectionPromise + startAthena(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.ATHENA_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.ATHENA_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
async function waitForUpdateToFinish() {
  let marker = readLiveUpdateMarker(ATHENA_HOME)
  ...
  while (marker && Date.now() < deadline) {
    ...
    marker = readLiveUpdateMarker(ATHENA_HOME)
  }
  if (marker) {
...
// findGitBash — locate bash.exe on Windows. Athena' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\athena\git\... — checked
  // first so users who installed via install.ps1 are detected before we
  // start probing system-wide locations.
  const localAppData = process.env.LOCALAPPDATA || ''
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'athena', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'athena', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// ATHENA_HOME/athena-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_athena_home). That binary owns
// ALL repo mutation — running `athena update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'athena-setup.exe' : 'athena-setup'
  const candidate = path.join(ATHENA_HOME, name)
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (athenaProcess && Number.isInteger(athenaProcess.pid)) pids.push(athenaProcess.pid)
  ...
  if (athenaProcess && !athenaProcess.killed) {
    try {
      athenaProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri athena-setup. Unlike Windows
      ...
      // whole update itself: `athena update` (backend) + `athena desktop
    ...
    if (!updater) {
      ...
      // athena-setup.exe into ATHENA_HOME). They DO have a working `athena`
      ...
      // checkout they're on — bare `athena update` defaults to main and would
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') command = `athena update --branch ${branch}`
        }
      } catch {
    ...
    emitUpdateProgress({
      stage: 'restart',
      message: 'Updating Athena — this window will close and the updater will open. Don’t reopen Athena yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const updaterArgs = fileExists(venvAthena) ? ['--update', '--branch', branch] : ['--repair', '--branch', branch]
...
// Resolve the athena CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `athena` on PATH.
function resolveAthenaCliBinary(updateRoot) {
  const venvAthena = path.join(updateRoot, 'venv', 'bin', 'athena')
  if (fileExists(venvAthena)) return venvAthena
  return findOnPath('athena') || null
}
...
// macOS/Linux in-app update: backend (`athena update`) + OS-aware GUI rebuild
// (`athena desktop --build-only`), then atomically swap the running .app bundle
// with the freshly built one and relaunch. Degrades to "backend updated,
// restart to load the new GUI" if the swap can't be performed.
async function applyUpdatesPosixInApp() {
  const updateRoot = resolveUpdateRoot()
  const athena = resolveAthenaCliBinary(updateRoot)
  if (!athena) {
  ...
  // Put the Athena-managed Node and the venv on PATH so `athena desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\athena\node, not node\bin.
  const env = {
    ATHENA_HOME,
    PATH: pathWithAthenaManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `athena update` reaps stale `athena dashboard` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads ATHENA_DESKTOP_CHILD_PID and excludes
  ...
  if (athenaProcess && Number.isInteger(athenaProcess.pid)) {
    desktopChildPids.push(athenaProcess.pid)
  }
  for (const entry of backendPool.values()) {
  ...
  if (desktopChildPids.length) {
    env.ATHENA_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Athena (git + dependencies)…', percent: 10 })
  const updated = await runStreamedUpdate(athena, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'athena update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'athena update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(athena, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Athena to retry.',
      error: rebuilt.error || 'rebuild-failed'
    })
    return { ok: false, backendUpdated: true, error: 'desktop rebuild failed' }
  }

  // Linux in-app update terminal state (#45205). `athena desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Athena…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `athena-desktop-update-${Date.now()}.sh`)
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Athena to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Athena desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Athena to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Athena.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Athena.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Athena to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.athena-update-new"; then
    rm -rf "$DST.athena-update-old" 2>/dev/null || true
    mv "$DST" "$DST.athena-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.athena-update-new" "$DST"
    rm -rf "$DST.athena-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Athena to load the new version.',
      percent: 100
    })
...
function isBootstrapComplete() {
  ...
  return isAthenaSourceRoot(ACTIVE_ATHENA_ROOT) && fileExists(getVenvPython(VENV_ROOT))
}
...
function createPythonBackend(root, label, dashboardArgs, options = {}) {
  ...
  return {
    ...
    label,
    command: python,
    args: ['-m', 'athena_cli.main', ...dashboardArgs],
...
function createActiveBackend(dashboardArgs) {
  ...
  return {
    ...
    env: buildDesktopBackendEnv({
      athenaHome: ATHENA_HOME,
      pythonPathEntries: [ACTIVE_ATHENA_ROOT],
      venvRoot: VENV_ROOT
    }),
...
function resolveAthenaBackend(dashboardArgs) {
  ...
  if (overrideRoot && isAthenaSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Athena source at ${overrideRoot}`, dashboardArgs)
  ...
  if (!IS_PACKAGED && isAthenaSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Athena source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
    if (backend) return backend
  }

  // 3. Bootstrap-complete ACTIVE_ATHENA_ROOT -- the canonical install at
  ...
  //    to spawning athena. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `athena update` from the CLI.
  ...
  if (process.env.ATHENA_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    if (athenaCommand) {
      ...
      if (verifyAthenaCli(athenaCommand, { shell: shellForProbe })) {
        return {
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError = new Error('Athena recovery was handed off to Athena Setup. The desktop will restart when recovery completes.')
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Athena install was cancelled.')
  ...
  if (!fileExists(venvPython)) {
    ...
    // isAthenaSourceRoot() and the bootstrap writes the marker only after
...
async function waitForAthena(baseUrl, token) {
  ...
  throw new Error(`Athena backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
const ZOOM_STORAGE_KEY = 'athena:desktop:zoomLevel'
...
// Futurebound Corp.) instead of a static session token. The auth model is
...
const OAUTH_SESSION_PARTITION = 'persist:athena-remote-oauth'
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Athena gateway',
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (athena #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Athena gateway uses OAuth, but you are not signed in. ' +
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.ATHENA_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.ATHENA_DESKTOP_REMOTE_TOKEN
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with Futurebound Corp." instead of a generic
...
function resetAthenaConnection() {
...
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetAthenaConnection() nulls athenaProcess.
...
// profile to startAthena() (the window backend: boot UI, bootstrap, remote
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForAthena(remote.baseUrl, remote.token)
    return {
      ...remote,
      profile,
  ...
  const backend = await ensureRuntime(resolveAthenaBackend(dashboardArgs))
  ...
  child.once('exit', (code, signal) => {
    ...
    if (!ready) {
      rejectStart?.(
        new Error(`Athena backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
...
async function startAthena() {
  ...
  connectionPromise = (async () => {
    ...
    athenaProcess.once('exit', (code, signal) => {
      ...
      if (!backendReady) {
        const message = `Athena backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
    ...
    // Discover the ephemeral port the child bound to
    const port = await Promise.race([waitForDashboardPort(athenaProcess), backendStartFailed])
    ...
    backendReady = true
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null athenaProcess when the child dies.
      childAlive: () => athenaProcess !== null && athenaProcess.exitCode === null && !athenaProcess.killed,
      rememberLog
    })
    updateBootProgress({
      phase: 'backend.ready',
...
ipcMain.handle('athena:connection', async (_event, profile) => ensureBackend(profile))
...
ipcMain.handle('athena:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetAthenaConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Athena backend failed liveness probe; dropping stale connection.')
    resetAthenaConnection()
...
ipcMain.handle('athena:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
...
ipcMain.handle('athena:window:openNewSession', async () => {
...
ipcMain.handle('athena:bootstrap:repair', async () => {
  ...
  // startAthena() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('athena:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('athena:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('athena:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
  return true
...
ipcMain.handle('athena:selectPaths', async (_event, options = {}) => {
...
function terminalShellCommand() {
  // ATHENA_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.ATHENA_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Athena' TUI into
  ...
  env.TERM_PROGRAM = 'Athena'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a athena/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from ATHENA_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.ATHENA_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `athena:terminal:${id}:${suffix}`
}
...
ipcMain.handle('athena:fs:worktrees', async (_event, cwds) => worktreesForIpc(cwds))

ipcMain.handle('athena:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Athena.')
  }
...
ipcMain.handle('athena:terminal:write', (_event, id, data) => {
...
ipcMain.handle('athena:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('athena:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('athena:updates:check', async () =>
...
// ---------------------------------------------------------------------------
const ATHENA_PROTOCOL = 'athena'
let _pendingDeepLink = null
let _rendererReadyForDeepLink = false

function _extractDeepLink(argv) {
  if (!Array.isArray(argv)) return null
  return argv.find(a => typeof a === 'string' && a.startsWith(`${ATHENA_PROTOCOL}://`)) || null
}
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(ATHENA_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
