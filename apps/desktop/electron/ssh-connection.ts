...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(ATHENA_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Athena-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.athena/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'athena-desktop-ssh')
  }

  return path.join(os.homedir(), '.athena', 'desktop-ssh')
}
