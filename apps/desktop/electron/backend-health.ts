...
export interface AthenaReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForAthenaReady`).
   */
 ...
 *  - readiness-probe exhaustion in waitForAthenaReady().
 *
...
/**
 * True when the backend URL points at a Nous-managed Athena Cloud instance
 ...
 * `isReauthRequired` so startAthena latches; a bare `needsOauthLogin` (the
...
export async function waitForAthenaReady(baseUrl: string, options: AthenaReadyOptions): Promise<void> {
  ...
  throw new Error(`Athena backend did not become ready: ${detail}`)
}
