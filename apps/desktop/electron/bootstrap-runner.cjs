...
function downloadInstallScript(commit, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/pavel4ai/athena/${commit}/scripts/${scriptName}`
...
async function resolveInstallScript({ installStamp, sourceRepoRoot, athenaHome, emit, _download = downloadInstallScript }) {
  ...
  const cached = cachedScriptPath(athenaHome, installStamp.commit)
  try {
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, athenaHome } = {}) {
  return new Promise((resolve, reject) => {
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, athenaHome } = {}) {
  return new Promise((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, athenaHome }) {
...
async function fetchManifest({ scriptPath, installerKind, emit, athenaHome, activeRoot, installStamp }) {
  const isPosix = installerKind === 'posix'
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, athenaHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, athenaHome, activeRoot, abortSignal, installStamp }) {
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, athenaHome })
      ]
