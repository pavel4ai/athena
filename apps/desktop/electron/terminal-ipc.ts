...
// registry, and the athena:terminal:* IPC surface. Extracted from main.ts; the
...
export function registerTerminalIpc({
  ...
  function terminalShellCommand() {
    ...
    const override = (process.env.ATHENA_DESKTOP_SHELL || (isWindows ? '' : process.env.SHELL) || '').trim()
  ...
  ipcMain.handle('athena:terminal:attach', (event, id) => {
  ...
  ipcMain.handle('athena:terminal:cwd', async (_event, id) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
