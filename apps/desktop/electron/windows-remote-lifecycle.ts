...
const READY_RE = /^ATHENA_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitAthenaPath = '') {
  const explicit = psLiteral(explicitAthenaPath)
  ...
  const script = [
    ...
    '$athenaHome=$env:ATHENA_HOME',
    'if(-not $athenaHome){$athenaHome=Join-Path $env:LOCALAPPDATA "athena"}',
    'Assert-NoReparse $athenaHome $true',
    '$candidate=[IO.Path]::Combine($athenaHome, "athena-agent\\venv\\Scripts\\athena.exe")',
    ...
    '$profileCandidate=[IO.Path]::Combine($HOME, "athena-agent\\.venv\\Scripts\\athena.exe")',
    ...
    '$fallbackHomeCandidate=Join-Path $athenaHome "athena-agent\\venv\\Scripts\\athena.exe"',
    '$fallbackProfileCandidate=Join-Path $HOME "athena-agent\\.venv\\Scripts\\athena.exe"',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command athena.exe -ErrorAction SilentlyContinue',
    ...
    '$athena=$null',
    'foreach($candidate in $candidates){Assert-NoReparse $candidate $true;$candidatePython=[IO.Path]::Combine([IO.Path]::GetDirectoryName($candidate), "python.exe");Assert-NoReparse $candidatePython $true;try{$item=Get-Item -LiteralPath $candidate -Force -ErrorAction Stop;if(($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0 -and -not $item.PSIsContainer){$athena=$item.FullName;break}}catch [Management.Automation.ItemNotFoundException]{continue}}',
    'if(-not $athena){throw "Athena is not installed on the remote Windows host."}',
    'Assert-NoReparse $athena $false',
    'if($explicit -and $athena -ne $explicit){throw "The configured Athena path is not an executable file."}',
    '$python=[IO.Path]::Combine([IO.Path]::GetDirectoryName($athena), "python.exe")',
    'Assert-NoReparse $python $false',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;athenaHome=$athenaHome;athenaPath=$athena;python=$python}|ConvertTo-Json -Compress'
  ].join(';')

  return JSON.parse((await ssh.exec(powerShellCommand(script))).trim())
}

function windowsUpdateMarkerProbeCommand(athenaHome) {
  const script = [
    '$ErrorActionPreference="Stop"',
...
public static class AthenaMarkerNoFollow {
...
`,
    ...
    '}',
    '}',
    `$home=${psLiteral(athenaHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".athena-update-in-progress"',
    '$result="UNCERTAIN"',
    '$stream=$null;$memory=$null',
    'try{',
    'Assert-NoReparse $marker $true',
    'if(-not (Test-Path -LiteralPath $marker -PathType Leaf)){$result="CLEAR"}else{$stream=[AthenaMarkerNoFollow]::OpenRead($marker)',
 ...
 */
async function assertWindowsRemoteInstallUpdateClear(ssh, athenaHome) {
  let observation = ''

  try {
    observation =
      String(await ssh.exec(windowsUpdateMarkerProbeCommand(athenaHome)))
        .replace(/^\uFEFF/, '')
        .trim()
        .split(/\r?\n/)
        .pop() || ''
...
async function detectRemotePlatform(ssh, explicitAthenaPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitAthenaPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'athena_cli.windows_ssh_runtime', operation, ...args]
...
function atomicWindowsSpawnCommand(runtime, reservation: any = {}) {
  const argv = [runtime.python, '-m', 'athena_cli.windows_ssh_runtime', 'spawn']
  const helper = operation => [runtime.python, '-m', 'athena_cli.windows_ssh_runtime', operation]

  const script = [
    '$ErrorActionPreference="Stop"',
    `$home=${psLiteral(runtime.athenaHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".athena-update-in-progress"',
    ...
    reservation.ownershipId
      ...
      : '  if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}',
    reservation.ownershipId
      ? `  $spawned=$spawnLines[-1]|ConvertFrom-Json; $lock=[ordered]@{schemaVersion=2;protocolVersion=1;ownershipId=${psLiteral(reservation.ownershipId)};spawnNonce=${psLiteral(reservation.spawnNonce)};pid=[int]$spawned.pid;creationTimeNs=[string]$spawned.creationTimeNs;port=0;profile=${psLiteral(reservation.profile)};athenaPath=${psLiteral(reservation.athenaPath)};athenaHome=${psLiteral(reservation.athenaHome)};tokenFingerprint=${psLiteral(reservation.tokenFingerprint)};startedAt=${psLiteral(reservation.startedAt)}}|ConvertTo-Json -Compress; ` +
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.athenaPath === 'string' &&
    typeof lock.athenaHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.athenaPath === runtime.athenaPath &&
    lock.athenaHome === runtime.athenaHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remoteAthenaPath)
  await assertWindowsRemoteInstallUpdateClear(ssh, runtime.athenaHome)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.athenaPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Athena on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.athenaPath = inspection.path
  const athenaVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located athena at ${runtime.athenaPath}`)
  ...
  try {
    ...
    spawned = await atomicWindowsSpawn(
      ssh,
      runtime,
      JSON.stringify({ ownershipId, spawnNonce, profile, athenaPath: runtime.athenaPath }),
      {
        ownershipId,
        spawnNonce,
        profile,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForAthena(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Athena SSH"', 'powershell.exe -NoLogo')
