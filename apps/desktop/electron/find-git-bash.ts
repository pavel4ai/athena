 ...
 *   1. ATHENA_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\athena\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect ATHENA_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.ATHENA_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'athena', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'athena', 'git', 'usr', 'bin', 'bash.exe'))
  }
