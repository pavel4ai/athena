...
test('Windows spawn holds the update mutex across marker check and helper spawn', () => {
  const command = atomicWindowsSpawnCommand({
    athenaHome: 'C:\\Users\\andre\\.athena',
    python: 'C:\\Users\\andre\\.athena\\python.exe'
  })

  const encoded = command.match(/-EncodedCommand\s+([^\s]+)$/)?.[1]
  const script = encoded ? Buffer.from(encoded, 'base64').toString('utf16le') : ''
  assert.match(script, /\.athena-update-in-progress/)
...
test('Windows spawn publishes the initial ownership record before releasing the mutex', () => {
  const command = atomicWindowsSpawnCommand(
    ...
    },
    {
      ...
      profile: 'default',
      athenaPath: 'C:\\Athena\\athena.exe',
      athenaHome: 'C:\\Users\\andre\\.athena',
...
test('Windows relaunch gate refuses live and uncertain markers before executing the remote runtime', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = sshWith(async command => {
      ...
      if (script.includes('Get-Command athena.exe')) {
        return JSON.stringify({
          os: 'Windows',
          arch: 'AMD64',
          athenaHome: 'C:\\Users\\alice\\.athena',
          athenaPath: 'C:\\Athena\\athena.exe',
          python: 'C:\\Athena\\python.exe'
        })
      }

      if (script.includes('.athena-update-in-progress')) {
        return observation
      }
    ...
    assert.equal(
      scripts.some(script => script.includes('athena_cli.windows_ssh_runtime')),
      false
    )
  }
})
...
test('Windows relaunch gate uses strict install-wide marker parsing and fail-closed PID probing', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(ssh, 'C:\\Users\\alice\\.athena\\profiles\\research')
  assert.match(script, /\.athena-update-in-progress/)
...
test('Windows probe validates Athena and Python topology before selection', async () => {
  let script = ''
  ...
  const fallbackJoin = script.indexOf('Join-Path $athenaHome')
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('Athena is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /Athena is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\Athena's\\python.exe" }, 'inspect', [
    'C:\\x y\\athena.exe'
  ])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'athena_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /Athena''s/)
  assert.match(script, /C:\\x y\\athena\.exe/)
})
...
test('Windows SSH reuse requires the requested remote profile to match the lock', () => {
  ...
  const runtime = { athenaPath: lock.athenaPath, athenaHome: lock.athenaHome }
