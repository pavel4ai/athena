 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `athena update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.athenaHome, 'Athena home')
  const athenaPath = validateRemoteValue(target.athenaPath, 'launcher path')
  ...
  const launcherWord = expandRemotePath(athenaPath)

  const updateCommand =
    `env ATHENA_HOME=${homeWord} ` +
    `ATHENA_UPDATE_CORRELATION_ID=${shq(correlation)} ` +
    'ATHENA_UPDATE_ORIGIN_PROFILE=default ' +
    `ATHENA_UPDATE_ORIGIN_HOME=${homeWord} ` +
    `ATHENA_UPDATE_OUTPUT_PATH=${outputWord} ` +
...
function buildWindowsManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const wrapper = [
    ...
    `$env:ATHENA_HOME=${psLiteral(home)}`,
    `$env:ATHENA_UPDATE_CORRELATION_ID=${psLiteral(correlation)}`,
    '$env:ATHENA_UPDATE_ORIGIN_PROFILE="default"',
    `$env:ATHENA_UPDATE_ORIGIN_HOME=${psLiteral(home)}`,
    `$env:ATHENA_UPDATE_OUTPUT_PATH=${psLiteral(outputPath)}`,
    ...
    `$env:ATHENA_UPDATE_WINDOWS_DETACHED=${psLiteral(correlation)}`,
    `$env:ATHENA_UPDATE_TAURI_OUTCOME_PATH=${psLiteral(statusPath)}`,
    `$env:ATHENA_UPDATE_TAURI_READY_PATH=${psLiteral(readyPath)}`,
    ...
    `& ${psLiteral(athenaPath)} update --yes *>> ${psLiteral(outputPath)}`,
    '$rc=$LASTEXITCODE',
...
marker_path=install_root/'.athena-update-in-progress'
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote Athena updated and every managed SSH profile is ready.'
        : restoreOk
