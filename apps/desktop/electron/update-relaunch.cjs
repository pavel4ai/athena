 ...
 * Background
 * ----------
 * After `athena update` + `athena desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything ATHENA_DESKTOP_* is preserved
// plus ATHENA_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['ATHENA_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['ATHENA_DESKTOP_']
