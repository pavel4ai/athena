// Launching the Athena TUI in the user's OWN terminal emulator.
//
...
// `athena --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's Athena runtime is often a venv Python invoked
//     as `python -m athena_cli.main`, not a `athena` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The Athena
