 ...
 * product name (`Athena`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Athena'
 *
 * This is a hard failure with no obvious cause for the user — `athena desktop`
 ...
 * (the mac-unpacked Athena.app bundle) and Windows (win-unpacked), so we
 ...
 * Athena.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `athena desktop --build-only` (athena_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Athena.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Athena'}.exe`
