#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Athena icon + version metadata onto the
// built Athena.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Athena.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `athena desktop`, the installer's
...
// path (which rebuilds via `athena desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.cjs <path-to-Athena.exe>
//
...
// Stamp the Athena icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'Athena',
      FileDescription: 'Athena',
      CompanyName: 'Futurebound Corp.',
      LegalCopyright: 'Copyright (c) 2026 Futurebound Corp.'
    }
  })

  console.log('[set-exe-identity] done — Athena icon + identity stamped')
}
