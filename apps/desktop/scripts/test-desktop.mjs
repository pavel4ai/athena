...
// no longer bundles the Athena Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Athena.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Athena'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Athena.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'athena'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default ATHENA_HOME for non-sandboxed runs -- matches main.cjs's
// resolveAthenaHome(). On Windows it's %LOCALAPPDATA%\athena; elsewhere
// it's ~/.athena. The fresh-install sandbox launchFresh() sets its own
// ATHENA_HOME and never touches this.
const DEFAULT_ATHENA_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'athena')
  }
  return path.join(os.homedir(), '.athena')
})()
const VENV_ROOT = path.join(DEFAULT_ATHENA_HOME, 'athena-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'athena-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.ATHENA_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Athena-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Athena-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Athena-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Athena-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.ATHENA_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.ATHENA_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const athenaHome = path.join(sandbox, 'athena-home')
  ...
  env.ATHENA_DESKTOP_CWD = cwd
  env.ATHENA_DESKTOP_IGNORE_EXISTING = '1'
  env.ATHENA_DESKTOP_TEST_MODE = 'fresh-install'
  env.ATHENA_DESKTOP_USER_DATA_DIR = userDataDir
  env.ATHENA_HOME = athenaHome
  delete env.ATHENA_DESKTOP_ATHENA
  delete env.ATHENA_DESKTOP_ATHENA_ROOT
  ...
  console.log(`  ATHENA_HOME: ${athenaHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(athenaHome, 'athena-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Athena Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of athena_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'athena-agent', 'athena_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Athena
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + ATHENA_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  ATHENA_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
