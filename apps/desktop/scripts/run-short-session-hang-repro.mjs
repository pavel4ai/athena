...
const UPSTREAM_URL = 'https://github.com/pavel4ai/athena.git'
...
const LEGACY_STORAGE_KEY = 'athena.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'athena.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.athenaDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `athena-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const athenaHome = join(sandbox, 'athena-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(athenaHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(athenaHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        ATHENA_DESKTOP_APP_NAME: `AthenaShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        ATHENA_DESKTOP_ATHENA_ROOT: target.targetRoot,
        ATHENA_DESKTOP_IGNORE_EXISTING: '1',
        ATHENA_DESKTOP_USER_DATA_DIR: userData,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'athena-short-session-ab-'))
  const prepared = []
