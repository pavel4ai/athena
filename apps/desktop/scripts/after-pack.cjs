 ...
 *
 * Stamps the Athena icon + identity onto the packed Windows Athena.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `athena desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Athena')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Athena'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Athena.exe keeps the stock Electron icon`)
  }
}
