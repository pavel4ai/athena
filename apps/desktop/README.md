...
It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing Athena gateway or install Athena locally. Local onboarding then walks you through choosing a provider and model.

### Prebuilt installers

Prebuilt installers are built and distributed via [the Athena Desktop website.](https://athena-agent.nousresearch.com/).
...
```bash
# throwaway ATHENA_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the Athena Agent runtime into `ATHENA_HOME`
(`~/.athena`, or `%LOCALAPPDATA%\athena` on Windows), using the same layout as a
CLI install.
...
- **Athena Agent** runs as a headless `athena serve` process and exposes the
...
1. `ATHENA_DESKTOP_ATHENA_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `ATHENA_DESKTOP_ATHENA`, or `athena` on `PATH`
5. a system Python that can import the Athena runtime
...
Desktop supports a managed local backend, explicit remote gateways, and Athena
...
screen offers **Connect to existing Athena** before starting the local installer.
...
terminal commands, and file operations run against the remote Athena host, not
...
transport- or Athena-managed header names such as `Authorization`, `Cookie`,
`Host`, `Origin`, `Referer`, and `X-Athena-Session-Token`.
