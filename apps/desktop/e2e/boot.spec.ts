 ...
 * packaged binary) with a real `athena serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → athena serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Athena title', async () => {
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { athenaDesktop?: Record<string, unknown> }).athenaDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → athena serve is spawned → WS connects → config
