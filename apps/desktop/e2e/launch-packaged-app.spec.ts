...
/**
 * E2E smoke tests for the packaged Athena desktop app.
 *
 ...
 * (credential stripping, isolated ATHENA_HOME + userData, unique app name).
 *
...
test('window opens with the Athena title', async () => {
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      athenaDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).athenaDesktop?.hud?.open({ sessionId: null })
  )
