/**
 * Shared E2E fixtures for the Athena desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → athena serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `athena desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `athena-e2e-${prefix}-${Math.random()}`))
  const athenaHome = path.join(root, 'athena-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(athenaHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(athenaHome, 'config.yaml')
 ...
 */
export function writeEnvFile(athenaHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(athenaHome, '.env')
 ...
 */
function writeEmptyConfig(athenaHome: string): void {
  const configPath = path.join(athenaHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - ATHENA_HOME → sandbox athena-home (isolated config/sessions)
 *  - ATHENA_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - ATHENA_DESKTOP_IGNORE_EXISTING=1 → don't pick up `athena` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - ATHENA_DESKTOP_ATHENA_ROOT → REPO_ROOT (dev checkout resolution)
 *  - ATHENA_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    ATHENA_HOME: sandbox.athenaHome,
    ATHENA_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    ATHENA_DESKTOP_IGNORE_EXISTING: '1',
    ATHENA_DESKTOP_ATHENA_ROOT: REPO_ROOT,
    ATHENA_DESKTOP_APP_NAME: `AthenaE2E-${Date.now()}`,
    ...
    ATHENA_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated ATHENA_HOME + userData
 * @param env      - the process environment (already has ATHENA_HOME etc.)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.athenaHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via ATHENA_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`athena serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.athenaHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { ATHENA_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Athena backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Athena.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Athena.app', 'Contents', 'MacOS', 'Athena')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'athena')
}
 ...
 * progress without spawning a real Athena backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * ATHENA_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    ATHENA_DESKTOP_BOOT_FAKE: '1',
    ATHENA_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + athena-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).ATHENA_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).ATHENA_DESKTOP_ATHENA
  delete (env as Record<string, string | undefined>).ATHENA_DESKTOP_ATHENA_ROOT
