 ...
 * AIAgent loop, using the E2E mock provider configured in `athenaHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(athenaHome: string) {
  ...
  static async start(athenaHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(athenaHome)
