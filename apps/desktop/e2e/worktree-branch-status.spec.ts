...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Athena E2E'], { cwd: repo })
...
function configureRepoCwd(athenaHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(athenaHome, mockUrl)
  fs.appendFileSync(path.join(athenaHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(athenaHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.athenaHome, mock.url, repo)
