...
export const $athenaHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.athena/logs/bootstrap-installer.log')
    $athenaHome.set('~/.athena')
  ...
  try {
    const [logPath, athenaHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_athena_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $athenaHome.set(athenaHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Athena" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Athena-Setup.exe --update);
...
export async function launchAthenaDesktop(): Promise<void> {
  ...
  await invoke('launch_athena_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Athena repository' },
