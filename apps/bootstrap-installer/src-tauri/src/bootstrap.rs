...
//! ATHENA_HOME/logs/bootstrap-<timestamp>.log.
//!
//! Lifecycle:
...
pub struct StartBootstrapArgs {
    ...
    /// Optional override for ATHENA_HOME. Tests use this; production
    /// almost always falls back to the OS default.
    pub athena_home: Option<String>,
}
...
/// Spawn the locally-built Athena desktop binary, then close the installer
...
#[tauri::command]
pub async fn launch_athena_desktop(
    app: AppHandle,
    install_root: String,
) -> Result<(), String> {
    let install_root = PathBuf::from(install_root);
    let exe_path = resolve_athena_desktop_exe(&install_root).ok_or_else(|| {
        format!(
            "Couldn't find a built Athena desktop at {}. The desktop build step \
             may have been skipped or failed. Run `athena desktop` from a \
             terminal to build and launch it.",
            install_root.join("apps").join("desktop").join("release").display()
        )
    })?;

    tracing::info!(?exe_path, "launching Athena desktop");

    // Detach from us — the installer is about to exit. On macOS launch the
    // bundle through LaunchServices instead of exec'ing Contents/MacOS/Athena
...
/// <os>-unpacked/<exe>).
pub(crate) fn resolve_athena_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    let release_dir = install_root.join("apps").join("desktop").join("release");
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        &[
            ("win-unpacked", "Athena.exe"),
            ("win-arm64-unpacked", "Athena.exe"),
        ]
    } else if cfg!(target_os = "macos") {
        &[
            ("mac/Athena.app/Contents/MacOS", "Athena"),
            ("mac-arm64/Athena.app/Contents/MacOS", "Athena"),
        ]
    } else {
        &[("linux-unpacked", "athena")]
    };
...
pub(crate) fn resolve_athena_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    let exe = resolve_athena_desktop_exe(install_root)?;
    #[cfg(target_os = "macos")]
    {
        // .../Athena.app/Contents/MacOS/Athena -> .../Athena.app
...
/// path so a bare re-open just opens Athena instead of re-running setup.
pub(crate) fn athena_is_installed(install_root: &std::path::Path) -> bool {
    install_root.join(".athena-bootstrap-complete").exists()
        && resolve_athena_desktop_exe(install_root).is_some()
}
...
fn write_bootstrap_complete_marker(install_root: &Path, pin: &Pin) -> Result<serde_json::Value> {
    ...
    // writeFileAtomic(). athena_is_installed() only checks existence, so a
    // partial direct write would incorrectly enable the launcher fast path.
    let tmp_path = install_root.join(".athena-bootstrap-complete.tmp");
    {
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_athena_desktop_exe(install_root).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "no built Athena desktop app")
    })?;
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // installer exits, mirroring launch_athena_desktop. Kept correct here
...
pub(crate) fn open_macos_app_detached(app_bundle: &std::path::Path) -> std::io::Result<()> {
    ...
    cmd.arg(app_bundle);
    cmd.current_dir(crate::paths::athena_home());
    cmd.spawn().map(|_child| ())
}
...
async fn run_bootstrap(
    ...
    // explicitly; we infer it from $AthenaHome which Stage-Repository clones
    // the repo INTO at $AthenaHome\athena-agent. Mirrors athena_constants.
    let athena_home = args
        .athena_home
        .clone()
        .unwrap_or_else(|| crate::paths::athena_home().to_string_lossy().into_owned());
    let install_root = PathBuf::from(&athena_home).join("athena-agent");
    ...
    // Copy ourselves to ATHENA_HOME/athena-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_athena_home() {
        tracing::warn!(?err, "failed to copy installer into ATHENA_HOME (non-fatal)");
        emit_log(&format!(
...
async fn run_install_script(
    ...
    args: &[String],
    athena_home_override: Option<&str>,
    ...
    powershell::run_script(script_path, args, sink, athena_home_override, cancel_rx)
        .await
        .map_err(|e| {
...
mod tests {
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            ...
            std::fs::write(macos_dir.join("Athena"), b"#!/bin/sh\n").unwrap();
            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../Athena.app
        } else if cfg!(target_os = "windows") {
            let dir = release.join("win-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("Athena.exe");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        } else {
            let dir = release.join("linux-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("athena");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        }
    }
    ...
    // what the updater ditto's over /Applications/Athena.app). A regression in
    // this derivation breaks the post-update auto-relaunch, so guard it.
    #[test]
    fn resolve_athena_desktop_app_finds_built_bundle() {
        let root = unique_tmp_dir("app-ok");
        let expected = make_release_tree(&root);

        let resolved = resolve_athena_desktop_app(&root)
    ...
    #[test]
    fn resolve_athena_desktop_app_is_none_without_a_build() {
        let root = unique_tmp_dir("app-none");
        // No release tree created.
        assert!(
            resolve_athena_desktop_app(&root).is_none(),
    ...
    fn bootstrap_complete_marker_uses_desktop_compatible_schema() {
        ...
        let marker_path = root.join(".athena-bootstrap-complete");
    ...
    fn bootstrap_complete_marker_is_published_atomically() {
        ...
        let marker_path = root.join(".athena-bootstrap-complete");
        let tmp_path = root.join(".athena-bootstrap-complete.tmp");
        assert!(
            marker_path.is_file(),
    ...
    #[test]
    fn athena_is_installed_treats_marker_existence_as_sufficient() {
        ...
        make_release_tree(&root);
        std::fs::write(root.join(".athena-bootstrap-complete"), b"").unwrap();
    ...
    fn marker_write_failure_leaves_no_final_marker() {
        ...
        assert!(
            !not_a_dir.join(".athena-bootstrap-complete").exists(),
            "failed write must not leave a final marker that enables the fast path"
        );
        assert!(
            !not_a_dir.join(".athena-bootstrap-complete.tmp").exists(),
