...
/// read side does not see EOF until the last of them closes it. `athena update`
...
/// stranded with it.
///
/// Same bound `Invoke-AthenaStep` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `athena_home_override` propagates to the child as $ATHENA_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, athena_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = athena_home_override {
        cmd.env("ATHENA_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, athena_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = athena_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_athena_home() {
    ...
    /// child. `athena update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]
