...
//!
//! Mirrors `athena_constants.get_athena_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\athena
//!   macOS:   ~/.athena
//!   Linux:   ~/.athena  (override via $ATHENA_HOME)
//!
//! NOTE (macOS): Python's get_athena_home(), scripts/install.sh, and the
//! Electron desktop's resolveAthenaHome() ALL use ~/.athena on macOS — there
...
/// Returns the canonical Athena home directory, respecting $ATHENA_HOME if set.
pub fn athena_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("ATHENA_HOME") {
    ...
    {
        // %LOCALAPPDATA%\athena — matches scripts/install.ps1's $AthenaHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("athena");
        }
    }

    // macOS + Linux + fallback: ~/.athena (matches Python get_athena_home(),
    // install.sh, and the Electron desktop's resolveAthenaHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".athena");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    athena_home().join("bootstrap-cache")
}
...
/// ATHENA_HOME so it survives repo checkout deletion (unlike anything under
/// athena-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\athena\athena-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_athena`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under ATHENA_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves ATHENA_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    athena_home().join(".athena-update-in-progress")
}
...
pub fn copy_self_to_athena_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to ATHENA_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_ATHENA_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".athena-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under ATHENA_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[athena-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("ATHENA_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_athena_home() -> String {
    athena_home().to_string_lossy().into_owned()
}
