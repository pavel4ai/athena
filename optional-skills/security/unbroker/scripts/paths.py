...
All per-subject data lives under PDD_DATA_DIR (default: $ATHENA_HOME/unbroker),
which is the same trust boundary Athena uses for .env and OAuth tokens.
"""
...
def athena_home() -> Path:
    return Path(os.environ.get("ATHENA_HOME") or (Path.home() / ".athena"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else athena_home() / "unbroker"
