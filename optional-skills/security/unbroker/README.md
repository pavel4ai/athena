...
The design is **Athena-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
athena skills install official/security/unbroker
```

Then start a new Athena session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Athena session:
...
- **PII handling.** Dossiers live under the Athena home directory (`0600`, optionally
