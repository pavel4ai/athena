...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ATHENA_HOME", os.path.expanduser("~/.athena")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ATHENA_HOME", Path.home() / ".athena")) / "skills" / "red-teaming" / "godmode"
...
ATHENA_HOME = Path(os.getenv("ATHENA_HOME", Path.home() / ".athena"))
CONFIG_PATH = ATHENA_HOME / "config.yaml"
PREFILL_PATH = ATHENA_HOME / "prefill.json"
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.athena/prefill.json."""
