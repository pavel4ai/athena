...
Capabilities:
- Persist telephony provider credentials to the Athena .env file ($ATHENA_HOME/.env)
...
def _athena_home() -> Path:
    return Path(os.environ.get("ATHENA_HOME", "~/.athena")).expanduser()


def _env_path() -> Path:
    return _athena_home() / ".env"


def _config_path() -> Path:
    return _athena_home() / "config.yaml"


def _state_path() -> Path:
    return _athena_home() / "telephony_state.json"
