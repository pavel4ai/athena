...
$ATHENA_OSINT_CACHE/icij/ (default: ~/.cache/athena-osint/icij/) and search
...
def _cache_dir() -> Path:
    base = os.environ.get("ATHENA_OSINT_CACHE")
    if base:
        return Path(base) / "icij"
    return Path.home() / ".cache" / "athena-osint" / "icij"
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $ATHENA_OSINT_CACHE/icij or ~/.cache/athena-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
