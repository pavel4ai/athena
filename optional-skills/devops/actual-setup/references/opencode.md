...
is typically only in `~/.athena/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike Athena,
...
`athena-custom-providers` skill, pitfall 2). Athena hits this because it forwards
