...
Usage (via cron with --no-agent):
    ...
    athena cron create athena-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$ATHENA_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name athena-issues --repo pavel4ai/athena --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Athena .env file
(``${ATHENA_HOME:-~/.athena}/.env``) to avoid the 60 req/hr
