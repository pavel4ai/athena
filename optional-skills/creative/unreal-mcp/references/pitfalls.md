...
### 1. Start order: editor first, Athena second

Athena probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Athena session.
...
restart the editor, restart the Athena session.
...
server fails to bind (Output Log) or Athena' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.athena/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Athena

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Athena' connection
lives in `~/.athena/config.yaml` via `athena mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Athena — just don't
mistake it for the Athena setup step.
...
explicitly warns against overlapping calls. Athena executes same-turn tool
...
### 9. Timeouts: Athena gives up before Unreal does

Athena' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Athena runs elsewhere (SSH backend,
