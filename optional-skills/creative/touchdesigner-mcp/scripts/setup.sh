...
ATHENA_HOME_DIR="${ATHENA_HOME:-$HOME/.athena}"
ATHENA_CFG="${ATHENA_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Athena config has twozero_td MCP entry ──
if [[ ! -f "$ATHENA_CFG" ]]; then
    echo -e " ${FAIL} Athena config not found at ${ATHENA_CFG}"
    manual_steps+=("Create ${ATHENA_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ATHENA_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Athena config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Athena config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ATHENA_CFG} manually"); }
    manual_steps+=("Restart Athena session to pick up config change")
fi
