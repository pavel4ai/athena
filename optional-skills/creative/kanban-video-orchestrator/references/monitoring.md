...
athena kanban watch --tenant <project-slug>

# Snapshot of the board
athena kanban list --tenant <project-slug>
athena kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
athena kanban stats --tenant <project-slug>

# Visual dashboard (browser)
athena dashboard

# Inspect a specific task (includes comments + events)
athena kanban show <task-id>

# Follow a single task's event stream
athena kanban tail <task-id>
```

Verify available subcommands with `athena kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `athena kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `athena kanban block <id>` then `athena kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `athena kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `athena kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `athena profile list` (profile exists?), `athena status` (gateway/dispatcher up?), and verify tenant. |
...
athena kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
athena kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(athena kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
athena kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
athena kanban block <task-id>
athena kanban archive <task-id>

# Diagnose what happened
athena kanban show <task-id>      # task body, comments, recent events
athena kanban tail <task-id>      # follow the live event stream
athena kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `athena kanban create` for the director
...
while true; do
    clear
    athena kanban list --tenant <slug>
    echo "---"
    athena kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `athena kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `athena profile list` and start
