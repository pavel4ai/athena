...
key is present in `${ATHENA_HOME:-~/.athena}/.env` (or macOS Keychain) before firing the kanban.
...
preference: macOS Keychain → `${ATHENA_HOME:-~/.athena}/.env` → environment variable.
