...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    ...
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.athena/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.athena/tts/.",
    },
    "discord-settings": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'athena cron'.",
    },
    "hooks-config": {
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ConfigReadError(
            ...
            f"({exc}). Fix it with `athena config edit` (or move it aside), then "
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`athena config edit` (or move it aside), then re-run the migration."
        )
    return data
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    ...
    ``~/.athena/config.yaml`` into a dotfiles repo or profile package.
    """
    if yaml is None:
...
# become ``~/.athena`` — the real Athena home — not the broken ``~/.Athena``).
...
def _case_preserving_replacement(replacement: str):
    ...
    ``~/.athena/config.yaml`` (the real Athena home) instead of the broken
    ``~/.Athena/config.yaml``.
    """
...
def parse_existing_memory_entries(path: Path) -> List[str]:
    """Parse a DESTINATION Athena memory store (memories/MEMORY.md, USER.md).
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-athena/apply.ts "blocked by earlier
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-athena/plan.ts warnings[].
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
        ...
        for oc_key, athena_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and athena_key not in secret_additions:
                secret_additions[athena_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, athena_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and athena_key not in secret_additions:
                        secret_additions[athena_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        athena_config = load_yaml_file(destination)
        current_model = athena_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = athena_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                athena_config["model"] = {"default": model_str}
            dump_yaml_file(destination, athena_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        athena_config = load_yaml_file(destination)
        existing_tts = athena_config.get("tts", {})
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        athena_cfg_path = self.target_root / "config.yaml"
        athena_cfg = load_yaml_file(athena_cfg_path)
        existing_mcp = athena_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            athena_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                athena_srv["command"] = srv["command"]
                if srv.get("args"):
                    athena_srv["args"] = srv["args"]
                if srv.get("env"):
                    athena_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    athena_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                athena_srv["url"] = srv["url"]
                if srv.get("headers"):
                    athena_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    athena_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                athena_srv["enabled"] = False
            if srv.get("timeout"):
                athena_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                athena_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                athena_srv["tools"] = {}
                if tools_cfg.get("include"):
                    athena_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    athena_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
        ...
        if added > 0 and self.execute:
            self.maybe_backup(athena_cfg_path)
            athena_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(athena_cfg_path, athena_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'athena cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = athena_cfg.get("agent") or {}
        ...
        if compaction:
            compression = athena_cfg.get("compression") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'athena gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("ATHENA_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if session.get("reset") or session.get("resetTriggers") or session.get("reset_triggers"):
            self.record("session-config", "session reset timers", None, "skipped",
                        "Athena conversations do not reset on idle or daily timers")
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        athena_cfg_path = self.target_root / "config.yaml"
        athena_cfg = load_yaml_file(athena_cfg_path)
        custom_providers = athena_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(athena_cfg_path)
            athena_cfg["custom_providers"] = custom_providers
            dump_yaml_file(athena_cfg_path, athena_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if discord_cfg:
            athena_cfg_path = self.target_root / "config.yaml"
            athena_cfg = load_yaml_file(athena_cfg_path)
            discord_athena = athena_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_athena["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_athena["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                athena_cfg["discord"] = discord_athena
                dump_yaml_file(athena_cfg_path, athena_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        athena_cfg_path = self.target_root / "config.yaml"
        athena_cfg = load_yaml_file(athena_cfg_path)
        browser_athena = athena_cfg.get("browser") or {}
        changed = False

        # Map fields that have Athena equivalents
        if browser.get("cdpUrl"):
            browser_athena["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_athena["headless"] = browser["headless"]
            changed = True

        if changed:
            athena_cfg["browser"] = browser_athena
            if self.execute:
                self.maybe_backup(athena_cfg_path)
                dump_yaml_file(athena_cfg_path, athena_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            athena_mode = mode_map.get(mode, "manual")
            athena_cfg.setdefault("approvals", {})["mode"] = athena_mode
            if self.execute:
                self.maybe_backup(athena_cfg_path)
                dump_yaml_file(athena_cfg_path, athena_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{athena_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        ...
        notes.extend([
            ...
            "- Review `~/.athena/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--target", default=os.environ.get("ATHENA_HOME") or str(Path.home() / ".athena"), help="Athena home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.athena/" + dest[len(str(report["target_root"])) + 1:]
