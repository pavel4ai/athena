...
Usage:
  ...
  <server_name> matches the files in $ATHENA_HOME/mcp-tokens/<server>.json etc.
...
def _athena_home():
    # Prefer Athena' own resolver (profile-safe); fall back to env then ~/.athena.
    try:
        from athena_constants import get_athena_home
        return str(get_athena_home())
    except Exception:
        return os.environ.get("ATHENA_HOME") or os.path.expanduser("~/.athena")


def _tokens_dir():
    return os.path.join(_athena_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "athena-diag", "version": "1.0"}},
        },
        headers={
