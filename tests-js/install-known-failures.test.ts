...
const lockedLog = [
  'error: failed to remove file `C:/install/venv/Lib/site-packages/../../Scripts/athena.exe`: Access is denied. (os error 5)',
  'File "C:/install/venv/Scripts/athena.exe/__main__.py", line 10, in <module>',
...
const base = {
  ...
  installMethod: 'installer-script', updateMethod: 'athena-update',
  error: 'E2E ASSERTION FAILED: athena update exited 1 (expected 0)', logs: { update: lockedLog },
}
...
describe('known install failures', () => {
  it('recognizes the released launcher self-lock, not generic access denied', () => {
    ...
    expect(matchKnownFailure({ ...base, logs: { update: lockedLog.replaceAll('athena.exe', 'other.exe') } })).toBeNull()
  ...
  it('matches manual-only app updates only for the three proven July script cases', () => {
    const sample = {
      ...
      updateMethod: 'athena-desktop-app-update',
      error: 'E2E ASSERTION FAILED: app driven via captured athena desktop spec; update completed',
      logs: { desktop: '[athena] [updates] no staged updater; surfacing manual `athena update` for CLI install at C:/install\n[athena] [updates] manual: athena update\n' },
    }
    ...
    expect(matchKnownFailure({ ...sample, logs: { desktop: '[updates] manual: athena update' } })).toBeNull()
  })
...
it('renders known receipts as footnotes, without suppressing a red job', async () => {
  ...
  const name = 'windows: installer-script -> athena-update (v2026.3.12 -> HEAD)'
