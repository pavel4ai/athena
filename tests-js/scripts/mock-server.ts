 ...
 * dev:mock dev flow.
 *
 * Implements just enough of the /v1/* surface for `athena serve` to resolve a
...
/**
 * Drives a real code edit followed by two finish attempts. Athena should add
...
export function createBackgroundReleaseHandle(): BackgroundReleaseHandle {
  const path = nodePath.join(
    os.tmpdir(),
    `athena-e2e-bg-release-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  )
...
/** Create an isolated ATHENA_HOME + Electron user-data dir in the OS temp dir. */
function createDevSandbox(): DevSandbox {
  const root = fs.mkdtempSync(nodePath.join(os.tmpdir(), `athena-dev-mock-${Date.now()}`))
  const athenaHome = nodePath.join(root, 'athena-home')
...
function writeMockConfig(athenaHome: string, mockUrl: string): void {
  fs.writeFileSync(
    nodePath.join(athenaHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(nodePath.join(athenaHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function runDevLaunch(): Promise<void> {
  ...
  writeMockConfig(sandbox.athenaHome, mock.url)
  console.log(`  ATHENA_HOME: ${sandbox.athenaHome}`)
  ...
  const env: Record<string, string> = {
    ...
    ATHENA_DESKTOP_ATHENA_ROOT: repoRoot,
    ATHENA_DESKTOP_APP_NAME: `AthenaDevMock-${Date.now()}`,
  }
