 ...
 *
 * The root ``package.json`` is installed by ``athena update`` on every user,
 ...
 *   workspace graph entirely. ``athena update`` and ``athena doctor --fix``
 ...
 *   ``athena tools`` → Browser Automation → Camofox, and only used at
 *   runtime when ``CAMOFOX_URL`` is set. Its postinstall fetches a ~300MB
 *   Firefox-fork binary, which silently blocked ``athena update`` for
 ...
 * fails — read the lazy-install guidance in the ``athena-agent-dev`` skill
...
test('camofox is not in root dependencies (must stay opt-in)', () => {
  ...
  assert.ok(
    ...
    'Camofox is a ~300MB binary-postinstall backend that must stay ' +
      ...
      'Camofox post_setup handler in athena_cli/tools_config.py so it ' +
      'only installs when the user explicitly selects Camofox via ' +
      '`athena tools` → Browser Automation → Camofox.'
  )
})
