...
# Ensure sibling modules (_athena_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _athena_home import display_athena_home, get_athena_home

ATHENA_HOME = get_athena_home()
TOKEN_PATH = ATHENA_HOME / "google_token.json"
CLIENT_SECRET_PATH = ATHENA_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = ATHENA_HOME / "google_oauth_pending.json"
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_athena_home()}/google_token.json")
