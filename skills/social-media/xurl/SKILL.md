...
> **Docker HOME pitfall:** In the official Athena Docker layout, `/opt/data` is `ATHENA_HOME`, but Athena tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for Athena-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
