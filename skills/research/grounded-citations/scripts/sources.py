...
Ledger path resolution (first wins):
  --ledger PATH
  $ATHENA_CITATION_LEDGER
  $ATHENA_HOME/cache/citations/ledger.json
"""
...
from _athena_home import get_athena_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("ATHENA_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_athena_home() / "cache" / "citations" / "ledger.json"
