...
Ledger location: `$ATHENA_HOME/cache/citations/ledger.json` (profile-aware).
Override per task with `--ledger <path>` or `ATHENA_CITATION_LEDGER`.

## How to Run

```bash
S=~/.athena/skills/research/grounded-citations/scripts/sources.py
...
`athena skills install official/social-media/reddit-reading` or
`athena skills install official/research/rss-feeds` before using them.
...
- **Parallel subagents.** Each subagent has its own working directory; point
  them all at one ledger with `--ledger` (or `ATHENA_CITATION_LEDGER`) if their
