...
## Layer 1 — Is this thing a Athena plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Athena process**. Its LLM calls go through Athena's already-configured provider. | Already uses Portal if user's Athena is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Athena. That LLM call is already authenticated
via Athena's provider config. The plugin's own server auth (e.g.
...
What Athena Agent has that *feels* like OAuth — `athena login --provider nous`
opening a browser, user signs in, token lands in `~/.athena/auth.json` — is a
**Athena-specific browser flow**. Under the hood it produces a credential
Athena uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Athena's existing Portal credential out of `~/.athena/auth.json`.
...
rides on the credential Athena already holds.

Where this could live in Athena:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Athena itself.
...
1. First decide: Athena plugin (runs inside Athena) or separate app? If plugin,
   it already uses Portal via Athena's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Athena agent itself") is fully supported. Athena treats agent
...
pitch in the Tool Gateway announcement is specifically about Athena Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Athena is configured to use Portal as its provider. It is
