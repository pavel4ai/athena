...
Write plugins for the Athena desktop app: statusbar items, layout panes,
...
system (`~/.athena/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$ATHENA_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$ATHENA_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
  ...
  `.athena-package.json` marker) — the ONLY root the renderer loads from — so
...
- The Athena desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$ATHENA_HOME/desktop-plugins/` (usually
  `~/.athena/desktop-plugins/`).

## How to Run

1. Create `$ATHENA_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.athena/...` by default, or `~/.athena/profiles/<profile>/...` under a
...
The ONLY import surface is `@athena/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Athena
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.athena/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@athena/plugin-sdk`, `react`, and
