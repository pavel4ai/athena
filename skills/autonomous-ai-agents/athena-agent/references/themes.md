# Themes / Skins — Author a Athena Color Theme

Author a Athena **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`athena_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.athena/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Athena itself.
...
- Write access to the Athena home dir — `~/.athena` by default, or `$ATHENA_HOME`
  / the active profile's dir. Skins live in `<athena-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `athena config set`).
...
3. `write_file` it to `<athena-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `athena config set display.skin default` (or they can `/skin default`).
...
```
athena skin set <key> <hex>      # e.g. athena skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `athena skin set background "#08201f"` changes
only the background; `athena skin use <name>` / `athena skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.athena`** when a profile is active — resolve the real home
  from `$ATHENA_HOME` first, falling back to `~/.athena`.
...
- **Never hand-edit `config.yaml` to activate.** Use `athena config set
...
- **You apply it, not the user.** `athena config set display.skin <name>` is
...
- `read_file` the written `<athena-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `athena config get display.skin` and confirm it reports `<name>`.
