...
`athena config edit` writes correctly.

### `execute_code` / Sandbox

**WinError 10106** from the sandbox child process — it can't create an
`AF_INET` socket. Root cause is usually Athena's env scrubber dropping
...
Athena-installed `venv/Scripts/` has no pip/pytest (stripped for size).
