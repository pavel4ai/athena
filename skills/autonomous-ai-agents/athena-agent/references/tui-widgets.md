...
Author widget apps for the Athena TUI (`athena --tui`): glanceable ambient
...
- The TUI must be in use (`athena --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.athena/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.athena/logs/tui_gateway_crash.log` if a widget never appears.
