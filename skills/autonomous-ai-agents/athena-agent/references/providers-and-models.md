...
Set via `athena model` (picker) or `athena setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://athena-agent.nousresearch.com/docs/integrations/providers
...
| nous | OAuth device code | `athena auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `athena auth add openai-codex` |
| qwen-oauth | OAuth | `athena auth add qwen-oauth` |
| minimax-oauth | OAuth | `athena auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`athena auth`).
Fallback chain when the primary fails: `athena fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`athena_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   athena config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
