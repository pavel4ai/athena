...
`_ATHENA_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_athena_home()` for paths,
never hardcode `~/.athena`. For custom/local-only tools, write a plugin in
`~/.athena/plugins/` instead of editing core — see the developer docs.
