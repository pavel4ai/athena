...
Full reference: https://athena-agent.nousresearch.com/docs/user-guide/configuration
...
`athena config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `athena tools` (interactive) or `athena tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_ATHENA_CORE_TOOLS` is the default bundle most platforms inherit).
