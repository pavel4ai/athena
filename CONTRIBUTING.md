...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo pavel4ai/athena "<your terms>"
  gh search prs --repo pavel4ai/athena --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/pavel4ai/athena/issues?q=) · [PRs (all states)](https://github.com/pavel4ai/athena/pulls?q=is%3Apr).
...
Related: #38284 covers the agent-side analog — Athena itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `athena skills browse` (labeled "official") and install it with `athena skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `athena skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.athena/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(athena_home, config)` for setup-wizard integration
...
- Integrate with `athena memory setup` via `post_setup()` — no need to touch core code
...
The reason is maintenance load, not quality. Every external product absorbed into the core tree becomes ours to keep working against a fast-moving codebase, for a backend we don't own and can't control. Athena ships a lot and the core moves quickly; coupling third-party products into it creates an open-ended burden on the maintainers.

Publish these as a **standalone plugin repo** instead:

- Implement the relevant ABC and use the existing plugin discovery path (`~/.athena/plugins/`, project `.athena/plugins/`, or a pip entry point) — see [Build a Athena Plugin](https://athena-agent.nousresearch.com/docs/guides/build-a-athena-plugin)
...
echo "OPENROUTER_API_KEY=***" >> ~/.athena/.env
```
...
├── cli.py                    # AthenaCLI class — interactive CLI orchestrator (~4.6k LOC + athena_cli/cli_*_mixin.py)
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (athena-cli, athena-telegram, etc.)
├── athena_state.py           # SessionDB facade (~1.4k LOC); implementation in athena_state_*.py (21 siblings) — FTS5 search, session titles
...
│
├── athena_cli/               # CLI command implementations
│   ├── main.py                   # Entry point, argument parsing, command dispatch
│   ├── cli_*_mixin.py            # AthenaCLI mixins (slash commands, display, session, ...)
...
│
├── skills/                   # Bundled skills (copied to ~/.athena/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (athena-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.athena/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.athena/`)

| Path | Purpose |
|------|---------|
| `~/.athena/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.athena/.env` | API keys and secrets |
| `~/.athena/auth.json` | OAuth credentials (Nous Portal) |
| `~/.athena/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.athena/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.athena/state.db` | SQLite session database |
| `~/.athena/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and explicit `/save` exports. Automatic per-session JSON snapshots are no longer written; state.db is canonical. |
| `~/.athena/cron/` | Scheduled job data |
| `~/.athena/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`athena_state.py`) with full-text search and unique session titles. Automatic per-session JSON snapshots have been removed. Existing files are left untouched; use `/save json` or `athena sessions export` for an explicit export.
...
(for example `_ATHENA_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.athena`:
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `athena setup` or update `~/.athena/.env` locally.
...
- **Test it.** Run `athena --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.
...
Add to `_BUILTIN_SKINS` dict in `athena_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `athena_cli/skin_engine.py` for the full schema and existing skins as examples.
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the athena wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `athena_cli/gateway.py::_scan_gateway_pids` for the pattern.
   ```
...
9. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `athena_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
15. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `athena_cli/gateway_windows.py::
