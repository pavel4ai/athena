...
If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Futurebound Corp. Discord](https://futurebnd.com). Users can install it with `athena skills install`.
...
├── cli.py                    # AthenaCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (athena-cli, athena-telegram, etc.)
├── athena_state.py           # SQLite session database with FTS5 full-text search, session titles
...
├── tests/                    # Test suite
├── website/                  # Documentation site (futurebnd.com)
│
...
| `~/.athena/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
...
- **Session persistence**: All conversations are stored in SQLite (`athena_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.athena/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
- **Discord**: [discord.gg/FutureboundCorp](https://futurebnd.com) — for questions, showcasing projects, and sharing skills
