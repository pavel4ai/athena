...
an unlinked SQLite DB/WAL/SHM generation.  ``athena_state`` supplies only the
...
_ATHENA_EXECUTABLES = frozenset({"athena", "athena-agent", "athena-acp"})
_ATHENA_PYTHON_MODULES = frozenset({"acp_adapter", "athena_cli.main"})
_ATHENA_PYTHON_SCRIPTS = frozenset({"athena_cli/main.py", "run_agent.py"})
...
def _looks_like_athena(argv: Sequence[str]) -> bool:
    """Return whether argv identifies a supported Athena execution target."""
    if not argv:
        return False
    program = os.path.basename(argv[0]).lower().removesuffix(".exe")
    if program in _ATHENA_EXECUTABLES:
        return True
    ...
    if kind == "module":
        return normalized in _ATHENA_PYTHON_MODULES
    return any(
        normalized == script or normalized.endswith(f"/{script}")
        for script in _ATHENA_PYTHON_SCRIPTS
    )
...
def _argv_scoped_to_other_home(argv: Sequence[str], db_path: Path) -> bool:
    ...
    ``state.db`` lives at the ATHENA_HOME root, so an absolute-path token
    containing a ``/.athena`` segment (or naming a ``state.db``/WAL/SHM under
    some other parent) identifies that token's own Athena home.  When at least
    ...
    of ours (issue #92401: a second gateway under /home/demo/.athena deferred
    ...
    for token in argv:
        ...
        if path_token is not None:
            ...
            if normalized in ours or normalized.startswith(this_home + os.sep):
                return False
            if "/.athena" in normalized or normalized.endswith("/.athena"):
