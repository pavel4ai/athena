...
``~/.athena/logs/`` (profile-aware via ``get_athena_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``athena_cli.web_server``,
    ``athena_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Athena (TUI, gateway, ``hy_memory`` server, MCP
...
from athena_constants import get_config_path, get_athena_home
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_athena_verbose", False):
                return
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the athena group. However, both ``_open()`` (initial creation)
