...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ATHENA_HOME."""
    try:
        from athena_constants import get_athena_home
        return get_athena_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ATHENA_HOME", Path.home() / ".athena")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from athena_constants import get_athena_home
        directory_file = get_athena_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("ATHENA_HOME", Path.home() / ".athena")
...
class EventBridge:
    ...
    def _poll_once(self, db):
        ...
        try:
            from athena_constants import get_athena_home
            db_file = get_athena_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("ATHENA_HOME", Path.home() / ".athena")) / "state.db"
