"""``athena kanban boards …`` — board directories, the ``current`` pointer and ``board.json``.
...
from athena_cli.kanban_output import _err, _fmt_counts, _json_out


def _dispatch_boards(args: argparse.Namespace) -> int:
    """``athena kanban boards <action>`` — filesystem-only, so it works before ``kanban init``."""
...
def _cmd_boards_export(args: argparse.Namespace) -> int:
    ...
    print(f"Exported board {res['board']!r} → {res['archive']}\n"
          ...
          "Import it with `athena kanban boards import <archive>`.")
    return 0
...
def _cmd_boards_import(args: argparse.Namespace) -> int:
    ...
    if res["activated"]:
        ...
        print(f"  Switch to it with `athena kanban boards switch {res['board']}`.")
    return 0
