...
TIPS = [
    ...
    "athena chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    ...
    "/plan writes a markdown implementation plan to .athena/plans/ without executing anything.",
    # --- Cron & Scheduling ---
    ...
    "execute_code helpers require explicit imports: from athena_tools import json_parse, shell_quote, retry.",
    ...
    'athena auth reset <provider> [target] clears cooldowns and exhaustion flags on a whole credential pool, or on one credential by index, id, or label.',
    ...
    'ATHENA_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs concise/all/error/off).',
    'ATHENA_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    ...
    'athena import restores a full Athena backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
...
# Task-oriented example prompts for the empty composer. Kept generic — Athena is
