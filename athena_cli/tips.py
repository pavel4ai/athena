...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or athena -c.",
    ...
    # --- CLI Flags ---
    "athena -c resumes your most recent CLI session. athena -c \"project name\" resumes by title.",
    "athena -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "athena -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "athena chat -t web,terminal enables only specific toolsets for a focused session.",
    "athena chat -s github-pr-workflow preloads a skill at launch.",
    "athena chat -q \"query\" runs a single non-interactive query and exits.",
    "athena chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "athena chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "athena --yolo bypasses all dangerous command approval prompts for the entire session.",
    "athena chat --source telegram tags the session for filtering in athena sessions list.",
    "athena -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "athena doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "athena dump outputs a compact setup summary — great for bug reports.",
    "athena config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "athena config edit opens config.yaml in your default editor.",
    "athena config check scans for missing or stale configuration options.",
    "athena sessions browse opens an interactive session picker with search.",
    "athena sessions stats shows session counts by platform and database size.",
    "athena sessions prune --older-than 30 cleans up old sessions.",
    "athena skills search react --source skills-sh searches the skills.sh public directory.",
    "athena skills check scans installed hub skills for upstream updates.",
    "athena skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "athena skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "athena mcp add github --command npx adds MCP servers from the command line.",
    "athena mcp serve runs Athena itself as an MCP server for other agents.",
    "athena auth add lets you add multiple API keys for credential pool rotation.",
    "athena completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "athena logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "athena backup creates a zip backup of your entire Athena home directory.",
    "athena profile create coder creates an isolated profile that becomes its own command.",
    "athena profile create work --clone copies your current config and keys to a new profile.",
    "athena update syncs new bundled skills to ALL profiles automatically.",
    "athena gateway install sets up Athena as a system service (systemd/launchd).",
    "athena memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "athena webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: athena tools disables unused tools, athena skills config trims skills down.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as athena -w).",
    ...
    "ATHENA_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    "Profile names become shell commands — 'athena profile create coder' creates the 'coder' command.",
    "athena profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "athena sessions export backup.jsonl exports all sessions for backup or analysis.",
    "athena -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "The plan skill saves markdown plans under .athena/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: athena cron add --skill blogwatcher \"Check for new posts\".",
    ...
    "athena gateway install sets it up as a system service that starts on boot.",
    ...
    "Context files (.athena.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Athena loads project context from .athena.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "athena mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "athena mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "athena mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "Checkpoints use shadow repos in ~/.athena/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "athena chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "athena plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "athena acp runs Athena as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "ATHENA_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "athena auth add nous or athena auth add openai-codex sets up OAuth-based providers.",
    ...
    "athena status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.athena/scripts/ and run before the agent — perfect for data collection pipelines.",
    ...
    "The skills audit log at ~/.athena/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope Athena state via ATHENA_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "ATHENA_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in ATHENA_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.athena/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'athena profile create ls' would be rejected.",
    "athena profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "athena claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "ATHENA_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via athena tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.athena/.env.",
    "Skills from trusted repos (FutureboundCorp) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.athena/skills/.hub/quarantine/ holds skills pending security review.",
    ...
    '/reload reloads ~/.athena/.env into the running session — pick up new API keys without restarting.',
    ...
    "ATHENA_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.athena/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.athena/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',
    ...
    # --- TUI & Dashboard ---
    'ATHENA_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "ATHENA_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'athena dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'athena dashboard embeds the full Athena TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.athena/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.athena/dashboard-plugins/ — no npm build required.',
    ...
    'ATHENA_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'ATHENA_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'ATHENA_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'ATHENA_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.athena/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.athena/skills/ so drop-in skills appear without restarting the session.',
    ...
    'athena -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'athena chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'athena chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'athena chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "athena chat --source tool tags programmatic chats so they don't clutter athena sessions list.",
    'athena dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'athena sessions rename <ID> "new title" renames any past session; athena sessions delete <ID> removes one.',
    'athena import restores a session export or profile archive produced by sessions export or profile export.',
    'athena fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'athena pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'athena setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'athena status --deep runs the full health sweep across every component; plain athena status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'ATHENA_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'ATHENA_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.athena/plugins/ — trust-gated by design.',
    "ATHENA_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'ATHENA_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'ATHENA_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'ATHENA_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'ATHENA_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'ATHENA_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'ATHENA_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'ATHENA_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'ATHENA_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'ATHENA_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'ATHENA_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    '--source tool chats are excluded from athena sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.athena/dashboard-plugins/.',
]
