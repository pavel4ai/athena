...
The provider list shown by ``athena model`` (CLI/TUI) and the desktop Settings → Providers tabs
...
come from :data:`athena_cli.auth.PROVIDER_REGISTRY` (credential truth); ``display_name`` /
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `athena model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """One descriptor per provider in the ``athena model`` universe (:data:`CANONICAL_PROVIDERS`,
    ...
    PROVIDER_REGISTRY = _safe_import("athena_cli.auth", "PROVIDER_REGISTRY", {})
    OPTIONAL_ENV_VARS = _safe_import("athena_cli.config", "OPTIONAL_ENV_VARS", {})
    ...
    ATHENA_OVERLAYS = _safe_import("athena_cli.providers", "ATHENA_OVERLAYS", {})
    try:
