...
The provider list shown by ``athena model`` (CLI/TUI) and the desktop Settings
→ Providers tabs (Accounts + API keys) **must be the same set**.  Historically
they were not: the CLI picker read :data:`athena_cli.models.CANONICAL_PROVIDERS`
...
the same universe ``athena model`` renders (``CANONICAL_PROVIDERS``), joining:

* ``auth_type`` / ``api_key_env_vars`` / ``base_url_env_var`` from
  :data:`athena_cli.auth.PROVIDER_REGISTRY` (credential truth), and
...
``CANONICAL_PROVIDERS`` universe, i.e. exactly what ``athena model`` shows.
"""
...
# athena_cli.auth.PROVIDER_REGISTRY and providers.base.ProviderProfile.
...
def provider_catalog() -> list[ProviderDescriptor]:
    """Return one descriptor per provider in the ``athena model`` universe.
