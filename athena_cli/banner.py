...
# (e.g. nix-built athena — no local git history to count against).
...
def check_for_updates() -> Optional[int]:
    ...
    Two paths: if ``ATHENA_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = athena_home / ".update_check"
    embedded_rev = os.environ.get("ATHENA_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # ATHENA_REVISION (that's nix-only). Without this guard the checks below
    ...
    # container anyway. The dashboard's REST `/api/athena/update/check`
    ...
    if embedded_rev:
        ...
        # $ATHENA_HOME/athena-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = athena_home / "athena-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    ...
    because ``$ATHENA_HOME/athena-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``athena_cli/build_info.py``) and return it as a frozen
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    canonical pavel4ai/athena repo (forks don't get a link).
    """
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    """Build and print a welcome banner with Athena profile art on left and info on right.
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else ATHENA_CADUCEUS
    except Exception:
        _bskin = None
        _hero = ATHENA_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Futurebound Corp.[/]")
    ...
    try:
        ...
        if behind is not None and behind != 0:
            ...
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built athena; we know an update
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else ATHENA_AGENT_LOGO
        console.print(_logo)
        console.print()
