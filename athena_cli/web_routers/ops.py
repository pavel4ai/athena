...
from athena_cli.config import redact_key
from athena_cli.web_deps import late
from athena_cli.web_server_files import _path_is_under
from athena_cli.web_server_gateway import _restart_gateway_after
from athena_cli.web_server_memory import _normalize_memory_provider_name, _require_memory_provider_ready
...
from athena_cli.web_routers._common import _CONFIG_MUTATION_LOCK, http_failure, spawn_profile_action
from athena_cli.web_routers.files import stream_upload_to_path

_log = logging.getLogger("athena_cli.web_server")
router = APIRouter()

# Late-bound so a test's monkeypatch on the owning module wins at call time.
_discover_memory_provider_statuses = late("_discover_memory_provider_statuses", "athena_cli.web_server_memory")
...
save_config = late("save_config", "athena_cli.config")


def _spawn_action(argv: List[str], name: str, *, log_msg: str, prefix: str) -> dict:
    """Spawn a dashboard-profile ``athena <argv>`` action; spawn failure -> 500."""
...
# --- Webhooks: same JSON store as the CLI (athena_cli.webhook); the adapter
...
# --- Gateway lifecycle: spawn the real `athena gateway <verb>` so behaviour
    ...
    def _run():
        try:
            ...
            pool.add_entry(PooledCredential(
                ...
                # every added account into the latest login: a second ``athena auth add openai-codex``
            ...
            # (mirrors `athena auth add`).
            if not provider.startswith(CUSTOM_POOL_PREFIX):
                try:
                    from athena_cli.auth import _load_auth_store, unsuppress_credential_source
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    the same RemovalStep registry as ``athena auth remove``: each source cleans
...
def _dashboard_backup_dir() -> Path:
    return get_athena_home() / "backups"
...
async def run_backup(body: BackupRequest):
    ...
    if not output:
        ...
        archive = _dashboard_backup_dir() / f"athena-backup-{stamp}-{secrets.token_hex(4)}.zip"
        try:
...
async def create_hook(body: HookCreate):
    ...
    valid_hooks = None
    with contextlib.suppress(Exception):
        from athena_cli.plugins import VALID_HOOKS as valid_hooks
