...
from athena_cli.web_server_oauth import (
...
from athena_cli.web_models import OAuthSubmitBody
...
_OAUTH_PROVIDER_CATALOG = LateState("_OAUTH_PROVIDER_CATALOG", "athena_cli.web_server_oauth")
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Athena "
...
def _codex_request_user_code(httpx) -> Dict[str, Any]:
    ...
    from athena_cli.auth import CODEX_OAUTH_CLIENT_ID
...
def _codex_exchange_tokens(httpx, code_resp: Dict[str, Any]) -> Dict[str, str]:
    ...
    from athena_cli.auth import CODEX_OAUTH_CLIENT_ID, CODEX_OAUTH_TOKEN_URL
...
async def _start_nous_device_code(profile: Optional[str]) -> Dict[str, Any]:
    ...
    from athena_cli.auth import PROVIDER_REGISTRY, _request_device_code
    from athena_cli.web_server_profiles import _config_profile_scope, _profile_scope
    pconfig = PROVIDER_REGISTRY["nous"]
    portal_base_url = (
        os.getenv("ATHENA_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL") or pconfig.portal_base_url
    ).rstrip("/")
...
async def _start_xai_device_code(profile: Optional[str]) -> Dict[str, Any]:
    from athena_cli.auth import _xai_oauth_request_device_code
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    # "anthropic" is flow == "external" (no in-dashboard login) but Athena still
    # OWNS its credential (the PKCE file ~/.athena/.anthropic_oauth.json and its
    # credential-pool entry, written by `athena auth add anthropic`), so it is
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    in ``athena model`` order, so plugin-added OAuth/external providers appear automatically."""
    ...
    try:
        ...
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "cli_command": f"athena auth add {d.slug}", "docs_url": d.signup_url or "", "status_fn": None,
            })
    except Exception:
        pass
    return rows
...
def _clear_anthropic_auth() -> bool:
    """Clear only the Athena-managed PKCE file and auth-store entry (never ~/.claude/*)."""
    cleared = False
    try:
        from agent.anthropic_credentials import _get_athena_oauth_file
        oauth_file = _get_athena_oauth_file()
