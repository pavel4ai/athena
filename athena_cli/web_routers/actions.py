"""Gateway restart/drain, Athena update and background-action status dashboard routes.

Extracted from ``athena_cli.web_server``; helpers/state that tests monkeypatch on
...
from athena_cli.config import format_docker_update_message, recommended_update_command_for_method
from athena_cli.web_deps import LateState, late
from athena_cli.web_server_gateway import _ACTION_LOG_FILES
from athena_cli.web_routers._common import http_failure
...
detect_install_method = late("detect_install_method", "athena_cli.config")
get_athena_home = late("get_athena_home", "athena_cli.config")
_ACTION_COMMANDS = LateState("_ACTION_COMMANDS", "athena_cli.web_server_gateway")
_ACTION_IDS = LateState("_ACTION_IDS", "athena_cli.web_server_gateway")
_ACTION_PROCS = LateState("_ACTION_PROCS", "athena_cli.web_server_gateway")
_ACTION_RESULTS = LateState("_ACTION_RESULTS", "athena_cli.web_server_gateway")
...
def _server_path(name: str) -> Path:
    ...
    import athena_cli.web_server_gateway as gw
...
_UPDATE_ACTION_COMPLETED_RE = re.compile(r"^=== athena-update completed ([0-9a-f]{32}) ===$")

_MANAGED_EXTERNALLY_MESSAGE = "Athena updates are managed outside this dashboard in containerized environments."
...
def _durable_completed_update_action_id(lines: List[str]) -> Optional[str]:
    ...
    for index, line in enumerate(lines):
        if line.startswith("=== athena update started "):
            last_start = index
...
def _update_refused(error: str, message: str, update_command: str) -> Dict[str, Any]:
    ...
    return {
        "ok": False, "pid": None, "name": "athena-update", "error": error, "message": message,
        "update_command": update_command,
    }


@router.post("/api/athena/update")
async def update_athena():
    ...
    existing = _ACTION_PROCS.get("athena-update")
    if existing is not None and existing.poll() is None:
        response = {"ok": True, "pid": existing.pid, "name": "athena-update", "already_running": True}
        action_id = _ACTION_IDS.get("athena-update")
        if action_id:
            response["action_id"] = action_id
        return response

    action_id = secrets.token_hex(16)
    with http_failure("Failed to spawn athena update", 500, "Failed to start update"):
        proc = _spawn_athena_action(["update"], "athena-update", env_overrides={"ATHENA_ACTION_ID": action_id})
    return {"ok": True, "pid": proc.pid, "name": "athena-update", "action_id": action_id}


_NON_APPLYABLE_MESSAGES = {
    "docker": format_docker_update_message,
    "apt": lambda: "Athena is managed by Termux APT; run `pkg upgrade athena-agent`.",
}


@router.get("/api/athena/update/check")
async def check_athena_update(force: bool = False):
    ...
    try:
        from athena_cli.banner import check_for_updates, upstream_commits_behind
...
async def get_action_status(name: str, lines: int = 200):
    ...
    if name == "athena-update":
        ...
        if durable_update_action_id:
            marker = f"=== athena-update completed {durable_update_action_id} ==="
...
def _latest_update_receipt_summary() -> Optional[Dict[str, Any]]:
    """Compact summary of the latest receipt (written by EVERY ``athena update`` run,
    incl. refused/failed), or None; never raises. Steps/skips stay in the full endpoint.

    Phase-1 bullet 3 (#91277): the receipt (written by EVERY ``athena update`` run since #91283, including
...
@status_router.get("/api/athena/update/receipt")
async def get_update_receipt():
    ...
    receipt = _read_latest_receipt()
    if not receipt:
        raise HTTPException(status_code=404, detail="No update receipt found (no `athena update` run recorded).")
