...
Approvals = auto-bypassed (ATHENA_YOLO_MODE=1 is set for the call).
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to ATHENA_INFERENCE_MODEL
...
def _run_agent(
    ...
    # Imports are local so they don't run when athena is invoked for
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("ATHENA_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    agent = AIAgent(
        ...
        #                ATHENA_INTERACTIVE which we never set
        #   - shell-hook approval → auto-approved via ATHENA_ACCEPT_HOOKS=1
        #                (set above); also falls back to deny on non-tty
        #   - dangerous-command approval → bypassed via ATHENA_YOLO_MODE=1
