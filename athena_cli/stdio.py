...
Entry points (``cli.py`` ``main``, ``athena_cli/main.py`` CLI dispatch,
...
def configure_windows_stdio() -> bool:
    ...
    Set ``ATHENA_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("ATHENA_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # already-running shells don't see the change — which means athena
...
def _augment_path_with_known_tools() -> None:
    ...
    ``%LOCALAPPDATA%\\athena\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    ...
    candidate_dirs = [
        ...
        # Athena venv Scripts directory — host of the athena.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes athena, but harmless to include.
