"""``athena doctor`` — diagnose (and with --fix, repair) a Athena install.
...
from athena_cli.config import get_env_path, get_athena_home, get_project_root
...
from athena_cli.doctor_report import Finding, _section, check_bool, check_info, doctor_check, warn_on_error
from athena_cli.doctor_connectivity import _has_healthy_oauth_fallback_for_apikey_provider, build_probes, run_probes
from athena_cli.doctor_tools import _safe_which

from athena_cli.doctor_config import (
    _check_config_drift,
    _check_config_file,
    _check_env_file,
    _check_mcp_security,
...
from athena_cli.doctor_platform import (
    _check_certificates,
...
from athena_cli.doctor_tools import (
    _check_git_and_rg,
...
from athena_cli.doctor_state import (
...
def _ack_advisory(ack_target: str) -> None:
    """`athena doctor --ack <id>`: persist the ack and return without running diagnostics."""
    from athena_cli.security_advisories import ADVISORIES, ack_advisory
    ...
    if ack_advisory(ack_target):
        ...
        print(color(f"  ✗ Failed to persist ack for {ack_target}. Check ~/.athena/config.yaml is writable.", Colors.RED))
        sys.exit(1)
...
def run_doctor(args):
    ...
    with warn_on_error(""):
        from athena_cli.doctor_live import maybe_run_live_checks
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'OPENROUTER_MODELS_URL': ('athena_constants', 'OPENROUTER_MODELS_URL'),
    'STATE_DB_SIZE_WARN_BYTES': ('athena_cli.doctor_state', 'STATE_DB_SIZE_WARN_BYTES'),
    'agent_browser_runnable': ('athena_constants', 'agent_browser_runnable'),
    'base_url_host_matches': ('utils', 'base_url_host_matches'),
    'check_certificates': ('athena_cli.doctor_platform', 'check_certificates'),
    'check_macos_full_disk_access': ('athena_cli.doctor_platform', 'check_macos_full_disk_access'),
    'check_macos_tcc_anchor': ('athena_cli.doctor_platform', 'check_macos_tcc_anchor'),
    'check_macos_tcc_grants': ('athena_cli.doctor_platform', 'check_macos_tcc_grants'),
    'collect_deprecated_config_keys': ('athena_cli.doctor_config', 'collect_deprecated_config_keys'),
    'collect_deprecated_env_vars': ('athena_cli.doctor_config', 'collect_deprecated_env_vars'),
    'collect_relay_plugin_cutover_findings': ('athena_cli.doctor_config', 'collect_relay_plugin_cutover_findings'),
    'describe_vercel_auth': ('athena_cli.vercel_auth', 'describe_vercel_auth'),
    ...
    'managed_scope_check': ('athena_cli.doctor_config', 'managed_scope_check'),
    'recommended_update_command_for_method': ('athena_cli.config', 'recommended_update_command_for_method'),
    'report_deprecated_config_and_env': ('athena_cli.doctor_config', 'report_deprecated_config_and_env'),
}
