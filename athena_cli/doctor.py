...
from athena_cli.config import get_project_root, get_athena_home, get_env_path
...
_DHH = display_athena_home()  # user-facing display path (e.g. ~/.athena or ~/.athena/profiles/coder)

# Load environment variables from ~/.athena/.env so API key checks work
_env_path = get_env_path()
load_athena_dotenv(athena_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from athena_cli.colors import Colors, color
from athena_cli.models import _ATHENA_USER_AGENT
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.athena/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("ATHENA_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from athena_cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches athena_cli.__version__.
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != athena_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'athena update', or set "
            "athena_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-athena and dashboard static services are up
    ...
    for static in ("main-athena", "dashboard"):
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from athena_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the ATHENA_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("ATHENA_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via ATHENA_MANAGED_DIR={managed_dir}")
...
def run_doctor(args):
    ...
    os.environ.setdefault("ATHENA_INTERACTIVE", "1")
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.athena/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    # Detect drift between pyproject.toml and athena_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.athena/.env (primary location for user config)
    env_path = ATHENA_HOME / '.env'
    if env_path.exists():
    ...
    # Check ~/.athena/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from athena_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from athena_cli.providers import (
                ...
                try:
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.athena/.env or run 'athena setup')",
                            (
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from athena_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    ...
                    issues.append("Run 'athena doctor --fix' or 'athena setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'athena doctor --fix'")
        except Exception:
            pass

        # Detect stale ATHENA_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives ATHENA_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            import yaml
            from athena_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
            ...
            if drift:
                check_warn(
                    f"ATHENA_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `athena setup` run)",
                )
                if should_fix:
                    if remove_env_value("ATHENA_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale ATHENA_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove ATHENA_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the ATHENA_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale ATHENA_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'athena doctor --fix'"
                    )
        except Exception:
            pass
    ...
    for subdir_name in expected_subdirs:
        subdir_path = athena_home / subdir_name
    ...
    memories_dir = athena_home / "memories"
    ...
    state_db_path = athena_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from athena_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'athena doctor --fix' "
                        "(or 'athena sessions repair') to recover hidden sessions"
                    )
            else:
    ...
    wal_path = athena_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'athena doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        if _venv_bin is None:
            check_warn(
                ...
                "(athena not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/athena → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/athena points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/athena → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/athena — run 'athena doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/athena exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/athena not found",
                    "(athena command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/athena → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/athena symlink — run 'athena doctor --fix'")
    ...
    try:
        from athena_constants import is_container as _is_container
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable ATHENA_HOME mirror
    ...
    _section("Skills Hub")
    hub_dir = ATHENA_HOME / "skills" / ".hub"
    if hub_dir.exists():
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = ATHENA_HOME / "config.yaml"
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                _fail_and_issue(
                    ...
                    "No Honcho API key — run 'athena memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run athena memory setup)",
    ...
    try:
        from athena_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "athena -p" in content:
                            _m = _re.search(r"athena -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'athena doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
