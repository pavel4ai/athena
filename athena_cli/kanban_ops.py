"""Dispatcher and maintenance verbs for ``athena kanban``: ``dispatch``,
...
from athena_cli.kanban_output import _err, _fmt_ts, _print_json
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"successfully. Check profile health (venv, PATH, credentials) and `athena "
                    f"kanban list --status ready` / `athena kanban list --status blocked` for "
