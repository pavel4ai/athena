...
Mixed into :class:`athena_cli.plugins.PluginManager`. ``_resolve_hook_callback_timeout`` stays on
...
PLUGIN_SECTIONS_START = "<!-- athena-plugin-sections:start -->"
PLUGIN_SECTIONS_END = "<!-- athena-plugin-sections:end -->"
...
def format_system_prompt_section(section_id: str, content: str) -> str:
    ...
    return (
        ...
        f"<!-- athena-plugin-section-chars:{len(content)} -->\n\n{content}")
...
# Reserved event namespace prefix — only core may publish ``athena:<event>``.
ATHENA_EVENT_NAMESPACE = "athena"
...
class PluginDispatchMixin:
    ...
    def invoke_hook(self, hook_name: str, **kwargs: Any) -> List[Any]:
        ...
        """
        from athena_cli.plugins import _resolve_hook_callback_timeout
    ...
    def _run_hook_callback_bounded(
        ...
        thread = threading.Thread(target=_runner, name=f"athena-hook-{callback_name}"[:40], daemon=True)
        try:
            thread.start()
    ...
    def _ensure_event_worker_locked(self) -> None:
        ...
        worker = threading.Thread(
            target=self._event_worker_loop, args=(self._event_queue,), name="athena-plugin-events",
            daemon=True,
        )
    ...
    def _deliver_event(self, item: _QueuedPluginEvent) -> None:
        ...
        from athena_cli.plugins import resolve_plugin_command_result
        with self._event_lock:
