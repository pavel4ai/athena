...
Re-exported from ``athena_cli/auth.py`` (patch targets unchanged); origin helpers are imported
lazily per function so ``athena_cli.auth.<helper>`` patches still intercept and no cycle forms.
"""
...
def _spotify_client_id(explicit: Optional[str] = None, state: Optional[Dict[str, Any]] = None) -> str:
    client_id = _spotify_setting(
        state, "client_id", ("ATHENA_SPOTIFY_CLIENT_ID", "SPOTIFY_CLIENT_ID"), "", explicit=explicit,
    )
    if client_id:
        return client_id
    raise _spotify_err(
...
def _spotify_redirect_uri(explicit: Optional[str] = None, state: Optional[Dict[str, Any]] = None) -> str:
    return _spotify_setting(
        state, "redirect_uri", ("ATHENA_SPOTIFY_REDIRECT_URI", "SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    return _spotify_setting(
        state, "api_base_url", ("ATHENA_SPOTIFY_API_BASE_URL",), DEFAULT_SPOTIFY_API_BASE_URL, strip_slash=True,
    )


def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    return _spotify_setting(
        state, "accounts_base_url", ("ATHENA_SPOTIFY_ACCOUNTS_BASE_URL",), DEFAULT_SPOTIFY_ACCOUNTS_BASE_URL,
        strip_slash=True,
    )
...
def _refresh_spotify_oauth_state(state: Dict[str, Any], *, timeout_seconds: float = 20.0) -> Dict[str, Any]:
    ...
    payload = _spotify_token_post(
        ...
        failed_suffix=" Run `athena auth spotify` again.", relogin_required=True,
    )
...
def resolve_spotify_runtime_credentials(
    *, force_refresh: bool = False, refresh_if_expiring: bool = True,
    ...
    from athena_cli.auth import _auth_store_lock, _is_expiring, _load_auth_store, _load_provider_state, _quarantine_flat_oauth_state, _refresh_spotify_oauth_state, _save_auth_store, _store_provider_state
    with _auth_store_lock():
        auth_store = _load_auth_store()
        state = _load_provider_state(auth_store, "spotify")
        if not state:
            raise _spotify_err(
                "Spotify is not authenticated. Run `athena auth spotify` first.", "spotify_auth_missing", relogin=True,
            )
...
def get_spotify_auth_status() -> Dict[str, Any]:
    from athena_cli.auth import _is_expiring, get_provider_auth_state
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    """Walk the user through creating a Spotify developer app; persist the client_id to ~/.athena/.env."""
    from athena_cli.auth import _is_remote_session
    ...
    print("\nSaved ATHENA_SPOTIFY_CLIENT_ID to ~/.athena/.env\n")
    return raw


def login_spotify_command(args) -> None:
    from athena_cli.auth import _auth_store_lock, _can_open_graphical_browser, _is_remote_session, _load_auth_store, _print_loopback_ssh_hint, _save_auth_store, _store_provider_state, get_provider_auth_state
    existing_state = get_provider_auth_state("spotify") or {}

    # No client_id anywhere -> wizard instead of "ATHENA_SPOTIFY_CLIENT_ID is required".
    try:
    ...
    print(
        ...
        f"Open this URL to authorize Athena:\n{authorize_url}\n\nFull setup guide: {SPOTIFY_DOCS_URL}\n"
    )
