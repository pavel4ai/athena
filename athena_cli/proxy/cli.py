...
from athena_cli.proxy.server import (
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not AIOHTTP_AVAILABLE:
        _err("athena proxy requires aiohttp. Run `athena setup` to install it.")
        return 1
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"athena auth add {adapter.name}")
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("\nStart the proxy with: athena proxy start [--provider <name>]")
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``athena proxy <subcommand>``; no/unknown subcommand prints the short help."""
    handler = _SUBCOMMANDS.get(getattr(args, "proxy_command", None))
    if handler is not None:
        return handler(args)
    _err(
        "athena proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  athena proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
