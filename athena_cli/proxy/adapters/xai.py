...
class XAIGrokAdapter(UpstreamAdapter):
    ...
    auth_hint = "athena auth add xai-oauth --type oauth"
    ...
    def get_credential(self) -> UpstreamCredential:
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    "No xAI OAuth credentials found. Run `athena auth add xai-oauth --type oauth` first."
                )
            entry = pool.select()
            if entry is None:
                raise RuntimeError(
                    "No available xAI OAuth credentials found. Run "
                    "`athena auth reset xai-oauth` or re-authenticate with `athena auth add xai-oauth --type oauth`."
                )
            self._pool = pool
