...
Saves per-platform tool configuration to ~/.athena/config.yaml under
...
from athena_cli.nous_account import format_nous_portal_entitlement_message
...
from athena_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _ATHENA_CORE_TOOLS (available at runtime if enabled),
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set ATHENA_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "ATHENA_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "ATHENA_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "ATHENA_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "ATHENA_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "ATHENA_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("ATHENA_CUA_DRIVER_CMD", "").strip() or "cua-driver"


def _cua_driver_env() -> dict:
    """cua-driver child env with the Athena telemetry policy applied.
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `athena update` calls this
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_athena_home()}/athena-agent && npm install --workspaces=false")
                if result.stderr:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.athena/config.yaml")
        ...
        # to ~/.athena/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from athena_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: athena auth spotify")
            return
        ...
        try:
            ...
            _print_info("    Run later: athena auth spotify")
        except Exception as exc:
        ...
        try:
            from athena_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: athena plugins enable observability/langfuse")
        _print_info("    Restart Athena for tracing to take effect.")
        _print_info("    Verify: athena plugins list")
        ...
        try:
            ...
            _print_info("    Run later: athena auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: athena auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: athena auth add xai-oauth"
                )
        else:
...
def _get_platform_tools(
    ...
    *,
    ...
    # "athena-cli" (which include all _ATHENA_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[athena-cli, spotify]`` after enabling Spotify via ``athena
        ...
        # (e.g. "athena-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
    ...
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"athena-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("athena-")}
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (athena-cli, athena-telegram, etc.)
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    print(color("  Guide: https://github.com/pavel4ai/athena/docs/user-guide/features/tools", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, athena mcp
