...
from athena_cli.colors import Colors
from athena_cli.cli_output import prompt_yes_no
from athena_cli.curses_ui import curses_single_select
from athena_cli.mcp_catalog import (
...
def _configure_tools(name: str) -> None:
    ...
    from athena_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    """Plain-text catalog dump: `athena mcp catalog` output and the non-curses fallback."""
    print()
    if not rows:
    ...
    print()
    _say("  Install: athena mcp install <name>    Picker: athena mcp", Colors.DIM)
    # Manifest-version warnings: the user's Athena is too old to install everything listed.
    future = [d for d in catalog_diagnostics() if d[1] == "future_manifest"]
    if future:
        print()
        for name, _, _msg in future:
            _say(f"  ⚠ '{name}' requires a newer Athena — run `athena update` to install this entry.", Colors.YELLOW)
        print()
    print()
...
def run_picker() -> None:
    """`athena mcp picker` (and default `athena mcp`) — interactive selector; re-renders after each
    action until ESC/q."""
    while True:
        rows = _build_rows()
...
def install_by_name(identifier: str) -> int:
    ...
    if entry is None:
        _say(
            f"  ✗ '{identifier}' is not in the catalog. Run `athena mcp catalog` to see available entries.",
            Colors.RED,
        )
        return 1
