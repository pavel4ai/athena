...
write ``ATHENA_DASHBOARD_OAUTH_CLIENT_ID`` (+ portal/public URL when warranted) into ``.env``
...
def _print_post_register_hint(
    *, client_id: str, portal_base_url: str, custom_redirect_uri: Optional[str],
    ...
    print(f"\n  Wrote to {get_env_path()}:\n    ATHENA_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
    if wrote_portal_url:
...
def _public_url_from_redirect(redirect_uri: Optional[str]) -> str:
    ...
    ``/auth/callback`` to ATHENA_DASHBOARD_PUBLIC_URL, so the raw URI would double the path."""
    try:
...
def cmd_dashboard_register(args) -> None:
    ...
    if is_managed():
        print("✗ `athena dashboard register` is not available in a managed/hosted install.\n"
    ...
    try:
        ...
        if isinstance(exc, AuthError) and getattr(exc, "relogin_required", False):
            print("✗ You're not logged into Nous Portal.\n"
                  "  Run `athena setup` (or `athena auth add nous`) first, then retry.")
        else:
    ...
    portal_override = getattr(args, "portal_url", None) or os.environ.get("ATHENA_DASHBOARD_PORTAL_URL")
    ...
    stored = _env_value("ATHENA_DASHBOARD_OAUTH_CLIENT_ID")
    ...
    try:  # client_id is load-bearing: fatal on failure
        ...
        print(f"✗ Failed to write ATHENA_DASHBOARD_OAUTH_CLIENT_ID to .env: {exc}\n"
              f"  Set it manually:  ATHENA_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
        sys.exit(1)
    # Explicit portal → always persist (the user asked); inferred → only if unset AND non-default.
    existing_portal = _env_value("ATHENA_DASHBOARD_PORTAL_URL")
    should_write_portal = (
    ...
    wrote_portal_url = should_write_portal and _save_env_quietly("ATHENA_DASHBOARD_PORTAL_URL", portal_base_url)
    ...
    wrote_public_url = bool(
        public_url
        and _env_value("ATHENA_DASHBOARD_PUBLIC_URL") != public_url
        and _save_env_quietly("ATHENA_DASHBOARD_PUBLIC_URL", public_url))
