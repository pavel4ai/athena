...
Extracted from ``athena_cli/main.py`` as part of the god-file decomposition
campaign (``~/.athena/plans/god-file-decomposition.md``, Phase 2 — splitting
...
in main.py re-imports them (``from athena_cli.model_setup_flows import *``-style
explicit import) so existing call sites — and test monkeypatches that target
``athena_cli.main._model_flow_*`` — keep resolving against main.py's namespace.
...
imported lazily inside the flows (``from athena_cli.main import ...`` resolves at
call time, when main.py is fully loaded) so this module never imports
``athena_cli.main`` at import time -> no import cycle.
"""
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from athena_cli.main import _prompt_api_key
    ...
    # in-flow (K/R/C) instead of having to edit ~/.athena/.env by hand. The
    ...
    from athena_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from athena_cli.main import _DEFAULT_QWEN_PORTAL_MODELS
    from athena_cli.auth import (
...
def _model_flow_custom(config):
    ...
    """
    from athena_cli.main import _auto_provider_name, _prompt_custom_api_mode_selection, _save_custom_provider
    from athena_cli.auth import _save_model_choice, deactivate_provider
    from athena_cli.config import get_env_value, load_config, save_config
    ...
    from athena_cli.models import probe_api_models
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from athena_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from athena_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from athena_cli import azure_detect
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from athena_cli.main import _custom_provider_api_key_config_value, _custom_provider_base_url_config_value, _save_custom_provider
    from athena_cli.auth import _save_model_choice, deactivate_provider
    from athena_cli.config import load_config, save_config
    from athena_cli.models import fetch_api_models
...
def _model_flow_copilot(config, current_model=""):
    ...
    from athena_cli.main import _current_reasoning_effort, _prompt_reasoning_effort_selection, _set_reasoning_effort
    from athena_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from athena_cli.config import save_env_value, load_config, save_config
    from athena_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set ATHENA_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from athena_cli.main import _infer_stepfun_region, _prompt_api_key, _prompt_provider_choice, _stepfun_base_url_for_region
    from athena_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so athena knows where to find it
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from athena_cli.main import _run_anthropic_oauth_flow
    from athena_cli.auth import (
        _prompt_model_selection,
        _save_model_choice,
        deactivate_provider,
