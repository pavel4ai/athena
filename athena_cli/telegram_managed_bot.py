...
def auto_setup_telegram_bot_result(
    api_url: str | None = None, manager_bot: str = "AthenaSetupBot",
    ...
    print(f"\n  Contacting Athena Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``athena_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``athena_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"athena_{generate_username_slug()}_bot"
