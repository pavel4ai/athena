...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.athena_build_sha`` by the Dockerfile's
    ``ATHENA_GIT_SHA`` build-arg (see ``athena_cli/build_info.py``).
...
def run_dump(args):
    ...
    lines.append(f"athena_home:      {display_athena_home()}")
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(athena_home)}")
    lines.append(f"  skills:             {_count_skills(athena_home)}")
