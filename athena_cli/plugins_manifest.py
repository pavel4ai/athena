...
Split out of :mod:`athena_cli.plugins`; validation warns and never fails a load.
"""
...
from athena_cli.plugin_capabilities import parse_declared_capabilities as _parse_declared_capabilities
...
# and continue loading. ``capabilities``/``emits``/``listens``/``athena``/``depends`` are reserved.
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "license", "homepage", "tags", "capabilities", "emits", "listens", "athena", "depends",
    "requires_athena",
}

# Highest manifest schema version this Athena understands.
...
def _plugins_debug() -> bool:
    from athena_cli import plugins as _origin
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this Athena "
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `athena plugins enable %s` if it is installed.",
                    k, dep_id, dep_id,
                )
            elif resolved == k:
...
class PluginManifest:
    ...
    kind: str = "standalone"
    # Path-derived registry key used by plugins.enabled/disabled and `athena plugins list`: ``disk-cleanup``
    # for a flat plugin, ``image_gen/openai`` for a category plugin. Empty -> name.
    key: str = ""
    # Athena version requirement (``">=0.19"``, comma-separated clauses allowed). Unsatisfied plugins are
    # recorded with an error and skipped before import — see ``requires_athena_error``.
    requires_athena: str = ""
    portable: bool = False
    skill_namespace: str = ""
    ...
    # VALIDATED AND SURFACED ONLY — Athena never auto-installs these (isolation design for the install seam
...
# ── requires_athena version gate ─────────────────────────────────────────────
_VERSION_COMPARATOR_RE = re.compile(r"^\s*(>=|<=|==|!=|>|<)\s*(.+?)\s*$")


def running_athena_version() -> str:
    """Installed ``athena-agent`` distribution version, else ``athena_cli.__version__`` (source checkout)."""
    try:
        return importlib.metadata.version("athena-agent")
    except Exception:
...
def requires_athena_error(manifest: "PluginManifest") -> Optional[str]:
    """Load-blocking reason when the manifest's ``requires_athena`` rejects the running version."""
    if not manifest.requires_athena:
        return None
    current = running_athena_version()
    if version_satisfies(manifest.requires_athena, current):
        return None
    return f"requires athena {manifest.requires_athena}, running {current}"
...
def parse_manifest_file(
    ...
    try:
        ...
        return PluginManifest(
            ...
            kind=kind, key=key, requires_athena=str(data.get("requires_athena") or "").strip(),
