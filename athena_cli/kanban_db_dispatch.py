...
Split out of ``athena_cli.kanban_db``; origin-resident helpers are reached
...
if TYPE_CHECKING:
    from athena_cli.kanban_db import Task
...
class DispatchResult:
    ...
    Code terminal like ``orion-cc``), not a Athena profile. Expected steady-state
    ...
    if sweep.exited_hook_payloads and _kb._kanban_observer_consumed("on_kanban_worker_exited"):
        ...
        for hook_fields in sweep.exited_hook_payloads:
            ...
            _kb._fire_kanban_lifecycle_hook(
                ...
                # fire in the DISPATCHER process (gateway-embedded dispatcher or ``athena kanban
...
def _profile_exists_fn() -> Optional[Callable[[str], bool]]:
    """``athena_cli.profiles.profile_exists``, or ``None`` when it cannot be
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    """True iff a ready+assigned+unclaimed task maps to a real Athena profile.
...
def review_dispatch_enabled() -> bool:
    """Whether review tasks dispatch automatically. Default true (Athena ships
...
def count_running_tasks_other_boards(board: Optional[str] = None) -> int:
    ...
    Boards are matched by resolved DB path, so ``ATHENA_KANBAN_DB`` (pins every
...
def _dispatch_lane_task(
    ...
    *,
    ...
    # would fail ``athena -p <assignee>`` at startup and loop ready→crash→ready
    ...
    if guard_reason is not None:
        ...
        # Event so ``athena kanban tail`` shows why the task looks stuck.
...
def _module_athena_argv() -> list[str]:
    """Interpreter-bound Athena CLI invocation (``athena_cli.main`` is the
    console-script target — there is no top-level ``athena`` package)."""
...
def _athena_path_argv(path: str) -> list[str]:
    """argv for a resolved Athena executable path. Windows batch shims
...
def _resolve_athena_argv() -> list[str]:
    """Resolve the ``athena`` invocation as argv for ``Popen``: ``$ATHENA_BIN``
    (path-like -> absolute; bare names keep PATH semantics, never a
    same-directory file), then ``which("athena")`` (Windows: safe PATH search,
    batch shims fall back to the module form), then ``sys.executable -m
    athena_cli.main`` for shim-less environments (cron, systemd ``User=``,
    launchd). Mirrors ``gateway.run._resolve_athena_bin``; local because
    ``athena_cli`` sits below ``gateway`` in the dependency order.
    """
    import shutil
    ...
    athena_bin = _safe_which_no_cwd("athena") if _kb._IS_WINDOWS else shutil.which("athena")
    if athena_bin:
...
def _resolve_worker_cli_toolsets(athena_home: Optional[str]) -> Optional[list[str]]:
    ...
    tools when ``ATHENA_KANBAN_TASK`` is set.
    """
    if not athena_home:
        return None
    try:
...
def _worker_argv(task: Task, profile_arg: str, athena_home: Optional[str]) -> list[str]:
    """Build the ``athena -p <profile> --cli ... chat -q ...`` worker command."""
    cmd = [
        *_resolve_athena_argv(),
        ...
        "--cli",
        # Workers run under a profile-scoped ATHENA_HOME and so see that
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(athena_home)
    if worker_toolsets:
...
def _open_worker_log(task: Task, board: Optional[str]):
    ...
    `athena kanban log` reads its own file and boards sharing task ids don't
    collide."""
...
def _default_spawn(task: Task, workspace: str, *, board: Optional[str] = None) -> Optional[int]:
    ...
    ``ATHENA_KANBAN_DB`` / ``ATHENA_KANBAN_BOARD`` / workspaces_root to the
    board the task was claimed from, so workers cannot see other boards.
    """
    if not task.assignee:
        raise ValueError(f"task {task.id} has no assignee")

    from athena_cli.profiles import normalize_profile_name, resolve_profile_env
    ...
    # without it the child's get_athena_home() falls back to the DEFAULT
    # profile root because `athena -p` applies its override before
    # athena_constants is imported.
    try:
        env["ATHENA_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # No profile dir (isolated test fixtures) — the CLI resolves it from
        # ATHENA_PROFILE (set below) instead.
        pass
    if task.tenant:
    ...
    env["ATHENA_SESSION_SOURCE"] = "kanban"
    ...
    # match after `athena -p` rewrites ATHENA_HOME (symlink / Docker layouts).
    env["ATHENA_KANBAN_DB"] = str(_kb.kanban_db_path(board=board))
    env["ATHENA_KANBAN_WORKSPACES_ROOT"] = str(_kb.workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["ATHENA_KANBAN_WORKSPACES_ROOT"])
    # Board slug — defense-in-depth pin if a path is resolved without the
    # DB / workspaces env vars.
    env["ATHENA_KANBAN_BOARD"] = _kb._normalize_board_slug(board) or _kb.get_current_board()
    # kanban_comment reads ATHENA_PROFILE for its default author; `-p` alone
    ...
    # `--cli` is the highest-precedence TUI override; dropping ATHENA_TUI covers
    # older athena builds on PATH that predate the flag's precedence.
    env.pop("ATHENA_TUI", None)

    cmd = _worker_argv(task, profile_arg, env.get("ATHENA_HOME"))
...
def run_daemon(
    *,
    ...
    the gateway dispatcher and ``athena kanban dispatch`` — the standalone
...
from athena_cli import kanban_db as _kb  # noqa: E402
from athena_cli import kanban_db_connect as _kbc  # noqa: E402
from athena_cli import kanban_db_workspace as _kbw  # noqa: E402
