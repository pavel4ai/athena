...
Usage:
    athena claw migrate              # Preview then migrate (always shows preview first)
    athena claw migrate --dry-run    # Preview only, no changes
    athena claw migrate --yes        # Skip confirmation prompt
    athena claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    athena claw migrate --no-backup  # Skip pre-migration snapshot
    athena claw cleanup              # Archive leftover OpenClaw directories
    athena claw cleanup --dry-run    # Preview what would be archived
"""
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    ...
    print_info("Recommendation: stop the gateway first with 'athena gateway stop'.")
    print()
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print()
        print("Run 'athena claw <command> --help' for options.")
...
def _cmd_migrate(args):
    ...
    # migrate-athena posture (two-phase: run once without secrets, rerun with
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: athena claw migrate --source /path/to/.openclaw")
        return
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: athena claw migrate --yes")
            return
    ...
    # Delegates to athena_cli.backup.create_pre_migration_backup(), which
    ...
    if not no_backup:
        try:
            from athena_cli.backup import create_pre_migration_backup, _format_size
            backup_archive = create_pre_migration_backup(athena_home=athena_home)
            if backup_archive:
    ...
    # 'athena claw cleanup' separately.
...
def _cmd_cleanup(args):
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: athena claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  athena claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  athena claw migrate --migrate-secrets")
            print()
