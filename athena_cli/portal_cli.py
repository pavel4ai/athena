...
def _cmd_login(args) -> int:
    ...
    Reuses the exact wiring behind ``athena setup --portal`` so the commands stay in lockstep.
    """
    from athena_cli.setup import _run_portal_one_shot
...
# Default (None/"") is the one-shot onboarding (alias for `athena auth add nous --type oauth` /
# `athena setup --portal`). `status` kept as a back-compat alias for `info`.
_SUBCOMMANDS = {
    None: _cmd_login,
    "": _cmd_login,
    "login": _cmd_login,
...
def add_parser(subparsers) -> None:
    ...
    # `status` is a hidden (no help) back-compat alias; registration order = `athena portal -h` order.
