...
Split out of ``athena_cli/main.py``. Names that still live in main (``PROJECT_ROOT``, ...)
...
logger = logging.getLogger("athena_cli.main")
...
def _print_tui_exit_summary(session_id: Optional[str], active_session_file: Optional[str] = None) -> None:
    ...
    from athena_cli.main import _resolve_last_session
    target = (
...
def _tui_selected_workspace_keys(tui_dir: Path, ws_root: Path) -> set:
    ...
    from athena_cli.main import _is_termux_startup_environment
    try:
...
def _tui_need_npm_install(root: Path) -> bool:
    ...
    if not (ws_root / "node_modules" / "@athena" / "ink" / "package.json").is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_DIRS = ("src", "packages/athena-ink/src")
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    ``ATHENA_TUI_FORCE_BUILD=1`` forces a rebuild."""
...
def _ensure_tui_node() -> None:
    ...
    the resolved node dir to PATH. ``ATHENA_SKIP_NODE_BOOTSTRAP=1`` disables auto-install."""
    from athena_cli.main import PROJECT_ROOT
    ...
    extras += [Path(athena_home) / "node" / "bin", Path.home() / ".local" / "bin"]
    for extra in extras:
        s = str(extra)
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    tracked files after ``athena update``); True when the directory exists afterwards.

    On Windows an antivirus / NTFS filter driver can leave tracked ``ui-tui/`` files deleted in the working
    tree after ``athena update`` (HEAD stays intact; the files just vanish — see issue #49145). Those files
...
def _npm_lifecycle_env(env: dict[str, str] | None = None) -> dict[str, str]:
    ...
    # the rebuild fails). ``$ATHENA_HOME`` lives outside the git tree and the
    # update hand-off already carries ``ATHENA_HOME`` down to every npm child.
    # An explicit ``NPM_CONFIG_USERCONFIG`` wins (#106373).
    from athena_constants import get_athena_home
    npmrc = get_athena_home() / "npmrc"
    if npmrc.is_file():
...
def _tui_node_bin(bin: str) -> str:
    """Resolve ``node``/``npm`` for the TUI launch, or exit with a hint. ``ATHENA_NODE`` wins for node;
    ``find_node_executable()`` sees the managed ``$ATHENA_HOME/node`` tree a bare which() misses."""
    if bin == "node":
...
def _install_tui_dependencies(tui_dir: Path, *, termux_startup: bool) -> None:
    ...
    def _run_tui_install() -> subprocess.CompletedProcess:
        from athena_constants import with_athena_node_path
        ...
        return subprocess.run(
            ...
            env=_npm_lifecycle_env(with_athena_node_path()))
    ...
    if result.returncode != 0:
        ...
        from athena_cli.npm_engine import maybe_repair_npm_engine
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    # athena_cli/tui_dist/entry.js but never ui-tui/ (git checkouts only).
    # 1. A prebuilt install (Docker image, Nix build, or prior `npm run build`) ships
    #   athena_cli/tui_dist/entry.js but never ships ui-tui/ at all (that directory only exists in a git
    ...
    if tui_dev:
        ...
        # packages/athena-ink/dist/entry-exports.js; a stale dist after a pull
        # leaves newer hooks/components missing at runtime. Prebuild it here.
        npm = _tui_node_bin("npm")
        _run_tui_npm_build(npm, tui_dir / "packages" / "athena-ink", "TUI dev prebuild failed.")
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    from athena_cli.main import PROJECT_ROOT
    src_root = str(env.get("ATHENA_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["ATHENA_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("ATHENA_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["ATHENA_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("ATHENA_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["ATHENA_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["ATHENA_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    import tempfile
    # TUI child is a athena process: propagate the profile-home contract via
    # the single factory; keep secrets (the TUI/agent needs provider creds).
    from tools.environments.local import build_subprocess_env
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=True)
    from athena_cli.shared_session_attach import configure_tui_attachment
    try:
    ...
    for key, value in (
        ("ATHENA_MODEL", model), ("ATHENA_INFERENCE_MODEL", model),
        ("ATHENA_TUI_PROVIDER", provider), ("ATHENA_INFERENCE_PROVIDER", provider),
        ("ATHENA_TUI_TOOLSETS", ",".join(_normalize_tui_toolsets(toolsets))),
        ("ATHENA_TUI_SKILLS", skills_value),
        ("ATHENA_TUI_QUERY", query), ("ATHENA_TUI_IMAGE", image),
        ("ATHENA_TUI_CHECKPOINTS", "1" if checkpoints else None),
        ("ATHENA_TUI_PASS_SESSION_ID", "1" if pass_session_id else None),
        ("ATHENA_TUI_MAX_TURNS", str(max_turns) if max_turns is not None else None),
        ("ATHENA_TUI_TOOL_PROGRESS", "verbose" if verbose else "off" if quiet else None),
        ("ATHENA_ACCEPT_HOOKS", "1" if accept_hooks else None)):
        if value:
            env[key] = value
    ...
    # ATHENA_TUI_RESUME is an internal hand-off to the Ink app. We start from a
    # full os.environ snapshot, so a stale exported value would make a plain
    # `athena --tui` try to resume a non-existent session; only forward the id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `athena update`;
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``ATHENA_KANBAN_BOARD`` so in-process tools and shelled-out
    ``athena kanban`` calls agree even if a concurrent ``boards switch`` flips the file mid-turn.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls (``athena kanban …``) resolve
    the board on different paths: the env-pin if set, otherwise the global ``<root>/kanban/current`` file. A
    concurrent ``athena kanban boards switch`` from another session can flip the file mid-turn, so the same
...
def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.athena/skills/`` with the bundled library (idempotent, milliseconds when synced).
...
def _resolve_use_tui(args) -> bool:
    ...
    ``ATHENA_TUI=1`` → TUI; ``display.interface`` config; default classic.

    The TTY gate is load-bearing: ambient preferences must never hijack a piped
    ``athena chat -q`` (kanban workers, cron) — the Ink no-TTY bail-out exits 0 and
    ...
    try:
        ...
        return False
    if os.environ.get("ATHENA_TUI") == "1":
        return True
    try:
