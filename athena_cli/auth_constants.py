...
Pure leaf: imports nothing from ``athena_cli.auth`` so the per-provider modules
...
# httpx is imported lazily (~30ms) because athena_cli.auth is on the interactive-CLI startup path
...
if TYPE_CHECKING:
    ...
    class _LazyHttpx:
        ...
        # set/del forward to the real module so monkeypatch.setattr("athena_cli.auth.httpx.Client")
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from athena_cli import __version__ as _ATHENA_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _ATHENA_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"athena-cli/{_ATHENA_CLI_VERSION}"
