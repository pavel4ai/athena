...
is persisted in ~/.athena/auth.json with cross-process file locking.
...
from athena_cli.config import get_athena_home, get_config_path, read_raw_config
from athena_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
DEFAULT_NOUS_CLIENT_ID = "athena-cli"
...
SPOTIFY_DOCS_URL = "https://github.com/pavel4ai/athena/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

XAI_OAUTH_DOCS_URL = "https://github.com/pavel4ai/athena/docs/guides/xai-grok-oauth"
OAUTH_OVER_SSH_DOCS_URL = "https://github.com/pavel4ai/athena/docs/guides/oauth-over-ssh"
...
def _resolve_api_key_provider_secret(
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.athena/.env file
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("ATHENA_OAUTH_TRACE", "").strip().lower()
...
def _auth_file_path() -> Path:
    ...
    # Seat belt: if pytest is running and ATHENA_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch ATHENA_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".athena" / "auth.json").resolve(strict=False)
        try:
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom ATHENA_HOME that is not a profile).
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.athena/auth.json`` even when ATHENA_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_athena_root()`` for a profile-shaped ATHENA_HOME can
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'athena model' for available providers, or run 'athena doctor' to diagnose config issues."
...
def _assert_nous_inference_jwt_usable(
    ...
    *,
    ...
    raise AuthError(
        ...
        f"({reason}). Re-authenticate with: athena auth add nous",
        provider="nous",
        code=reason,
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("ATHENA_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
# =============================================================================
# Spotify auth — PKCE tokens stored in ~/.athena/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("ATHENA_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set ATHENA_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("ATHENA_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("ATHENA_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("ATHENA_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _make_xai_callback_handler(expected_path: str) -> tuple[type[BaseHTTPRequestHandler], dict[str, Any]]:
    ...
    class _XAICallbackHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            # into the log file.  Run with ``ATHENA_LOG_LEVEL=INFO`` (or check
            # ``~/.athena/logs/agent.log`` which captures INFO+ unconditionally).
            try:
                logger.info(
            ...
            if incoming["code"] is None and incoming["error"] is None:
                ...
                body = (
                    ...
                    "<code>athena auth add xai-oauth</code> to retry.</p>"
                    "</body></html>"
                )
                self.wfile.write(body.encode("utf-8"))
                return
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.athena/.env, and return it.
    ...
    save_env_value("ATHENA_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("ATHENA_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved ATHENA_SPOTIFY_CLIENT_ID to ~/.athena/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "ATHENA_SPOTIFY_CLIENT_ID is required".
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.athena/auth.json (not ~/.codex/)
#
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Athena auth store (~/.athena/auth.json).
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to Athena auth store (~/.athena/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    if response.status_code == 429:
        ...
        # "run athena auth" prompt (see issue #32790).
...
def resolve_codex_runtime_credentials(
    *,
    ...
    if data is None:
        ...
        if pool_token:
            base_url = (
                os.getenv("ATHENA_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("ATHENA_CODEX_REFRESH_TIMEOUT_SECONDS", 20)
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.athena/auth.json
# =============================================================================
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    # ~/.athena/auth.json even when ATHENA_HOME points at a profile path
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.athena/auth.json``. A single MITM during initial
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``ATHENA_XAI_BASE_URL``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("ATHENA_XAI_REFRESH_TIMEOUT_SECONDS", 20)
...
#
# File lives at ${ATHENA_SHARED_AUTH_DIR}/nous_auth.json, defaulting to
# ``<athena-root>/shared/nous_auth.json`` where ``<athena-root>`` is what
# ``get_default_athena_root()`` returns — ``~/.athena`` on Linux/macOS,
# ``%LOCALAPPDATA%\athena`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's ATHENA_HOME so named profiles (which
# typically live under ``<athena-root>/profiles/<name>/``) all see the
# same file.
#
...
def _nous_shared_auth_dir() -> Path:
    ...
    Honors ``ATHENA_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<athena-root>/shared/``, where ``<athena-root>`` is what
    :func:`athena_constants.get_default_athena_root` returns — so
    Linux/macOS classic installs land at ``~/.athena/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\athena\\shared\\``, and
    Docker / custom ``ATHENA_HOME`` deployments at
    ``<ATHENA_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("ATHENA_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from athena_constants import get_default_athena_root
    return get_default_athena_root() / "shared"
...
def _nous_shared_store_path() -> Path:
    ...
    # state. Tests must set ATHENA_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from athena_constants import get_default_athena_root
        real_home_shared = (
            get_default_athena_root() / "shared" / NOUS_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                ...
                f"{path}. Set ATHENA_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No ATHENA_HOME yet (pre-setup): fall through without locking.
        yield
        return
    ...
    *,
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "~/.athena/auth.json) called POST /api/oauth/token with Athena's "
            ...
            "instead.\n"
            "Re-authenticate with: athena auth add nous"
        )
        relogin = True
    ...
    *,
    ...
    with _auth_store_lock():
        ...
        portal_base_url = (
            ...
            or os.getenv("ATHENA_PORTAL_BASE_URL")
...
def refresh_nous_oauth_from_state(
    ...
    *,
    ...
    return refresh_nous_oauth_pure(
        ...
        state.get("client_id", "athena-cli"),
...
# `athena auth login/logout/add/remove` invalidate naturally on the next call.
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("ATHENA_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("ATHENA_COPILOT_ACP_ARGS", "").strip()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    try:
        from athena_cli.config import load_config, get_env_value
        cfg = load_config()
    except Exception:
        cfg = {}
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set ATHENA_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _prompt_model_selection(
    ...
    """
    from athena_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from athena_cli.config import save_config, load_config
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.athena/auth.json."""
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("ATHENA_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
    ...
    *,
    ...
    if not code_verifier:
        raise AuthError(
            ...
            "https://github.com/pavel4ai/athena/issues/26990.",
            provider="xai-oauth",
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Athena auth store (~/.athena/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def step_up_nous_billing_scope(
    *,
    ...
    targets the same deployment (incl. a preview via ``ATHENA_PORTAL_BASE_URL`` set
