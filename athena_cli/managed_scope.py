...
DISTINCT from ``athena_cli.config.is_managed()`` / ``ATHENA_MANAGED`` (a coarse package-manager
...
def _under_pytest() -> bool:
    """True inside the test suite: ignore the system ``/etc/athena`` so a real managed scope on a
    dev/CI box can't leak policy into the suite. An explicit ``ATHENA_MANAGED_DIR`` still wins."""
...
def get_managed_dir() -> Optional[Path]:
    ...
    Priority: ``$ATHENA_MANAGED_DIR`` (IT-only bootstrap override; never persisted to any .env;
    honored only when non-empty AND the directory exists), then ``/etc/athena`` when it exists.
