...
Subcommands (all live under ``athena profile``, not a parallel tree):
    ...
    athena profile install <source> [--name N] [--alias] [--force] [--yes]
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            athena_requires=str(data.get("athena_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.athena_requires:
            out["athena_requires"] = self.athena_requires
        if self.author:
...
def plan_install(
    ...
    check_athena_requires(manifest.athena_requires, athena_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.athena).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
