...
- Keep data: Remove code but keep ~/.athena/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing athena-agent or athena PATH entries
            new_lines = []
            skip_next = False
            ...
            for line in content.split('\n'):
                ...
                # Remove any PATH line containing athena
...
def remove_wrapper_script():
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains athena_cli reference)
                content = wrapper.read_text()
                if 'athena_cli' in content or 'athena-agent' in content:
                    wrapper.unlink()
...
#
#   1. Sets User-scope env vars ``ATHENA_HOME`` and ``ATHENA_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\athena\git\cmd``, ``%LOCALAPPDATA%\athena\git\bin``,
#      ``%LOCALAPPDATA%\athena\git\usr\bin``, ``%LOCALAPPDATA%\athena\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\athena\git\`` and Node to
...
#      ``%LOCALAPPDATA%\athena\gateway-service\`` and sometimes
...
def _athena_path_markers(athena_home: Path) -> list[str]:
    ...
    # Also match if ATHENA_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "athena".  We don't want to catch
    # unrelated entries like "cathena-foo" or "ephermeral", so we look for
    # backslash-athena as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(athena_home: Path) -> list[str]:
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _athena_path_markers(athena_home)
            kept: list[str] = []
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ATHENA_HOME directory.
    ...
    current ATHENA_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m athena_cli.main` so we don't depend on a `athena`
    #    wrapper that may be half-removed mid-uninstall.
    athena_invocation = [_sys.executable, "-m", "athena_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                athena_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
...
def run_gui_uninstall(args):
    ...
    userData dir — nothing under ``$ATHENA_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from athena_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.athena/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.athena/ for future reinstall
    """
    project_root = get_project_root()
    athena_home = get_athena_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie ATHENA_HOMEs
    ...
    # full wipe (code + ~/.athena data); otherwise keep-data. Named profiles
    ...
    # their alias wrappers, and wiping their ATHENA_HOME dirs. Otherwise
...
def _perform_uninstall(
    *,
    ...
    wipe ``$ATHENA_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in athena_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\athena\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(athena_home))))
        ...
        log_info("Removing ATHENA_HOME / ATHENA_GIT_BASH_PATH User env vars...")
        removed_env = remove_athena_env_vars_windows()
        if removed_env:
    ...
    log_info("Removing athena command...")
    ...
    removed_node_links = remove_node_symlinks(athena_home)
    ...
    #     step-5 rmtree(athena_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE ATHENA_HOME and
    ...
    try:
        ...
        gui_removed = uninstall_gui(athena_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.athena/, just remove the athena-agent subdir
            if athena_home in project_root.parents or project_root.parent == athena_home:
    ...
    #     under ATHENA_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(athena_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(athena_home)
    ...
    if full_uninstall:
        ...
        #     alias wrapper. The profile ATHENA_HOME dirs live under
        ...
        try:
            if athena_home.exists():
                shutil.rmtree(athena_home)
                log_success(f"Removed {athena_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {athena_home}: {e}")
            log_info("You may need to manually remove it")
    else:
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://github.com/pavel4ai/athena/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://github.com/pavel4ai/athena/install.sh | bash", Colors.DIM))
        print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m athena_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import athena_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``athena_constants`` + ``athena_cli.colors``
    (and lazily ``athena_cli.gui_uninstall``), so it runs fine under a bare
