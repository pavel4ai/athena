...
def _strip_athena_path_lines(content: str) -> str:
    """Drop the ``# Athena Agent`` marker (+ its PATH line) and any athena PATH line; squash blank runs."""
    new_lines = []
    skip_next = False
...
def remove_path_from_shell_configs():
    ...
    for config_path in (c for c in (Path.home() / n for n in _SHELL_RC_NAMES) if c.exists()):
        try:
            ...
            new_content = _strip_athena_path_lines(content)
...
def remove_wrapper_script():
    """Remove the athena wrapper script if it exists."""
    def _unlink_ours(wrapper: Path) -> bool:
        # Only our wrapper (contains a athena_cli / athena-agent reference).
        content = wrapper.read_text(encoding="utf-8")
        return _unlink_if('athena_cli' in content or 'athena-agent' in content, wrapper)

    candidates = (
        bin_dir / name
        for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin"))
        for name in ("athena", "athena-acp", "athena-agent"))
...
def remove_node_symlinks(athena_home: Path) -> list:
    ...
    node_dir = (athena_home / "node").resolve()
...
def uninstall_gateway_service():
    ...
    try:
        from athena_cli.gateway import kill_gateway_processes, find_gateway_pids
...
def _remove_launchd_gateway() -> bool:
    ...
    from athena_cli.gateway import get_launchd_plist_path
...
def _remove_windows_gateway() -> bool:
    ...
    from athena_cli import gateway_windows as gw
...
# ATHENA_HOME / ATHENA_GIT_BASH_PATH (HKCU\Environment), User-scope PATH entries
# (%LOCALAPPDATA%\athena\git\{cmd,bin,usr\bin}, ...\athena\node), PortableGit + Node copies
...
def _athena_path_markers(athena_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Prefixes identifying Athena-owned User-PATH entries (prefix match sweeps git\cmd, git\bin,
    ...
    root = str(athena_home).rstrip("\\/")
    subs = ("athena-agent", "git", "node", "venv") + (("bin",) if include_managed_bin else ())
    return [f"{root}\\{sub}" for sub in subs]


def remove_path_from_windows_registry(athena_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip Athena-owned entries from User-scope PATH in the registry (see ``_athena_path_markers``)."""
    markers = tuple(m.lower() for m in _athena_path_markers(athena_home, include_managed_bin=include_managed_bin))
...
def remove_athena_env_vars_windows() -> list[str]:
    """Delete ATHENA_HOME and ATHENA_GIT_BASH_PATH from User-scope env vars."""
    def edit(winreg, key, removed):
        for name in ("ATHENA_HOME", "ATHENA_GIT_BASH_PATH"):
            try:
...
def remove_portable_tooling_windows(athena_home: Path) -> list[Path]:
    """Delete the PortableGit / Node / gateway-service dirs the Windows installer created under
    ``athena_home`` (isolated from any system Git / Node, so nothing else breaks)."""
    targets = (athena_home / sub for sub in ("git", "node", "gateway-service"))
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``athena`` launchers install.ps1 staged in the managed ``bin`` dir: every mode
    ...
    try:
        ...
        from athena_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
        from athena_constants import get_default_athena_root
        bin_dir = get_default_athena_root() / "bin"
    except Exception as e:
...
def _is_default_athena_home(athena_home: Path) -> bool:
    """Return True when ``athena_home`` points at the default (non-profile) root."""
    try:
        from athena_constants import get_default_athena_root
        return athena_home.resolve() == get_default_athena_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    out to ``athena -p <name> gateway stop|uninstall`` because service names / unit paths derive
    from the current ATHENA_HOME and can't be switched in-process."""
    name = profile.name
    log_info(f"Uninstalling profile '{name}'...")

    # 1. Gateway service, via `python -m athena_cli.main` (the `athena` wrapper may be half-gone).
    athena_invocation = [sys.executable, "-m", "athena_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                athena_invocation + ["gateway", subcmd], capture_output=True, text=True,
    ...
    # 3. Wipe the profile's ATHENA_HOME directory.
    _rmtree_step(profile.path, indent="  ", fully=False)


def run_gui_uninstall(args):
    """``athena uninstall --gui``: remove the desktop app's built artifacts, packaged bundle
    (best-effort) and Electron userData — never config/sessions/.env, the agent or its venv."""
    from athena_cli.gui_uninstall import agent_is_installed, gui_install_summary, uninstall_gui
    athena_home = get_athena_home()
    summary = gui_install_summary(athena_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    _print_box("│         ⚕ Athena Chat GUI Uninstaller                  │", Colors.MAGENTA)
    print()

    if not summary["gui_installed"]:
        print("No Athena Chat GUI installation was found.")
        print(f"  Checked: {athena_home}, and the standard app locations for this OS.")
        return
    ...
    print()
    if agent_is_installed(athena_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Athena agent at {athena_home / 'athena-agent'}")
        print(f"  • Your config, sessions, and secrets under {athena_home}")
        print()
    ...
    print()
    uninstall_gui(athena_home)

    print()
    _print_box("│            ✓ Chat GUI Uninstalled!                      │", Colors.GREEN)
    print()
    print("The Athena agent is still installed. Run 'athena' to use the CLI,")
    print("or 'athena uninstall' to remove the agent too.")
    print()


def run_uninstall(args):
    """Interactive/``--yes`` uninstall: full removes code and ``~/.athena/``; keep-data removes only
    ...
    if bool(getattr(args, "dry_run", False)):
        _print_uninstall_dry_run(
            project_root=project_root, athena_home=athena_home, full_uninstall=full_flag)
        return

    # Named profiles (only when uninstalling from the default root) are offered for cleanup too,
    # instead of leaving zombie ATHENA_HOMEs and systemd units behind.
    is_default_profile = _is_default_athena_home(athena_home)
    ...
    print()
    _print_box("│            ⚕ Athena Agent Uninstaller                  │", Colors.MAGENTA)
    print()

    # Show what will be affected
    print(color("Current Installation:", Colors.CYAN, Colors.BOLD))
    print(f"  Code:    {project_root}")
    print(f"  Config:  {athena_home / 'config.yaml'}")
    print(f"  Secrets: {athena_home / '.env'}")
    print(f"  Data:    {athena_home / 'cron/'}, {athena_home / 'sessions/'}, {athena_home / 'logs/'}")
    print()
    ...
    # services, alias wrappers, ATHENA_HOME dirs) — otherwise they leave zombie services behind.
    ...
    _perform_uninstall(
        project_root=project_root, athena_home=athena_home, full_uninstall=full_uninstall,
        remove_profiles=remove_profiles, named_profiles=named_profiles)


def _print_uninstall_dry_run(*, project_root: Path, athena_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Athena PATH entries from shell configs / Windows User PATH")
    print("  • Athena wrapper scripts and Athena-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if not full_uninstall:
        print(f"  • Keep Athena config/data: {athena_home}")
    else:
        print(f"  • Athena config/data: {athena_home}")
        profiles = _discover_named_profiles() if _is_default_athena_home(athena_home) else []
        if profiles:
...
def _perform_uninstall(
    *,
    ...
    the checkout -> (Windows) PortableGit/Node -> optionally ``$ATHENA_HOME`` and named profiles."""
    print()
    ...
    # 2-3b. PATH entries, wrapper, Windows launchers, node symlinks. Windows: athena_home is
    #    %VAR%-expanded because install.ps1 writes literal C:\Users\<u>\...; athena\bin (launchers +
    ...
    windows = _is_windows()
    sweep_managed_bin = windows and full_uninstall and _is_default_athena_home(athena_home)
    for on_this_platform, label, remove, success_fmt, none_msg in (
        ...
        (windows, "Removing PATH entries from Windows User environment...",
         lambda: remove_path_from_windows_registry(
             Path(os.path.expandvars(str(athena_home))), include_managed_bin=sweep_managed_bin),
         "Removed from User PATH: {}", "No Athena-owned PATH entries in User environment"),
        (windows, "Removing ATHENA_HOME / ATHENA_GIT_BASH_PATH User env vars...",
         remove_athena_env_vars_windows, "Removed User env var: {}", "No Athena-set User env vars to remove"),
        (True, "Removing athena command...", remove_wrapper_script, "Removed {}", "No wrapper script found"),
        (windows, "Removing Windows athena launchers...",
         remove_windows_bin_launchers, "Removed {}", "No Windows athena launchers found"),
        (True, "Removing Athena-managed node/npm/npx symlinks...",
         lambda: remove_node_symlinks(athena_home), "Removed {}", "No Athena-managed node/npm/npx symlinks found"),
    ):
        if on_this_platform:
    ...
    #     .env (safe in keep-data mode); the packaged app + Electron userData live OUTSIDE ATHENA_HOME.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from athena_cli.gui_uninstall import uninstall_gui
        if not uninstall_gui(athena_home):
    ...
    if windows:
        _remove_step(
            ...
            lambda: remove_portable_tooling_windows(athena_home), "Removed {}",
            "No Windows installer artifacts to remove")

    # 5. Optionally remove ~/.athena/ data directory (and named profiles)
    if full_uninstall:
        ...
        _rmtree_step(athena_home)
    else:
        log_info(f"Keeping configuration and data in {athena_home}")
...
_REINSTALL_HINT = {
    True: "  iex (irm https://athena-agent.nousresearch.com/install.ps1)",
    False: "  curl -fsSL https://athena-agent.nousresearch.com/install.sh | bash"}
...
def main(argv=None) -> int:
    """``python -m athena_cli.uninstall --mode <gui|lite|full>``. Imports only stdlib +
    ``athena_constants`` + ``athena_cli.colors``, so it runs under a bare system Python (no venv)."""
    import argparse
    parser = argparse.ArgumentParser(prog="python -m athena_cli.uninstall")
    parser.add_argument(
