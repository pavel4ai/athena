"""Streamlined setup flows: the Nous Portal one-shot (`athena portal`), first-time quick setup,
Blank Slate setup and the `--quick` missing-items pass. Names from setup.py are imported lazily
per function so test patches on ``athena_cli.setup`` take effect."""
...
logger = logging.getLogger("athena_cli.setup")
...
def _blank_slate_done(config: dict, athena_home, tools_line: str, *extra: str, intro: str | None = None) -> None:
    """Shared Blank Slate epilogue: success banner, the "enable later" hints, then the summary."""
    from athena_cli.setup import _info, _print_setup_summary, print_success
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    _info(*([intro] if intro else []), tools_line, "  Seed skills:         athena skills opt-in --sync",
          "  Add MCP servers:     athena mcp add", *extra, "  Tune agent settings: athena setup agent", None)
...
def _reload_config_into(config: dict, *, dict_only: bool = False) -> None:
    ...
    from athena_cli.setup import load_config
...
def _run_nous_flow(config: dict, *, context: str, cancel_exc: tuple, cancel_lines: tuple, print_error) -> bool:
    ...
    single source of truth shared with ``athena model``. False when cancelled or failed (the
    message is already printed)."""
    from athena_cli.setup import _info
    try:
        from athena_cli.model_setup_flows import _model_flow_nous
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot Nous Portal setup (``athena setup --portal`` / ``athena portal``)."""
    from athena_cli.setup import _info, _print_banner, print_error, print_info, print_success
    _print_banner("│     ⚕ Athena Setup — Nous Portal (one-shot)             │")
    ...
    if not _run_nous_flow(config, context="`athena portal`", cancel_exc=(KeyboardInterrupt, EOFError, SystemExit),
                          cancel_lines=(None, "  Setup cancelled.", "  You can retry later with `athena portal`."),
    ...
    _info("  Run `athena portal info` to inspect routing.", "  Run `athena` to start chatting.")
...
def _run_first_time_quick_setup(config: dict, athena_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice("Connect a messaging platform? (Telegram, Discord, etc.)", [
        "Set up messaging now (recommended)", "Skip — set up later with 'athena setup gateway'",
    ], 0)
    if gateway_choice == 0:
        ...
        # platforms come alive as soon as tokens are added later (e.g. via `athena import`).
        from athena_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    print()
    print_success("Setup complete! You're ready to go.")
    _info(None, "  Configure all settings:    athena setup")
    if gateway_choice != 0:
...
def _print_macos_fda_tip() -> None:
    ...
    _info(None, "  macOS tip: silence ALL folder permission prompts with one switch —",
          ...
          "  your terminal (and Athena.app if you use Desktop), or run:",
          "    open \"x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles\"",
          "  The grant is permanent — it survives every Athena update.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    ``vision`` (``read_file`` can't read images) and ``skills`` (the seeded ``athena-agent`` skill
    ...
    try:
        ...
        # Plain TOOLSETS entries catch recovered toolsets like ``kanban``. Skip "athena-*" platform
        ...
        for k, tdef in TOOLSETS.items():
            if k.startswith("athena-") or (isinstance(tdef, dict) and (tdef.get("includes") or tdef.get("posture"))):
                continue
...
def _blank_slate_minimize_config(config: dict):
    """Turn OFF every optional config feature; all opt back in via ``athena setup agent``."""
...
def _run_blank_slate_setup(config: dict, athena_home, is_existing: bool):
    ...
    _info("Everything starts OFF. First we force-enable only what's required",
          ...
          "essential `athena-agent` skill is always kept so the agent",
          "can help you drive and configure Athena itself.", None)
    ...
    if path != 0:
        ...
        return
    save_config(config)
    # Blank Slate means no bundled skills; record the opt-out so future `athena update` runs
    # don't re-inject them.
    _set_bundled_skills_opt_out(True, "skill opt-out")
    _blank_slate_done(config, athena_home, "  Enable tools:        athena tools", "  Enable plugins:      athena plugins",
...
def _blank_slate_walkthrough(config: dict, athena_home):
    ...
    def _opted_out(_result) -> None:
        _info("No skills seeded (except the essential `athena-agent`",
              "skill). A .no-bundled-skills marker keeps future",
              "`athena update` runs from re-injecting them. Opt back in any",
              "time with `athena skills opt-in --sync`.")
    ...
    for header, question, yes_msg, no_msg in (
        ("Plugins", "Review and enable built-in plugins now?",
         "Manage plugins with `athena plugins list` / `athena plugins install`.",
         "No plugins enabled. Add later with `athena plugins`."),
        ("MCP Servers", "Add an MCP server now?",
         "Add servers with `athena mcp add <name> --url ... | --command ...`.",
         "No MCP servers configured. Add later with `athena mcp add`."),
    ):
    ...
    if prompt_yes_no("Connect a messaging platform (Telegram, Discord, …)?", default=False):
        setup_gateway(config)
    save_config(config)
    _blank_slate_done(config, athena_home, "  Enable more tools:   athena tools")
...
def _run_quick_setup(config: dict, athena_home):
    ...
    from athena_cli.config import (get_missing_env_vars, get_missing_config_fields, check_config_version)
    ...
    if not (missing_required or missing_optional or missing_config or current_ver < latest_ver):
        ...
        _info(None, "Run 'athena setup' and choose 'Full Setup' to reconfigure,",
    ...
    if missing_messaging:  # checklist, then prompt for each selected platform's vars
        ...
        _info("Connect Athena to messaging apps to chat from anywhere.",
              "You can configure these later with 'athena setup gateway'.")
