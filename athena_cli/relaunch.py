"""Unified self-relaunch for Athena CLI: preserves inherited flags (--tui, --dev, --profile, --model…)
across process replacement so ``athena sessions browse`` / post-setup relaunch keep the user's mode."""
...
def resolve_athena_bin() -> Optional[str]:
    """Athena entry point: ``sys.argv[0]`` if a real executable, else ``which athena``, else ``None``
    (caller falls back to ``python -m athena_cli.main``)."""
    argv0 = sys.argv[0]
    ...
    if not argv0.startswith("-") and os.path.isfile(argv0):
        ...
        if os.access(abs_path, os.X_OK) and not (_is_windows and _is_python_script(abs_path)):
            return abs_path
    return shutil.which("athena") or None
...
def build_relaunch_argv(
    ...
    argv = [bin_path] if bin_path else [sys.executable, "-m", "athena_cli.main"]
...
def relaunch(
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # Raw ``[Errno 8] Exec format error`` is cryptic; usual causes are ``athena`` not on
