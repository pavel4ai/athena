...
from athena_cli._parser import (
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched athena."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_athena_bin() -> Optional[str]:
    """Find the athena entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("athena")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m athena_cli.main``.
    ...
    ``athena.exe`` on PATH or the ``sys.executable -m athena_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with athena.
    ...
    Args:
        ...
        original_argv: The original argv to scan for flags (defaults to
            ``sys.argv[1:]``).
    """
    bin_path = resolve_athena_bin()

    if bin_path:
        argv = [bin_path]
    else:
        argv = [sys.executable, "-m", "athena_cli.main"]
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh athena invocation.
    ...
    relaunch contract wants: "run athena again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``athena.exe`` (a Python console-script shim that wraps
    ``python -m athena_cli.main``) or a ``.cmd`` batch file, and both
    ...
    That's functionally equivalent — the user sees "athena exited, then
    new athena started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            print(
                ...
                f"Fix: open a new terminal so PATH picks up, then re-run athena.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
