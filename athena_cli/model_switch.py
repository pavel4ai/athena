...
# Re-exported: callers/tests patch athena_cli.model_switch.<name>.
from athena_cli.model_switch_providers import list_authenticated_providers
...
def _declared_model_ids(value: Any) -> list[str]:
    ...
    if isinstance(value, str):
        ...
        # Pre-fix Athena wrote sentinel keys inside the user-facing ``models`` mapping.
...
def _entry_models_discovered(entry: Any) -> bool:
    """True when the entry's ``models`` mapping was auto-discovered by Athena.
...
def _models_config_is_allowlist(value: Any, discovered: bool = False) -> bool:
    ...
    ``discover_models: false``. A catalog Athena itself persisted (``discovered``) is never a pin."""
    if discovered:
        return False
    if isinstance(value, str):
...
def is_nous_athena_non_agentic(model_name: str) -> bool:
    ...
    return bool(model_name and _NOUS_ATHENA_NON_AGENTIC_RE.search(model_name))


def _check_athena_model_warning(model_name: str) -> str:
    """Warning string if *model_name* is a Nous Hermes 3/4 chat model, else ""."""
    return _ATHENA_MODEL_WARNING if is_nous_athena_non_agentic(model_name) else ""
...
def _direct_alias_source_identity() -> Optional[tuple]:
    ...
    try:
        from athena_constants import get_config_path
...
def resolve_startup_model_route(
    ...
    ``AthenaCLI`` is constructed before the interactive ``/model`` pipeline runs; resolving here
    ...
    if current_provider:
        try:
            from athena_cli.providers import is_routing_aggregator, normalize_provider as _norm_prov
...
def _ollama_configured_base() -> tuple[dict, str]:
    from athena_cli.models import _get_provider_config_dict
...
def _unknown_provider_message(explicit_provider: str) -> str:
    msg = (
        f"Unknown provider '{explicit_provider}'. Check 'athena model' for available "
    ...
    try:  # Surface common config issues that cause provider resolution failures
        ...
        if issues:
            msg += "\n\nRun 'athena doctor' — config issues detected:" + "".join(f"\n  • {ci.message}" for ci in issues[:3])
    except Exception:
        pass
    return msg
...
def _route_from_model_input(st: _Switch) -> Optional[ModelSwitchResult]:
    ...
    from athena_cli.models import detect_provider_for_model
    raw_input, current_provider = st.raw_input, st.current_provider
    try:
        from athena_cli.config import load_config
        from athena_cli.moa_config import exact_moa_preset_name, normalize_moa_config
...
def _creds_for_switched_provider(st: _Switch) -> Optional[ModelSwitchResult]:
    ...
    if st.explicit_provider and st.user_providers:
        from athena_cli.providers import resolve_user_provider
...
def _creds_for_current_provider(st: _Switch) -> None:
    ...
    from athena_cli.models_local import _get_ollama_request_headers, _same_ollama_native_root
...
def _copilot_api_mode(provider: str, model: str, api_key: str) -> str:
    from athena_cli.models import copilot_model_api_mode
...
def _opencode_api_mode(provider: str, model: str, api_key: str) -> str:
    ...
    # #16878.
    from athena_cli.models import opencode_model_api_mode
...
def _build_switch_result(st: _Switch) -> ModelSwitchResult:
    ...
    from athena_cli.models import normalize_opencode_base_url, opencode_provider_family
    ...
    warnings = [w for w in (st.validation.get("message"), _check_athena_model_warning(st.new_model)) if w]
    ...
    try:
        from athena_cli.runtime_provider import _get_named_custom_provider, _custom_provider_request_overrides
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'list_picker_providers': ('athena_cli.model_switch_providers', 'list_picker_providers'),
    'prewarm_picker_cache_async': ('athena_cli.model_switch_providers', 'prewarm_picker_cache_async'),
}
