...
- ``athena_cli.providers``        -- canonical provider identity + overlays
- ``athena_cli.model_normalize``  -- per-provider name formatting
...
_ATHENA_MODEL_WARNING = (
    "Futurebound Corp. Athena 3 & 4 models are NOT agentic and are not designed "
    "for use with Athena Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Futurebound Corp. Athena 3 / Athena 4 chat families.
# The previous substring check (`"athena" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``athena-brain:qwen3-14b-ctx16k`` that just
# happen to carry "athena" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   FutureboundCorp/Athena-3-Llama-3.1-70B, athena-4-405b, openrouter/athena3:70b
# Negative examples it must NOT match:
#   athena-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
...
def is_nous_athena_non_agentic(model_name: str) -> bool:
    ...
    if not model_name:
        return False
    return bool(_NOUS_ATHENA_NON_AGENTIC_RE.search(model_name))


def _check_athena_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Athena 3/4 chat model."""
    if is_nous_athena_non_agentic(model_name):
        return _ATHENA_MODEL_WARNING
    return ""
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from athena_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'athena doctor' — config issues detected:"
        ...
        from athena_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from athena_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from athena_cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in athena_cli.runtime_provider.resolve_runtime_provider;
    ...
    athena_warn = _check_athena_model_warning(new_model)
    if athena_warn:
        warnings.append(athena_warn)
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from athena_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from athena_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by athena provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://github.com/pavel4ai/athena/docs/api/model-catalog.json so
    ...
    if "lmstudio" not in curated and (
        ...
        from athena_cli.models import fetch_lmstudio_models
    ...
    for athena_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(athena_id)
        if (
            _alias_target
            and _alias_target != athena_id
        ...
        # section 2 (ATHENA_OVERLAYS) with proper auth store checking.
        ...
        model_ids = cached_provider_model_ids(athena_id)
        if not model_ids:
            model_ids = curated.get(athena_id, [])
            if athena_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(athena_id, model_ids)
        total = len(model_ids)
    ...
    from athena_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Athena provider ID.
    # ATHENA_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Athena IDs ("copilot").
    _mdev_to_athena = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    ...
    for pid, overlay in ATHENA_OVERLAYS.items():
        ...
        athena_slug = _mdev_to_athena.get(pid, pid)
        if athena_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(athena_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, athena_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or athena_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds and athena_slug == "anthropic":
            try:
                ...
                if (athena_creds and athena_creds.get("accessToken")) or \
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                _ids = cached_provider_model_ids(athena_slug)
                model_ids = _ids if _ids else (curated.get(athena_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(athena_slug, []) or curated.get(pid, [])
            ...
            try:
                ...
                from athena_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(athena_slug, []) or curated.get(pid, [])
                if athena_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(athena_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models] if max_models is not None else model_ids

        results.append({
            "slug": athena_slug,
            "name": get_label(athena_slug),
            "is_current": athena_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "athena",
        ...
        seen_slugs.add(athena_slug.lower())
        _record_builtin_endpoint(athena_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or ATHENA_OVERLAYS (keeps /model in sync
    # with `athena model`).
    try:
        from athena_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # (see athena_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # run_agent.py, athena_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`athena_cli.models.fetch_openrouter_models`, which filters the
