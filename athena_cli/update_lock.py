"""Cross-process mutual exclusion for in-flight Athena updates.
...
MARKER_NAME = ".athena-update-in-progress"

# Set by an orchestrating updater (Tauri `athena-setup --update`) to its own pid before
# spawning `athena update` as a child stage; the parent holds the marker for its whole run,
...
HANDOFF_PID_ENV = "ATHENA_UPDATE_HANDOFF_PID"
...
# Tauri updater (UPDATE_EXIT_CONCURRENT in update.rs) to show "Athena is still running".
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Athena home (never the context-local profile override): the Rust
    updater resolves ``$ATHENA_HOME`` or the platform default and the desktop pins that same
    ...
    return get_process_athena_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``athena update`` as a (grand)child, so a live marker
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Athena update is already running (PID {holder.pid}, "
