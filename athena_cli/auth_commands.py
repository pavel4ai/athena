...
def _anthropic_oauth_login(args) -> dict:
    ...
    creds = anthropic_mod.run_athena_oauth_login_pure()
    if not creds:
...
def _qwen_oauth_login(args) -> dict:
    from athena_cli.auth_qwen import _mark_qwen_oauth_active
...
class _OAuthAddSpec:
    """Per-provider parameters for the generic ``athena auth add <provider> --type oauth`` path."""
...
_OAUTH_ADD_SPECS: dict[str, _OAuthAddSpec] = {
    "anthropic": _OAuthAddSpec(
        ...
        source=f"{SOURCE_MANUAL}:athena_pkce",
...
def _add_nous_oauth_credential(args, provider: str) -> PooledCredential:
    """``athena auth add nous --type oauth``: shared-credential import, else device-code login."""
    ...
    # Codex-style auto-import: a shared Nous credential at <athena-root>/shared/nous_auth.json
    # (written by any previous login) makes `athena --profile <name> auth add nous --type oauth`
...
def _report_priority(provider: str, pool, moved, requested: int, verb: str, prep: str) -> None:
    ...
    print(f'{verb} {provider} credential "{moved.label}" {prep} priority {moved.priority} '
          f"(#{moved.priority + 1} in `athena auth list {provider}`)")
...
def auth_priority_command(args) -> None:
    """`athena auth priority <provider> <target> <priority>`: reorder one pooled credential."""
    provider = _normalize_provider(getattr(args, "provider", ""))
    pool = load_pool(provider)
...
def _free_tier_lines() -> tuple[str, str]:
    ...
    from athena_cli.anon_auth import FREE_TIER_LABEL, GUEST_MODEL, UPGRADE_HINT
...
def auth_remove_command(args) -> None:
    ...
    # Every credential source Athena reads from (env vars, external OAuth files, auth.json blocks,
...
def auth_refresh_command(args) -> None:
    """`athena auth refresh <provider> [target]`: force one pooled OAuth entry to refresh.
    ...
    if target is None or not str(target).strip():
        if len(entries) != 1:
            raise SystemExit(
                ...
                f"label (see `athena auth list {provider}`).")
    ...
    if provider == "nous" and matched.source != "device_code":
        raise SystemExit(
            ...
            "Reauthenticate with `athena auth add nous --type oauth`.")
...
def auth_upgrade_command(args) -> None:
    """``athena auth upgrade``: sign the free tier into a Nous account, keeping its connectors."""
    from athena_cli.anon_auth import upgrade_guest
