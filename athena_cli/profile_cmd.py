"""``athena profile`` command — one handler per action, dispatched by ``PROFILE_ACTIONS``.

Imports from ``athena_cli.profiles`` stay lazy (inside each handler) so tests can monkeypatch
...
def _profile_status(args):
    ...
    from athena_cli.profiles import format_profile_label, get_active_profile_name, list_profiles
    ...
    if current is not None:
        ...
        if p.alias_path:
            print(f"Alias:          {p.alias_name or p.name} → athena -p {p.name}")
    print()
...
def _profile_use(args):
    ...
    try:
        ...
        print("Switched to: default (~/.athena)" if name == "default" else f"Switched to: {name}")
...
def _profile_create(args):
    ...
    if not no_alias:
        ...
        if collision:
            ...
            print(f"  Choose a custom alias:  athena profile alias {name} --name <custom>")
...
def _profile_delete(args):
    from athena_cli.profiles import delete_profile
    try:
...
def _describe_target_dir(name: str) -> Path:
    """Profile dir for ``describe``: ``default`` maps to the CURRENT home (get_athena_home),
...
def _profile_show(args):
    ...
    if alias_name:
        print(f"Alias:   {alias_name} → athena -p {name}  ({_wrapper_path(alias_name)})")
    print()
...
def _profile_rename(args):
    from athena_cli.profiles import normalize_profile_name, rename_profile
    try:
...
def _profile_export(args):
    from athena_cli.profiles import export_profile, get_profile_export_path
    name = args.profile_name
    try:
...
def _profile_install(args):
    import tempfile
    from athena_cli.profile_distribution import DistributionError, install_distribution, plan_install
    try:
...
def _profile_update(args):
    from athena_cli.profile_distribution import DistributionError, read_manifest, update_distribution
    ...
    try:
        ...
        if plan.has_cron:
            print(f"  Cron files were refreshed.  Review with:  athena -p {plan.manifest.name} cron list")
    except (DistributionError, ValueError) as e:
        _die(f"Error: {e}")
...
_INFO_FIELDS = (
    ...
    ("author", "Author:       "),
    ("license", "License:      "),
    ("athena_requires", "Requires:     Athena "),
    ("source", "Source:       "),
...
# Order mirrors the original if/elif chain; None = bare ``athena profile``.
PROFILE_ACTIONS = {
