...
# load_athena_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_athena_dotenv().
...
# ATHENA_HOME paths we've already pulled external secrets for during this
# process.  ``load_athena_dotenv()`` is called at module-import time from
# several hot modules (cli.py, athena_cli/main.py, run_agent.py,
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_athena_dotenv()`` call.  Returns
...
def reset_secret_source_cache() -> None:
    """Forget which ATHENA_HOME paths have already had external secrets applied.
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``athena_cli.config._sanitize_env_lines`` which
...
def load_athena_dotenv(
    *,
    ...
    Behavior:
    - `~/.athena/.env` overrides stale shell-exported values when present.
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of ATHENA_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    ``home_path`` are no-ops.  ``load_athena_dotenv()`` runs at import
    time from several hot modules (cli.py, athena_cli/main.py,
