...
Extracted verbatim from ``athena_cli/main.py:main()`` (god-file Phase 2).
...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``athena login`` was removed in favor of ``athena auth`` / ``athena model``
    (the runtime handler in ``athena_cli/auth.py::login_command`` just prints a
    deprecation message and exits).  The subparser is kept registered so that
    old scripts/aliases invoking ``athena login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``athena --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``athena login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``athena model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `athena auth` to manage credentials, "
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `athena model`",
    )
    login_parser.add_argument(
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: athena-cli)"
    )
