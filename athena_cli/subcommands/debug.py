...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Athena Agent. Use 'athena debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    athena debug share              Upload debug report (asks for confirmation)
    athena debug share --yes        Skip confirmation (for scripts/CI)
    athena debug share --lines 500  Include more log lines
    athena debug share --expire 30  Keep dpaste.com fallback pastes for 30 days
    athena debug share --local      Print report locally (no upload)
    athena debug share --no-redact  Disable upload-time secret redaction
    athena debug share --nous       Upload to Nous-internal storage (private)
    athena debug delete <url>       Delete a previously uploaded paste
""")
    ...
    delete_parser = debug_sub.add_parser(
        "delete", help="Delete a paste uploaded by 'athena debug share'")
