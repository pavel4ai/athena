"""``athena computer-use`` subcommand parser."""

from __future__ import annotations

import sys

from athena_cli.subcommands._shared import add_json_flag


def _cu_install(args) -> int:
    from athena_cli.tools_config import _cua_driver_contract_status, install_cua_driver
...
def _cu_status(args) -> int:
    import os as _os
    import subprocess
    from athena_cli.tools_config import _cua_driver_contract_status
    ...
    override = _os.environ.get("ATHENA_CUA_DRIVER_CMD", "").strip()
    if not path:
    ...
    try:
        ...
        pass
    from athena_cli.tools_config import _cua_version_summary
    version = _cua_version_summary(version)
    # Name the override here too. Without it the operator is told to repair an
    # install that `athena computer-use install` will (correctly) refuse to touch,
    # with nothing pointing at the env var that actually selected the binary.
    origin = " [custom binary from ATHENA_CUA_DRIVER_CMD]" if override else ""
    ...
    if not contract.get("ready"):
        ...
        if override:
            print(
                "    Update the binary selected by ATHENA_CUA_DRIVER_CMD, or unset "
                "the override and run: athena computer-use install --upgrade")
        else:
            print("    Run: athena computer-use install")
        return 1
    try:
