"""``athena dashboard`` / ``athena serve`` subcommand parsers.
...
def _add_server_runtime_args(parser) -> None:
    ...
    # manager / PID file: they scan the process table for `athena dashboard|serve`
    # cmdlines and SIGTERM them — the same path `athena update` uses.
    parser.add_argument(
        "--stop", action="store_true", help="Stop all running Athena web server processes and exit")
    parser.add_argument(
        "--status", action="store_true", help="List running Athena web server processes and exit")
...
def build_serve_parser(
    *, cmd_dashboard: Callable, add_help: bool = True, exit_on_error: bool = True,
    ...
    parser = argparse.ArgumentParser(
        prog="athena serve",
        description="Run the Athena backend server - the JSON-RPC/WebSocket gateway the "
...
def build_dashboard_parser(
    ...
    # Compat shim: desktop shells <= 0.15.x spawn `athena dashboard --no-open --tui ...`;
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Athena backend server (headless; powers the desktop app and remote backends)",
        description="Run the Athena backend server — the JSON-RPC/WebSocket gateway the "
            "desktop app and remote clients connect to. Headless: it never opens "
            "a browser UI.")
    _configure_serve_parser(serve_parser, cmd_dashboard=cmd_dashboard)

    # `register` is nested so bare `athena dashboard` keeps launching the server.
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description="Register this install as a self-hosted dashboard with your Nous "
            ...
            "ATHENA_DASHBOARD_OAUTH_CLIENT_ID into ~/.athena/.env, and prints "
    ...
    dashboard_register_parser.add_argument(
        ...
        help="Optional public HTTPS OAuth redirect URI for the dashboard, e.g. "
            "https://athena.example.com/auth/callback. Omit for localhost-only use.")
    dashboard_register_parser.add_argument(
        ...
        help="Override the Nous Portal base URL for registration (default: the "
            ...
            "portal. Also settable via ATHENA_DASHBOARD_PORTAL_URL. Mainly for "
