...
def _config_base_url_trustworthy_for_bare_custom(cfg_base_url: str, cfg_provider: str) -> bool:
    ...
    try:
        from athena_cli.auth import resolve_provider as _resolve_provider
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = _getenv("ATHENA_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if requested_norm != "custom" and not requested_norm.startswith("custom:"):
        try:
            ...
            # the built-in. See tests/athena_cli/test_runtime_provider_resolution.py
...
def canonical_custom_identity(
    *,
    ...
    2. ``config_provider`` — the active ``config.model.provider`` (or its
       ``provider``/``ATHENA_INFERENCE_PROVIDER`` equivalent). When the agent
    ...
    if not candidate:
        try:
            ...
            candidate = ""
    if not candidate:
        candidate = os.environ.get("ATHENA_INFERENCE_PROVIDER", "").strip()
    ...
    *,
    ...
    if not api_key:
        raise AuthError(
            ...
            "~/.athena/.env or run 'athena model' to configure. To use "
    ...
    if provider == "nous":
        ...
        api_key = explicit_api_key or (
            ...
            if _agent_key_is_usable(
                state,
                max(60, env_int("ATHENA_NOUS_MIN_KEY_TTL_SECONDS", 1800)),
            )
            else ""
        )
        ...
        if not api_key:
            creds = resolve_nous_runtime_credentials(
                timeout_seconds=float(_getenv("ATHENA_NOUS_TIMEOUT_SECONDS", "15")),
            )
            api_key = creds.get("api_key", "")
    ...
    *,
    ...
    if pool and pool.has_credentials():
        ...
        if provider == "nous" and entry is not None:
            min_ttl = max(60, env_int("ATHENA_NOUS_MIN_KEY_TTL_SECONDS", 1800))
            nous_state = {
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "athena-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from athena_cli.auth import resolve_minimax_oauth_runtime_credentials
