"""Post-setup install hooks and installed-state predicates for `athena tools` provider rows."""
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    Athena-managed ``$ATHENA_HOME/bin`` copy does."""
...
def _post_setup_lightpanda() -> None:
    ...
    if lightpanda_bin:
        ...
        if os.name == "nt":
            _print_info("    Lightpanda has no native Windows build; run Athena under WSL2.")
...
def _post_setup_agent_browser(post_setup_key: str) -> None:
    ...
    global/Athena-managed install), so there is no ``npm install`` step here."""
    ...
    try:
        ...
        # PATH/node_modules probes above. Mirror the rung athena_cli.doctor uses so this probe can't diverge
        ...
        # agent-browser is no longer a root package.json dependency (#43564) — it resolves lazily via npx
        # (or a global/Athena-managed install) instead of a local `npm install`, so there's no node_modules/
    ...
    # Reuse the runtime resolution cascade (PATH -> Homebrew/Athena-managed node -> npx) rather than
    # a bare shutil.which — Athena-managed-Node-only setups resolve agent-browser/npx only that way.
    try:
...
_PIP_POST_SETUP_HOOKS: dict = {
    "faster_whisper": _pip_hook(
        ...
        on_install=("Model sizes: tiny, base (default), small, medium, large-v3",
                    "Change via stt.local.model in ~/.athena/config.yaml")),
    "kittentts": _pip_hook(
    ...
    "piper": _pip_hook(
        ...
        always=("Default voice: en_US-lessac-medium (downloaded on first TTS call)",
                ...
                "Switch voices by setting tts.piper.voice in ~/.athena/config.yaml")),
    "ddgs": _pip_hook(
...
def _post_setup_spotify() -> None:
    # Full `athena auth spotify` flow: no client_id yet → interactive wizard (persists to ~/.athena/.env)
    ...
    try:
        ...
        _info_lines("Run manually: athena auth spotify")
        return
    ...
    try:
        ...
        _info_lines("Run later: athena auth spotify")
    except Exception as exc:
...
def _post_setup_langfuse() -> None:
    ...
    try:
        ...
        _info_lines("Run manually: athena plugins enable observability/langfuse")
    _info_lines("Restart Athena for tracing to take effect.", "Verify: athena plugins list")
...
def _post_setup_xai_grok() -> None:
    ...
    try:
        from athena_cli.setup import prompt_choice, prompt as _setup_prompt
        from athena_cli.setup_tts import _run_xai_oauth_login_from_setup
        from athena_cli.config import save_env_value
    except Exception as exc:
        _print_warning(f"    Could not load setup helpers: {exc}")
        _info_lines("Run later: athena auth add xai-oauth   (or set XAI_API_KEY)")
        return
    ...
    if idx == 0:
        if _run_xai_oauth_login_from_setup():
            ...
            _print_warning("    xAI Grok OAuth login did not complete. Run later: athena auth add xai-oauth")
    elif idx == 1:
        ...
        if api_key:
            ...
            _print_warning("    No API key provided. Run later: athena auth add xai-oauth")
    else:
...
def run_post_setup_command(args) -> int:
    """``athena tools post-setup <key>`` — non-interactive runner the dashboard spawns so the GUI can drive
...
# Python deps installed via ``athena tools`` aren't in the managed runtime's locked ``all`` sync, so a
...
def active_restorable_python_tool_dependencies() -> list[str]:
    """Return ``athena tools`` Python dependencies present in this runtime."""
    return [
...
def _agent_browser_installed() -> bool:
    ...
    *and* the Chromium build it drives (or the Lightpanda engine, which needs no Chromium), so "Run
    ...
    from athena_cli.nous_subscription import _local_browser_runnable
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from athena_cli.nous_subscription import _has_agent_browser
