...
store at ``~/.athena/checkpoints/``.  Actions:

    athena checkpoints               # same as `status`
    athena checkpoints status        # total size, project count, breakdown
    athena checkpoints list          # per-project checkpoint counts + workdir
    athena checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    athena checkpoints clear [-f]    # nuke the entire base (asks first)
    athena checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    athena checkpoints
    athena checkpoints prune --retention-days 3 --max-size-mb 200
    athena checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: athena checkpoints clear-legacy")
    return 0
