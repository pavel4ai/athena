...
Split out of ``athena_cli.models``. OpenRouter and
...
``athena_cli.models`` — tests reset them there — and are read/written by attribute name.
...
# that must never block on HTTP — so without a disk copy, `athena -p`, a cron job, or a freshly
...
def _reasoning_caps_disk_path() -> Path:
    ...
    return get_athena_home() / "cache" / "reasoning_caps.json"
...
def _fetch_reasoning_caps_catalog(url: str, timeout: float) -> Optional[Caps]:
    ...
    m = _origin()
    try:
        req = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": m._ATHENA_USER_AGENT})
...
class _CapsSource:
    """One catalog's cache slots on ``athena_cli.models`` plus how to name its URL.
