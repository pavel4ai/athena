...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with athena-agent;
   ...
   own discovery paths)
2. **User plugins**   – ``~/.athena/plugins/<name>/``
3. **Project plugins** – ``./.athena/plugins/<name>/`` (opt-in via
   ``ATHENA_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``athena_agent.plugins``
   entry-point group.
...
from athena_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE


def get_bundled_plugins_dir() -> Path:
    """Locate the bundled ``plugins/`` directory.

    Honours ``ATHENA_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
...
# ---------------------------------------------------------------------------
#
# Set ``ATHENA_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.athena/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("ATHENA_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When ATHENA_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.
    ...
    logger.debug(
        "ATHENA_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # Kanban task lifecycle hooks. Fired by athena_cli.kanban_db when a task
    ...
    # separate `athena -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `athena kanban dispatch`),
...
ENTRY_POINTS_GROUP = "athena_agent.plugins"

_NS_PARENT = "athena_plugins"
...
class PluginManifest:
    ...
    #              in ~/.athena/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        """Return the active Athena profile name (e.g. ``"default"``).

        Derived from ``ATHENA_HOME`` via
        :func:`athena_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.athena/profiles/<name>``, or ``"custom"`` when
        ``ATHENA_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`athena_cli.dashboard_auth.DashboardAuthProvider`. Used by
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        # Lazy import to avoid circular: athena_cli.main imports plugins indirectly
        from athena_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.athena/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if self._discovered and not force:
            return
        # Safe mode (--safe-mode / ATHENA_SAFE_MODE=1): troubleshooting run
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to athena_cli/. Two layouts are
        ...
        # 2. User plugins (~/.athena/plugins/)
        user_dir = get_athena_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.athena/plugins/)
        if _env_enabled("ATHENA_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".athena" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set ATHENA_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with athena and must
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``athena_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``athena_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="athena-plugin-command-await",
        daemon=True,
    )
    thread.start()
