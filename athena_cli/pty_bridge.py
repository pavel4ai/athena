...
``athena_cli.web_server``. POSIX-only: depends on ``fcntl``, ``termios`` and ``ptyprocess`` (native
...
# ui-tui/packages/athena-ink/src/ink/termio/host.ts — keep the two in sync.
PTY_HOST_ENV = "ATHENA_PTY_HOST"
