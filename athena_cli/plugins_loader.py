...
dependency/config-schema warnings. Mixed into :class:`athena_cli.plugins.PluginManager`.

Origin-internal names (``PluginContext``, ``LoadedPlugin``, ``_PLUGINS_DEBUG`` …) are imported lazily
through ``athena_cli.plugins`` so tests that patch them on the origin keep working.
"""
...
from athena_cli.plugins_discovery import ENTRY_POINTS_GROUP, _select_entry_point_group
from athena_cli.plugins_manifest import PluginManifest, manifest_key, validate_config_schema
from athena_cli.plugins_state import _plugin_settings_entry

if TYPE_CHECKING:  # pragma: no cover
    from athena_cli.plugins import LoadedPlugin

logger = logging.getLogger("athena_cli.plugins")
...
def _plugin_home_scope(home: Path):
    """Bind discovery and loading to the manager's immutable Athena home."""
...
class PluginLoaderMixin:
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``athena plugins list`` until then."""
        from athena_cli.plugins import LoadedPlugin
    ...
    def _register_deferred_platform_tools(self, manifest: PluginManifest, loaded: LoadedPlugin) -> None:
        ...
        would miss them in ``athena tools`` / ``platform_toolsets``. Opt-in is explicit via ``provides_tools``;
        ...
        returns ``[]``, the toolset is missing from the ``athena tools`` checklist, and even an explicit
        ...
        """
        from athena_cli.plugins import PluginContext, _PLUGINS_DEBUG
        ...
        try:
            ...
            # Tools registered before the raise are live: credit them or `athena plugins list` under-reports
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        See #64165.
        python_dependencies is a declaration seam ONLY: Athena validates and prints the requirements with an
        ...
        if missing:
            logger.warning(
                ...
                "installed: %s. Athena does not install plugin dependencies "
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        from athena_cli.plugins import LoadedPlugin, PluginContext, _PLUGINS_DEBUG
        ...
        if manifest.portable:
            ...
            return
        # requires_athena gate: skip cleanly (no import, no traceback) on a version mismatch.
        from athena_cli.plugins_manifest import requires_athena_error
        reason = requires_athena_error(manifest)
        if reason:
            loaded.error = reason
        ...
        # skipped with a clear reason instead of dying on ImportError mid-register (athena_cli.plugin_compat).
        from athena_cli.plugin_compat import disable_reason
        ...
        try:
            ...
            if register_fn is None:
                ...
                loaded.enabled = True
                from athena_cli.plugins_ledger import _hook_source_of
    ...
    def _attribute_registrations(
        ...
        # Discovery-time tools predate registration_start; credit them back or `athena plugins list`
    ...
    def _load_portable_plugin(self, manifest: PluginManifest, loaded: LoadedPlugin) -> None:
        ...
        lookup_key = manifest_key(manifest)
        try:
            from athena_cli.agent_plugins import load_agent_plugin
            package = load_agent_plugin(
                Path(manifest.path), get_athena_home() / "plugin-data" / manifest.skill_namespace)
    ...
    def _directory_module_name(self, manifest: PluginManifest) -> str:
        """Profile-safe import namespace for a directory plugin: the bare ``athena_plugins.<slug>`` for the
    ...
    def _load_directory_module(
        ...
        """Import a directory plugin as ``athena_plugins.<slug>`` (slug from ``manifest.key`` so
        ...
        # Evict stale entries for this slug (same slug cached from another Athena home, or an earlier force
