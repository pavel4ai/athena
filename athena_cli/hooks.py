"""athena hooks — inspect and manage shell-script hooks.

Usage::

    athena hooks list
    athena hooks test <event> [--for-tool X] [--payload-file F]
    athena hooks revoke <command>
    athena hooks doctor

Consent records live under ``~/.athena/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.athena/config.yaml``
...
def hooks_command(args) -> None:
    ...
    if not sub:
        print("Usage: athena hooks {list|test|revoke|doctor}")
        print("Run 'athena hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.athena/config.yaml.")
