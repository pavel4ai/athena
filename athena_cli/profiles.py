...
Each profile is a fully independent ATHENA_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.athena/profiles/<name>/`` by default.

The "default" profile is ``~/.athena`` itself — backward compatible,
zero migration needed.

Usage::

    athena profile create coder          # fresh profile + bundled skills
    athena profile create coder --clone  # also copy config, .env, SOUL.md, skills
    athena profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    athena -p coder chat                 # or via flag
    athena profile use coder             # set as sticky default
    athena profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See athena_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.athena``).  Named profiles never contain these
...
# a profile's root, callers of seed_profile_skills() (fresh-create, `athena
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         ...
         (``~/.athena``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_athena_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.athena) profile.
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the athena root, NOT to the current ATHENA_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ATHENA_HOME points outside
    ``~/.athena``, profiles live under ``ATHENA_HOME/profiles/`` so
...
def _get_default_athena_home() -> Path:
    """Return the default (pre-profile) ATHENA_HOME path.

    In standard deployments this is ``~/.athena``.
    In Docker/custom deployments where ATHENA_HOME is outside ``~/.athena``
    (e.g. ``/opt/data``), returns ATHENA_HOME directly.
    """
...
def validate_profile_name(name: str) -> None:
    ...
    collisions (a ``athena`` profile inside ``~/.athena/``) or get refused
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, athena subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Athena. If we
    ...
    try:
        ...
        # not be migrated. The next `athena doctor --fix` can still surface the
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, athena_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own ATHENA_HOME.
    ...
    The returned ``athena_home`` is the path to pass to
    ``set_athena_home_override`` when scoping a turn to that profile.
    """
...
def create_profile(
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.athena)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.athena/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Cloned configs can be older than the running Athena (or predate schema
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ATHENA_HOME at module level.
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.athena).\n"
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``ATHENA_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from athena_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ATHENA_HOME so _profile_suffix resolves correctly
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.athena/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: athena profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ATHENA_HOME.

    Returns ``"default"`` if ATHENA_HOME is not set or points to ``~/.athena``.
    Returns the profile name if ATHENA_HOME points into ``~/.athena/profiles/<name>``.
    Returns ``"custom"`` if ATHENA_HOME is set to an unrecognized path.
    """
    from athena_constants import get_athena_home
    athena_home = get_athena_home()
    resolved = athena_home.resolve()

    default_resolved = _get_default_athena_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.athena itself — its parent is ~/ and its
        # directory name is ".athena", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    # dir.  Importing as "default" would target ~/.athena itself — disallow
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    candidates = [
        new_dir / "honcho.json",
        _get_default_athena_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("athena_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ATHENA_HOME path string.

    Called early in the CLI entry point, before any athena modules
    are imported, to set the ATHENA_HOME environment variable.
    """
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    profile_dir = get_profile_dir(canon)
