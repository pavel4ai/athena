...
from athena_cli.archive_safe import archive_root_dirs, make_targz, normalize_archive_parts, safe_extract_targz
from athena_constants import clear_named_profile_deleted, mark_named_profile_deleted, named_profile_is_deleted
...
# stay visible; containers persist HOME state here). See athena_constants.get_subprocess_home().
...
# (``~/.athena``): git checkout (+ ~3 GB venv), worktrees, sibling profiles, shared bins,
...
# (+wal/shm, can reach many GB), session dirs, `athena backup` archives, quick-backup
...
# Marker written by `athena profile create --no-skills`. When present at a profile root,
# seed_profile_skills() callers (fresh-create, `athena update` all-profile sync, the
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    if source_resolved == _get_default_athena_home().resolve():
...
# Allow-list for ``export_profile("default")``: when ATHENA_HOME equals the cwd
# (Docker/custom deployments) the default home holds arbitrary user files that must NOT
# be bundled. Only known Athena profile artifacts at the root survive; sensitive runtime
...
# when introduced in ``athena_constants``.
# See #58394.
...
_RESERVED_NAMES = frozenset({"athena", "default", "test", "tmp", "root", "sudo"})

# Athena subcommands that cannot be used as profile names/aliases
_ATHENA_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    """Named-profiles root, anchored to the athena root (NOT the current ATHENA_HOME, which
    may itself be a profile) so ``coder profile list`` sees all profiles."""
    return _get_default_athena_home() / "profiles"


def _get_default_athena_home() -> Path:
    """Default (pre-profile) ATHENA_HOME: ``~/.athena``, or ATHENA_HOME itself in
    Docker/custom deployments (e.g. ``/opt/data``)."""
    from athena_constants import get_default_athena_root
    return get_default_athena_root()


def _get_active_profile_path() -> Path:
    return _get_default_athena_home() / "active_profile"
...
def _is_our_wrapper(path: Path) -> bool:
    """True when *path* reads as a Athena-generated wrapper (contains ``athena -p``)."""
    try:
        return "athena -p" in path.read_text(encoding="utf-8")
    except Exception:
        return False


def _missing_profile_error(canon: str) -> FileNotFoundError:
    return FileNotFoundError(f"Profile '{canon}' does not exist. Create it with: athena profile create {canon}")
...
def validate_profile_name(name: str) -> None:
    ...
    """
    if name == "default":
        return  # special alias for ~/.athena
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its ATHENA_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_athena_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    if canon in _ATHENA_SUBCOMMANDS:
        return f"'{canon}' conflicts with a athena subcommand"
    try:
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    try:
        if sys.platform == "win32":
            ...
            athena_exe = shutil.which("athena") or "athena"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(athena_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    if not (profile_dir / "config.yaml").exists():
        return
    # Creation must not fail over an unmigratable old config; `athena doctor --fix` surfaces
    # the detailed error in the target profile.
    with contextlib.suppress(Exception):
        from athena_constants import reset_athena_home_override, set_athena_home_override
        from athena_cli.config import check_config_version, migrate_config
...
# wrappers are a few hundred bytes with the ``athena -p X`` needle near the top; the wrapper
...
def _seed_model_config(profile_dir: Path) -> None:
    ...
    if config_path.exists():
        return
    with contextlib.suppress(Exception):  # creation must not fail over this; `athena model` sets it later
        import yaml
        from athena_constants import get_athena_home
        from athena_cli.config import read_user_config_raw
        source = get_athena_home() / "config.yaml"
...
def _check_gateway_running(profile_dir: Path) -> bool:
    """Gateway liveness for a profile dir, never mutating ATHENA_HOME.
...
def _served_by_running_multiplexer(profile_name: str) -> bool:
    ...
    """
    try:
        from athena_cli.gateway import named_profile_served_by_running_multiplexer
...
# Deliberately tiny and separate from ``config.yaml`` (user-facing Athena config, ~5000
...
def list_profiles() -> List[ProfileInfo]:
    ...
    profiles = []
    default_home = _get_default_athena_home()
...
def profiles_to_serve(multiplex: bool, profile_allowlist: Optional[List[str]] = None) -> List[Tuple[str, Path]]:
    """``(profile_name, athena_home)`` pairs a gateway should serve — the single chokepoint
    ...
    *profile_allowlist* (invalid entries skipped, missing ones warned once)."""
    ...
    serve: List[Tuple[str, Path]] = [("default", _get_default_athena_home())]
...
def create_profile(
    ...
    ``no_skills`` creates an empty profile and writes a marker so ``athena update`` skips
    ...
    canon = _canon_valid(name)
    if canon == "default":
        raise ValueError("Cannot create a profile named 'default' — it is the built-in profile (~/.athena).")
    profile_dir = get_profile_dir(canon)
    ...
    # profile-scoped env writes (dashboard Channels/Keys pages, `athena -p <name> auth add`)
    ...
    # Opt-out marker read by seed_profile_skills() and `athena update`'s all-profile sync
    # (the feature still works via the empty skills/ dir if this fails).
    if no_skills:
        _seed_file_if_missing(
            profile_dir / NO_BUNDLED_SKILLS_MARKER,
            "This profile opted out of bundled-skill seeding (`athena profile create --no-skills`).\n"
    ...
    if description and description.strip():
        with contextlib.suppress(Exception):  # non-fatal — `athena profile describe` works later
    ...
    # `athena -p <profile> gateway start` supervises via `s6-svc -u` instead of a bare
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    """Seed bundled skills into a profile via subprocess (sync_skills() caches ATHENA_HOME at
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ATHENA_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    """
    backfilled: List[str] = []
    default_env = _get_default_athena_home() / ".env"
    for entry in _iter_named_profile_dirs():
        env_path = entry / ".env"
        if env_path.exists():
            continue
...
_ATHENA_ARGV_MARKERS = ("athena_cli.main", "athena-gateway", "tui_gateway")
...
# `findOnPath('athena')`) spawns the shim by handing the interpreter its path — then the
# OS-reported argv[0] is the interpreter, not "athena".
...
# argv[1] is matched against exact names, not ``startswith("athena")``: with a bare
# interpreter argv[0], argv[1] can be ANY user script ("athena-notes.py").
_ATHENA_CONSOLE_SCRIPT_NAMES = frozenset({"athena", "athena-agent", "athena-acp"})


def _is_athena_argv(argv: list) -> bool:
    """True for a Athena process: entrypoint marker in argv, executable named ``athena*``,
    or a python interpreter directly exec'ing a known ``athena`` console-script shim."""
    joined = " ".join(argv)
    exe_name = os.path.basename(argv[0]).lower()
    if any(marker in joined for marker in _ATHENA_ARGV_MARKERS) or exe_name.startswith("athena"):
        return True
    if len(argv) >= 2 and _PYTHON_INTERPRETER_RE.match(exe_name):
        script_name = os.path.basename(str(argv[1])).lower()
        return script_name.rsplit(".", 1)[0] in _ATHENA_CONSOLE_SCRIPT_NAMES
    return False
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Athena *backends* bound to this profile (``gateway.pid`` only tracks
    ...
    # Never terminate ourselves or a parent (`athena -p <canon> profile delete` runs under
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            if not argv or not _is_athena_argv(argv):
                continue
            if not ({tok.lower() for tok in argv} & _BACKEND_TOKENS):
                continue

            # Bound to THIS profile by selector flag, or by ATHENA_HOME pointing at its dir.
            ...
            if not bound:
                with contextlib.suppress(Exception):  # environ() can raise AccessDenied even same-user
                    env_home = (proc.environ() or {}).get("ATHENA_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    canon = normalize_profile_name(name)
    if canon == "default":
        raise ValueError("Cannot delete the default profile (~/.athena).\nTo remove everything, use: athena uninstall")
    ...
    with contextlib.suppress(Exception):
        from athena_state_registry import close_all_under as _close_session_dbs_under
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    Port selection: each supervised profile gateway loads its own ``ATHENA_HOME`` and binds the port
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # ATHENA_HOME is set temporarily so _profile_suffix resolves the service name.
    old_home = os.environ.get("ATHENA_HOME")
    try:
        os.environ["ATHENA_HOME"] = str(profile_dir)
        from athena_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        os.environ.pop("ATHENA_HOME", None)
        if old_home is not None:
            os.environ["ATHENA_HOME"] = old_home
...
def _stop_gateway_process(profile_dir: Path) -> None:
    ...
    try:
        ...
        pid = int(data["pid"])
        # Cross-profile kill refusal: the record's athena_home stamp names the gateway's TRUE
        ...
        if recorded_gateway_home_conflicts(data, expected_home=profile_dir):
            print(
                f"✗ Refusing to stop PID {pid}: its recorded ATHENA_HOME "
...
def get_active_profile_name() -> str:
    """Profile name inferred from ATHENA_HOME: ``"default"`` when unset or ``~/.athena``, the
    name under ``~/.athena/profiles/<name>``, ``"custom"`` for any other path."""
    from athena_constants import get_athena_home
    resolved = get_athena_home().resolve()
    if resolved == _get_default_athena_home().resolve():
        return "default"
...
def _inside_git_checkout(path: Path) -> bool:
    ...
    (not cwd) so the check holds when ATHENA_HOME sits in a checkout but the process runs
...
def _profile_export_directory() -> Path:
    ...
    import tempfile
    export_dir = _get_default_athena_home() / "profile-exports"
    if not _inside_git_checkout(export_dir):
        return export_dir

    # A custom deployment may point ATHENA_HOME at its source checkout: use a sibling store,
    # falling back to the OS temp dir only when the user's home itself is a checkout (dotfiles
    # repo). Per-uid temp name: a fixed /tmp/athena-profile-exports is a predictable shared
    # path another local user could pre-create (or symlink) first.
    uid_suffix = f"-{os.getuid()}" if hasattr(os, "getuid") else ""
    candidates = (
        Path.home() / ".athena-profile-exports", Path(tempfile.gettempdir()) / f"athena-profile-exports{uid_suffix}"
    )
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in ``_DEFAULT_EXPORT_INCLUDE_ROOT``
    ...
    ATHENA_HOME equals the cwd) is excluded. Blacklisting was tried first and proved unable to anticipate
    every non-Athena file the user may have lying alongside ATHENA_HOME (#58394). * **Universal exclusions
...
def _scrub_export_secrets(staged: Path) -> None:
    """Force-redact secret-shaped strings in a staged export tree (same pass as ``athena
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    # The default profile IS ~/.athena (dir name ".athena"), so both paths stage a filtered
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: athena profile import <archive> --name <name>"
        )
    ...
    # target ~/.athena itself.
    canon = _canon_valid(inferred_name)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.athena). "
            "Specify a different name: athena profile import <archive> --name <name>"
        )
    profile_dir = get_profile_dir(canon)
    if profile_dir.exists():
        raise FileExistsError(f"Profile '{canon}' already exists at {profile_dir}")
    _get_profiles_root().mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="athena_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"athena_{old_name}"
    legacy_old_host = f"athena.{old_name}"
    new_host = f"athena_{new_name}"
    candidates = [
        new_dir / "honcho.json", _get_default_athena_home() / "honcho.json", Path.home() / ".honcho" / "config.json"
    ]
    seen: set[Path] = set()
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            block["aiPeer"] = old_name  # source_host is ``athena_<old>`` or legacy ``athena.<old>``
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ATHENA_HOME path string. Called early in the CLI entry
    point, before athena modules are imported, to set ATHENA_HOME.

    When ATHENA_HOME is already set, the configured spelling IS the launch root (it may be a
    junction/symlink alias of the platform default). Keep that spelling so profile re-home does not destroy
    the launcher's lexical provenance -- the subprocess sanitizer needs it to match Athena-owned PYTHONPATH
    ...
    env_home = os.environ.get("ATHENA_HOME", "").strip()
    if env_home:
