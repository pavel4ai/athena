...
_REDACTION_BANNER = (
    "[athena debug share: log content redacted at upload time. "
...
_USER_AGENT = "athena-agent/debug-share"
...
# ``athena debug`` sweeps on entry (CLI-only users). Replaced a fork-and-sleep subprocess that
# leaked ~20 MB per share.


def _pending_file() -> Path:
    return get_athena_home() / "pastes" / "pending.json"
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        pass  # non-fatal — worst case the user runs ``athena debug delete`` manually
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from athena_cli.logs import LOG_FILES
    filename = LOG_FILES.get(log_name)
    return (get_athena_home() / "logs" / filename) if filename else None
...
_CLIENT_SIDE_LOGS = {
    "desktop": (
        "written by Athena Desktop on the machine running the app, not by this "
...
def collect_debug_report(
    *, log_lines: int = 200, dump_text: str = "",
    ...
    ``dump_text`` is pre-captured dump output; when empty, ``athena dump`` is run internally.
    """
    buf = io.StringIO()
...
_NOUS_BUNDLE_FORMAT = "athena-debug-share/1"
...
def build_debug_share(
    ...
    Shared by ``athena debug share`` and the dashboard ``POST /api/ops/debug-share``. Blocking
    ...
    if redact:
        logger.info(
            "athena debug share: applied force-mode redaction to log snapshots before upload")
...
⚠️  --nous: This uploads your debug bundle to Nous-INTERNAL storage (AWS S3),
  ...
  • System info (OS, Python/Athena version, provider, which API keys are
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """``athena debug share --nous``: gzip the same bundle into the Nous envelope → Nous-S3."""
    ...
    bundle = collect_share_bundle(log_lines=log_lines, redact=redact)
    if redact:
        logger.info("athena debug share --nous: applied force-mode redaction before upload")
    ...
    try:
        ...
        print(f"\nNous upload failed: {exc}\n"
              ...
              "Run `athena debug share --local` to print the report instead, "
              "or `athena debug share` to upload to a public paste service.\n", file=sys.stderr)
        sys.exit(1)
    ...
    print("\nShare this private link with the Nous team — only Nous staff "
          ...
          "  GitHub Issues        https://github.com/pavel4ai/athena/issues\n"
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: athena debug delete <url> [<url> ...]\n"
              "  Deletes paste.rs pastes uploaded by 'athena debug share'.")
        return
    for url in urls:
        try:
            if delete_paste(url):
