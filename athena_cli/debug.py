...
Currently supports:
    athena debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.athena/logs/*.log`` are not leaked into
...
def _pending_file() -> Path:
    """Path to ``~/.athena/pastes/pending.json``.
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    The replacement is stateless: we append to ``~/.athena/pastes/pending.json``
...
def run_debug_share(args):
    ...
    print(f"To delete now:  athena debug delete <url>")
...
def run_debug_delete(args):
    ...
    if not urls:
        ...
        print("  Deletes paste.rs pastes uploaded by 'athena debug share'.")
        return
...
def run_debug(args):
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: athena debug <command>")
        print()
        print("Commands:")
