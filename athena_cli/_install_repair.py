...
Callers: ``_early_recovery.recover_if_needed`` (stdlib-only, runs BEFORE ``athena_cli.main``'s
third-party imports so a pending update completes while no native extension is mapped yet) and
``athena_cli.main_install_repair._recover_core_update_marker_locked`` (the post-import recovery path).
...
from athena_cli import _early_recovery as _er
...
def _is_termux_env(env: dict | None = None) -> bool:
    """Stdlib Termux probe (athena_cli.main's version lives behind imports)."""
...
def _stdout_to_stderr():
    ...
    ``athena acp`` speaks JSON-RPC on stdout; an inherited-fd install child writing there would
...
def _venv_scripts_dir(root: Path) -> Path | None:
    """Project venv Scripts/bin dir, when present (athena_constants is stdlib-only)."""
    # athena_constants is stdlib-only, so the canonical layout helpers are safe to use from this
...
#: Athena root's ``bin``, next to uv.exe). Keep in lockstep with scripts/install.ps1.
_WINDOWS_BIN_LAUNCHERS = ("athena", "athena-acp")
...
def _default_athena_root() -> Path | None:
    """The DEFAULT Athena root (not ``get_athena_home()``, which under ``athena -p <name>`` is
    ...
    try:
        return Path(get_default_athena_root())
    except Exception:
        return None
...
def ensure_windows_bin_launchers(
    ...
    r"""Re-stage the Windows ``athena`` launchers when they vanish.

    On Windows, ``athena`` resolves through launchers derived from the venv console scripts — never
    ...
    The canonical launcher home is the managed binary dir — the default Athena root's ``bin``
    (``%LOCALAPPDATA%\\athena\\bin``, next to the managed uv) — which lives OUTSIDE the git checkout so no
    git operation can ever touch it. It is a per-machine dir shared by every profile: ``get_athena_home()``
    would point inside ``profiles\\<name>`` under ``athena -p``, so the anchor here is
    :func:`athena_constants.get_default_athena_root`. See #83797.
    """
    if windows is None:
        windows = _is_windows()
    if not windows:
        return []
    root = Path(root)
    home = _default_athena_root()
    if home is None:
        return []

    targets: list[Path] = []
    # Runs at every athena_cli.main process start, so the healthy path must stay a few stat calls.
    ...
    if restored:
        ...
        with contextlib.suppress(OSError, ValueError):
            print("  ✓ Restored athena launcher(s): " + ", ".join(restored), file=sys.stderr)
    return restored
...
def migrate_windows_bin_path(
    ...
    """One-time PATH migration to the ``ATHENA_HOME\\bin`` launcher layout (``athena update`` tail).
    ...
    *read_user_path*/*write_user_path* are injectable for tests.
    ...
    if kept != entries:
        try:
            ...
            return False
        with contextlib.suppress(OSError, ValueError):
            print(f"  ✓ athena launchers now resolve from {home_bin} "
...
def _quarantine_running_athena_exe(
    ...
    """Rename live athena*.exe shims aside so the installer can rewrite them.
    ...
    names = set(_load_console_script_names(scripts_dir.parent.parent)) or {
        "athena", "athena-agent", "athena-acp"}
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    ...
    (#75584). Warnings land on stderr — this module runs in the early-recovery path and ``athena acp``
...
def _run_install_cmd(cmd: list[str], *, env: dict | None, root: Path) -> None:
    ...
    failed: list[str] = []
    moved = _quarantine_running_athena_exe(scripts_dir, failed_out=failed) if scripts_dir else []
    if failed:
    ...
    try:
        ...
        # the shims renamed aside and `athena` gone from PATH. Restore only renames back when the
