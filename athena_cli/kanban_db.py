...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``ATHENA_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``ATHENA_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
In standard installs ``<root>`` is ``~/.athena``. In Docker / custom
deployments where ``ATHENA_HOME`` points outside ``~/.athena`` (e.g.
``/opt/athena``), ``<root>`` is ``ATHENA_HOME``. Legacy env-var
overrides still work:

* ``ATHENA_KANBAN_DB`` — pin the database file path directly.
* ``ATHENA_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``ATHENA_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``ATHENA_KANBAN_DB``,
``ATHENA_KANBAN_WORKSPACES_ROOT``, and ``ATHENA_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/athena-kanban-v1-spec.pdf`` for the full
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active ATHENA_HOME so dispatcher- and
    worker-side hooks both carry the right profile without the caller plumbing
    it through.
    """
    try:
        from athena_cli.plugins import invoke_hook
...
# ``ATHENA_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``ATHENA_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("ATHENA_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``ATHENA_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("ATHENA_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``ATHENA_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "ATHENA_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "athena_kanban_current_board_override",
    default=None,
)
...
def kanban_home() -> Path:
    ...
    1. ``ATHENA_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_athena_root()``, which already returns ``<root>``
       when ``ATHENA_HOME`` is ``<root>/profiles/<name>``, and returns
       ``ATHENA_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``ATHENA_HOME`` would silently fork the board per profile,
...
def get_current_board() -> str:
    ...
    1. ``ATHENA_KANBAN_BOARD`` env var (set by the dispatcher on worker
    ...
    env = os.environ.get("ATHENA_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``ATHENA_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("ATHENA_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``ATHENA_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("ATHENA_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``ATHENA_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
...
class Task:
    ...
    # within an agent loop that propagated ``ATHENA_SESSION_ID``. NULL for
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("ATHENA_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``athena gateway run --replace`` /
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``ATHENA_KANBAN_DB`` env → ``ATHENA_KANBAN_BOARD`` env →
    ...
    # holder (e.g. an external `athena kanban list` mid-integrity-probe) block
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See athena_state._WAL_INCOMPAT_MARKERS for detection logic.
                from athena_state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``ATHENA_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``ATHENA_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the Athena checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, Athena treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, Athena creates/reuses that linked worktree. With
      no ``workspace_path``, Athena anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, Athena uses
      ``wt/<task-id>``.
...
# without thrashing. Overridable via ``ATHENA_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``athena kanban tail``. The next dispatch
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    if not rows:
        return False
    try:
        from athena_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _dispatch_once_locked(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from athena_cli.profiles import profile_exists as _pe
...
def _module_athena_argv() -> list[str]:
    ...
    # ``athena_cli.main`` is the console-script target declared in
...
def _athena_path_argv(path: str) -> list[str]:
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_athena_argv()
    return [_absolute_athena_path(path)]


def _resolve_athena_argv() -> list[str]:
    """Resolve the ``athena`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$ATHENA_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("athena")`` — the console-script shim, normalized to
       ...
       ``.\\athena.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m athena_cli.main`` — fallback for setups where
       Athena is launched from a venv and the ``athena`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_athena_bin`` for the same reason. Kept
    local (not imported from gateway) because ``athena_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("ATHENA_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _athena_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _athena_path_argv(resolved_env_bin)
        return _module_athena_argv()

    athena_bin = _safe_which_no_cwd("athena") if _IS_WINDOWS else shutil.which("athena")
    if athena_bin:
        return _athena_path_argv(athena_bin)
    return _module_athena_argv()
...
def _resolve_worker_cli_toolsets(athena_home: Optional[str]) -> Optional[list[str]]:
    ...
    task-scoped kanban lifecycle tools when ``ATHENA_KANBAN_TASK`` is set.
    """
    if not athena_home:
        return None
    try:
        ...
        token = set_athena_home_override(athena_home)
        try:
            cfg = load_config()
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for ATHENA_HOME=%r (%s)",
            athena_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    ``ATHENA_KANBAN_DB`` / ``ATHENA_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject ATHENA_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `athena -p <name>` the
    # _apply_profile_override() runs *before* athena_constants is imported.
    # If ATHENA_HOME is absent from the child's env, get_athena_home() falls
    # back to Path.home() / ".athena" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from athena_cli.profiles import resolve_profile_env
    try:
        env["ATHENA_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via ATHENA_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # ATHENA_HOME never had profiles created.
        pass
    if task.tenant:
        env["ATHENA_TENANT"] = task.tenant
    env["ATHENA_KANBAN_TASK"] = task.id
    env["ATHENA_KANBAN_WORKSPACE"] = workspace
    ...
    if task.branch_name:
        env["ATHENA_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["ATHENA_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["ATHENA_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["ATHENA_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["ATHENA_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # rewrites ATHENA_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_athena_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["ATHENA_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["ATHENA_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["ATHENA_KANBAN_BOARD"] = resolved_board
    # ATHENA_PROFILE is the author the kanban_comment tool defaults to.
    # `athena -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["ATHENA_PROFILE"] = profile_arg

    cmd = [
        *_resolve_athena_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped ATHENA_HOME above,
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled ATHENA_PROFILE values like "athena-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    ``athena_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from athena_constants import get_default_athena_root
