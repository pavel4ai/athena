"""Bounded product contract for the first Athena shared-metrics slice."""
...
SCHEMA_KEY = "athena.metrics.schema_version"
SCHEMA_VERSION = "athena.metrics.event.v2"
MODEL_CALL_SCOPE = "athena.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "athena.task_run"
TOOL_CALL_SCOPE = "athena.tool_call"
CLIENT_ACTIVE_MARK = "athena.client.active"
TOOL_APPROVAL_MARK = "athena.tool_approval"
SKILL_LIFECYCLE_MARK = "athena.skill.lifecycle"
SKILL_LOAD_MARK = "athena.skill.load"
SUBSCRIBER_NAME = "athena.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "athena.client.active"
LEGACY_MODEL_CALL_METRIC = "athena.model_call.count"
MODEL_ROUTE_METRIC = "athena.model_route.count"
TASK_STARTED_METRIC = "athena.task_run.started"
TASK_FINISHED_METRIC = "athena.task_run.finished"
TOOL_CALL_METRIC = "athena.tool_call.count"
TOOL_APPROVAL_METRIC = "athena.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "athena.skill.lifecycle.count"
SKILL_LOAD_METRIC = "athena.skill.load.count"
...
CLIENT_RESOURCE_KEYS = frozenset({"architecture", "athena_version", "install_method", "os_family"})
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Athena installation method."""
    normalized = _norm(value)
    return _allowlisted("nixos" if normalized == "nix" else normalized, CLIENT_INSTALL_METHODS)


def client_resource(
    athena_version: Any, *, os_name: Any, architecture: Any, install_method: Any
) -> dict[str, str]:
    """Build the bounded client resource attached to aggregate packages."""
    version = str(athena_version or "").strip()
    return {
        "architecture": client_architecture(architecture),
        "athena_version": version if 0 < len(version) <= 64 else "unknown",
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("athena_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Athena logical scope."""
    metadata = _relay_metadata(
        event, RUNTIME_SCHEMA_KEY, RUNTIME_SCHEMA_VERSION, "athena.call_role"
    )
    call_role = (metadata or {}).get("athena.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from athena_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Athena terminal state to bounded (outcome, end_reason, termination)."""
...
_TOOL_CATEGORY_PREFIXES = (
    ...
    (("discord", "email", "feishu", "athena-yuanbao", "slack", "sms"), "communication"),
)


def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Athena registry toolset metadata to a low-cardinality category."""
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Athena tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Athena."""
