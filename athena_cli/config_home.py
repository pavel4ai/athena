"""Directory initialization and storage diagnostics for the active Athena home."""
...
def _ensure_directory(path: Path, *, create: bool, secure: bool) -> None:
    ...
    try:
        ...
        raise HomeInitializationError(
            f"Cannot initialize Athena directory {path}: {exc}. "
            ...
            "Athena has not replaced the link or created its missing target."
        ) from exc


def initialize_home(home: Path, subdirs: tuple[str, ...], ensured: set[str]) -> None:
    from athena_cli.config import _ensure_default_soul_md, is_managed
    ...
    try:
        ...
        try:
            ...
            raise HomeInitializationError(
                f"Cannot initialize Athena home {home}: {exc}. "
...
def config_load_issue(exc: Exception):
    from athena_cli.config import ConfigIssue

    if isinstance(exc, (HomeInitializationError, OSError)):
        return ConfigIssue(
            "error", f"Athena storage is unavailable: {exc}",
