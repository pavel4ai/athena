...
from athena_cli.pty_session import PtySessionRegistry

# Same logger the code used before extraction (record parity).
_log = logging.getLogger("athena_cli.web_server")


# /api/pty spawns ``athena --tui`` behind a pseudo-terminal and forwards bytes +
...
def _ws_host_origin_reason(ws: "WebSocket") -> Optional[str]:
    ...
    """
    from athena_cli.web_server import _is_accepted_host, app
...
_GATEWAY_WS_PROTOCOL = "athena-gateway-v1"
_GATEWAY_WS_TICKET_PROTOCOL_PREFIX = "athena-gateway-ticket."
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    """
    from athena_cli.web_server import _SESSION_TOKEN, app
    ...
    if auth_required:
        ...
        try:
            ...
            if protocol_ticket:
                ...
                ws._athena_ws_subprotocol = _GATEWAY_WS_PROTOCOL
...
def _resolve_chat_argv(
    ...
    """Resolve the argv + cwd + env for the chat PTY (what ``athena --tui`` runs).

    Tests monkeypatch this with a tiny fake command.  Env contract: resume goes
    through ``ATHENA_TUI_RESUME`` (``ui-tui`` does not parse argv), resolved to
    the newest descendant; ``ATHENA_TUI_GATEWAY_URL`` attaches to this process's
    ...
    ``profile`` scopes the ENTIRE chat by pointing ``ATHENA_HOME`` at the profile
    dir, the same propagation ``athena -p <name>`` performs.
    """
    from athena_cli.web_server_profiles import _config_profile_scope, _resolve_profile_dir
    from athena_cli.web_server_sessions import _open_session_db_for_profile, _session_latest_descendant
    from athena_cli.main import PROJECT_ROOT
    from athena_cli.main_tui_launch import _apply_tui_python_env, _make_tui_argv
    ...
    # scope overrides ATHENA_HOME before config is bridged into the env.
    ...
    # which inherits the profile ATHENA_HOME set above.
...
def _resolve_client_ws_host() -> Optional[str]:
    """Host the in-container WS client should dial: ``ATHENA_DASHBOARD_WS_HOST``
    wins always; a wildcard bind becomes ``127.0.0.1``; others verbatim."""
    from athena_cli.web_server import app
    explicit = os.environ.get("ATHENA_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
async def _resolve_chat_argv_async(
    ...
    from athena_cli.web_server import _get_chat_argv_lock, app
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    from athena_cli.web_server import _get_pty_active_session_files
    files = _get_pty_active_session_files(app)
    if files.get(channel) is None:
        fd, raw_path = tempfile.mkstemp(prefix="athena-pty-active-", suffix=".json")
        os.close(fd)
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    max_workers=_CONSOLE_EXECUTOR_MAX_WORKERS, thread_name_prefix="athena-console")
