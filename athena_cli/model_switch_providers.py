...
Split out of ``athena_cli/model_switch.py``; every moved name is re-imported there so
``athena_cli.model_switch.<name>`` keeps resolving (and monkeypatching) as before."""
...
from athena_cli.providers import custom_provider_aliases, custom_provider_slug, get_label
from utils import base_url_host_matches

# Log-record parity with the origin module.
logger = logging.getLogger("athena_cli.model_switch")
...
def _save_discovered_models_to_config(
    ...
    from athena_cli.model_switch import _extra_headers_from_config
...
def _discovered_catalog_stale(entry: dict, model_ids: list[str]) -> bool:
    ...
    A mapping Athena itself discovered (entry flag or legacy in-mapping sentinel) is ours to
...
def _fetch_picker_live_models(
    ...
    from athena_cli.models import _get_ollama_native_headers, cached_fetch_api_models, fetch_api_models
    from athena_cli.models_local import (
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from athena_cli.models import update_provider_cache_entry
...
def _iter_builtin_candidates(models_dev_data: dict, excluded: set, seen: set):
    """Yield ``(athena_id, mdev_id, pconfig, env_vars)`` for section-1 rows.
    ...
    from athena_cli.auth import PROVIDER_REGISTRY, is_runtime_provider_routable
    from athena_cli.models import _AGGREGATOR_PROVIDERS
    from athena_cli.providers import ALIASES
    for athena_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        alias_target = ALIASES.get(athena_id)
        if alias_target and alias_target != athena_id and alias_target in _AGGREGATOR_PROVIDERS:
            continue
        try:
            from providers import get_provider_profile
            prof = get_provider_profile(athena_id)
            if prof is not None and prof.name != athena_id:
                continue
        except Exception:
            pass
        if athena_id.lower() in seen:
            continue
        if athena_id.lower() in excluded or mdev_id.lower() in excluded:
            continue
        pdata = models_dev_data.get(mdev_id)
        if not isinstance(pdata, dict):
            continue
        pconfig = PROVIDER_REGISTRY.get(athena_id)
        if (pconfig and pconfig.auth_type != "api_key") or not is_runtime_provider_routable(athena_id):
            continue
        env_vars = list(pconfig.api_key_env_vars) if pconfig and pconfig.api_key_env_vars else pdata.get("env", [])
        if isinstance(env_vars, list):
            yield athena_id, mdev_id, pconfig, env_vars


def _auth_store_has_provider(*keys: str) -> bool:
    """True when ``auth.json`` has a ``providers`` entry under any of *keys*."""
    try:
        from athena_cli.auth import _load_auth_store
...
def _raw_pool_usable(athena_id: str) -> bool:
    """Section-1 pool check: only consult the pool when auth.json lists a raw entry."""
    try:
        from athena_cli.auth import _load_auth_store
        store = _load_auth_store()
        if store and store.get("credential_pool", {}).get(athena_id):
            return _credential_pool_is_usable(athena_id, raw_pool_present=True)
    except Exception:
        pass
    return False
...
def _overlay_has_env_creds(pid: str, athena_slug: str, overlay, read_env) -> bool:
...
def _live_or_curated_ids(slug: str, curated: dict, *fallback_keys: str, merge_models_dev: bool = True) -> list:
    """``cached_provider_model_ids`` (the SAME disk-cached list ``athena model`` builds), falling
    back to the curated list (merged with models.dev for preferred providers) when live is empty."""
    from athena_cli.models import _MODELS_DEV_PREFERRED, _merge_with_models_dev, cached_provider_model_ids
...
def _nous_picker_model_ids(curated: dict, force_fresh_nous_tier: bool) -> list:
    ...
    try:
        from athena_cli.models_pricing import get_pricing_for_provider
        from athena_cli.models import (
        ...
        pass
    try:
        from athena_cli.models_pricing import nous_policy_allowed_ids, restrict_to_nous_policy
...
def _display_prefix(name: str) -> str:
    """Text before the per-model separator Athena's own writer uses ("—" / " - ")."""
...
def _discover_endpoint_models(
    ...
    if probe_live:
        ...
        try:
            from athena_cli.models import cached_fetch_api_models
...
def _collect_authed_provider_slugs(
    ...
    (heavier detection)."""
    from athena_cli.model_switch import _scoped_key_env
    from agent.models_dev import PROVIDER_TO_MODELS_DEV
    from athena_cli.auth import PROVIDER_REGISTRY
    from athena_cli.providers import ATHENA_OVERLAYS
    from athena_cli.models import CANONICAL_PROVIDERS
    ...
    for athena_id, _mdev_id, _pconfig, env_vars in _iter_builtin_candidates(models_dev_data, excluded_set, seen):
        if _any_env(env_vars, _scoped_key_env) or _raw_pool_usable(athena_id):
            _emit(athena_id, athena_id)

    mdev_to_athena = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in ATHENA_OVERLAYS.items():
        athena_slug = mdev_to_athena.get(pid, pid)
        if _skip(seen, excluded_set, pid, athena_slug) or overlay.auth_type == "aws_sdk":
            continue
        if (
            _overlay_has_env_creds(pid, athena_slug, overlay, _scoped_key_env)
            or _auth_store_has_provider(pid, athena_slug) or _pool_usable(athena_slug)):
            _emit(athena_slug, pid, athena_slug)
...
class _PickerBuild:
    ...
    models.dev, 2 Athena-only overlays, 2b canonical providers missed by 1/2, 3 ``providers:``
    entries + 3b the bare active custom endpoint, 4 ``custom_providers:`` entries. Row-builder
    imports of ``athena_cli.auth/models`` stay lazy so tests can patch those modules."""
...
def _lap_builtin_rows(b: _PickerBuild, data: dict, user_providers: dict) -> None:
    ...
    from athena_cli.model_switch import _declared_model_ids
    from agent.models_dev import get_provider_info
    for athena_id, mdev_id, pconfig, env_vars in _iter_builtin_candidates(data, b.excluded, b.seen_slugs):
        if not (_any_env(env_vars) or _raw_pool_usable(athena_id)):
            continue
        model_ids = _live_or_curated_ids(athena_id, b.curated)
        ...
        configured = user_providers.get(athena_id) if isinstance(user_providers, dict) else None
        ...
        b.add_builtin_row(
            athena_id, display_name, b.current_provider in (athena_id, mdev_id), model_ids, "built-in")


def _overlay_has_creds(b: _PickerBuild, pid: str, athena_slug: str, overlay) -> bool:
    ...
    if overlay.auth_type == "aws_sdk":
        has_creds = _has_aws_sdk_creds_for_listing(athena_slug, b.current_provider)
    else:
        has_creds = _overlay_has_env_creds(pid, athena_slug, overlay, os.environ.get)
    ...
    if not has_creds and overlay.auth_type == "external_process":
        try:
            ...
            _ext_status = get_auth_status(athena_slug) or {}
    ...
    has_creds = has_creds or _auth_store_has_provider(pid, athena_slug)
    if not has_creds:
        # Full auto-seeding pool check catches external stores (Codex CLI ~/.codex/auth.json)
        # not yet in auth.json.
        try:
            if _credential_pool_is_usable(athena_slug):
                has_creds = True
            elif b.for_picker:
                ...
                try:
                    from agent.credential_pool import load_pool
                    has_creds = load_pool(athena_slug).has_credentials()
                except Exception:
                    pass
        except Exception as exc:
            logger.debug("Credential pool check failed for %s: %s", athena_slug, exc)
    if not has_creds and athena_slug == "anthropic":
        ...
        try:
            from agent.anthropic_credentials import read_claude_code_credentials, read_athena_oauth_credentials
            athena_creds = read_athena_oauth_credentials()
            cc_creds = read_claude_code_credentials()
            if (athena_creds and athena_creds.get("accessToken")) or (cc_creds and cc_creds.get("accessToken")):
                has_creds = True
        except Exception as exc:
...
def _lap_overlay_rows(b: _PickerBuild, data: dict) -> None:
    """Section 2: Athena-only providers (nous, openai-codex, copilot, opencode-go, ...)."""
    from agent.models_dev import PROVIDER_TO_MODELS_DEV
    from athena_cli.providers import ATHENA_OVERLAYS

    # ATHENA_OVERLAYS keys may be models.dev IDs ("github-copilot") while config.yaml uses
    # Athena IDs ("copilot").
    mdev_to_athena = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in ATHENA_OVERLAYS.items():
        athena_slug = mdev_to_athena.get(pid, pid)
        if _skip(b.seen_slugs, b.excluded, pid, athena_slug):
            continue
        if not _overlay_has_creds(b, pid, athena_slug, overlay):
            continue
        if athena_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            from athena_cli.models import cached_provider_model_ids
            model_ids = cached_provider_model_ids(athena_slug)
        elif overlay.auth_type == "aws_sdk":
            model_ids = _aws_live_or_curated_ids(athena_slug, b.curated, athena_slug, pid)
        elif athena_slug == "nous":
            ...
            tier_row = _free_tier_nous_row({"name": get_label(athena_slug), "models": []})
            ...
            model_ids = _live_or_curated_ids(athena_slug, b.curated, athena_slug, pid)
        b.add_builtin_row(
            athena_slug, get_label(athena_slug), b.current_provider in (athena_slug, pid), model_ids, "athena")
...
def _lap_user_provider_rows(b: _PickerBuild, user_providers: dict) -> None:
    ...
    from athena_cli.model_switch import _extra_headers_from_config, _scoped_key_env
    from athena_cli.config import coerce_provider_id, is_provider_enabled
...
def _lap_custom_provider_rows(b: _PickerBuild, custom_providers: list) -> None:
    ...
    their own rows."""
    from athena_cli.model_switch import _extra_headers_from_config, _scoped_key_env
    from athena_cli.config import coerce_provider_id
...
def _build_curated_lists(current_provider: str, current_base_url: str, current_model: str) -> dict[str, list[str]]:
    """Curated model lists keyed by athena provider id, plus the dynamic ones (nous manifest,
    Ollama Cloud, LM Studio live probe)."""
    from athena_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS, get_curated_nous_model_ids
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # Remote manifest so new Portal models surface without a release; in-repo snapshot fallback.
    curated["nous"] = get_curated_nous_model_ids()
    if "ollama-cloud" not in curated:
        from athena_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (os.environ.get("LM_API_KEY") or os.environ.get("LM_BASE_URL") or is_current_lmstudio):
        from athena_cli.models_local import fetch_lmstudio_models
...
def list_authenticated_providers(
    ...
    ("built-in", "athena", "canonical", "user-config", "model-config").
    ...
    from athena_cli.config import coerce_provider_id, stringify_provider_map
    ...
    # as (athena_id / mdev_id / canonical slug).
    b = _PickerBuild(
    ...
    if current_model:
        for row in results:
            ...
            if current_model not in models:
                from athena_cli.models import _model_requires_account_discovery
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    try:
        from athena_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    OpenRouter's list is replaced with :func:`athena_cli.models.fetch_openrouter_models` (curated
    ...
    from athena_cli.model_switch import list_authenticated_providers
    from athena_cli.models import fetch_openrouter_models
