...
# Install-scoped breadcrumbs live next to the venv (not under $ATHENA_HOME)
...
def _stdout_to_stderr():
    ...
    a protocol stream (``athena acp`` speaks JSON-RPC on stdout) must never get install noise."""
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``athena update``.
...
def _recover_core_update_marker_locked() -> None:
    ...
    # Windows: a ``athena.exe`` launch has the launcher as an ancestor; the quarantined full
    ...
    self_locked = _windows_running_athena_launcher_locked()
    if self_locked:
        install_prefix, install_env = _default_venv_install_target()
        print(
            "  → Running from athena.exe; applying package-only first aid, "
    ...
    try:
        from athena_cli import _install_repair as _ir
        ...
        manual = (
            "  Athena is still running from the launcher that needs "
            "replacing. Close other Athena windows, restart from a "
...
def _windows_shim_in_process_chain() -> Path | None:
    ...
    ``venv\\Scripts\\athena.exe`` holds itself open (no ``FILE_SHARE_DELETE``) for the whole
    ...
    with the project venv's own shims so a foreign ``athena.exe`` never matches.
    ...
    if scripts_dir is None:
        return None
    shims = {_norm_exe_path(shim): shim for shim in _athena_exe_shims(scripts_dir)}
    if not shims:
        return None
...
def _windows_running_athena_launcher_locked() -> bool:
    """True when a venv ``athena*.exe`` shim is this process or an ancestor (best-effort)."""
    return _windows_shim_in_process_chain() is not None


# Set on the re-exec'd child so it can never spawn another one.
_UPDATE_REEXEC_ENV = "ATHENA_UPDATE_REEXEC"
...
def _reexec_dependency_sync_off_windows_shim() -> bool:
    ...
    ``.update_exit_code``. The child re-runs ``athena update`` so the sync and its tail happen
    ...
    user's own console.
    ``venv\\Scripts\\athena.exe`` is a launcher that runs the interpreter with the shim as its script and
    ...
    cmd = [str(python_exe), "-m", "athena_cli.main", *sys.argv[1:]]
...
def _run_install_with_heartbeat(
    ...
    try:
        ...
        # is imported when `athena update` starts, so an update running
...
def _venv_scripts_dir() -> Path | None:
    ...
    from athena_cli.main import PROJECT_ROOT
    from athena_constants import project_venv_dir, venv_bin_dir
...
def _quarantine_running_athena_exe(
    ...
    """Pre-empt the Windows file lock on the running ``athena.exe``.
    ...
    transient; Athena Desktop backend child: until closed) — retry with backoff, then warn
...
def _cleanup_pending_shim_renames(scripts_dir: Path) -> int:
    """Drop reboot renames older Athena versions queued for our shims: ``MOVEFILE_DELAY_UNTIL_REBOOT``
    ...
    try:
        ...
        with winreg.OpenKey(
            ...
            if value_type != winreg.REG_MULTI_SZ or not isinstance(entries, list):
                return 0
            kept, removed = _filter_pending_shim_renames(entries, _athena_exe_shims(scripts_dir))
            if not removed:
                return 0
            if kept:
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_athena_exe`` if uv didn't write replacements. Safety-
    critical: a failed quarantine only aborts an update; a failed restore leaves no ``athena``
...
class ShimQuarantineError(RuntimeError):
    """A live ``athena*.exe`` shim could not be renamed aside. Raised by
...
def _run_quarantined_install(
    ...
    Every editable install rewrites the entry-point shims; on Windows the live ``athena.exe``
    can be neither deleted nor overwritten, so without quarantine ``athena`` drops off PATH.
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_athena_exe(scripts_dir, failed_out=failed)
...
# rewrites no entry points, which would otherwise leave the shims quarantined aside and `athena` missing
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep — and where necessary RESCUE — ``athena.exe.old.*`` from updates.

    Called early on every invocation. Two cases an unconditional ``unlink()`` gets wrong:
    (1) orphan rescue — ``athena.exe`` missing while ``athena.exe.old.*`` exists means the .old
...
def _run_package_only_install(cmd: list[str], *, env: dict[str, str] | None = None) -> None:
    ...
    ``athena.exe``, and the quarantine path would rename shims uv then never recreates.
...
def _repair_venv_via_import_probes(
    ...
    ``athena.exe``. Never raises. Returns ``"healthy"``, ``"repaired"``, ``"failed"``
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `athena update`:")
...
def _interpreter_scripts_dir() -> Path | None:
    ...
    See #76105.
    """
    from athena_constants import venv_bin_dir
...
def _install_python_dependencies_with_optional_fallback(
    ...
    attempt quarantines the live ``athena*.exe`` shims first. When ``env`` carries a
    ...
    if pin_python:
        ...
        # dir, not PROJECT_ROOT/venv; quarantining the wrong dir leaves athena.exe locked.
...
def _verify_console_scripts_installed(
    ...
    On Windows ``uv pip install -e .`` can register ``athena.exe`` in the wheel RECORD while the
    file never lands (live shim locked, launcher write skipped), so ``athena`` drops off PATH
    after a "successful" install. Missing shims get ``--reinstall -e .`` under quarantine.

    The symptom is ``athena-agent.exe`` and ``athena-acp.exe`` present but ``athena.exe`` missing, so
    ``athena`` drops off PATH even though the install reported success (issue #52931).
    """
    if not _is_windows():
        return
    scripts_dir = _venv_scripts_dir()
    ...
    if not _run_repair_step(
        ...
        fail_msg=(
            "  ⚠ Entry point repair failed; try `athena update --force` after "
            "closing other athena processes.")):
        return
    _report_still_missing(
        _missing(), "Workaround: python -m athena_cli.main <command>",
...
def _verify_core_dependencies_installed(
    ...
    # ``athena update``; the install prefix/env encode which environment we targeted.
    venv_python = _resolve_install_target_python(install_cmd_prefix, env)
    if venv_python is None:
        return
    ...
    if not _run_repair_step(
        ...
        fail_msg="  ⚠ Repair install failed; check `athena update` output above."):
        return
    ...
    if not _run_repair_step(
        ...
        return
    _report_still_missing(
        _missing_deps(), "Run `athena update --force` after closing other athena processes.",
