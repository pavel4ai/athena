...
Profile-aware location: ``$ATHENA_HOME/logs/dashboard-auth.log``.
...
from ``athena_constants`` or other athena_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$ATHENA_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``athena_constants.get_athena_home`` semantics: env var wins,
    else ``~/.athena``. A local copy avoids an import cycle with the
    middleware which lives below ``athena_cli``.
    """
    home = os.environ.get("ATHENA_HOME") or str(Path.home() / ".athena")
