...
Usage:
    athena pairing list              # Show all pending + approved users
    athena pairing approve <platform> <code>  # Approve a pairing code
    athena pairing revoke <platform> <user_id> # Revoke user access
    athena pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle athena pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: athena pairing {list|approve|revoke|clear-pending}")
        print("Run 'athena pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.athena/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'athena pairing list' to see pending codes.\n")
