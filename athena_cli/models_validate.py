...
Split out of ``athena_cli.models``. Catalog fetchers defined in ``athena_cli.models`` are looked up
there at call time (``_m.<name>``) so ``patch("athena_cli.models.<name>")`` mocks keep intercepting;
local-server probes are looked up on ``athena_cli.models_local`` (``_ml.<name>``) the same way.
...
def _validate_moa(req: _Request) -> dict[str, Any]:
    try:
        ...
        if req.requested in cfg["presets"]:
            return _accept()
        return _reject(f"MoA preset `{req.requested}` was not found. Run `athena moa list`.")
...
def _validate_lmstudio(req: _Request) -> dict[str, Any]:
    from athena_cli import models_local as _ml
...
def _ollama_probe_headers(req: _Request) -> dict[str, str]:
    ...
    from athena_cli import models as _m
    from athena_cli import models_local as _ml
    from athena_cli.models_local import _configured_ollama_base_url, _drop_authorization
...
def _validate_ollama_native(req: _Request) -> Optional[dict[str, Any]]:
    ...
    if models is None:
        return _soft_accept(
            ...
            "Athena will save the model name, but local Ollama model discovery could not verify it."
        )
...
def _validate_custom(req: _Request) -> dict[str, Any]:
    ...
    message = (
        ...
        f"Athena will still save `{req.requested}`, but the endpoint should expose `/models` for verification."
    )
    if anthropic_style:
...
def _validate_static_catalog(req: _Request) -> Optional[dict[str, Any]]:
    ...
    if req.normalized == "openai-codex":
        ...
        # the suffix is a Athena picker convention, so an unknown `*-900k` can never be a real
...
def _validate_live_listing(req: _Request) -> Optional[dict[str, Any]]:
    ...
    # docs-hosted manifest catches up; `athena chat` already accepts those at model-list build
