...
``athena logs [name] [-n N] [-f] [--level L] [--session S] [--component C] [--since 1h]``;
``athena logs list`` shows the available files.
"""
...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print("(Logs are created when Athena runs — try 'athena chat' first)")
        sys.exit(1)
    ...
    print(f"--- {display_athena_home()}/logs/{filename}{filter_desc} ({mode}) ---")
