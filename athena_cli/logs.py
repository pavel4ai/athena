...
Usage examples::
    ...
    athena logs                    # last 50 lines of agent.log
    athena logs -f                 # follow agent.log in real time
    athena logs errors             # last 50 lines of errors.log
    athena logs gateway -n 100    # last 100 lines of gateway.log
    athena logs gui -f            # follow gui.log (dashboard/pty/ws)
    athena logs desktop -f        # follow desktop.log (Electron app boot/backend)
    athena logs --level WARNING    # only WARNING+ lines
    athena logs --session abc123   # filter by session ID substring
    athena logs --component tools  # only tool-related lines
    athena logs --since 1h         # lines from the last hour
    athena logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_athena_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Athena runs — try 'athena chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_athena_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_athena_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_athena_home()}/logs/")
        return

    print(f"Log files in {display_athena_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'athena chat' to generate logs)")
