"""
athena fallback — manage the fallback provider chain.
...
Subcommands:
  athena fallback [list]   Show the current fallback chain (default when no subcommand)
  athena fallback add      Pick provider + model via the same picker as `athena model`,
                           then append the selection to the chain
  athena fallback remove   Pick an entry to delete from the chain
  athena fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.athena/config.yaml`` (top-level, list of
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  athena fallback add")
        print()
        return
    ...
    print("  Docs: https://github.com/pavel4ai/athena/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    ...
    from athena_cli.main import _require_tty, select_provider_and_model
