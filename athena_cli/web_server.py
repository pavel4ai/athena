...
Usage:
    python -m athena_cli.main web          # Start on http://127.0.0.1:9119
    python -m athena_cli.main web --port 8080
"""
...
from athena_cli.memory_providers import (
    MemoryProvider,
    ProviderField,
    get_memory_provider,
)
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    real gateway on the same ATHENA_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _warm_gateway_module() -> None:
    try:
        import athena_cli.gateway  # noqa: F401
    except Exception:
        pass
...
async def _lifespan(app: "FastAPI"):
    ...
    # Fire athena_cli.gateway import into a background thread so the event
    # loop is not blocked and ATHENA_DASHBOARD_READY fires without delay.
...
# ATHENA_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("ATHENA_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
...
#
# This list is defined in ``athena_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from athena_cli.dashboard_auth.public_paths import (
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``athena-0day``
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    """
    from athena_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from athena_cli.models import _AGGREGATOR_PROVIDERS
...
_MANAGED_FILES_ROOT_ENV = "ATHENA_DASHBOARD_FILES_ROOT"
...
def _default_athena_root_is_opt_data() -> bool:
    raw = os.environ.get("ATHENA_HOME", "").strip()
    if not raw:
        return False
    try:
        from athena_constants import get_default_athena_root

        root = get_default_athena_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``athena update``.
    ...
    a container — e.g. the athena-webui image sharing the Athena source tree),
    the dashboard's ``athena update`` button is the correct update path and
    ...
    """
    if _default_athena_root_is_opt_data():
        return True
    try:
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # (the container/hosted layout) or when ATHENA_DASHBOARD_FILES_ROOT is set.
    if _default_athena_root_is_opt_data():
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_athena_home() at call time
    ...
    try:
        ...
        # Windows install the first import of athena_cli.gateway blocks the
        ...
        # SPA's StatusPage can show "OAuth gate ON via Futurebound Corp." or
        # "loopback only — no auth gate" with no extra round trips.
        auth_required = bool(getattr(app.state, "auth_required", False))
        auth_providers: list[str] = []
        try:
            from athena_cli.dashboard_auth import list_providers as _list_providers
        ...
        status = {
            ...
            "can_update_athena": not _dashboard_local_update_managed_externally(),
            "gateway_running": gateway_running,
            "gateway_state": gateway_state,
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the athena home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_athena_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_athena_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from athena_cli.auth import get_nous_auth_status
    ...
    features = []
    try:
        from athena_cli.nous_subscription import get_nous_subscription_features
...
async def run_prompt_size():
    try:
        proc = _spawn_athena_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_athena_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_athena_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from athena_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.athena/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_athena_home() / "logs"
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "athena-update": "athena-update.log",
...
def _spawn_athena_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``athena <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``athena_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "athena_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "ATHENA_NONINTERACTIVE": "1"},
    }
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["athena", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    ...
    return _spawn_athena_action(subcommand, "gateway-restart"), False
...
@app.post("/api/athena/update")
async def update_athena():
    """Kick off ``athena update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "Athena updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("athena-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "athena-update",
    ...
    try:
        proc = _spawn_athena_action(["update"], "athena-update")
    except Exception as exc:
        _log.exception("Failed to spawn athena update")
...
@app.get("/api/athena/update/check")
async def check_athena_update(force: bool = False):
    """Report whether a Athena update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/athena/update`` actually runs ``athena update``.
    ...
    try:
        ...
        if force:
            try:
                (get_athena_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.athena/config.yaml`` under ``tts.``.
    """
...
def _memory_provider_config_path(provider: MemoryProvider) -> Path:
    return get_athena_home() / provider.name / "config.json"
...
# in athena_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.athena/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from athena_cli.model_cost_guard import expensive_model_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (athena_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from athena_cli.nous_subscription import apply_nous_managed_defaults
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from athena_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``athena model`` universe). This is what lets the desktop Keys tab
    ...
    """
    try:
        from athena_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from athena_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `athena model` provider — would be
...
async def get_env_vars(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict) -> dict:
        ...
        return {
            ...
            # CLI `athena model` picker uses (not desktop-only prefix guesses).
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Athena from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "email": {
        ...
        "docs_url": "https://github.com/pavel4ai/athena/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "weixin": {
        ...
        "docs_url": "https://github.com/pavel4ai/athena/docs/user-guide/messaging/weixin/",
    ...
    "webhook": {
        ...
        "docs_url": "https://github.com/pavel4ai/athena/docs/user-guide/messaging/webhooks/",
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in athena gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in athena gateway setup",
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's ATHENA_HOME.
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.athena/.anthropic_oauth.json`` — Athena-managed PKCE flow (what
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_athena_dotenv()``, so the same check covers them)
    ...
    try:
        ...
        read_athena_oauth_credentials = None  # type: ignore
        _ATHENA_OAUTH_FILE = None  # type: ignore

    athena_creds = None
    if read_athena_oauth_credentials:
        try:
            athena_creds = read_athena_oauth_credentials()
        except Exception:
            athena_creds = None
    if athena_creds and athena_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "athena_pkce",
            "source_label": f"Athena PKCE ({_ATHENA_OAUTH_FILE})",
            "token_preview": _truncate_token(athena_creds.get("accessToken")),
            "expires_at": athena_creds.get("expiresAt"),
            "has_refresh_token": bool(athena_creds.get("refreshToken")),
        }
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Athena never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "athena auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "athena auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "athena auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "athena auth add minimax-oauth",
    ...
    },
    {
        ...
        "flow": "loopback",
        "cli_command": "athena auth add xai-oauth",
        "docs_url": "https://github.com/pavel4ai/athena/docs/guides/xai-grok-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "athena auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from athena_cli import auth as hauth
        if provider_id == "nous":
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Athena, so the disconnect
    ...
    user sees exactly what executes, and Athena then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Athena — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``athena model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `athena model` universe (zero-edit for new plugins).
    try:
        from athena_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"athena auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("athena_pkce", "claude_code", ...)
    ...
    sync with the `athena model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the Athena-managed PKCE file and auth-store entry.
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _ATHENA_OAUTH_FILE
                if _ATHENA_OAUTH_FILE.exists():
                    _ATHENA_OAUTH_FILE.unlink()
                    cleared = True
            except Exception:
                pass
            # Also clear the credential pool entry if present.
            try:
                from athena_cli.auth import clear_provider_auth
        ...
        try:
            from athena_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
#          → persists to ~/.athena/.anthropic_oauth.json AND credential pool
...
# Guarded so athena web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    _ATHENA_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _ATHENA_OAUTH_FILE.with_name(
        f"{_ATHENA_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _ATHENA_OAUTH_FILE)
        try:
            _ATHENA_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
def _submit_anthropic_pkce(
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "athena-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("ATHENA_PORTAL_BASE_URL")
...
def _xai_loopback_worker(session_id: str) -> None:
    ...
    try:
        ...
        base_url = hauth._xai_validate_inference_base_url(
            os.getenv("ATHENA_XAI_BASE_URL", "").strip().rstrip("/")
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from athena_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from athena_cli.auth import _save_codex_tokens
...
async def prune_sessions_endpoint(body: SessionPrune):
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_athena_home()
    db = _open_session_db_for_profile(body.profile)
    try:
...
async def get_logs(
    ...
    from athena_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_athena_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from athena_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, ATHENA_HOME)."""
    from athena_cli import profiles as profiles_mod
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["athena_home"] = str(home)
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process ATHENA_HOME at import time. The dashboard is a single
...
#
# Wraps the same config data layer the CLI uses (athena_cli.mcp_config), so
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from athena_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from athena_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from athena_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    def _probe_scoped():
        ...
        # ATHENA_HOME override (see mcp_tool._wrap_with_home_override), so
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds ATHENA_HOME-derived paths in the child.
    if entry.install is not None:
        try:
            proc = _spawn_athena_action(
...
#
# Wraps the same JSON store the CLI uses (athena_cli.webhook); the webhook
...
async def list_webhooks():
    import athena_cli.webhook as wh
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_athena_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_athena_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from athena_cli.auth import read_credential_pool
...
async def run_doctor():
    try:
        proc = _spawn_athena_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_athena_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_athena_action(args, "backup")
    except Exception as exc:
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_athena_action(args, "import")
    except Exception as exc:
...
async def list_checkpoints():
    ...
    # Checkpoints live under <athena_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_athena_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_athena_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
async def preview_skill_hub(identifier: str = ""):
    ...
    def _run():
        from athena_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
class ProfileCreate(BaseModel):
    ...
    # because skills_hub.SKILLS_DIR is import-time-bound and the ATHENA_HOME
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_athena_home()
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local ATHENA_HOME override so the write lands in the target
    ...
    token = set_athena_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local ATHENA_HOME override (same mechanism as
    ...
    """
    from athena_constants import set_athena_home_override, reset_athena_home_override
    from athena_cli.mcp_security import validate_mcp_server_entry
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the ATHENA_HOME override. Returns the
    number of skills newly disabled.
    """
    from athena_constants import set_athena_home_override, reset_athena_home_override
    from athena_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's ATHENA_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    the running dashboard/gateway is scoped to (derived from ATHENA_HOME).
    """
    from athena_cli import profiles as profiles_mod
    try:
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the ATHENA_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_athena_home()`` at call
       time — the context-local override from ``set_athena_home_override``
    ...
    retargeted to the *current* ``get_athena_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a ATHENA_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_athena_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_athena_home()`` at call time
...
async def get_skills(profile: Optional[str] = None):
    ...
    from athena_cli.skills_config import get_disabled_skills
    with _profile_scope(profile):
        config = load_config()
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.athena/.env`` via ``save_env_value`` —
...
async def run_toolset_post_setup(
    ...
    write per-profile state must see the same ATHENA_HOME the rest of the
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Athena,
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``athena computer-use permissions grant`` as a background action.
...
if sys.platform.startswith("win"):
    try:
        from athena_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from athena_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from athena_cli.dashboard_auth.audit import AuditEvent, audit_log
...
def _resolve_chat_argv(
    ...
    Session resume is propagated via the ``ATHENA_TUI_RESUME`` env var —
    matching what ``athena_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``ATHENA_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``ATHENA_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `profile` (when set) scopes the ENTIRE chat to that profile by pointing
    ``ATHENA_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_athena_home()`` from that env var at its own import, so the child
    ...
    ``ATHENA_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from athena_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("ATHENA_TUI_DISABLE_MOUSE", "1")
    env.setdefault("ATHENA_TUI_INLINE", "1")
    env["ATHENA_TUI_DASHBOARD"] = "1"

    if profile_dir is not None:
        env["ATHENA_HOME"] = str(profile_dir)

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["ATHENA_TUI_RESUME"] = resume

    if sidecar_url:
        env["ATHENA_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile ATHENA_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["ATHENA_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from athena_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
# ATHENA_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`athena_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from athena_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/athena/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /athena`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__ATHENA_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/athena``)
        ...
        ``__ATHENA_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__ATHENA_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__ATHENA_BASE_PATH__="{prefix}";'
                f"window.__ATHENA_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__ATHENA_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__ATHENA_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__ATHENA_BASE_PATH__="{prefix}";'
                f"window.__ATHENA_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/athena`` they'd hit ``mission-control.tilos.com/fonts/...``
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.athena/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.athena/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_athena_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.athena/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    2. User plugins:    ~/.athena/plugins/<name>/dashboard/manifest.json
    3. Project plugins: ./.athena/plugins/  (only if ATHENA_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from athena_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (bundled_root / "memory", "bundled"),
        (bundled_root, "bundled"),
        (get_athena_home() / "plugins", "user"),
    ]
    ...
    # ``athena_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("ATHENA_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".athena" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from athena_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_athena_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"athena auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from athena_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from athena_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from athena_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from athena_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"athena_dashboard_plugin_{plugin['name']}"
...
def start_server(
    ...
    # the athena-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing ATHENA_DASHBOARD_OAUTH_CLIENT_ID).
    ...
    # ATHENA_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            print(f"ATHENA_DASHBOARD_READY port={actual_port}", flush=True)
