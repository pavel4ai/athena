"""Default configuration data for Athena Agent: DEFAULT_CONFIG and OPTIONAL_ENV_VARS.

Pure-data leaf module — must not import from athena_cli.config. Comments are the user-facing
docs of config.yaml.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["athena-cli"],
    # journal_mode: SQLite journal mode for every Athena DB. "wal" default; use "delete" on
    ...
    "session": {
        # Per-terminal `athena -c`: each CLI session writes a breadcrumb under
        # $ATHENA_HOME/terminal-sessions/<terminal-id>, so bare -c/--continue resumes THIS
    ...
    "agent": {
        ...
        # Default 30 min is a safety valve for wedged agents, not a target latency — an interactive `athena
        ...
        # Athena-level retry attempts for API errors (connection drops, timeouts, 5xx) wrapping the
        ...
        # wrapping Athena (sandbox runner, managed platform) can describe proxy/credential/ mount
        # layout without editing SOUL.md. Env ATHENA_ENVIRONMENT_HINT overrides it.
        "environment_hint": "",
        ...
        # share or `athena doctor` (status channel only; prompt cache untouched). 0 = no escalation
        ...
        # gateway status (`athena status` / fleet monitoring). Retries never stop — a signal, not a
        ...
        # ATHENA_LOCAL_STREAM_STALE_TIMEOUT overrides.
    ...
    "terminal": {
        ...
        # sandboxes). Empty = TMPDIR/TMP/TEMP if set, else ATHENA_HOME/cache/terminal (auto-pruned
        ...
        # ATHENA_HOME/home; "real" = force real HOME; "profile" = force ATHENA_HOME/home when it
        ...
        # ["/home/user/.athena/cache/documents:/output"]. For gateway MEDIA delivery, write to
        ...
        # default for images whose entrypoints must start as root (e.g. the bundled Athena image,
        # which drops to `athena` via s6-setuidgid). When on, SETUID/SETGID caps are omitted.
    ...
    "web": {
        ...
        # parallel, firecrawl, keenable; tavily is opt-in keyless via `athena tools`, not a ring
        # member). Set by the `athena tools` picker. "free" = always anonymous endpoint even with a
    ...
    "browser": {
        ...
        # Consent to browse with the user's REAL logins locally: runs on a Athena-managed SNAPSHOT
        # of the ACTIVE default-Chromium profile (Local State -> profile.last_used; cookies, logins,
        # prefs copied and re-synced per fresh session) driven by Athena' packaged Chromium. The
        # snapshot dir sidesteps Chrome 136+'s default-profile debugging block and never contends
        # with the running browser. Turning off deletes ~/.athena/browser-profile/ so credentials
        ...
        # runs `athena browser close-profile` (kills that profile's browser tree, unsaved tabs lost)
    ...
    # write_file/patch call); restore with /rollback. Opt-in via `athena chat --checkpoints` or
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ~/.athena/checkpoints/ size (MB); the oldest checkpoint per project
        ...
        # must not guess. Orphans: `athena checkpoints prune` (`--keep-orphans` to skip).
        "auto_prune": True,
        "retention_days": 7,
        "min_interval_hours": 24,
    },
    # Hard cap (chars) for one auto-loaded context file (SOUL.md, AGENTS.md, CLAUDE.md, .athena.md,
    ...
    # Same bound for single-query mode (``athena -q/-z``). With only ONE turn there is no
    ...
    "compression": {
        ...
        # Athena' summarizer cannot shrink it. native = codex decides; athena = Athena' threshold
    ...
    "auxiliary": {
        ...
        # of skills); route cheaper via `athena model` → auxiliary → Curator.
        "curator": _aux(600),
    ...
    "display": {
        ...
        # Interface bare `athena`/`athena chat` launches: "cli" (prompt_toolkit REPL) | "tui" (Ink).
        # Flags win: `--cli` forces the REPL, `--tui` / ATHENA_TUI=1 forces the TUI.
        "interface": "cli",
        # `athena --tui` auto-resumes the most recent human-facing session (like `athena -c`).
        # ATHENA_TUI_RESUME=<id> always wins.
        ...
        # `path`/`paths` = ATHENA_HOME-relative YAML files/dirs (or conventional status_phrases.yaml
        ...
        # Gateway runtime footer on the FINAL message, e.g. `model · 68% · ~/projects/athena`.
        ...
        # CLI/TUI/desktop, managed with `athena pets`. No effect on prompt caching.
        "pet": {
            "enabled": False,
            "slug": "",   # active pet slug in get_athena_home()/pets/; empty → first installed
    ...
    "dashboard": {
        ...
        # client disconnects. 0 = park forever. Env: ATHENA_TUI_WS_ORPHAN_REAP_GRACE_S.
        ...
        # newest message older than ATHENA_TUI_SESSION_TTL_S, default 6h) with
        ...
        # plugin. Env ATHENA_DASHBOARD_OAUTH_CLIENT_ID / ATHENA_DASHBOARD_PORTAL_URL win when
        ...
        # no-op. Env ATHENA_DASHBOARD_BASIC_AUTH_USERNAME / _PASSWORD_HASH / _PASSWORD / _SECRET /
        ...
        # ATHENA_DASHBOARD_DRAIN_SECRET; no-op unless >=256-bit, weak secrets rejected
        # (fail-closed). scope = capability label; min_secret_chars in url-safe-b64 chars.
        "drain_auth": {"scope": "drain", "min_secret_chars": 43},
        # Public URL (env ATHENA_DASHBOARD_PUBLIC_URL): full authority (scheme + host + optional
        # prefix, e.g. https://example.com/athena) for the OAuth redirect_uri; its hostname is
    ...
    "tts": {
        ...
        "piper": {
            ...
            # (~/.athena/cache/piper-voices/), use_cuda, length_scale (2.0 = twice as slow),
    ...
    # "Hey Athena" hands-free wake word: always-on, on-device hotword detection that starts a fresh
    ...
    "wake_word": {
        ...
        "openwakeword": {
            # "hey_athena" | built-in openWakeWord name ("hey_jarvis", "alexa", ...) | path to a
    ...
    # plugins/context_engine/<name>/ or ~/.athena/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # ~/.athena/cache/delegation/ with a head+tail window + read_file offset footer, nothing
    ...
    "moa": {
        ...
        # <athena_home>/moa-traces/<session_id>.jsonl (or trace_dir) for auditing.
        "save_traces": False,
        "trace_dir": "",
    ...
    # and resolved; read-only — creation goes to ~/.athena/skills/ unless create_dir redirects it.
    "skills": {
        ...
        # ATHENA_HOME, scanned alongside the local dir.
        "create_dir": "",
        # In a git checkout, <root>/.athena/skills/ and <root>/.agents/skills/ load as the
        # highest-precedence tier — ONLY if the root is in trusted_project_dirs. false = no scan, no
        # untrusted-skills notice.
        "project_discovery": True,
        # Trusted project roots; managed by `athena skills trust` / `untrust`.
        "trusted_project_dirs": [],
        # Substitute ${ATHENA_SKILL_DIR} / ${ATHENA_SESSION_ID} in SKILL.md content.
        "template_vars": True,
        ...
        # Advisory NVIDIA SkillEvaluator Tier 1 scan on `athena skills install` (alongside the
        ...
        "write_approval": False,
        # Audit ledger: every skill mutation appends to ~/.athena/skills/.curator_ledger.jsonl with
        # before/after hashes (blobs under ~/.athena/.curator_backups/blobs/); powers `athena
    ...
    # daemon. `athena curator status` shows the last run.
    "curator": {
        "enabled": True,
        ...
        # aux-model cost. `athena curator run --consolidate` overrides once.
        "consolidate": False,
        # Also prune bundled built-ins (a suppression list stops `athena update` restoring them);
        ...
        "prune_builtins": True,
        # TTL purge of skills/.archive/: 0 = never; > 0 lets the explicit `athena curator purge`
        # delete older archived skills (never automatic; logged in the ledger).
        "archive_ttl_days": 0,
        # Before every real (non-dry-run) pass, snapshot ~/.athena/skills/ to
        # ~/.athena/skills/.curator_backups/<utc-iso>/skills.tar.gz (`athena curator rollback`).
        "backup": {
            "enabled": True,
    ...
    # peerName, sessions, enabled); athena-specific overrides only here.
    "honcho": {},
    ...
    "whatsapp": {
        # reply_prefix: None = built-in "⚕ *Athena Agent*" header; "" disables; \n allowed.
    },
    ...
    #   cron job, a -q session (ATHENA_INTERACTIVE=1 but nobody to answer), or an unattended
    ...
    "approvals": {
        ...
        # export ATHENA_INTERACTIVE=1 (for interactive sudo prompts) but have NO user waiting to answer
        ...
        # Approve" → false. ATHENA_TUI_NO_CONFIRM=1 skips the TUI modal.
    ...
    # are given.
    "platform_hints": {},
    # Plugin system. `enabled`/`disabled` lists are written by `athena plugins enable|disable` and
    ...
    "plugins": {
        ...
        # 2026-09-14 removal date (see COMPAT_MANIFEST.md, `athena plugins compat`). Stopgap only: the
    ...
    # approvals persist in ~/.athena/shell-hooks-allowlist.json. Schema + examples:
    # website/docs/user-guide/features/hooks.md.
    "hooks": {},
    # Auto-accept shell-hook registrations without a TTY prompt (also --accept-hooks or
    # ATHENA_ACCEPT_HOOKS=1). Gateway/cron/non-interactive runs need one of these to pick up
    # newly-added hooks.
    ...
    "security": {  # Security: pre-exec scanning via tirith plus related guards.
        ...
        # .athena config) always need human approval, even under yolo. Extra patterns are fnmatch
        ...
        # banner. Add via `athena doctor --ack <id>`; remove by editing the list. Catalog:
        # athena_cli/security_advisories.py.
        "acked_advisories": [],
    ...
    "cron": {
        ...
        # an installed provider (plugins/cron_providers/<name>/ or $ATHENA_HOME/plugins/ <name>/),
        ...
        # Env override: ATHENA_CRON_MAX_PARALLEL.
        ...
        "output_retention": 50,
        # Timeout (seconds) for a no-agent cron script. Env: ATHENA_CRON_SCRIPT_TIMEOUT. Keep in
        ...
        # job's dispatch guard forever. Env: ATHENA_CRON_SESSION_DB_TIMEOUT. 0 = unlimited.
        "session_db_timeout_seconds": 10,
        # Timeout (seconds) per media attachment send during gateway delivery; large attachments
        # (long TTS audio, big exports) need more than 30s. Env: ATHENA_CRON_MEDIA_SEND_TIMEOUT.
    ...
    # promotes dependency-satisfied todos to ready, and fires `athena -p <assignee> chat -q ...` per
    ...
    "kanban": {
        ...
        # Auto-run the decomposer on Triage tasks every tick. False = manual via `athena kanban
    ...
    "code_execution": {  # execute_code settings (programmatic tool calls).
        ...
        # relative paths resolve. strict = isolated temp dir with athena-agent's own python
    ...
    # by tool_search / tool_describe / tool_call bridges and surfaced on demand. Core Athena tools
    ...
    "logging": {  # File logging to ~/.athena/logs/: agent.log captures INFO+, errors.log WARNING+.
    ...
    "model_catalog": {
        ...
        # CLI refetches on the next /model or `athena model` once the cache is older. Network
    ...
    # defaults (200K context, tools on, vision/reasoning off) and get patched. Provider keys: Athena
    ...
    "monitoring": {
        ...
        "gateway_health_export": {  # Gateway health & diagnostics export.
            ...
            "resource_attributes": {
                "service.name": "athena-gateway", "deployment.environment.name": "production"
            },
        },
    ...
    "gateway": {  # Gateway settings (messaging platforms: Telegram, Discord, Slack, ...).
        ...
        # to ATHENA_GATEWAY_PLATFORM_CONNECT_TIMEOUT, which wins if set explicitly.
        ...
        # run_gateway() bridges these to ATHENA_STARTUP_WATCHDOG / ATHENA_STARTUP_WATCHDOG_TIMEOUT_S
        # and re-arms the live handle; explicit env wins.
        "startup_watchdog": True,
        "startup_watchdog_timeout_seconds": 300,
        # Keep writing the legacy ~/.athena/sessions/sessions.json mirror of the routing index
        ...
        # (ATHENA_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent AND a wakeUrl is
        ...
        # ATHENA_GATEWAY_MAX_STARTS / ATHENA_GATEWAY_START_WINDOW_S.
        ...
        # credential/system denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.athena/.env, auth.json). True:
        # files must be under the Athena cache, media_delivery_allow_dirs, or fresher than
        # trust_recent_files_seconds — recommended for public-facing gateways so prompt injection
        # can't exfiltrate host secrets. Bridged to ATHENA_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra roots (project/scratch dirs, mounted shares) from which bare file paths may be
        # uploaded; the Athena cache is always trusted. List of absolute paths or one
        # os.pathsep-separated string; tildes expanded. Bridged to ATHENA_MEDIA_ALLOW_DIRS. Honored
        # in both modes.
        ...
        # pure-allowlist mode. Bridged to ATHENA_MEDIA_TRUST_RECENT_FILES. Only consulted when
        # strict is true.
        "trust_recent_files": True,
        # Recency window in seconds; 600 covers a multi-tool turn. Bridged to
        # ATHENA_MEDIA_TRUST_RECENT_SECONDS. Only consulted when strict is true.
    ...
    # Automatic cleanup of ~/.athena/state.db, which otherwise grows without bound and slows FTS5
    ...
    "sessions": {
        ...
        "auto_prune": True,
        # Inactive days of ended-session history to keep (= `athena sessions prune`).
        ...
        # stay until `athena sessions optimize-storage` runs, since the rebuild is disk-heavy on
        # large DBs. advise = `athena update` prints a one-line notice with reclaimable size when a
        ...
        # (native/fts5_cjk/build.sh → ~/.athena/lib/libfts5_cjk.so), 1-2 char CJK terms get exact
        # index matches instead of LIKE scans. True = use when present (inert otherwise); False =
        # never load/serve it. Bridged to ATHENA_CJK_FTS.
        "cjk_fts": True,
        ...
        # Max active messages per session for in-memory export (`athena sessions export`); checked
    ...
    "doctor": {
        # Per-probe timeout (seconds) for `athena doctor --live` real-call probes.
        "live_probe_timeout": 10,
    },

    "updates": {
        # Passive version/banner checks only; explicit `athena update --check` remains enabled.
        "check": True,
        # Pre-update backup. quick = snapshot small critical state (pairing JSONs, cron jobs,
        # config.yaml, .env, auth.json, profile DBs) into <ATHENA_HOME>/state-snapshots/, skipping
        # files >1 GiB; restore via ``/snapshot``. full = quick PLUS a ``athena backup`` zip in
        # <ATHENA_HOME>/backups/ (``athena import`` restores; slow on large homes; ``--backup``
        ...
        # update cleanly. `athena update --switch-branch` overrides to switch for one run.
        "parked_branch_strategy": "switch",
        # Refresh an installed cua-driver during `athena update` (best-effort, macOS only). Turn off
    ...
    "lsp": {
        ...
        "wait_mode": "document",
        "wait_timeout": 5.0,
        # Missing server binaries: auto = install via npm/go/pip into <ATHENA_HOME>/lsp/bin/ on
    ...
    # (SuperGrok OAuth or XAI_API_KEY) AND the toolset is enabled in `athena tools`.
    "x_search": {
    ...
    # encrypted vault (`athena vault add`, Desktop → Settings → Credential Vault) is always on.
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # ~/.athena/.env (or the shell), never in config.yaml.
            ...
            # Last-good fallback for NETWORK/TIMEOUT outages: AES-GCM cache under ~/.athena/cache/,
            ...
            # Auto-download bws into ~/.athena/bin/ on first use; False = bws must be on PATH.
            "auto_install": True,
    ...
    "computer_use": {
        # cua-driver's upstream PostHog telemetry defaults ON; Athena sets
        ...
        # #47032; Linux/WSL2 idle spin #28152). cua-driver ≥ 0.6.x supports --no-overlay; Athena also calls
    ...
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned binary into ~/.athena/bin/; False = iron-proxy on PATH.
        "auto_install": True,
    ...
    "desktop": {  # Athena Desktop (Electron) launch options; only affect `athena desktop`.
        ...
        # Bridged to ATHENA_DESKTOP_DISABLE_GPU: auto = disable GPU only on remote displays
        ...
        # to ATHENA_DESKTOP_PASSWORD_STORE; ignored off-Linux.
        "password_store": "auto",
    ...
    "nous": {
        ...
        "anthropic_wire": "chat",
        # Nous free tier: with no other provider configured, Athena sets up a free Nous identity on
        # first use (inference on nous/welcome + connectors) and offers `/login` (terminal:
        # `athena auth upgrade`) to sign in. false turns the free tier off entirely: nothing is set
    ...
    "local_runtime": {
        # Off = detection-only (Athena still finds an external llama-server you run).
        "enabled": False,
        # Pinned llama.cpp release tag; bumped by Athena releases after validation.
        "tag": "b10679",
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": _prov(
        ...
        "OAuth2, not a static API key — this points at the credentials Athena mints short-lived "
    ...
    "AZURE_FOUNDRY_BASE_URL": _prov(
        "Azure Foundry base URL (set via 'athena model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": _tool(
        ...
        "read from the Athena auth store)", "Tool-gateway user token", None, advanced=True),
    ...
    "PORCUPINE_ACCESS_KEY": _tool(
        "Picovoice access key for the Porcupine 'Hey Athena' wake word engine (optional; "
    ...
    "ATHENA_LANGFUSE_PUBLIC_KEY": _tool("Langfuse project public key (pk-lf-...)",
        "Langfuse public key", "https://cloud.langfuse.com", password=False),
    "ATHENA_LANGFUSE_SECRET_KEY": _tool("Langfuse project secret key (sk-lf-...)",
        "Langfuse secret key", "https://cloud.langfuse.com"),
    "ATHENA_LANGFUSE_BASE_URL": _tool("Langfuse server URL (default: https://cloud.langfuse.com)",
    ...
    "SLACK_ALLOWED_USERS": _msg(
        "Comma-separated Slack member IDs allowed to use Athena, e.g. U01ABC2DEF3. Without "
    ...
    "MATRIX_USER_ID": _msg("Matrix user ID (e.g. @athena:example.org)",
    ...
    "MATRIX_DEVICE_ID": _msg(
        "Stable Matrix device ID for E2EE persistence across restarts (e.g. ATHENA_BOT)",
    ...
    "IRC_CHANNEL": _msg("IRC channel to join (e.g. #athena)", "IRC channel", None),
    "IRC_NICKNAME": _msg("Bot nickname on IRC (default: athena-bot)", "IRC nickname", None),
    ...
    "API_SERVER_MODEL_NAME": _msg(
        "Model name advertised on /v1/models. Defaults to the profile name (or 'athena-agent' "
    ...
    "GATEWAY_PROXY_URL": _msg(
        "URL of a remote Athena API server to forward messages to (proxy mode). When set, the "
        ...
        "Remote Athena API server URL (e.g. http://192.168.1.100:8642)", None, advanced=True),
    "GATEWAY_PROXY_KEY": _msg(
        "Bearer token for authenticating with the remote Athena API server (proxy mode). Must "
    ...
    # ATHENA_TOOL_PROGRESS_MODE (deprecated; use display.tool_progress) is intentionally NOT listed:
    # this dict feeds user-facing surfaces (dashboard keys page, setup checklists), so deprecated
    # knobs stay in config._EXTRA_ENV_KEYS only. ATHENA_TOOL_PROGRESS is unsupported.
    "ATHENA_PREFILL_MESSAGES_FILE": _setting(
        "Path to JSON file with ephemeral prefill messages for few-shot priming",
        "Prefill messages file path", None),
    "ATHENA_EPHEMERAL_SYSTEM_PROMPT": _setting(
