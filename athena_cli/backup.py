...
`athena backup` creates a zip archive of the entire ~/.athena/ directory
...
from athena_constants import get_default_athena_root, get_athena_home, display_athena_home
...
# ATHENA_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    ...
    # Python dependency trees (plugin / MCP-server venvs under ATHENA_HOME) —
...
# File names that ``athena import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE ATHENA_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside ATHENA_HOME, resolved from config only (no network, no init).
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside ATHENA_HOME.
...
def run_backup(args) -> None:
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"athena-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"athena-backup-{stamp}.zip"
    ...
    for dirpath, dirnames, filenames in os.walk(athena_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(athena_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(athena_root)
    ...
    # outside ATHENA_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_athena_home()}."
        )
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    ...
    # Look for telltale files that a athena home would have
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.athena/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a athena dir name
        if prefix in {".athena", "athena"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_athena_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (athena_root / "config.yaml").exists()
        has_env = (athena_root / ".env").exists()
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under ATHENA_HOME.
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # athena_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: athena profile list  (after installing athena)")
...
# Critical state files to include in quick snapshots (relative to ATHENA_HOME).
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(athena_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside ATHENA_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    Args:
        ...
        athena_home: Override for the Athena home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(athena_home), keep=keep)
...
def run_quick_backup(args) -> None:
    ...
    if snap_id:
        ...
        print(f"  {len(snaps)} snapshot(s) stored in {display_athena_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, athena_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``athena_root`` to ``out_path``.
...
def _pre_update_backup_dir(athena_home: Optional[Path] = None) -> Path:
    home = athena_home or get_athena_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of ATHENA_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<ATHENA_HOME>/backups/pre-update-<timestamp>.zip`` and
    ...
    backup_dir = _pre_update_backup_dir(athena_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, athena_root)
    if result is None:
        return None
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of ATHENA_HOME under ``backups/`` before a
    ...
    ``<ATHENA_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
