...
def _print_unmanaged_summary() -> None:
    ...
    print("  never auto-staled or archived — `athena curator adopt <name>` hands one over")
...
_PIN_MESSAGES = {
    True: (
        ...
        "external). `athena curator list-unmanaged` shows which skills the curator tracks.",
        ...
        "it. Run `athena curator adopt {skill}` to put it under curator management)",
...
def _cmd_list_unmanaged(args) -> int:
    ...
    print("\nadopt one with `athena curator adopt <name>`, "
          "or all with `athena curator adopt --all-unmanaged`")
    return 0
...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at ~/.athena/skills/.curator_backups/{snap.name}")
    return 0
...
def _cmd_ledger(args) -> int:
    ...
    for r in rows:
        ...
        print(
            ...
            f"{r.get('skill', '?')}{extra}")
    print(
        "\nRoll back a single mutation with `athena curator rollback <id>`; "
        "whole-tree snapshots remain available via `athena curator rollback --list`.")
    return 0
...
def _rollback_ledger_entry(args, entry_id: str) -> int:
    ...
    if entry is None:
        print(
            ...
            "See `athena curator ledger` for entry ids, or use "
...
def _cmd_rollback(args) -> int:
    ...
    print(
        "\nThis will replace the current ~/.athena/skills/ tree (a safety "
...
_SUBCOMMANDS = (
    ...
    (
        "backup",
        "Take a manual tar.gz snapshot of ~/.athena/skills/ "
    ...
    (
        "rollback",
        "Restore ~/.athena/skills/ from a curator snapshot, or a single "
        "mutation by ledger entry id (see `athena curator ledger`)",
        _cmd_rollback,
        _arg("entry_id", nargs="?", default=None,
             help="Ledger entry id for single-mutation rollback (from "
                  "`athena curator ledger`). Omit for whole-tree snapshot rollback."),
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by athena_cli.main fallthrough)."""
