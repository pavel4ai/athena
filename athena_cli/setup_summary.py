...
resolved through the module object so test patches on ``athena_cli.setup.<name>`` take effect."""

import logging
from tools import tool_backend_helpers
from athena_cli import nous_subscription
...
_TTS_SUMMARY_ROWS = {
    ...
    "neutts": ("NeuTTS", "neutts", "run 'athena setup tts'"),
    "kittentts": ("KittenTTS", "kittentts", "run 'athena setup tts'")}
...
_STT_SUMMARY_DEFAULT = ("Local Whisper", "faster_whisper", "run 'athena tools' → Speech-to-Text")
...
_EDIT_WIZARD_ROWS = (
    ("athena setup", "          Re-run the full wizard"), ("athena setup model", "    Change model/provider"),
    ("athena setup terminal", " Change terminal backend"), ("athena setup gateway", "  Configure messaging"),
    ("athena setup tools", "    Configure tool providers"))
_EDIT_CONFIG_ROWS = (
    ("athena config", "         View current settings"), ("athena config edit", "    Open config in your editor"),
    ("athena config set <key> <value>", ""))
_READY_ROWS = (
    ("athena", "              Start chatting"), ("athena gateway", "      Start messaging gateway"),
    ("athena doctor", "       Check for issues"))
...
def _vision_row(config, feats):
    ...
    try:
        ...
        ok = False
    return ("Vision (image analysis)", ok, None if ok else "run 'athena setup' to configure")
...
def _video_gen_row(config, feats):
    # Opt-in via `athena tools` → Video Generation. Only show the row when a plugin reports
...
def _modal_row(config, feats):
    ...
    if _setup.cfg_get(config, "terminal", "backend") == "modal":
        ...
        return ("Modal Execution", False, "run 'athena setup terminal'")
...
def _spotify_row(config, feats):
    # OAuth via athena auth spotify — check auth.json, not env vars
    try:
...
def _print_setup_summary(config: dict, athena_home):
    ...
    try:
        ...
        print()
        _setup.print_warning("No inference provider is configured — Athena cannot chat yet.")
        _setup._info("  Finish this one step with either of:",
              "    athena model            (pick any provider/model)",
              "    athena setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if available_count < len(tool_status):
        _setup.print_warning("Some tools are disabled. Run 'athena setup tools' to configure them,")
    ...
    for label, value in (("Settings:", f"  {_setup.get_config_path()}"), ("API Keys:", f"  {_setup.get_env_path()}"),
                         ("Data:", f"      {athena_home}/cron/, sessions/, logs/")):
...
import athena_cli.setup as _setup  # noqa: E402  (bottom: athena_cli.setup imports this module)
