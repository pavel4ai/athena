...
Usage:
    athena                     # Interactive chat (default)
    athena chat                # Interactive chat
    athena gateway             # Run gateway in foreground
    athena gateway start       # Start gateway as service
    athena gateway stop        # Stop gateway service
    athena gateway status      # Show gateway status
    athena gateway install     # Install gateway service
    athena gateway uninstall   # Uninstall gateway service
    athena setup               # Interactive setup wizard
    athena logout              # Clear stored authentication
    athena status              # Show status of all components
    athena cron                # Manage cron jobs
    athena cron list           # List cron jobs
    athena cron status         # Check if cron scheduler is running
    athena doctor              # Check configuration and dependencies
    athena honcho setup                    # Configure Honcho AI memory integration
    athena honcho status                   # Show Honcho config and connection status
    athena honcho sessions                 # List directory → session name mappings
    athena honcho map <name>               # Map current directory to a session name
    athena honcho peer                     # Show peer names and dialectic settings
    athena honcho peer --user NAME         # Set user peer name
    athena honcho peer --ai NAME           # Set AI peer name
    athena honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    athena honcho mode                     # Show current memory mode
    athena honcho mode [hybrid|honcho|local]  # Set memory mode
    athena honcho tokens                   # Show token budget settings
    athena honcho tokens --context N       # Set session.context() token cap
    athena honcho tokens --dialectic N     # Set dialectic result char cap
    athena honcho identity                 # Show AI peer identity representation
    athena honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    athena honcho migrate                  # Step-by-step migration guide: OpenClaw native → Athena + Honcho
    athena version             Show version
    athena update              Update to latest version
    athena uninstall           Uninstall Athena Agent
    athena acp                 Run as an ACP server for editor integration
    athena sessions browse     Interactive session picker with search

    athena claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: athena_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``athena_bootstrap`` is a
...
# new code references ``athena_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, athena crashes on import and the user can't run
...
def _set_process_title() -> None:
    ...
    Strategy (try in order):
      ...
      4. No-op on Windows (the .exe name is already ``athena.exe``).
    """
    ...
    try:
        ...
        # Windows: the .exe name is already ``athena.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* athena_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("ATHENA_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".athena", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``ATHENA_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
        return False
    if os.environ.get("ATHENA_TUI") == "1" or "--tui" in argv:
        return True
...
# `entry.tsx`; this is just the earlier cousin. ``ATHENA_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("ATHENA_TUI_NO_EARLY_DISABLE") == "1":
        return
...
def _print_fast_version_info() -> None:
    from athena_cli import __release_date__, __version__
...
def _try_termux_ultrafast_version() -> bool:
    ...
    if os.environ.get("ATHENA_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from athena_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from athena_cli.subcommands.cron import build_cron_parser
from athena_cli.subcommands.gateway import build_gateway_parser
from athena_cli.subcommands.profile import build_profile_parser
from athena_cli.subcommands.model import build_model_parser
from athena_cli.subcommands.setup import build_setup_parser
from athena_cli.subcommands.postinstall import build_postinstall_parser
from athena_cli.subcommands.whatsapp import build_whatsapp_parser
from athena_cli.subcommands.slack import build_slack_parser
from athena_cli.subcommands.login import build_login_parser
from athena_cli.subcommands.logout import build_logout_parser
from athena_cli.subcommands.auth import build_auth_parser
from athena_cli.subcommands.status import build_status_parser
from athena_cli.subcommands.webhook import build_webhook_parser
from athena_cli.subcommands.hooks import build_hooks_parser
from athena_cli.subcommands.doctor import build_doctor_parser
from athena_cli.subcommands.security import build_security_parser
from athena_cli.subcommands.dump import build_dump_parser
from athena_cli.subcommands.debug import build_debug_parser
from athena_cli.subcommands.backup import build_backup_parser
from athena_cli.subcommands.import_cmd import build_import_cmd_parser
from athena_cli.subcommands.config import build_config_parser
from athena_cli.subcommands.version import build_version_parser
from athena_cli.subcommands.update import build_update_parser
from athena_cli.subcommands.uninstall import build_uninstall_parser
from athena_cli.subcommands.dashboard import build_dashboard_parser
from athena_cli.subcommands.gui import build_gui_parser
from athena_cli.subcommands.logs import build_logs_parser
from athena_cli.subcommands.prompt_size import build_prompt_size_parser
from athena_cli.subcommands.memory import build_memory_parser
...
from athena_cli.subcommands.plugins import build_plugins_parser
from athena_cli.subcommands.mcp import build_mcp_parser
from athena_cli.subcommands.claw import build_claw_parser
...
def _require_tty(command_name: str) -> None:
    ...
    if not sys.stdin.isatty():
        print(
            f"Error: 'athena {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any athena module import.
#
# Many modules cache ATHENA_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ATHENA_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.athena/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ATHENA_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo athena -p <name>` against the invoking user's home.
        ...
        candidate = home / ".athena" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # Mirrors athena_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If ATHENA_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.athena/profiles/coder or
    # /opt/data/profiles/coder).  If ATHENA_HOME points to the athena root
    # instead (e.g. systemd hardcodes ATHENA_HOME=/root/.athena), we must
    # still read active_profile — the user may have switched profiles via
    # `athena profile use` and the gateway should honour that choice.
    # See issue #22502.
    athena_home_env = os.environ.get("ATHENA_HOME", "")
    if profile_name is None and athena_home_env:
        if Path(athena_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the athena root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as ATHENA_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``athena gateway run`` to mean
    # "the root ATHENA_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("ATHENA_S6_SUPERVISED_CHILD"):
        try:
            from athena_constants import get_default_athena_root

            active_path = get_default_athena_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set ATHENA_HOME
    if profile_name is not None:
        try:
            from athena_cli.profiles import resolve_profile_env

            athena_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            athena_home = _resolve_sudo_user_profile_env(profile_name)
            if not athena_home:
            ...
            # A bug in profiles.py must NEVER prevent athena from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["ATHENA_HOME"] = athena_home
...
# Load .env from ~/.athena/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from athena_cli.config import get_athena_home
from athena_cli.env_loader import load_athena_dotenv

load_athena_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → ATHENA_REDACT_SECRETS env
# var BEFORE athena_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_athena_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            ...
            pass
        if "ATHENA_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["ATHENA_REDACT_SECRETS"] = str(_early_redact).lower()
...
try:
    from athena_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from athena_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if athena_constants not importable yet
...
# Provider model-selection wizard flows extracted to athena_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (athena_cli.main._model_flow_*) keep resolving unchanged.
from athena_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_athena_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("ATHENA_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("ATHENA_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from athena_cli.config import get_env_path, get_athena_home, load_config
    from athena_cli.auth import get_auth_status
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_athena_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_athena_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, athena_bin
    ...
    athena_bin = container_info["athena_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo athena {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo athena {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  athena --tui --resume {target}")
    if title:
        print(f'  athena --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @athena/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@athena" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/athena-ink/package.json",
    "packages/athena-ink/index.js",
    "packages/athena-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``ATHENA_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("ATHENA_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``ATHENA_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("ATHENA_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    athena_home = os.environ.get("ATHENA_HOME") or str(Path.home() / ".athena")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "ATHENA_HOME": athena_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(athena_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(athena_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if athena_cli_dir is None:
        athena_cli_dir = Path(__file__).parent
    bundled = athena_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``athena update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("ATHENA_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this Athena checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `athena update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Athena checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `athena --tui`\n"
        "If the checkout is still inconsistent, run `athena update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (ATHENA_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("ATHENA_NODE")
    ...
    ext_dir = os.environ.get("ATHENA_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with ATHENA_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset ATHENA_TUI_DIR (e.g. `unset ATHENA_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if tui_dev:
        # Keep the local @athena/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @athena/ink resolves through
        # packages/athena-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "athena-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from athena_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from athena_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="athena-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["ATHENA_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["ATHENA_PYTHON_SRC_ROOT"] = os.environ.get(
        "ATHENA_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("ATHENA_PYTHON", sys.executable)
    env.setdefault("ATHENA_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["ATHENA_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["ATHENA_MODEL"] = model
        env["ATHENA_INFERENCE_MODEL"] = model
    if provider:
        env["ATHENA_TUI_PROVIDER"] = provider
        env["ATHENA_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["ATHENA_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["ATHENA_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["ATHENA_TUI_SKILLS"] = value
    if query:
        env["ATHENA_TUI_QUERY"] = query
    if image:
        env["ATHENA_TUI_IMAGE"] = image
    if checkpoints:
        env["ATHENA_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["ATHENA_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["ATHENA_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["ATHENA_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["ATHENA_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["ATHENA_ACCEPT_HOOKS"] = "1"
    ...
    # ATHENA_TUI_RESUME is an internal hand-off from the Python wrapper to the
    ...
    # still set ATHENA_TUI_RESUME themselves.
    env.pop("ATHENA_TUI_RESUME", None)
    if resume_session_id:
        env["ATHENA_TUI_RESUME"] = resume_session_id
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``ATHENA_KANBAN_BOARD`` for the chat session.
    ...
    """
    if os.environ.get("ATHENA_KANBAN_BOARD"):
        return
    try:
        from athena_cli.kanban_db import get_current_board

        os.environ["ATHENA_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.athena/skills/`` with the bundled skill library on first launch.
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``ATHENA_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("ATHENA_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'athena sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from athena_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from athena_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'athena doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    # plugin/MCP discovery suppression (ATHENA_SAFE_MODE is checked by
    # athena_cli/plugins.py and tools/mcp_tool.py).
    if getattr(args, "safe_mode", False):
        os.environ["ATHENA_SAFE_MODE"] = "1"
        os.environ["ATHENA_IGNORE_USER_CONFIG"] = "1"
        os.environ["ATHENA_IGNORE_RULES"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.athena/config.yaml and return built-in defaults. Set BEFORE
    ...
    if getattr(args, "source", None):
        os.environ["ATHENA_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from athena_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from athena_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from athena_constants import find_node_executable, with_athena_node_path
    ...
    session_dir = get_athena_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: athena gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  athena gateway")
        ...
        print()
        print("  Or install as a service: athena gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'athena whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    ``athena_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from athena_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_postinstall(args):
    ...
    from athena_cli.config import stamp_install_method
    from athena_cli.dep_ensure import ensure_dependency
...
def select_provider_and_model(args=None):
    ...
    from athena_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("ATHENA_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                ...
                "available providers, or run 'athena doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in athena_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.athena/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.athena/.env if the active provider is not 'custom'.
    ...
    """
    from athena_cli.config import get_env_value, save_env_value, load_config
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`athena_cli.plugins.PluginContext.register_auxiliary_task`) appear
...
def _aux_flow_provider_model(
    ...
    from athena_cli.auth import _prompt_model_selection
    from athena_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from athena_cli.setup import _curses_prompt_choice
...
# downstream call sites read `athena_cli.main._PROVIDER_MODELS` directly,
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    recover from a malformed paste without editing ``~/.athena/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from athena_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    from athena_cli.env_loader import format_secret_source_suffix
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from athena_constants import display_athena_home as _dhh_fn
...
def cmd_login(args):
    ...
    from athena_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from athena_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from athena_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from athena_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from athena_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from athena_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    if sub == "manifest":
        from athena_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from athena_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from athena_cli.hooks import hooks_command
...
def cmd_security(args):
    ...
    if sub in ("audit", None):
        from athena_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from athena_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from athena_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from athena_cli.config import config_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from athena_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from athena_cli.backup import run_backup
...
def cmd_import(args):
    ...
    from athena_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from athena_cli.banner import check_for_updates
...
def cmd_uninstall(args):
    ...
    if getattr(args, "gui_summary", False):
        from athena_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from athena_cli.uninstall import run_gui_uninstall
    ...
    from athena_cli.uninstall import run_uninstall
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="athena-syntax-check-") as tmpdir:
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``athena_cli/web_dist/`` (per vite.config.ts
    outDir: "../athena_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "athena_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``athena`` launcher present but ``athena_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the athena venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — athena venv python3, no nix-shell overhead
    ...
    # a athena venv (pip / non-managed / bare-git installs).  The resolved
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    # build path usable on a stock `py -m athena_cli.main web` invocation.
    ...
    if not npm:
        ...
        return not fatal
    build_env = with_athena_node_path()
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (athena web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#
# Stamp file: $ATHENA_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $ATHENA_HOME."""
    from athena_constants import get_athena_home
    return get_athena_home() / "desktop-build-stamp.json"
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_athena_node_path(env)
    if mirror:
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from athena_logging import setup_logging as _setup_logging_gui
    ...
    # with_athena_node_path() copies os.environ when called with no arg.
    env = with_athena_node_path()
    if getattr(args, "fake_boot", False):
        env["ATHENA_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["ATHENA_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "athena_root", None):
        env["ATHENA_DESKTOP_ATHENA_ROOT"] = str(Path(args.athena_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["ATHENA_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if build_result.returncode != 0:
                ...
                print("    ELECTRON_MIRROR=<mirror-base-url> athena desktop --force-build")
...
def _find_stale_dashboard_pids(
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    environment variable ``ATHENA_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "athena dashboard",
        "athena_cli.main dashboard",
        "athena_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "athena.*dashboard"`) keeps us consistent
            # with `athena_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    ...
    print(
        "  Docs:         https://github.com/pavel4ai/athena/docs/user-guide/features/curator"
    )
...
def _kill_stale_dashboard_processes(
    ...
    # backend child, it sets ATHENA_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("ATHENA_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    if killed:
        ...
        print("    athena dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    """
    staging = f"{dst}.athena-update-staging"
    backup = f"{dst}.athena-update-old"
...
def _update_via_zip(args):
    ...
    if branch != "main":
        ...
        sys.exit(1)
    zip_url = (
        f"https://github.com/pavel4ai/athena/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="athena-update-")
    try:
    ...
    from athena_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: athena skills list-modified  "
    ...
    try:
        from athena_cli.model_catalog import seed_cache_from_checkout
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "athena-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes athena completely unrunnable (SyntaxError on import).
...
OFFICIAL_REPO_URLS = {
    ...
    "git@github.com:pavel4ai/athena.git",
    "https://github.com/pavel4ai/athena",
    "git@github.com:pavel4ai/athena",
}
OFFICIAL_REPO_URL = "https://github.com/pavel4ai/athena.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_athena_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_athena_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from pavel4ai/athena.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/pavel4ai/athena.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/pavel4ai/athena.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    default_home = get_default_athena_root()
...
# never landed).  Lives next to the venv (not under $ATHENA_HOME) because the
...
def _athena_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "athena.exe",
        scripts_dir / "athena-gateway.exe",
    ]


def _detect_concurrent_athena_instances(
    ...
    The Athena Desktop Electron app spawns ``athena.EXE`` as a backend child,
    ...
    shims (``athena.exe`` / ``athena-gateway.exe``) and returns ``(pid,
    ...
    launcher (``athena.exe``) is a separate process from the Python
    ...
    for shim in _athena_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated athena.exe launcher is a separate native process
    ...
    #      athena.exe sitting *under* a non-Athena parent (e.g. a Athena
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "athena.exe"
    lines = ["✗ Another athena.exe is running:"]
...
def _quarantine_running_athena_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``athena.exe``.
    ...
    when ``athena update`` runs, ``athena.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``athena.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next athena invocation by ``_cleanup_quarantined_exes``.
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_athena_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``athena.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``athena.exe`` is the running process —
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_athena_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``athena.exe.old.*`` left by prior updates.

    Called early on every athena invocation. The .old files are unlocked once
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``athena.exe`` / ``athena-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_athena_exe`` for the rationale.
    """
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    #
    # Quarantine the running ``athena.exe`` first: ``--reinstall -e .``
    ...
    try:
        ...
        print(
            ...
            "Run `athena update --force` after closing other athena processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from athena_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$ATHENA_HOME/bin/uv``, but on Termux the official
...
def _update_node_dependencies() -> None:
    ...
    nixos_env = with_athena_node_path(_nixos_build_env())
...
class _UpdateOutputStream:
    ...
    * Every write is also mirrored to an append-only log file
      (``~/.athena/logs/update.log``) that users can inspect after the
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.athena/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    try:
        ...
        # athena_cli.config.get_athena_home to simulate setup failure.
        from athena_cli.config import get_athena_home as _get_athena_home

        logs_dir = _get_athena_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    dropping the flag.
    """
    from athena_cli.config import detect_install_method
    ...
    if method == "docker":
        ...
        from athena_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from athena_cli.config import recommended_update_command
        from athena_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    at /usr/local/bin/athena.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v athena'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/athena, code at /usr/local/lib/athena-agent).
    fhs_link = Path("/usr/local/bin/athena")
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of ATHENA_HOME before running the update.
    ...
    ATHENA_HOME directories.  The ``--backup`` flag on ``athena update``
    ...
    # ``athena_cli/config.py``). Fall back to true if the key is missing
    ...
    try:
        from athena_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_athena_home so the user sees ~/.athena/...
    try:
        from athena_constants import get_athena_home, display_athena_home

        home = get_athena_home()
        try:
            display_path = f"{display_athena_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  athena import {out_path}")
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    athena.exe concurrent-instance guard does not see them. They still import
    ...
    if not running_pids:
        ...
        try:
            from athena_cli import gateway_windows
    ...
    print("→ Stopping Windows gateway process(es) before updating Athena...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m athena_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: athena gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``athena gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from athena_cli import gateway_windows
        from athena_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _cmd_update_pip(args):
    ...
    from athena_cli.config import is_uv_tool_install
    ...
    print("✓ Update complete! Restart athena to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # On Windows, abort early if another athena.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_athena_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        # though `git pull` can't touch $ATHENA_HOME, this is cheap
        ...
        try:
            from athena_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in athena_cli/config.py bricked
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``athena
        ...
        # the old bytecode (e.g. get_athena_home added to athena_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        if (desktop_dir / "package.json").exists() and find_node_executable("npm") and has_desktop_app:
            ...
            _desktop_build_cmd = [sys.executable, "-m", "athena_cli.main", "desktop", "--build-only"]
        ...
        # current — so copy it straight over ~/.athena/cache/model_catalog.json
        ...
        # modules in this process.  Reload athena_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_athena_home() added since the last release.
        try:
            import importlib
            import athena_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit ATHENA_HOME so
        # it is not affected by sync_skills()'s module-level ATHENA_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's ATHENA_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from athena_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'athena config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: athena config migrate")
            else:
                print()
                print("Skipped. Run 'athena config migrate' later to configure.")
        else:
        ...
        try:
            from athena_cli.backup import restore_cron_jobs_if_emptied
        ...
        # don't have it. Tying the refresh to ``athena update`` gives users a
        ...
        if gateway_mode:
            _exit_code_path = get_athena_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... athena-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... athena-gateway*)
            ...
            # Discover all athena-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. athena-gateway.service or athena-gateway-coder.service
                        ...
                        print(
                            ...
                            f"Check the gateway with: athena gateway status"
                        )
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: athena gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: athena -p <profile> gateway run  for each profile)"
                        )
        ...
        # When both athena.service (from a pre-rename install) and the
        # current athena-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `athena update` surfaces the issue until the user migrates.
        try:
            from athena_cli.gateway import (
                has_legacy_athena_units,
                _find_legacy_athena_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_athena_units():
                print()
                print("⚠ Legacy Athena gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_athena_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (athena.service) fight the current")
                print("  athena-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    athena gateway migrate-legacy")
                print()
...
def cmd_profile(args):
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → athena -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.athena)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  athena profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     athena -p {name} chat")
                else:
        ...
        from athena_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from athena_constants import get_athena_home as _hh
        ...
        from athena_cli import profile_describer as _pd
        ...
        if alias_name:
            ...
            print(f"Alias:   {alias_name} → athena -p {name}  ({wrapper})")
        print()
        ...
        from athena_cli.profiles import profile_exists
        ...
        from athena_cli.profiles import rename_profile
        ...
        from athena_cli.profiles import export_profile
        ...
        from athena_cli.profiles import import_profile
        ...
        import tempfile
        from athena_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="athena_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  athena -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      athena -p {plan.manifest.name} chat")
        ...
        from athena_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if plan.has_cron:
                print(
                    ...
                    f"athena -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from athena_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("athena_requires"):
            print(f"Requires:     Athena {data['athena_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from athena_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.athena_requires:
        print(f"  Requires: Athena {mf.athena_requires}")
...
def _report_dashboard_status() -> int:
    ...
    if not pids:
        print("No athena dashboard processes running.")
        return 0

    print(f"{len(pids)} athena dashboard process(es) running:")
    for pid in pids:
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``athena dashboard register`` for OAuth.
    ...
    try:
        from athena_cli.web_server import should_require_auth
    ...
    try:
        from athena_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    print("    [2] OAuth via Nous Portal (run `athena dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    if choice == "2":
        ...
        print(
            ...
            "ATHENA_DASHBOARD_OAUTH_CLIENT_ID into ~/.athena/.env for you.\n"
            "  Docs: https://github.com/pavel4ai/athena/docs/"
...
def cmd_dashboard(args):
    ...
    # launches the dashboard (`worker dashboard` → ATHENA_HOME points into
    ...
    if (
        ...
        and os.environ.get("ATHENA_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "athena_cli.main",
            "-p", "default",
            "dashboard",
            "--port", str(args.port),
            "--host", args.host,
        ...
        # ATHENA_HOME.  We must resolve the root explicitly instead of just
        # dropping ATHENA_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV ATHENA_HOME=/opt/data`), so an unset
        # ATHENA_HOME falls back to $HOME/.athena = /opt/data/.athena — an
        ...
        # update-button guard also misfires).  get_default_athena_root()
        # returns the root for both layouts: ~/.athena for a standard install
        ...
        try:
            from athena_constants import get_default_athena_root
            env["ATHENA_HOME"] = str(get_default_athena_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop ATHENA_HOME) rather than block the reroute.
    ...
    if "ATHENA_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["ATHENA_WEB_DIST"])
            if "ATHENA_WEB_DIST" in os.environ
            else PROJECT_ROOT / "athena_cli" / "web_dist"
        )
    ...
    from athena_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from athena_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from athena_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from athena_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from athena_cli.prompt_size import cmd_prompt_size as _impl
...
def cmd_logs(args):
    ...
    from athena_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from athena_cli.models import CANONICAL_PROVIDERS as _cp
...
# the top-level flags declared in ``athena_cli/_parser.py``.
#
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("ATHENA_TUI") == "1")
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if getattr(args, "oneshot", None):
        ...
        from athena_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["ATHENA_DEFER_AGENT_STARTUP"] = "1"
            os.environ["ATHENA_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["ATHENA_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        mem_dir = get_athena_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_athena_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_athena_home()}/memories/\n")
    else:
        from athena_cli.memory_setup import memory_command
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from athena_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from athena_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from athena_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from athena_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from athena_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from athena_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from athena_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from athena_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from athena_cli.claw import claw_command
...
def main():
    ...
    # Sweep stale ``athena.exe.old.*`` quarantine files left by previous
    # ``athena update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_athena_exe``.
    try:
    ...
    # =========================================================================
    # model command  (parser built in athena_cli/subcommands/model.py)
    # =========================================================================
    ...
    # =========================================================================
    from athena_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://github.com/pavel4ai/athena/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.athena/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://github.com/pavel4ai/athena/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from athena_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from athena_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in athena_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in athena_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in athena_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in athena_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    # =========================================================================
    # slack command  (parser built in athena_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from athena_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in athena_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in athena_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in athena_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in athena_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in athena_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in athena_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from athena_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from athena_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    # hooks command  (parser built in athena_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in athena_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in athena_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in athena_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in athena_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in athena_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.athena/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo athena uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from athena_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in athena_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in athena_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # pairing command  (parser built in athena_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in athena_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from athena_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in athena_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    try:
        from athena_cli.curator import register_cli as _register_curator_cli
    ...
    # =========================================================================
    # memory command  (parser built in athena_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in athena_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `athena computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Athena app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import shutil
            import subprocess
            from athena_cli.tools_config import _cua_driver_cmd
            # Honor ATHENA_CUA_DRIVER_CMD for local-build testing — same
            ...
            if path:
                version = ""
                try:
                    from athena_cli.tools_config import _cua_driver_env
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: athena computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: athena computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: athena computer-use install --upgrade")
                return
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: athena computer-use permissions grant")
    ...
    # =========================================================================
    # mcp command  (parser built in athena_cli/subcommands/mcp.py)
    # =========================================================================
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_athena_home() / "sessions"
            ...
            # Launch athena --resume <id> by replacing the current process
    ...
    # =========================================================================
    # claw command  (parser built in athena_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in athena_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in athena_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in athena_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in athena_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in athena_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in athena_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # =========================================================================
    # gui command  (parser built in athena_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in athena_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in athena_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    from athena_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'athena -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (athena hooks list, cron
