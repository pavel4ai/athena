...
Usage:
    ...
    athena chat / gateway / setup / status / cron / doctor / update / ...
    athena --version           # Show version and update status
    athena <cmd> --help        # Per-command help
"""

# athena_bootstrap must be the very first import — it sets up UTF-8 stdio on
# Windows (no-op on POSIX). Guarded: after a ``git pull`` / interrupted
# ``athena update`` the editable install's ``.pth`` may not list it yet; crashing
# here would block ``athena update``.
try:
...
from athena_cli._subprocess_compat import suppress_platform_ver_console
...
# Inline path math so ``python athena_cli/main.py`` (script mode: sys.path[0]
# is athena_cli/, not the repo root) can import athena_cli._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
from athena_cli import _startup_fast  # noqa: E402

# Early venv self-heal — MUST run before any third-party import below. A prior
# ``athena update`` may have left a recovery marker with a core package wiped;
# the athena_cli.config/env_loader imports further down would then crash before
...
# PID" class it exists for. ``athena_startup_watchdog`` is stdlib-only so it
...
if _argv_is_gateway_run(sys.argv[1:]):
    try:
        from athena_startup_watchdog import arm_startup_watchdog as _arm_sw
...
def _set_process_title() -> None:
    """Cosmetic: show 'athena' instead of 'python3.xx' in ps/top/htop.

    Order: opt-in ``setproctitle`` dep; ctypes ``prctl(PR_SET_NAME)`` (Linux,
    15-char limit); ``pthread_setname_np`` (macOS — lldb/top only, not ``ps
    aux``); no-op on Windows (the .exe is already ``athena.exe``). Never fatal.
    """
    try:
...
# athena_cli.config is importable. Cached so early callers don't re-parse YAML.
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    Precedence: ``--cli`` wins, then ``--tui``/``ATHENA_TUI=1``, then a
...
# `resetTerminalModes()` is the later cousin. ``ATHENA_TUI_NO_EARLY_DISABLE``
...
# ``athena --version`` is answered before config/logging imports.
...
from athena_cli.subcommands.sync import build_sync_parser
...
from athena_cli.subcommands.whatsapp import build_whatsapp_parser, build_whatsapp_cloud_parser
...
from athena_cli.subcommands.pause import build_pause_parser
...
from athena_cli.subcommands.verify import build_verify_parser
from athena_cli.subcommands.security import build_security_parser
from athena_cli.subcommands.approvals import build_approvals_parser
...
from athena_cli.subcommands.import_agent import build_import_agent_parser
from athena_cli.subcommands.config import build_config_parser
from athena_cli.subcommands.skin import build_skin_parser
from athena_cli.subcommands.console import build_console_parser
...
from athena_cli.subcommands.dashboard import build_dashboard_parser, build_serve_parser
...
from athena_cli.subcommands.vault import build_vault_parser
from athena_cli.subcommands.moa import build_moa_parser
from athena_cli.subcommands.fallback import build_fallback_parser
from athena_cli.subcommands.worktree import build_worktree_parser
from athena_cli.subcommands.browser import build_browser_parser
from athena_cli.subcommands.secrets import build_secrets_parser
from athena_cli.subcommands.egress import build_egress_parser
from athena_cli.subcommands.migrate import build_migrate_parser
from athena_cli.subcommands.checkpoints import build_checkpoints_parser
from athena_cli.subcommands.bundles import build_bundles_parser
from athena_cli.subcommands.curator import build_curator_parser
from athena_cli.subcommands.pets import build_pets_parser
from athena_cli.subcommands.journey import build_journey_parser
from athena_cli.subcommands.computer_use import build_computer_use_parser
from athena_cli.subcommands.sessions import build_sessions_parser
from athena_cli.subcommands.completion import build_completion_parser
...
# Profile override — MUST happen before any athena module import: many modules
# cache ATHENA_HOME at import time. --profile/-p is pre-parsed from sys.argv,
# ATHENA_HOME set, and the flag stripped so argparse never sees it. Falls back
# to ~/.athena/active_profile for the sticky default.
_PROFILE_NAME_RE = r"^[a-z0-9][a-z0-9_-]{0,63}$"  # mirrors athena_cli.profiles._PROFILE_ID_RE
...
def _scan_profile_flag(argv: list) -> tuple:
    ...
    Historically the flag worked even after the subcommand (`athena chat -p
    ...
    """
    from athena_cli._parser import top_level_value_flag_sets
...
def _resolve_sudo_user_profile_env(name: str) -> str | None:
    ...
    try:
        ...
        candidate = Path(pwd.getpwnam(sudo_user).pw_dir) / ".athena" / "profiles" / name
...
def _under_gateway_supervisor(argv: list) -> bool:
    ...
    ``-p <name>`` or pin ATHENA_HOME to the profile dir; a bare invocation
    means "the root ATHENA_HOME profile". If a supervised default-profile
    child read active_profile, switching the active profile (dashboard,
    ``athena profile use``) would silently redirect the default gateway into
    ...
    ATHENA_SUPERVISED_CHILD (systemd unit / launchd plist / Windows task),
    ATHENA_S6_SUPERVISED_CHILD (legacy s6 container), INVOCATION_ID (systemd
    ...
    self-hosted CI runners), ATHENA_GATEWAY_EXTERNAL_SUPERVISOR (explicit
    opt-in). XPC_SERVICE_NAME is deliberately NOT consulted: interactive macOS
    terminals set it too.
    """
    if os.environ.get("ATHENA_SUPERVISED_CHILD") or os.environ.get("ATHENA_S6_SUPERVISED_CHILD"):
        return True
...
def _apply_profile_override() -> None:
    ...
    # ATHENA_HOME already set with no explicit flag: trust it only when it
    # points at a specific profile dir ("profiles" as immediate parent). If it
    # points at the athena root (systemd hardcodes ATHENA_HOME=/root/.athena)
    # we must still read active_profile — the user may have run
    # `athena profile use` and the gateway should honour it (#22502).
    athena_home_env = os.environ.get("ATHENA_HOME", "")
    if profile_name is None and athena_home_env and Path(athena_home_env).parent.name == "profiles":
        return
...
# Windows launcher self-heal — the ``athena`` command is a COPY of the venv
# console script staged into the managed bin dir (outside the checkout, since
# ``athena update``'s autostash once swept ``<checkout>\bin`` copies off disk;
...
# app's ``python -m athena_cli.main`` spawn. Gates fail toward inaction. Sits
# AFTER the profile override on purpose — no athena module may import before
...
# ``<checkout>\bin``, where ``athena update``'s autostash (``git stash push --include-untracked``) swept
# them off disk; with the desktop updater's ``--keep-stash`` nothing restored them and ``athena`` stopped
...
if sys.platform == "win32":
    try:
        from athena_cli import _install_repair as _install_repair_mod
...
# Bridge security.redact_secrets → ATHENA_REDACT_SECRETS BEFORE athena_logging
...
try:
    ...
    # athena_logging and later raw reads: 3-4 config.yaml parses become one.
    from athena_cli.config import read_raw_config as _read_raw_early
...
from athena_cli.main_agent_cmds import (
    cmd_acp,
    cmd_insights,
    cmd_memory,
    cmd_monitoring,
...
from athena_cli.main_platform_setup import (
    cmd_slack,
    cmd_sync,
    cmd_whatsapp,
    cmd_whatsapp_cloud,
)
from athena_cli.main_dashboard import (
...
from athena_cli.main_dashboard import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _respawn_dashboard_processes,
)
from athena_cli.main_provider_setup import (
...
from athena_cli.main_install_repair import (
    _cleanup_quarantined_exes,
    _recover_from_interrupted_install,
)
from athena_cli.main_install_repair import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    ShimQuarantineError,
    _UPDATE_REEXEC_ENV,
...
from athena_cli.main_desktop import (
    cmd_gui,
)
from athena_cli.main_desktop import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _desktop_build_needed,
    _desktop_dist_exists,
    _desktop_macos_relaunchable_fixup,
    _desktop_packaged_executable,
)
from athena_cli.main_web_build import (
    _sweep_stale_bytecode_if_checkout_changed,
)
from athena_cli.main_web_build import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _build_web_ui,
    _nixos_build_env,
    _record_bytecode_fingerprint,
    _run_npm_install_deterministic,
)
from athena_cli.main_tui_launch import (
    _launch_tui,
...
def _dotenv_has_provider_key(env_file: Path, provider_env_vars: set) -> bool:
    """True if ~/.athena/.env assigns a non-empty value to any provider key."""
...
def _has_any_provider_configured(*, strict_profile_scope: bool = False) -> bool:
    ...
    counts only once its identity exists, and the boot bootstrap (``athena_cli.free_tier_bootstrap``)
    ...
    """
    from athena_cli.config import DEFAULT_CONFIG, get_env_path, get_athena_home, load_config
    from athena_cli.auth import PROVIDER_REGISTRY, get_auth_status
    ...
    _has_athena_config = _model_name and _model_name != DEFAULT_CONFIG.get("model", "")
    ...
    if _auth_store_logged_in(get_athena_home() / "auth.json", PROVIDER_REGISTRY, strict_profile_scope):
        return True
    ...
    # Claude Code OAuth credentials count only once Athena is explicitly
    # configured — having Claude Code installed isn't consent to use its tokens.
    if _has_athena_config and not strict_profile_scope:
        try:
    ...
    try:
        from athena_cli.anon_auth import guest_enabled, has_guest
...
def _confirm_startup_expensive_model_override(args) -> None:
    ...
    try:
        ...
        from athena_cli.model_selection_guards import (
            combined_message,
            selection_warnings,
        )
    except Exception as exc:
...
def _session_db():
    """Yield a ``SessionDB`` (lazy import, so test patches on ``athena_state``
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``athena -c`` from repo A continues repo A's last session rather than the
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print(
                    "Use 'athena sessions list' to see available sessions, or "
            ...
            try:
                from athena_cli.terminal_breadcrumbs import resolve_breadcrumb_session
...
def _import_foreign_resume(args) -> None:
    ...
    if not (isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in ("@claude", "@codex")):
        return
    from athena_cli.foreign_sessions import import_foreign_session, pick_foreign_session
    ...
    print(f"  (later: athena --resume {_imported_id})")
...
def _start_chat_background_prefetch() -> None:
    ...
    The ONE exception is an unseeded ~/.athena/skills: there the banner
    ...
    if _termux_should_prefetch_update_check():
        try:
            from athena_cli.banner import prefetch_banner_data, prefetch_update_check
    ...
    def _skills_dir_is_unseeded() -> bool:
        try:
            from athena_cli.config import get_athena_home
            skills_dir = Path(get_athena_home()) / "skills"
    ...
    if _skills_dir_is_unseeded():
        ...
        try:
            import athena_cli.banner as _banner_mod
...
def _first_run_setup_guard(args) -> None:
    """No provider configured: offer `athena setup` (TTY) or exit 1 with guidance."""
    print()
    print(
...
def cmd_chat(args):
    ...
    try:
        ...
        # Mixed-version installs (new cli.py, older athena_cli.config) crash
        # here — e.g. missing resolve_turn_limit / split_model_config_default
        # (#96900). The agent-setup mixin prints this hint too late: AthenaCLI
        # construction already failed. Fast-chat launch also goes through
        # cmd_chat, so this one catch covers `athena` / `athena chat`.
...
def _forward_command(name: str, module: str, attr: str, *, forward_return: bool = False, doc: str = ""):
    """A ``athena <cmd>`` handler that hands ``args`` to ``<module>.<attr>``.
...
cmd_setup = _forward_command("cmd_setup", "athena_cli.setup", "run_setup_wizard", doc='Interactive setup wizard.')
cmd_login = _forward_command("cmd_login", "athena_cli.auth", "login_command", doc='Authenticate Athena CLI with a provider.')
cmd_logout = _forward_command("cmd_logout", "athena_cli.auth", "logout_command", doc='Clear provider authentication.')
cmd_auth = _forward_command("cmd_auth", "athena_cli.auth_commands", "auth_command", doc='Manage pooled credentials.')
cmd_status = _forward_command("cmd_status", "athena_cli.status", "show_status", doc='Show status of all components.')
cmd_cron = _forward_command("cmd_cron", "athena_cli.cron", "cron_command", forward_return=True, doc='Cron job management.')
cmd_webhook = _forward_command("cmd_webhook", "athena_cli.webhook", "webhook_command", doc='Webhook subscription management.')
cmd_kanban = _forward_command("cmd_kanban", "athena_cli.kanban", "kanban_command", forward_return=True, doc='Multi-profile collaboration board.')
cmd_project = _forward_command("cmd_project", "athena_cli.projects_cmd", "projects_command", forward_return=True, doc='Manage projects (named, multi-folder workspaces).')
cmd_hooks = _forward_command("cmd_hooks", "athena_cli.hooks", "hooks_command", doc='Shell-hook inspection and management.')
cmd_doctor = _forward_command("cmd_doctor", "athena_cli.doctor", "run_doctor", doc='Check configuration and dependencies.')
cmd_dump = _forward_command("cmd_dump", "athena_cli.dump", "run_dump", doc='Dump setup summary for support/debugging.')
cmd_debug = _forward_command("cmd_debug", "athena_cli.debug", "run_debug", doc='Debug tools (share report, etc.).')
cmd_skin = _forward_command("cmd_skin", "athena_cli.skin_cmd", "skin_command", doc='Skin management (list / use / set).')
cmd_import = _forward_command("cmd_import", "athena_cli.backup", "run_import", doc='Restore a Athena backup from a zip file.')
cmd_dashboard_register = _forward_command("cmd_dashboard_register", "athena_cli.dashboard_register", "cmd_dashboard_register", doc='Register a self-hosted dashboard OAuth client with Nous Portal.')
cmd_gateway_enroll = _forward_command("cmd_gateway_enroll", "athena_cli.gateway_enroll", "cmd_gateway_enroll", doc='Enroll a self-hosted gateway with a relay connector.')
cmd_prompt_size = _forward_command("cmd_prompt_size", "athena_cli.prompt_size", "cmd_prompt_size", doc='Show a byte/char breakdown of the system prompt + tool schemas.')
cmd_pairing = _forward_command("cmd_pairing", "athena_cli.pairing", "pairing_command")
cmd_plugins = _forward_command("cmd_plugins", "athena_cli.plugins_cmd", "plugins_command")
cmd_mcp = _forward_command("cmd_mcp", "athena_cli.mcp_config", "mcp_command")
cmd_claw = _forward_command("cmd_claw", "athena_cli.claw", "claw_command")
cmd_import_agent = _forward_command("cmd_import_agent", "athena_cli.agent_import", "import_agent_command")
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            ...
            pass
    from athena_cli.setup import run_setup_action_with_navigation
...
def _resolve_active_provider(config, model_cfg, effective_provider, custom_provider_map):
    ...
    """
    from athena_cli.auth import AuthError, format_auth_error, resolve_provider
    from athena_cli.config import get_compatible_custom_providers, get_env_value
    from athena_cli.providers import custom_provider_aliases, resolve_provider_full
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            print(
                f"Warning: Unknown provider '{effective_provider}'. Check 'athena model' for "
...
def select_provider_and_model(args=None):
    ...
    effective_provider = config_provider or os.getenv("ATHENA_INFERENCE_PROVIDER") or "auto"
    ...
    from athena_cli.models import _PROVIDER_LABELS
    ...
    # athena_cli.main keep intercepting.
    ...
    # "custom") leaves a stale OPENAI_BASE_URL in ~/.athena/.env that poisons
...
# ``athena_cli/update_cmd*.py`` files resolve these names via ``_m().<name>``
# on athena_cli.main; importing update_cmd eagerly would cost every ``athena``
...
_FROZEN_UPDATER_SURFACE: dict[str, tuple[str, ...]] = {
    "athena_cli.update_cmd": (
        ...
        "_print_parked_branch_skip_warning", "_purge_stale_athena_modules",
    ...
    "athena_cli.dashboard_procs": (
        "_detect_concurrent_athena_instances", "_kill_stale_dashboard_processes",
    ),
}
...
def cmd_approvals(args):
    """Dispatch `athena approvals <subcmd>`."""
    from athena_cli.approvals_suggest import approvals_command
...
def _finalize_update_receipt(code: int, reason: str) -> None:
    ...
    try:
        ...
        from athena_cli.update_receipt import finalize_pending_update_receipt
...
def _update_preflight_handled(args) -> bool:
    ...
    if getattr(args, "plan", False):
        # Read-only plan phase (#91277 Phase 2): inventory every running Athena runtime across profiles, its
        # supervisor, and its running code version — without mutating anything. Safe on a live fleet.
        from athena_cli.update_inventory import (
    ...
    from athena_cli.update_contract import (
    ...
    if getattr(args, "check", False):
        ...
        from athena_cli.update_cmd import _cmd_update_check
...
def cmd_update(args):
    """Update Athena Agent: hangup protection + update lock around ``_cmd_update_impl``."""
    ...
    from athena_cli.update_lock import (
    ...
    from athena_cli.update_cmd import _cmd_update_impl
    ...
    try:
        ...
        from athena_cli.update_receipt import COMMAND_BOUNDARY_STOP_REASON
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    ``athena -c Pokemon Agent Dev`` → ``['-c', 'Pokemon Agent Dev']``; tokens
...
from athena_cli.profile_cmd import cmd_profile
...
def _dashboard_lifecycle_flags(args, token_file) -> None:
    ...
    if getattr(args, "stop", False):
        ...
        from athena_cli.dashboard_procs import _kill_stale_dashboard_processes
...
def _dashboard_validate_serve_args(args, headless_backend, token_file):
    ...
    # `athena serve` is headless/non-interactive: fail closed on a corrupt
    ...
    if token_file and not headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with athena serve")
...
def _dashboard_sanitize_desktop_env(headless_backend) -> None:
    ...
    Desktop Electron spawns its backend with ATHENA_DESKTOP=1 plus
    ATHENA_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    ATHENA_SERVE_HEADLESS=1). A shell inheriting those then running
    `athena dashboard` would serve the desktop renderer ("Desktop IPC bridge
    ...
    (ATHENA_DESKTOP=1) keeps its dist. Headless `serve` re-sets
    ATHENA_SERVE_HEADLESS itself.
    """
    if os.environ.get("ATHENA_DESKTOP") != "1":
        if _is_electron_packaged_web_dist(os.environ.get("ATHENA_WEB_DIST", "")):
            os.environ.pop("ATHENA_WEB_DIST", None)
    if not headless_backend:
        os.environ.pop("ATHENA_SERVE_HEADLESS", None)
...
def _dashboard_prepare_runtime(args, headless_backend) -> bool:
    ...
    mcp_discovery_after_bind = headless_backend and os.environ.get("ATHENA_DESKTOP") == "1"
...
def cmd_completion(args, parser=None):
    ...
    from athena_cli import completion
...
def cmd_console(args):
    """Open the safe Athena command console."""
    from athena_cli.console_engine import run_console_repl
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    """
    first = _first_positional_argv()  # None = bare ``athena`` → chat
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    on import; ``discover_plugins()`` alone leaves ``athena photon`` failing
    ...
    not import it, so the CLI registration never happens and ``athena photon`` fails with argparse ``invalid
    choice`` (issue #54678).
    """
    if not command_name:
        return
...
def _is_tui_chat_launch(args) -> bool:
    ...
    # flags, TTY gate, ATHENA_TUI env, display.interface config). Bare
    # `athena`/`athena chat` with a TUI display config was previously missed
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo chokepoint: ATHENA_YOLO_MODE must be set before any discovery
    ...
    if not _is_tui_chat_launch(args):
        ...
        try:
            from athena_cli.plugins import start_background_plugin_discovery
    ...
    try:
        from athena_cli.mcp_startup import set_mcp_server_filter
    ...
    if _run_inline_mcp_discovery:
        try:  # synchronous for entrypoints without a later bounded startup path
            from athena_cli.mcp_startup import get_mcp_server_filter
...
def _try_fast_serve_launch() -> bool:
    ...
    """
    if os.environ.get("ATHENA_DISABLE_FAST_SERVE_LAUNCH") == "1":
        return False
...
def _try_fast_chat_launch() -> bool:
    ...
    """
    if os.environ.get("ATHENA_DISABLE_FAST_CHAT_LAUNCH") == "1":
        return False
    argv = sys.argv[1:]
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `athena --tui` is the hot path on phones and the TUI immediately execs
...
def _advertise_agent_env() -> None:
    ...
    (``athena-agent``) — matching is exact. ``ATHENA_AGENT`` is the
    Athena-specific marker. setdefault: never clobber an outer harness.
    ...
    harness that spawned them. Athena running inside another agent's terminal).
    """
...
def _cmd_sessions_lazy(args, **kwargs):
    """``athena sessions`` handler; sessions_cmd imports only when the subcommand runs."""
    from athena_cli.sessions_cmd import cmd_sessions

    return cmd_sessions(args, **kwargs)


def _build_cli_parser():
    """Build the full ``athena`` argparse tree -> ``(parser, subparsers)``.

    Registration ORDER is the ``athena --help`` order; keep it stable. Groups
    live in ``athena_cli/subcommands/<group>.py`` with handlers injected so
    ...
    # OUTBOUND egress firewall; ``athena proxy`` (gateway group) is the INBOUND one.
    ...
    from athena_cli.subcommands.peer import build_peer_parser
    ...
    from athena_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # "desktop" is canonical (Athena-Setup.exe tells users to run it, so it
...
def _parse_cli_args(parser, subparsers, argv):
    ...
    routing; if that fails (``athena -c model`` — 'model' is the session name)
...
def main():
    ...
    # Sweep stale ``athena.exe.old.*`` quarantine files from previous Windows
    # updates (see ``_quarantine_running_athena_exe``). No-op elsewhere.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass
    ...
    # Self-heal a venv left half-built by an interrupted ``athena update``, and
    ...
    # substring match is deliberately loose: over-matching (``athena skills
    # install update``) only defers recovery one launch; under-matching
    # (``athena -p work update``) would race. Never raises.
    # See #95294.
    if "update" not in sys.argv[1:]:
        try:
            ...
            pass
        try:
            from athena_cli.update_cmd_fleet import _warn_pending_fleet_restart_on_startup
...
_PLUGIN_COMPAT_LAZY = {
    'line_input': ('athena_cli.cli_output', 'line_input'),
}
