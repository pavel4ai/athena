...
from athena_constants import agent_browser_runnable, find_node_executable
from tools.environments.local import athena_subprocess_env

_IS_WINDOWS = platform.system() == "Windows"

_DEP_CHECKS = {
    # find_node_executable() rather than a bare which(): $ATHENA_HOME/node is not on PATH, so
    ...
    "browser": lambda: (agent_browser_runnable(shutil.which("agent-browser")) or _has_system_browser()
                        or _has_athena_agent_browser() or _has_npx_agent_browser()),
...
def _has_athena_agent_browser() -> bool:
    from athena_constants import get_athena_home  # late import: tests patch athena_constants.get_athena_home
    ...
    # install.sh installs into $ATHENA_HOME/node/bin/ via npm -g --prefix; legacy git clones used node_modules/.bin/.
...
def ensure_dependency(dep: str, interactive: bool = True) -> bool:
    ...
    if shell == "powershell":
        ...
        if not ps_bin:
            ...
            return False
        cmd = [ps_bin, "-ExecutionPolicy", "Bypass", "-File", str(script), "-Ensure", dep, "-AthenaHome", str(get_athena_home())]
    else:
        cmd = ["bash", str(script), "--ensure", dep]
    run_env = {**athena_subprocess_env(inherit_credentials=False), "IS_INTERACTIVE": "false"}
