...
def _codex_curated_models() -> list[str]:
    ...
    """
    from athena_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $ATHENA_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Athena-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $ATHENA_HOME/models_dev_cache.json directly (no network) so this
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$ATHENA_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
# in athena_cli/web_server.py and ``partition_nous_models_by_tier`` — which can
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``athena_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         when the env var is set in ``~/.athena/.env``.
...
#
# Cache strategy:
#   - One JSON file at $ATHENA_HOME/provider_models_cache.json
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$ATHENA_HOME/auth.json`` and external
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _ATHENA_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from athena_constants import get_athena_home
    return get_athena_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Athena actually ships.  (#46850)
