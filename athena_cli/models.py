...
``athena_cli.models.<name>`` stays the stable import/monkeypatch surface:
...
from athena_cli.models_catalog_static import (
    CANONICAL_PROVIDERS,
    OPENROUTER_MODELS,
...
from athena_cli.models_reasoning_caps import (
...
_ATHENA_USER_AGENT = f"athena-cli/{_ATHENA_VERSION}"
...
def _openrouter_catalog_disk_ttl() -> float:
    ...
    from athena_cli.model_catalog import refresh_interval_seconds
...
def _openrouter_catalog_disk_path() -> Path:
    ...
    return get_athena_home() / "cache" / "openrouter_curated_catalog.json"
...
def _nous_recommended_disk_path() -> "Path":
    from athena_constants import get_athena_home
    return get_athena_home() / "cache" / "nous_recommended_cache.json"
...
def _resolve_nous_portal_url() -> str:
    ...
    try:
        from athena_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    try:
        from athena_cli.model_catalog import get_default_model_from_cache
...
def recommended_nous_default_model() -> dict[str, Any]:
    ...
    sign-in completion in ``athena_cli.anon_auth`` so both land on the same model.
    """
    from athena_cli import models_pricing as mp
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    """True when ``supported_parameters`` advertises ``tools`` (athena-agent is tool-calling-first).
...
def fetch_openrouter_models(
    ...
    try:
        from athena_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id, _ in fallback:
        ...
        if live_item is None or not _openrouter_model_supports_tools(live_item):
            continue
        # Hide models that don't advertise tool-calling support — athena-agent requires it and surfacing
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    try:
        from athena_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    from athena_constants import AI_GATEWAY_BASE_URL
...
def _provider_has_credentials(pid: str) -> bool:
    try:
        from athena_cli.auth import get_auth_status, has_usable_secret
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    derived from :data:`CANONICAL_PROVIDERS` (shared with ``athena model`` and ``/model``)."""
...
def _resolve_static_model_alias(
    ...
    try:
        from athena_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_provider_for_model(
    ...
    from athena_cli.models_detect import (
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``athena_cli.auth.resolve_provider()`` to resolve it from credentials."""
...
def _first_exchangeable_copilot_token(raw_tokens) -> str:
    ...
    from athena_cli.copilot_auth import exchange_copilot_token, validate_copilot_token
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from athena_cli.model_search import model_alias_canonical
...
def _codex_catalog(normalized: str, force_refresh: bool) -> list[str]:
    ...
    try:
        from athena_cli.auth import resolve_codex_runtime_credentials
...
def _nous_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    try:
        from athena_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
    ...
    # Portal models still surface without a Athena release.
...
def _api_key_credentials(normalized: str) -> tuple[str, str]:
    ...
    try:
        from athena_cli.auth import resolve_api_key_provider_credentials
...
def _openai_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    ...
    # entries, so intersect with the curated agentic catalog so ``/model`` matches ``athena model``.
    ...
    # pure-catalog helper (no extra live fetch) so we only accept models Athena actually ships. (#46850)
    ...
    from athena_cli.providers import is_official_openai_host
...
# every authed provider's /v1/models). One JSON file at $ATHENA_HOME/provider_models_cache.json;
...
def _provider_models_cache_path() -> Path:
    from athena_constants import get_athena_home
    return get_athena_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    try:
        ...
        for rel in ("auth.json", "credentials.json"):
            _mtime_part(rel, get_athena_home() / rel)
    except Exception:
        pass
...
def clear_provider_models_cache(provider: Optional[str] = None) -> None:
    ...
    ``/model --refresh`` and ``athena model --refresh``."""
    try:
...
def copilot_default_headers(*, is_agent_turn: bool = True) -> dict[str, str]:
    ...
    try:
        from athena_cli.copilot_auth import copilot_request_headers
...
def opencode_zen_free_headers() -> dict:
    ...
    try:
        from athena_cli import __version__ as _v
    except Exception:
        _v = "0"
    return {
        "Authorization": "",
...
def probe_api_models(
    ...
    headers: dict[str, str] = {"User-Agent": _ATHENA_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"athena-agent/{_ATHENA_VERSION}"
    ...
    if isinstance(request_headers, dict):
        ...
        from athena_cli.config import normalize_extra_headers
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=athena"
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    headers = {"Authorization": f"Bearer {api_key}", "User-Agent": _ATHENA_USER_AGENT}
    try:
...
_PLUGIN_COMPAT_LAZY = {
    'LMStudioLoadResult': ('athena_cli.models_local', 'LMStudioLoadResult'),
    'PROVIDER_GROUPS': ('athena_cli.models_catalog_static', 'PROVIDER_GROUPS'),
    'ProviderEntry': ('athena_cli.models_catalog_static', 'ProviderEntry'),
    'atomic_json_write': ('utils', 'atomic_json_write'),
    'base_url_host_matches': ('utils', 'base_url_host_matches'),
    'compute_sale_discount': ('athena_cli.models_pricing', 'compute_sale_discount'),
    'ensure_lmstudio_model_loaded': ('athena_cli.models_local', 'ensure_lmstudio_model_loaded'),
    'fetch_ai_gateway_pricing': ('athena_cli.models_pricing', 'fetch_ai_gateway_pricing'),
    'fetch_lmstudio_models': ('athena_cli.models_local', 'fetch_lmstudio_models'),
    'fetch_models_with_pricing': ('athena_cli.models_pricing', 'fetch_models_with_pricing'),
    'fetch_ollama_local_models': ('athena_cli.models_local', 'fetch_ollama_local_models'),
    'get_cached_nous_inference_base_url': ('athena_cli.models_pricing', 'get_cached_nous_inference_base_url'),
    'get_pricing_for_provider': ('athena_cli.models_pricing', 'get_pricing_for_provider'),
    'group_providers': ('athena_cli.models_catalog_static', 'group_providers'),
    'lmstudio_model_reasoning_options': ('athena_cli.models_local', 'lmstudio_model_reasoning_options'),
    'nous_catalog_url': ('athena_cli.models_reasoning_caps', 'nous_catalog_url'),
    'nous_model_reasoning_capabilities': ('athena_cli.models_reasoning_caps', 'nous_model_reasoning_capabilities'),
    'nous_policy_allowed_ids': ('athena_cli.models_pricing', 'nous_policy_allowed_ids'),
    'ollama_model_supports_thinking': ('athena_cli.models_local', 'ollama_model_supports_thinking'),
    'openrouter_model_reasoning_capabilities': ('athena_cli.models_reasoning_caps', 'openrouter_model_reasoning_capabilities'),
    'parse_openrouter_reasoning_capabilities': ('athena_cli.models_reasoning_caps', 'parse_openrouter_reasoning_capabilities'),
    'peek_cached_pricing': ('athena_cli.models_pricing', 'peek_cached_pricing'),
    'pricing_cache_scope': ('athena_cli.models_pricing', 'pricing_cache_scope'),
    'probe_lmstudio_models': ('athena_cli.models_local', 'probe_lmstudio_models'),
    'probe_ollama_local_models': ('athena_cli.models_local', 'probe_ollama_local_models'),
    'provider_group_for_slug': ('athena_cli.models_catalog_static', 'provider_group_for_slug'),
    'refresh_reasoning_caps_async': ('athena_cli.models_reasoning_caps', 'refresh_reasoning_caps_async'),
    'restrict_to_nous_policy': ('athena_cli.models_pricing', 'restrict_to_nous_policy'),
    'should_use_ollama_native_catalog': ('athena_cli.models_local', 'should_use_ollama_native_catalog'),
    'url_origin': ('athena_cli.urllib_security', 'url_origin'),
    'validate_requested_model': ('athena_cli.models_validate', 'validate_requested_model'),
    'warm_nous_reasoning_caps_async': ('athena_cli.models_reasoning_caps', 'warm_nous_reasoning_caps_async'),
    'warm_openrouter_reasoning_caps_async': ('athena_cli.models_reasoning_caps', 'warm_openrouter_reasoning_caps_async'),
}
