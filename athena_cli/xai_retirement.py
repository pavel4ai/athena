...
Pure logic: walks a Athena config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `athena doctor` and a future `athena migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Athena config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Athena config and return retirement issues.
