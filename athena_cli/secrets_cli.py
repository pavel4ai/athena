...
# module makes the ``athena update`` self-lock preflight defer. This module is registered
# parse-time from ``athena_cli.main``, so the backend import stays lazy (nothing touches ``bw``
...
def __getattr__(name: str):
    ...
    Existing callers (and upstream tests) monkeypatch attributes on ``athena_cli.secrets_cli.bw`` directly.
    ...
    so ``import athena_cli.secrets_cli`` stays crypto-free while ``secrets_cli.bw.find_bws`` still resolves.
    """
    if name == "bw":
        return _load_bw()
...
# ── Argparse wiring — called from athena_cli.main ──
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print("  Status:  [cyan]athena secrets bitwarden status[/cyan]\n"
                  "  Refresh: [cyan]athena secrets bitwarden sync[/cyan]\n"
                  "  Disable: [cyan]athena secrets bitwarden disable[/cyan]")
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]athena secrets bitwarden setup[/cyan] to enable.")
        return 0
    if not token:
...
def cmd_token(args: argparse.Namespace) -> int:
    ...
    def verify(token: str) -> bool:
        ...
        if project_id and projects and project_id not in {p["id"] for p in projects}:
            console.print(
                ...
                "app or re-run `athena secrets bitwarden setup` to pick a different project.[/yellow]")
        return True
    ...
    return rotate_token(
        ...
        disabled_note=None if bw_cfg.get("enabled") else (
            ...
            "run `athena secrets bitwarden setup` (or set "
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not args.apply:
        console.print("\n  This was a dry-run — secrets are picked up automatically on the "
                      "next [cyan]athena[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] "
...
_PROJECT_LIST_HINTS = (
    (("invalid_client", "400 bad request"),
     ...
     "[cyan]athena secrets bitwarden setup[/cyan] and pick EU or "
