...
from athena_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_athena_home,  # noqa: F401 — used by test mocks
)
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    Mirrors ``_load_mcp_config()`` in ``tools/mcp_tool.py``: load ``~/.athena/.env`` into ``os.environ`` and
...
def cmd_mcp_add(args):
    ...
    # --command uses dest="mcp_command" (see athena_cli/main.py for why the dest is renamed).
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  athena mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  athena mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  athena mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: athena mcp test " + name)
        return
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(
            f"Saved '{name}' to {display_athena_home()}/config.yaml ({tool_count}/{len(tools)} tools enabled)"
        )
...
def cmd_mcp_remove(args):
    ...
    # an earlier `athena mcp test`) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  athena mcp add <name> --url <endpoint>')
        _info('  athena mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _reauth_oauth_server(name: str, server_config: dict, *, flow: str | None = None) -> bool:
    ...
    # re-auth path in web_server.py. force_interactive_oauth: `athena mcp login` is explicitly
...
def cmd_mcp_reauth(args):
    ...
    if not name:
        ...
        _info("Usage: athena mcp reauth <name>   |   athena mcp reauth --all")
        return
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'athena mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    cfg = _lookup_server(name, _get_mcp_servers(), "Available")
    if cfg is None:
...
_MCP_USAGE = (
    "athena mcp                                    Open the catalog picker (default)",
    "athena mcp catalog                            List Nous-approved MCPs",
    "athena mcp install <name>                     Install a catalog MCP",
    "athena mcp serve                              Run as MCP server",
    "athena mcp add <name> --url <endpoint>        Add a custom MCP server",
    "athena mcp add <name> --command <cmd>         Add a stdio server",
    "athena mcp add <name> --preset <preset>       Add from a known preset",
    "athena mcp remove <name>                      Remove a server",
    "athena mcp list                               List configured servers",
    "athena mcp test <name>                        Test connection",
    "athena mcp configure <name>                   Toggle tools",
    "athena mcp login <name>                       Re-authenticate OAuth",
    "athena mcp reauth <name> | --all              Re-auth one or all OAuth servers",
)
...
def mcp_command(args):
    ...
    if action in ("picker", "catalog", "install"):
        ...
        from athena_cli import mcp_picker
    ...
    if handler:
        handler(args)
        return
    # No subcommand — drop the user into the catalog picker (same UX as `athena plugin`).
    from athena_cli.mcp_picker import run_picker
    run_picker()
