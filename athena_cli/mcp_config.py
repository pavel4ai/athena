...
configuration in ~/.athena/config.yaml under the ``mcp_servers`` key.
"""
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from athena_cli.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.athena/.env`` into ``os.environ`` and recursively interpolate any
...
def cmd_mcp_add(args):
    ...
    # athena_cli/main.py for why the dest is renamed.
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(f"Saved '{name}' to {display_athena_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def mcp_command(args):
    ...
    if handler:
        ...
        _info("athena mcp                                    Open the catalog picker (default)")
        _info("athena mcp catalog                            List Nous-approved MCPs")
        _info("athena mcp install <name>                     Install a catalog MCP")
        _info("athena mcp serve                              Run as MCP server")
        _info("athena mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("athena mcp add <name> --command <cmd>         Add a stdio server")
        _info("athena mcp add <name> --preset <preset>       Add from a known preset")
        _info("athena mcp remove <name>                      Remove a server")
        _info("athena mcp list                               List configured servers")
        _info("athena mcp test <name>                        Test connection")
        _info("athena mcp configure <name>                   Toggle tools")
        _info("athena mcp login <name>                       Re-authenticate OAuth")
        print()
