...
``~/.athena/cache/model_catalog.json`` → master URL fetch; any fetch failure keeps the stale copy
...
DEFAULT_CATALOG_URL = (
    "https://github.com/pavel4ai/athena/docs/api/model-catalog.json")
...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/pavel4ai/athena/main/website/static/api/model-catalog.json",
)
...
def _cache_path() -> Path:
    ...
    return get_athena_home() / "cache" / "model_catalog.json"
...
def _fetch_manifest(url: str, timeout: float) -> dict[str, Any] | None:
    ...
    try:
        req = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": _ATHENA_USER_AGENT})
...
def refresh_catalogs() -> bool:
    ...
    try:
        from athena_cli.models import fetch_nous_recommended_models, fetch_openrouter_models
...
def get_default_model_from_cache(provider: str) -> str | None:
    """The manifest's labeled default for ``provider`` (the model Athena silently lands on when the
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    After ``athena update`` that file IS the newest catalog, so the picker stays current even when
