"""Pydantic request/response models for the Athena dashboard web server."""
...
class ImportRequest(BaseModel):
    archive: str
    # --force: the spawned `athena import` has stdin=DEVNULL, so its "Continue? [y/N]" prompt would
...
class ProfileCreate(BaseModel):
    ...
    # Installed async via `athena -p <name> skills install` (skills_hub.SKILLS_DIR is import-time-bound,
    # so ATHENA_HOME can't redirect it); PIDs go back for the UI to poll.
    hub_skills: List[str] = []
...
class ProfileExport(BaseModel):
    ...
    output: str = ""  # archive path; empty → a staging path under ATHENA_HOME
