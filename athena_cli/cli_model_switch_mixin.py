...
Mixin split out of ``cli.py``; bound onto ``AthenaCLI`` via the MRO. cli.py-internal
...
module-level functions taking ``cli`` and siblings are called as ``AthenaCLI.<name>(self, ...)``.
"""
...
def stored_session_route(session_meta, *, current_model, current_provider):
    ...
    if not stored_model:
        return None
    from athena_state import SessionDB as _SessionDB
...
def _print_switch_summary(cli, result, old_model, *, one_turn: bool, strict_context: bool) -> None:
    ...
    from athena_cli.model_switch import format_model_for_display, resolve_display_context_length
...
def _commit_model_switch(
    ...
    from cli import AthenaCLI, _cprint
    old_model = cli.model
    ...
    if not one_turn:
        AthenaCLI._persist_model_switch_to_session(cli, result)
...
def _persist_global_switch(cli, result) -> None:
    ...
    from cli import AthenaCLI, save_config_value
    AthenaCLI._clear_persisted_context_for_model_switch(cli, result)
...
class CLIModelSwitchMixin:
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # Curated list (same as `athena model` / gateway pickers); live catalog only when
