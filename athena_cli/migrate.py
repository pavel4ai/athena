...
Currently exposes only ``athena migrate xai`` — diagnoses and (with --apply) rewrites references to
...
def cmd_migrate(args: Any) -> int:
    ...
    print("usage: athena migrate xai [--apply] [--no-backup]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    print()
    print(color("Run `athena doctor` to confirm no retired xAI models remain.", Colors.DIM))
    return 0
