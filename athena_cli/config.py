"""Configuration management for Athena Agent: config.yaml / .env loading, saving,
validation, migration, and the ``athena config`` command."""
...
from athena_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
from athena_constants import get_athena_home, get_process_athena_home  # noqa: F401
...
_PARSE_FAILURE_FALLBACK_MSG = {
    ...
    "refuse-write": (
        ...
        "Fix the YAML (athena config edit) and retry.")}
...
def _warn_config_parse_failure(
    ...
    from athena_cli.config_backups import backup_config
...
# interpreter init (PYTHON*, NODE_*: Athena restarts through them), PATH (fix tool lookup
...
# and Athena runtime-location / security-policy flags (config.yaml is the supported surface).
#
# ``ATHENA_*`` overall is NOT blocked — many integration credentials use that prefix
# (ATHENA_LANGFUSE_PUBLIC_KEY, ATHENA_SPOTIFY_CLIENT_ID, ...). The denylist is name-by-name so
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Athena runtime location
    "ATHENA_HOME", "ATHENA_PROFILE", "ATHENA_CONFIG", "ATHENA_ENV",
    "ATHENA_CONFIG_PATH", "ATHENA_ENV_PATH",
    # MCP catalog trust root; package-manager wrappers may still set it in the process env.
    "ATHENA_OPTIONAL_MCPS",
    # Local ACP subprocess selection (executable/argv authority).
    "ATHENA_COPILOT_ACP_COMMAND", "ATHENA_COPILOT_ACP_ARGS",
    # Security policy / approval-routing context — set via their dedicated controls only.
    "ATHENA_YOLO_MODE", "ATHENA_ACCEPT_HOOKS", "ATHENA_REDACT_SECRETS",
    "ATHENA_INTERACTIVE", "ATHENA_EXEC_ASK", "ATHENA_GATEWAY_SESSION",
    "ATHENA_CRON_SESSION", "ATHENA_SINGLE_QUERY_SESSION",
    "ATHENA_SESSION_KEY", "ATHENA_SESSION_PLATFORM"})
...
def validate_env_var_name_for_write(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "or Athena runtime location and security policy (ATHENA_HOME, ATHENA_YOLO_MODE, ...) "
            "cannot be persisted via the env writer. If you really need this, edit ~/.athena/.env "
            "directly.")
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # back-compat fallback. The boolean ATHENA_TOOL_PROGRESS variant is unsupported (its only
    # consumer, the v3->4 migration, is below the v12 support floor); doctor flags it as ignored.
    "ATHENA_TOOL_PROGRESS_MODE",
    ...
    "ATHENA_LANGFUSE_ENV", "ATHENA_LANGFUSE_RELEASE", "ATHENA_LANGFUSE_SAMPLE_RATE",
    "ATHENA_LANGFUSE_MAX_CHARS", "ATHENA_LANGFUSE_CAPTURE", "ATHENA_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY", "LANGFUSE_SECRET_KEY", "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so profiles can use different backends.
    "ATHENA_ACP_AUTH_METHOD", "ATHENA_ACP_AUTO_APPROVE", "ATHENA_COPILOT_ACP_COMMAND",
    "ATHENA_COPILOT_ACP_ARGS", "COPILOT_CLI_PATH", "COPILOT_ACP_BASE_URL"})
...
# ATHENA_MANAGED). Module-level so tests can patch it without touching /nix/store.
...
def get_managed_system() -> Optional[str]:
    ...
    Signals: ATHENA_MANAGED env var (systemd service) or a ``.managed`` marker file in
    ATHENA_HOME (NixOS activation script — interactive shells don't see the service env)."""
    marker = os.getenv("ATHENA_MANAGED", "").strip().lower() or None
...
_NIX_UPDATE_MSG = (
    "Update Athena through the Nix source that installed it "
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Athena was installed: apt/docker/nix/nixos/home-manager/git/unknown.
    Order: code-scoped ``<install tree>/.install_method`` stamp (authoritative) -> legacy
    ``$ATHENA_HOME/.install_method`` -> managed marker -> /nix/store path -> .git dir -> unknown.
    The stamp lives next to the code because ATHENA_HOME is shared data: a container and a host
    install can bind-mount the same home, so a home-scoped ``docker`` stamp would make the host
    ``athena update`` refuse to run. A legacy ``docker`` value is therefore ignored unless we are
    ...
    the code; - the published ``nousresearch/athena-agent`` image bakes a ``docker`` stamp into
    ``/opt/athena`` at build time. An unsupported manual install dropped into a container (no stamp) falls
    through to the ``.git`` checks and behaves like any off-path install. See issue #34397.
    """
    # The stamp is a property of the running code tree (parent of athena_cli/), NOT of $ATHENA_HOME,
    ...
    method = _install_method_stamp(get_athena_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Import-safe wrapper around ``athena_constants.is_container``."""
    try:
...
_UPDATE_COMMAND_BY_METHOD = {
    "docker": "docker pull nousresearch/athena-agent:latest",
    "apt": "pkg upgrade athena-agent",  # "apt" == Termux APT by contract; uses Termux's `pkg`.
}
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    return _UPDATE_COMMAND_BY_METHOD.get(method, "athena update")
...
# Shared by ``cmd_update`` and ``_cmd_update_check`` (athena_cli/main.py) so the wording never
# forks. The published image excludes ``.git``, so the git update path can never succeed there
# and the generic "reinstall via install.sh" fallback would install a NEW host-side Athena.
...
def get_container_exec_info() -> Optional[dict]:
    ...
    already inside the container, or when ATHENA_DEV=1 is set. Only FileNotFoundError is
    ...
    try:
        info = {}
        with open(get_athena_home() / ".container-mode", "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
...
# ---- Config paths / ATHENA_HOME skeleton ----
...
def require_parseable_user_config(*, ignore_user_config: bool = False) -> None:
    ...
    states; ``--ignore-user-config`` / ATHENA_IGNORE_USER_CONFIG=1 remain authoritative."""
    if ignore_user_config or os.environ.get("ATHENA_IGNORE_USER_CONFIG") == "1":
        return
...
def _resolve_athena_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read ATHENA_UID / ATHENA_GID (set by Docker deployments); (None, None) if unset/invalid/Windows.
    The entrypoint chowns ATHENA_HOME once, but subdirs created at runtime (``profiles/<name>/``)
    need the same chown or they land root:root and block later uid-mapped workers.

    Docker containers running Athena commonly set these to map the in-container user to a host user so
    ...
    return _env_int("ATHENA_UID"), _env_int("ATHENA_GID")


def _chown_to_athena_uid(path) -> None:
    """Chown ``path`` to ``ATHENA_UID:ATHENA_GID`` when set; EPERM/ENOENT are non-fatal (the
    ...
    :func:`ensure_athena_home` on Docker deployments. See #34107.
    """
...
def _secure_dir(path):
    """chmod a directory owner-only (0700) and apply ATHENA_UID/GID ownership. No-op when managed.
    ATHENA_HOME_MODE (e.g. 0701) overrides the mode so a web server can traverse ATHENA_HOME to
    a served subdirectory without directory listings.

    Also applies ``ATHENA_UID``/``ATHENA_GID``-based ownership when those env vars are set (#34107 — Docker
    ...
    if is_managed():
        return
    try:
        mode = int(os.environ.get("ATHENA_HOME_MODE", "").strip() or "700", 8)
    except ValueError:
        mode = 0o700
    try:
        os.chmod(path, mode)
...
def _is_container() -> bool:
    """Detect Docker/Podman/LXC (or ATHENA_CONTAINER / ATHENA_SKIP_CHMOD opt-out).
...
_ATHENA_HOME_ENSURED: set = set()
_ATHENA_HOME_SUBDIRS = (
    "cron", "sessions", "logs", "logs/curator", "memories",
    "pairing", "hooks", "image_cache", "audio_cache", "skills")


def ensure_athena_home():
    """Ensure the ~/.athena directory skeleton exists with secure permissions.
    ...
    from athena_constants import assert_named_profile_home_live
    assert_named_profile_home_live(home)
    if key in _ATHENA_HOME_ENSURED and home.is_dir():
        return
    from athena_cli.config_home import initialize_home
    initialize_home(home, _ATHENA_HOME_SUBDIRS, _ATHENA_HOME_ENSURED)


# ---- Config loading/saving ----

from athena_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: E402,F401
from athena_cli.config_providers import (  # noqa: E402,F401  (re-exported; callers/tests use athena_cli.config.<name>)
...
# Back-compat re-exports — :mod:`athena_cli.personality` owns personality/overlay semantics.
from athena_cli.personality import (  # noqa: E402,F401
...
def _split_key_path(key: str) -> list[str]:
    ...
    ``athena config set`` uses ``.`` as the nesting separator, so a key that itself contains a literal dot
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `athena config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    return (
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # A malformed SKILL.md must never break `athena update`; this prompting is a nicety.
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (athena_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard
    "platform_toolsets",     # written by the setup wizard
    "known_plugin_toolsets", # athena_cli/tools_config.py toolset-save flow
    ...
    for _key in ("backend", "search_backend", "extract_backend"):
        ...
        if note:
            _issue(issues, "warning",
                   ...
                   "Run 'athena tools' and pick a different Web Search & Extract provider")
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            from athena_cli.config_home import config_load_issue
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ athena config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
...
def _warn_invalid_platform_toolsets(results: Dict[str, Any], quiet: bool) -> None:
    ...
    try:
        ...
        from athena_cli.toolset_scope import toolset_allowed_for_platform
...
def _env_expand_match(m: re.Match) -> str:
    ...
    if inner.startswith("env:"):
        logger.warning(
            "Config ref %r: %s is not set (check ~/.athena/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    made against a different environment (load before ``load_athena_dotenv()``, in-process
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    LiteLLM users reach for, and ``athena config set`` blindly accepts any dotted key — so
    ...
    from OpenAI-compatible backends) — while display paths (``athena status``/``dump``) read ``name`` and
...
# the ATHENA_MAX_ITERATIONS env bridge, works in every ``<``/``>=``/``max - used`` comparison in
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    """Whether the remote SSH shell must expand *cwd* itself: ``~`` expanded on the Athena host
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # load_athena_dotenv() pins unexpanded literals for the process lifetime.
            # Without this, a load_config() that ran before load_athena_dotenv() pins unexpanded literals
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Athena writes itself (bare, 'single', or "double" with
    ``\\"`` / ``\\\\`` escapes)."""
...
def load_env() -> Dict[str, str]:
    """Load ~/.athena/.env as a dict (memoised; ``get_env_value()`` runs hundreds of times per
...
def sanitize_env_file() -> int:
    """Rewrite ~/.athena/.env with normalized line formatting; returns the number of changed lines."""
    env_path = get_env_path()
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.athena/.env (also matching ``export KEY=`` lines, so a save
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    so two endpoints on one host get separate slots. The fixed ``ATHENA_CUSTOM_`` prefix keeps the
    ...
    return f"ATHENA_CUSTOM_{slug}_API_KEY" if slug else "ATHENA_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.athena/.env and os.environ; True if it was found and removed."""
...
def save_env_value_secure(key: str, value: str) -> Dict[str, Any]:
    ...
    from athena_cli.credential_lifecycle import save_provider_env_credential
...
def reload_env() -> int:
    """Re-read ~/.athena/.env into os.environ; returns count of vars changed.
    Removes deleted vars only when known to Athena (OPTIONAL_ENV_VARS and _EXTRA_ENV_KEYS) so
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` (scope-aware) or ``~/.athena/.env``.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a Athena-managed credential preferring ``~/.athena/.env`` over ``os.environ``, so a
...
def _show_model_section(config: Dict[str, Any]) -> None:
    ...
    # Read the .env FILE directly so a stale ATHENA_MAX_ITERATIONS ghost is caught even when the
    # gateway bridge already overrode os.environ.
    try:
        env_ghost = load_env().get("ATHENA_MAX_ITERATIONS")
    except Exception:
        env_ghost = None
    if env_ghost is not None and str(env_ghost).strip() != str(cfg_max_turns).strip():
        print(color(f"                ⚠ .env has stale ATHENA_MAX_ITERATIONS={env_ghost} "
                    f"(run 'athena doctor --fix' to remove)", Colors.YELLOW))
...
def resolve_cron_model_drift_defaults(
    ...
    Mirrors the scheduler's precedence: a truthy configured model wins over ``ATHENA_MODEL``; the
    ...
    return provider, configured_model or _model_assignment_text(env.get("ATHENA_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "`athena cron edit <job_id> --provider <provider> --model <model>` or set a fleet default "
        "with `athena config set cron.model <model>`.")
...
def _coerce_config_set_value(key: str, value: str) -> Any:
    """Auto-coerce a ``athena config set`` string to bool/None/int/float/list/dict.
...
def _redirect_platform_display_key(key: str) -> tuple[str, Optional[str]]:
    ...
    Before #71047 a write such as ``athena config set platforms.telegram.streaming false`` landed on a key
...
def _guard_section_overwrite(key: str, value: Any, user_config: Dict[str, Any], force: bool) -> str:
    ...
    err += [
        ...
        f"    athena config set {key}.<sub-key> <value>",
        "  Or use --force to replace the entire section:",
        f"    athena config set --force {key} {value!r}"]
...
def _touch_skin_file(key: str, value: Any) -> None:
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_athena_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
            pass
...
def _print_unknown_key_notice(key: str, suggestion: Optional[str]) -> None:
    print(color(
        ...
        "but Athena may not read it.", Colors.YELLOW))
    if suggestion:
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    if key.strip().lower() in ("model.api_base", "api_base"):
        # Normalize the api_base → base_url alias at set-time too (issue #8919), so a fresh `athena config
...
_USAGE_GET = ("Usage: athena config get <key> [--json]", [
    "athena config get model", "athena config get terminal.backend",
    "athena config get skills.config --json"], None)
_USAGE_SET = ("Usage: athena config set [--force] <key> <value>", [
    "athena config set model anthropic/claude-sonnet-4", "athena config set terminal.backend docker",
    "athena config set OPENROUTER_API_KEY sk-or-..."], [
    "", "  --force: skip the unknown-key notice for unrecognized keys,",
    "           and allow a scalar to replace a whole mapping section"])
_USAGE_UNSET = ("Usage: athena config unset <key>", [
    "athena config unset model", "athena config unset terminal.backend",
    "athena config unset OPENROUTER_API_KEY"], None)
...
_CONFIG_USAGE = """Available commands:
  ...
  athena config get <key>          Print a resolved config value
  athena config set <key> <value>   Set a config value
  athena config unset <key>        Remove a config value
  athena config check     Check for missing/outdated config
  athena config migrate   Update config with new options
  athena config path      Show config file path
  athena config env-path  Show .env file path"""
...
def _inject_platform_plugin_env_vars() -> None:
    ...
    Chat etc. are configurable in ``athena config`` UI without the core knowing they exist.
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    installs, ``/opt/athena`` inside the published image. It is a property of
    the running interpreter, NOT of ``$ATHENA_HOME``, which is why a
...
_PLUGIN_COMPAT_LAZY = {
    'normalize_route_base_url': ('athena_cli.route_identity', 'normalize_route_base_url'),
}
