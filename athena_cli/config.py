...
- ~/.athena/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.athena/.env         - API keys and secrets

This module provides:
- athena config          - Show current configuration
- athena config edit     - Open config in editor
- athena config set      - Set a specific value
- athena config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    ``config.yaml`` in place: athena never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.athena/config.yaml`` causes ``load_config()``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  athena config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
# * ``ATHENA_HOME`` / ``ATHENA_PROFILE`` / ``ATHENA_CONFIG`` /
#   ``ATHENA_ENV`` — Athena runtime location flags. Writing these into
...
#
# IMPORTANT: ``ATHENA_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (ATHENA_LANGFUSE_PUBLIC_KEY,
# ATHENA_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # NOT a ATHENA_* blanket: integration credentials (ATHENA_GEMINI_*,
    # ATHENA_LANGFUSE_*, ATHENA_SPOTIFY_*, ...) ARE allowed.
    "ATHENA_HOME", "ATHENA_PROFILE", "ATHENA_CONFIG", "ATHENA_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "(ATHENA_HOME, ATHENA_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "ATHENA_TOOL_PROGRESS", "ATHENA_TOOL_PROGRESS_MODE",
    ...
    "ATHENA_LANGFUSE_ENV",
    "ATHENA_LANGFUSE_RELEASE",
    "ATHENA_LANGFUSE_SAMPLE_RATE",
    "ATHENA_LANGFUSE_MAX_CHARS",
    "ATHENA_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from athena_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ATHENA_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_athena_home() / ".managed"
...
def is_managed() -> bool:
    ...
    Two signals: the ATHENA_MANAGED env var (set by the systemd service),
    or a .managed marker file in ATHENA_HOME (set by the NixOS activation
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``athena_cli/`` — i.e. the git checkout for source
    installs, ``/opt/athena`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$ATHENA_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    2. Legacy home-scoped stamp ``$ATHENA_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. ATHENA_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.athena``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$ATHENA_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.athena:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$ATHENA_HOME``, a
    ...
    ``docker`` and ``athena update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/athena-agent`` image bakes a ``docker``
        stamp into ``/opt/athena`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $ATHENA_HOME.
    try:
    ...
    #    host install whose shared $ATHENA_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``athena update``.
    try:
        method = (
            (get_athena_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``athena_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$ATHENA_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/athena`` in the published image, which instead bakes the stamp at
...
# athena_cli/main.py; lives here so the wording stays consistent and we
...
Notes:
  ...
  • Your config and session history live under ``$ATHENA_HOME`` (``/opt/data``
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ATHENA_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, athena_bin
    or None if container mode is not active, we're already inside the
    container, or ATHENA_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ATHENA_DEV") == "1":
        return None
    ...
    container_mode_file = get_athena_home() / ".container-mode"
    ...
    athena_bin = info.get("athena_bin", "/data/current-package/bin/athena")
...
# Re-export from athena_constants — canonical definition lives there.
from athena_constants import get_athena_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_athena_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_athena_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_athena_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the ATHENA_UID / ATHENA_GID env vars set by Docker deployments.
    ...
    ownership. The entrypoint chowns the top-level ATHENA_HOME once, but
    subdirectories created at runtime by ``ensure_athena_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("ATHENA_UID", "").strip()
    gid_str = os.environ.get("ATHENA_GID", "").strip()
    try:
...
def _chown_to_athena_uid(path) -> None:
    """Chown ``path`` to ``ATHENA_UID:ATHENA_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_athena_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_athena_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the athena group can
    share state with the gateway service.

    The mode can be overridden via the ATHENA_HOME_MODE environment variable
    (e.g. ATHENA_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ATHENA_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``ATHENA_UID``/``ATHENA_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ATHENA_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    """
    # Explicit opt-out
    if os.environ.get("ATHENA_CONTAINER") or os.environ.get("ATHENA_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set ATHENA_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ATHENA_HOME if the user doesn't have one yet."""
...
def ensure_athena_home():
    """Ensure ~/.athena directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_athena_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_athena_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ATHENA_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # identity slot (SOUL.md). Empty by default. The ATHENA_ENVIRONMENT_HINT
    ...
    "terminal": {
        ...
        #             ATHENA_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force ATHENA_HOME/home when it exists (old strict
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.athena/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: athena sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .athena.md, or .cursorrules before Athena applies
    ...
    # See tools/kanban_tools.py and athena_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``athena dashboard`` -> Notifications).
    "kanban": {
    ...
    "display": {
        ...
        # e.g. `model · 68% · ~/projects/athena`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # ``ATHENA_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``ATHENA_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``ATHENA_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # Public URL override (env: ``ATHENA_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/athena``) —
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.athena/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.athena/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # always goes to ~/.athena/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${ATHENA_SKILL_DIR} and ${ATHENA_SESSION_ID} in SKILL.md
    ...
    "curator": {
        ...
        # class-level umbrellas. `athena curator run --consolidate` overrides
        ...
        # skipped), snapshot ~/.athena/skills/ into
        # ~/.athena/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
    ...
    # This section is only needed for athena-specific overrides; everything else
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (ATHENA_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Athena' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.athena/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ATHENA_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # ``athena_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
    ...
    "cron": {
        ...
        # $ATHENA_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via ATHENA_CRON_MAX_PARALLEL env var.
    ...
    # Logging — controls file logging to ~/.athena/logs/.
    ...
    "model_catalog": {
        "enabled": True,
        "url": "https://github.com/pavel4ai/athena/docs/api/model-catalog.json",
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.athena/.env,
        ...
        # user. Bridged to ATHENA_MEDIA_DELIVERY_STRICT.
        "strict": False,
        ...
        # cache (~/.athena/cache/{documents,images,audio,video,screenshots})
        ...
        # to ATHENA_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # ATHENA_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to ATHENA_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.athena/state.db.
    ...
    "sessions": {
        ...
        # rewrites ``~/.athena/sessions/session_{sid}.json`` on every turn
    ...
    "updates": {
        # Run a full ``athena backup``-style zip of ATHENA_HOME before every
        # ``athena update``.  Backups land in ``<ATHENA_HOME>/backups/`` and
        # can be restored with ``athena import <path>``.  Defaults to true
        # after the #48200 incident: a ``athena update --yes`` run that
    ...
    "lsp": {
        ...
        #              ``<ATHENA_HOME>/lsp/bin/`` on first use.
    ...
    # rather than storing them in ~/.athena/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.athena/.env (or your shell) and never in config.yaml.
            ...
            # ~/.athena/bin/ on first use.  When False you must install
    ...
    "computer_use": {
        ...
        # by default upstream. Athena disables it for our users unless they
        # explicitly opt in here. When false (default), Athena sets
...
OPTIONAL_ENV_VARS = {
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Athena, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ATHENA_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #athena)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: athena-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    # ATHENA_TOOL_PROGRESS and ATHENA_TOOL_PROGRESS_MODE are deprecated —
    ...
    "ATHENA_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.athena.config`` entries, then checks
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``athena_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'athena doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("ATHENA_HOME", "~/.athena")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("ATHENA_TOOL_PROGRESS")
            old_mode = get_env_value("ATHENA_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from ATHENA_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from ATHENA_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("ATHENA_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from ATHENA_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$ATHENA_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_athena_home() / "plugins"
    ...
    #   3. Creates `~/.athena/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_athena_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_athena_home() / "logs" / "curator"
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from athena_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    # metadata.athena.config in their SKILL.md frontmatter.
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``athena config set``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _athena_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.athena/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.athena/config.yaml.
    ...
    (which change ``ATHENA_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/athena/config.yaml invalidates the
...
#   openai-codex (OAuth — athena auth) — OpenAI Codex
#   nous         (OAuth — athena auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.athena/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.athena/.env.
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.athena/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.athena/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.athena/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.athena/.env into os.environ. Returns count of vars updated.
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.athena/.env or environment."""
...
def show_config():
    ...
    from athena_cli.auth import get_anthropic_key
    ...
    # Warn on stale ATHENA_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("ATHENA_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale ATHENA_MAX_ITERATIONS={_env_ghost} "
                f"(run 'athena doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  athena config edit     # Edit config file", Colors.DIM))
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `athena config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `athena config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if missing_config:
            ...
            print("    Run 'athena config migrate' to add them")
        ...
        print("  athena config           Show current configuration")
        print("  athena config edit      Open config in editor")
        print("  athena config set <key> <value>   Set a config value")
        print("  athena config check     Check for missing/outdated config")
        print("  athena config migrate   Update config with new options")
        print("  athena config path      Show config file path")
        print("  athena config env-path  Show .env file path")
        sys.exit(1)
