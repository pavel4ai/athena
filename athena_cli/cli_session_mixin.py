...
Mixin bound onto ``AthenaCLI`` via the MRO. cli.py-internal symbols are imported LAZILY
...
def _reset_model_to_config_default(cli, silent: bool) -> None:
    ...
    if not _config_model or _config_model == getattr(cli, "model", None):
        return
    try:
        from athena_cli.model_switch import switch_model as _switch_model
...
class CLISessionMixin:
    ...
    def _show_session_status(self):
        ...
        lines = [
            "Athena CLI Status", "", f"Session ID: {self.session_id}", f"Path: {display_athena_home()}",
        ]
        if title:
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from athena_cli.timefmt import relative_time as _relative_time
    ...
    def _notify_session_boundary(self, event_type: str) -> None:
        ...
        with contextlib.suppress(Exception):
            from athena_cli.lifecycle import finalize_session, invoke_hook
    ...
    def save_conversation(self, cmd: str = "/save"):
        ...
        if filename:
            ...
            path = saved_dir / f"athena_conversation_{timestamp}.{fmt}"
        ...
        try:
            ...
            # tools/subprocesses on this thread resolve ATHENA_SESSION_ID to the child id after an
    ...
    def _write_terminal_breadcrumb(self) -> None:
        """Record this terminal's live session for bare ``athena -c``. Called whenever
        ...
        with contextlib.suppress(Exception):
            from athena_cli.terminal_breadcrumbs import write_breadcrumb
    ...
    def _toggle_yolo(self):
        ...
        ``ATHENA_YOLO_MODE``: that env var is frozen into ``tools.approval._YOLO_MODE_FROZEN``
        ...
        if _YOLO_MODE_FROZEN:
            _cprint(
                ...
                " for this process (started with --yolo / ATHENA_YOLO_MODE)."
    ...
    def _print_exit_summary(self, clear_screen: bool = True):
        ...
        # the hint ("default"/"custom" use the standard ATHENA_HOME).
        try:
