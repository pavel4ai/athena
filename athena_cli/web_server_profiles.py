...
from athena_cli.config import DEFAULT_CONFIG, get_process_athena_home
from athena_cli.web_models import MCPServerCreate
from athena_cli.web_server_gateway import _ACTION_LOG_FILES
from athena_cli.web_server_mcp import _normalize_mcp_server_create
...
@contextmanager
def _athena_home_scope(path) -> Any:
    """Scope ``load_config``/``save_config`` (anything resolving ``get_athena_home()`` at call
    time) to ``path`` for the block via the context-local ATHENA_HOME override."""
    from athena_constants import set_athena_home_override, reset_athena_home_override
    token = set_athena_home_override(str(path))
    try:
        yield
    finally:
...
def _is_other_profile(profile: Optional[str]) -> bool:
    ...
    try:
        ...
        return True
    return target.resolve() != get_process_athena_home().resolve()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    """Write MCP server entries into ``profile_dir``'s config.yaml (ATHENA_HOME-scoped).
    ...
    from athena_cli.mcp_config import _save_bearer_auth_token
    written = 0
    with _athena_home_scope(profile_dir):
        cfg = load_config()
...
def _profile_scope(profile: Optional[str]):
    ...
    Two seams: (1) ``load_config``/``save_config`` resolve ``get_athena_home()`` at call
    ...
    ``get_athena_home()`` so writes land in the live home even when the import-time binding
    is stale (test isolation, late ATHENA_HOME override). Yields the profile dir for a named
    ...
    with _config_profile_scope(profile) as scoped:
        profile_dir = get_athena_home() if scoped is None else scoped
        modules = (_skills_tool, _skill_mgr)
        with _SKILLS_PROFILE_LOCK:
            saved = [(m.ATHENA_HOME, m.SKILLS_DIR) for m in modules]
            for m in modules:
                m.ATHENA_HOME, m.SKILLS_DIR = profile_dir, profile_dir / "skills"
            try:
                yield scoped
            finally:
                for m, (home, skills_dir) in zip(modules, saved):
                    m.ATHENA_HOME, m.SKILLS_DIR = home, skills_dir


@contextmanager
def _config_profile_scope(profile: Optional[str]):
    """Await-safe, config-only profile scope: touches ONLY the task-local ATHENA_HOME
    ...
    with _athena_home_scope(profile_dir):
        yield None if profile_dir.resolve() == get_process_athena_home().resolve() else profile_dir
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    a fresh ``athena`` subprocess whose ``_apply_profile_override()`` reads ``-p`` from argv —
...
def _hub_action_name(verb: str, key: str) -> str:
    """Unique per-skill hub action name (+ registered log file): ``_spawn_athena_action``
