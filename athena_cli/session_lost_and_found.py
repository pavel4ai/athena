...
from athena_cli.session_schema_history import SCHEMA_HISTORY, reachable_physical_layouts
...
# Athena session ids are timestamps (20260812_135332_ab12cd): the strongest sentinel for schema-less rows.
...
# 3.50.7 and 3.44.6 — the same version gate athena_state applies to the
...
# caught exactly this shell converting a live Athena state.db into two
# generations. A salvage shell must therefore be version-gated, not just
# capability-gated, before it is pointed at (a copy of) a Athena database.
#
# The predicate lives in athena_cli.sqlite_runtime (stdlib-only, shared with
# the installer/update gates) so the embedded runtime and the salvage shell
# can never disagree about which versions are safe.
from athena_cli.sqlite_runtime import is_sqlite_wal_reset_vulnerable as _wal_reset_vulnerable  # noqa: E502

_WAL_RESET_VULNERABLE_GUIDANCE = (
    "salvage against a Athena database with the WAL-reset bug "
...
def _cli_supports_recover(binary: str) -> bool:
    ...
    scratch_dir = tempfile.mkdtemp(prefix="athena-recover-probe-")
...
def _type_conflicts(value: Any, declared: str) -> bool:
    ...
    non-numeric string in an INTEGER column, but Athena never writes one, so
...
# The closed sets SessionDB writes (athena_state.py: request/claim/complete/
