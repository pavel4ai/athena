"""Top-level argparse construction for the athena CLI.

Only the top-level parser and the ``chat`` subparser live here. Every other subparser (model,
gateway, sessions, …) is built by ``athena_cli/subcommands/<group>.py`` and wired in
...
# (it sets ``ATHENA_HOME`` and strips itself from ``sys.argv``), so it isn't on the parser.
...
# live parser fails. The derived path is authoritative; tests/athena_cli/
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    argparse surface — the drift that made ``athena --reasoning high chat …`` misread ``high`` as
...
def _inherited_flag(parser, *args, **kwargs):
    """``parser.add_argument`` + tag the Action ``inherit_on_relaunch`` for ``athena_cli.relaunch``."""
...
Examples:
    ...
    athena chat -q "Hello"        Single query mode
    athena --tui                  Launch the modern TUI (or set display.interface: tui)
    athena --cli                  Force the classic REPL (overrides display.interface: tui)
    athena -c                     Resume the most recent session
    athena -c "my project"        Resume a session by name (latest in lineage)
    athena --resume <session_id>  Resume a specific session by ID
    athena --resume latest        Resume the most recent session (same as -c)
    athena --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    athena setup                  Run setup wizard
    athena logout                 Clear stored authentication
    athena auth add <provider>    Add a pooled credential
    athena auth list              List pooled credentials
    athena auth remove <p> <t>    Remove pooled credential by index, id, or label
    athena auth reset <p> [t]     Clear exhaustion status for a provider, or one credential
    athena auth priority <p> <t> <n>  Move a pooled credential to priority n (0 = tried first)
    athena auth refresh <p> [t]   Refresh a pooled OAuth credential and clear its cooldown
    athena model                  Select default model
    athena fallback [list]        Show fallback provider chain
    athena fallback add           Add a fallback provider (same picker as `athena model`)
    athena fallback remove        Remove a fallback provider from the chain
    athena config                 View configuration
    athena config edit            Edit config in $EDITOR
    athena config set model gpt-4 Set a config value
    athena gateway                Run messaging gateway
    athena -s athena-agent-dev,github-auth
    athena -w                     Start in isolated git worktree
    athena gateway install        Install gateway background service
    athena sessions list          List past sessions
    athena sessions browse        Interactive session picker
    athena sessions rename ID T   Rename/title a session
    athena logs                   View agent.log (last 50 lines)
    athena logs -f                Follow agent.log in real time
    athena logs errors            View errors.log
    athena logs --since 1h        Lines from the last hour
    athena debug share             Upload debug report for support
...
def _add_top_level_flags(parser: argparse.ArgumentParser) -> None:
    ...
    inherited(parser, "-m", "--model", default=None, help=(
        ...
        "Applies to -z/--oneshot and --tui. Also settable via ATHENA_INFERENCE_MODEL env var."))
    ...
    inherited(parser, "--accept-hooks", action="store_true", default=False, help=(
        ...
        "without a TTY prompt.  Equivalent to ATHENA_ACCEPT_HOOKS=1 or "
    ...
    inherited(parser, "--ignore-user-config", action="store_true", default=False,
              help="Ignore ~/.athena/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)")
...
def _build_chat_parser(subparsers) -> argparse.ArgumentParser:
    ...
    ``athena -m foo chat`` argparse first sets ``args.model`` from the top-level parser, then
    ...
    unless the flag is actually passed after the subcommand (tests/athena_cli/
    ...
    inherited(chat_parser, "--accept-hooks", action="store_true", default=SUPPRESS, help=(
        "Auto-approve any unseen shell hooks declared in config.yaml "
        "without a TTY prompt (see also ATHENA_ACCEPT_HOOKS env var and "
    ...
    inherited(chat_parser, "--ignore-user-config", action="store_true", default=SUPPRESS,
              help="Ignore ~/.athena/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.")
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="athena", description="Athena Agent - AI assistant with tool-calling capabilities",
