"""Desktop (Electron) app: build/stamp, stage-and-swap pack, exe integrity gate, macOS signing/TCC, Linux sandbox, launch (athena gui/desktop).
...
from athena_cli.main_tui_launch import _npm_lifecycle_env
...
def _desktop_stamp_path() -> Path:
    """Path of the desktop build stamp under $ATHENA_HOME."""
...
def _renderer_bundle_dir(desktop_dir: Path, *, source_mode: bool) -> Optional[Path]:
    ...
    # macOS: …/Athena.app/Contents/MacOS/Athena → …/Contents/Resources
    resources = (
...
def _desktop_packaged_executable_in(release_dir: Path) -> Optional[Path]:
    ...
    *release_dir* is electron-builder's ``directories.output`` — the live ``apps/desktop/release`` or a
    ...
    if sys.platform == "darwin":
        ...
        candidates = [
            release_dir / d / "Athena.exe" for d in ("win-unpacked", "win-ia32-unpacked", "win-arm64-unpacked")
        ]
    else:
        candidates = [
            release_dir / d / n for d in ("linux-unpacked", "linux-arm64-unpacked") for n in ("athena", "Athena")
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # mtime can hand a wrong-architecture Athena.exe to the launcher. Prefer
        ...
        # wrong-architecture Athena.exe to the launcher, which Windows rejects with "This app can't run on
        # your computer" (#69179).
...
# PLACE: before-pack.mjs wipes ``release/<platform>- unpacked`` (or the mac ``Athena.app``) and the Electron
# unpack + asar + rename then rebuild it. Any failure after that wipe — corrupt cached zip, blocked
# download, missing dep, disk full — leaves the user with NO app, and ``athena update`` used to report
...
# (Desktop → athena-setup --update → `athena update` → `athena desktop --build-only` → relaunch) rebuilds
# Athena.exe on the end user's machine and used to verify only that the file EXISTS before declaring
...
# Athena.exe is caught BEFORE the updater replaces the working app, and the previous build can be restored
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    emulated process arch) and rejected a correctly-built ARM64 ``Athena.exe``. Binding
...
def _windows_native_machine() -> str:
    ...
    chain runs an x64 athena-setup.exe (and thus x64 Python) on Windows-on-ARM devices, where
...
def _ensure_desktop_exe_launchable(desktop_dir: Path, packaged_executable: Optional[Path]) -> tuple:
    ...
    print(f"✗ The built Athena.exe failed its integrity check: {error}\n    at: {packaged_executable}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Athena.exe from backup.")
        print("    Your existing version was kept and still works. Run `athena desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `athena desktop --force-build` to rebuild, or re-run the Athena")
...
def _desktop_macos_local_codesign(app: Path, *, desktop_dir: Path, identity: str = "-") -> bool:
    ...
    #    the outer Athena.app component.
...
def _desktop_macos_relaunchable_fixup(
    ...
    "Athena is damaged" and loses every grant. Clear quarantine xattrs, then sign
...
def _macos_create_signing_identity(
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="athena-tcc-"))
    try:
        ...
        def _export_p12(extra_args: list) -> None:
            subprocess.run(
                [
                    ...
                    "-out", str(p12), "-passout", "pass:athenalocal",
                ],
                capture_output=True, check=True)
...
def _desktop_macos_setup_tcc_identity(identity: str = "Athena Local Signing") -> bool:
    ...
    print(
        ...
        "stuck, reset it with:  tccutil reset All com.nousresearch.athena"
    )
    return True
...
def _register_linux_desktop_entry() -> None:
    """Install the XDG desktop entry for Athena Desktop (Linux only, best-effort).

    ``Exec`` and ``Icon`` are absolute so the entry works outside a login shell.
    ``athena uninstall --gui`` removes it.
    """
    from athena_cli.main import PROJECT_ROOT
    try:
        from athena_cli.linux_desktop_entry import install_desktop_entry, is_supported
        if not is_supported():
            return
...
def _install_desktop_workspace_deps(npm: str, env: dict) -> None:
    ...
    from athena_cli.main import PROJECT_ROOT
    from athena_cli.main_web_build import _run_npm_install_deterministic
...
def _run_desktop_pack_with_recovery(
    ...
    if build_result.returncode != 0 and staging_dir is not None and _staged_exe() is None:
        ...
        if restored:
            ...
            for p in purged:
                print(f"    - {p}")
            # The purge can't remove a win-unpacked tree whose Athena.exe is
...
def _promote_staged_desktop_app(desktop_dir: Path, staging_dir: Path) -> Path:
    ...
    # Athena.exe Windows cannot load. Verified on the STAGED exe, so a failure
...
def _build_desktop_app(desktop_dir: Path, *, source_mode: bool, npm: str, env: dict) -> Optional[Path]:
    ...
    if not source_mode:
        ...
        # A running desktop instance holds Athena.exe locked on Windows, so the
...
def _desktop_launch_env(args: argparse.Namespace) -> tuple[dict, list[str]]:
    ...
    for attr, key in (
        ("fake_boot", "ATHENA_DESKTOP_BOOT_FAKE"), ("ignore_existing", "ATHENA_DESKTOP_IGNORE_EXISTING")):
        if getattr(args, attr, False):
            env[key] = "1"
    if getattr(args, "athena_root", None):
        env["ATHENA_DESKTOP_ATHENA_ROOT"] = str(Path(args.athena_root).expanduser().resolve())
    cwd = getattr(args, "cwd", None)
    env["ATHENA_DESKTOP_CWD"] = str(Path(cwd).expanduser().resolve()) if cwd else os.getcwd()

    config_electron_flags, config_disable_gpu, config_password_store, config_ozone_hint = (
        _desktop_launch_options())
    if config_disable_gpu != "auto" and "ATHENA_DESKTOP_DISABLE_GPU" not in os.environ:
        env["ATHENA_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if sys.platform == "linux" and "ATHENA_DESKTOP_PASSWORD_STORE" not in os.environ:
        password_store = (
            config_password_store if config_password_store != "auto" else _detect_linux_password_store()
        )
        if password_store:
            env["ATHENA_DESKTOP_PASSWORD_STORE"] = password_store
...
def cmd_gui(args: argparse.Namespace):
    ...
    from athena_cli.main import PROJECT_ROOT
    from athena_cli.main_install_repair import _resolve_node_runtime_npm
    desktop_dir = PROJECT_ROOT / "apps" / "desktop"
    ...
    if getattr(args, "setup_tcc_identity", False):
        identity = getattr(args, "identity", None) or "Athena Local Signing"
    ...
    if not source_mode:
        print(f"→ Launching packaged Athena Desktop: {' '.join(launch_command)}")
