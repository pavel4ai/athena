"""Athena Desktop (Chat GUI) uninstaller: removes only GUI state — built Electron artifacts, the packaged
...
def desktop_userdata_dir() -> Path:
    """Electron ``app.getPath('userData')`` for an app named "Athena" on each platform (GUI-only state)."""
    home = Path.home()
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Athena"
    if sys.platform == "win32":
        return _env_dir("APPDATA", home / "AppData" / "Roaming") / "Athena"
    return _env_dir("XDG_CONFIG_HOME", home / ".config") / "Athena"


def source_built_gui_artifacts(athena_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``athena desktop`` inside the checkout (same ``athena-agent/`` layout
    ...
    return [desktop_dir / "dist", desktop_dir / "release", desktop_dir / "node_modules",
            agent_root / "node_modules", athena_home / "desktop-build-stamp.json"]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    home = Path.home()
    if sys.platform == "darwin":
        return [Path("/Applications/Athena.app"), home / "Applications" / "Athena.app"]
    if sys.platform == "win32":
        ...
        return [local_base / "Programs" / "Athena", local_base / "athena-desktop"] + (
            [Path(program_files) / "Athena"] if program_files else [])
    ...
    from athena_cli.linux_desktop_entry import desktop_entry_path
    ...
    return [desktop_entry_path(), data_base / "applications" / "Athena.desktop"] + [
        icons / size / "apps" / "athena.png"
        for size in ("scalable", "24x24", "32x32", "48x48", "256x256", "512x512", "1024x1024")]


def agent_is_installed(athena_home: Path) -> bool:
    """True when a usable Python agent install exists under ATHENA_HOME (gates the desktop UI's options).
    Package source or a venv alone is enough — a source checkout without a venv is still "the agent is here"."""
    return any((athena_home / "athena-agent" / sub).is_dir() for sub in ("athena_cli", "venv", ".venv"))


def gui_is_installed(athena_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    return any(p.exists() for p in (*source_built_gui_artifacts(athena_home), *packaged_gui_app_paths(), desktop_userdata_dir()))


def gui_install_summary(athena_home: "Path | None" = None) -> dict:
    ...
    return {"athena_home": str(home), "agent_installed": agent_is_installed(home),
...
def uninstall_gui(athena_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    if sys.platform.startswith("linux"):
        # The desktop entry was removed above but the menu caches still list it; reindex so Athena
        # disappears from the launcher.
        try:
            from athena_cli.linux_desktop_entry import desktop_entry_path, refresh_desktop_databases
        ...
        log_info("If you installed the desktop via a .deb / .rpm package, remove it with your package manager "
                 "(e.g. 'sudo apt remove athena' or 'sudo dnf remove athena'). AppImage builds are a single "
