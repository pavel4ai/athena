...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``athena desktop`` / ``athena gui``)
     Built inside the agent checkout under ``$ATHENA_HOME/athena-agent/``:
       ...
       - ``$ATHENA_HOME/desktop-build-stamp.json`` (the build freshness stamp)
...
the app name ("Athena"), separate from ``$ATHENA_HOME``:
...
def _agent_root(athena_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return athena_home / "athena-agent"
...
def source_built_gui_artifacts(athena_home: Path) -> "list[Path]":
    ...
    """
    agent_root = _agent_root(athena_home)
    ...
    return [
        ...
        athena_home / "desktop-build-stamp.json",
    ]
...
def agent_is_installed(athena_home: Path) -> bool:
    """Return True when a usable Python agent install exists under ATHENA_HOME.
    ...
    if (agent_root / "athena_cli").is_dir():
        return True
...
def gui_is_installed(athena_home: Path) -> bool:
    ...
    for p in source_built_gui_artifacts(athena_home):
        if p.exists():
            return True
...
def uninstall_gui(athena_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``athena-agent/athena_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$ATHENA_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove athena' or "
            "'sudo dnf remove athena'). AppImage builds are a single file you "
