...
``$ATHENA_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-athena-setup
(the stage2 hook) has chowned the volume and seeded $ATHENA_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$ATHENA_HOME``,
    not under ``profiles/``). The dispatcher in ``athena_cli.gateway``
    ...
    ``$ATHENA_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        athena_home: The container's ATHENA_HOME (typically /opt/data).
            Profiles live under ``<athena_home>/profiles/<name>/``;
            the default profile lives at ``<athena_home>`` itself.
    ...
    default_prior_state = legacy_default_state or _read_desired_state(athena_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(athena_home)
    ...
    profiles_root = athena_home / "profiles"
    ...
    if not dry_run:
        _write_reconcile_log(athena_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = athena_home / "gateway_state.json"
    if state_file.exists():
        return None

    if os.environ.get("ATHENA_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the athena args.

    Two container-command argv shapes are handled:

    * **s6-overlay v2 / tini:** PID 1 argv is
      ``/init /opt/athena/docker/main-wrapper.sh <subcommand> [args...]``.
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
      rc.init-launched process as ``/bin/sh -e
      /run/s6/basedir/scripts/rc.init top /opt/athena/docker/main-wrapper.sh
    ...
    ``athena`` invocations carry no wrapper, so fall back to peeling a bare
    ``init`` prefix. The wrapper re-execs ``athena <subcommand>``, so an
    explicit leading ``athena`` is peeled too. Shared by the legacy-gateway
    ...
    # The wrapper re-execs `athena <subcommand>`; peel an explicit athena.
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``athena dashboard ...``) never spawns or
    ...
    the gateway and dashboard containers share a bind-mounted ATHENA_HOME,
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit athena
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the athena user) won't hit EACCES. See
...
def _write_reconcile_log(
    athena_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $ATHENA_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    # bind-mounted ATHENA_HOME, both race to flock() the same s6-log lock
    ...
    actions = reconcile_profile_gateways(
        athena_home=athena_home, scandir=scandir,
    )
    for a in actions:
        print(
