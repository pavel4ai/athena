...
def detect_service_manager() -> ServiceManagerKind:
    ...
    backend-agnostic code (profile hooks, the s6 dispatch in ``athena gateway``).
    """
    # Deferred so importing this module (Protocol type, validate_profile_name) doesn't drag in
    # the whole gateway dependency graph.
    from athena_cli.gateway import is_macos, is_windows, supports_systemd_services
    ...
    # made the s6 dispatch inert on Fly, so `athena gateway start` spawned a foreground gateway
...
def _s6_running() -> bool:
    ...
    Must work for the unprivileged athena user too: ``/proc/1/exe`` is unreadable for other UIDs
    ...
    resolved name is the literal ``"exe"`` and detection always fails. Since every Athena runtime call
    inside the container drops to athena via ``s6-setuidgid``, that silent failure made the entire
...
# ---------------------------------------------------------------------------
# Host backends: thin facades over ``athena_cli.gateway`` (systemd/launchd) and
# ``athena_cli.gateway_windows``. The protocol's ``name`` parameter is unused here — host backends
# operate on the currently active profile (``athena -p <profile>``); the shape exists for s6 where
# each profile maps to a distinct service directory.
# ---------------------------------------------------------------------------


class _HostServiceManager:
    """``start``/``stop``/``restart`` resolve to ``<_fn_prefix><op>`` on ``athena_cli.<_backend>``
    ...
    def _backend_module(self):
        import importlib
        import athena_cli
        importlib.import_module(f"athena_cli.{self._backend}")
        return getattr(athena_cli, self._backend)
...
class SystemdServiceManager(_HostServiceManager):
    """Wraps the ``systemd_*`` functions in athena_cli.gateway (host call sites still use those
...
class LaunchdServiceManager(_HostServiceManager):
    """Wraps the ``launchd_*`` functions in athena_cli.gateway."""
...
class WindowsServiceManager(_HostServiceManager):
    """Wraps ``athena_cli.gateway_windows`` (Scheduled Task / Startup-folder fallback).
...
# `athena profile create` inside the container. Static services (main-athena, dashboard) live in
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    the caller's ATHENA_HOME blindly: derive the shared profile root and map the suffix to the root
    ...
    root = athena_home.parent.parent if athena_home.parent.name == "profiles" else athena_home
...
# UID/GID of the in-image ``athena`` user; hardcoded to match what ``stage2-hook.sh`` enforces
# (tests/docker/test_uid_remap.py). s6-supervise starts as root and drops via ``s6-setuidgid``.
_ATHENA_UID = 10000
_ATHENA_GID = 10000


def _chown_athena(path: Path) -> None:
    try:
        os.chown(path, _ATHENA_UID, _ATHENA_GID)
    except PermissionError:
        # Already running as athena → the dir is athena-owned by default; swallowing keeps root
        # and unprivileged callers on one code path.
        pass


def _seed_supervise_skeleton(svc_dir: Path) -> None:
    """Pre-create athena-owned ``supervise/`` and top-level ``event/`` inside a service directory.

    s6-supervise (root) creates ``event/``/``supervise/`` 0700 and the control FIFO 0600, so the
    athena user gets EACCES on every ``s6-svc``/``s6-svstat``. s6 treats EEXIST as success and skips
...
class S6ServiceManager:
    ...
    def _render_run_script(profile: str, extra_env: dict[str, str]) -> str:
        ...
        Sources ATHENA_HOME via with-contenv (run time, not baked in), resets ``HOME`` before the
        privilege drop so root's HOME does not leak, activates the venv, drops to athena.
        ``profile == "default"`` emits NO ``-p`` flag: it is the sentinel for the root ATHENA_HOME
        ...
        allocator: because each supervised profile gateway loads its own ``ATHENA_HOME``, two profiles that
        ...
        deterministic SHA-256 allocator in ``athena_cli.profiles._allocate_gateway_port``). PR #30136 review
        ...
        # athena_cli.main._apply_profile_override; kept alongside the s6 one for back-compat.
        lines.append("export ATHENA_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its ATHENA_HOME. Without it
        # a gateway started OUTSIDE s6 (stray ``athena gateway run``, an agent action, the Open
    ...
    def _render_log_run(profile: str) -> str:
        ...
        return (
            ...
            # Create the leaf and clear a stale s6-log lock AS ATHENA when starting as root. Never
            # chown/unlink athena-writable volume paths from this restartable root-context script:
            # an unprivileged user can race a pathname op through a symlink swap (CWE-59/CWE-367).
            # Parent logs/gateways is seeded athena-owned at stage2 boot (test_log_dir_seed.py).
            # See #45258.
            f'if [ "$(id -u)" = 0 ]; then\n'
            f'  s6-setuidgid athena mkdir -p "$log_dir"\n'
            f'  s6-setuidgid athena rm -f "$log_dir/lock"\n'
            f'else\n'
            f'  mkdir -p "$log_dir"\n'
            f'  rm -f "$log_dir/lock"\n'
            f'fi\n'
    ...
    def register_profile_gateway(
        ...
        try:
            ...
            # Seed athena-owned supervise/ BEFORE publishing so the athena-user s6-svc/s6-svstat/
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        root-owned but the parent ``supervise/`` is athena-owned (see ``_seed_supervise_skeleton``)
