...
the existing module-level functions in athena_cli.gateway and
athena_cli.gateway_windows directly. This protocol is a thin facade
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    # `athena gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged athena
    ...
    container drops to athena via ``s6-setuidgid``, that silent failure
...
# in ``athena_cli.gateway`` (systemd/launchd) and ``athena_cli.gateway_windows``
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in athena_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from athena_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from athena_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from athena_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from athena_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in athena_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from athena_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from athena_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from athena_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from athena_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``athena_cli.gateway_windows`` (Scheduled Task /
...
# runs inside the container (Phase 4). Static services (main-athena, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's ATHENA_HOME blindly;
    derive the shared profile root from the current ATHENA_HOME and map the
    ...
    athena_home = Path(os.environ.get("ATHENA_HOME", "/opt/data"))
    if athena_home.parent.name == "profiles":
        root = athena_home.parent.parent
    else:
        root = athena_home
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the athena user.
    ...
    is dispatched as the athena user.
    ...
    with athena ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           athena:athena, 0755 (parent must already exist)
    ``svc_dir/event/``                     athena:athena, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 athena:athena, 0755
    ``svc_dir/supervise/event/``           athena:athena, 03730
    ``svc_dir/supervise/control``          athena:athena, 0660    (FIFO)
    ...
    slot's supervise/ was athena-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _ATHENA_UID, _ATHENA_GID)
        except PermissionError:
            # Running as the athena user already — directory is athena-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _ATHENA_UID, _ATHENA_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is athena-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _ATHENA_UID, _ATHENA_GID)
            except PermissionError:
                pass
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-athena, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources ATHENA_HOME (and any extra env) via with-contenv —
             so e.g. ``-e ATHENA_HOME=/data/athena`` is honored at run
          ...
          4. Drops to the athena user and exec's
             ``athena -p <profile> gateway run`` (or just ``athena
        ...
        root ATHENA_HOME profile" (the implicit profile that exists at
        the top of $ATHENA_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``athena_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$ATHENA_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``ATHENA_HOME``,
        ...
        allocator in ``athena_cli.profiles._allocate_gateway_port``).
        ...
        # guard.
        lines.append("export ATHENA_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's ATHENA_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``athena gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-ATHENA_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The ATHENA_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own ATHENA_HOME and s6 guarantees a
        # single supervised instance per slot, so there is no legitimate
        # supervised sibling for ``--replace`` to clobber.
        if profile == "default":
            gateway_cmd = "athena gateway run --replace"
        else:
            gateway_cmd = f"athena -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid athena {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${ATHENA_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the ATHENA_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e ATHENA_HOME=/data/athena``
        gets its logs under /data/athena/logs/..., not the build-time
        default.
        ...
        return (
            ...
            f': "${{ATHENA_HOME:=/opt/data}}"\n'
            f'log_dir="$ATHENA_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            ...
            # later then runs its log service as athena and crash-loops on
            ...
            f'chown athena:athena "$ATHENA_HOME/logs/gateways" 2>/dev/null || true\n'
            f'chown -R athena:athena "$log_dir" 2>/dev/null || true\n'
            f'rm -f "$log_dir/lock"\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid athena s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``athena -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with athena ownership
            ...
            # the athena user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is athena-owned (see ``_seed_supervise_skeleton``), and on
