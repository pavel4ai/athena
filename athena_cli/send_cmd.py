...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"athena send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"athena send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"athena send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
...
def _load_athena_env() -> None:
    """Populate ``os.environ`` from ``~/.athena/.env`` AND bridge top-level
    ...
    1. ``load_dotenv(~/.athena/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.athena/config.yaml`` into
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.athena/.env and ~/.athena/config.yaml into os.environ so the
