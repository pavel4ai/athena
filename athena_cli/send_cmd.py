"""CLI subcommand: ``athena send`` — pipe text from shell scripts to any configured messaging platform
...
def _read_message_body(positional: Optional[str], file_path: Optional[str]) -> Optional[str]:
    ...
    if file_path:
        ...
        try:
            ...
            _fail(
                f"athena send: {file_path} is not a text file. --file reads the "
                ...
                f'  athena send --to telegram "MEDIA:{file_path}"\n'
                f'  athena send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  athena send --to telegram "[[as_document]] MEDIA:{file_path}"',
                _USAGE_EXIT)
        except OSError as exc:
            _fail(f"athena send: cannot read {file_path}: {exc}", _USAGE_EXIT)
...
def _emit_result(result_json: str, *, json_mode: bool, quiet: bool) -> int:
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"athena send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        return _fail(f"athena send: failed to load channel directory: {exc}")
    try:
        raw = load_directory()
    except Exception as exc:
        return _fail(f"athena send: failed to read channel directory: {exc}")
    ...
    if platform_filter:
        ...
        if not filtered:
            return _fail(
                f"athena send: no targets found for platform '{platform_filter}'. "
    ...
    if not platforms:
        ...
        print("channel discovery can populate ~/.athena/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_athena_env() -> None:
    """Populate ``os.environ`` from ``~/.athena/.env`` AND bridge top-level ``config.yaml`` keys into
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    _load_athena_env()  # the downstream gateway config loader reads credentials from os.environ
    if getattr(args, "list_targets", False):  # --list short-circuits everything else
        # `athena send --list telegram` lands "telegram" in the `message` positional.
        exit_code = _list_targets(getattr(args, "message", None), json_mode=getattr(args, "json", False))
        sys.exit(exit_code)
    target = (getattr(args, "to", None) or "").strip()
    if not target:
        _fail(
            "athena send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  athena send --to telegram \"hello\"\n"
            "  athena send --to discord:#ops --file report.md\n"
            "  athena send --list      # list available targets",
            _USAGE_EXIT)
    message = _read_message_body(getattr(args, "message", None), getattr(args, "file", None))
    if message is None or not message.strip():
        _fail(
            "athena send: no message provided. Pass text as a positional "
    ...
    # Lazy import keeps `athena send --help` fast (no tool registry / gateway config stack).
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            ...
            "credentials (~/.athena/.env + ~/.athena/config.yaml) — no LLM, "
        ...
        epilog=(
            ...
            "  athena send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
