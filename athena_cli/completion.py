"""Shell completion script generation for athena CLI. Walks the live argparse parser tree, so
    ...
    for cmd, info in subcommands:
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_athena_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;")
...
_athena_profiles() {{
    local profiles_dir="$HOME/.athena/profiles"
...
_athena_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_athena_profiles)" -- "$cur"))
        return
    fi
...
complete -F _athena_completion athena
"""
...
_athena_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.athena/profiles" ]]; then
        profiles+=($HOME/.athena/profiles/*(N/:t))
    fi
...
_athena() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_athena_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'athena command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        ...
        "# Add to your config:",
        "#   athena completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __athena_profiles",
        "    echo default",
        "    if test -d $HOME/.athena/profiles",
        "        for d in $HOME/.athena/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c athena -f -s p -l profile"
        " -d 'Profile name' -xa '(__athena_profiles)'",
        "",
        "# Top-level subcommands"]
    for cmd, info in subcommands:
        lines.append(
            f"complete -c athena -f "
        ...
        if cmd == "profile":  # profile names for the actions that take one
            for action in sorted(_PROFILE_NAME_ACTIONS):
                lines.append(
                    ...
                    f"-a '(__athena_profiles)' -d 'Profile name'")
    lines.append("")
    return "\n".join(lines)
