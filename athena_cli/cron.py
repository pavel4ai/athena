...
_GATEWAY_LIFECYCLE_PATTERNS = re.compile(
    r"(?i)"
    r"(athena\s+gateway\s+(restart|stop|start))"
    r"|(launchctl\s+(kickstart|unload|load|stop|restart)\s+.*athena)"
    r"|(systemctl\s+(-\S+\s+)*(restart|stop|start)\s+.*athena)"
    r"|(p?kill\s+.*athena.*gateway)"
)
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'athena cron create ...' or the /cron command in chat.", Colors.DIM))
        return
    ...
    if not find_gateway_pids():
        ...
        print(color("     Start it with: athena gateway install", Colors.DIM))
        print(color("                    sudo athena gateway install --system  # Linux servers", Colors.DIM))
        print()
...
def cron_status():
    ...
    if pids:
        ...
        print("    athena gateway install    # Install as a user service")
        print("    sudo athena gateway install --system  # Linux servers: boot-time system service")
        print("    athena gateway            # Or run in foreground")
...
def cron_command(args):
    ...
    print("Usage: athena cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
