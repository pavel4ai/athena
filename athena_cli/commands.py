...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.athena/skills/ for newly installed or removed skills",
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`athena_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/athena`` subcommand mapping, and
...
# unrelated command landed. These claim their slots right after /athena,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /athena <command>).
...
# native slot, the alias spelling stays reachable via /athena reset).
...
# ``/athena <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/athena" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /athena credits on Slack.
#   - billing: the terminal-billing surface (buy/auto-reload/limit); /athena billing.
#   - debug: the log/report upload surface; reached via /athena debug on Slack.
_SLACK_VIA_ATHENA_ONLY = frozenset({"credits", "billing", "debug"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/athena <verb>`` subcommand.
    ...
    can still reach them via ``/athena <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/athena`` is always reserved as the first entry so the
    legacy ``/athena <subcommand>`` form keeps working for anything that
    ...
    # Reserve /athena as the catch-all top-level command.
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_ATHENA_ONLY:
            # Intentionally Slack-via-/athena only (see _SLACK_VIA_ATHENA_ONLY).
            return
    ...
    # everything except /athena, so a new canonical command can never silently
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /athena handler.

    Maps both canonical names and aliases so /athena bg do stuff works
    the same as /athena background do stuff.

    Plugin-registered slash commands are included so ``/athena <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from athena_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
