...
2. Python deps declared by user-installed plugins under ``~/.athena/plugins``
...
def _discover_plugins(athena_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.athena/plugins``.
...
def run_audit(
    *,
    ...
    home = athena_home or Path(get_athena_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, athena_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(athena_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, athena_home=home
    )
    if total == 0:
