...
def _discover_plugins(athena_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.athena/plugins``. Plugins typically don't install
...
def _discover_components(
    *, skip_venv: bool = False, skip_plugins: bool = False, skip_mcp: bool = False, athena_home: Optional[Path] = None
) -> list[Component]:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    ...
    components = _discover_components(athena_home=home, **skips)
    ...
    try:
        findings = run_audit(athena_home=home, components=components)
