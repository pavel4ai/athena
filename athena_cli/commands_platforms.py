...
# Logger name parity with the origin module (tests capture "athena_cli.commands").
logger = logging.getLogger("athena_cli.commands")
...
def discord_skill_commands_by_category(
    ...
    parity, external-dir skills are visible via ``athena skills list`` and the agent's ``/skill-name``
...
# Canonical commands deliberately routed through ``/athena <command>`` on Slack only: the registry
...
_SLACK_VIA_ATHENA_ONLY = frozenset({
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    _SLACK_VIA_ATHENA_ONLY are skipped. ``/athena`` is always first for anything dropped."""
...
def slack_app_manifest(
    request_url: str = "https://athena-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """name/alias -> "/command" for the Slack ``/athena`` handler, plugin commands included."""
