"""``athena plugins validate`` — admission checks for a plugin directory.
...
scratch child process (with a throwaway ``ATHENA_HOME``), so a crashing or
...
_PROBE_TIMEOUT = 30
_PROBE_SENTINEL = "ATHENA_VALIDATE_JSON:"
...
def _requires_athena_spec_valid(spec: str) -> bool:
    """Strictly validate a ``requires_athena`` spec.

    Unlike :func:`athena_cli.plugins._version_satisfies` (permissive at load
    ...
    """
    from athena_cli.plugins import _VERSION_COMPARATOR_RE, _version_tuple
...
def _check_requires_athena(report: ValidationReport, manifest: dict) -> None:
    spec = str(manifest.get("requires_athena") or "").strip()
    if not spec:
        report.add("requires_athena", True, "not declared")
        return
    if _requires_athena_spec_valid(spec):
        report.add("requires_athena", True, f"spec {spec!r} parses")
    else:
        report.add(
            "requires_athena",
            False,
            f"requires_athena spec {spec!r} does not parse "
...
# from athena so a hostile plugin only sees a bare interpreter.
_PROBE_SCRIPT = r"""
...
def _run_capability_probe(plugin_dir: Path) -> Tuple[Optional[dict], str]:
    ...
    *error* is a human-readable failure description otherwise.
    """
    with tempfile.TemporaryDirectory(prefix="athena-validate-") as scratch:
        env = dict(os.environ)
        env["ATHENA_HOME"] = scratch
        try:
...
def validate_plugin_dir(plugin_dir: Path) -> ValidationReport:
    ...
    _check_requires_athena(report, manifest)
