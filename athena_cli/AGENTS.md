# athena_cli/ + cli.py — CLI, slash commands, config, skins, updater, profiles

Applies on top of the root `AGENTS.md`. Long-form: `website/docs/developer-guide/cli-internals.md`.

## CLI architecture

`cli.py` holds `AthenaCLI` (REPL loop, config, slash dispatch); behaviour lives in mixins
`athena_cli/cli_commands_mixin.py`, `cli_stream_mixin.py`, `cli_status_bar_mixin.py`,
...
`AthenaCLI._SLASH_DISPATCH` (`canonical -> (method name, pass_arg)`), falling back to a
`_handle_<name>_command` method by naming convention. **There is no `elif` ladder — do not add one.**
Skill slash commands (`agent/skill_commands.py`) scan `~/.athena/skills/` and inject as a **user
message**, never into the system prompt (prompt caching).

Rules: all interactive menu-pickers use curses (`athena_cli/curses_ui.py`; example
`athena_cli/tools_config.py`). Never emit `\033[K` (ANSI erase-to-EOL) in spinner/display code —
...
`athena_cli/goal_command.py::dispatch_goal_command` owns `/goal` parsing and manager
...
## Config system (`athena_cli/config.py`)
...
- **Three loaders — know which you're in:** `load_cli_config()` (CLI, `cli.py`); `load_config()`
  (`athena tools/setup`, most subcommands, `athena_cli/config.py`, merges `DEFAULT_CONFIG`); raw
...
## Skin engine (`athena_cli/skin_engine.py`)
...
`load_skin(name)` (user `~/.athena/skins/*.yaml` → built-ins → default; missing values inherit
...
## Update pipeline (`athena update`) — transactional; every stage guards a real field failure
...
- **Plan** (`update_inventory.py`, `athena update --plan`): read-only inventory — install kind, all
...
- **Restart-per-kind**: systemd and launchd restarts are FLEET-WIDE (every `athena-gateway*` unit /
  `ai.athena.gateway*` LaunchAgent), drain-first (SIGUSR1), with per-unit/per-label failure
...
- **Report**: every run writes a machine-readable receipt to `~/.athena/logs/update_receipts/`
...
`_apply_profile_override()` in `athena_cli/main.py` sets `ATHENA_HOME` before any module import, so
every `get_athena_home()` scopes to the active profile (rules in root). Profiles are independent
...
## Nous free tier (`athena_cli/anon_auth.py`)
...
which never contains a raw exception, a URL or a `athena` verb) and `.copy_terminal`, so no caller
...
contextvars. `upgrade_guest` (`athena auth upgrade`), the CLI `/login` handler and the desktop
