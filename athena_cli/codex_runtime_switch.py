...
_ARG_SYNONYMS = {
    ...
    "off": "auto", "default": "auto", "disable": "auto", "athena": "auto"}

_ATHENA_TOOLS_CALLBACK_NOTE = (
...
def _migration_lines(config: dict) -> list[str]:
    ...
    lines: list[str] = []
    try:
        from athena_cli.codex_runtime_plugin_migration import migrate
        mig_report = migrate(config)
        # The athena-tools callback is internal plumbing — surfaced separately below.
        user_servers = [s for s in mig_report.migrated if s != "athena-tools"]
        if user_servers:
        ...
        if "athena-tools" in mig_report.migrated:
            lines.extend(_ATHENA_TOOLS_CALLBACK_NOTE)
...
def apply(
    ...
    # correct but the world state (managed block in ~/.codex/config.toml, athena-tools MCP
    ...
    if new_value == "codex_app_server":
        ...
        # Migrate Athena' MCP servers + Codex's curated plugins into ~/.codex/config.toml so the
        # spawned codex subprocess sees the same tool surface AND can call back into Athena.
