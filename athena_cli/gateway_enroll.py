    ...
    try:
        ...
        if exc.code == 401:
            message = (
                ...
                "token could not be verified — try `athena auth add nous` and retry."
            )
        elif exc.code == 403:
...
def cmd_gateway_enroll(args) -> None:
    ...
    # Persist idempotently; save_env_value writes the sensitive values to ~/.athena/.env and never
...
def _warn_if_secondary_multiplex_profile() -> bool:
    ...
    try:
        ...
        default_root = Path(get_default_athena_root()).resolve()
