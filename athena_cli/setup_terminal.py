...
setup.py names are resolved through the module object so test patches on ``athena_cli.setup.<name>``
...
def _pip_install_vercel(package):
    """uv when Athena has one ($ATHENA_HOME/bin is never on PATH, so which() misses it and
...
def _setup_backend_local(config: dict) -> None:
    ...
    # Gateway cwd defaults to home; sudo stays off. Both configurable via `athena setup terminal`.
...
def _setup_backend_docker(config: dict) -> None:
    ...
    if _setup.prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
        ...
        _setup.print_info(
            "Run `athena egress setup` then `athena egress start` to mint tokens and launch the proxy.")
    else:
        _setup.print_info("Skipping egress firewall. You can enable it later with `athena egress setup`.")
...
def _setup_backend_vercel(config: dict) -> None:
    ...
    _setup._info("Cloud microVM sandboxes with snapshot-backed filesystem persistence.",
                 "Requires the optional SDK: pip install 'athena-agent[vercel]'")
    _ensure_sdk("vercel", "pip install 'athena-agent[vercel]'", show_stderr=True, install=_pip_install_vercel)
...
def setup_terminal_backend(config: dict):
    ...
    _setup._info("Choose where Athena runs shell commands and code.",
    ...
    # Plugin-registered backends (~/.athena/plugins/). Fail-soft: a broken plugin must not take
    # the wizard down.
