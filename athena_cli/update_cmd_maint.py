"""Post-update maintenance for ``athena update``: pre-update backup snapshot, state-db verify/restore, curator/FTS notices, FHS path guard, completion summary, stale-module purge.

Split out of ``update_cmd.py``, which re-imports every name so ``athena_cli.update_cmd.<name>``
...
_UPDATE_RUNTIME_RELOAD_MODULES = "athena_constants", "tools.environments.local", "tools.lazy_deps"
...
_STALE_PURGE_PREFIXES = "athena_cli", "gateway", "tools", "tui_gateway", "agent"
...
_STALE_PURGE_PROTECTED = frozenset({"athena_cli", "athena_cli.main", "athena_cli.athena_logging"})
...
_STALE_PURGE_PROTECTED_PREFIX = "athena_cli.update_"
...
def _purge_stale_athena_modules() -> None:
    """Evict every cached Athena module after the checkout changed in-place. Never raises.
    ...
    with _best_effort('Could not purge stale Athena modules: %s'):
        importlib.invalidate_caches()
        ...
        if purged:
            logger.debug("Purged %d stale Athena module(s) after checkout update", len(purged))
...
def _print_curator_first_run_notice() -> None:
    ...
    print("  Docs:         https://athena-agent.nousresearch.com/docs/user-guide/features/curator")
...
def _print_fts_optimize_available_notice() -> None:
    ...
    if interrupted:
        ...
        print(
            "  A previous `athena sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    athena sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  athena sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    if "\n" in summary:
        ...
        for line in summary.splitlines():
            print(f"  {line}")
        print("  (This message shows once per curator run. View anytime: athena curator status)")
...
def _reload_process_scan_modules() -> None:
    ...
    ``_scan_dashboard_processes`` does a function-level ``from athena_cli._subprocess_compat import
    ...
    """
    _reload_modules(
        ("athena_cli._subprocess_compat", "athena_cli.dashboard_procs"),
        modules=sys.modules,
...
def _finish_dashboard_update_cleanup(
    ...
    *already_restarted_units*: systemd unit names (no ``.service``) the fleet-restart loop
    ...
    See #83595.
    """
    from athena_cli.update_cmd import _m, _reload_process_scan_modules
    if node_failures:
        print()
...
def _print_update_completion(message: str) -> None:
    ...
    See #47359, #58764.
    """
    from athena_cli.update_cmd import _branch_head_suffix
    print(f"{message}{_branch_head_suffix()}")
    action_id = os.environ.get("ATHENA_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== athena-update completed {action_id} ===")
...
def _post_update_sqlite_runtime_status():
    ...
    from athena_cli.update_cmd import _m
    from athena_constants import project_venv_dir
    from athena_cli.sqlite_runtime import probe_sqlite_runtime
...
def _print_verified_update_completion(message: str) -> bool:
    """Print a success completion only after probing the next Athena runtime."""
    from athena_cli.update_cmd import _post_update_sqlite_runtime_status
    ...
    print("  Rebuild the Athena venv with a uv-managed Python, restart Athena, then verify with `athena doctor`.")
    return False
...
def _print_update_summary(*, node_failures: list, desktop_build_ok: bool, pre_update_version: str | None) -> bool:
    ...
    See #88251.
    """
    from athena_cli.update_cmd import _post_update_sqlite_runtime_status, _update_complete_message
    sqlite_runtime_ok, sqlite_info = _post_update_sqlite_runtime_status()
    if sqlite_info is None:
    ...
    if node_failures or not desktop_build_ok or not sqlite_runtime_ok:
        ...
        if not desktop_build_ok:
            print("  Run `athena desktop` to retry the desktop rebuild.")
        if not sqlite_runtime_ok:
            print(
                "  The Python runtime remediation did not complete. Run `athena "
                "update` again; if SQLite is unchanged, rebuild the Athena venv "
                "with a uv-managed Python, restart Athena, then verify with "
                "`athena doctor`."
            )
    else:
...
def _restore_state_db_from_snapshot(state_path: Path, snap_state: Path) -> bool:
    ...
    """
    from athena_cli.backup import _foreign_db_holder_pids, verify_sqlite_integrity
    ...
    if holders:
        print(
            ...
            "state.db or its WAL open. Stop them (athena gateway stop), "
    ...
    try:
        ...
        print(
            ...
            "handles (or restart Athena) and retry."
        )
        return False
...
def _verify_and_restore_one_state_db(home: Path, *, label: str) -> None:
    ...
    try:
        from athena_cli.backup import _quick_snapshot_root, verify_sqlite_integrity
...
def _verify_and_restore_state_dbs_post_update() -> None:
    ...
    with _best_effort('Sibling-profile state.db guard sweep failed: %s'):
        from athena_cli.backup import _sibling_profile_homes
...
def _print_bundled_skills_sync_report() -> None:
    ...
    if result.get("user_modified"):
        ...
        print("    → see them: athena skills list-modified  (diff/reset to resume updates)")
...
def _ensure_fhs_path_guard() -> None:
    ...
    no-op on non-Linux/non-root/non-FHS or when ``bash -i -c 'command -v athena'`` resolves."""
    from athena_cli.update_cmd import _m
    ...
    try:
        ...
        return
    # Only for FHS-layout installs (link at /usr/local/bin/athena).
    ...
    try:
        probe = subprocess.run(
            ...
            # recovery path in `athena gateway restart` (`systemd_restart()`) as of PR #20949.
            capture_output=True,
...
def _ensure_acp_launcher() -> None:
    r"""Self-heal a ``athena-acp`` launcher next to ``athena`` (mirrors install.sh): ACP hosts
    resolve it on the login-shell PATH but the console script lives in the venv. The shim
    delegates to the sibling ``athena acp``, correct for every layout.

    No-op on Windows (install.ps1 stages launchers into ``$AthenaHome\bin``, never
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        athena_cmd = bin_dir / "athena"
        acp_cmd = bin_dir / "athena-acp"
        try:
            if not (athena_cmd.is_file() or athena_cmd.is_symlink()):
                continue
...
def _verify_state_db_after_snapshot(snapshot_id: str) -> None:
    ...
    _src_path = get_athena_home() / "state.db"
    ...
    _snap_state = _quick_snapshot_root(get_athena_home()) / snapshot_id / "state.db"
...
def _run_quick_snapshots() -> Optional[str]:
    ...
    from athena_cli.update_cmd import _record_update_step
    ...
    with _best_effort('Sibling profile snapshots failed: %s'):
        from athena_cli.backup import create_pre_update_snapshots_all_profiles
        ...
        if _sibling_snaps:
            ...
            import athena_cli.update_cmd_config as _cfg
...
def _run_full_backup() -> None:
    """Zip ATHENA_HOME under ``backups/`` (restorable via ``athena import``). Never raises."""
    try:
    ...
    # display_athena_home so the user sees ~/.athena/...
    try:
        from athena_constants import get_athena_home, display_athena_home
        display_path = f"{display_athena_home()}/{out_path.relative_to(get_athena_home())}"
    except Exception:
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    ``full`` — quick snapshot PLUS a zip of ATHENA_HOME under ``backups/`` (``athena import``).
...
def _sweep_bytecode_after_update(branch: str) -> None:
    ...
    # symbols in athena_constants or lazy_deps. The dependency install above may also have regenerated
...
def _sync_profiles_after_update() -> None:
    ...
    # ATHENA_HOME, so sync_skills()'s module-level ATHENA_HOME cache can't skew it.
    with suppress(Exception):
        from athena_cli.profiles import list_profiles, seed_profile_skills
...
def _print_plugin_compat_notice() -> None:
    ...
    from athena_cli.plugin_compat import compat_report, removal_in_effect, summary_lines
...
def _print_post_update_notices_and_self_heals() -> None:
    ...
    from athena_cli.update_cmd import _m, _print_curator_first_run_notice, _print_curator_recent_run_notice
    ...
    def _migrate_windows_bin_path() -> None:
        ...
        from athena_cli._install_repair import migrate_windows_bin_path
    ...
    for message, step in (
        ...
        ('athena-acp launcher self-heal failed: %s', _ensure_acp_launcher),
...
def _run_post_update_maintenance(
    *, assume_yes, gateway_mode, pre_update_snapshot_id, had_desktop_app_before_update, node_failures, desktop_build_ok,
    ...
    from athena_cli.update_cmd import _check_and_apply_config_migration, _m
    ...
    if sys.platform == "darwin" and had_desktop_app_before_update:
        print()
        print(
            "  ℹ macOS: if Athena re-prompts for permissions you already "
            "granted (toggle shows ON), the stored grant is stale — run "
            "`tccutil reset ScreenCapture com.nousresearch.athena` (repeat "
    ...
    try:
        # See #95596.
        from athena_cli.macos_tcc_anchor import ensure_tcc_anchor
        ensure_tcc_anchor()
    except Exception:
