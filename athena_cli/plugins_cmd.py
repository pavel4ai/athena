...
Plugins are installed from Git repositories into ``~/.athena/plugins/``.
...
def _missing_requires_env_names(manifest: dict) -> list[str]:
    """Return declared ``requires_env`` names that are unset in ``~/.athena/.env``."""
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from athena_cli.config import get_env_value, save_env_value  # noqa: F811
...
def _install_plugin_core(identifier: str, *, force: bool) -> tuple[Path, dict, str]:
    """Clone Git plugin into ``~/.athena/plugins``.
...
def _run_composite_ui(curses, plugin_names, plugin_labels, plugin_selected,
    ...
    from athena_cli.curses_ui import flush_stdin
