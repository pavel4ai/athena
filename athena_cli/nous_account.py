...
FREE_TIER_NEEDS_ACCOUNT = "This needs a Nous account. Run `athena auth upgrade`."
...
def format_nous_portal_entitlement_message(
    ...
    if account_info is None:
        return (
            f"Athena could not verify your Nous Portal entitlement, so {capability} is unavailable. "
            f"Run `athena model` to refresh your login, or check billing at {billing_url}."
        )
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Nous inference credentials are configured, but Athena cannot verify your Nous Portal "
                f"paid access for {capability}. Log in with `athena model` to enable Portal-managed "
    ...
    if account_info.paid_service_access is None:
        detail = f"Athena could not verify your Nous Portal paid access, so {capability} is unavailable."
    ...
    if reason == "account_missing":
        return (
            f"Athena could not find a Nous Portal account or organisation for this login, so {capability} "
            f"is unavailable. Run `athena model` to authenticate again; if the problem persists, contact Nous support."
        )
...
def _no_paid_access_message(
    ...
    if access.member_spend_cap_exceeded:
        ...
        return (
            ...
            f"member spend cap at {billing_url}, then run `athena model` to refresh."
        )
    if active and paid:
        return (
...
def nous_policy_present() -> Optional[bool]:
    ...
    """
    try:
        from athena_cli.auth import get_provider_auth_state, _decode_jwt_claims
...
def _fresh_account_info(state: dict[str, Any], force_fresh: bool, portal_base_url: Optional[str]) -> NousPortalAccountInfo:
    ...
    try:
        from athena_cli.auth import get_provider_auth_state, resolve_nous_access_token
...
def _info_from_valid_jwt(
    ...
    try:
        from athena_cli.auth import _decode_jwt_claims
    except Exception:
        return None
