...
_DOCS_BASE = "https://github.com/pavel4ai/athena/docs"
...
# display_athena_home imported lazily at call sites (stale-module safety during athena update)
...
from athena_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from athena_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from athena_cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, athena_home):
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from athena_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    # Spotify (OAuth via athena auth spotify — check auth.json, not env vars)
    try:
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {athena_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('athena config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('athena config edit', Colors.GREEN)}    Open config in your editor"
    )
    ...
    print(f"   {color('athena gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('athena doctor', Colors.GREEN)}       Check for issues")
    print()
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    """
    try:
        from athena_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.athena/.env."
            )
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into ATHENA_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("ATHENA_MAX_ITERATIONS")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # ATHENA_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        from athena_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_athena_home(Path(get_athena_home()))
    except Exception:
        pass
...
def _profile_name_from_athena_home(athena_home) -> str | None:
    """Return the active profile name when ATHENA_HOME is a profile dir."""
    if athena_home.parent.name == "profiles":
        return athena_home.name
    return None
...
def _setup_qqbot():
    ...
    from athena_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://github.com/pavel4ai/athena/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://github.com/pavel4ai/athena/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  athena config edit")
    print_info("   Open config in your editor:  athena config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from athena_cli.gateway import _all_platforms, _platform_status, _configure_platform
    ...
    if any_messaging:
        ...
        if supports_systemd and has_legacy_athena_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  athena gateway")
        else:
            ...
            if is_container():
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   athena gateway              # Run in foreground")
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``athena_cli.auth`` — lists every supported
    ...
    """
    try:
        from athena_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from athena_cli.config
    ...
    if section_key == "model":
        ...
        from athena_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_athena.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_athena migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_athena", _OPENCLAW_SCRIPT
    )
...
def _offer_openclaw_migration(athena_home: Path) -> bool:
    ...
    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: athena claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=athena_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: athena claw migrate"
        )
        print_info(
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Athena (use athena claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, athena_home)
            return
        ...
        migration_ran = _offer_openclaw_migration(athena_home)
        if migration_ran:
            config = load_config()
    ...
    print_info(f"Data folder:  {athena_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'athena config edit'")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``athena tools`` (which
    ...
    try:
        ...
        from athena_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``athena setup agent`` /
    ``athena config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, athena_home, is_existing: bool):
    ...
    if path == 0:
        ...
        print_info("  Enable tools:        athena tools")
        print_info("  Seed skills:         athena skills opt-in --sync")
        print_info("  Add MCP servers:     athena mcp add")
        print_info("  Enable plugins:      athena plugins")
        print_info("  Tune agent settings: athena setup agent")
        print()
        _print_setup_summary(config, athena_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, athena_home)


def _blank_slate_walkthrough(config: dict, athena_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`athena update` runs from re-injecting them. Opt back in any")
            print_info("time with `athena skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `athena tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `athena plugins list` / `athena plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `athena plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `athena mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `athena mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   athena tools")
    print_info("  Seed skills:         athena skills opt-in --sync")
    print_info("  Add MCP servers:     athena mcp add")
    print_info("  Tune agent settings: athena setup agent")
    print()
...
def _run_quick_setup(config: dict, athena_home):
