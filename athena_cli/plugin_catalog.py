"""Plugin catalog — curated, Nous-approved Athena plugins shipped with the repo.
...
an installed Athena sees new entries and removals without updating. Any fetch failure falls back to the
...
LIVE_CATALOG_URL = "https://athena-agent.nousresearch.com/docs/api/plugin-catalog.json"
...
class PluginCatalogEntry:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "maintainer": self.maintainer, "tier": self.tier, "requires_athena": self.requires_athena,
...
def entry_from_mapping(data: Any, label: str) -> Optional[PluginCatalogEntry]:
    ...
    return PluginCatalogEntry(
        ...
        requires_athena=str(data.get("requires_athena") or "").strip(),
...
def _live_cache_path() -> Path:
    ...
    return get_athena_home() / "cache" / "plugin-catalog.json"
...
def fetch_live_catalog(*, force: bool = False) -> Optional[Dict[str, Any]]:
    ...
    ``ATHENA_HOME/cache`` for :data:`LIVE_CATALOG_TTL_SECONDS`. ``None`` on ANY failure — callers fall
...
def entry_capability_summary(entry: PluginCatalogEntry) -> str:
    ...
    if entry.requires_athena:
        bits.append(f"Requires Athena {entry.requires_athena}.")
    return " ".join(bits)
