...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m athena_cli.main gateway run --replace`` invocation) and
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "athena_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("ATHENA_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("ATHENA_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("ATHENA_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["ATHENA_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["ATHENA_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["ATHENA_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("ATHENA_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("ATHENA_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("ATHENA_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    """
    _assert_windows()
    # Local import to avoid circular module initialization during athena_cli boot.
    from athena_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\athena\\gateway-service\\<task_name>.cmd``
    (or ``<ATHENA_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Athena installs stay self-contained).
    """
    _assert_windows()
    from athena_cli.config import get_athena_home

    script_dir = Path(get_athena_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``ATHENA_HOME`` whenever it
    ...
    source checkout only if ``ATHENA_HOME`` cannot be resolved yet.
    """
    from athena_cli.config import get_athena_home
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports ATHENA_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m athena_cli.main [--profile X] gateway run``
    ...
    lines.append(f'set "ATHENA_HOME={athena_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "ATHENA_GATEWAY_DETACHED=1"')
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports athena_constants-based logic during startup.
    venv_dir = str(Path(python_path).resolve().parent.parent)
    lines.append(f'set "VIRTUAL_ENV={venv_dir}"')

    pythonw_path = _derive_venv_pythonw(python_path)
    prog_args = [pythonw_path, "-m", "athena_cli.main"]
    if profile_arg:
...
def _write_task_script() -> Path:
    ...
    athena_home = str(Path(get_athena_home()).resolve())
    profile_arg = _profile_arg(athena_home)

    content = _build_gateway_cmd_script(python_path, working_dir, athena_home, profile_arg)
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m athena_cli.main gateway run``
    ...
    log_dir = Path(get_athena_home()) / "logs"
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("ATHENA_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("ATHENA_GATEWAY_INSTALL_START_ON_LOGIN")
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    from athena_cli.gateway import find_gateway_pids, _profile_arg
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from athena_cli.config import get_athena_home
        print(f"    type {Path(get_athena_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_athena_home()).resolve()}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from athena_cli.config import get_athena_home

    athena_home = Path(get_athena_home()).resolve()
    print()
    print("Next steps:")
    print("  athena gateway status                      # Check status")
    print(f"  type {athena_home}\\logs\\gateway.log       # View logs")
...
def _print_deep_probes() -> None:
    ...
    home = Path(get_athena_home()).resolve()
...
def stop() -> None:
    ...
    """
    _assert_windows()
    from athena_cli.gateway import kill_gateway_processes, _get_restart_drain_timeout
    from gateway.status import get_running_pid
...
def restart() -> None:
    ...
    """
    _assert_windows()
    from athena_cli.gateway import kill_gateway_processes
