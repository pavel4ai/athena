...
_GATEWAY_ENV = (("PYTHONIOENCODING", "utf-8"), ("ATHENA_GATEWAY_DETACHED", "1"), ("ATHENA_SUPERVISED_CHILD", "1"))
...
def _preserve_athena_home_path(path: str | Path) -> str:
    r"""Render Athena-owned paths under the configured ATHENA_HOME spelling.

    ``%LOCALAPPDATA%\athena`` may be a symlink/junction to another drive; launcher files must not
    bake in the resolved target for paths under ATHENA_HOME.
    """
    candidate = Path(path)
    try:
        home = _athena_home()
...
def _current_profile_cli_args() -> list[str]:
    ...
    from athena_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "athena_cli.main", *_current_profile_cli_args(), "gateway", command, *(extra_args or [])]
...
def _launch_elevated_install(force: bool = False, *, start_now: bool | None = None, start_on_login: bool | None = None) -> bool:
    ...
    overrides = {"ATHENA_GATEWAY_ELEVATED_HANDOFF": "1"}
    extra_args = ["--elevated-handoff"]
    if force:
        extra_args.append("--force")
    for choice, env_key, flag in (
        (start_now, "ATHENA_GATEWAY_INSTALL_START_NOW", "start-now"),
        (start_on_login, "ATHENA_GATEWAY_INSTALL_START_ON_LOGIN", "start-on-login"),
    ):
...
def get_task_name() -> str:
    ...
    _assert_windows()
    from athena_cli.gateway import _profile_suffix  # local: avoids circular init during boot
...
def get_task_script_path() -> Path:
    """The generated ``gateway.cmd`` wrapper under ``<ATHENA_HOME>/gateway-service/`` (per-profile
    installs stay self-contained); the VBS launcher lives beside it."""
    _assert_windows()
    script_dir = _athena_home() / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    """Stable cwd for detached/startup runs: anchor at ATHENA_HOME when it exists (mirrors the POSIX
...
def _gateway_run_argv(python_exe: str, profile_arg: str) -> list[str]:
    """``python -m athena_cli.main [--profile X] gateway run`` — shared by every launcher renderer."""
    argv = [python_exe, "-m", "athena_cli.main"]
    if profile_arg:
...
def _launcher_settings() -> tuple[str, str, str, str]:
    """Return (python_path, working_dir, athena_home, profile_arg) for generated launchers."""
    from athena_cli.gateway import PROJECT_ROOT, _profile_arg, get_python_path  # avoid circular init

    athena_home = str(_athena_home())
    return (
        _preserve_athena_home_path(get_python_path()),
        _stable_gateway_working_dir(PROJECT_ROOT),
        athena_home,
        _profile_arg(athena_home),
    )


def _launcher_pythonpath_entries(extra_pythonpath: list[str]) -> list[str]:
    return [
        _preserve_athena_home_path(Path(__file__).resolve().parent.parent),
        *[_preserve_athena_home_path(entry) for entry in extra_pythonpath],
    ]


def _build_gateway_cmd_script(python_path: str, working_dir: str, athena_home: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        f'set "ATHENA_HOME={athena_home}"',
        *[f'set "{k}={v}"' for k, v in _GATEWAY_ENV],
        # VIRTUAL_ENV lets the gateway's own python detection find the venv.
        f'set "VIRTUAL_ENV={_preserve_athena_home_path(venv_dir)}"',
...
def _build_gateway_vbs_script(python_path: str, working_dir: str, athena_home: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        f"env.Item({q('ATHENA_HOME')}) = {q(athena_home)}",
        *[f"env.Item({q(k)}) = {q(v)}" for k, v in _GATEWAY_ENV],
        f"env.Item({q('VIRTUAL_ENV')}) = {q(_preserve_athena_home_path(venv_dir))}",
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    _assert_windows()
    from athena_cli.gateway import PROJECT_ROOT

    python_path, working_dir, athena_home, profile_arg = _launcher_settings()
    python_exe, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    env_overlay = {"ATHENA_HOME": athena_home, **dict(_GATEWAY_ENV), "VIRTUAL_ENV": _preserve_athena_home_path(venv_dir)}
    _prepend_pythonpath(env_overlay, [_preserve_athena_home_path(p) for p in (PROJECT_ROOT, *extra_pythonpath)])
...
def windowless_gateway_restart_spec(run_argv: list[str]) -> tuple[list[str], str, dict[str, str]]:
    ...
    interpreter via ``_resolve_detached_python`` and supplies the stable cwd + env overlay (ATHENA_HOME,
    ...
    try:
        athena_home = str(_athena_home().resolve())
    except Exception:
        athena_home = ""
    env_overlay: dict[str, str] = {"PYTHONIOENCODING": "utf-8", "ATHENA_GATEWAY_DETACHED": "1", "VIRTUAL_ENV": str(venv_dir)}
    if athena_home:
        env_overlay["ATHENA_HOME"] = athena_home
...
def _prompt_install_choices(start_now: bool | None = None, start_on_login: bool | None = None) -> tuple[bool, bool]:
    ...
    if start_now is None:
        start_now = _install_choice_from_env("ATHENA_GATEWAY_INSTALL_START_NOW")
    if start_on_login is None:
        start_on_login = _install_choice_from_env("ATHENA_GATEWAY_INSTALL_START_ON_LOGIN")
    if start_now is not None and start_on_login is not None:
        return start_now, start_on_login

    from athena_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    if running_pids or start_now:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"athena {profile_arg} gateway start" if profile_arg else "athena gateway start"
...
def install(
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: athena gateway start")
        return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: athena gateway start")
        _print_next_steps()
        return
...
def _start_attestation_path() -> Path:
    return _athena_home().joinpath(*_START_ATTESTATION_RELATIVE)
...
def _attested_pid_exited_cleanly(pid: int) -> bool:
    ...
    try:
        ...
        data = json.loads(get_lifecycle_sentinel_path(_athena_home()).read_text(encoding="utf-8"))
    except Exception:
        return False
...
def check_start_attestation(current_pids: list[int] | None = None) -> str | None:
    ...
    lines = [
        ...
        "shutdown record.",
        "  This usually means the shell that ran `athena gateway start` was inside "
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        print(f"  Check the log for startup errors:\n    type {_athena_home()}\\logs\\gateway.log\n    type {_athena_home()}\\logs\\gateway-stdio.log")
...
def _print_next_steps() -> None:
    ...
    print(f"  type {_athena_home()}\\logs\\gateway.log       # View logs")
...
def restart() -> None:
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `athena gateway status`."
        )
