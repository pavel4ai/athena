...
# display_athena_home is imported lazily at call sites to avoid ImportError
# when athena_constants is cached from a pre-update version during `athena update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "athena CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        # the command doesn't advertise *some other* profile. ATHENA_HOME
        ...
        # ATHENA_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "athena_home=" in command_lc
            and f"athena_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m athena_cli.main gateway
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose ATHENA_HOME or argv doesn't match a known profile).
...
def _sync_athena_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``ATHENA_HOME``.

    Under ``sudo``, ``ATHENA_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_athena_home` falls back to ``/root/.athena`` — the wrong
    profile. The unit file pins ``ATHENA_HOME`` for the actual gateway
    ...
    unit_home = env.get("ATHENA_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("ATHENA_HOME", "").strip()
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}athena gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}athena gateway restart{scope_flag}"
    )
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (ATHENA_HOME-scoped).
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    ``ATHENA_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("ATHENA_GATEWAY_DETACHED", "").strip().lower()
...
def _profile_suffix() -> str:
    """Derive a service-name suffix from the current ATHENA_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (ATHENA_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    default = get_default_athena_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary ATHENA_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(athena_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when ATHENA_HOME is a named profile.

    For ``~/.athena/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        athena_home: Optional explicit ATHENA_HOME path. Defaults to the current
            ``get_athena_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional Athena root to compare against. Used when
            generating a system service for another user from a sudo/root
            process, where ``Path.home()`` and ``get_default_athena_root()``
...
def _profile_arg_for_target_user(athena_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".athena"
    try:
        Path(athena_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(athena_home, default_root=target_root)
    except ValueError:
        return _profile_arg(athena_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this ATHENA_HOME.

    Default ``~/.athena`` returns ``athena-gateway`` (backward compatible).
    Profile ``~/.athena/profiles/coder`` returns ``athena-gateway-coder``.
    Any other ATHENA_HOME appends a short hash for uniqueness.
    """
...
# athena-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (athena-gateway-*.service) and unrelated third-party
...
def _find_legacy_athena_units() -> list[tuple[str, Path, bool]]:
    ...
    different service name (e.g. ``athena.service`` before the rename to
    ``athena-gateway.service``). When both a legacy unit and the current
    ``athena-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``athena-gateway-coder.service`` and unrelated third-party
      ``athena-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``athena.service`` running an unrelated
...
def remove_legacy_athena_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Athena gateway unit files.

    Iterates over whatever ``_find_legacy_athena_units()`` returns — which is
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo athena gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo athena gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo athena gateway start --system")
            return scope, False
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.athena`` → ``ai.athena.gateway.plist`` (backward compatible).
    Profile ``~/.athena/profiles/coder`` → ``ai.athena.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.athena/athena-agent  -> /home/alice/.athena/athena-agent
      /opt/athena                 -> /opt/athena  (kept as-is)
...
def _athena_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current ATHENA_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_athena_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.athena                    → /home/alice/.athena
      /root/.athena/profiles/coder     → /home/alice/.athena/profiles/coder
      /opt/custom-athena               → /opt/custom-athena  (kept as-is)
    """
    current_athena = get_athena_home().resolve()
    current_default = (Path.home() / ".athena").resolve()
    target_default = Path(target_home_dir) / ".athena"

    # Default ~/.athena → remap to target user's default
    if current_athena == current_default:
        return str(target_default)

    # Profile or subdir of ~/.athena → preserve the relative structure
    try:
        relative = current_athena.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.athena) — keep as-is
        return str(current_athena)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    athena_node = athena_home / "node" / "bin"
    if _is_dir(athena_node):
        candidates.append(str(athena_node))
    athena_nm = athena_home / "node_modules" / ".bin"
    if _is_dir(athena_nm):
        candidates.append(str(athena_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m athena_cli.main``, so module
    ...
    ``ATHENA_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if ATHENA_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        profile_arg = _profile_arg_for_target_user(athena_home, home_dir)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's ATHENA_HOME (stable, always exists)
...
User={username}
Group={group_name}
ExecStart={python_path} -m athena_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir ATHENA_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved ATHENA_HOME
    ...
    exported a throwaway ``ATHENA_HOME=/tmp/...`` — writing it to the real
    ...
    with ``ATHENA_HOME=/tmp/athena-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="ATHENA_HOME=..."`` lines and launchd
    ``<key>ATHENA_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile
    ...
    candidates += re.findall(
        r"<key>ATHENA_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    ...
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: ATHENA_HOME resolves to a "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only ATHENA_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp ATHENA_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../athena_test`` path into
    # ``Environment="ATHENA_HOME=..."``. Writing that to the developer's
    ...
    if not system and (
        ...
        or '/athena_test"' in new_unit
        or "/athena_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # ATHENA_HOME into the unit (manual E2E homes like /tmp/athena-e2e-NNN
...
def systemd_install(
    ...
    # Offer to remove legacy units (athena.service from pre-rename installs)
    # before installing the new athena-gateway.service. If both remain, they
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}athena gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}athena gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}athena gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}athena gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}athena gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}athena gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}athena gateway start{scope_flag}"
        )
...
# (the documented `nohup athena gateway run` workaround). See #23387.
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m athena_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active ATHENA_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "athena_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup athena gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup athena gateway run --replace "
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``ATHENA_S6_SUPERVISED_CHILD``.
    """
    if os.environ.get("INVOCATION_ID"):
        return True
    if os.environ.get("ATHENA_S6_SUPERVISED_CHILD"):
        return True
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``athena gateway run`` is always a misconfiguration, so we
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``athena gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``athena gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other ATHENA_HOME roots.
    """
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("ATHENA_ALLOW_ROOT_GATEWAY")):
        return
    ...
    print(
        "  Set ATHENA_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # Service-style launchers set ATHENA_GATEWAY_DETACHED=1; older wrappers
    ...
    # ATHENA_GATEWAY_EXIT_DIAG env var (default: on while we're still
...
def _all_platforms() -> list[dict]:
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run athena under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.athena/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_athena_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: athena pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'athena gateway setup'"
                    )
            continue
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``athena_cli.setup`` (which
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.athena/plugins/
        ...
        if service_running:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    ...
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo athena gateway install --system"
                        )
                    print_info("  Or run in foreground:  athena gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: athena gateway run")
                print_info(
                ...
                print_info("  Run in foreground: athena gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup athena gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`athena_cli.service_manager` are caught and surfaced as
    ...
    if profile is None:
        ...
        # ATHENA_HOME=<root>/profiles/<name>, "" for the default root,
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      2. ``ATHENA_S6_SUPERVISED_CHILD`` is exported by
         ...
         → ...).
      3. ``--no-supervise`` (or ``ATHENA_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    # `docker logs` and ${ATHENA_HOME}/logs/gateways/<profile>/current,
    ...
    #
    # Prefer `sleep infinity` (matches the static main-athena service's
    # pattern in docker/s6-rc.d/main-athena/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: athena gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  athena profile create <name>     # creates the s6 service slot")
                print("  athena -p <name> gateway start   # bring it up via s6")
                print("  athena status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: athena gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: athena gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: athena gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  athena profile delete <name>     # tears down the s6 service slot")
                print("  athena -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: athena gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_ATHENA_GATEWAY") == "1":
            print_error(
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: athena gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  athena gateway install")
                else:
                    print("To install as a service:")
                    print("  athena gateway install")
                    print("  sudo athena gateway install --system")
            else:
                ...
                if is_termux():
                    ...
                    print(
                        "  athena gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  athena gateway install  # Install as user service")
                    print(
        ...
        # pre-rename installs (e.g. athena.service). Profile units and
        # unrelated third-party services are never touched.
