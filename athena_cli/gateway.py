"""Gateway subcommand for athena CLI.

Handles: athena gateway [run|start|stop|restart|status|install|uninstall|setup]
"""

import asyncio
import contextlib
from athena_cli.cli_output import line_input
import json
import logging
import os
import shlex
...
# display_athena_home is imported lazily: athena_constants may be a cached pre-update version.
...
from athena_cli.colors import Colors, color
...
def _get_service_pids(all_profiles: bool = False) -> set:
    ...
    ``all_profiles`` widens the current profile's unit/label to the whole ``athena-gateway*`` /
    ``ai.athena.gateway*`` fleet so update/reaper never kill a sibling's service gateway as "manual".

    ``all_profiles`` widens the launchd branch to every installed ``ai.athena.gateway*`` LaunchAgent — the
    ...
    the ``athena-gateway*`` fleet glob.
    """
    pids: set = set()

    # --- systemd (Linux): user and system scopes ---
    if supports_systemd_services():
        pattern = "athena-gateway*" if all_profiles else get_service_name()
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                # Belt-and-suspenders for the EXCLUDE use case (#74075): a bare ``launchctl list`` prefix
                # scan also catches ai.athena.gateway* agents the label derivation can't map (renamed
    ...
    if is_macos():
        ...
        if all_profiles:
            # Whole fleet, mirroring the systemd ``athena-gateway*`` glob above.
            # Every gateway LaunchAgent, not just the invoking profile's — mirrors the systemd branch's
            # ``athena-gateway*`` pattern above. The update path restarts the whole fleet, and its
        ...
        for label in sorted(labels):
            ...
            if pid is not None and pid > 0:
                pids.add(pid)
        if all_profiles:
            # Prefix scan also catches ai.athena.gateway* agents the label derivation can't map
            ...
            try:
                ...
                if result.returncode == 0:
                    for line in result.stdout.strip().splitlines():
                        parts = line.split()
                        if len(parts) >= 3 and parts[-1].startswith("ai.athena.gateway"):
                            try:
                                pid = int(parts[0])
                                if pid > 0:
                                    pids.add(pid)
...
# its full budget and `athena update` can deadlock. Two witnesses classify the loop BEFORE any
...
# fail", and `athena update` can deadlock behind it. The loop publishes a liveness signal precisely for this
...
def _get_ancestor_pids() -> set[int]:
    """PIDs of this process and its ancestors, so scans never count the invoking ``athena`` CLI as a gateway.

    Walks from the current PID up to PID 1 (init) so that process-table scans never match the calling CLI
    process or any of its parents. This prevents ``athena gateway status`` from falsely counting the
    ``athena`` CLI that invoked it as a running gateway instance (see #13242).
    """
...
def _scan_gateway_pids(
    ...
    # Exclude the entire ancestor chain so the CLI process that invoked this scan (e.g. ``athena gateway
    ...
    current_home = str(get_athena_home().resolve())
    # Forward slashes on both sides of the ATHENA_HOME= match (mirrors gateway.status).
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"athena_home={current_home_lc}" in command_lc
            )

        # Default profile: accept unless argv advertises another profile. ATHENA_HOME may come via
        # env (invisible to wmic/CIM), so only a non-matching explicit ATHENA_HOME= disqualifies.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        return not ("athena_home=" in command_lc and f"athena_home={current_home_lc}" not in command_lc)
...
def _windows_process_listing() -> str | None:
    ...
    # failure or timeout (result is None) trips the fallback. ``athena update`` hung exactly there on
    ...
    from athena_cli._subprocess_compat import bounded_probe_run
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    """Find running gateway PIDs for the current profile, or every profile with ``all_profiles`` (``athena update``)."""
...
def find_windows_gateway_services(
    *, psutil_module=None, profile_processes: list[ProfileGatewayProcess] | None = None
    ...
    command lines, so identity = Athena's own PID file + a parent chain ending at a running SCM service
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "athena_cli.main"]
    if profile != "default":
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``athena update``: ``--external-supervisor`` gateways
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    if old_pid <= 0 or not run_argv:
        return False
    from athena_cli._subprocess_compat import windows_detach_flags_without_breakaway, windows_detach_popen_kwargs
    ...
    # normalizes the interpreter and captures a stable cwd + env overlay (ATHENA_HOME,
    ...
    if sys.platform == "win32":
        try:
            from athena_cli.gateway_windows import windowless_gateway_restart_spec
    ...
    watcher = textwrap.dedent(
        ...
        try:
            ...
            from pathlib import Path
            _log_dir = Path(get_athena_home()) / "logs"
        ...
        # ATHENA_HOME) the windowless base interpreter needs to import athena_cli. Empty on POSIX.
        if _respawn_cwd:
...
def _athena_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """``ATHENA_HOME`` from the on-disk unit file — what refresh/compare already read, and reliable under ``sudo``."""
    ...
    for line in text.splitlines():
        ...
        if body.startswith("Environment="):
            ...
            if body.startswith("ATHENA_HOME="):
                return body.split("=", 1)[1].strip().strip('"') or None
    return None


def _sync_athena_home_from_systemd_unit(system: bool) -> None:
    """Adopt a system-scope unit's ``ATHENA_HOME``: under ``sudo`` it is stripped and HOME=/root, so
    get_athena_home() would pick the wrong profile for runtime-status/PID reads."""
    if not system:
        return
    # On-disk unit first; ``systemctl show`` for units that only exist in the manager.
    unit_home = (_athena_home_from_systemd_unit_file(system=True) or "").strip()
    if not unit_home:
        env_line = _systemctl_show(("Environment",), system=True).get("Environment", "")
        unit_home = _parse_kv_pairs(env_line.split()).get("ATHENA_HOME", "").strip()
    if unit_home and os.environ.get("ATHENA_HOME", "").strip() != unit_home:
        os.environ["ATHENA_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {sudo}athena gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {sudo}athena gateway restart{scope_flag}")
...
def _s6_gateway_snapshot(gateway_pids: tuple[int, ...]) -> GatewayRuntimeSnapshot | None:
    ...
    from athena_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: athena gateway stop")
    else:
...
def _print_other_profiles_gateway_status() -> None:
    """Print other profiles' running gateways at the bottom of ``athena gateway status``."""
    try:
...
def _gateway_list() -> None:
    ...
    try:
        from athena_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    if is_windows():
        try:
            from athena_cli.gateway_windows import get_task_name  # profile-aware task name
            _task_name = get_task_name()
        except Exception:
            _task_name = "Athena_Gateway"
...
def _reaper_exclusion_pids(extra_exclude: set | None) -> set[int]:
    ...
    with contextlib.suppress(Exception):
        ...
        # cover the whole ai.athena.gateway* fleet — not just the current profile's label — or a sibling
...
from athena_constants import is_container, is_termux, is_wsl
...
def _gw_windows():
    """Lazily import :mod:`athena_cli.gateway_windows` (Windows-only service backend)."""
...
def _gateway_detached_env() -> bool:
    return _truthy_env(os.getenv("ATHENA_GATEWAY_DETACHED"))
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    """True for detached Windows gateway runs that should ignore Ctrl+C (``ATHENA_GATEWAY_DETACHED=1``
...
def _windows_gateway_breakaway_state() -> bool | None:
    ...
    if not is_windows():
        return None
    from athena_cli._subprocess_compat import _WINDOWS_GATEWAY_BREAKAWAY_ENV
...
def _profile_suffix() -> str:
    """Service-name suffix for ATHENA_HOME: "" for the platform-native default home (``~/.athena``), the
    profile name for ``<root>/profiles/<name>``, else a short hash of the path.

    The bare name is reserved for the NATIVE default, not ``get_default_athena_root()``: that helper
    treats any ATHENA_HOME outside ``~/.athena`` (Docker ``/opt/data``, a temp dir) as "the root itself",
    which let a temp-home harness resolve to the default profile's ``athena-gateway`` unit and uninstall
    the production gateway. Service names are host-wide identities; only the real default home owns the
    bare one."""
    import hashlib
    from athena_constants import _get_platform_default_athena_home, get_default_athena_root
    home = get_athena_home().resolve()
    if home == _get_platform_default_athena_home().resolve():
        return ""
    name = _profile_name_from_home(home, get_default_athena_root().resolve())
...
def _current_profile_name() -> str:
    ...
    return profile_name_for_home(get_athena_home()) or _profile_suffix()


def _profile_arg(athena_home: str | None = None, default_root: str | Path | None = None) -> str:
    """``--profile <name>`` for ``<root>/profiles/<name>``, else "". *athena_home*/*default_root* let a
    ...
    home = Path(athena_home or str(get_athena_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_athena_root().resolve()
    if home == default:
        return ""
...
def get_service_name() -> str:
    """Systemd service name: ``athena-gateway`` for default ATHENA_HOME, ``athena-gateway-<profile>``
    or ``-<hash>`` otherwise."""
...
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("athena.service",)
...
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "athena_cli.main gateway",
    "athena_cli/main.py gateway",
    "gateway/run.py",
    " athena gateway ",
    "/athena gateway ",
)
...
def _find_legacy_athena_units() -> list[tuple[str, Path, bool]]:
    """``[(unit_name, unit_path, is_system)]`` for legacy gateway units (e.g. ``athena.service``), which
    ...
    Detects unit files installed by older Athena versions that used a different service name (e.g. When both
    a legacy unit and the current ``athena-gateway.service`` are active, they fight over the same bot token
...
def print_legacy_unit_warning() -> None:
    ...
    print_info("  These run alongside the current athena-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    athena gateway migrate-legacy")


def remove_legacy_athena_units(interactive: bool = True, dry_run: bool = False) -> tuple[int, list[Path]]:
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: athena gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo athena gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    athena gateway uninstall")
    print_info("    sudo athena gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: athena gateway install"
            )
            return scope, False
...
def ensure_gateway_service(context: str = "setup") -> bool:
    """Install and start a user-scope gateway service without prompting (``athena setup``/``import``).
    ...
    if is_container():
        ...
        print_info("   athena gateway run          # Run as container main process")
        print_info("")
    ...
    if not (supports_systemd or is_macos() or is_windows()):
        ...
        print_info("  Run the gateway in the foreground with: athena gateway")
        return False
    ...
    try:
        ...
        print_info("  You can retry manually: athena gateway install")
...
def get_launchd_plist_path() -> Path:
    """``~/Library/LaunchAgents/ai.athena.gateway[-<profile>].plist`` under the real account home."""
    import pwd
    suffix = _profile_suffix()
    name = f"ai.athena.gateway-{suffix}" if suffix else "ai.athena.gateway"
...
def launchd_gateway_labels_for_install() -> list[str]:
    ...
    the profile layout, NOT by globbing ``~/Library/LaunchAgents``, so a sandboxed ATHENA_HOME never
    ...
    for profile in list_profiles():
        if profile.is_default:
            root_label.append("ai.athena.gateway")
        elif _re.match(r"^[a-z0-9][a-z0-9_-]{0,63}$", profile.name):
            profile_labels.append(f"ai.athena.gateway-{profile.name}")
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            from athena_constants import venv_python_path
        except ImportError:
            # Update-boundary: a gateway restarted mid-update can hold a stale athena_constants
            # without this symbol; see _reload_athena_constants() in athena_cli/managed_uv.py.
            from athena_cli.managed_uv import _reload_athena_constants
            venv_python_path = _reload_athena_constants().venv_python_path
...
def _athena_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current ATHENA_HOME (root's, under sudo) to the target user's equivalent:
    ``/root/.athena[/profiles/x]`` → ``/home/alice/.athena[/profiles/x]``; custom paths kept as-is."""
    current_athena_raw = os.environ.get("ATHENA_HOME", "").strip()
    current_athena = Path(current_athena_raw).expanduser() if current_athena_raw else get_athena_home()
    # Keep paths lexical: resolving a non-existent path can bake a different ATHENA_HOME into the unit.
    current_default = Path.home() / ".athena"
    target_default = Path(target_home_dir) / ".athena"
    try:
        # Default ~/.athena or a profile/subdir of it → preserve the relative structure under the target.
        return str(target_default / current_athena.relative_to(current_default))
    except ValueError:
        return str(current_athena)  # Completely custom path (not under ~/.athena) — keep as-is
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    extras = (project_root / "node_modules" / ".bin", athena_home / "node" / "bin", athena_home / "node_modules" / ".bin")
    for extra in extras:
        if _is_dir(extra):
...
def _stable_service_working_dir() -> str:
    """WorkingDirectory that won't disappear under systemd (ATHENA_HOME, else PROJECT_ROOT). cwd is
...
def _systemd_watchdog_seconds(athena_home: str | Path | None = None) -> int:
    ...
    if athena_home is not None:
        ...
        override_token = set_athena_home_override(athena_home)
        reset_home_override = reset_athena_home_override
    try:
...
def _append_node_dir_for_service(path_entries: list[str], athena_root: Path | None = None) -> None:
    """Append the Node dir a service unit should use: managed ``<athena_root>/node`` (profile-scoped)
    first — a unit survives reboots, so baking a shell-PATH Node is permanent breakage — else PATH lookup."""
    from athena_constants import (athena_managed_node_tree_present, iter_athena_node_dirs)
    managed_node_present = athena_managed_node_tree_present(athena_root)
    for directory in iter_athena_node_dirs(athena_root) if managed_node_present else ():
        entry = str(directory)
        try:
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        athena_home = _athena_home_for_target_user(home_dir)
        # Profile arg relative to the TARGET user's ~/.athena when athena_home lives under it.
        target_root = Path(home_dir) / ".athena"
        try:
            Path(athena_home).resolve().relative_to(target_root.resolve())
            profile_arg = _profile_arg(athena_home, default_root=target_root)
        except ValueError:
            profile_arg = _profile_arg(athena_home)
        # Remap paths under the calling user's home (/root/) to the target user's so the service can read them.
        python_path = _remap_path_for_user(python_path, home_dir)
        working_dir = str(athena_home) if athena_home else _remap_path_for_user(working_dir, home_dir)
        ...
        _append_node_dir_for_service(_target_node_entries, Path(athena_home) if athena_home else None)
        ...
        athena_home = str(get_athena_home().resolve())
        profile_arg = _profile_arg(athena_home)
        user_home = Path.home()
        identity_lines = env_lines = ordering_lines = ""
        wanted_by = "default.target"

    watchdog_seconds = _systemd_watchdog_seconds(athena_home)
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}{identity_lines}ExecStart={python_path} -m athena_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
{env_lines}Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="ATHENA_HOME={athena_home}"
Environment="ATHENA_SUPERVISED_CHILD=1"
Restart=always
RestartSec=5
...
def _normalize_launchd_plist_for_comparison(text: str) -> str:
    ...
    import re
    return re.sub(
        r"(<key>PATH</key>\s*<string>)(.*?)(</string>)", r"\1__ATHENA_PATH__\3",
        _normalize_service_definition(text), flags=re.S,
    )


def systemd_unit_is_current(system: bool = False) -> bool:
    # ATHENA_HOME sync chokepoint for every compare/regenerate path: under `sudo … --system` it is often
    # stripped to /root/.athena, so refresh would rewrite a correct unit and status warn forever.
    # Idempotent; the os.environ mutation persists for later runtime reads (restart's PID/drain).
    _sync_athena_home_from_systemd_unit(system=system)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Temp-dir ATHENA_HOME baked into a systemd unit / launchd plist, or None. A temp home means a
    test/E2E harness generated it; installing it leaves the gateway "running" but deaf to every platform."""
    import re
    import tempfile
    candidates = re.findall(r'ATHENA_HOME=([^"\n]+)', definition)
    candidates += re.findall(r"<key>ATHENA_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S)
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp ATHENA_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(f"✗ Refusing to write the gateway {kind}: ATHENA_HOME resolves to a temporary directory ({temp_home}).")
    print(
        "  This usually means a test/E2E environment exported ATHENA_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # systemd_unit_is_current is the ATHENA_HOME-sync chokepoint; its env mutation persists for the regenerate below.
    ...
    # pytest-tmp ATHENA_HOME baked into the developer's real unit breaks their gateway on next reboot.
    if not system and any(m in new_unit for m in ("/pytest-of-", '/athena_test"', "/athena_test/")):
        return False

    # Structural variant: refuse ANY temp-dir ATHENA_HOME (manual E2E homes lack the pytest markers).
    if _refuse_temp_home_service_write(new_unit, "systemd unit"):
        return False

    unit_path.write_text(new_unit, encoding="utf-8")
    _run_systemctl(["daemon-reload"], system=system, check=True, timeout=30)
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Athena install")
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo athena gateway uninstall --system")
    print_info("         athena gateway install")
    print_info("         athena gateway start")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = os.getenv("ATHENA_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def _get_cron_drain_timeout() -> float:
    ...
    See #82161.
    """
    return _agent_timeout_setting("ATHENA_CRON_DRAIN_TIMEOUT", "cron_drain_timeout", parse_cron_drain_timeout)
...
def _get_restart_exit_wait_budget() -> float:
    ...
    return resolve_restart_exit_wait_budget(
        ...
        _agent_timeout_setting(
            "ATHENA_RESTART_AFTER_TURN_TIMEOUT", "restart_after_turn_timeout", parse_restart_after_turn_timeout
        ),
    )
...
def systemd_install(
    ...
    # Existing system units already pin ATHENA_HOME; adopt it before any regenerate.
    if unit_path.exists():
        _sync_athena_home_from_systemd_unit(system=system)
    ...
    print()
    print("Next steps:")
    print(f"  {sudo}athena gateway start{scope_flag}              # Start the service")
    print(f"  {sudo}athena gateway status{scope_flag}             # Check status")
...
def _systemd_unit_belongs_to_current_home(system: bool = False) -> bool:
    """False (with a warning) when the installed unit pins a ATHENA_HOME other than this process's: the
    service name then resolved to ANOTHER install's gateway, and stop/disable/unlink would take it down."""
    _sync_athena_home_from_systemd_unit(system=system)  # sudo strips ATHENA_HOME; adopt the unit's first
    unit_home = _athena_home_from_systemd_unit_file(system=system)
    if unit_home is None or Path(unit_home).expanduser().resolve() == get_athena_home().resolve():
        return True
    print_warning(
        f"Refusing to remove {get_systemd_unit_path(system=system)}: it runs ATHENA_HOME={unit_home}, "
        f"but this process has ATHENA_HOME={get_athena_home()}"
    )
    return False
...
def _print_service_not_installed(system: bool) -> None:
    ...
    print(f"  Run: {sudo}athena gateway install{scope_flag}")
...
def systemd_start(system: bool = False):
    ...
    # ATHENA_HOME sync happens in refresh's systemd_unit_is_current gate; the unit is guaranteed to exist here.
    refresh_systemd_unit_if_needed(system=system)
...
def systemd_restart(system: bool = False):
    ...
    # ATHENA_HOME sync happens in refresh's systemd_unit_is_current gate; its os.environ mutation
    ...
    if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
        ...
        print(
            ...
            # graceful shutdown, so waiting the full drain budget only stalls the restart (and `athena
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {sudo}athena gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if result.stdout.strip() == "active":
        ...
        print(f"  Run: {sudo}athena gateway start{scope_flag}")
    ...
    if active_state == "activating" and unit_props.get("SubState", "") == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {sudo}athena gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {user_flag}reset-failed {svc} && {sudo}athena gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.athena.gateway-{suffix}" if suffix else "ai.athena.gateway"
...
# at all and we degrade to a detached background process (the `nohup athena gateway run` workaround). See
# #23387.
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_athena_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_athena_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build ``python -m athena_cli.main [--profile X] gateway run --replace``, honoring the active profile."""
    return [get_python_path(), "-m", "athena_cli.main", *_profile_arg().split(), "gateway", "run", "--replace"]
...
def _timestamped_stderr_gateway_command(error_log: Path, *, external_supervisor: bool = False) -> list[str]:
    ...
    (launchd ProgramArguments only) adds ``--external-supervisor`` so ``athena update`` hands back to
    ...
    ``--external-supervisor`` so ``athena update`` sees the flag on the live grandchild argv and hands the
    ...
    return [get_python_path(), "-m", "athena_cli.stderr_timestamp", "--error-log", str(error_log), "--", *inner]
...
def _spawn_detached_gateway() -> bool:
    ...
    the `nohup athena gateway run --replace` workaround but keeps it CLI-managed: stdout goes to
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    from athena_constants import display_athena_home as _dhh
    ...
    print(f"  Try manually: nohup athena gateway run --replace > {_dhh()}/logs/gateway.log 2>&1 &")
    if exit_on_failure:
        sys.exit(1)
    return False
...
def generate_launchd_plist() -> str:
    ...
    nofile_block = ""
    try:
        from athena_cli.resource_limits import configured_nofile_soft_limit
...
<dict>
    ...
    <dict>
        ...
        <key>ATHENA_HOME</key>
        <string>{athena_home}</string>
        <key>ATHENA_SUPERVISED_CHILD</key>
        <string>1</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Athena install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  athena gateway status             # Check status")
    from athena_constants import display_athena_home as _dhh
...
def launchd_stop():
    ...
    _mark_planned_stop()
    # bootout unloads the definition so KeepAlive doesn't respawn; `athena gateway start` re-bootstraps.
    try:
...
def _wait_for_gateway_exit(timeout: float = 10.0, force_after: float | None = 5.0) -> bool:
    ...
    ATHENA_HOMEs work) to exit; SIGKILL it after ``force_after`` s of graceful waiting."""
...
def launchd_restart():
    ...
    try:
        ...
        if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
            ...
            # only stalls the restart (and `athena update`). Bounded SIGTERM → SIGKILL, ~10s.
...
def launchd_status(deep: bool = False):
    ...
    # Athena PID may be a detached fallback process; when launchd IS supervising both PIDs match — don't double-count.
    from gateway.status import get_running_pid
    ...
    if not service_listed:
        ...
        if fallback_pid:
            ...
            print("  Cron jobs will fire. Stop with: athena gateway stop")
        else:
    ...
    if deep:
        log_file = get_athena_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _is_official_docker_checkout() -> bool:
    return str(PROJECT_ROOT) == "/opt/athena" and (PROJECT_ROOT / "docker" / "entrypoint.sh").is_file()
...
def _running_under_gateway_supervisor() -> bool:
    ...
    (shells inherit "0"), s6 ATHENA_S6_SUPERVISED_CHILD, or ``--external-supervisor``."""
...
def named_profile_served_by_running_multiplexer(profile_name: str | None = None) -> bool:
    ...
    try:
        from athena_constants import get_default_athena_root
        default_root = get_default_athena_root()
    except Exception:
        return False
    ...
    try:
        ...
        from athena_cli.profiles import normalize_profile_name
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    "already running". Same user-facing contract; never scans other ATHENA_HOME roots."""
    ...
    try:
        ...
        return
    if pid is None:
        # get_running_pid() filters by the current profile's ATHENA_HOME; warn if the PID file
        ...
        try:
            ...
            if stale is not None and not _pid_record_belongs_to_current_profile(stale):
                logger.warning(
                    "PID file belongs to another profile (athena_home=%s). "
                    "The old gateway may still be running under that profile.",
                    stale.get("athena_home", "<unknown>"),
                )
        except Exception:
            pass
        return
    ...
    print("  Use 'athena gateway restart' to replace it,")
    print("  or 'athena gateway stop' first.")
    print("  Or use 'athena gateway run --replace' to auto-replace.")
    sys.exit(1)


def _guard_official_docker_root_gateway() -> None:
    """Refuse gateway startup when the official Docker privilege drop was bypassed."""
    if not hasattr(os, "geteuid") or os.geteuid() != 0 or _truthy_env(os.getenv("ATHENA_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error("Refusing to run the Athena gateway as root inside the official Docker image.")
    print(
        "  The image entrypoint normally drops privileges to the 'athena' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/athena/docker/entrypoint.sh before the Athena command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$ATHENA_HOME and break later non-root dashboard/gateway runs."
    )
    print("  Set ATHENA_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk.")
    sys.exit(1)
...
def _apply_startup_watchdog_config() -> None:
    ...
    try:
        from athena_startup_watchdog import (
        ...
        try:
            from athena_cli.config import load_config as _sw_load_config
...
def _make_exit_diag():
    ...
    ``asyncio.run()`` can return, for chasing silent Windows gateway deaths. ATHENA_GATEWAY_EXIT_DIAG=0 opts out."""
    from datetime import datetime as _dt, timezone as _tz

    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("ATHENA_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from athena_constants import get_athena_home as _ghh
            log_dir = _ghh() / "logs"
            log_dir.mkdir(parents=True, exist_ok=True)
            line = {
...
def _respawn_storm_backoff() -> None:
    ...
    DEFAULT_CONFIG ``gateway.respawn_storm``; ATHENA_GATEWAY_MAX_STARTS / ATHENA_GATEWAY_START_WINDOW_S
    ...
    try:
        ...
        try:
            ...
            pass
        try:
            _max_starts = int(os.environ["ATHENA_GATEWAY_MAX_STARTS"])
        except (KeyError, ValueError):
            pass
        try:
            _win = float(os.environ["ATHENA_GATEWAY_START_WINDOW_S"])
        except (KeyError, ValueError):
            pass
        ...
        if _storm is not None:
            ...
            try:
                from athena_startup_watchdog import kick_startup_watchdog
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # Detached Windows runs (ATHENA_GATEWAY_DETACHED=1, or non-TTY for older wrappers) ignore
    ...
    # exit-code-75 respawn (stale-code or /restart), which bypasses `athena gateway restart`.
    if supports_systemd_services():
        try:
...
_PLATFORMS = [
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. athena) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL",
             ...
             "password": False, "help": "Channel ID where Athena delivers cron results and notifications."},
    ...
    {"key": "weixin", "label": "Weixin / WeChat", "emoji": "💬", "token_var": "WEIXIN_ACCOUNT_ID"},
    {
        ...
        "setup_instructions": [
            ...
            "6. To authorize users, use DM pairing: athena pairing generate bluebubbles",
...
_UNAUTHORIZED_ACCESS_CHOICES = {
    ...
    False: (1,
        ...
        "Use DM pairing (unknown users request access, you approve with 'athena pairing approve')",
...
def _prompt_unauthorized_access(*, is_email: bool) -> None:
    ...
    if access_idx == 0:
        ...
        print_info("  Approve with: athena pairing approve <platform> <code>")
    elif is_email:
        print_success("  Unknown email senders will be ignored.")
    else:
        print_info("  Skipped — configure later with 'athena gateway setup'")
...
def _telegram_auto_setup(token_var: str) -> tuple[bool, object]:
    ...
    if prompt("  Choice [1/2]", default="1").strip() != "1":
        return False, None
    try:
        from athena_cli.telegram_managed_bot import (
...
def _setup_standard_platform(platform: dict):
    ...
    from athena_cli.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
...
def _running_under_s6() -> bool:
    from athena_cli.service_manager import detect_service_manager
...
def _setup_weixin():
    ...
    print()
    _print_info_lines(
        "  1. Athena will open Tencent iLink QR login in this terminal.",
        "  2. Use WeChat to scan and confirm the QR code.",
        "  3. Athena will store the returned account_id/token in ~/.athena/.env.",
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_athena_home())))
    except KeyboardInterrupt:
        print()
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown users can request access; approve with `athena pairing approve`.")
    elif access_idx == 1:
...
def _builtin_setup_fn(key: str):
    """Resolve a built-in platform's setup function; late-bound to dodge the athena_cli.setup cycle."""
    from athena_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    if required:
        print_info(f"  Set these env vars in ~/.athena/.env: {', '.join(required)}")
    else:
...
def _setup_service_action(
    ...
    try:
        ...
        if backend is not None:
            ...
            print_info("Start manually: athena gateway")
    except UserSystemdUnavailableError as e:
...
_WIZARD_NO_SERVICE_LINES = {
    "wsl": (
        "  WSL detected but systemd is not running.", "  Run in foreground: athena gateway run",
        "  For persistence:   tmux new -s athena 'athena gateway run'",
        "  To enable systemd: add systemd=true to /etc/wsl.conf, then 'wsl --shutdown'",
    ),
    "termux": (
        "  Termux does not use systemd/launchd services.", "  Run in foreground: athena gateway run",
        "  Or start it manually in the background (best effort): nohup athena gateway run >{home}/logs/gateway.log 2>&1 &",
    ),
    "unsupported": (
        "  Service install not supported on this platform.", "  Run in foreground: athena gateway run",
    ),
}
...
def _wizard_install_service(backend: str) -> None:
    ...
    if not (start_now or start_on_login):
        ...
        print_info("  You can install later: athena gateway install")
        if supports_systemd_services():
            print_info("  Or as a boot-time service: sudo athena gateway install --system")
        print_info("  Or run in foreground:  athena gateway run")
        return
    try:
        ...
        print_info("  You can try manually: athena gateway install")
...
def gateway_setup():
    ...
    if any(_is_progress(_platform_status(p)) for p in _all_platforms()):
        ...
        print()
        print_info("No platforms configured. Run 'athena gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(action: str, profile: str | None = None) -> bool:
    ...
    from athena_cli.service_manager import (
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    ``ATHENA_S6_SUPERVISED_CHILD`` (set by ``S6ServiceManager._render_run_script``) marks the supervised
    child, which must run in foreground or we'd recurse run → start → run; ``--no-supervise`` /
    ATHENA_GATEWAY_NO_SUPERVISE=1 opts out (CI smoke, debugging)."""
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("ATHENA_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    # ATHENA_S6_SUPERVISED_CHILD: we ARE the supervised child; fall through so the gateway starts.
    if no_supervise or os.environ.get("ATHENA_S6_SUPERVISED_CHILD"):
        return False
    ...
    print(
        ...
        "  dashboard supervised alongside if ATHENA_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or ATHENA_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    try:
        ...
        # down the supervised gateway cleanly). Prefer `sleep infinity` (matches the static main-athena
        # service's pattern in docker/s6-rc.d/main-athena/run, and frees the Python interpreter — the
...
def _refuse_from_inside_gateway(verb: str, reason: str) -> None:
    ...
    if _is_supervised_gateway_process():
        print_error(
            ...
            f"Use `athena gateway {verb}` from a shell outside the running gateway."
        )
        sys.exit(1)
...
_WSL_FOREGROUND_HINT = (
    "", "  athena gateway run                              # direct foreground",
    "  tmux new -s athena 'athena gateway run'         # persistent via tmux",
    "  nohup athena gateway run > ~/.athena/logs/gateway.log 2>&1 &  # background",
)
...
_NO_BACKEND_MESSAGES = {
    ("install", "termux"): (1,
        "Gateway service installation is not supported on Termux.", "Run manually: athena gateway"),
    ("install", "wsl"): (1,
    ...
    ("install", "s6"): (None,
        ...
        "  athena profile create <name>     # creates the s6 service slot",
        "  athena -p <name> gateway start   # bring it up via s6",
        "  athena status                    # see currently-supervised gateways"),
    ("install", "container"): (0,
        ...
        "To run the gateway: athena gateway run"),
    ("install", "unsupported"): (1,
        "Service installation not supported on this platform.", "Run manually: athena gateway run"),
    ("uninstall", "termux"): (1,
        "Gateway service uninstall is not supported on Termux because there is no managed service to remove.",
        "Stop manual runs with: athena gateway stop"),
    ("uninstall", "s6"): (None,
        "Per-profile gateways are auto-unregistered when you delete the profile.", "",
        "  athena profile delete <name>     # tears down the s6 service slot",
        "  athena -p <name> gateway stop    # stop without deleting the profile"),
    ...
    ("start", "container"): (0,
        ...
        "Or run the gateway directly: athena gateway run"),
...
def _install_systemd_from_cli(args, *, force: bool, system: bool, run_as_user) -> None:
    if is_wsl():
        ...
        _print_info_lines(
            "  Consider running in foreground instead: athena gateway run",
            "  Or use tmux/screen for persistence: tmux new -s athena 'athena gateway run'",
        )
        print()
...
def _cmd_restart(args):
    ...
    # detached run below is the restart; bailing here left `athena gateway restart` a
    ...
    if kind == "systemd" and supports_systemd_services():
        ...
        if linger_ok is not True:
            ...
            _print_lines(
                ...
                "  Then restart the gateway:", "    athena gateway restart",
            )
            return
    ...
    if service_configured:
        _print_lines(
            ...
            "  Fix the service, then retry: athena gateway start",
        )
        sys.exit(1)
...
# ``athena gateway status`` hints for a manually-run / stopped gateway, keyed by host kind.
_STATUS_RUNNING_HINTS = {
    ...
    "windows": ("To install as a Windows Scheduled Task (auto-start on login):", "  athena gateway install"),
    "other": (
        "To install as a service:", "  athena gateway install", "  sudo athena gateway install --system",
    ),
}
_STATUS_STOPPED_HINTS = {
    "termux": (
        "  nohup athena gateway run > ~/.athena/logs/gateway.log 2>&1 &  # Best-effort background start",
    ),
    "wsl": (
        "  tmux new -s athena 'athena gateway run'         # persistent via tmux",
        "  nohup athena gateway run > ~/.athena/logs/gateway.log 2>&1 &  # background",
    ),
    "windows": ("  athena gateway install  # Install as Windows Scheduled Task (auto-start on login)",),
    "other": (
        "  athena gateway install  # Install as user service",
        "  sudo athena gateway install --system  # Install as boot-time system service",
    ),
}
...
def _cmd_status(args):
    ...
    if not snapshot.running and named_profile_served_by_running_multiplexer():
        ...
        print("  Manage it from the default profile: athena gateway status")
        ...
        if pids:
            ...
            print()
            print("To start:")
            print("  athena gateway run      # Run in foreground")
...
def _cmd_migrate_legacy(args):
    """Stop, disable, and remove legacy Athena gateway unit files (e.g. athena.service)."""
    dry_run = getattr(args, "dry_run", False)
    yes = getattr(args, "yes", False)
    if not supports_systemd_services() and not is_macos():
        print("Legacy unit migration only applies to systemd-based Linux hosts.")
        return
    remove_legacy_athena_units(interactive=not yes, dry_run=dry_run)
