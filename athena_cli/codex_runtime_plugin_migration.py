...
def _translate_one_server(
    ...
    has_command = bool(athena_cfg.get("command"))
    has_url = bool(athena_cfg.get("url"))
    ...
    # Timeouts
    if "timeout" in athena_cfg:
        try:
            out["tool_timeout_sec"] = float(athena_cfg["timeout"])
        except (TypeError, ValueError):
            skipped.append("timeout (not numeric)")
    if "connect_timeout" in athena_cfg:
        try:
            out["startup_timeout_sec"] = float(athena_cfg["connect_timeout"])
        except (TypeError, ValueError):
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (ATHENA_HOME, PYTHONPATH) could
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a ATHENA_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed athena-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.athena`` symlink that happens to live under ``/private/var/folders``
...
def _build_athena_tools_mcp_entry() -> dict:
    ...
    so a athena installed under /opt/, /usr/local/, or a venv all work.
    ATHENA_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # ATHENA_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_athena_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever ATHENA_HOME its launcher sets
    ...
    # — that would override the launcher's ATHENA_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("ATHENA_HOME", tmp_path) would
    ...
    if athena_home and _looks_like_test_tempdir(athena_home):
        athena_home = ""
    if athena_home:
        env["ATHENA_HOME"] = athena_home
    # PYTHONPATH passes through so a worktree-launched athena finds the
...
def migrate(
    ...
    *,
    ...
    expose_athena_tools: bool = True,
) -> MigrationReport:
    """Translate Athena mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        athena_config: full ~/.athena/config.yaml dict
        ...
        expose_athena_tools: when True (default), register Athena' own
    ...
    for name, cfg in athena_servers.items():
    ...
    # The server itself is agent/transports/athena_tools_mcp_server.py
