"""Migrate Athena MCP server config and Codex's installed curated plugins into ~/.codex/config.toml.
"""
...
MIGRATION_END_MARKER = "# end athena-agent managed section"
...
# Athena MCP keys codex understands (transport stdio/http, timeouts, general). Any other key is
# dropped with a warning: ``sampling`` has no codex equivalent; the rest are unknown Athena keys.
_KNOWN_ATHENA_KEYS = {
...
# (athena key, codex key, skip note) — timeouts are emitted as floats or skipped when non-numeric.
_TIMEOUT_KEYS = (
...
def _translate_one_server(name: str, athena_cfg: dict) -> tuple[Optional[dict], list[str]]:
    """Translate one Athena MCP server config to codex's inline-table dict.

    Returns ``(codex_entry, skipped_keys)``; ``codex_entry`` is None when the config is unusable.
    stdio (``command``) wins over ``url`` when both are set. Athena' ``transport: sse`` hint is
    informational only — codex auto-negotiates. ``enabled`` is emitted only when explicitly false
    (codex defaults to true).
    """
    if not isinstance(athena_cfg, dict):
        return None, []
    skipped: list[str] = []
    out: dict[str, Any] = {}
    if athena_cfg.get("command"):
        if athena_cfg.get("url"):
            skipped.append("url (both command and url set; preferring stdio)")
        out["command"] = str(athena_cfg["command"])
        if athena_cfg.get("args"):
            out["args"] = [str(a) for a in athena_cfg["args"]]
        if athena_cfg.get("env"):
            out["env"] = _str_map(athena_cfg["env"])
        if athena_cfg.get("cwd"):
            out["cwd"] = str(athena_cfg["cwd"])
    elif athena_cfg.get("url"):
        out["url"] = str(athena_cfg["url"])
        if athena_cfg.get("headers"):
            out["http_headers"] = _str_map(athena_cfg["headers"])
        if athena_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]
    for athena_key, codex_key, note in _TIMEOUT_KEYS:
        if athena_key in athena_cfg:
            try:
                out[codex_key] = float(athena_cfg[athena_key])
            except (TypeError, ValueError):
                skipped.append(note)
    if athena_cfg.get("enabled") is False:
        out["enabled"] = False
    for key in athena_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            skipped.append(f"{key} (no codex equivalent)")
        elif key not in _KNOWN_ATHENA_KEYS:
...
# env-var passthrough (ATHENA_HOME, PYTHONPATH) could carry one in pathological cases.
_TOML_ESCAPES = (
    ("\\", "\\\\"), ('"', '\\"'), ("\b", "\\b"), ("\t", "\\t"),
    ("\n", "\\n"), ("\f", "\\f"), ("\r", "\\r"))
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out += ["# (no MCP servers, plugins, or permissions configured by Athena)", MIGRATION_END_MARKER]
        return "\n".join(out) + "\n"
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(codex_home=str(codex_home) if codex_home else None) as client:
            client.initialize(client_name="athena-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    Such dirs are reaped between sessions; a ATHENA_HOME pointing there burned into
...
def _build_athena_tools_mcp_entry() -> dict:
    """Codex stdio entry launching Athena' own tool surface as an MCP server (browser/web/
    delegate_task/vision/memory/skills call-backs).

    ATHENA_HOME passes through only IF SET, read from os.environ (not get_athena_home()): when
    unset the codex subprocess must inherit its launcher's runtime ATHENA_HOME (systemd, gateway,
    kanban), not a migrate-time default burned into config.toml that pins the wrong profile. The
    pytest-tempdir guard keeps a sibling test's monkeypatched ATHENA_HOME out of the user's real
    config. PYTHONPATH passes through so a worktree-launched athena finds the branch's modules.
    """
    import sys
    env: dict[str, str] = {}
    # ATHENA_HOME passes through IF SET so the MCP subprocess sees the same config / auth / sessions DB as
    # the parent CLI. Read from os.environ (not get_athena_home()) on purpose: when the env var is unset we
    # want codex's subprocess to inherit whatever ATHENA_HOME its launcher sets at runtime (systemd unit,
    # gateway, kanban dispatcher, custom shell), rather than burning the migrate-time resolved default into
    # config.toml — that would override the launcher's ATHENA_HOME and pin the subprocess to the wrong
    # profile. The pytest-tempdir guard below catches the issue #26250 Bug C scenario: a sibling test's
    # monkeypatch.setenv("ATHENA_HOME", tmp_path) would otherwise leak a transient pytest tempdir into the
    # user's real ~/.codex/config.toml and silently brick codex once the tempdir is GC'd.
    athena_home = os.environ.get("ATHENA_HOME") or ""
    if athena_home and not _looks_like_test_tempdir(athena_home):
        env["ATHENA_HOME"] = athena_home
    if os.environ.get("PYTHONPATH"):
        env["PYTHONPATH"] = os.environ["PYTHONPATH"]
    # Quiet mode + redaction defaults so the MCP wire stays clean.
    env["ATHENA_QUIET"] = "1"
    env["ATHENA_REDACT_SECRETS"] = env.get("ATHENA_REDACT_SECRETS", "true")
    return {
...
def migrate(
    athena_config: dict, *, codex_home: Optional[Path] = None, dry_run: bool = False,
    discover_plugins: bool = True, default_permission_profile: Optional[str] = ":workspace",
    expose_athena_tools: bool = True) -> MigrationReport:
    """Translate Athena mcp_servers config + Codex curated plugins into ~/.codex/config.toml.
    ...
    approval prompt on every write. ``expose_athena_tools`` registers Athena' own tool surface
    (agent/transports/athena_tools_mcp_server.py, launched on demand by codex over stdio) as an MCP
    ...
    athena_servers = (athena_config or {}).get("mcp_servers") or {}
    if not isinstance(athena_servers, dict):
        report.errors.append("mcp_servers in Athena config is not a dict; cannot migrate.")
        return report
    translated: dict[str, dict] = {}
    for raw_name, cfg in athena_servers.items():
        name = str(raw_name)
    ...
    if expose_athena_tools:
        translated["athena-tools"] = _build_athena_tools_mcp_entry()
        if "athena-tools" not in report.migrated:
            report.migrated.append("athena-tools")
