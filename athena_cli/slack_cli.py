...
Typical workflow::
    ...
    $ athena slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(bot_name: str, bot_description: str) -> dict:
    ...
    pasting.
    """
    from athena_cli.commands import slack_app_manifest
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``athena_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$ATHENA_HOME/slack-manifest.json``)
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("ATHENA_HOME") or str(Path.home() / ".athena")) / "slack-manifest.json"
        else:
