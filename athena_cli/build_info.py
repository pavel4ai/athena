...
``athena_cli/dump.py`` and ``athena_cli/banner.py``).  That doesn't work inside
...
image was built from, the Docker build writes the build-time ``$ATHENA_GIT_SHA``
arg into ``<project_root>/.athena_build_sha``.  This module is the single
...
- Returns ``None`` when the file is absent.  Source installs and dev images
  built without the ``ATHENA_GIT_SHA`` build-arg fall through to live-git
...
def get_build_sha(short: int = 8) -> Optional[str]:
    ...
    Reads ``<project_root>/.athena_build_sha`` if present.  The file is
    written by the Dockerfile's ``ATHENA_GIT_SHA`` build-arg and contains
