"""Image-authored deployment provenance for immutable Athena runtimes.

The image bakes ``/etc/athena/image-provenance.json`` outside ``$ATHENA_HOME`` and the checkout, so a
...
IMAGE_PROVENANCE_PATH = Path("/etc/athena/image-provenance.json")
