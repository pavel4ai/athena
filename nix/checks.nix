    ...
    in {
      ...
      checks = {
        ...
        bundled-skills = pkgs.runCommand "athena-bundled-skills" { } ''
          ...
          SKILL_COUNT=$(find ${athena-agent}/share/athena-agent/skills -name "SKILL.md" | wc -l)
        ...
        bundled-locales = pkgs.runCommand "athena-bundled-locales" { } ''
          ...
          LOC_COUNT=$(find ${athena-agent}/share/athena-agent/locales -name "*.yaml" | wc -l)
          ...
          echo "=== Rendering via the wrapper override (ATHENA_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          ...
          BARE_DIR=$(cd "$HOME" && ${athenaVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${athenaVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without ATHENA_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        # Verify ATHENA_NODE is set in wrapper and points to Node 20+
