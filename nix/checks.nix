...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      athena-agent = self'.packages.default;
      athenaVenv = athena-agent.athenaVenv;
      ...
      evalHomeModule =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              home = {
                username = "athena-check";
                homeDirectory = "/home/athena-check";
                stateVersion = "24.11";
              };
            }
            { services.athena-agent = settings; }
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.athena-agent = programs;
              services.athena-agent = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.athena-agent. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.athena-agent);
      ...
      configKeys = pkgs.runCommand "athena-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${athenaVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "athena-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "athena-build-package" { } ''
          echo "PASS: package built at ${athena-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "athena-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.ATHENA_TEST = "1";
                environmentFiles = [ "/run/secrets/athena-env" ];
                athenaHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "athena" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "athena" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.athenaAgentSetup.data;

            failures =
              lib.optional (names != [
                "athena-agent"
                "athena-backend"
              ]) "expected athena-agent + athena-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/athena gateway" (argvOf "athena-agent")
              ) "gateway process does not run `athena gateway`: ${argvOf "athena-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/athena serve" (argvOf "athena-backend")
              ) "backend process does not run `athena serve`: ${argvOf "athena-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "athena-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/athena-check/.athena" (envOf n)) names
              ) "gateway and backend must share one ATHENA_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.ATHENA_HOME or null != "/home/athena-check/.athena"
              ) "programs.athena-agent.enable must export ATHENA_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "athena-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/athena-check/.athena/SOUL.md" activation
              ) "athenaHomeFiles must install into ATHENA_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads ATHENA_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "ATHENA_MANAGED=home-manager" (envOf n)) names
              ) "processes must report ATHENA_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "athena-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "athena-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            # the priority.
            sameAsDefault = "/home/athena-check";
            ...
            cases = [
              ...
              {
                name = "athenaHomeFiles needs no directory";
                ok = accepts { athenaHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "athena-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one ATHENA_HOME ───────────────
        # `programs.enable` exports ATHENA_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.athena
        # while the services use the ATHENA_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/athena-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                athenaHome = "/home/athena-check/.athena-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "athena-desktop") cfg.home.packages;
            ...
            # second Athena runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "athena-agent") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.athena-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.athena-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one athena-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "ATHENA_HOME" != "/home/athena-check/.athena-work"
              ) "the launcher must carry ATHENA_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "ATHENA_HOME")})"
              ++ lib.optional (
                setValue "ATHENA_MANAGED" != "home-manager"
              ) "the launcher must report ATHENA_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "ATHENA_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "ATHENA_DESKTOP_ATHENA" != "${lib.head agentPackages}/bin/athena"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "ATHENA_DESKTOP_ATHENA")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "ATHENA_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "ATHENA_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "ATHENA_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "ATHENA_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "ATHENA_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "athena-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares ATHENA_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set ATHENA_HOME" wrapper
              ) "the launcher must carry ATHENA_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "ATHENA_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "ATHENA_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? athena-backend || cfg.launchd.agents ? athena-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "athena-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `athena`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.athena-agent.enable = true;";
              }
              {
                value = false;
                expect = "programs.athena-agent.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "athena-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "athena-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "athena" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."athena-agent-setup".text;

            failures =
              lib.optional (names != [
                "athena-agent"
                "athena-backend"
              ]) "expected athena-agent + athena-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/athena gateway" (execOf "athena-agent")
              ) "gateway unit does not run `athena gateway`: ${execOf "athena-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/athena dashboard" (execOf "athena-backend")
              ) "backend unit does not run `athena dashboard`: ${execOf "athena-backend"}"
              ++ lib.optional (
                units.athena-agent.environment.ATHENA_HOME != units.athena-backend.environment.ATHENA_HOME
              ) "gateway and backend must share one ATHENA_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/athena/.athena/SOUL.md" activation
              ) "athenaHomeFiles must install into ATHENA_HOME";
          ...
          in
          pkgs.runCommand "athena-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs athena, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.athena-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/athena serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec athena directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "athena-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "athena-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec athena, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/athena dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "athena-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        # $ATHENA_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          let
            envScript = (import ./moduleCommon.nix { inherit lib; }).mkEnvScript {
              inherit pkgs;
              environment = {
                ATHENA_PUBLIC = "visible";
              };
            };
          in
          pkgs.runCommand "athena-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'ATHENA_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--athena-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "athena-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "athena-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${athena-agent}/bin/athena || (echo "FAIL: athena binary missing"; exit 1)
          test -x ${athena-agent}/bin/athena-agent || (echo "FAIL: athena-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${athena-agent}/bin/athena --version 2>&1 | grep -qi "athena" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "athena-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in athena athena-agent athena-acp; do
            test -x ${athena-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "athena-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking athena --help ==="
          ${athena-agent}/bin/athena --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${athena-agent}/bin/athena --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "athena-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${athena-agent}/share/athena-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${athena-agent}/share/athena-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "ATHENA_BUNDLED_SKILLS" ${athena-agent}/bin/athena || \
            (echo "FAIL: ATHENA_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: ATHENA_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${athena-agent}/share/athena-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${athena-agent}/share/athena-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "ATHENA_OPTIONAL_SKILLS" ${athena-agent}/bin/athena || \
            (echo "FAIL: ATHENA_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, ATHENA_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "athena-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${athena-agent}/share/athena-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${athena-agent}/share/athena-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "ATHENA_BUNDLED_PLUGINS" ${athena-agent}/bin/athena || \
            (echo "FAIL: ATHENA_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: ATHENA_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "athena-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${athena-agent}/share/athena-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${athena-agent}/share/athena-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${athena-agent}/share/athena-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "ATHENA_BUNDLED_LOCALES" ${athena-agent}/bin/athena || \
            (echo "FAIL: ATHENA_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: ATHENA_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + ATHENA_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && ATHENA_BUNDLED_LOCALES=${athena-agent}/share/athena-agent/locales \
            ${athenaVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with ATHENA_BUNDLED_LOCALES set"; exit 1)
        ...
        # ATHENA_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "athena-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${athena-agent}/share/athena-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${athena-agent}/share/athena-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "ATHENA_OPTIONAL_MCPS" ${athena-agent}/bin/athena || \
            (echo "FAIL: ATHENA_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: ATHENA_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${athena-agent}/bin/athena mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: athena mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "athena-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${athena-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${athena-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "ATHENA_TUI_DIR" ${athena-agent}/bin/athena || \
            (echo "FAIL: ATHENA_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: ATHENA_TUI_DIR set in wrapper"
        ...
        # Verify ATHENA_NODE is set in wrapper and points to Node 26+
        # (Athena pins its toolchain to Node 26 everywhere)
        athena-node = pkgs.runCommand "athena-node-version" { } ''
          set -e
          echo "=== Checking ATHENA_NODE in wrapper ==="
          grep -q "ATHENA_NODE" ${athena-agent}/bin/athena || \
            (echo "FAIL: ATHENA_NODE not set in wrapper"; exit 1)
          echo "PASS: ATHENA_NODE present in wrapper"

          ATHENA_NODE=$(sed -n "s/^export ATHENA_NODE='\(.*\)'/\1/p" ${athena-agent}/bin/athena)
          test -x "$ATHENA_NODE" || (echo "FAIL: ATHENA_NODE=$ATHENA_NODE not executable"; exit 1)
          echo "PASS: ATHENA_NODE executable at $ATHENA_NODE"

          NODE_MAJOR=$("$ATHENA_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Athena requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All ATHENA_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify ATHENA_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "athena-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(ATHENA_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking ATHENA_MANAGED guards ==="
          check_blocked "config set" ${athena-agent}/bin/athena config set model foo
          check_blocked "config edit" ${athena-agent}/bin/athena config edit
        ...
        extra-python-packages = let
          ...
          athenaWithExtra = athena-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "athena-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${athenaWithExtra}/bin/athena || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${athenaWithExtra}/bin/athena || \
          ...
          if grep -q "PYTHONPATH" ${athena-agent}/bin/athena; then
        ...
        extra-dependency-groups = let
          athenaWithGroups = athena-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "athena-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${athenaWithGroups}"
          echo "venv: ${athenaWithGroups.athenaVenv}"
        ...
        messaging-variant = pkgs.runCommand "athena-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.athenaVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "athena-config-roundtrip" {
          ...
          merge_and_load() {
            local athena_home="$1"
            export ATHENA_HOME="$athena_home"
            ${configMergeScript} ${nixSettings} "$athena_home/config.yaml"
            ${athenaVenv}/bin/python3 -c '
import json, sys
