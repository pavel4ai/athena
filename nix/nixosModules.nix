# nix/nixosModules.nix — the NixOS module for athena-agent
#
...
{ inputs, ... }:
{
  flake.nixosModules.default =
    ...
    let
      ...
      athenaHome = "${cfg.stateDir}/.athena";
      ...
      containerModeFile = pkgs.writeText "athena-container-mode" ''
      ...
      // common.processEnvironment { inherit athenaHome; };

      unitPath = common.processPath { inherit pkgs cfg; };

    in
    {
      options.services.athena-agent =
        common.sharedOptions {
          defaultPackage = athena-agent;
          defaultPackageText = lib.literalExpression "athena-agent.packages.\${system}.default";
      ...
      config = lib.mkIf cfg.enable (
        lib.mkMerge [
          ...
          (lib.mkIf (cfg.mcpServers != { }) {
            services.athena-agent.settings.mcp_servers = common.mcpServersToConfig cfg.mcpServers;
          })
          ...
          (lib.mkIf cfg.addToSystemPackages {
            ...
            environment.variables.ATHENA_HOME = athenaHome;
          })
          ...
          # ── Assertions ─────────────────────────────────────────────────────
          {
            assertions =
              common.pluginNameAssertions {
                inherit cfg;
                optionPath = "services.athena-agent";
              }
              ++ common.workspaceFilesAssertions {
                inherit cfg;
                opt = options.services.athena-agent.workingDirectory;
                optionPath = "services.athena-agent";
              }
              ...
              ++ [
                {
                  ...
                  message = "services.athena-agent: backend.mode is not supported together with container.enable — the container runs the gateway only.";
                }
              ];
          }
          ...
          # ── Directories ───────────────────────────────────────────────────
          {
            systemd.tmpfiles.rules = [
              ...
              "d ${athenaHome}                  2770 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.stateDir}/home           0750 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.workingDirectory}        2770 ${cfg.user} ${cfg.group} - -"
            ]
            ++ map (d: "d ${athenaHome}/${d} 2770 ${cfg.user} ${cfg.group} - -") common.stateSubdirs;
          }

          # ── Activation: link config + auth + documents ────────────────────
          {
            system.activationScripts."athena-agent-setup" =
              lib.stringAfter
                (
                ...
                ''
                  ...
                  chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${athenaHome} ${cfg.stateDir}/home ${cfg.workingDirectory}
                  chmod 2770 ${cfg.stateDir} ${athenaHome} ${cfg.workingDirectory}
                  ...
                  find ${athenaHome} -maxdepth 1 \
                    \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
                    -exec chmod g+rw {} + 2>/dev/null || true
                  for _subdir in ${lib.concatStringsSep " " common.stateSubdirs}; do
                    mkdir -p "${athenaHome}/$_subdir"
                    chown ${cfg.user}:${cfg.group} "${athenaHome}/$_subdir"
                    chmod 2770 "${athenaHome}/$_subdir"
                    find "${athenaHome}/$_subdir" -type f \
                      -exec chmod g+rw {} + 2>/dev/null || true
                  done

                  ${common.mkStateScript {
                    inherit pkgs cfg athenaHome;
                    workingDirectory = cfg.workingDirectory;
                  ...
                  chown -h ${cfg.user}:${cfg.group} ${athenaHome}/plugins/nix-managed-* 2>/dev/null || true
                  ...
                  ${
                    if cfg.container.enable then
                      ''
                        install -o ${cfg.user} -g ${cfg.group} -m 0644 ${containerModeFile} ${athenaHome}/.container-mode
                      ''
                    else
                      ''
                        rm -f ${athenaHome}/.container-mode
                        ...
                        ${lib.concatStringsSep "\n" (
                          map (
                            ...
                            in
                            ''
                              if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${athenaHome}" ]; then
                                rm -f "${symlinkPath}"
          ...
          (lib.mkIf (!cfg.container.enable && cfg.backend.mode != "none") {
            systemd.services.athena-backend = {
