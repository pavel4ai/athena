...
let
  nodejs = nodejs_22;
  athenaVenv = callPackage ./python.nix {
...
stdenv.mkDerivation (finalAttrs: {
  ...
  installPhase = ''
    ...
    cp -r ${bundledSkills} $out/share/athena-agent/skills
    cp -r ${bundledPlugins} $out/share/athena-agent/plugins
    cp -r ${bundledLocales} $out/share/athena-agent/locales
    cp -r ${athenaWeb} $out/share/athena-agent/web_dist

    mkdir -p $out/ui-tui
    cp -r ${athenaTui}/lib/athena-tui/* $out/ui-tui/
    ...
    ${lib.concatMapStringsSep "\n"
      (name: ''
        makeWrapper ${athenaVenv}/bin/${name} $out/bin/${name} \
          ...
          --set ATHENA_NODE ${lib.getExe nodejs} \
          ${lib.optionalString (rev != null) ''--set ATHENA_REVISION ${rev} \''}
  ...
  passthru = {
    ...
    devShellHook = ''
      STAMP=".nix-stamps/athena-agent"
      STAMP_VALUE="${pyprojectHash}:${uvLockHash}"
      if [ ! -f "$STAMP" ] || [ "$(cat "$STAMP")" != "$STAMP_VALUE" ]; then
        echo "athena-agent: installing Python dependencies..."
        ...
        source .venv/bin/activate
        export ATHENA_PYTHON=${athenaVenv}/bin/python3
      fi
    '';
  };
