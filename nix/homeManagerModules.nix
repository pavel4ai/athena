# nix/homeManagerModules.nix — the Home Manager module for athena-agent
#
# This module is the user-level equivalent of nixosModules.default. Athena is
...
#
# `services.athena-agent` is the same option set on both modules. All of the
...
#   changed   addToSystemPackages      -> programs.athena-agent.enable and
#                                        home.sessionVariables
#   added     programs.athena-agent    the CLI and the desktop application,
#                                      because Home Manager separates an
#                                      installation from a daemon
#   changed   stateDir (+ "/.athena")  -> athenaHome, set directly
#
# To use the module:
#   imports = [ athena-agent.homeManagerModules.default ];
#   programs.athena-agent = {
#     enable = true;          # the athena CLI on your PATH
...
{ inputs, ... }:
{
  flake.homeManagerModules.default =
    ...
    let
      ...
      cfgPrograms = config.programs.athena-agent;
      ...
      # not reach it, and the application would open ~/.athena while the
      # services use athenaHome. Thus the launcher carries the value itself.
      #
      # ATHENA_MANAGED rides along only when the services are enabled. That
      ...
      desktopEnvironment = {
        ATHENA_HOME = cfg.athenaHome;
      }
      // lib.optionalAttrs cfg.enable {
        inherit (processEnvironment) ATHENA_MANAGED;
      }
      // lib.optionalAttrs desktopUsesService {
        ATHENA_DESKTOP_REMOTE_URL = "http://${cfg.backend.host}:${toString cfg.backend.port}";
      };
      ...
      # token the desktop resolver throws ("ATHENA_DESKTOP_REMOTE_URL is set
      # but ATHENA_DESKTOP_REMOTE_TOKEN is not"), so the two variables travel
      ...
      desktopRun = lib.optional desktopUsesService ''
        if [ -r ${lib.escapeShellArg cfg.backend.sessionTokenFile} ]; then
          ATHENA_DESKTOP_REMOTE_TOKEN="$(tr -d '\r\n' < ${lib.escapeShellArg cfg.backend.sessionTokenFile})"
          export ATHENA_DESKTOP_REMOTE_TOKEN
        else
          echo "athena-desktop: cannot read the session token at ${cfg.backend.sessionTokenFile}." >&2
          echo "athena-desktop: the application starts its own backend instead of the one of the service." >&2
        fi
      '';
    ...
    in
    {
      # ── programs.athena-agent — the installation ───────────────────────
      # Home Manager separates "install this application for me" from "run
      # this daemon". Athena needs both, and a person can want one without
      # the other: an application with no gateway, or a headless gateway on
      # a machine with no display.
      #
      # `services.athena-agent` stays the authority for the state and the
      # configuration. This module reads athenaHome and the backend address
      # from it, and never the reverse.
      options.programs.athena-agent = {
        enable = lib.mkEnableOption ''
          the Athena Agent command line application.

          This adds `athena` to home.packages, and exports ATHENA_HOME with
          home.sessionVariables. An interactive shell then uses the same
          state as `services.athena-agent`
        '';

        package = lib.mkOption {
          type = lib.types.package;
          default = effectivePackage;
          defaultText = lib.literalExpression "config.services.athena-agent.package";
          description = ''
            The athena-agent package to install.

            The default follows `services.athena-agent.package`, and applies
        ...
        desktop = {
          enable = lib.mkEnableOption ''
            the Athena Desktop application (Electron).

            This adds `athena-desktop` to home.packages, with an XDG
            launcher entry on Linux. The launcher starts the same Athena
            runtime that `package` gives, and reads the ATHENA_HOME of
            `services.athena-agent`. Thus the application, the interactive
            shell and the services share one state directory.

            The Electron application carries its own Athena runtime with
            the usual distribution. This module gives it the Nix package
            instead, with ATHENA_DESKTOP_ATHENA. It installs no second copy
            of Athena, and it downloads nothing on the first start
          '';

          package = lib.mkOption {
            type = lib.types.package;
            default = cfgPrograms.package.athenaDesktop;
            defaultText = lib.literalExpression "config.programs.athena-agent.package.athenaDesktop";
            description = ''
              The athena-desktop package to use.

              The default follows `package`, and thus also
              `services.athena-agent.extraPythonPackages` and
              `extraDependencyGroups`, because the desktop application is a
              passthru of the agent package. A package that you set here
              carries its own Athena runtime, and this module cannot make
      ...
      options.services.athena-agent =
        ...
        // {
          athenaHome = lib.mkOption {
            type = lib.types.str;
            default = "${config.home.homeDirectory}/.athena";
            defaultText = lib.literalExpression ''"''${config.home.homeDirectory}/.athena"'';
            description = ''
              The value of ATHENA_HOME. This state directory holds
              config.yaml, .env, auth.json, the sessions, the skills, the
              memory and the cron jobs.

              The NixOS module takes a `stateDir` and adds `/.athena` to it.
              This module sets ATHENA_HOME directly. Thus an existing
              ~/.athena continues to work, and you can give the directory any
              name.
            '';
            example = "/home/alice/.athena-work";
          };

          # `installPackage` moved to `programs.athena-agent.enable`. The
          ...
          # quiet removal gives them a machine with no `athena` and no
          ...
          installPackage = lib.mkOption {
            ...
            visible = false;
            description = ''
              Removed. Use `programs.athena-agent.enable` instead.
            '';
          };
      ...
      config = lib.mkMerge [
        ...
        (lib.mkIf cfgPrograms.enable {
          ...
          home.sessionVariables.ATHENA_HOME = cfg.athenaHome;
        })

        # A launcher from the desktop menu reads no shell profile, so the
        # ATHENA_HOME that `programs.enable` exports does not reach it. Home
        # Manager writes only systemd.user.sessionVariables into
        # environment.d, and this module does not put ATHENA_HOME there,
          ...
          assertions = [
            {
              ...
              # with no `athena` on the PATH and no message.
        ...
        (lib.mkIf cfg.enable (
          lib.mkMerge [
                ...
                ++ [
                  {
                    ...
                    message = "services.athena-agent.backend.waitFor = \"interface\" works on Linux only. Use \"hostname\" on Darwin.";
                  }
                ];
            }
            ...
            # ── Activation: directories, config, secrets, documents ────────
            {
              ...
              home.activation.athenaAgentSetup =
                lib.hm.dag.entryAfter
                  ...
                  (
                    common.mkStateScript {
                      inherit pkgs cfg;
                      inherit (cfg) athenaHome workingDirectory;
                      run = "$DRY_RUN_CMD ";
            ...
            (lib.mkIf (isLinux && cfg.gateway.enable) {
              systemd.user.services.athena-agent = mkUnit {
            ...
            (lib.mkIf (isLinux && cfg.backend.mode != "none") {
              systemd.user.services.athena-backend = mkUnit {
            ...
            (lib.mkIf (isDarwin && cfg.gateway.enable) {
              launchd.agents.athena-agent = mkAgent {
                argv = common.gatewayArgv cfg;
                logName = "athena-agent";
              };
            })

            (lib.mkIf (isDarwin && cfg.backend.mode != "none") {
              launchd.agents.athena-backend = mkAgent {
                argv = common.backendArgv { inherit pkgs cfg; };
                logName = "athena-backend";
              };
            })
          ]
        ))
