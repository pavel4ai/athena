# nix/web.nix — Athena Web Dashboard (Vite/React) frontend build
{ athenaNpmLib, ... }:
athenaNpmLib.buildNpmPackage {
  dirs = [
    "web"

    # @athena/shared ships as a file: workspace dep of web, so its source
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../athena_cli/web_dist for the
