...
#
# `services.athena-agent` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  athenaHome, programs.athena-agent (the CLI and
...
let
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "Athena Agent";
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The Athena configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `athena config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $ATHENA_HOME/.env. Athena reads that file at each start, with
          load_athena_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."athena/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that Athena refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          Athena reads SOUL.md and memories/ from ATHENA_HOME, so put those
          files in `athenaHomeFiles`.
      ...
      athenaHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into ATHENA_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          Athena reads SOUL.md and the memory files from ATHENA_HOME and not
          from the working directory. Declare those files here, or Athena
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      # ── The backend: `athena serve` or `athena dashboard` ──────────────
      # `athena serve` and `athena dashboard` are the same entry point,
      # athena_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that Athena Desktop
      ...
      # controls an external gateway, with `athena gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that Athena
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            ATHENA_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. Athena Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."athena/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `athena` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.athena-agent.installPackage was removed. Athena now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.athena-agent.enable = ${lib.boolToString (value != false)};  # the athena CLI, and ATHENA_HOME for your shells
        programs.athena-agent.desktop.enable = true;  # the desktop application

      `services.athena-agent` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "athena-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "athena-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "athena-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "athena-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "athena-agent: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # ATHENA_MANAGED variable of the service. The value tells the shell
    ...
    in
    ''
      # Directories. The service units and Athena make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${athenaHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. Athena
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${athenaHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${athenaHome}/config.yaml
            ${run}chmod ${modes.config} ${athenaHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "athena-managed" managedSystem} ${athenaHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${athenaHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${athenaHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${athenaHome}/auth.json"
        else
          ''
            if [ ! -e ${athenaHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${athenaHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree athenaHome cfg.athenaHomeFiles}
      ...
      ${run}find ${athenaHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "athena-agent: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${athenaHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  gatewayArgv =
    cfg:
    [
      "${effectivePackage cfg}/bin/athena"
      "gateway"
    ]
    ++ cfg.extraArgs;
  ...
  #
  # `exec` on the last line keeps athena as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "athena-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "athena-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "athena-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          ATHENA_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export ATHENA_DASHBOARD_SESSION_TOKEN

          if [ -z "$ATHENA_DASHBOARD_SESSION_TOKEN" ]; then
            echo "athena-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "athena-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "athena-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "athena-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "athena-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "athena-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "athena-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "Athena Agent web dashboard and desktop backend"
    else
      "Athena Agent backend for Athena Desktop";

  # The environment that each Athena process needs, from either module.
  #
  # managedSystem gives the value of ATHENA_MANAGED. The CLI reads that
    ...
    [
      {
        ...
        message = ''
          ...
          To give Athena an identity and a memory, use
          ${optionPath}.athenaHomeFiles instead. Those files go to
          ATHENA_HOME. Athena reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of ATHENA_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
