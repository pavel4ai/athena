...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      athenaNpmLib = self'.packages.default.passthru.athenaNpmLib;
      ...
      athenaAgentDevShellHook = self'.packages.default.passthru.devShellHook;
    in
    {
      devShells.default = pkgs.mkShell {
        packages = with pkgs; [
          (pkgs.runCommand "athena" { } ''
            mkdir -p $out/bin
            install -Dm755 ${../athena} $out/bin/athena
          '')
        ...
        shellHook = ''
          ${athenaAgentDevShellHook}
          ${athenaNpmLib.mkNpmDevShellHook npmPackageJsonPaths}
          ...
          export ATHENA_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          ...
          echo "Athena Agent dev shell in $ATHENA_PYTHON_SRC_ROOT"
          echo "Ready. Run 'athena' or 'sandbox athena' to start."
        '';
      };
    };
}
