# nix/packages.nix — Athena Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    { pkgs, lib, inputs', ... }:
    let
      athenaAgent = pkgs.callPackage ./athena-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = athenaAgent.athenaTui;
        web = athenaAgent.athenaWeb;
        desktop = athenaAgent.athenaDesktop;
      };
    };
}
