...
class _TurnScopes:
    ...
    home: Any = None  # per-turn ATHENA_HOME override for a resumed remote profile
    secret: Any = None
    terminal: Any = None
    ...
    try:
        ...
        def _thinking_should_play() -> bool:
            if is_audio_output_active():
                return False
            try:
                from athena_cli.voice import is_continuous_active
...
def _finish_turn(sid: str, session: dict, st: _TurnRun) -> None:
    ...
    if isinstance(run_kwargs, dict):
        run_kwargs.clear()
    try:  # while the profile ATHENA_HOME override is still active (session's own config)
