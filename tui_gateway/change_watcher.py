...
def _watcher_home() -> Path:
    ...
    return Path(override if isinstance(override, str) and override else _athena_home)
...
def _ensure_skin_watcher() -> None:
    ...
    threading.Thread(target=_loop, name="athena-change-watcher", daemon=True).start()
