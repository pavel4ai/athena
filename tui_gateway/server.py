...
load_athena_dotenv(
    athena_home=_athena_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.athena/logs/tui_gateway_crash.log
...
try:
    _slash_timeout = float(os.environ.get("ATHENA_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _ws_orphan_reap_grace = float(
        os.environ.get("ATHENA_TUI_WS_ORPHAN_REAP_GRACE_S") or "20"
    )
except (ValueError, TypeError):
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("ATHENA_TUI_RPC_POOL_WORKERS") or "4")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 4
...
try:
    _SESSION_TTL_S = float(os.environ.get("ATHENA_TUI_SESSION_TTL_S") or 6 * 3600)
...
# ``profile`` on those calls; we open that profile's db and bind its ATHENA_HOME
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    def _build() -> None:
        ...
        try:
            ...
            # ATHENA_HOME so config/skills/model resolve to it, and hand the
...
def _load_cfg() -> dict:
    ...
    try:
        ...
        # launch profile's _athena_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_athena_home_override()
        home = override if isinstance(override, str) and override else _athena_home
...
def _apply_managed(cfg: dict) -> dict:
    ...
    athena_cli.config.load_config, so without this a managed skin / reasoning_effort
...
def resolve_skin() -> dict:
    try:
        from athena_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_model() -> str:
    env = (
        os.environ.get("ATHENA_MODEL", "")
        or os.environ.get("ATHENA_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
...
def _config_model_target() -> tuple[str, str]:
    ...
    config.yaml wins over ATHENA_MODEL / ATHENA_INFERENCE_MODEL here, the
    reverse of `_resolve_model()`'s startup order. Those env vars are a
    provision-time seed (hosted instances set ATHENA_INFERENCE_MODEL in the
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    model = _resolve_model()
    explicit_provider = os.environ.get("ATHENA_TUI_PROVIDER", "").strip()
    ...
    try:
        ...
        current_provider = (
            ...
            or os.environ.get("ATHENA_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _load_enabled_toolsets() -> list[str] | None:
    explicit = [
        item.strip()
        for item in os.environ.get("ATHENA_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] ATHENA_TUI_TOOLSETS=all enables every toolset; "
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown ATHENA_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in ATHENA_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid ATHENA_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid ATHENA_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(
    ...
    *,
    ...
    #
    # We deliberately do NOT write process-global env vars (ATHENA_MODEL /
    # ATHENA_INFERENCE_MODEL / ATHENA_TUI_PROVIDER / ATHENA_INFERENCE_PROVIDER)
...
def _get_usage(agent) -> dict:
    ...
    # ATHENA_DEV_CREDITS so the payload stays clean when the flag is off.
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from athena_cli.config import load_config as _load_full_cfg
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("ATHENA_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses athena_cli.mcp_startup; TUI stdio keeps
    ...
    return AIAgent(
        ...
        skip_context_files=is_truthy_value(os.environ.get("ATHENA_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("ATHENA_IGNORE_RULES")),
...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind ATHENA_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # custom `ATHENA_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    _enable_gateway_prompts()
    home_token = (
        set_athena_home_override(str(profile_home)) if profile_home is not None else None
    )
    try:
        db.reopen_session(target)
        history = db.get_messages_as_conversation(target)
    ...
    with _session_resume_lock:
        ...
        try:
            init_home_token = (
                set_athena_home_override(str(profile_home))
                if profile_home is not None
                else None
            )
            try:
                ...
                if init_home_token is not None:
                    reset_athena_home_override(init_home_token)
            if sid in _sessions:
                ...
                # Remember the profile home so each turn re-binds ATHENA_HOME (the
...
# (agent/billing_view.py + athena_cli/nous_billing.py) — same as /credits.
...
def _(rid, params: dict) -> dict:
    ...
    """
    from athena_cli.nous_billing import BillingError, post_charge
...
def _(rid, params: dict) -> dict:
    ...
    """
    from athena_cli.nous_billing import BillingError, get_charge_status
...
def _(rid, params: dict) -> dict:
    ...
    """
    from athena_cli.nous_billing import BillingError, patch_auto_top_up
...
def _(rid, params: dict) -> dict:
    ...
    # (~/.athena/sessions/saved/) rather than the project/workspace CWD, and
...
#
# Layout:  $ATHENA_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _run_prompt_submit(rid, sid: str, session: dict, text: Any) -> None:
    ...
    def run():
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn ATHENA_HOME override for a resumed remote profile
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_athena_home_override(_profile_home_str)
            ...
            if images:
                try:
                    ...
                    from athena_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    ...
                    logger.warning("voice TTS skipped: athena_cli.voice unavailable")
                except Exception as e:
...
def _desktop_attachment_dir(session: dict) -> Path:
    root = Path(_session_cwd(session)).resolve() / ".athena" / "desktop-attachments"
    root.mkdir(parents=True, exist_ok=True)
    return root
    ...
    *,
    ...
    Three cases:
      ...
      2. The path resolves to a gateway-visible file OUTSIDE the workspace — copy
         it into ``.athena/desktop-attachments/`` so the ``@file:`` ref resolves.
      3. The path doesn't exist on the gateway (the common remote case: it's a
         ...
         write them into ``.athena/desktop-attachments/``.
    ...
    if key == "model":
        try:
            ...
            if session:
                ...
                from athena_cli.model_switch import parse_model_flags
    ...
    if key == "yolo":
        ...
        try:
            ...
            if session:
                ...
                current = is_truthy_value(os.environ.get("ATHENA_YOLO_MODE"))
                enable = _resolve_toggle(current)
                if enable:
...
def _(rid, params: dict) -> dict:
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_athena_home), "display": display_athena_home()})
    if key == "project":
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.athena/.env`` into the gateway process via
    ``athena_cli.config.reload_env``, matching classic CLI's ``/reload``
...
def _(rid, params: dict) -> dict:
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "athena_cli.main", *argv],
            capture_output=True,
            text=True,
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from athena_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from athena_cli.config import is_managed, save_env_value
        from athena_cli.inventory import build_models_payload, load_picker_context
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from athena_cli.config import remove_env_value
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action == "tts":
        ...
        os.environ["ATHENA_VOICE_TTS"] = "1" if new_value else "0"
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from athena_cli.browser_connect import manual_chrome_debug_command
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from athena_cli.browser_connect import try_launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("ATHENA_API_KEY", "") or cfg.get("api_key", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_athena_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "browse":
            from athena_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from athena_cli.skills_hub import inspect_skill
