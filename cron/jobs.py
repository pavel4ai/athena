...
Jobs are stored in ~/.athena/cron/jobs.json
Output is saved to ~/.athena/cron/output/{job_id}/{timestamp}.md
"""
...
# gateway process and the (separate) ``athena cron status`` process share it
...
# threshold in `athena cron status` (athena_cli/cron.py), so the two never
# drift apart.
...
def _jobs_lock():
    ...
    and standalone ``athena`` CLI invocations, which previously shared no lock
...
def _stored_wall_clock_is_future(stored: datetime, current: datetime) -> bool:
    ...
    Cron schedules express local wall-clock intent. If Athena/system local time
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    concurrent reader in another process (``athena cron status``) never sees a
...
def record_ticker_heartbeat(success: bool = False) -> None:
    ...
    `athena cron status` can tell a thread that is merely *alive and looping*
...
def _save_jobs_unlocked(jobs: List[Dict[str, Any]]):
    ...
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            json.dump({"jobs": jobs, "updated_at": _athena_now().isoformat()}, f, indent=2)
            f.flush()
            os.fsync(f.fileno())
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.athena/scripts/; ``.sh`` / ``.bash`` files run via bash,
...
def trigger_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            ...
            "paused_at": None,
            "paused_reason": None,
            "next_run_at": _athena_now().isoformat(),
        },
    )
...
def _machine_id() -> str:
    ...
    Uses ``ATHENA_MACHINE_ID`` if set, else hostname + pid. The CAS correctness
...
def save_job_output(job_id: str, output: str):
    ...
    timestamp = _athena_now().strftime("%Y-%m-%d_%H-%M-%S")
