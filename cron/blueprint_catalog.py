...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``athena://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``athena://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"athena://blueprint/{quote(blueprint.key)}{qs}"
