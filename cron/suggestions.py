...
Storage mirrors ``cron/jobs.py``: ``~/.athena/cron/suggestions.json``, atomic
...
CRON_DIR = get_athena_home().resolve() / "cron"
...
def _save_raw(suggestions: List[Dict[str, Any]]) -> None:
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as f:
            json.dump(
                {"suggestions": suggestions, "updated_at": _athena_now().isoformat()},
                f,
                indent=2,
            )
            f.flush()
