...
Uses a file-based lock (~/.athena/cron/.tick.lock) so only one tick
...
# the module) fail with ModuleNotFoundError for athena_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from athena_constants import get_athena_home
from athena_cli._subprocess_compat import windows_hide_flags
from athena_cli.config import load_config, _expand_env_vars
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy athena_cli import at cron module load (matches
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("ATHENA_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid ATHENA_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within ATHENA_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Athena-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against ATHENA_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_athena_home() / "scripts"
    ...
    # escape — scripts MUST reside within ATHENA_HOME/scripts/.
    try:
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    # gateway message. Do not seed ATHENA_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads ATHENA_SESSION_PLATFORM
    #     and ATHENA_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via ATHENA_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     ATHENA_SESSION_PLATFORM == "telegram".
    ...
    #     send_message gate read ATHENA_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the ATHENA_CRON_AUTO_DELIVER_* vars set
    # below, so clearing ATHENA_SESSION_* here does not affect delivery.
    ...
    try:
        ...
        try:
            load_dotenv(str(_get_athena_home() / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_get_athena_home() / ".env"), override=True, encoding="latin-1")
        ...
        # Model resolution precedence: per-job override > ATHENA_MODEL env >
        ...
        try:
            # Do not inject ATHENA_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # ATHENA_HOME. When a workdir is configured, also inject project
        ...
        # override via ATHENA_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("ATHENA_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid ATHENA_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
**Job ID:** {job_id}
**Run Time:** {_athena_now().strftime('%Y-%m-%d %H:%M:%S')}
...
def tick(verbose: bool = True, adapters=None, loop=None, sync: bool = True) -> int:
    ...
    try:
        ...
        # Set ATHENA_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
