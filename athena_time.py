...
``ATHENA_TIMEZONE`` env var, then ``timezone`` in ``~/.athena/config.yaml``, else server-local
...
# ATHENA_HOME, so one unkeyed global would leak the first profile's timezone into later
...
def _timezone_cache_identity() -> Tuple[str, str]:
    tz_env = os.getenv("ATHENA_TIMEZONE", "").strip()
