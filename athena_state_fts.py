...
from athena_state_common import FTS_CJK_STALE_KEY, FTS_STALE_KEY, _FTS_CJK_TRIGGERS, _FTS_TRIGGERS
from athena_state_errors import is_fts_scoped_corruption_error

# caplog tests pin the "athena_state" logger name.
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("ATHENA_FTS5_CJK_SO")
    return Path(env).expanduser() if env else get_athena_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("ATHENA_CJK_FTS", "1").strip().lower() not in ("0", "false", "off", "no")
...
class SessionFtsSetupMixin:
                ...
                if cjk_present:
                    ...
                    if live:
                        ...
                        logger.warning(
                            ...
                            "run `athena sessions optimize-storage` on a host "
    ...
    def _is_fts_write_corruption_error(exc: sqlite3.DatabaseError) -> bool:
        ...
        gateway transcript retry: see :func:`athena_state_errors.is_fts_scoped_corruption_error`."""
