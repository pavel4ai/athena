...
def get_env_value(name: str, default=None):
    ...
    Wraps :func:`athena_cli.config.get_env_value` so tests can patch
...
def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    ...
    to ``XAI_API_KEY`` resolved via ``athena_cli.config.get_env_value`` so keys
    stored in ``~/.athena/.env`` (the standard Athena location) are honored —
