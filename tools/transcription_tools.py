...
LOCAL_STT_COMMAND_ENV = "ATHENA_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "ATHENA_LOCAL_STT_LANGUAGE"
...
#
# The single-env-var ``ATHENA_LOCAL_STT_COMMAND`` escape hatch is preserved
...
def _render_command_stt_template(
    ...
    def replace_match(match: "re.Match[str]") -> str:
        ...
        token = f"__ATHENA_STT_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _transcribe_xai(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    language = str(
        ...
        or os.getenv("ATHENA_LOCAL_STT_LANGUAGE")
        or DEFAULT_LOCAL_STT_LANGUAGE
    ).strip()
