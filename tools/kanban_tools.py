...
running under the dispatcher (env var ``ATHENA_KANBAN_TASK`` set) or when
...
1. **Backend portability.** A worker whose terminal tool points at Docker
   ...
   always reach ``~/.athena/kanban.db`` regardless of terminal backend.
...
def _check_kanban_mode() -> bool:
    ...
    1. ``ATHENA_KANBAN_TASK`` is set (dispatcher-spawned worker), OR
...
def _worker_run_id(task_id: str) -> Optional[int]:
    ...
    if os.environ.get("ATHENA_KANBAN_TASK") != task_id:
        return None
    raw = os.environ.get("ATHENA_KANBAN_RUN_ID")
    if not raw:
        return None
    try:
        return int(raw)
...
def _enforce_worker_task_ownership(tid: str) -> Optional[str]:
    ...
    A process spawned by the dispatcher has ``ATHENA_KANBAN_TASK`` set
    ...
    ``ATHENA_KANBAN_TASK`` in env) aren't subject to this check — their
...
def _connect(board: Optional[str] = None):
    ...
    (``ATHENA_KANBAN_DB`` → ``ATHENA_KANBAN_BOARD`` env → current symlink
...
#   - No-op outside dispatcher-spawned worker context (no ``ATHENA_KANBAN_TASK``).
...
def heartbeat_current_worker_from_env() -> bool:
    ...
    Identity comes from:
      * ``ATHENA_KANBAN_TASK`` — task id (required; absence means no-op)
      * ``ATHENA_KANBAN_RUN_ID`` — pins the run row so we don't heartbeat
        a stale run that may have already been reclaimed
      * ``ATHENA_KANBAN_CLAIM_LOCK`` — claim lock for ``heartbeat_claim``;
    ...
    try:
        kb, conn = _connect()
        try:
            claim_lock = os.environ.get("ATHENA_KANBAN_CLAIM_LOCK")
            try:
                kb.heartbeat_claim(conn, tid, claimer=claim_lock)
            except Exception:
                logger.debug("auto-heartbeat: heartbeat_claim failed", exc_info=True)
            run_id_raw = os.environ.get("ATHENA_KANBAN_RUN_ID")
            run_id: Optional[int]
            try:
...
def _handle_show(args: dict, **kw) -> str:
    ...
    if not tid:
        return tool_error(
            "task_id is required (or set ATHENA_KANBAN_TASK in the env)"
        )
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            # ATHENA_KANBAN_CLAIM_LOCK in the worker env at spawn time
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    # an authoritative-looking name like ``athena-system`` and poison
...
def _handle_create(args: dict, **kw) -> str:
    ...
    tenant = args.get("tenant") or os.environ.get("ATHENA_TENANT")
    # Stamp the originating session id when the agent loop runs under
    # ACP (which sets ATHENA_SESSION_ID before invoking tools). NULL on
    # CLI / dashboard paths and on legacy hosts that don't set the env.
    session_id = args.get("session_id") or os.environ.get("ATHENA_SESSION_ID")
    priority = args.get("priority")
    # Resolve workspace. If the caller passed one explicitly, honor it.
    # Otherwise, a dispatcher-spawned worker (ATHENA_KANBAN_TASK set)
    ...
    # (kanban toolset, no ATHENA_KANBAN_TASK) and CLI/dashboard callers
    ...
    try:
        ...
        try:
            ...
            if _inherit_workspace:
                _self_tid = os.environ.get("ATHENA_KANBAN_TASK")
                if _self_tid:
            ...
            new_tid = kb.create_task(
                ...
                created_by=os.environ.get("ATHENA_PROFILE") or "worker",
...
def _maybe_auto_subscribe(conn: Any, task_id: str) -> bool:
    ...
    toggle (see ``athena dashboard``).

    Subscription paths:

    - **Gateway** (telegram/discord/slack/etc): ``ATHENA_SESSION_PLATFORM``
      and ``ATHENA_SESSION_CHAT_ID`` are set in ContextVars by the
    ...
    - **TUI** (herm desktop / herm TUI): the platform/chat_id ContextVars
      ...
      ``ATHENA_SESSION_KEY`` from the parent session. We subscribe with
    ...
    try:
        ...
        if not platform or not chat_id:
            ...
            # poller keys on ATHENA_SESSION_KEY.
            session_key = (
                get_session_env("ATHENA_SESSION_KEY", "")
                or os.environ.get("ATHENA_SESSION_KEY", "")
            )
            if not session_key:
        ...
        notifier_profile = os.environ.get("ATHENA_PROFILE")
...
_DESC_TASK_ID_DEFAULT = (
    "Task id. If omitted, defaults to ATHENA_KANBAN_TASK from the env "
    "(the task the dispatcher spawned you to work on)."
)

_DESC_BOARD = (
    "Kanban board slug to target. When omitted, the call resolves the "
    "active board the usual way: ATHENA_KANBAN_DB env → "
    "ATHENA_KANBAN_BOARD env → the 'current' symlink under the kanban "
        ...
        "properties": {
            ...
            "tenant": {
                ...
                "description": (
                    ...
                    "Defaults to ATHENA_TENANT env if set."
                ),
            },
            "priority": {
                "type": "integer",
