...
-----------------------------------------------------------------------------
    ...
    ~/.athena/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/athena/<hash16>        — per-project branch tip
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "athena@local"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``ATHENA_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at ATHENA_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "ATHENA_WORKDIR").write_text(
    ...
    * its ``last_touch`` is older than ``retention_days`` days.
    ...
    for child in base.iterdir():
        ...
        if delete_orphans:
            ...
            wd_marker = child / "ATHENA_WORKDIR"
            if wd_marker.exists():
                try:
