...
Configuration is read from ~/.athena/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.athena/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            athena_home = os.path.expanduser(
                os.getenv(
                    "ATHENA_HOME", os.path.join(os.path.expanduser("~"), ".athena")
                )
            )
            candidates = [
                os.path.join(athena_home, "node", "bin", resolved_command),
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Athena' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (ATHENA_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit ATHENA_SESSION_PLATFORM, so the
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Athena' approval system.
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"file under ~/.athena/mcp-tokens/ and restart). Do NOT retry "
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local ATHENA_HOME override into ``coro``.
    ...
    try:
        ...
        home_override = get_athena_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_athena_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local ATHENA_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_athena_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    ...
    ``os.environ`` (which includes ``~/.athena/.env`` loaded at startup).
    """
    try:
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple athena
