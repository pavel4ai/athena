...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``athena_constants.get_subprocess_home()`` so that ``~`` resolves
...
def _get_athena_config_resolved() -> str | None:
    ...
    global _athena_config_resolved, _athena_config_resolved_loaded
    if _athena_config_resolved_loaded:
        return _athena_config_resolved
    _athena_config_resolved_loaded = True
    try:
        from athena_cli.config import get_config_path
        _athena_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _athena_config_resolved = str(Path(_expand_tilde("~/.athena/config.yaml")).resolve())
        except Exception:
            _athena_config_resolved = None
    return _athena_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if athena_config and (resolved == athena_config or normalized == athena_config):
        return (
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.athena/…`` mirror created by a
    ...
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.athena/…`` path.
    ...
    # in a session that cd'd into ``~/.athena/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # and block credential stores under ATHENA_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == ATHENA_HOME (e.g. "auth.json") still hits the
