...
`health_report` (surfaced by `athena computer-use doctor`) reports the
...
def _capture_response(cap: CaptureResult, max_elements: int = _DEFAULT_MAX_ELEMENTS) -> Any:
    ...
    if cap.png_b64 and cap.mode != "ax" and not image_too_small:
        ...
        # parts (Surface 7 of pavel4ai/athena#47072 — trycua/cua#1961
...
def _route_capture_through_aux_vision(
    ...
    The captured base64 PNG is materialised to ``$ATHENA_HOME/cache/vision/``
    ...
    try:
        ...
        if _mime_for_ext == "image/jpeg" or (not _mime_for_ext and cap.png_b64[:8].startswith("/9j/")):
            ...
            ext = ".png"
        cache_dir = get_athena_dir("cache/vision", "temp_vision_images")
...
def check_computer_use_requirements() -> bool:
    ...
    `athena computer-use doctor` if their session is incomplete (e.g. no
    DISPLAY set).
    """
