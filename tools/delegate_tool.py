...
# Must match athena_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
# (athena-* prefixed), and scenario toolsets.
#
...
_SUBAGENT_TOOLSETS = sorted(
    ...
    and not name.startswith("athena-")
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``athena-cli`` (which bundles
...
def _build_child_agent(
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. athena-cli) so that individual
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.athena/logs/subagent-<sid>-<ts>.log``
...
def _load_config() -> dict:
    ...
    to the persistent config (athena_cli/config.py load_config()) so that
