...
def _get_token_dir() -> Path:
    ...
    Uses ATHENA_HOME so each profile gets its own OAuth tokens.
    Layout: ``ATHENA_HOME/mcp-tokens/``
    """
    try:
        from athena_constants import get_athena_home
        base = Path(get_athena_home())
    except ImportError:
        base = Path(os.environ.get("ATHENA_HOME", str(Path.home() / ".athena")))
...
class AthenaTokenStorage:
    ...
    File layout::
        ...
        ATHENA_HOME/mcp-tokens/<server_name>.json         -- tokens
        ATHENA_HOME/mcp-tokens/<server_name>.client.json   -- client info
        ATHENA_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://github.com/pavel4ai/athena/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`athena mcp login {server_name}` interactively first to complete "
