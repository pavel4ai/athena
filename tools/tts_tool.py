...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.athena/config.yaml``. Athena
...
Configuration is loaded from ~/.athena/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``athena_cli.config.get_env_value``
...
def _get_default_output_dir() -> str:
    ...
    return str(get_athena_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.athena/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.athena/config.yaml.
    ...
    try:
        ...
        logger.debug("athena_cli.config not available, using default TTS config")
        return {}
    except Exception as e:
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__ATHENA_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _get_piper_voices_dir() -> Path:
    ...
    Resolves to ``~/.athena/cache/piper-voices/`` under the active
    ATHENA_HOME so voice downloads follow profile boundaries.
    """
    from athena_constants import get_athena_dir
    root = Path(get_athena_dir("cache/piper-voices", "piper_voices_cache"))
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.athena/config.yaml (tts: section).
    ...
    platform = get_session_env("ATHENA_SESSION_PLATFORM", "").lower()
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_athena_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
