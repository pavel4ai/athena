...
def _default_output_dir() -> str:
    ...
    ATHENA_HOME and later scope requests to a different profile via
    ``athena_constants.set_athena_home_override()`` — a frozen module constant keeps writing synthesized
    ...
    re-resolve from the live profile-scoped ATHENA_HOME on every call.
    """
...
_BUILTIN_DISPATCH: Dict[str, tuple] = {
    ...
    "mistral": (lambda: _importable(_import_mistral_client), "Mistral Voxtral TTS", "_generate_mistral_tts",
                ...
                "Run `athena setup` to install Mistral support."),
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'athena_xai_user_agent': ('tools.xai_http', 'athena_xai_user_agent'),
