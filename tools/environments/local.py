...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_ATHENA_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if key not in _ATHENA_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_ATHENA_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_ATHENA_PROVIDER_ENV_FORCE_PREFIX):]
            sanitized[real_key] = value
        elif key not in _ATHENA_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    _inject_context_athena_home(sanitized)
...
def _find_bash() -> str:
    ...
    #   PortableGit: %LOCALAPPDATA%\athena\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\athena\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _athena_portable_git = os.path.join(_local_appdata, "athena", "git") if _local_appdata else ""
    if _athena_portable_git:
        for candidate in (
            os.path.join(_athena_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_athena_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
...
def _resolve_athena_bin_dir() -> str | None:
    """Return the directory holding the ``athena`` console-script, or None.
    ...
    athena install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``athena``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``athena`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``athena`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("athena")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``athena`` executable (covers nix-store / venv wrappers).
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "athena" or base.startswith("athena."))
    ...
    _ATHENA_BIN_DIR = candidate
    return candidate


def _prepend_athena_bin_dir(existing_path: str) -> str:
    """Prepend the athena install dir to ``existing_path`` if it's missing.
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_ATHENA_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_ATHENA_PROVIDER_ENV_FORCE_PREFIX):]
            run_env[real_key] = v
        elif k not in _ATHENA_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    path_key = _path_env_key(run_env)
    if path_key is not None:
        new_path = _append_missing_sane_path_entries(run_env.get(path_key, ""))
        # Ensure the athena install dir is reachable so plugins can shell out
        # to bare ``athena`` via the terminal tool even when the gateway was
        # launched without it on PATH (systemd, service managers, cron, etc.).
        run_env[path_key] = _prepend_athena_bin_dir(new_path)

    _inject_context_athena_home(run_env)
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``ATHENA_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under ATHENA_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_athena_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "athena_terminal"
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_athena_pgid", None)
                    if pgid is None:
                        raise
