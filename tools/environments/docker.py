...
from tools.environments.local import _ATHENA_PROVIDER_ENV_BLOCKLIST
...
def _load_athena_env_vars() -> dict[str, str]:
    """Load ~/.athena/.env values without failing Docker command execution."""
    try:
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale athena-tagged containers left behind by prior processes.
    ...
    * (optional) ``label=athena-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a athena process in profile A must not
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``ATHENA_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.athena/sandboxes/). Non-persistent
        ...
        # Labels make athena-created containers identifiable to:
        #   * the orphan reaper (`athena-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`athena-task-id`, `athena-profile`)
        ...
        label_args = [
            ...
            "--label", f"athena-task-id={task_label}",
            "--label", f"athena-profile={profile_name}",
        ]
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _ATHENA_PROVIDER_ENV_BLOCKLIST)
        athena_env = _load_athena_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if not value:
                value = athena_env.get(key)
            if value:
                exec_env[key] = value
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("athena-task-id", "")
        profile_label = self._labels.get("athena-profile", "")
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"athena-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``athena-*`` but were
