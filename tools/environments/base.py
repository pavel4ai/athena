...
# ATHENA_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {ATHENA_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_athena_home() / "sandboxes"
...
class BaseEnvironment(ABC):
    ...
    def init_session(self):
        ...
        # caused ``C:/Users/.../athena-snap-*.sh: No such file or directory``
    ...
    def _wait_for_process(self, proc: ProcessHandle, timeout: int = 120) -> dict:
        ...
        # --- Debug tracing (opt-in via ATHENA_DEBUG_INTERRUPT=1) -------------
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __ATHENA_CWD_{session}__ marker from stdout output.
