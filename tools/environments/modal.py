...
_SNAPSHOT_STORE = get_athena_home() / "modal_snapshots.json"
...
class ModalEnvironment(BaseEnvironment):
    ...
    def _modal_bulk_download(self, dest: Path) -> None:
        """Download remote .athena/ as a tar archive (sandboxes run as root, so /root/.athena)."""
        data = self._exec("tar cf - -C / root/.athena", timeout=120, fail_label="bulk download", capture=True)
