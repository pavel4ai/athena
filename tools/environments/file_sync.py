...
def iter_sync_files(container_base: str = "/root/.athena") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.athena to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``ATHENA_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, athena_home: Path | None = None) -> None:
        ...
        Downloads the remote ``.athena/`` directory as a tar archive,
        ...
        # against an uninitialized remote .athena/ directory.
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.athena/skills/a.md`` →
        ``~/.athena/skills/a.md``, a new remote file at
        ``/root/.athena/skills/b.md`` maps to ``~/.athena/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
