...
it is automatically downloaded from GitHub releases to $ATHENA_HOME/bin/tirith.
...
def _get_athena_home() -> str:
    """Return the Athena home directory, respecting ATHENA_HOME env var."""
    return str(get_athena_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_athena_home(), ".tirith-install-failed")
...
def _athena_bin_dir() -> str:
    """Return $ATHENA_HOME/bin, creating it if needed."""
    d = os.path.join(_get_athena_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ATHENA_HOME/bin/tirith.
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ATHENA_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ATHENA_HOME/bin/tirith
    ...
    if os.path.isfile(athena_bin) and os.access(athena_bin, os.X_OK):
        _resolved_path = athena_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return athena_bin
