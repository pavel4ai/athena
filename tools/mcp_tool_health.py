...
class MCPServerHealthMixin:
    ...
    def _make_logging_callback(self):
        """``logging_callback`` forwarding server ``notifications/message`` into Athena logging (SDK default drops them).

        Routes MCP ``notifications/message`` log notifications from the server into Athena' logging
        (agent.log via athena_logging), tagged with the server name. Without this, the SDK's default
