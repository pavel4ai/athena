...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            athena_meta = metadata.get("athena", {})
            if isinstance(athena_meta, dict):
                tags = athena_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Futurebound Corp.) but not activated
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                athena_meta = meta_block.get("athena", {})
                if isinstance(athena_meta, dict):
                    tags = athena_meta.get("tags", [])
...
ATHENA_INDEX_URL = "https://github.com/pavel4ai/athena/docs/api/skills-index.json"
ATHENA_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "athena-index.json"
ATHENA_INDEX_TTL = 6 * 3600  # 6 hours


def _load_athena_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for ATHENA_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if ATHENA_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - ATHENA_INDEX_CACHE_FILE.stat().st_mtime
            if age < ATHENA_INDEX_TTL:
                return json.loads(ATHENA_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(ATHENA_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
    ...
    # Cache locally
    try:
        ATHENA_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        ATHENA_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class AthenaIndexSource(SkillSource):
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_athena_index()
            self._loaded = True
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "athena-index"),
            identifier=entry.get("identifier", ""),
