...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    athena_node_bin = str(athena_home / "node" / "bin")
    athena_node_root = str(athena_home / "node")
    athena_nm_bin = str(athena_home / "node_modules" / ".bin")
    return [athena_node_bin, athena_node_root, athena_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
# plugins drop in under ``~/.athena/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.athena/plugins/browser/<vendor>/``) participate
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-athena_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed athena processes — this way every clean athena exit sweeps
    ...
    # Sweep orphans from other crashed athena processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live athena processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current athena PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live athena process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning athena process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced athena PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent athena instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-athena_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live athena process.
            continue
...
def _find_agent_browser() -> str:
    ...
    try:
        ...
        if ensure_dependency("browser"):
            ...
            if not recheck:
                athena_nm = str(get_athena_home() / "node_modules" / ".bin")
                recheck = shutil.which("agent-browser", path=athena_nm)
            if not recheck:
                athena_node_bin = str(get_athena_home() / "node" / "bin")
                recheck = shutil.which("agent-browser", path=athena_node_bin)
            if not recheck:
                athena_node_root = str(get_athena_home() / "node")
                recheck = shutil.which("agent-browser", path=athena_node_root)
            if recheck:
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this athena PID as the session owner (cross-process safe
