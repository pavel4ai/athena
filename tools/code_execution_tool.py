...
Architecture (two transports):
  ...
  **Local backend (UDS):**
  1. Parent generates a `athena_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `athena_tools.py` with file-based RPC stubs
...
# match a safe prefix, the operational ATHENA_ allowlist, or (on Windows) an
# OS-essential name.
#
# NB: the broad "ATHENA_" prefix was deliberately removed (#27303) — it leaked
# ATHENA_*-named config that lacks a secret substring (e.g. ATHENA_BASE_URL,
# ATHENA_KANBAN_DB, ATHENA_*_WEBHOOK).  The child only needs the few
# location/profile vars in _ATHENA_CHILD_ALLOWED below; ATHENA_RPC_SOCKET /
# ATHENA_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational ATHENA_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set athena_cli treats as the
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational ATHENA_* vars (_ATHENA_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret ATHENA_* vars dropped by the tightened allowlist (#27303). The
    # broad "ATHENA_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like ATHENA_KANBAN_DB / ATHENA_BASE_URL), but a sandbox script
...
def generate_athena_tools_module(enabled_tools: List[str],
    ...
    """
    Build the source code for the athena_tools.py stub module.
...
def _execute_remote(
    ...
    The script and the generated athena_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/athena_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_athena_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/athena_tools.py", tools_src)
        ...
        env_prefix = (
            f"ATHENA_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
...
def execute_code(
    ...
    # --- Set up temp directory with athena_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="athena_sandbox_")
    ...
    # selector from ATHENA_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"athena_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated athena_tools module.
        ...
        tools_src = generate_athena_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "athena_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  ATHENA_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        # the staging tmpdir so ``from athena_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _athena_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _athena_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # ATHENA_TIMEZONE is an internal Athena setting and must not leak
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.athena/.env')).
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    description = (
        ...
        "Also available (no import needed — built into athena_tools):\n"
