...
# All skills live in ~/.athena/skills/ (seeded from bundled skills/ on install).
...
def _capture_required_environment_variables(
    ...
    # — the desktop app / TUI — set ATHENA_INTERACTIVE and register a
    # secret-capture callback that routes to a secure secret.request overlay, so
    # they fall through and actually prompt. (ATHENA_INTERACTIVE is the same flag
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_athena_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.athena/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``ATHENA_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``ATHENA_PLATFORM`` environment variable
    3. ``ATHENA_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from athena_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("ATHENA_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.athena/skills/ and external dirs.
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": f"No skills found. Skills directory created at {display_athena_home()}/skills/",
                },
                ensure_ascii=False,
            )
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: athena plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.athena/skills/): {skill_md}")
        ...
        # Check metadata.athena.* first (agentskills.io convention), fall back to top-level
        athena_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            athena_meta = metadata.get("athena", {}) or {}

        tags = _parse_tags(athena_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            athena_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
