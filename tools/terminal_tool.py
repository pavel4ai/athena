...
# display_athena_home imported lazily at call site (stale-module safety during athena update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of athena directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "athena-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = os.getenv("ATHENA_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("ATHENA_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (ATHENA_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    (ATHENA_INTERACTIVE=1 or a registered sudo password callback):
    ...
    should_prompt_for_sudo = (
        env_var_enabled("ATHENA_INTERACTIVE") or has_sudo_prompt_callback
    )
...
def _get_env_config() -> Dict[str, Any]:
    ...
    return {
        ...
        # Startup orphan reaper for athena-tagged containers left behind by
...
def terminal_tool(
    ...
    try:
        ...
        # Hard-block: gateway lifecycle commands (systemctl/launchctl/athena
        # restart|stop targeting athena-gateway) must never run inside the
        ...
        # never restart. This mirrors the `athena gateway restart` guard in
        # athena_cli/gateway.py and the cron-path guard in athena_cli/cron.py,
        # but applies unconditionally (force=True cannot help here).
        if os.environ.get("_ATHENA_GATEWAY") == "1":
            from athena_cli.cron import _contains_gateway_lifecycle_command
            if _contains_gateway_lifecycle_command(command):
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "Run `athena gateway restart` from a separate shell outside "
                        "the running gateway."
                    ),
                    "status": "error",
                }, ensure_ascii=False)
            ...
            try:
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    if not _async_ok():
                        ...
                        _gw_platform = _gse("ATHENA_SESSION_PLATFORM", "")
                        if _gw_platform:
                            _gw_chat_id = _gse("ATHENA_SESSION_CHAT_ID", "")
                            _gw_thread_id = _gse("ATHENA_SESSION_THREAD_ID", "")
                            _gw_user_id = _gse("ATHENA_SESSION_USER_ID", "")
                            _gw_user_name = _gse("ATHENA_SESSION_USER_NAME", "")
                            _gw_message_id = _gse("ATHENA_SESSION_MESSAGE_ID", "")
