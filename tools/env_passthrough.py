...
def _is_athena_provider_credential(name: str) -> bool:
    ...
    token, or similar) per ``_ATHENA_PROVIDER_ENV_BLOCKLIST``.
    ...
    try:
        ...
        return False
    return name in _ATHENA_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    ``_ATHENA_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
