...
def _dump_subagent_timeout_diagnostic(
    *, child: Any, task_index: int, timeout_seconds: float, duration_seconds: float,
    ...
    response", 0 API calls, nothing to inspect): ``~/.athena/logs/subagent-timeout-<sid>-<ts>.log`` with the
    child's config, prompt/schema sizes, activity snapshot and worker stack. Path, or None on failure."""
    try:
        from athena_constants import get_athena_home
        import datetime as _dt
        logs_dir = get_athena_home() / "logs"
        try:
...
def _register_child(
    ...
    if owner_session_id is None:
        with _quiet(None):
            ...
            owner_session_id = get_session_env("ATHENA_UI_SESSION_ID", "") or None
