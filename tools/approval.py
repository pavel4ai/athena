...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("ATHENA_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("ATHENA_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set ATHENA_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind ATHENA_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds ATHENA_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("ATHENA_CRON_SESSION"):
        return False
...
# via shell expansions like $HOME or $ATHENA_HOME, or by the resolved absolute
# active profile home path such as /home/athena/.athena/config.yaml. The
# resolved-absolute form is folded into the ~/.athena/ patterns at detection
...
# go stale when ATHENA_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_ATHENA_ENV_PATH = (
    r'(?:~\/\.athena/|'
    r'(?:\$home|\$\{home\})/\.athena/|'
    r'(?:\$athena_home|\$\{athena_home\})/)'
    r'\.env\b'
)
# ~/.athena/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _ATHENA_ENV_PATH; matches the ATHENA_HOME override form as
# well as ~/.athena/.
_ATHENA_CONFIG_PATH = (
    r'(?:~\/\.athena/|'
    r'(?:\$home|\$\{home\})/\.athena/|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bathena\s+gateway\s+(stop|restart)\b', "stop/restart athena gateway (kills running agents)"),
    ...
    # The name-based pattern above catches `pkill athena` but not
    # `kill -9 $(pgrep -f athena)` because the substitution is opaque
    ...
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Athena file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.athena/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Athena-managed security file (~/.athena/config.yaml or
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/.athena/ form so the Athena config/env patterns catch it. In Docker and
    # gateway deployments the agent often references the resolved absolute path
    # directly (e.g. `sed -i ... /home/athena/.athena/config.yaml`) rather than
    # ~, $HOME, or $ATHENA_HOME. Done at detection time (not via an import-time
    # pattern snapshot) so it tracks the live ATHENA_HOME even when that is set
...
def _rewrite_resolved_athena_home(command: str) -> str:
    """Rewrite the resolved absolute Athena home prefix to ``~/.athena/``.

    Resolves the active ``ATHENA_HOME`` at call time (and its symlink-resolved
    form) and replaces an occurrence of ``<home>/`` in *command* with
    ``~/.athena/`` so the static ``_ATHENA_CONFIG_PATH`` / ``_ATHENA_ENV_PATH``
    ...
    for path in candidates:
        if not path or path in seen:
            continue
        seen.add(path)
        # Guard against a degenerate ATHENA_HOME (e.g. "/" or "") rewriting
        ...
        # /home/athena/.athena or a per-test tempdir, never a bare root.
        normalized = path.rstrip("/")
        if not normalized.startswith("/") or normalized.count("/") < 2:
            continue
        command = command.replace(normalized + "/", "~/.athena/")
    return command
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    try:
        ...
        if "ATHENA_SPINNER_PAUSE" in os.environ:
            del os.environ["ATHENA_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("ATHENA_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set ATHENA_INTERACTIVE or ATHENA_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("ATHENA_EXEC_ASK"):
