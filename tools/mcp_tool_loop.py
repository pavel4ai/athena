...
def _try_acquire_mcp_discovery_lock() -> Any:
    ...
    try:
        ...
        if _origin._MCP_DISCOVERY_LOCK_PATH is None:
            _origin._MCP_DISCOVERY_LOCK_PATH = str(get_athena_home() / ".mcp-discovery.lock")
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local ATHENA_HOME override into ``coro`` (task-local on the MCP
    loop, so concurrent scopes don't interfere)."""
    try:
        from athena_constants import get_athena_home_override, reset_athena_home_override, set_athena_home_override
