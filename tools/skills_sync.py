...
Copies bundled skills from the repo's skills/ directory into ~/.athena/skills/
...
The manifest lives at ~/.athena/skills/.bundled_manifest.
"""
...
# and by the installer's `--no-skills` flag (the default ~/.athena profile).
# When present in ATHENA_HOME, sync_skills() is a no-op so neither the
# installer, `athena update`, nor a direct sync re-injects bundled skills.
# Delete the file to opt back in. Mirrors
# athena_cli.profiles.NO_BUNDLED_SKILLS_MARKER (kept as a literal here to
...
def _get_bundled_dir() -> Path:
    ...
    Checks ATHENA_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.athena/skills/.curator_suppressed`` directly if that import
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.athena/skills/mlops/axolotl
    """
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.athena/skills/ using the manifest.

    Returns:
        dict with keys: copied (list), updated (list), skipped (int),
                        user_modified (list), cleaned (list), total_bundled (int)
    """
    # Opt-out: a profile (named or the default ~/.athena) that wrote the
    ...
    if (ATHENA_HOME / NO_BUNDLED_SKILLS_MARKER).exists():
        if not quiet:
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.athena/skills/.curator_suppressed) is written when the curator
...
def _rmtree_writable(path: Path) -> None:
    ...
    # ``ATHENA_HOME/skills/`` to prevent the catastrophic wipe of
    # ``~/.athena/`` (``.env``, ``MEMORY.md``, ``kanban.db``, custom
    ...
    # ``ATHENA_HOME``. Require a strict-child relationship so both escape
...
def _is_tracked_user_modification(origin_hash: str, user_hash: str) -> bool:
    """Whether an on-disk skill counts as a user modification ``athena update`` keeps.
...
def list_user_modified_bundled_skills() -> List[dict]:
    """Return the bundled skills that ``athena update`` keeps because the user
    edited them locally.
...
def diff_bundled_skill(name: str) -> dict:
    ...
    edits or ``athena skills reset`` back to upstream.
    ...
    if bundled_src is None:
        return {
            ...
            "message": (
                ...
                f"diff against). Hub-installed skills use `athena skills inspect`."
            ),
        }
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    When ``enabled`` is True, writes ATHENA_HOME/.no-bundled-skills so the
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = ATHENA_HOME / NO_BUNDLED_SKILLS_MARKER
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (ATHENA_HOME / NO_BUNDLED_SKILLS_MARKER).exists()
