...
# Athena WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: ATHENA_TINI_SHIM_TARGET overrides the `/init` path (and
# ATHENA_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${ATHENA_TINI_SHIM_TARGET:-/init}"
WRAPPER="${ATHENA_TINI_SHIM_WRAPPER:-/opt/athena/docker/main-wrapper.sh}"
