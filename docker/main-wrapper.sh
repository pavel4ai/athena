...
# with-contenv before touching ATHENA_HOME / PATH. On the non-PID-1
...
if [ -z "${ATHENA_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${ATHENA_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export ATHENA_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset ATHENA_MAIN_WRAPPER_ENV_READY
