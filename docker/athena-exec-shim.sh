#!/bin/sh
# shellcheck shell=sh
# /opt/athena/bin/athena — `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `athena` user (UID 10000). When an operator runs `docker exec <c> athena ...`
# the default UID is root (0), and any file the command writes under
# $ATHENA_HOME — auth.json, .env, config.yaml — ends up root-owned and
# unreadable to the supervised gateway. The most common manifestation: the
# user runs `docker exec <c> athena login`, this writes
# /opt/data/auth.json as root:root mode 0600, and from then on the gateway
# returns "Provider authentication failed: Athena is not logged into Nous
# Portal" on every incoming message — even though `docker exec <c> athena
...
# Fix
# ---
# This shim sits at /opt/athena/bin/athena and is placed earliest on PATH.
# When invoked as root, it drops to the athena user (via s6-setuidgid)
# before exec'ing the real venv binary, so anything that writes under
# $ATHENA_HOME is uid-aligned with the supervised processes. When invoked
# as any non-root UID — including the supervised processes themselves,
# `docker exec --user athena`, kanban subagents, etc. — it short-circuits
...
# (/opt/athena/.venv/bin/athena), so the second hop cannot re-enter this
# shim regardless of PATH state. No sentinel env var needed.
#
# Opt-out: set ATHENA_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
# state via the athena CLI. Default is to drop.

set -e

REAL=/opt/athena/.venv/bin/athena

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "athena-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user athena`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi

# Root, with opt-out set? Honor it.
case "${ATHENA_DOCKER_EXEC_AS_ROOT:-}" in
    1|true|TRUE|True|yes|YES|Yes)
        exec "$REAL" "$@"
        ;;
esac

# Root, no opt-out. Drop to the athena user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "athena-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "athena-shim: re-run with --user athena or set ATHENA_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the athena user's home before dropping privileges. Without
