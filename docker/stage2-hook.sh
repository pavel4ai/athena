...
chown_athena_tree() {
    ...
    chown -R athena:athena "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_athena_owner() {
    target="$1"
    find "$target" \( ! -user athena -o ! -group athena \) -print -quit 2>/dev/null | grep -q .
}
...
if [ "$needs_chown" = true ]; then
    ...
    if refuse_symlinked_path "chown" "$ATHENA_HOME"; then
        :
    else
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$ATHENA_HOME/$sub" ] && tree_has_non_athena_owner "$ATHENA_HOME/$sub"; then
            chown_athena_tree "$ATHENA_HOME/$sub"
        fi
    done
fi
...
# $ATHENA_HOME/lazy-packages dir on the data volume (Dockerfile sets
# ATHENA_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to athena
...
if [ -d "$ATHENA_HOME/profiles" ] && tree_has_non_athena_owner "$ATHENA_HOME/profiles"; then
    chown_athena_tree "$ATHENA_HOME/profiles"
fi
...
# gate as profiles/).
if [ -d "$ATHENA_HOME/cron" ] && tree_has_non_athena_owner "$ATHENA_HOME/cron"; then
    chown_athena_tree "$ATHENA_HOME/cron"
fi

# Always ensure logs/gateways is athena-owned (#45258). Formerly healed by
...
# top-level $ATHENA_HOME is mis-owned, so a warm volume with athena-owned
# ATHENA_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid athena mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as athena.
if [ -d "$ATHENA_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$ATHENA_HOME/logs/gateways"; then
        :
    else
        chown athena:athena "$ATHENA_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# athena pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged athena gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $ATHENA_HOME is
...
# and cron/.
if [ -d "$ATHENA_HOME/platforms/pairing" ] && tree_has_non_athena_owner "$ATHENA_HOME/platforms/pairing"; then
    chown_athena_tree "$ATHENA_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$ATHENA_HOME/pairing" ] && tree_has_non_athena_owner "$ATHENA_HOME/pairing"; then
    chown_athena_tree "$ATHENA_HOME/pairing"
fi
...
for f in \
    ...
    if [ -e "$ATHENA_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$ATHENA_HOME/$f"; then
            :
        else
...
if [ -f "$ATHENA_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$ATHENA_HOME/config.yaml"; then
        :
    else
...
seed_one() {
    ...
    if [ ! -f "$ATHENA_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$ATHENA_HOME/$dest"; then
            :
        else
...
# the first-boot seed above depends on /opt/athena/.env.example being present
...
# not generate one. Athena loads $ATHENA_HOME/.env with override=True, so
...
if [ -n "${API_SERVER_KEY:-}" ]; then
    if [ -f "$ATHENA_HOME/.env" ] && grep -q '^API_SERVER_KEY=..*' "$ATHENA_HOME/.env" 2>/dev/null; then
        echo "[stage2] Warning: API_SERVER_KEY is set in both the container environment and $ATHENA_HOME/.env — the .env value wins at runtime (loaded with override=True)"
    else
        ...
        if [ -f "$ATHENA_HOME/.env" ] && ! refuse_symlinked_path "clean" "$ATHENA_HOME/.env"; then
            sed -i '/^API_SERVER_KEY=$/d' "$ATHENA_HOME/.env" 2>/dev/null || true
        fi
        echo "[stage2] API_SERVER_KEY provided via container environment — skipping generation"
    fi
elif ! grep -q '^API_SERVER_KEY=..*' "$ATHENA_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$ATHENA_HOME/.env"; then
        :
    else
        if [ ! -f "$ATHENA_HOME/.env" ]; then
            ...
            (umask 077 && as_athena touch "$ATHENA_HOME/.env") 2>/dev/null || true
        fi
        if [ -f "$ATHENA_HOME/.env" ]; then
            ...
            if [ -n "$_gen_key" ]; then
                ...
                sed -i '/^API_SERVER_KEY=$/d' "$ATHENA_HOME/.env" 2>/dev/null || true
                if printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$ATHENA_HOME/.env" 2>/dev/null; then
                    echo "[stage2] Generated API_SERVER_KEY for the loopback gateway api_server"
                else
                    echo "[stage2] Warning: could not write API_SERVER_KEY to $ATHENA_HOME/.env (read-only volume?) — gateway api_server (cron fires) will be unavailable"
                fi
            fi
            unset _gen_key
        else
            echo "[stage2] Warning: could not create $ATHENA_HOME/.env — gateway api_server (cron fires) will be unavailable"
        fi
    fi
fi
...
if [ -f "$ATHENA_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$ATHENA_HOME/.env"; then
        :
    else
...
# as the unprivileged `athena` user — without this it gets EACCES on every
...
# athena group (any group member could e.g. stop/suspend this machine), not
...
if [ -S /.fly/api ]; then
    if refuse_symlinked_path "chgrp/chmod" /.fly/api; then
        :
    elif chgrp athena /.fly/api 2>/dev/null && chmod g+w /.fly/api 2>/dev/null; then
        echo "[stage2] Granted athena group access to the Fly Machines API socket"
    else
...
if [ ! -f "$ATHENA_HOME/auth.json" ] && [ -n "${ATHENA_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$ATHENA_HOME/auth.json"; then
        :
    else
...
# supply a freshly-issued session via ATHENA_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$ATHENA_HOME/auth.json" ] && [ -n "${ATHENA_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$ATHENA_HOME/auth.json"; then
        :
    else
        s6-setuidgid athena "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$ATHENA_HOME/auth.json" \
...
if [ ! -f "$ATHENA_HOME/gateway_state.json" ] && \
    ...
    if refuse_symlinked_path "seed" "$ATHENA_HOME/gateway_state.json"; then
        :
    else
