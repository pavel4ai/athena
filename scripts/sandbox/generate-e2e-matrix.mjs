 ...
 *   The artifact published on the website right now -- Athena-Setup.exe has
 ...
 * @typedef {InstallMethod | 'athena-update' | 'open-app-update' | 'athena-desktop-app-update'} UpdateMethod
 ...
 *   desktop-installer install; athena-desktop-app-update starts the app
 *   via `athena desktop`, which every install method provides on every
...
export const SPEC = {
  windows: {
    install: [
      // irm https://athena.nousresearch.com/install.ps1 | iex
      { method: 'installer-script' },
      // The same one-liner with -IncludeDesktop: builds Athena.exe AND
      ...
      // Website Athena-Setup.exe, clicked through the GUI.
    ...
    update: [
      ...
      // Same button, app launched via `athena desktop`.
      { method: 'athena-desktop-app-update' },
    ],
  },
  macos: {
    install: [
      { method: 'installer-script' },
      { method: 'installer-script+desktop' },
      // The published Athena-Setup.dmg from the website, mounted and run.
      { method: 'desktop-installer', versions: ['latest'] },
    ],
    update: [
      { method: 'installer-script' },
  ...
  linux: {
    ...
    update: [
      ...
      // linux, so there is no open-app-update; `athena desktop` is always
...
export function methodNeedsDesktop(m) {
  return m.startsWith('desktop-installer') || m === 'installer-script+desktop' ||
    m === 'open-app-update' || m === 'athena-desktop-app-update';
}
