...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  Athena home, same model, same tasks, same reps.
...
1. Create a dedicated Athena home with credentials for the models under test:
   ...
   integration at it; no Athena observability plugin needs to be enabled.
