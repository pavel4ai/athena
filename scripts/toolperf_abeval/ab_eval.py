...
measured in the production session DB) through `athena chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    ATHENA_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Athena home with credentials for the
...
""".strip(),
            ...
            env.update({
                ...
                "ATHENA_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "athena_cli.main", "chat", "--query", q,
