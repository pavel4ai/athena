...
# Force-isolate the test environment BEFORE any athena imports.
ORIGINAL_HOME = os.environ.get("ATHENA_HOME")
ORIGINAL_AUTH = Path.home() / ".athena" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.athena/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.athena/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="athena_ts_live_"))
    athena_home = home_dir / ".athena"
    athena_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, athena_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".athena" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, athena_home / ".env")
        ...
        load_athena_dotenv(athena_home=str(Path.home() / ".athena"))
    ...
    (athena_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return athena_home
...
def reset_module_state():
    """Drop cached modules so the new ATHENA_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "athena_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
