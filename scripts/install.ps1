...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$AthenaHome = $(if ($env:ATHENA_HOME) { $env:ATHENA_HOME } else { "$env:LOCALAPPDATA\athena" }),
    [string]$InstallDir = $(if ($env:ATHENA_HOME) { "$env:ATHENA_HOME\athena-agent" } else { "$env:LOCALAPPDATA\athena\athena-agent" }),
    ...
    # builds apps/desktop into a launchable Athena.exe.
    #
    # Why opt-in:
    #   * Athena-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Athena.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Athena.exe
    ...
    #     `athena desktop` already builds on demand.
...
# ATHENA_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[athena] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'AthenaInstall.LongPath').Type) {
            Add-Type -Namespace 'AthenaInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [AthenaInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -AthenaHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('AthenaHome')) {
    $AthenaHome = ConvertTo-LongPath $AthenaHome
} else {
    $AthenaHome = ConvertTo-LongPath $(
        if ($env:ATHENA_HOME) { $env:ATHENA_HOME } else { "$env:LOCALAPPDATA\athena" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:ATHENA_HOME) { "$env:ATHENA_HOME\athena-agent" } else { "$env:LOCALAPPDATA\athena\athena-agent" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: AthenaHome=$AthenaHome InstallDir=$InstallDir"
}
...
$RepoUrlSsh = "git@github.com:pavel4ai/athena.git"
$RepoUrlHttps = "https://github.com/pavel4ai/athena.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  Improved by Futurebound Corp.                          |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $AthenaHome)) {
        New-Item -ItemType Directory -Force -Path $AthenaHome | Out-Null
    }
    $envFile = Join-Path $AthenaHome ".env"
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `athena update`'s npx
    ...
    $prefixDir = Join-Path $AthenaHome "node"
...
function Install-Uv {
    # Athena owns its own uv at $AthenaHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (athena_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `athena update` stay in sync.
    $managedUv = Join-Path $AthenaHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $AthenaHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $AthenaHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $AthenaHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $AthenaHome "bin"
        ...
        # (athena_cli/managed_uv.py looks only at $AthenaHome\bin\uv.exe).
...
# Put the Athena-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone athena-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Athena-managed Node tree's bundled npm into $NpmRange when
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Athena-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("athena-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and ATHENA_HOME wasn't propagated).
...
function Initialize-ManagedPythonEnvironment {
    # Python used by Athena belongs to the checkout, never to another
    # application or a user-level uv configuration. Keep this aligned with
    # athena_cli.managed_uv.managed_python_env(), which owns the update path.
    foreach ($name in @(
    ...
    $managedRoot = Join-Path $InstallDir ".athena-runtime\python"
...
function Resolve-AvailablePythonVersion {
    # Return the path and minor version of the first Athena-managed interpreter
    ...
    #
    # Under Athena-Setup.exe each stage runs in a fresh powershell.exe. The
    ...
    foreach ($ver in $candidates) {
        ...
        try {
            ...
            throw "Failed to resolve Athena-managed Python $ver`: $_"
        } finally {
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Athena setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Athena uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\athena\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Athena needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Athena copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\athena\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``ATHENA_GIT_BASH_PATH`` (User scope) so Athena can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Athena-managed PortableGit install instead..."
    }

    # Download PortableGit into $AthenaHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $AthenaHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Athena features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Athena needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``ATHENA_GIT_BASH_PATH`` (User env scope) so Athena can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $AthenaHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $AthenaHome\git\usr\bin\bash.exe
    $candidates += "$AthenaHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$AthenaHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("ATHENA_GIT_BASH_PATH", $candidate, "User")
            $env:ATHENA_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set ATHENA_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Athena may not find Git Bash."
    Write-Info "If needed, set ATHENA_GIT_BASH_PATH manually to your bash.exe path."
}
...
# range used to provision the Athena-managed tree. Keeping this probe separate
...
function Test-SystemNodeReady {
    ...
    if (Test-NodeVersionOk $version) {
        ...
        Write-Warn "Node.js $version is unsupported (Athena requires Node 22.22+, 24.11+, or 26+)"
        return $false
    }
    ...
    if ($npmVersion) {
        Write-Warn "Node.js $version uses npm $npmVersion, which does not satisfy Athena requirement $npmRange"
    } else {
...
function Test-Node {
    ...
    Write-Info "Using a Athena-managed Node.js installation instead..."

    # Prefer a Athena-managed Node from a previous run over a too-old system one.
    $managedNode = "$AthenaHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$AthenaHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$AthenaHome\node"
        Write-Success "Node.js $version found (Athena-managed)"
        ...
        Update-ManagedNpm "$AthenaHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Athena-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $AthenaHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $AthenaHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\athena-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$AthenaHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$AthenaHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$AthenaHome\node") {
                    try {
                        Rename-Item "$AthenaHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "Athena-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$AthenaHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $AthenaHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\athena-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `athena
                    # update` path (#4735).
                    ...
                    $stashName = "athena-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. athena-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``athena update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Athena behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running athena processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/pavel4ai/athena/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/pavel4ai/athena/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/pavel4ai/athena/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                ...
                if ($extractedDir) {
                    ...
                    # `athena update` (see the notes at the shared clone-path
    ...
    # next `athena update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Athena-Setup.exe
    ...
    if (-not $resolvedPython) {
        throw "Athena-managed Python is unavailable. Run install.ps1 -Stage python first."
    }
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running athena process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running athena processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Athena_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (athena.exe) plus its child tree.
            & taskkill /F /T /IM athena.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM athena.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m athena_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not athena.exe.
            ...
            # Athena children re-exec through .athena-runtime, so killing only
            ...
            # user (see athena_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Athena " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `athena update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Athena (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `athena.exe` exists
...
"@ 2>$null
    ...
    # isn't set, leaving venv\ empty and athena.exe broken with
    ...
    # here, not 30 seconds later when the user runs `athena`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Athena processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Athena processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Athena and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register athena.exe in dist-info/RECORD but fail to
        ...
        # `athena` command while athena-agent.exe / athena-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m athena_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `athena dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\athena_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `athena dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `athena dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: athena_cli/web_server.py"
        }
    }
...
function Install-AthenaCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the athena launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring athena.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "athena.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the athena command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # athena_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("athena", "athena-acp")) {
    ...
    $requiredExe = Join-Path $Destination "athena.exe"
    $requiredCmd = Join-Path $Destination "athena.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the athena command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up athena command..."
    
    if ($NoVenv) {
        $athenaBin = "$InstallDir"
    } else {
        # $AthenaHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `athena update`'s autostash
        ...
        # installer staged under athena-agent\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-AthenaCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $athenaBin = "$AthenaHome\bin"
        Install-AthenaCommandLaunchers -Root $InstallDir -Destination $athenaBin | Out-Null
    }
    ...
    #   athena-agent\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The athena-agent\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept athena via $athenaBin)"
        }
    }
    
    if ($currentPath -notlike "*$athenaBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$athenaBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $athenaBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set ATHENA_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\athena instead
    # of the Unix default ~/.athena
    $currentAthenaHome = [Environment]::GetEnvironmentVariable("ATHENA_HOME", "User")
    if (-not $currentAthenaHome -or $currentAthenaHome -ne $AthenaHome) {
        [Environment]::SetEnvironmentVariable("ATHENA_HOME", $AthenaHome, "User")
        Write-Success "Set ATHENA_HOME=$AthenaHome"
    }
    $env:ATHENA_HOME = $AthenaHome
    
    # Update current session
    $env:Path = "$athenaBin;$env:Path"
    
    Write-Success "athena command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.athena-bootstrap-complete which tells the Athena
    ...
    # Athena-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `athena update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".athena-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the ATHENA_HOME directory structure ($AthenaHome, default %LOCALAPPDATA%\athena)
    New-Item -ItemType Directory -Force -Path "$AthenaHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AthenaHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AthenaHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AthenaHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AthenaHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AthenaHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AthenaHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AthenaHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AthenaHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$AthenaHome\.env"
    ...
    # Create config.yaml
    $configPath = "$AthenaHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Athena's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$AthenaHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in athena_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $AthenaHome"
    
    # Seed bundled skills into $AthenaHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $AthenaHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $AthenaHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$AthenaHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $AthenaHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Athena-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'athena setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\athena-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\athena-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\athena-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `athena tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $AthenaHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only Athena' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Athena' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'athena tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `athena
...
# paths still lazy-install via install_cua_driver() (athena_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `athena computer-use install` runs. Bounded
        ...
        # Athena' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'athena tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: athena computer-use install"
            }
        } else {
...
# the final `electron` -> `Athena` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Athena.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_athena_desktop command
    # resolves apps/desktop/release/win-unpacked/Athena.exe directly,
    ...
    # enforces the supported Node lines and prepends the Athena-managed Node to
    ...
    # The --dir mode produces an unpacked Athena.exe in
    ...
    # are NOT trying to work around). The Athena icon + product name are
    # stamped onto Athena.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\athena-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Athena.exe",
        "$desktopDir\release\win-arm64-unpacked\Athena.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Athena.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Athena icon + identity are stamped onto Athena.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, athena-agent#38216).
    ...
    #    Athena.exe. We deliberately do NOT point them at `athena desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Athena.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Athena.lnk')
        )
        ...
        # with the Athena icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.athena/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'athena setup'."
        return
    }
    ...
    # Run athena setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m athena_cli.main setup
    } else {
        python -m athena_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $athenaCmd = "$InstallDir\venv\Scripts\athena.exe"
    if (-not (Test-Path $athenaCmd)) {
        $athenaCmd = "athena"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$AthenaHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'athena whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: athena gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$AthenaHome\logs\gateway.log"
            Start-Process -FilePath $athenaCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$AthenaHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: athena gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: athena gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$AthenaHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$AthenaHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$AthenaHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$AthenaHome\athena-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   athena              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   athena setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   athena config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   athena config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   athena gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   athena update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Athena repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    # Managed Python lives under $InstallDir\.athena-runtime, so the checkout
...
if ($IncludeDesktop) {
    ...
    # (Athena-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Athena to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
