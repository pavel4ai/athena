...
function Write-Banner {
    ...
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by Futurebound Corp..              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Test-Node {
    ...
    try {
        ...
        if ($zipName) {
            ...
            if ($extractedDir) {
                if (Test-Path "$AthenaHome\node") { Remove-Item -Recurse -Force "$AthenaHome\node" }
                Move-Item $extractedDir.FullName "$AthenaHome\node"
...
function Install-Venv {
    ...
    if (Test-Path "venv") {
        ...
        # DLLs by any running athena process. Windows denies deletion of loaded
        # DLLs, so kill any athena.exe tree before removing the venv.
...
function Set-PathVariable {
    ...
    if ($NoVenv) {
        ...
        $athenaBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so athena is globally available
    # On Windows, the athena.exe in venv\Scripts\ has the venv Python baked in
...
function Install-Desktop {
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Athena-managed
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://github.com/pavel4ai/athena/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
