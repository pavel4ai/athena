"""Run a real Athena CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_athena_executable(athena_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "athena",
        Path(".venv") / "Scripts" / "athena.exe",
    ):
        candidate = athena_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("athena")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Athena executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Athena source checkout containing .venv/bin/athena",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated ATHENA_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["athena.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['athena.client.active']}"
        )
    [model] = by_name["athena.model_route.count"]
    expected_model = {
        "name": "athena.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['athena.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["athena.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['athena.task_run.started']}"
        )
    [terminal] = by_name["athena.task_run.finished"]
    ...
    [tool] = by_name["athena.tool_call.count"]
    ...
    lifecycle = by_name["athena.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["athena.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Athena development environment requires jsonschema"
        ) from exc
    ...
    if metrics["athena.client.active"] != [
        {
            "name": "athena.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['athena.client.active']}"
        )
    [model] = metrics["athena.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['athena.model_route.count']}"
        )
    [terminal] = metrics["athena.task_run.finished"]
    ...
    [tool] = metrics["athena.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["athena.skill.lifecycle.count"]
    ...
    loads = metrics["athena.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    athena_repo = args.athena_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    athena = _resolve_athena_executable(athena_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="athena-relay-shared-metrics-"))
    home = root / "athena-home"
    ...
    (root / "athena.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "athena.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Athena exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Athena model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Athena did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Athena did not print the mock model response")
    ...
    print("Athena -> NeMo Relay shared-metrics smoke test passed")
