...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[athena] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[athena`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[athena`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        AthenaHome = $(if ($paths) { $paths.athena_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\athena\athena-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-athena'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-athena') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
