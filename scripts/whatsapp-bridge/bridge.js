 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.athena/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.athena', 'whatsapp', 'session'));
// Cache directories: the Python gateway passes the profile-aware paths via
// env (ATHENA_HOME-aware, new cache/ layout).  Fall back to the legacy
// hardcoded locations for bridges launched outside the gateway.
const IMAGE_CACHE_DIR = process.env.ATHENA_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.athena', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.ATHENA_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.athena', 'document_cache');
const AUDIO_CACHE_DIR = process.env.ATHENA_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.athena', 'audio_cache');
...
// `athena update` updates bridge.js on disk but a long-lived bridge process
...
function enqueuePollUpdateEvent({ key, update, selectedOptions, aggregation }) {
  ...
  const pollId = key?.id
    ...
    || '';
  // Only surface votes on polls Athena itself created (tracked when
    ...
    for (const msg of messages) {
      ...
      if (msg.key.fromMe) {
        ...
        if (WHATSAPP_MODE === 'bot') {
          ...
          // the owner replied to would leak into Athena and trigger
      ...
      // Ignore Athena' own reply messages in self-chat mode to avoid loops.
    ...
    switch (type) {
      case 'image':
        if (ext === 'gif') {
          ...
          let tmpGifMp4 = null;
          try {
            tmpGifMp4 = path.join(tmpdir(), `athena_gif_${randomBytes(6).toString('hex')}.mp4`);
            execFileSync(
              'ffmpeg',
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `athena_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execFileSync(
              'ffmpeg',
