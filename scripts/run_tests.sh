...
#   * Proper venv activation (probes .venv, venv, then ~/.athena/...)
#
# Usage:
...
# (ATHENA_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.athena/athena-agent/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.athena/athena-agent/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${ATHENA_PYTHON:-}" ] && [ -x "$ATHENA_PYTHON" ] \
    && "$ATHENA_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: ATHENA_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `athena` binary rather
  # than the devShell hook.
  PYTHON="$ATHENA_PYTHON"
  echo "▶ no local venv — using Nix dev venv via ATHENA_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and ATHENA_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.athena/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.athena"
...
#
#   * ATHENA_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * ATHENA_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     ATHENA_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# ATHENA_RUN_SLOW_PET_TESTS / ATHENA_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no ATHENA_TEST_* glob) so the "no
# credential can leak" property stays auditable at a glance.
TEST_ENV=()
for _test_var in ATHENA_TEST_IMAGE ATHENA_TEST_WORKERS ATHENA_TEST_PATHS \
  ATHENA_TEST_FILE_TIMEOUT ATHENA_TEST_FILE_RETRIES ATHENA_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${ATHENA_RUN_SLOW_PET_TESTS:+ATHENA_RUN_SLOW_PET_TESTS="$ATHENA_RUN_SLOW_PET_TESTS"} \
  ${ATHENA_E2E_BROWSER:+ATHENA_E2E_BROWSER="$ATHENA_E2E_BROWSER"} \
