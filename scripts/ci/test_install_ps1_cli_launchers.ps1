# Behavioral test for install.ps1's athena launcher staging (PR #92092,
...
#
# The test lifts the real Install-AthenaCommandLaunchers function from the
...
# the caller (Set-PathVariable passes $AthenaHome\bin -- the managed binary
...
$fn = $ast.Find({
    ...
    $n.Name -eq 'Install-AthenaCommandLaunchers'
}, $true)

if (-not $fn) {
    throw "Install-AthenaCommandLaunchers not found in $installPs1"
}
...
$caseRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase (
    'athena-cli-launcher-test-' + [guid]::NewGuid().ToString('N')
)))
...
try {
    $installRoot = Join-Path $caseRoot 'athena-agent'
    ...
    $missingThrew = $false
    try {
        Install-AthenaCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    } catch {
        $missingThrew = $_.Exception.Message -like '*required launcher not found*'
    }
    Assert-True $missingThrew 'missing athena.exe fails the launcher stage'
    ...
    $athenaV1 = [byte[]](77, 90, 1)
    $athenaV2 = [byte[]](77, 90, 2)
    $acp = [byte[]](77, 90, 3)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'athena.exe'), $athenaV1)
    Set-Content -Path (Join-Path $installRoot 'venv\pyvenv.cfg') `
        -Value "home = X" -Encoding Ascii

    $staged = Install-AthenaCommandLaunchers -Root $installRoot -Destination $binDir
    Assert-True ($staged -eq $binDir) 'returns the destination it staged into'
    Assert-BytesEqual $athenaV1 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'athena.exe'))) `
        'normal venv: exe copy lands in the destination'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'athena-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'athena.exe'), $athenaV2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'athena-acp.exe'), $acp)
    Install-AthenaCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $athenaV2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'athena.exe'))) `
        'installer refreshes an existing Athena launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'athena-acp.exe'))) `
    ...
    Install-AthenaCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-True (Test-Path -LiteralPath (Join-Path $binDir 'athena.cmd')) `
        'relocatable venv: .cmd delegator staged'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'athena.exe'))) `
        'relocatable venv: stale exe copy removed'
    $cmdBody = [System.IO.File]::ReadAllText((Join-Path $binDir 'athena.cmd'))
    Assert-True ($cmdBody.Contains((Join-Path $scriptsDir 'athena.exe')) -and $cmdBody.Contains('%*')) `
