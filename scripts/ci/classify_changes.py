...
# tree generates llms.txt — the index every LLM (Athena included, via the
# athena-agent skill) reads to learn what Athena can do — and
...
_MCP_CATALOG_FILES = {"athena_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (Athena-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``athena serve`` backend, the Docker image)
