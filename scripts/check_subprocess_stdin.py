...
# ``get_athena_home() / "plugins"`` (user) and ``./.athena/plugins/`` (project,
# gated behind ``ATHENA_ENABLE_PROJECT_PLUGINS``) — see
# ``athena_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``athena_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import athena_constants (this script
    ...
    # plugins in get_athena_home()/plugins/ and project plugins in
    # ./.athena/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_athena_home() / "plugins"]
    if os.environ.get("ATHENA_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".athena" / "plugins")
