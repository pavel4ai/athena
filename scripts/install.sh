...
# Python-driven tool session (e.g. Athena terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:pavel4ai/athena.git"
REPO_URL_HTTPS="https://github.com/pavel4ai/athena.git"
ATHENA_HOME="${ATHENA_HOME:-$HOME/.athena}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${ATHENA_INSTALL_DIR:-}" ]; then
...
#   code at /usr/local/lib/athena-agent, command at /usr/local/bin/athena,
#   data still at /root/.athena (ATHENA_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$ATHENA_HOME/.no-bundled-skills so future"
            echo "                   'athena update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Athena.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.athena/athena-agent"
            echo "                   default (root, Linux): /usr/local/lib/athena-agent"
            echo "  --athena-home PATH  Data directory (default: ~/.athena, or \$ATHENA_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Athena installs the code under"
            echo "  /usr/local/lib/athena-agent and links the command into"
            echo "  /usr/local/bin/athena (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$ATHENA_HOME"
            echo "  (default /root/.athena).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$ATHENA_HOME/athena-agent are preserved in-place."
...
# intentional, but they leave the checkout dirty — which forces `athena update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Athena-Setup) passes it so
...
# Decide where the repo checkout + venv live, and where the `athena` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $ATHENA_HOME/athena-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $ATHENA_HOME/athena-agent
...
#                              $ATHENA_HOME/athena-agent — then preserve it)
#
# Always no-op when the user set --dir or $ATHENA_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in ATHENA_HOME.
    if is_termux; then
        INSTALL_DIR="$ATHENA_HOME/athena-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$ATHENA_HOME/athena-agent/.git" ]; then
            INSTALL_DIR="$ATHENA_HOME/athena-agent"
        ...
        # /usr/local/bin/athena wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/athena"
        log_info "  Data:    $ATHENA_HOME (unchanged)"
...
# Point a Athena-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $ATHENA_HOME/node/bin — which is NOT on
...
# Athena's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Athena-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$ATHENA_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$ATHENA_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$ATHENA_HOME/node/etc/npmrc"
}

get_athena_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/athena" ]; then
        echo "$link_dir/athena"
    else
        echo "athena"
    fi
}
...
install_uv() {
    ...
    # Athena owns its own uv at $ATHENA_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (athena_cli/managed_uv.py) looks in the same
    # place, so install.sh and `athena update` stay in sync.
    local _managed_uv="$ATHENA_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $ATHENA_HOME/bin ..."
    mkdir -p "$ATHENA_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/athena-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/athena-uv-installer.$$.sh")"
    ...
    # directly into $ATHENA_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$ATHENA_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
check_python() {
    if [ "$DISTRO" = "termux" ]; then
        ...
        # Athena currently declares requires-python >=3.11,<3.14.  Termux can
        ...
        log_error "Termux Python $PYTHON_FOUND_VERSION is not supported; Athena requires Python >=3.11,<3.14"
...
# replaced with the Athena-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing Athena-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing Athena-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Athena-managed Node from a previous run over a too-old system one.
    if [ -x "$ATHENA_HOME/node/bin/node" ] && [ -x "$ATHENA_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$ATHENA_HOME/node/bin/node" --version)"; then
        export PATH="$ATHENA_HOME/node/bin:$PATH"
        log_success "Node.js $("$ATHENA_HOME/node/bin/node" --version) found (Athena-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing Athena-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is unsupported (Athena requires Node 22.22+, 24.11+, or 26+) — installing Athena-managed Node $NODE_VERSION..."
...
# Download and adopt one Node release line (e.g. 26) into ~/.athena/node/.
#
...
install_node_line() {
    ...
    log_info "Extracting to ~/.athena/node/..."
    ...
    # Place into ~/.athena/node/ and symlink binaries into the same bin dir
    # the athena command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$ATHENA_HOME/node"
    mkdir -p "$ATHENA_HOME"
    mv "$extracted_dir" "$ATHENA_HOME/node"
    rm -rf "$tmp_dir"
    ...
    ln -sf "$ATHENA_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$ATHENA_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$ATHENA_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    if ! installed_ver=$("$ATHENA_HOME/node/bin/node" --version 2>&1); then
    ...
    log_success "Node.js $installed_ver installed to ~/.athena/node/"
    HAS_NODE=true
    return 0
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Athena install may complete, but web search and dependency downloads can fail."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `athena update` path
                # (#4735).
                ...
                local stash_name
                stash_name="athena-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``athena update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Athena behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up athena command..."

    if [ "$USE_VENV" = true ]; then
        ATHENA_BIN="$INSTALL_DIR/venv/bin/python"
        ATHENA_ENTRYPOINT="$INSTALL_DIR/athena"
    else
        ATHENA_BIN="$(which athena 2>/dev/null || echo "")"
        if [ -z "$ATHENA_BIN" ]; then
            log_warn "athena not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$ATHENA_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$ATHENA_ENTRYPOINT" ]; }; then
        log_warn "Athena launcher prerequisites not found"
    ...
    # Create a user-facing shim for the athena command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $ATHENA_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$ATHENA_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/athena"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/athena" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ATHENA_BIN" "$ATHENA_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/athena" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/athena"
    log_success "Installed athena launcher → $command_link_display_dir/athena"

    # Also expose `athena-agent`. The `athena-agent` console script declared in
    ...
    rm -f "$command_link_dir/athena-agent"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/athena-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ATHENA_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/athena-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ATHENA_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/athena-agent"
    log_success "Installed athena-agent launcher → $command_link_display_dir/athena-agent"

    # Also expose `athena-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `athena-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report Athena as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/athena-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/athena-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ATHENA_BIN" "$ATHENA_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/athena-acp" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/athena-acp"
    log_success "Installed athena-acp launcher → $command_link_display_dir/athena-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "athena command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v athena' \
                >/dev/null 2>&1; then
        ...
        log_info "athena not on PATH in non-login shells (common on RHEL-family)"
    ...
    # Export for current session so athena works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.athena directory structure (config at top level, code in subdir)
    mkdir -p "$ATHENA_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.athena/.env (top level, easy to find)
    if [ ! -f "$ATHENA_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$ATHENA_HOME/.env"
            log_success "Created ~/.athena/.env from template"
        else
            touch "$ATHENA_HOME/.env"
            log_success "Created ~/.athena/.env"
        fi
    else
        log_info "~/.athena/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$ATHENA_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.athena/config.yaml (top level, easy to find)
    if [ ! -f "$ATHENA_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$ATHENA_HOME/config.yaml"
            log_success "Created ~/.athena/config.yaml from template"
        fi
    else
        log_info "~/.athena/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in athena_cli/default_soul.py — the
    ...
    if [ ! -f "$ATHENA_HOME/SOUL.md" ]; then
        cat > "$ATHENA_HOME/SOUL.md" << 'SOUL_EOF'
...
SOUL_EOF
        log_success "Created ~/.athena/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.athena/"

    # Seed bundled skills into ~/.athena/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `athena update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            > "$ATHENA_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $ATHENA_HOME/.no-bundled-skills"
        log_info "  Future 'athena update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.athena/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.athena/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$ATHENA_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$ATHENA_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.athena/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$ATHENA_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Athena Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Athena will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Athena Agent browser tools — explicit browser override."
...
# install — the same closure `athena update` installs
# (athena_cli/main.py::_update_node_dependencies). Prebuilt/partial checkouts
...
install_node_deps() {
    ...
    # Keep the checkout clean so `athena update` doesn't autostash every run.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
install_browser_use_cli() {
    ...
    # `athena tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if [ -x "$ATHENA_HOME/bin/browser-use" ]; then
        log_success "Browser Use CLI already installed"
        return 0
    fi

    log_info "Installing Browser Use CLI (default browser backend)..."
    # UV_TOOL_BIN_DIR keeps the binary inside Athena' managed bin dir, where
    # the browser tool resolves it — no reliance on the user's PATH.
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$ATHENA_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'athena tools')"
    fi
}
...
install_computer_use_driver() {
    ...
    # `athena tools`, the dashboard, or the desktop app — is a config flip,
    # not a surprise multi-minute binary fetch (the confusion this fixes:
    # users had to discover `athena computer-use install` on their own).
    ...
    # Same upstream installer `athena computer-use install` runs; time-boxed
    # so a stalled GitHub download can't hang the Athena install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching Athena'
    ...
    if run_with_timeout 660 /bin/bash -c \
        ...
        >"$cua_log" 2>&1; then
        log_success "Computer Use driver installed (enable via 'athena tools' → Computer Use)"
    else
        log_warn "Computer Use driver install failed — it will install on demand when you enable the tool."
        log_info "Install later with: athena computer-use install"
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'athena setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m athena_cli.main setup < /dev/tty
    else
        python -m athena_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$ATHENA_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Athena to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$ATHENA_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'athena whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                ATHENA_CMD="$(get_athena_command_path)"
                $ATHENA_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'athena whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'athena gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $ATHENA_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $ATHENA_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: athena gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: athena gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $ATHENA_CMD gateway > "$ATHENA_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.athena/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: athena gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: athena gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.athena-bootstrap-complete, which tells the Athena
    ...
    local marker_path="$INSTALL_DIR/.athena-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $ATHENA_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $ATHENA_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $ATHENA_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}athena${NC}              Start chatting"
    echo -e "   ${GREEN}athena setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}athena config${NC}       View/edit configuration"
    echo -e "   ${GREEN}athena config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}athena gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}athena update${NC}       Update to latest version"
    echo ""
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$ATHENA_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$ATHENA_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$ATHENA_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$ATHENA_HOME/node" --silent --ignore-scripts \
...
# extract a tree MISSING the electron binary, so the `electron`->`Athena` rename
...
# _purge_electron_build_cache() used by `athena desktop`; install.sh was the only
...
install_desktop() {
    ...
    # supported Node lines and prepends the Athena-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `athena desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Athena" ]; then
            app="$desktop_dir/release/linux-unpacked/Athena"
        elif [ -x "$desktop_dir/release/linux-unpacked/athena" ]; then
            app="$desktop_dir/release/linux-unpacked/athena"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Athena.app" \
            "$desktop_dir/release/mac/Athena.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `athena desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading ATHENA_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if ATHENA_HOME="$ATHENA_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from athena_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `athena update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Athena-managed Node
            # provisioned during prerequisites/node-deps (at $ATHENA_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $ATHENA_HOME.
    # $ATHENA_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'athena update' on this host install.
