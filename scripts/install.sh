...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   Installs optional deps + runs athena setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
print_banner() {
    ...
    echo "│             ⚕ Athena Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Futurebound Corp..              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# string clears that floor; anything below it is replaced with the Athena-
...
check_node() {
    ...
    if [ -x "$ATHENA_HOME/node/bin/node" ] && node_satisfies_build "$("$ATHENA_HOME/node/bin/node" --version)"; then
    ...
    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Athena-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    local installed_ver
    installed_ver=$("$ATHENA_HOME/node/bin/node" --version 2>/dev/null)
...
setup_path() {
    ...
    if [ "$USE_VENV" = true ]; then
        ATHENA_BIN="$INSTALL_DIR/venv/bin/athena"
    else
    ...
    if [ ! -x "$ATHENA_BIN" ]; then
        log_warn "athena entry point not found at $ATHENA_BIN"
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Athena will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (athena --tui may not work)"
        }
...
ensure_browser() {
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$ATHENA_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$ATHENA_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Athena for pip install"
    ...
    ATHENA_CMD="$(command -v athena 2>/dev/null || echo "")"
    if [ -n "$ATHENA_CMD" ]; then
        log_info "Running athena setup..."
        "$ATHENA_CMD" setup
    else
        log_warn "athena command not found on PATH"
        log_info "Try: python -m athena_cli.main setup"
    fi
}
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Athena-managed Node to PATH, so
    ...
    # trips Gatekeeper's tamper check ("Athena is damaged and can't be opened").
