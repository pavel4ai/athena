...
# - ensures ~/.athena/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-athena.sh
...
#   ATHENA_API_PORT=8642
#   ATHENA_API_HOST=127.0.0.1
#   ATHENA_API_MODEL_NAME='Athena Agent'
...
ATHENA_ENV_FILE="${ATHENA_ENV_FILE:-$HOME/.athena/.env}"
ATHENA_API_PORT="${ATHENA_API_PORT:-8642}"
ATHENA_API_HOST="${ATHENA_API_HOST:-127.0.0.1}"
ATHENA_API_CONNECT_HOST="${ATHENA_API_CONNECT_HOST:-127.0.0.1}"
ATHENA_API_MODEL_NAME="${ATHENA_API_MODEL_NAME:-Athena Agent}"
ATHENA_API_BASE_URL="http://${ATHENA_API_CONNECT_HOST}:${ATHENA_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-athena.sh"
LOG_DIR="$HOME/.athena/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$ATHENA_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$ATHENA_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.athena.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.athena</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.athena"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.athena"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-athena.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
...
[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-athena.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.athena/logs/openwebui.log
StandardError=append:%h/.athena/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-athena.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$ATHENA_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Athena API server is configured...'
  upsert_env API_SERVER_ENABLED true "$ATHENA_ENV_FILE"
  upsert_env API_SERVER_HOST "$ATHENA_API_HOST" "$ATHENA_ENV_FILE"
  upsert_env API_SERVER_PORT "$ATHENA_API_PORT" "$ATHENA_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$ATHENA_API_MODEL_NAME" "$ATHENA_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$ATHENA_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Athena gateway so API server settings take effect...'
  athena gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${ATHENA_API_CONNECT_HOST}:${ATHENA_API_PORT}/health" >/dev/null; then
    log 'Athena API server did not answer on the first check. Trying to start gateway in the background...'
    nohup athena gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${ATHENA_API_CONNECT_HOST}:${ATHENA_API_PORT}/health" >/dev/null
  ...
  log "Athena API endpoint: ${ATHENA_API_BASE_URL}"
