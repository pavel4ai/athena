...
sandbox = tempfile.TemporaryDirectory(prefix="athena-104260-")
os.environ.clear()
os.environ.update(
    HOME=sandbox.name,
    ATHENA_HOME=sandbox.name + "/athena",
    PATH="/usr/bin:/bin",
    PYTHONDONTWRITEBYTECODE="1",
    ATHENA_DISABLE_MODEL_METADATA_FETCH="1",
)
...
# JSON is valid YAML; only the temporary Athena home is written.
Path(os.environ["ATHENA_HOME"], "config.yaml").write_text(
