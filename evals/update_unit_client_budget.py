...
os.environ.clear()
os.environ.update(allowed, HOME=str(home), ATHENA_HOME=str(home / "athena"))
...
def scoped_systemctl(argv, *, timeout):
    if "list-units" in argv:
        ...
        argv = [arg for arg in argv if arg not in ("athena-gateway*", "athena-serve*")] + [unit + ".service"]
...
unit = f"athena-serve-audit-089c35aa-{tag}"
cmd = ["systemctl", "--user"]
def run(args):
