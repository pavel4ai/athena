...
def main():
    ...
    home = Path(tempfile.mkdtemp(prefix='athena-acp-empty-'))
    athena = home / '.athena'
    athena.mkdir()
    (athena / 'config.yaml').write_text(
    ...
    env.update(HOME=str(home), ATHENA_HOME=str(athena), PYTHONPATH=os.pathsep.join([str(repo), os.environ.get('PYTHONPATH', '')]),
               ATHENA_ACP_SKIP_CONFIGURED_MCP='1', OPENAI_API_KEY='local-fixture-key',
    ...
    def rows():
        with sqlite3.connect(athena / 'state.db') as db:
