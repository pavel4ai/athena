...
if len(sys.argv) == 2:
    with tempfile.TemporaryDirectory(prefix="kanban-transport-") as home:
        env = {"HOME": home, "ATHENA_HOME": home + "/athena", "PATH": os.environ["PATH"], "PYTHONDONTWRITEBYTECODE": "1"}
...
hh = Path(os.environ["ATHENA_HOME"])
...
os.environ.update({"ATHENA_KANBAN_DB": str(db), "ATHENA_KANBAN_BOARD": "default", "ATHENA_KANBAN_TASK": own, "ATHENA_KANBAN_RUN_ID": str(task.current_run_id), "ATHENA_KANBAN_CLAIM_LOCK": task.claim_lock})
ch = home / "codex"
ch.mkdir()
(ch / "config.toml").write_text(
    'model="fixture"\nmodel_provider="fixture"\n'
    '[model_providers.fixture]\nname="fixture"\nbase_url="http://127.0.0.1:9/v1"\nwire_api="responses"\n'
    '[mcp_servers.athena-mcp]\ncommand=' + json.dumps(sys.executable) + '\nargs=["-m","agent.transports.athena_tools_mcp_server"]\nstartup_timeout_sec=40\n'
    '[mcp_servers.athena-mcp.env]\nPYTHONPATH=' + json.dumps(str(repo)) + '\nATHENA_HOME=' + json.dumps(str(hh)) + '\n'
)
report = {}
with CodexAppServerClient(codex_bin=shutil.which("codex") or "codex", codex_home=str(ch)) as c:
    try:
        ...
        raise
    script = "import os,sys,json;sys.path.insert(0," + repr(str(repo)) + ");from tools import kanban_tools as kt;print(json.dumps({'marker':os.getenv('ATHENA_DELEGATED_CHILD_CONTEXT'),'db':os.getenv('ATHENA_KANBAN_DB'),'mutation':json.loads(kt._handle_complete({'task_id':" + repr(own) + ",'summary':'must refuse'}))}))"
    ...
    for name, tid in (("foreign", foreign), ("own", own)):
        report[name] = c.request("mcpServer/tool/call", {"threadId": thread_id, "server": "athena-mcp", "tool": "kanban_complete", "arguments": {"task_id": tid, "summary": "supervised parent handoff"}}, timeout=50)
