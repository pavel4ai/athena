...
Run with a fresh HOME and ATHENA_HOME using the repository virtualenv:

```sh
probe=$(mktemp -d)
HOME="$probe" ATHENA_HOME="$probe/state" .venv/bin/python evals/gateway_completion/delivery_lifecycle.py
```
...
and unavailable raw API-owner retention. `ATHENA_EVAL_REPO` selects another checkout for A/B.
