...
env.update(HOME=str(home), ATHENA_HOME=str(home), ATHENA_DASHBOARD_SESSION_TOKEN='history-fixture-token', PYTHONPATH=repo, ATHENA_NONINTERACTIVE='1')
cmd=[sys.executable, '-m', 'athena_cli.main', 'serve', '--host', '127.0.0.1', '--port', str(args.port), '--isolated']
...
try:
    for _ in range(120):
        try:
            req=urllib.request.Request(f'http://127.0.0.1:{args.port}/api/status',headers={'X-Athena-Token':'history-fixture-token'})
