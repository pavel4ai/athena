...
def main():
    ...
    env = {"PATH": "/usr/bin:/bin", "HOME": str(root / "home"),
           "ATHENA_HOME": str(root / "home/.athena"),
           "XDG_DATA_HOME": str(root / "xdg"), "LANG": "C.UTF-8",
           "ATHENA_NONINTERACTIVE": "1"}
    Path(env["ATHENA_HOME"]).mkdir(parents=True)
    venv_dir = root / "venv"
    ...
    install = (
        ...
        "from athena_cli.linux_desktop_entry import install_desktop_entry; "
        f"sys.argv[0]={str(repo / 'athena')!r}; "
    ...
    rows["venv_import"] = run([str(python), "-I", "-c", "import yaml,athena_cli.main; print(yaml.__version__)"], env, "/")
    rows["base_import_negative"] = run([str(python.resolve()), "-I", "-c", "import yaml,athena_cli.main"], env, "/")
    ...
    custom = original.replace(b"Name=Athena\n", b"Name=Athena custom\n").replace(b"Terminal=false", b"Terminal=true")
    entry.write_bytes(custom)
    config = Path(env["ATHENA_HOME"]) / "config.yaml"
