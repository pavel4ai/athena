...
Run with the Athena venv Python and --repo CHECKOUT --output RECEIPT_DIR.
No user environment/config is inherited by Athena children. Unix PTY required.
"""
import argparse
import http.server
import json
...
def rows_worker():
    ...
    from athena_cli.model_switch_providers import (
...
def cli_pty(repo, env):
    ...
    proc = subprocess.Popen([sys.executable, "-m", "athena_cli.main", "model"],
    ...
    try:
        for schema in ("providers", "custom_providers"):
            for surface in ("cli", "rows"):
                ...
                with tempfile.TemporaryDirectory(prefix="athena-keycmd-live-") as tmp:
                    home = Path(tmp)
                    state = home / ".athena"
                    state.mkdir()
                    ...
                    env = {"HOME": str(home), "ATHENA_HOME": str(state),
