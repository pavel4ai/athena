"""Run the read-tool eval through the REAL Athena AIAgent.

For each task: fresh temp ATHENA_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    athena_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".athena"
    ...
    try:
        ...
        shutil.rmtree(athena_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.athena/.env; set +a  — then relaunch."
        )
