...
def run_probe(repo, receipt):
    ...
    owner = Path(os.environ["ATHENA_HOME"])
    ...
    with serving(Provider()) as provider, serving(CallbackServer(
        ...
        reloaded = asyncio.run(AthenaTokenStorage("positive", athena_home=owner).get_tokens())
        ...
        override = set_athena_home_override(other)
        try:
...
def main():
    ...
    if not args.isolated_worker:
        with tempfile.TemporaryDirectory(prefix="athena-oauth-http-") as temp:
            home = Path(temp)
            (home / ".athena").mkdir()
            env = {"PATH": os.environ.get("PATH", ""), "HOME": temp,
                   "ATHENA_HOME": str(home / ".athena"), "LANG": "C.UTF-8", "TZ": "UTC",
                   "PYTHONNOUSERSITE": "1"}
