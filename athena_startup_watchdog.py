...
timeout. Config is env-only (``ATHENA_STARTUP_WATCHDOG=0``,
``ATHENA_STARTUP_WATCHDOG_TIMEOUT_S``) because config.yaml parsing is itself in scope.
...
ENV_STARTUP_WATCHDOG = "ATHENA_STARTUP_WATCHDOG"
ENV_STARTUP_WATCHDOG_TIMEOUT_S = "ATHENA_STARTUP_WATCHDOG_TIMEOUT_S"
...
# Module singleton: the arm sites (athena_cli.main / athena_cli.gateway /
...
def _process_athena_home() -> Path:
    """ATHENA_HOME for diagnostic files — stdlib-only replica of the athena_constants default."""
...
def get_startup_watchdog_dump_path(home: Optional[Path] = None) -> Path:
    """Return ``<ATHENA_HOME>/logs/gateway-startup-watchdog.log``."""
    base = home if home is not None else _process_athena_home()
    return base.joinpath(*_DUMP_RELATIVE)


def startup_watchdog_disabled() -> bool:
    """True when ``ATHENA_STARTUP_WATCHDOG`` opts out explicitly."""
...
def arm_startup_watchdog(
    ...
    *,
    ...
    ``ATHENA_STARTUP_WATCHDOG=0`` or when the thread could not be started.
    """
    global _handle
    try:
