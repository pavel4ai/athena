...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/athena-uv-install.$$.log")"
...
if is_termux; then
    # Athena currently declares requires-python >=3.11,<3.14. Termux can expose
    ...
    if [ -z "${PYTHON_PATH:-}" ]; then
        if command -v python >/dev/null 2>&1; then
            ...
            echo -e "${RED}✗${NC} Termux Python $PYTHON_FOUND_VERSION is not supported; Athena requires Python >=3.11,<3.14"
...
# ============================================================================
# PATH setup — symlink athena into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up athena command..."

ATHENA_BIN="$SCRIPT_DIR/venv/bin/athena"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$ATHENA_BIN" "$COMMAND_LINK_DIR/athena"
echo -e "${GREEN}✓${NC} Symlinked athena → $COMMAND_LINK_DISPLAY_DIR/athena"
...
# ============================================================================
# Seed bundled skills into ~/.athena/skills/
# ============================================================================

ATHENA_SKILLS_DIR="${ATHENA_HOME:-$HOME/.athena}/skills"
mkdir -p "$ATHENA_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.athena/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ATHENA_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  athena status        # Check configuration"
if is_termux; then
    echo "  athena gateway       # Run gateway in foreground"
else
    echo "  athena gateway install # Install gateway service (messaging + cron)"
fi
echo "  athena cron list     # View scheduled jobs"
echo "  athena doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m athena_cli.main setup
fi
