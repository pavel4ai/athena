...
Interpreter resolution order is: `ATHENA_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `athena setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.athena/.athena_history` or under `ATHENA_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `athena setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/athena-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external athena setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.athena/tui-parent.log
    ...
    types/
      athena-ink.d.ts            type declarations for @athena/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent AthenaCLI subprocess for slash commands
```