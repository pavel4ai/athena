import type { InputEvent, Key } from '@athena/ink'
import * as Ink from '@athena/ink'
 ...
 * but that accepts `key.meta` on macOS — and athena-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.ATHENA_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which athena-ink decodes as meta+'d'; without this branch it
