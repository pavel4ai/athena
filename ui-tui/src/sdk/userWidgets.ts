...
import { Box, Text } from '@athena/ink'
...
/**
 * User widget apps — Athena authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$ATHENA_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.athena/plugins/`: files under ATHENA_HOME execute
...
const widgetsDir = () => join(process.env.ATHENA_HOME?.trim() || join(homedir(), '.athena'), 'tui-widgets')
 ...
 *  every change, so a widget Athena writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Athena) ever writes must hot-load too — a 10s poll here read
