...
// ATHENA_TUI_THEME=light (or ATHENA_TUI_BACKGROUND set to something
...
describe('detectLightMode', () => {
  ...
  it('honors ATHENA_TUI_LIGHT on/off', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ ATHENA_TUI_LIGHT: '1' })).toBe(true)
    expect(detectLightMode({ ATHENA_TUI_LIGHT: 'true' })).toBe(true)
    expect(detectLightMode({ ATHENA_TUI_LIGHT: 'on' })).toBe(true)
    expect(detectLightMode({ ATHENA_TUI_LIGHT: '0' })).toBe(false)
    expect(detectLightMode({ ATHENA_TUI_LIGHT: 'off' })).toBe(false)
  })
  ...
  it('lets ATHENA_TUI_LIGHT=0 override a light COLORFGBG', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ COLORFGBG: '0;15', ATHENA_TUI_LIGHT: '0' })).toBe(false)
  })

  it('honors ATHENA_TUI_THEME=light/dark as a symmetric explicit override', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ ATHENA_TUI_THEME: 'light' })).toBe(true)
    expect(detectLightMode({ ATHENA_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '0;15', ATHENA_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '15;0', ATHENA_TUI_THEME: 'light' })).toBe(true)
  })

  it('uses ATHENA_TUI_BACKGROUND luminance when COLORFGBG is missing', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ ATHENA_TUI_BACKGROUND: '#ffffff' })).toBe(true)
    expect(detectLightMode({ ATHENA_TUI_BACKGROUND: '#000000' })).toBe(false)
    expect(detectLightMode({ ATHENA_TUI_BACKGROUND: '#1e1e1e' })).toBe(false)
    // Three-char hex normalises like CSS.
    expect(detectLightMode({ ATHENA_TUI_BACKGROUND: '#fff' })).toBe(true)
    // Garbage falls through to the default-dark path.
    expect(detectLightMode({ ATHENA_TUI_BACKGROUND: 'not-a-colour' })).toBe(false)
  })
  ...
  it('rejects partially-invalid hex instead of silently truncating', async () => {
    ...
    expect(detectLightMode({ ATHENA_TUI_BACKGROUND: '#fffgff' })).toBe(false)
    expect(detectLightMode({ ATHENA_TUI_BACKGROUND: 'ffggff' })).toBe(false)
    expect(detectLightMode({ ATHENA_TUI_BACKGROUND: '#xyz' })).toBe(false)
    // Wrong length also rejected (no implicit padding/truncation).
    expect(detectLightMode({ ATHENA_TUI_BACKGROUND: '#fffff' })).toBe(false)
    expect(detectLightMode({ ATHENA_TUI_BACKGROUND: '#fffffff' })).toBe(false)
  })
...
describe('fromSkin', () => {
  ...
  // ATHENA_TUI_THEME=light would flip the base palette and make these
  ...
  it('derives completion current background from resolved completion background (polarity-compatible)', async () => {
    ...
    const { fromSkin } = await importThemeWithEnv({ ATHENA_TUI_BACKGROUND: '#ffffff' })
  ...
  it('skinIsLight: the authored background decides; host detection only when absent', async () => {
    const { skinIsLight } = await importThemeWithEnv({ ATHENA_TUI_BACKGROUND: '#ffffff' })
...
describe('derived tone ladder', () => {
  it('reproduces the original hand-tuned tones from seeds (reverse-engineered knobs)', async () => {
    ...
    const light = await importThemeWithEnv({ ATHENA_TUI_BACKGROUND: '#ffffff' })
...
describe('background-aware adaptation (OSC-11 light terminals)', () => {
  it('renders a dark-authored skin on light like minimumContrastRatio hosts do (the standardized look)', async () => {
    const { contrastRatio, fromSkin } = await importThemeWithEnv({ ATHENA_TUI_BACKGROUND: '#ffffff' })
  ...
  it('leaves the same skin untouched on a dark background', async () => {
    const { fromSkin } = await importThemeWithEnv({ ATHENA_TUI_BACKGROUND: '#1e1e2e' })
    const { color } = fromSkin(SLATE_COLORS, {})
  ...
  it('empty skin on a light background resolves to the light base palette', async () => {
    const { fromSkin, LIGHT_THEME } = await importThemeWithEnv({ ATHENA_TUI_BACKGROUND: '#ffffff' })
  ...
  it('defaultThemeForCurrentBackground follows a late ATHENA_TUI_BACKGROUND write', async () => {
    ...
    expect(defaultThemeForCurrentBackground({ ATHENA_TUI_BACKGROUND: '#ffffff' }).color).toEqual(LIGHT_THEME.color)
  })
