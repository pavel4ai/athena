...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.ATHENA_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Athena on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via ATHENA_TUI_TRUECOLOR=1.
