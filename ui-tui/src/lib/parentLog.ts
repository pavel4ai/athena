...
const logDir = join(process.env.ATHENA_HOME?.trim() || join(homedir(), '.athena'), 'logs')
...
// a real ~/.athena (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('athena-tui: parent lifecycle log unavailable\n')
    }
  }
}
