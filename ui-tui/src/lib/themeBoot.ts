 ...
 * `athena-boot-background` / `athena-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports ATHENA_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.athena matches get_athena_home()'s default.
const bootFilePath = () => join(process.env.ATHENA_HOME ?? join(homedir(), '.athena'), 'tui-theme-boot.json')

// Never touch the user's real ~/.athena from test runs (the TS suite has no
// ATHENA_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into ATHENA_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.ATHENA_TUI_THEME || env.ATHENA_TUI_LIGHT) {
    return result
  }
  ...
  if (boot.mode) {
    env.ATHENA_TUI_THEME = boot.mode
  ...
  if (
    ...
    !env.ATHENA_TUI_BACKGROUND
  ) {
    env.ATHENA_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.ATHENA_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.ATHENA_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into ATHENA_TUI_THEME.
