...
import { terminalBackgroundHex } from '@athena/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $ATHENA_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see ATHENA_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['ATHENA_TUI_BACKGROUND', process.env.ATHENA_TUI_BACKGROUND ?? '(unset)'],
            ['ATHENA_TUI_THEME', process.env.ATHENA_TUI_THEME ?? '(unset)'],
