import { forceRedraw, type MouseTrackingMode } from '@athena/ink'
...
export const coreCommands: SlashCommand[] = [
...

  {
    ...
    run: (_arg, ctx) => {
      ...
      // Exit code 42 signals the Python wrapper to exec `athena update`.
    ...
    run: async (arg, ctx) => {
      ...
      if (!arg && ctx.composer.hasSelection) {
        ...
        if (text) {
          ...
          return sys('clipboard copy failed — try ATHENA_TUI_FORCE_OSC52=1 to force the escape sequence')
        }
      }
...

  {
    ...
    run: (arg, ctx) => {
      ...
      const lines = items.map((m, i) => {
        const tag = m.role === 'user' ? `You #${i + 1}` : `Athena #${i + 1}`
