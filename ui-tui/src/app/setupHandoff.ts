import type { RunExternalProcess } from '@athena/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`athena ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching athena: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`athena ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
