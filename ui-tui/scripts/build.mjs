...
await build({
  ...
  // Skip the prebuilt @athena/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@athena/ink': resolve(root, 'packages/athena-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The athena_cli launcher
