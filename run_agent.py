...
def _session_source_for_agent(platform: Optional[str]) -> str:
    try:
        ...
        source = os.environ.get("ATHENA_SESSION_SOURCE", "")
...
from athena_cli.timeouts import (
...
_loaded_env_paths = load_athena_dotenv(athena_home=_athena_home, project_env=_project_env)
...
class AIAgent:
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        ...
        try:
            ...
            from athena_cli.models import ensure_lmstudio_model_loaded
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``ATHENA_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``ATHENA_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return env_float("ATHENA_API_TIMEOUT", 1800.0)
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``ATHENA_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("ATHENA_API_CALL_STALE_TIMEOUT")
    ...
    def _invoke_api_request_error_hook(
        ...
        *,
        ...
        # ``monkeypatch.setattr("athena_cli.plugins.has_hook", ...)`` still
    ...
    def _redact_message_content(content):
        ...
        Respects ``ATHENA_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.athena/sessions/session_{sid}.json``
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects ATHENA_REDACT_SECRETS via
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.athena/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``ATHENA_TURN_COMPLETION_EXPLAINER`` env var overrides
        config.  Exposed as a method so tests can patch a single seam,
        mirroring ``_file_mutation_verifier_enabled``.
        """
        try:
            import os as _os
            env = _os.environ.get("ATHENA_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
            # Read from the persisted config.yaml so gateway and CLI share
            # the same setting.  Import lazily to avoid a startup-time cycle.
    ...
    def _touch_activity(self, desc: str) -> None:
        ...
        process is a dispatcher-spawned worker (ATHENA_KANBAN_TASK set),
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        """
        # Dev test fixture (ATHENA_DEV_CREDITS_FIXTURE): inject a chosen notice state
        ...
        if not headers:
            return
        _dev = is_truthy_value(os.environ.get("ATHENA_DEV_CREDITS"))
        ...
        if _dev:
            # ATHENA_DEV_CREDITS: stream each capture to agent.log — watch live with
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from athena_cli.copilot_auth import resolve_copilot_token
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from athena_cli.models import lmstudio_model_reasoning_options
