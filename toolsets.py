...
_ATHENA_CORE_TOOLS = [
    ...
    "terminal", "process",
    # Read the desktop GUI's embedded terminal pane (gated on ATHENA_DESKTOP
    ...
    # spawned as a kanban worker (ATHENA_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (ATHENA_KANBAN_TASK env "
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in athena_cli/tools_config.py).
        "posture": True,
    },
    ...
    # the `athena send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "athena-cli": {
        ...
        "tools": _ATHENA_CORE_TOOLS,
        "includes": []
    },

    "athena-cron": {
        # Mirrors athena-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `athena tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as athena-cli; gated by `athena tools`",
        "tools": _ATHENA_CORE_TOOLS,
        "includes": []
    },
    ...
    "athena-webhook": {
        ...
        "tools": _ATHENA_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "athena-gateway": {
        ...
        "tools": [],
        "includes": ["athena-telegram", "athena-discord", "athena-whatsapp", "athena-slack", "athena-signal", "athena-bluebubbles", "athena-homeassistant", "athena-email", "athena-sms", "athena-mattermost", "athena-matrix", "athena-dingtalk", "athena-feishu", "athena-wecom", "athena-wecom-callback", "athena-weixin", "athena-qqbot", "athena-webhook", "athena-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``athena-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_ATHENA_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``athena-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (athena-<name>).
        # Gives them _ATHENA_CORE_TOOLS plus any tools the plugin registered
        ...
        if name.startswith("athena-"):
            ...
            try:
                ...
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_ATHENA_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
