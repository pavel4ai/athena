...
DEFAULT_DB_PATH = get_athena_home() / "state.db"
...
# athena_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``athena_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see athena_cli/kanban_db.py) would
...
class SessionDB:
    ...
    # With multiple athena processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            # ``athena_state._set_last_init_error(None)`` explicitly.
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``ATHENA_DISABLE_FTS_TRIGRAM`` or not yet created), so
