...
or change arbitrary Athena configuration.
...
small host fact requested by the RFC. Settings writes use Athena'
...
`ATHENA_HOME`, so one globally loaded plugin context follows multiplexed
...
```text
<ATHENA_HOME>/plugin-data/<plugin-data-namespace>/state.json
```
...
- **Profile isolation:** the data root resolves from the active context-local
  Athena home on every operation.
...
The implementation is covered with real temporary-Athena-home tests for:
