...
_profile_runtime_scope(profile_home)           (gateway/run.py)
   ├── set_athena_home_override(home)          config / state.db / skills /
...
- `get_secret(name)` resolves: global allowlist → active scope → fallback.
  ...
  - multiplexing **on**, no scope installed: **raises `UnscopedSecretError`**
    ...
    value.
- A small allowlist (`ATHENA_HOME`, `ATHENA_PROFILE`, proxy settings,
...
reload helper: `athena_cli.env_loader.load_athena_dotenv` skips the
...
## The ATHENA_HOME override

`athena_constants.py` holds a context-local override consulted by
`get_athena_home()` before the `ATHENA_HOME` env var. Everything that resolves
...
automatically. `get_process_athena_home()` exists for the few machine-level
assets that must not follow the override. `athena_home_key()` gives
...
`profiles_to_serve(multiplex, profile_allowlist)` in `athena_cli/profiles.py`
...
DB handles are resolved at call time through the active ATHENA_HOME override —
...
ATHENA_HOME override. Asset writes are atomic, type- and size-capped.
...
| Built-in tool registry | Built-ins are process-global; plugin-registered tools are overlaid per profile via `athena_home_key()`. |
...
- `agent/secret_scope.py`, `athena_constants.py`, `gateway/profile_routing.py`,
  `gateway/run.py` (`_profile_runtime_scope`), `athena_cli/profiles.py`
