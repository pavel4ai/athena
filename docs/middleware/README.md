...
- `telemetry_schema_version`: currently `athena.observer.v1`
- `middleware_schema_version`: currently `athena.middleware.v1`
...
For isolated local testing, use one `ATHENA_HOME` for plugin enablement and the
agent run:

```bash
export ATHENA_HOME=/tmp/athena-middleware-test
mkdir -p "$ATHENA_HOME"
athena plugins enable <plugin-name>
athena chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run athena plugins enable <plugin-name>
uv run athena chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["athena_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/athena-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
