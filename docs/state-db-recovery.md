...
`athena doctor` reports the explicit repair command.
...
repaired or restored file. Do not run `athena doctor --fix` while the gateway
is still up. Next steps:

```bash
athena gateway stop
ATHENA_HOME="$HOME/.athena" athena sessions recover --source "$HOME/.athena/state.db" --inspect-only
# if recoverable:
ATHENA_HOME="$HOME/.athena" athena sessions recover --source "$HOME/.athena/state.db" --output "$HOME/recovered-state.db"
```
...
```bash
athena gateway stop
ATHENA_HOME="$HOME/.athena" athena sessions repair --check-only
ATHENA_HOME="$HOME/.athena" athena sessions repair
```
