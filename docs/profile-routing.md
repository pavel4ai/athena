...
`athena config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.athena/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `ATHENA_HOME`, secret scope, and
