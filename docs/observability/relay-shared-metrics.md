...
Athena includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Athena and does not require a Athena observability plugin. Athena remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
Athena execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `athena-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the Athena `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `ATHENA_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `ATHENA_NEMO_RELAY_ATOF_*` and `ATHENA_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, Athena does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

Athena requires NeMo Relay 0.8.3 or later within the 0.8 release line. That
line provides the provider-codec and canonical tool-result contracts Athena
uses for managed provider and tool calls.

## Runtime Dependency and Data Boundary

Athena installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Athena process so configured interceptors can
...
Collection remains off unless Athena policy enables it:
...
opt-in. Set `ATHENA_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, Athena does not invoke Relay's plugin initializer, so Relay
...
selected file cannot be loaded, Athena reports the error and does not invoke
...
retain the same `session_id` and add `athena.session.segment` plus
`athena.session.segment_reason` (`compaction` or `max_turns`).

## Process-Wide Plugin Policy and Profile Isolation

Relay plugin configuration is a process-level deployment choice, not a Athena
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that Athena process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, Athena logs:
...
profile's Relay scope stack. Native dynamic plugins are loaded into the Athena
process and share the same policy boundary.

Run profiles in separate Athena processes when they require different trust
...
Athena core owns one Relay host and one isolated Relay session scope per Athena
...
marks do not require Athena plugin registration. Shared-metrics marks must
...
```text
Athena turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Athena shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Athena sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Athena used for the logical call, such as
...
New calls use `athena.model_route.count`. The previous
`athena.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `athena.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Athena
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Athena processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `athena.task_run`, parented to
the owning Athena session. The start counter contains only bounded execution
...
provider attempts for the same Athena API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Athena tool-call
...
cancellations. Active task ownership follows the task ID if Athena rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`athena.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Athena
...
tool or plugin names. Athena does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `athena.tool_approval` marks and
...
Successful skill mutations emit `athena.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `athena.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Athena derives reuse and patch-generation
...
```text
$ATHENA_HOME/telemetry/shared_metrics/metrics.sqlite3
$ATHENA_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Athena version,
...
schema field name, its current scope is one `ATHENA_HOME`, so it is more
...
Deleting `$ATHENA_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `ATHENA_HOME`. To reset it, stop Athena
processes and remove `$ATHENA_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Athena CLI turn against the deterministic local model server:
...
`$ATHENA_HOME/telemetry/shared_metrics/outbox/` to the Athena telemetry ingest
...
Removing `$ATHENA_HOME/telemetry/shared_metrics` still resets local identity,
