...
The canonical transcript store is SQLite via `SessionDB` (from `athena_state`). The
...
Counts consecutive restarts via a JSON file (`{ATHENA_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `athena update`, `athena gateway restart`,
or `/restart`.
