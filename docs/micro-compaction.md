...
has to be thrown away or summarized. Athena has always done this in one batch:
...
Athena folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Athena's normal session flush is
...
With `micro_compact` unset or `false` Athena behaves exactly as it always has:
...
Defaults to `$ATHENA_HOME/logs/agent.log`. It reports passes, outcome mix, net
