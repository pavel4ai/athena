"""The conftest WAL gate must agree with athena_state, and must not import it.

``tests/conftest.py::_wal_is_usable`` duplicates the SQLite WAL-reset version
predicate instead of importing ``athena_state``. That is deliberate: importing
``athena_state`` during collection caches ``DEFAULT_DB_PATH`` from the real
``~/.athena`` before the per-test ``ATHENA_HOME`` redirect, which makes tests
...
def test_conftest_gate_agrees_with_athena_state(version_info, monkeypatch):
    ...
    assert _wal_is_usable() is not is_sqlite_wal_reset_vulnerable(version_info), (
        f"conftest gate and athena_state disagree for SQLite {version_info}"
    )


def test_conftest_does_not_import_athena_state_at_collection():
    """The gate must stay import-free of athena_state.

    Importing it during collection caches DEFAULT_DB_PATH from the real
    ~/.athena, so tests read live production sessions instead of a tempdir.
    Reading the source is not an option here (banned), so assert on behavior:
    the gate must work with ``athena_state`` absent from ``sys.modules`` and
    ...
    def guard(name, *args, **kwargs):
        if name == "athena_state" or name.startswith("athena_state."):
            blocked.append(name)
            raise AssertionError(
                "conftest._wal_is_usable imported athena_state — this caches "
                "DEFAULT_DB_PATH from the real ~/.athena during collection"
            )
        return real_import(name, *args, **kwargs)

    saved = sys.modules.pop("athena_state", None)
    ...
    try:
        ...
        if saved is not None:
            sys.modules["athena_state"] = saved
    assert not blocked
