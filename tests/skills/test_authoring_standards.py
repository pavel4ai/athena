...
def test_required_frontmatter_fields(p):
    ...
    athena = (fm.get("metadata") or {}).get("athena") or {}
    if not (athena.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.athena.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    athena = (fm.get("metadata") or {}).get("athena") or {}
    dangling = [
        rs for rs in (athena.get("related_skills") or []) if rs not in _all_names()
    ]
