...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `athena cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # athena_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.athena/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "athena_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    ...
    # real filesystem path ``~/.athena`` (Athena home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "athena should always respond concisely"
...
def test_rebrand_text_replaces_legacy_bot_names():
    ...
    assert mod.rebrand_text("clawdbot prefers tabs") == "athena prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Athena was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "athena uses Python"
...
def test_rebrand_text_preserves_filesystem_path_casing():
    ...
    rewrite to lowercase ``.athena`` (the real Athena home), not the broken
    ``.Athena``.
    ...
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.athena/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .athena directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.athena'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "athena config path: ~/.athena/"
