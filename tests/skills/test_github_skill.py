...
def test_frontmatter_required_fields():
    ...
    athena = fm["metadata"]["athena"]
    assert athena["tags"]
    assert "related_skills" in athena
...
def test_author_credits_human_first():
    fm, _ = _frontmatter_and_body()
    assert not fm["author"].startswith("Athena Agent"), "human contributor must be credited first"
...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["athena"]["related_skills"]:
        hits = (
