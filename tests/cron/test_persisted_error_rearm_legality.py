...
def cron_store(tmp_path, monkeypatch):
    ...
    (athena_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))
    monkeypatch.setattr(J, "ATHENA_DIR", athena_home)
    monkeypatch.setattr(J, "CRON_DIR", athena_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", athena_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", athena_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_athena_now", lambda: now):
            due = J.get_due_jobs()
