...
def test_run_job_no_agent_reloads_dotenv_before_script(athena_env, monkeypatch):
    ...
    def fake_load(*, athena_home=None, project_env=None):
        loaded_homes.append(athena_home)
        return []

    monkeypatch.setattr(env_loader, "load_athena_dotenv", fake_load)

    script_path = athena_env / "scripts" / "probe.sh"
    script_path.write_text('#!/bin/bash\necho "ok"\n')
    ...
    assert success is True
    assert error is None
    assert loaded_homes, "load_athena_dotenv was not called on the no_agent path"
    assert str(loaded_homes[0]) == str(athena_env)


def test_timed_out_no_agent_script_delivery_is_not_mislabeled_as_provider_failure(
    athena_env, monkeypatch,
):
    ...
    (athena_env / "scripts" / "slow.py").write_text("import time; time.sleep(999)\n")
    job = create_job(
        prompt=None,
        schedule="every 5m",
        script="slow.py",
...
def test_agent_provider_timeout_delivery_keeps_fallback_guidance(athena_env, monkeypatch):
...
def test_run_job_script_nul_path_fails_cleanly(athena_env):
...
def test_run_job_script_nul_rejected_before_any_path_call(athena_env, monkeypatch):
...
def test_run_job_script_accepts_pathlike_script_path(athena_env):
    ...
    script = athena_env / "scripts" / "probe.py"
...
@pytest.mark.parametrize(
    "error",
    [
        "Script timed out after 900s: /home/u/.athena/scripts/nightly.sh",
