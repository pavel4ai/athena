...
class TestRunJobSessionPersistence:
    ...
    def test_run_job_heartbeats_oneshot_claim_in_both_wait_modes(
        ...
        monkeypatch.setenv("ATHENA_CRON_TIMEOUT", timeout_value)
    ...
    def test_run_job_resets_secret_source_cache_before_reload(self, tmp_path, monkeypatch):
        ...
        this ATHENA_HOME in ``_APPLIED_HOMES``, so the external-secret pull
        ...
        ``load_athena_dotenv`` (which then re-applies external secret sources).
        """
        ...
        with patch("cron.scheduler._athena_home", tmp_path), \
             patch("cron.scheduler_delivery._resolve_origin", return_value=None), \
             patch("athena_cli.env_loader.reset_secret_source_cache", _record_reset), \
             patch("athena_cli.env_loader.load_athena_dotenv", _record_load), \
             patch("athena_state_registry.acquire", return_value=fake_db), \
             patch(
                 "athena_cli.runtime_provider.resolve_runtime_provider",
                 return_value={
...
class TestFailureStreakNudge:
    ...
    def test_nudges_at_threshold(self):
        ...
        assert "athena cron pause scout" in out
