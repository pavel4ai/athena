...
- Path resolution (absolute, relative to ATHENA_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp ATHENA_HOME."""
    athena_home = tmp_path / ".athena"
    athena_home.mkdir()
    (athena_home / "cron").mkdir()
    (athena_home / "cron" / "output").mkdir()
    (athena_home / "scripts").mkdir()
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "ATHENA_DIR", athena_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", athena_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", athena_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", athena_home / "cron" / "output")
...
class TestRunJobScript:
    ...
    def test_script_subprocess_env_sanitized(self, cron_env, monkeypatch):
        """Cron scripts must not inherit Athena provider env (SECURITY.md §2.3)."""
        ...
        blocked_var = sorted(_ATHENA_PROVIDER_ENV_BLOCKLIST)[0]
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.athena/scripts/ must be rejected."""
        from cron.scheduler_script import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("ATHENA_SESSION_PLATFORM") is None
        assert os.environ.get("ATHENA_SESSION_CHAT_ID") is None
        assert os.environ.get("ATHENA_SESSION_CHAT_NAME") is None
...
class TestScriptTimeoutTreeKill:
    ...
    def test_timeout_leaves_no_setsid_grandchild(self, cron_env, monkeypatch):
        ...
        monkeypatch.setenv("ATHENA_CRON_SCRIPT_TIMEOUT", "2")
