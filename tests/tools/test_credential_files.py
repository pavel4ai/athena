...
class TestRegisterCredentialFiles:
    ...
    def test_dict_with_name_key_fallback(self, tmp_path):
        ...
        athena_home = tmp_path / ".athena"
        athena_home.mkdir()
        (athena_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        athena_home = tmp_path / ".athena"
        athena_home.mkdir()
        (athena_home / "secret.key").write_text("key")
...
class TestPathTraversalSecurity:
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside ATHENA_HOME must still be registered."""
        athena_home = tmp_path / ".athena"
        athena_home.mkdir()
        monkeypatch.setenv("ATHENA_HOME", str(athena_home))
        (athena_home / "token.json").write_text('{"token": "abc"}')
...
class TestCacheDirectoryMounts:
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.athena/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        athena_home = tmp_path / ".athena"
        athena_home.mkdir()
        # Use legacy dir name — get_athena_dir prefers old if it exists
        (athena_home / "document_cache").mkdir()
        (athena_home / "image_cache").mkdir()
        monkeypatch.setenv("ATHENA_HOME", str(athena_home))
...
class TestMapCachePathToContainer:
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.athena")
            == "/home/agent/.athena/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        athena_home = tmp_path / ".athena"
        (athena_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("ATHENA_HOME", str(athena_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(athena_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        athena_home = tmp_path / ".athena"
        ss_dir = athena_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.athena/cache/screenshots/session_abc/screen1.png"
