...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        athena_home.mkdir()
        (athena_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"ATHENA_HOME": str(athena_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(athena_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.athena/token.json"
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        athena_home = tmp_path / ".athena"
        athena_home.mkdir()
        (athena_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.athena/skills"


    def test_custom_container_base(self, tmp_path):
        athena_home = tmp_path / ".athena"
        (athena_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"ATHENA_HOME": str(athena_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.athena")

        assert mounts[0]["container_path"] == "/home/user/.athena/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.athena/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.athena/skills/cat/myskill/scripts/run.sh" in paths
    ...
    def test_skips_excluded_bookkeeping_dirs(self, tmp_path):
        ...
        paths = {f["container_path"] for f in files}
        assert "/root/.athena/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.athena/skills/cat/myskill/references/api.md" in paths
        for excluded in (
            ".hub",
            ".archive",
            ".curator_backups",
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape ATHENA_HOME."""
        monkeypatch.setenv("ATHENA_HOME", str(tmp_path / ".athena"))
        (tmp_path / ".athena").mkdir()

        # Create a sensitive file one level above athena_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside athena_home
    ...
    def test_nested_subdir_inside_athena_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of ATHENA_HOME must be allowed."""
        athena_home = tmp_path / ".athena"
        athena_home.mkdir()
        subdir = athena_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside ATHENA_HOME pointing outside must be rejected."""
        athena_home = tmp_path / ".athena"
        athena_home.mkdir()
        monkeypatch.setenv("ATHENA_HOME", str(athena_home))

        # Create a sensitive file outside athena_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside athena_home pointing outside
        symlink = athena_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes ATHENA_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, athena_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape ATHENA_HOME."""
        athena_home = tmp_path / ".athena"
        athena_home.mkdir()
        monkeypatch.setenv("ATHENA_HOME", str(athena_home))
        ...
        sensitive.write_text("{}")
        self._write_config(athena_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(athena_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside ATHENA_HOME via config must still mount."""
        athena_home = tmp_path / ".athena"
        athena_home.mkdir()
        monkeypatch.setenv("ATHENA_HOME", str(athena_home))

        (athena_home / "oauth.json").write_text("{}")
        self._write_config(athena_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        athena_home = tmp_path / ".athena"
        athena_home.mkdir()
        (athena_home / "cache" / "documents").mkdir(parents=True)
        (athena_home / "cache" / "audio").mkdir(parents=True)
        (athena_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("ATHENA_HOME", str(athena_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.athena/cache/documents" in paths
        assert "/root/.athena/cache/audio" in paths
        assert "/root/.athena/cache/videos" in paths
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_athena_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        athena_home = tmp_path / ".athena"
        athena_home.mkdir()
        # Use legacy dir name with content — get_athena_dir prefers
        # populated old over new.
        legacy_doc = athena_home / "document_cache"
        legacy_img = athena_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(athena_home / "document_cache") in host_paths
        assert str(athena_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.athena/cache/documents" in container_paths
        assert "/root/.athena/cache/images" in container_paths
    ...
    def test_empty_athena_home(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.athena/attachments" in container_paths
        assert "/root/.athena/images" in container_paths
        assert "/root/.athena/cache/images" in container_paths
        for mount in mounts:
    ...
    def test_images_upload_dir_is_mounted(self, tmp_path, monkeypatch):
        ...
        Desktop / clipboard / PDF uploads land in ``ATHENA_HOME/images``, not
        ...
        """
        athena_home = tmp_path / ".athena"
        (athena_home / "images").mkdir(parents=True)
        monkeypatch.setenv("ATHENA_HOME", str(athena_home))

        mounts = get_cache_directory_mounts()
        by_container = {m["container_path"]: m["host_path"] for m in mounts}
        assert "/root/.athena/images" in by_container
        assert by_container["/root/.athena/images"] == str(athena_home / "images")
    ...
    def test_images_upload_file_maps_into_container(self, tmp_path, monkeypatch):
        ...
        athena_home = tmp_path / ".athena"
        (athena_home / "images").mkdir(parents=True)
        upload = athena_home / "images" / "upload_20260722_181019_1.png"
        upload.write_bytes(bytes.fromhex("89504e470d0a1a0a"))
        monkeypatch.setenv("ATHENA_HOME", str(athena_home))

        assert (
            map_cache_path_to_container(str(upload))
            == "/root/.athena/images/upload_20260722_181019_1.png"
        )
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        athena_home = tmp_path / ".athena"
        img_dir = athena_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.athena/cache/images/generated.png"
        )
    ...
    def test_maps_path_even_when_cache_dir_missing(self, tmp_path, monkeypatch):
        ...
        mapped = map_cache_path_to_container(str(athena_home / "cache" / "images" / "x.png"))
        assert mapped == "/root/.athena/cache/images/x.png"
        assert (athena_home / "cache" / "images").is_dir()


class TestToAgentVisiblePathPerBackend:
    """#76577 follow-up: translation covers every backend that relocates the
    Athena cache — not just docker — and skips the ones where the host path
    stays correct (local; singularity auto-binds the host home)."""

    def _staged(self, tmp_path, monkeypatch):
        athena_home = tmp_path / ".athena"
        (athena_home / "attachments").mkdir(parents=True)
        monkeypatch.setenv("ATHENA_HOME", str(athena_home))
        return str(athena_home / "attachments" / "drop.zip")

    def test_docker_maps_to_root_athena(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "docker")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "/root/.athena/attachments/drop.zip"

    def test_ssh_maps_to_tilde_athena(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "ssh")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "~/.athena/attachments/drop.zip"
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        athena_home = tmp_path / ".athena"
        doc_dir = athena_home / "cache" / "documents"
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — ATHENA_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping ATHENA_HOME, but every master
    ...
    def test_master_credential_store_is_refused(self, tmp_path, rel_path):
        ...
        with patch.dict(os.environ, {"ATHENA_HOME": str(home)}):
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.athena/google_token.json" in paths
        assert "/root/.athena/.env" not in paths
