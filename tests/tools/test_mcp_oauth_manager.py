...
def test_athena_provider_subclass_exists():
    """AthenaMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _ATHENA_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _ATHENA_PROVIDER_CLS is not None
    assert issubclass(_ATHENA_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_athena_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns AthenaMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _ATHENA_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("ATHENA_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _ATHENA_PROVIDER_CLS is not None
    assert isinstance(provider, _ATHENA_PROVIDER_CLS)
    assert provider._athena_server_name == "srv"
