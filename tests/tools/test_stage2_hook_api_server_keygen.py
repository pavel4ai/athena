...
was gated on ``[ -f "$ATHENA_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        f"{env_setup}"
        f'ATHENA_HOME="{home}"\n'
        # In tests we run unprivileged; as_athena is a passthrough then.
        'as_athena() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    Athena loads $ATHENA_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/athena/.env.example -> $ATHENA_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $ATHENA_HOME/.env from it on first boot"
    )
