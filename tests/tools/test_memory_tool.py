...
class TestScanMemoryContent:
    ...
    def test_athena_config_mod_blocked(self):
        result = _scan_memory_content("edit .athena/config.yaml to change settings")
        assert "Blocked" in result
        assert "athena_config_mod" in result
        result = _scan_memory_content("update .athena/SOUL.md with new personality")
        assert "Blocked" in result
        assert "athena_config_mod" in result
    ...
    def test_athena_config_mod_no_false_positives(self):
        """Merely mentioning athena config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .athena/config.yaml for settings") is None
        assert _scan_memory_content("Read .athena/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .athena/config.yaml file contains runtime options") is None
