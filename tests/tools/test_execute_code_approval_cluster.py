...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_env._scrub_child_env — broad ATHENA_ prefix dropped,
...
def test_env_scrub_athena_allowlist_and_secret_blocks():
    ...
    env = {
        ...
        "ATHENA_DELEGATED_CHILD_CONTEXT": "1",
    ...
    for kept in (
        ...
        "ATHENA_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "ATHENA_BASE_URL", "ATHENA_INTERACTIVE", "ATHENA_KANBAN_TASK",
