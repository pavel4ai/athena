...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad ATHENA_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: ATHENA_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_athena_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "ATHENA_HOME": "/h", "ATHENA_PROFILE": "p",
        "ATHENA_CONFIG": "/c.yaml", "ATHENA_ENV": "/e",
        # other ATHENA_* → dropped (broad prefix removed)
        "ATHENA_BASE_URL": "https://x", "ATHENA_INTERACTIVE": "1",
        "ATHENA_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("ATHENA_HOME", "ATHENA_PROFILE", "ATHENA_CONFIG", "ATHENA_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "ATHENA_BASE_URL", "ATHENA_INTERACTIVE", "ATHENA_KANBAN_DB",
...
def test_env_scrub_logs_dropped_athena_vars(caplog):
    """Dropping a non-allowlisted, non-secret ATHENA_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset ATHENA_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "ATHENA_HOME": "/h",          # allowlisted → kept, not logged
        "ATHENA_BASE_URL": "https://x",   # dropped → logged
        "ATHENA_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "ATHENA_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "ATHENA_HOME" in out and "PATH" in out
    assert "ATHENA_BASE_URL" not in out and "ATHENA_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "ATHENA_BASE_URL" in msgs and "ATHENA_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "ATHENA_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped ATHENA_* vars."""
    import logging
