...
class TestWriteDenyExactPaths:
    ...
    def test_athena_env(self):
        # ``.env`` under the active ATHENA_HOME (profile-aware, not just
        # ``~/.athena``) must be write-denied. The hermetic test conftest
        # points ATHENA_HOME at a tempdir — resolve via get_athena_home()
        # to match the denylist.
        from athena_constants import get_athena_home
        path = str(get_athena_home() / ".env")
        assert _is_write_denied(path) is True

    def test_athena_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.athena/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "athena_root"
        ...
        # Sanity check: ATHENA_HOME does point to the profile dir, not the root.
        from athena_constants import get_athena_home, get_default_athena_root
...
class TestWriteAllowed:
    ...
    def test_athena_control_files_requested_writable(self):
        from athena_constants import get_athena_home
