...
Older images made /opt/athena/ui-tui and /opt/athena/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV ATHENA_TUI_DIR=/opt/athena/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R athena:athena /opt/athena/ui-tui" not in text
    assert "chown -R athena:athena /opt/athena/node_modules" not in text
