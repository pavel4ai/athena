...
def test_request_review_rejects_unknown_reviewer_without_mutation(monkeypatch, worker_env, tmp_path):
    ...
    (tmp_path / ".athena" / "profiles" / "verifier").mkdir(parents=True)
    with kbc.connect() as conn:
        before = kb.get_task(conn, worker_env)
        before_events = kb.list_events(conn, worker_env)
    monkeypatch.setenv("ATHENA_KANBAN_RUN_ID", str(before.current_run_id))
...
def test_request_review_accepts_installed_profile(monkeypatch, worker_env, tmp_path):
    ...
    (tmp_path / ".athena" / "profiles" / "verifier").mkdir(parents=True)
    with kbc.connect() as conn:
        monkeypatch.setenv("ATHENA_KANBAN_RUN_ID", str(kb.get_task(conn, worker_env).current_run_id))
...
def test_complete_goal_mode_rejected_by_judge(monkeypatch, tmp_path):
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("ATHENA_KANBAN_TASK", goal_task_id)
...
def _make_goal_mode_worker_env(monkeypatch, tmp_path):
    """Set up an isolated ATHENA_HOME with one claimed goal_mode task,
...
def test_unblock_with_pending_parents_returns_todo(monkeypatch, tmp_path):
    ...
    home.mkdir()
    monkeypatch.setenv("ATHENA_HOME", str(home))
    monkeypatch.setenv("ATHENA_PROFILE", "orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
def test_create_subscribes_gateway_session(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("ATHENA_SESSION_USER_ID_ALT", "alt-user-9")
    monkeypatch.setenv("ATHENA_SESSION_CHAT_TYPE", "forum")
...
def allow_private_urls(monkeypatch):
    ...
    Mirrors a user setting ATHENA_ALLOW_PRIVATE_URLS on a private network.
    Resets the url_safety process-lifetime cache on both sides so the
    override neither leaks in nor out of the test.
    """
    from tools import url_safety

    monkeypatch.setenv("ATHENA_ALLOW_PRIVATE_URLS", "true")
...
def default_url_guard(monkeypatch):
    ...
    Clears ATHENA_ALLOW_PRIVATE_URLS and resets url_safety's process-lifetime
    cache on both sides so a prior test's opt-in can't leak in.
    """
    from tools import url_safety

    monkeypatch.delenv("ATHENA_ALLOW_PRIVATE_URLS", raising=False)
    url_safety._reset_allow_private_cache()
    yield
    url_safety._reset_allow_private_cache()
