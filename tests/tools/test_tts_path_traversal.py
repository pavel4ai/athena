...
def test_output_path_rejects_athena_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    athena_home = tmp_path / "athena-home"
    athena_home.mkdir()
    monkeypatch.setattr(file_safety, "_athena_home_path", lambda: athena_home)
    monkeypatch.setattr(file_safety, "_athena_root_path", lambda: athena_home)

    target = athena_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    athena_home = tmp_path / "athena-home"
    token_dir = athena_home / "mcp-tokens"
