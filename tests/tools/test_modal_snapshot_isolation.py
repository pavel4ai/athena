...
def _restore_tool_modules():
    original_athena_home = os.environ.get("ATHENA_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_athena_home is None:
            os.environ.pop("ATHENA_HOME", None)
        else:
            os.environ["ATHENA_HOME"] = original_athena_home
        _reset_modules(("tools", "athena_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    athena_cli = types.ModuleType("athena_cli")
    athena_cli.__path__ = [str(REPO_ROOT / "athena_cli")]  # type: ignore[attr-defined]
    sys.modules["athena_cli"] = athena_cli
    athena_home = tmp_path / "athena-home"
    os.environ["ATHENA_HOME"] = str(athena_home)
    sys.modules["athena_cli.config"] = types.SimpleNamespace(
        get_athena_home=lambda: athena_home,
    )
    ...
    return {
        "snapshot_store": athena_home / "modal_snapshots.json",
