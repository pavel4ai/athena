...
def test_dockerfile_makes_opt_athena_readonly_for_athena_user() -> None:
    ...
    assert "chown -R root:root /opt/athena" not in text
    assert "chmod -R a+rX /opt/athena" not in text
    assert "chmod -R a-w /opt/athena" not in text
...
def test_dockerfile_bakes_code_scoped_install_method_stamp() -> None:
    ...
    The stamp is created by root in the shim-wiring RUN block; the athena
...
def test_dockerfile_redirects_lazy_installs_to_durable_target() -> None:
    ...
    # never under the immutable /opt/athena tree.
    assert f"ENV ATHENA_LAZY_INSTALL_TARGET={target}" in text
    assert target.startswith("/opt/data/"), "target must be on the durable volume"
    assert "ENV ATHENA_LAZY_INSTALL_TARGET=/opt/athena" not in text
    ...
    # succeed as the unprivileged athena runtime user.
    stage2 = (REPO_ROOT / "docker" / "stage2-hook.sh").read_text()
    assert '"$ATHENA_HOME/lazy-packages"' in stage2, (
...
def test_dockerfile_bakes_photon_sidecar_deps() -> None:
    ...
    assert not re.search(
        r"chown\s+-R\s+athena:athena\s+/opt/athena/plugins", text
    )
