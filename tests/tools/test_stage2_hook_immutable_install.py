...
Hosted/container Athena keeps user-writable state under ATHENA_HOME
(/opt/data). The installed source, venv, TUI bundle, and node_modules under
/opt/athena must remain root-owned/non-writable by the runtime athena user so
...
def test_stage2_does_not_chown_install_tree_to_athena(stage2_text: str) -> None:
    ...
    for install_tree in (
        ...
        assert install_tree not in stage2_text, (
            f"stage2 must not chown {install_tree} back to athena; "
            "the Dockerfile keeps /opt/athena immutable and writable state "
            "belongs under ATHENA_HOME"
        )
...
def test_stage2_documents_immutable_install_contract(stage2_text: str) -> None:
    ...
    assert "ATHENA_DISABLE_LAZY_INSTALLS=1" in stage2_text
    assert "/opt/athena" in stage2_text
