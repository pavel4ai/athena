...
@pytest.mark.asyncio
async def test_athena_provider_forwards_asend_values(tmp_path, monkeypatch):
    ...
    from tools.mcp_oauth_manager import _ATHENA_PROVIDER_CLS, reset_manager_for_tests

    assert _ATHENA_PROVIDER_CLS is not None, "SDK OAuth types must be available"
...
@pytest.mark.asyncio
async def test_athena_provider_forwards_401_triggers_refresh(tmp_path, monkeypatch):
    ...
    assert _ATHENA_PROVIDER_CLS is not None
...
async def test_long_lived_resource_request_does_not_block_concurrent_post(
    ...
    acquiring its Bearer token.  Athena narrows that lock around resource I/O
