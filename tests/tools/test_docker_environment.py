...
def test_run_command_tags_task_and_profile_labels(monkeypatch):
    ...
    labels = _labels_in_run_args(_run_args_from_calls(calls))
    assert "athena-task-id=kanban-42" in labels, (
        f"athena-task-id=kanban-42 missing; got: {sorted(labels)}"
    )
    assert "athena-profile=research-bot" in labels, (
        f"athena-profile=research-bot missing; got: {sorted(labels)}"
    )
...
def test_reap_orphan_scopes_to_profile_filter_via_label(monkeypatch):
    """The reaper must pass ``--filter label=athena-profile=<profile>`` to
    ...
    assert "label=athena-profile=research-bot" in flat, (
...
def test_credential_mount_works_when_source_is_valid_file(monkeypatch, tmp_path):
    ...
    fake_mounts = [
        {"host_path": str(valid_file), "container_path": "/root/.athena/token.json"},
    ]
    monkeypatch.setattr(
        "tools.credential_files.get_credential_file_mounts",
        lambda: fake_mounts,
...
def test_is_container_gone_matches_removal_errors(monkeypatch):
    ...
    assert env._is_container_gone(
        "Error response from daemon: No such container: athena-abc123"
    )
...
def test_execute_recovers_from_out_of_band_removal(monkeypatch):
    ...
    outputs = iter([
        {"output": "Error response from daemon: No such container: athena-x", "returncode": 1},
