...
See: https://github.com/pavel4ai/athena/issues/1002
See: https://github.com/pavel4ai/athena/issues/1264
"""
...
class TestAthenaBinDirOnPath:
    ...
    def test_resolves_via_sys_executable_dir(self, monkeypatch, tmp_path):
        ...
        (venv_bin / "athena").write_text("#!/bin/sh\n")
        ...
        assert local_mod._resolve_athena_bin_dir() == str(venv_bin)
    ...
    def test_returns_none_when_unresolvable(self, monkeypatch):
        ...
        assert local_mod._resolve_athena_bin_dir() is None

    def test_prepend_adds_missing_dir_at_front(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        local_mod._ATHENA_BIN_DIR = "/opt/athena/bin"
        out = local_mod._prepend_athena_bin_dir("/usr/bin:/bin")
        assert out.split(os.pathsep)[0] == "/opt/athena/bin"
    ...
    def test_prepend_is_idempotent(self, monkeypatch):
        ...
        self._reset_cache()
        local_mod._ATHENA_BIN_DIR = "/opt/athena/bin"
        once = local_mod._prepend_athena_bin_dir("/usr/bin:/bin")
        twice = local_mod._prepend_athena_bin_dir(once)
        assert twice == once
        assert once.split(os.pathsep).count("/opt/athena/bin") == 1
    ...
    def test_make_run_env_injects_athena_bin_dir(self, monkeypatch):
