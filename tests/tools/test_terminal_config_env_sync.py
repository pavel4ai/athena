...
at startup, by THREE separate code paths:
  ...
  2. gateway/run.py    -> ``_terminal_env_map`` dict (gateway / messaging
                          platforms)
  3. athena_cli/config.py:set_config_value
...
dicts) keeps the test independent of the user's ~/.athena/config.yaml and
mirrors the pattern used in tests/athena_cli/test_config_drift.py.
"""
...
def _save_config_env_sync_keys() -> set[str]:
    ...
    """
    from athena_cli import config as hc_config
...
def test_save_config_set_supports_critical_bridged_keys():
    ...
    assert not missing, (
        ...
        f"in athena_cli/config.py (set_config_value bridges through it)."
    )
...
def test_docker_extra_args_is_bridged_everywhere():
    ...
    present in DEFAULT_CONFIG, TERMINAL_CONFIG_ENV_MAP (so ``athena config
    ...
    producing the "Athena partially reads the Docker config" symptom.  Guard
...
def test_docker_snap_compat_is_bridged_everywhere():
    ...
    ``athena config set`` paths, like every other docker_* key (see docker_extra_args above)."""
