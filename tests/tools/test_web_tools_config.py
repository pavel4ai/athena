...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_athena_home_override(self, tmp_path):
        """Auth lookup should read from ATHENA_HOME/auth.json, not ~/.athena/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".athena").mkdir(parents=True)
...
class TestCheckWebApiKey:
    ...
    def test_tool_gateway_availability_skips_refresh_for_expired_cached_token(
        ...
        monkeypatch.setattr(
            "athena_cli.auth.resolve_nous_access_token",
            _record_refresh,
        )
