...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(athena_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(athena_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_athena_bin(self):
        """After _INSTALL_FAILED, manual install in ATHENA_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        athena_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(athena_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(athena_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._athena_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == athena_bin
            assert _tirith_mod._resolved_path == athena_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# ATHENA_HOME isolation
# ---------------------------------------------------------------------------

class TestAthenaHomeIsolation:
    def test_athena_bin_dir_respects_athena_home(self):
        """_athena_bin_dir must use ATHENA_HOME, not hardcoded ~/.athena."""
        from tools.tirith_security import _athena_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"ATHENA_HOME": tmpdir}):
            result = _athena_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_athena_home(self):
        """_failure_marker_path must use ATHENA_HOME, not hardcoded ~/.athena."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"ATHENA_HOME": "/custom/athena"}):
            result = _failure_marker_path()
        assert result == "/custom/athena/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets ATHENA_HOME; verify it's active."""
        athena_home = os.getenv("ATHENA_HOME")
        assert athena_home is not None, "ATHENA_HOME should be set by conftest"
        assert "athena_test" in athena_home, "Should point to test temp dir"

    def test_get_athena_home_fallback(self):
        """Without ATHENA_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_athena_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove ATHENA_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".athena")
            result = _get_athena_home()
