...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    PID 1 and athena would run without zombie reaping.
    """
    ...
    assert "/opt/athena/docker/entrypoint-dispatch.sh" in entrypoint_line, (
    ...
    assert "exec /init /opt/athena/docker/main-wrapper.sh" in dispatcher, (
