...
1. `athena secrets bitwarden setup --help` works (dispatch path)
2. `athena update --check` works (update path)
3. `athena secrets bitwarden disable` works (handler execution)
4. `athena secrets onepassword status` works (lazy backend loads on demand)
...
def _run_athena(args: list[str], timeout: int = 30) -> subprocess.CompletedProcess[str]:
    """Run athena CLI as a subprocess from repo root.
    ...
    connectivity — but ``athena update --check`` really does ``git fetch``
    ...
    env.update(
        {
            ...
            "GIT_CONFIG_KEY_0": "url.file:///nonexistent-athena-test-remote/.insteadOf",
            "GIT_CONFIG_VALUE_0": "https://",
            "GIT_CONFIG_KEY_1": "url.file:///nonexistent-athena-test-remote/.insteadOf",
...
class TestSecretsDispatchE2E:
    ...
    def test_bitwarden_status(self) -> None:
        """`athena secrets bitwarden status` must exit 0 (runs lazy backend)."""
        result = _run_athena(["secrets", "bitwarden", "status"])
    ...
    def test_bitwarden_disable(self) -> None:
        """`athena secrets bitwarden disable` must exit 0."""
        result = _run_athena(["secrets", "bitwarden", "disable"])
        assert result.returncode == 0, (
    ...
    def test_onepassword_status(self) -> None:
        """`athena secrets onepassword status` must exit 0 (1Password lazy backend)."""
        result = _run_athena(["secrets", "onepassword", "status"])
        assert result.returncode in (0, 1), (
    ...
    def test_onepassword_setup_help(self) -> None:
        """`athena secrets onepassword setup --help` must exit 0."""
        result = _run_athena(["secrets", "onepassword", "setup", "--help"])
...
class TestUpdatePathE2E:
    ...
    These tests invoke the real `athena update --check` path as a subprocess.
    ...
    def test_update_check_clean(self) -> None:
        """`athena update --check` must not load cryptography._rust."""
        result = _run_athena(["update", "--check"])
    ...
    def test_main_update_check_crypto_absent_in_sys_modules(self) -> None:
        """Decisive invariant: invoking main() with argv=['athena','update','--check']
...
sys.argv = ['athena', 'update', '--check']

import athena_cli.main as m
...
# resolves the athena_cli.main module's lazily re-exported attribute —
# so the patch must land on update_cmd._cmd_update_check.
with patch('athena_cli.update_cmd._cmd_update_check', capture_update_check):
    try:
        m.main()
