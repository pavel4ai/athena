...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.athena/honcho.json
        monkeypatch.setenv("ATHENA_HOME", str(tmp_path / "isolated"))
...
class TestResolveConfigPath:
    def test_prefers_athena_home_when_exists(self, tmp_path):
        athena_home = tmp_path / "athena"
        athena_home.mkdir()
        local_cfg = athena_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: ATHENA_HOME points at ~/.athena/profiles/<name>, so
        # _get_default_athena_home() must resolve back to ~/.athena — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".athena"
        ...
        assert _get_default_athena_home() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_athena_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".athena".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".athena"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "athena_coder"
        assert profile_host_key("default") == "athena"

    def test_default_returns_athena(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("ATHENA_HONCHO_HOST", None)
            os.environ.pop("ATHENA_HOME", None)
            assert resolve_active_host() == "athena"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"ATHENA_HONCHO_HOST": "athena.coder"}):
            assert resolve_active_host() == "athena.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ATHENA_HONCHO_HOST", None)
            with patch("athena_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "athena_coder"

    def test_default_profile_returns_athena(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ATHENA_HONCHO_HOST", None)
            with patch("athena_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "athena"

    def test_custom_profile_returns_athena(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ATHENA_HONCHO_HOST", None)
            with patch("athena_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "athena"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ATHENA_HONCHO_HOST", None)
            # Temporarily remove athena_cli.profiles to simulate import failure
            saved = sys.modules.get("athena_cli.profiles")
            sys.modules["athena_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "athena"
            finally:
                if saved is not None:
                    sys.modules["athena_cli.profiles"] = saved
                else:
                    sys.modules.pop("athena_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="athena_coder")
        assert config.host == "athena_coder"
        assert config.workspace_id == "athena"  # shared workspace
        assert config.ai_peer == "athena_coder"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config = HonchoClientConfig.from_global_config(
            host="athena_coder", config_path=config_file,
        )
        assert config.host == "athena_coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "athena_dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="athena_dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "athena_dreamer"
    ...
    def test_from_global_config_reads_legacy_dot_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "athena.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        config = HonchoClientConfig.from_global_config(
            host="athena_dreamer",
        ...
        assert config.host == "athena_dreamer"
        assert config.peer_name == "dreamer-user"
        assert config.workspace_id == "athena_dreamer"
...
class TestGetHonchoClient:
    ...
    def test_athena_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="athena",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("athena_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_athena_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="athena",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("athena_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
