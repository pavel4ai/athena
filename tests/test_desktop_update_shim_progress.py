...
def progress(tmp_path):
    ...
    status = tmp_path / "athena-update-status"
...
def test_every_state_carries_a_clock(progress):
    ...
    for state, message in [
        ...
        ("done", ""),
        ("manual", "Reopen Athena to finish."),
        ("error", "Update failed."),
    ]:
...
# Stands in for `athena update`, and reports the stage that was on screen
...
n="$(cat "$ATHENA_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$ATHENA_TEST_CALLS"
for f in "$TMPDIR"/athena-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$ATHENA_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$ATHENA_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    athena = install_root / "venv" / "bin" / "athena"
    athena.write_text(FAKE_ATHENA)
    athena.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "ATHENA_TEST_CAPTURE": str(capture),
        "ATHENA_TEST_CALLS": str(calls),
        "ATHENA_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".athena-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`athena update` is the longest wait in the hand-off and the one the
