...
Older versions of ``install.sh`` created ``$command_link_dir/athena`` as a
symlink to the pip-generated entry point at ``$ATHENA_BIN`` (i.e.
``venv/bin/athena``). When ``setup_path()`` later switched to writing a bash
shim with ``cat > "$command_link_dir/athena" <<EOF``, the redirect followed
the existing symlink and overwrote the pip entry point with the shim. The
shim's ``exec "$ATHENA_BIN" "$@"`` then self-recursed and ``athena`` hung on
every invocation.

These tests pin the fix: ``setup_path()`` must remove ``$command_link_dir/athena``
...
def _extract_setup_path_shim_block() -> str:
    ...
    match = re.search(
        r"(?P<block>mkdir -p \"\$command_link_dir\".*?chmod \+x \"\$command_link_dir/athena\")",
        text,
        re.DOTALL,
    )
...
def test_setup_path_shim_block_removes_old_link_before_writing() -> None:
    ...
    rm_idx = block.find('rm -f "$command_link_dir/athena"')
    cat_idx = block.find('cat > "$command_link_dir/athena" <<EOF')
    assert rm_idx != -1, (
        "setup_path() must `rm -f` $command_link_dir/athena before the "
...
def test_re_running_setup_path_block_preserves_pip_entry_point(tmp_path: Path) -> None:
    ...
    Layout mirrors a real install:
        ...
        tmp/
          venv/bin/athena        <- pip entry point (the one we must preserve)
          local_bin/athena       <- symlink → ../venv/bin/athena  (old install)

    Then we run the exact shim-write block from setup_path() with
    ``ATHENA_BIN`` and ``command_link_dir`` pointed at this fixture. The fix
    requires that, after the run:

      * ``venv/bin/athena`` still contains its original pip-script body
      * ``local_bin/athena`` is a regular file (not a symlink) holding the shim
    """
    ...
    assert pip_entry.read_text() == pip_marker, (
        "venv/bin/athena was overwritten by setup_path() — symlink-stomp "
        "regression (#21454)."
    )
    ...
    assert not shim_path.is_symlink(), (
        "command_link_dir/athena must be replaced with a regular file, not "
