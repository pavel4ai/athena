...
The ``athena update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m athena_cli.main gateway run`` — image name
    ``pythonw``, not ``athena.exe`` — so the ``taskkill /IM athena.exe`` guard
    ...
    # The athena.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM athena.exe' in text
