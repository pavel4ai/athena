...
def _real_finalize(cli_obj):
    """Call the real AthenaCLI.finalize_preloaded_skills on a dummy object."""
    return _REAL_FINALIZE(cli_obj)


def _capture_real_finalize():
    import cli as cli_mod
    return cli_mod.AthenaCLI.__dict__["finalize_preloaded_skills"]
...
def test_show_banner_does_not_print_skills():
    ...
    with patch("athena_cli.banner.build_welcome_banner") as mock_banner, patch(
