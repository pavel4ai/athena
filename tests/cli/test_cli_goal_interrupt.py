...
def athena_home(tmp_path, monkeypatch):
    """Isolated ATHENA_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".athena"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("ATHENA_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves ATHENA_HOME each test.
...
class TestInterruptAutoPause:
    def test_interrupted_turn_pauses_goal_and_skips_continuation(self, athena_home):
        ...
        with patch("athena_cli.goals.judge_goal") as judge_mock:
    ...
    def test_interrupted_turn_is_resumable(self, athena_home):
        ...
        with patch("athena_cli.goals.judge_goal"):
            cli._maybe_continue_goal_after_turn()
...
class TestEmptyResponseSkip:
    def test_empty_response_does_not_invoke_judge(self, athena_home):
    ...
    def test_no_assistant_message_skipped(self, athena_home):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, athena_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, athena_home):
