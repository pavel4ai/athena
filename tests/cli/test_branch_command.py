...
def session_db(tmp_path):
    ...
    os.environ["ATHENA_HOME"] = str(tmp_path / ".athena")
    os.makedirs(tmp_path / ".athena", exist_ok=True)
    from athena_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".athena" / "test_sessions.db")
    yield db
    db.close()
...
class TestBranchCommandCLI:
    ...
    def test_branch_rotates_athena_session_id_env_and_context(self, cli_instance, session_db):
        """Branching must update process-local session-id readers too."""
        from cli import AthenaCLI
        from gateway.session_context import _UNSET, _VAR_MAP, get_session_env

        old_session_id = cli_instance.session_id
        os.environ["ATHENA_SESSION_ID"] = old_session_id
        _VAR_MAP["ATHENA_SESSION_ID"].set(old_session_id)

        try:
            AthenaCLI._handle_branch_command(cli_instance, "/branch")

            assert cli_instance.session_id != old_session_id
            assert os.environ["ATHENA_SESSION_ID"] == cli_instance.session_id
            assert get_session_env("ATHENA_SESSION_ID") == cli_instance.session_id
        finally:
