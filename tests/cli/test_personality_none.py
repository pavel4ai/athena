...
class TestGatewayPersonalityNone:
    ...
    async def test_empty_personality_list_uses_profile_display_path(self, tmp_path):
        ...
        with patch("gateway.run._athena_home", tmp_path), \
             patch("athena_constants.display_athena_home", return_value="~/.athena/profiles/coder"):
            event = self._make_event("")
            result = await runner._handle_personality_command(event)

        assert result == "No personalities configured in `~/.athena/profiles/coder/config.yaml`"
