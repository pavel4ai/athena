...
class TestSaveConfigValueAtomic:
    ...
    def config_env(self, tmp_path, monkeypatch):
        ...
        # save_config_value resolves the target live via get_athena_home(), so
        # point ATHENA_HOME at the temp dir (the _athena_home import-time
        # constant is no longer consulted).
        monkeypatch.setenv("ATHENA_HOME", str(athena_home))
        monkeypatch.setattr("cli._athena_home", athena_home)
        return config_path
    ...
    def test_model_write_runs_shared_cron_drift_warning(self, config_env, monkeypatch):
        warning = MagicMock()
        monkeypatch.setattr(
            "athena_cli.config.warn_unpinned_cron_jobs_after_model_config_change",
            warning,
        )
...
class TestSaveConfigValueTargetsUserConfig:
    """Regression: persisted runtime settings must land in ATHENA_HOME/config.yaml
    ...
    install whose ATHENA_HOME/config.yaml did not exist yet, save_config_value
    fell back to the checked-in cli-config.yaml. The toggle reported success, but
    startup read ATHENA_HOME/config.yaml and never saw the setting."""

    def test_creates_user_config_when_absent(self, tmp_path, monkeypatch):
        # Fresh ATHENA_HOME with NO config.yaml (managed/desktop first launch).
        athena_home = tmp_path / ".athena"
        athena_home.mkdir()
        monkeypatch.setenv("ATHENA_HOME", str(athena_home))
