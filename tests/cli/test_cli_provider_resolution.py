...
import athena_cli.main_provider_setup as athena_cli_main_provider_setup
from athena_cli import model_setup_flows
...
def test_provider_flag_uses_named_custom_default_model(monkeypatch):
    ...
    monkeypatch.setattr("athena_cli.config.load_config", lambda: config)
    monkeypatch.setattr("athena_cli.runtime_provider.load_config", lambda: config)

    shell = cli.AthenaCLI(provider="gmk-lan", compact=True, max_turns=1)
...
def test_provider_flag_logs_when_custom_default_model_cannot_resolve(monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        "athena_cli.runtime_provider._get_named_custom_provider",
        _boom,
    )
...
def test_model_flow_nous_does_not_restore_stale_custom_api_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("athena_cli.models_pricing.get_pricing_for_provider", lambda provider: {})
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    ...
    monkeypatch.setattr("athena_cli.main_provider_setup._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr(
        "athena_cli.config.save_env_value",
        lambda key, value: saved_env.__setitem__(key, value),
    )
    monkeypatch.setattr(
        "athena_cli.main_provider_setup._save_custom_provider",
...
def test_cmd_model_forwards_nous_login_tls_options(monkeypatch):
    ...
    monkeypatch.setattr(athena_cli_main_provider_setup, "_prompt_provider_choice", lambda choices, **kwargs: 0)
...
def test_auto_provider_name_localhost():
    from athena_cli.main_provider_setup import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from athena_cli.main_provider_setup import _save_custom_provider
...
def test_save_custom_provider_references_the_key_instead_of_inlining_it(monkeypatch, tmp_path):
    ...
    _save_custom_provider(
        ...
        api_key="sk-secret",
        name="Ollama",
        key_env="ATHENA_CUSTOM_LOCALHOST_11434_API_KEY",
    )

    entry = saved["custom_providers"][0]
    assert entry["key_env"] == "ATHENA_CUSTOM_LOCALHOST_11434_API_KEY"
...
def test_custom_endpoint_key_env_is_a_valid_posix_name_for_ip_endpoints():
    ...
    from athena_cli.config import _ENV_VAR_NAME_RE, custom_endpoint_key_env
