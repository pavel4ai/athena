...
Override the image with ``ATHENA_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("ATHENA_TEST_IMAGE", "athena-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``ATHENA_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("ATHENA_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged athena user
# ---------------------------------------------------------------------------
#
# Background: every Athena runtime path inside the container drops to UID
# 10000 (the ``athena`` user) via ``s6-setuidgid athena``. ``docker exec``
...
# fails (PermissionError swallowed) for athena, so a test that ran as root
...
# helpers below run every probe under ``-u athena`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: athena).
