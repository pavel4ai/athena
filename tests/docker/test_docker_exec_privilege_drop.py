...
The shim (docker/athena-exec-shim.sh, installed at /opt/athena/bin/athena)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> athena login` would write /opt/data/auth.json as
...
1. ``docker exec <c> athena …`` (defaulting to root) gets dropped to the
   athena user before the real binary runs.
2. ``docker exec --user athena <c> athena …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $ATHENA_HOME from a ``docker exec`` session land
   as athena:athena — the actual user-visible invariant.
4. The ATHENA_DOCKER_EXEC_AS_ROOT opt-out lets diagnostic sessions keep
...
def test_shim_drops_root_to_athena_uid(sleep_container: str) -> None:
    ...
    pure-athena "print my uid" command, so we use the venv's python directly
    ...
    Use `athena config set` which writes config.yaml under ATHENA_HOME.
    ...
    # The written file must be owned by athena, not root.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    assert r.stdout.strip() == "athena:athena", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected athena:athena. "
        "The shim did not drop privileges before invoking athena."
    )


def test_shim_short_circuits_for_non_root_exec(sleep_container: str) -> None:
    """docker exec --user athena already runs as 10000; shim should be a no-op.
    ...
    assert r.returncode == 0, (
        f"docker exec --user athena failed: {r.stderr!r} stdout={r.stdout!r}. "
        "If the shim mis-handled the non-root path, this would fail with EPERM."
    )

    # File still ends up athena:athena — orthogonally confirms uid.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "stat", "-c", "%U:%G", "/opt/data/config.yaml"],
        capture_output=True, text=True, timeout=10,
...
def test_shim_opt_out_keeps_root(sleep_container: str) -> None:
    """ATHENA_DOCKER_EXEC_AS_ROOT=1 should suppress the privilege drop.
    ...
    r = subprocess.run(
        ["docker", "exec",
         "-e", "ATHENA_DOCKER_EXEC_AS_ROOT=1",
         sleep_container,
    ...
    assert r.stdout.strip() == "root:root", (
        f"With ATHENA_DOCKER_EXEC_AS_ROOT=1, expected root:root, "
...
def test_shim_opt_out_strict_truthiness(
    ...
    Strict truthiness so a typo (``ATHENA_DOCKER_EXEC_AS_ROOT=0``) doesn't
    silently keep the user as root. Mirrors the policy used by
    ``ATHENA_GATEWAY_NO_SUPERVISE`` in #33583.
    """
    subprocess.run(
        ["docker", "exec", "--user", "root", sleep_container,
         "rm", "-f", "/opt/data/config.yaml"],
    ...
    r = subprocess.run(
        ["docker", "exec",
         "-e", f"ATHENA_DOCKER_EXEC_AS_ROOT={falsy_value}",
         sleep_container,
    ...
    assert r.stdout.strip() == "athena:athena", (
        ...
        f"file owner is {r.stdout.strip()!r}, expected athena:athena"
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/athena/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid athena athena <args>` which resolves `athena`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> athena login`` (root) wrote
    ...
    `docker exec` path produces is athena-readable.

    Specifically: pretend the operator ran `athena login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the athena UID. We use `athena auth list` since that touches the
    ...
    # `athena auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `athena config set` first to
    # provoke a write into ATHENA_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    # ATHENA_HOME that docker exec just wrote.
    r = subprocess.run(
        ["docker", "exec", "--user", "athena", sleep_container,
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the athena user "
