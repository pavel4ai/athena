...
def _wait_for_cont_init(container: str) -> None:
    ...
    ``01-athena-setup`` cont-init hook (docker/stage2-hook.sh) has
    finished seeding + ``chown athena:athena`` config.yaml and running the
    ...
    clobbered the root-written file's ownership back to athena:athena,
    failing ``test_shim_opt_out_keeps_root`` non-deterministically.

    The reliable "cont-init is done" signal is
    ``$ATHENA_HOME/logs/container-boot.log``: it is written by
    ``02-reconcile-profiles`` (athena_cli.container_boot), which s6 runs
    *strictly after* ``01-athena-setup`` in lexicographic order. The
