...
package-lock.json. Without ATHENA_TUI_DIR set, ``_make_tui_argv`` falls
...
The fix is ``ENV ATHENA_TUI_DIR=/opt/athena/ui-tui`` in the Dockerfile, which
...
def _exec_py(image: str, py: str) -> str:
    """Run a Python snippet inside the image as the athena user, return stdout."""
    inner = (
        "source /opt/athena/.venv/bin/activate && "
        "cd /opt/athena && "
        f"python3 -c {shlex.quote(py)}"
    )
    # Drop to the athena user (UID 10000) so we exercise the same path the
...
def test_athena_tui_dir_env_is_set(built_image: str) -> None:
    """ATHENA_TUI_DIR must point at the prebuilt bundle dir in the image."""
    r = subprocess.run(
        ["docker", "run", "--rm", "--entrypoint", "sh", built_image,
         "-c", 'printf "%s" "$ATHENA_TUI_DIR"'],
        capture_output=True, text=True, timeout=60,
    )
    assert r.returncode == 0, r.stderr[-2000:]
    assert r.stdout.strip() == "/opt/athena/ui-tui", (
        f"ATHENA_TUI_DIR={r.stdout.strip()!r} (expected /opt/athena/ui-tui)"
    )
...
def test_prebuilt_bundle_present_and_no_runtime_install(built_image: str) -> None:
    ...
    py = (
        ...
        "from athena_cli.main_tui_launch import _tui_need_npm_install, _find_bundled_tui, _make_tui_argv\n"
        "ui = Path('/opt/athena/ui-tui')\n"
    ...
    # With ATHENA_TUI_DIR set, _make_tui_argv returns the prebuilt path BEFORE
