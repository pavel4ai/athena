"""Runtime smoke test for Docker $ATHENA_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the athena user after container boot.
...
registered later runs its log service as the dropped athena user and
...
def test_logs_gateways_seeded_and_athena_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by athena after boot."""
    ...
    assert "logs=athena" in r.stdout, (
        f"logs/ not owned by athena: {r.stdout}"
    )
    assert "gateways=athena" in r.stdout, (
        f"logs/gateways/ not owned by athena: {r.stdout}"
    )
