...
2. **Isolated ATHENA_HOME.** ATHENA_HOME points to a per-test tempdir so
   code reading ``~/.athena/*`` via ``get_athena_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".athena"`` instead of the canonical
   ``get_athena_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ATHENA_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox ATHENA_HOME before ANY test module is imported ──────────────────
# `athena_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_athena_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.athena/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes ATHENA_HOME, but fixtures run
...
# the REAL Athena root — capture it BEFORE the sandbox rewires ATHENA_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.athena (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("ATHENA_KANBAN_HOME", "").strip()
_PRE_SANDBOX_ATHENA_HOME = os.environ.get("ATHENA_HOME", "")


def _athena_home_points_at_production(value: str) -> bool:
    """True when a pre-set ATHENA_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    ATHENA_HOME=~/.athena``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``athena_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.athena`` — the escape vector that landed pytest
    ...
    ATHENA_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".athena").resolve()
    except Exception:
        return True
...
if _athena_home_points_at_production(os.environ.get("ATHENA_HOME", "")):
    _SESSION_ATHENA_HOME = tempfile.mkdtemp(prefix="athena-test-home-")
    os.environ["ATHENA_HOME"] = _SESSION_ATHENA_HOME
    atexit.register(shutil.rmtree, _SESSION_ATHENA_HOME, True)
...
# like a real CLI") — which used to disarm athena_state's live-DB guard in
# the child at the same moment the child lost the ATHENA_HOME redirect.
# ATHENA_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# athena_state_guard._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export ATHENA_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["ATHENA_TEST_ISOLATION"] = os.environ.get("ATHENA_HOME", "") or "1"

#: ATHENA_HOME as it stood when conftest was imported - i.e. before any test
...
ATHENA_HOME_AT_CONFTEST_IMPORT = os.environ.get("ATHENA_HOME", "")
...
# ATHENA_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_ATHENA_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``athena_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a Athena agent session inherits them and
    # athena_constants home-resolution helpers prefer them over monkeypatched
    # HOME (test_subprocess_home_isolation red locally, green on CI).
    "ATHENA_REAL_HOME",
    "TERMINAL_HOME_MODE",
    "ATHENA_INTERACTIVE",
    "ATHENA_QUIET",
    "ATHENA_TOOL_PROGRESS",
    "ATHENA_TOOL_PROGRESS_MODE",
    "ATHENA_MAX_ITERATIONS",
    "ATHENA_SESSION_PLATFORM",
    "ATHENA_SESSION_CHAT_ID",
    "ATHENA_SESSION_CHAT_NAME",
    "ATHENA_SESSION_CHAT_TYPE",
    "ATHENA_SESSION_THREAD_ID",
    "ATHENA_SESSION_SOURCE",
    "ATHENA_SESSION_KEY",
    "ATHENA_GATEWAY_SESSION",
    "ATHENA_CRON_SESSION",
    "_ATHENA_GATEWAY",
    "ATHENA_PLATFORM",
    "ATHENA_MODEL",
    "ATHENA_INFERENCE_MODEL",
    "ATHENA_INFERENCE_PROVIDER",
    "ATHENA_TUI_PROVIDER",
    "ATHENA_MANAGED",
    "ATHENA_MANAGED_DIR",
    "ATHENA_DEV",
    ...
    "ATHENA_TIMEZONE",
    "ATHENA_REDACT_SECRETS",
    "ATHENA_BACKGROUND_NOTIFICATIONS",
    "ATHENA_EXEC_ASK",
    "ATHENA_HOME_MODE",
    "ATHENA_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.athena/kanban.db instead of the per-test ATHENA_HOME.
    "ATHENA_KANBAN_DB",
    "ATHENA_KANBAN_BOARD",
    "ATHENA_KANBAN_HOME",
    "ATHENA_KANBAN_WORKSPACES_ROOT",
    "ATHENA_KANBAN_LOGS_ROOT",
    "ATHENA_KANBAN_TASK",
    "ATHENA_KANBAN_WORKSPACE",
    "ATHENA_KANBAN_RUN_ID",
    "ATHENA_KANBAN_CLAIM_LOCK",
    "ATHENA_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    "ATHENA_DELEGATED_CHILD_CONTEXT",
    "ATHENA_TENANT",
    ...
    # otherwise-unrelated config tests away from the default "athena" host.
    "ATHENA_HONCHO_HOST",
    ...
    "ATHENA_DASHBOARD_OAUTH_CLIENT_ID",
    "ATHENA_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ATHENA_HOME to per-test tempdirs so code that
    reads ``~/.athena/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral ATHENA_* vars that could change test semantics.
    for name in _ATHENA_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("ATHENA_HONCHO_HOST", "athena")

    # 3. Redirect ATHENA_HOME to a per-test tempdir. Code that reads
    #    ``~/.athena/*`` via ``get_athena_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.athena/*`` via
    #    ``Path.home() / ".athena"`` instead of ``get_athena_home()``
    #    is a bug to fix at the callsite.
    fake_athena_home = tmp_path / "athena_test"
    fake_athena_home.mkdir()
    (fake_athena_home / "sessions").mkdir()
    (fake_athena_home / "cron").mkdir()
    (fake_athena_home / "memories").mkdir()
    (fake_athena_home / "skills").mkdir()
    monkeypatch.setenv("ATHENA_HOME", str(fake_athena_home))
    ...
    # athena_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("ATHENA_TEST_ISOLATION", str(fake_athena_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("ATHENA_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. athena_state computes ``DEFAULT_DB_PATH = get_athena_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from athena_state import ...``) that
    ...
    athena_state_mod = sys.modules.get("athena_state")
    if athena_state_mod is not None and hasattr(athena_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            athena_state_mod, "DEFAULT_DB_PATH", fake_athena_home / "state.db"
        )
    ...
    monkeypatch.setenv("ATHENA_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.athena/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import athena_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path ATHENA_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_athena_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from athena_cli import kanban_db_dispatch as _kbd_mod
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.athena. This autouse
...
# allow-list because test-level fixtures legitimately move ATHENA_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session ATHENA_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``ATHENA_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Athena root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_ATHENA_HOME and not _athena_home_points_at_production(
        _PRE_SANDBOX_ATHENA_HOME
    ):
        # ATHENA_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_athena_root().resolve()
    # No pre-existing ATHENA_HOME: the real root is the platform default,
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.athena`` captured at import time. Hermetic tests that legitimately
    move ATHENA_HOME to sibling tempdirs are unaffected.

    Only patches when ``athena_cli.kanban_db_connect`` is *already imported*
    ...
    """
    _kdb = sys.modules.get("athena_cli.kanban_db")
    _kdbc = sys.modules.get("athena_cli.kanban_db_connect")
    if _kdb is None or _kdbc is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import athena_cli.kanban_db`
    ...
    def _guarded_connect(db_path=None, *args, **kwargs):
        ...
        raise RuntimeError(
            ...
            f"to the real ~/.athena. See #69283."
        )
...
# ``athena_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Athena root. This fixture wires the
# test-side knobs:
...
#     installs where ATHENA_HOME is not ~/.athena) into the guard's
...
# so subprocess children that import athena_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("athena_state")
    ...
    if _PRE_SANDBOX_ATHENA_HOME and not _athena_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_ATHENA_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Athena home override redirects every later
    # ``get_athena_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from athena_constants import get_athena_home_override, set_athena_home_override

        if get_athena_home_override() is not None:
            set_athena_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal athena config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``athena-gateway`` process
...
# correlated with a ``tests/athena_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> athena-gateway``
...
def _wal_is_usable() -> bool:
    """True when Athena will actually put a database into WAL mode here.

    Athena refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Athena can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Athena managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``athena_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_athena_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_athena_home`` fixture
    redirects ``ATHENA_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.athena/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``athena_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["ATHENA_VOICE_TTS"] = "1"``. The
...
#      ``athena_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``athena_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``athena_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``athena_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Athena runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.athena``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Athena falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Athena uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart athena-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting athena/python patterns are also blocked.
    """
    ...
    def _matches_athena_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _ATHENA_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_athena_gateway(cmd_str):
            return False
        try:
            tokens = _shlex.split(cmd_str)
        except ValueError:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch athena-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m athena_cli.main".
                if (
                    "athena" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live athena-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting athena/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `athena update` (or the
        # equivalent `python -m athena_cli.main update`).  These commands
        ...
        if "update" in low and (
            # athena update / athena update --gateway / setsid bash -c ... athena update
            ("athena" in low and "update" in low.split())
            or
            # python -m athena_cli.main update --gateway
            ("athena_cli" in low and "update" in low.split())
            or
            # venv/bin/athena update  (absolute path variant used in tests)
            (".venv/bin/athena" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
        ...
        # Block spawning a REAL gateway runtime (``python -m athena_cli.main
        # gateway run|start|restart``). ``_spawn_athena_action`` launches it
        # with start_new_session=True, so it outlives the pytest worker; the
        # child inherits the pytest-tmp ATHENA_HOME, resolves the DEVELOPER's
        # ``athena-gateway`` systemd unit (a tmp home hashes to no profile
        ...
        # A gateway launched INSIDE a container (`docker exec … athena gateway start`) cannot
        ...
        if (
            ...
            raise RuntimeError(
                ...
                "athena gateway runtime that outlives the test (it is "
                "detached), restarts the developer's live gateway, and "
                "holds the webhook port. Patch the spawn seam where "
                "production reads it (athena_cli.web_server_gateway."
                "_spawn_athena_action), or mark with "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_ATHENA_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import athena_cli.voice as _voice
    except Exception:
