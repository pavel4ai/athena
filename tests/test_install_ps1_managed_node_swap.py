...
def test_managed_node_swap_defines_staged_and_backup_as_siblings() -> None:
    ...
    assert re.search(r'\$staged\s*=\s*"\$AthenaHome\\node\.new-\$stamp"', swap), (
        "$staged must be defined as a sibling of the live tree "
        '("$AthenaHome\\node.new-$stamp")'
    )
    assert re.search(r'\$backup\s*=\s*"\$AthenaHome\\node\.old-\$stamp"', swap), (
        "$backup must be defined as a sibling of the live tree "
        '("$AthenaHome\\node.old-$stamp")'
    )


def test_managed_node_swap_renames_stay_within_athena_home() -> None:
    swap = _swap_block()
