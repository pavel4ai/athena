...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.athena/athena-agent/utils.py)
...
2. ``athena update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``athena_cli/main.py`` (the ``athena update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``athena_constants`` after the
