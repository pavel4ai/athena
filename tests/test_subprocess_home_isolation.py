...
Athena state stays profile-scoped through ATHENA_HOME. Host subprocesses should
...
See: https://github.com/pavel4ai/athena/issues/25114
See: https://github.com/pavel4ai/athena/issues/36144
See: https://github.com/pavel4ai/athena/issues/29015
"""
...
class TestGetSubprocessHome:
    """Unit tests for athena_constants.get_subprocess_home()."""

    def _host_mode(self, monkeypatch):
        monkeypatch.setattr(athena_constants, "is_container", lambda: False)
    ...
    def test_returns_none_when_athena_home_unset(self, monkeypatch):
    ...
    def test_host_auto_keeps_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        athena_home = real_home / ".athena" / "profiles" / "coder"
        profile_home = athena_home / "home"
    ...
    def test_real_mode_repairs_parent_home_already_pointing_at_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ATHENA_REAL_HOME", str(real_home))

        from athena_constants import get_subprocess_home, get_real_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ATHENA_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("ATHENA_HOME", str(base / "beta"))
    ...
    def test_context_override_is_thread_local(self, tmp_path, monkeypatch):
        ...
        def read_from_other_thread():
            ...
            seen.append(str(get_athena_home()))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_host_auto_preserves_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["ATHENA_REAL_HOME"] == str(real_home)
    ...
    def test_profile_mode_injects_profile_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(athena_home / "home")
        assert result["ATHENA_REAL_HOME"] == str(real_home)
    ...
    def test_no_injection_when_athena_home_unset(self, monkeypatch):
        monkeypatch.delenv("ATHENA_HOME", raising=False)
    ...
    def test_context_override_bridges_to_subprocess_env(self, tmp_path, monkeypatch):
        ...
        assert result["ATHENA_HOME"] == str(profile)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from athena_cli.profiles import _PROFILE_DIRS
...
class TestPythonProcessUnchanged:
    ...
    def test_path_home_unchanged_after_subprocess_home_resolved(
        ...
        assert sub_home in (None, str(athena_home / "home"), original_home)
