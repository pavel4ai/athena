...
Uses a throwaway ATHENA_HOME so it never touches ~/.athena. No real names.
"""
...
def _fresh_home():
    """Point ATHENA_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_athena_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["ATHENA_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER ATHENA_HOME is set.
...
# a different get_athena_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"ATHENA_HOME (throwaway): {HOME}")
