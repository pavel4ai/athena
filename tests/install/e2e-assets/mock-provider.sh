...
#   mock_start <workroot>      start, write config into $ATHENA_HOME
#   mock_stop                  kill the background server
#
# Requires: ASSETS (e2e-assets dir), LOG_DIR, ATHENA_HOME, ok/fail helpers.
...
mock_start() {
  ...
  cat > "$ATHENA_HOME/config.yaml" <<EOF
model:
  default: mock-model
  provider: mock
providers:
...
EOF
  ...
  ok "provider 'mock' configured in $ATHENA_HOME (api $url/v1)"
}
