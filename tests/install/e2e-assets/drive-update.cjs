// drive-update.cjs — launch the INSTALLED Athena.exe (real Electron desktop
...
//
//   node <this file> <path-to-Athena.exe> <proof-dir>
//
...
if (!exePath || !proofDir) {
  console.error('usage: node drive-update.cjs <Athena.exe> <proof-dir>')
  process.exit(1)
}
...
async function main() {
  ...
  const app = await _electron.launch({
    ...
    // Inherit the driver's env: ATHENA_HOME (isolated install) and
  ...
  // The real backend (`athena serve`) is booting underneath; give it time.
  ...
  // buttonless boot-progress card ("Starting Athena... 86%"), then the
  ...
  if (!visible) {
    ...
    const status = await page.evaluate(() =>
      window.athenaDesktop?.updates?.check?.() ?? Promise.resolve('no updates.check bridge')
  ...
  // The "Updating Athena — this window will close" overlay should appear,
  ...
  // the staged binary), which claims ATHENA_HOME/.athena-update-in-progress
  ...
  // proved the desktop had exited and `athena update` was already running).
  //
  ...
  const athenaHome = process.env.ATHENA_HOME
  const markerPath = athenaHome ? path.join(athenaHome, '.athena-update-in-progress') : null
  const resultPath = athenaHome ? path.join(athenaHome, '.athena-update-result.json') : null
