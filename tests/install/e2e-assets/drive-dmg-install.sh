#!/usr/bin/env bash
# Drive the Athena-Setup dmg bootstrap through its first-run GUI.
#
...
# the redirect env), click "Install Athena ->" with native input, then watch
...
tell application "System Events"
  set procs to (every process whose name contains "Athena")
...
install_complete() {
  [ -d "$INSTALL_DIR/.git" ] && [ -x "$ATHENA_BIN" ] && installed_app
}
...
BOOTSTRAP_LOG="$HOME/.athena/logs/bootstrap-installer.log"
bootstrap_error() {
...
while :; do
  if install_complete; then
    log "install landed: checkout + venv console script + Athena.app present"
  ...
  if ! kill -0 "$SETUP_PID" 2>/dev/null; then
    ...
    log "Athena-Setup exited (pid $SETUP_PID) before the install landed"
    exit 1
  fi
  err="$(bootstrap_error)"
  if [ -n "$err" ]; then
