"""Driver-side spawn interception for athena desktop E2E legs.

The installed ``athena`` is a venv console script, so its interpreter
imports ``sitecustomize`` at startup when this directory is on
``PYTHONPATH``. Behind an explicit env-var opt-in the module wraps
``subprocess.run`` so the FINAL electron launch call of ``athena
...
Opt-in: ``ATHENA_E2E_CAPTURE_LAUNCH=<path>`` -- the spec is written
there, and the marker file ``<path>.captured`` distinguishes "athena
...
_SPEC_PATH = os.environ.get("ATHENA_E2E_CAPTURE_LAUNCH")
...
if _SPEC_PATH:
    ...
    def _match_shape(argv: "list[str]") -> str:
        ...
        # apps/desktop/release/ (win-unpacked/Athena.exe, linux-unpacked/...,
        # mac*/Athena.app/Contents/MacOS/...).
