...
#              install landed on OLD with a working `athena`
#   update     advance served main to HEAD, apply ONE update method, assert
#              the checkout landed on HEAD with a working `athena`
#
# Usage:
#   tests/install/installer-script-e2e.sh --update-method athena-update|installer-script|installer-script+desktop
...
case "$UPDATE_METHOD" in
  athena-update|installer-script|installer-script+desktop|athena-desktop-app-update) ;;
  *) echo "error: --update-method must be athena-update, installer-script, installer-script+desktop or athena-desktop-app-update, got '$UPDATE_METHOD'" >&2; exit 1 ;;
esac
...
WORK_ROOT="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/athena-installer-script-e2e"
LOG_DIR="${ATHENA_E2E_LOG_DIR:-$WORK_ROOT/logs}"
...
# Isolated HOME: the runner's real one may carry a preinstalled athena or a
# developer config, and old installer scripts hardcode $HOME/.athena (the
# ATHENA_HOME env override is newer than tags we sample). GIT_CONFIG_GLOBAL
...
assert_desktop_artifact() {
  ...
  for cand in \
    ...
    "$release_dir/linux-unpacked/athena" \
    "$release_dir/mac-arm64/Athena.app" \
    "$release_dir/mac/Athena.app"; do
...
assert_checkout() {
  ...
  ok "checkout is $2 ($1)"
  local athena="$INSTALL_DIR/venv/bin/athena"
  [ -x "$athena" ] || fail "no athena console script at $athena"
  "$athena" --version 2>&1 | ts_prefix > "$LOG_DIR/version-$2.log" \
    || fail "athena --version failed after $2; log in $LOG_DIR/version-$2.log"
  ok "athena --version works: $(head -c 120 "$LOG_DIR/version-$2.log" | tr -d '\n')"
}

smoke_desktop() {
  # $1: label (old|head). Prove the installed CLI can produce the desktop
  # app: `athena desktop --build-only` runs the full desktop pipeline
  # (workspace install, renderer build, stamp write) and stops before the
  # launch -- the same call `athena update` itself makes. Probe the
  # INSTALLED athena for the flag rather than assuming this checkout's
  # surface: sampled OLD releases may predate `athena desktop` or
  # --build-only entirely, and for them the phase skips, loudly.
  local athena="$INSTALL_DIR/venv/bin/athena"
  if ! "$athena" desktop --help 2>/dev/null | grep -qF -- --build-only; then
    ok "athena desktop --build-only not supported at $1; skipping desktop smoke"
    return 0
  fi
  local rc=0
  (cd "$INSTALL_DIR" && "$athena" desktop --build-only < /dev/null 2>&1 \
    | ts_prefix > "$LOG_DIR/desktop-smoke-$1.log") || rc=$?
  log_group "athena desktop --build-only ($1) transcript" "$LOG_DIR/desktop-smoke-$1.log"
  [ "$rc" -eq 0 ] || fail "athena desktop --build-only ($1) exited $rc; transcript above"
  ok "athena desktop --build-only works at $1"
...
case "$UPDATE_METHOD" in
  athena-update)
    ...
    # installed athena; older ones read the prompt from stdin, so close it.
    ATHENA="$INSTALL_DIR/venv/bin/athena"
    if "$ATHENA" update --help 2>&1 | grep -qF -- --yes; then
      update_cmd=("$ATHENA" update --yes)
    else
      update_cmd=("$ATHENA" update)
    fi
    rc=0
    (cd "$INSTALL_DIR" && "${update_cmd[@]}" < /dev/null 2>&1 | ts_prefix > "$LOG_DIR/update.log") || rc=$?
    log_group "athena update transcript" "$LOG_DIR/update.log"
    [ "$rc" -eq 0 ] || fail "athena update exited $rc; transcript above, log at $LOG_DIR/update.log"
    ;;
  installer-script)
  ...
  athena-desktop-app-update)
    # The real user surface: `athena desktop` launches the app, the user
    ...
    # suite's own) is configured into ATHENA_HOME exactly like the dev:mock
    ...
    step "capturing the athena desktop launch spec (build runs for real)"
    rc=0
    (cd "$INSTALL_DIR" && \
      PYTHONPATH="$ASSETS/launch-capture${PYTHONPATH:+:$PYTHONPATH}" \
      ATHENA_E2E_CAPTURE_LAUNCH="$SPEC" \
      "$ATHENA" desktop < /dev/null 2>&1 | ts_prefix > "$LOG_DIR/desktop-launch-capture.log") || rc=$?
    log_group "athena desktop (launch capture) transcript" "$LOG_DIR/desktop-launch-capture.log"
    [ "$rc" -eq 0 ] || fail "athena desktop exited $rc during launch capture; transcript above"
    # Exit 0 without a capture means a version that never reached its
    # launch - that must fail loudly, not pass as a no-op.
    [ -f "$SPEC.captured" ] || fail "athena desktop exited 0 but no launch was captured at $SPEC"
...
mkdir -p "$ildest"
cp -R "$ATHENA_HOME/logs" "$ildest/athena-logs" 2>/dev/null || true
if [ -n "${XDG_DATA_HOME:-}" ]; then
  cp -R "$XDG_DATA_HOME/athena/logs" "$ildest/desktop-userdata-logs" 2>/dev/null || true
fi
cp "$ATHENA_HOME/.athena-update-result.json" "$ildest" 2>/dev/null || true
ls -la "$ATHENA_HOME" > "$ildest/athena-home-ls.txt" 2>/dev/null || true
