...
# ============================================================================
# Proves, on a real Windows machine, that a user who installs Athena the way
...
# actually touches:
#
#   INSTALL   - downloads the production Athena-Setup.exe from the website,
#               launches it HEADED, and AutoHotkey clicks Install, waits,
#               then clicks Launch. The real Electron Athena.exe must appear.
...
#   installer's `git clone` and `athena update`'s `git fetch origin`
...
param(
    ...
    # open-app-update (from a desktop-installer install) and athena-update /
    ...
    # they land.
    [ValidateSet("open-app-update", "athena-desktop-app-update", "athena-update", "desktop-installer@latest", "installer-script", "installer-script+desktop")]
    [string]$Route = "open-app-update",

    # The OLD version: the ref served as `main` while the installer runs,
    # i.e. what the user starts on. The published Athena-Setup.exe carries
    ...
    [string]$WorkRoot = $(if ($env:ATHENA_E2E_WORKROOT) { $env:ATHENA_E2E_WORKROOT } else { Join-Path $env:TEMP "athena-desktop-gui-e2e" }),

    [string]$SetupExeUrl = "https://athena-assets.nousresearch.com/Athena-Setup.exe",
...
$AthenaHome  = Join-Path $WorkRoot "athena-home"
$InstallDir  = Join-Path $AthenaHome "athena-agent"
...
function Invoke-Git([string[]]$GitArgs) {
    ...
    # if athena ever adds a git command that calls something with ^ this will break, lol.
...
function Set-GitRedirect {
    # we redirect to our own repo so we can play around with what commit athena thinks we're on.
...
function Get-DesktopExe {
    foreach ($c in @(
        (Join-Path $InstallDir "apps\desktop\release\win-unpacked\Athena.exe"),
        (Join-Path $InstallDir "apps\desktop\release\win-arm64-unpacked\Athena.exe")
    )) {
...
# Install-side state snapshot, taken BEFORE Test-AthenaRuns can throw: on
...
function Save-InstallSideState([string]$Label) {
    ...
    $logsDir = Join-Path $AthenaHome "logs"
    if (Test-Path -LiteralPath $logsDir) {
        Copy-Item $logsDir (Join-Path $dest "athena-logs") -Recurse -Force -ErrorAction SilentlyContinue
    }
    $resultFile = Join-Path $AthenaHome ".athena-update-result.json"
    ...
    Get-ChildItem -LiteralPath $AthenaHome -ErrorAction SilentlyContinue |
        Select-Object Name, Length, LastWriteTime |
        Format-Table -AutoSize | Out-String |
        Set-Content (Join-Path $dest "athena-home-ls.txt")
}

function Test-AthenaRuns([string]$Label) {
    Save-InstallSideState $Label
    $athenaExe = Join-Path $InstallDir "venv\Scripts\athena.exe"
    Assert-True (Test-Path -LiteralPath $athenaExe) "$Label -- venv\Scripts\athena.exe exists"
    & $athenaExe --version 2>&1 | ForEach-Object { Write-Host "    athena --version| $_" }
    Assert-True ($LASTEXITCODE -eq 0) "$Label -- athena --version exits 0"
}
...
function Invoke-RefInstaller {
    ...
    $flags = @("-SkipSetup", "-AthenaHome", $AthenaHome, "-InstallDir", $InstallDir)
...
function Invoke-AthenaUpdate {
    # The venv updater. --yes reaches the update subcommand only in later
    # releases; ask the installed binary, never parse its source.
    $athenaExe = Join-Path $InstallDir "venv\Scripts\athena.exe"
    $updateArgs = @("update")
    $prevEap = $ErrorActionPreference; $ErrorActionPreference = "Continue"
    $helpText = & $athenaExe update --help 2>&1 | Out-String
    if ($helpText -match '--yes') { $updateArgs += "--yes" }
    New-Item -ItemType Directory -Path (Join-Path $WorkRoot "logs") -Force | Out-Null
    $log = Join-Path $WorkRoot "logs\update.log"
    Push-Location $InstallDir
    try {
        & $athenaExe @updateArgs 2>&1 | Add-TsPrefix | Out-File -Encoding UTF8 $log
        $updateExit = $LASTEXITCODE
    } finally {
        Pop-Location
        $ErrorActionPreference = $prevEap
    }
    Write-LogGroup "athena update transcript" $log
    Assert-True ($updateExit -eq 0) "athena update exited $updateExit (expected 0)"
}

function Invoke-AthenaDesktopAppUpdate([string]$TargetSha) {
    # The athena-desktop launch surface: `athena desktop` runs its whole
    ...
    $prevCap = $env:ATHENA_E2E_CAPTURE_LAUNCH
    $env:PYTHONPATH = if ($prevPy) { "$capDir;$prevPy" } else { $capDir }
    $env:ATHENA_E2E_CAPTURE_LAUNCH = $spec
    $prevEap = $ErrorActionPreference; $ErrorActionPreference = "Continue"
    Push-Location $InstallDir
    try {
        & $athenaExe desktop 2>&1 | Add-TsPrefix | Out-File -Encoding UTF8 $log
        $capExit = $LASTEXITCODE
    } finally {
        Pop-Location
        $ErrorActionPreference = $prevEap
        $env:PYTHONPATH = $prevPy
        $env:ATHENA_E2E_CAPTURE_LAUNCH = $prevCap
    }
    Write-LogGroup "athena desktop (launch capture) transcript" $log
    Assert-True ($capExit -eq 0) "athena desktop exited 0 during launch capture"
    ...
    try {
        & $node "launch-from-spec.mjs" --spec $spec `
            --result (Join-Path $AthenaHome ".athena-update-result.json") `
    ...
    Assert-True ($driveExit -eq 0) "app driven via captured athena desktop spec; update completed"
}
...
function Stop-AthenaAppProcesses([string]$Label) {
    # Close the desktop app the blunt way between phases (a user quitting).
    # Only Athena.exe (Electron) -- never athena.exe (the venv CLI shim).
    $procs = @(Get-Process -Name "Athena" -ErrorAction SilentlyContinue)
    foreach ($p in $procs) {
        try { Stop-Process -Id $p.Id -Force -ErrorAction SilentlyContinue } catch {}
    }
    if ($procs.Count -gt 0) {
        Write-Host "  [$Label] stopped $($procs.Count) Athena.exe process(es)"
        Start-Sleep -Seconds 3
    }
}

function Get-ManagedNode {
    # `athena update`/desktop builds use the Athena-managed Node; use the same
    # one to run the Playwright driver so no system Node is required.
    $candidates = @(
        (Join-Path $AthenaHome "node\node.exe"),
        (Join-Path $AthenaHome "bin\node\node.exe"),
...
# ----------------------------------------------------------------------------
# Phase: install-gui -- website Athena-Setup.exe, headed, AHK-driven
# ----------------------------------------------------------------------------
function Invoke-PhaseInstallGui {
    ...
    Write-Step "$($Mode.ToUpper()) (GUI): Athena-Setup.exe from the website, headed, AHK clicks"
    ...
    $setupExe = Join-Path $WorkRoot "Athena-Setup.exe"
    if (-not (Test-Path -LiteralPath $setupExe)) {
        Write-Host "  downloading $SetupExeUrl"
        Invoke-WebRequest -Uri $SetupExeUrl -OutFile $setupExe
    }
    Assert-True ((Get-Item $setupExe).Length -gt 1MB) "Athena-Setup.exe downloaded ($([math]::Round((Get-Item $setupExe).Length / 1MB, 1)) MB)"
    ...
    Remove-Item Env:ATHENA_SETUP_DEV_REPO_ROOT -ErrorAction SilentlyContinue
    New-Item -ItemType Directory -Path $AthenaHome -Force | Out-Null
    ...
    try {
        ...
        Write-Host "  Athena-Setup.exe launched (pid $($installer.Id))"

        # Drive it: Install click -> wait -> Launch click -> Athena.exe window.
        ...
        $ahk = Start-Process -FilePath $ahkExe `
            -ArgumentList (Join-Path $AhkDir "install-and-launch.ahk"), $ahkLog, "Athena-Setup.exe", (Join-Path $AthenaHome "logs\bootstrap-installer.log") `
            -PassThru
        ...
        # actually be running.
        Assert-True ($null -ne (Get-Process -Name "Athena" -ErrorAction SilentlyContinue)) "Athena.exe process is running (installer Launch hand-off worked)"

        # Installer should have exited after Launch.
        if (-not $installer.HasExited) {
            Start-Sleep -Seconds 10
        }
        Assert-True $installer.HasExited "Athena-Setup.exe exited after Launch"
    }
    finally {
        Stop-DesktopRecorder $recorder (Join-Path $proof "desktop-frames")
        # Surface the installer's own log win or lose, full and folded.
        $bootLog = Join-Path $AthenaHome "logs\bootstrap-installer.log"
    ...
    Stop-AthenaAppProcesses "post-install"
    ...
    Test-AthenaRuns "post-$Mode-gui"
    Assert-True ($null -ne (Get-DesktopExe)) "packaged Desktop Athena.exe exists"
...
function Invoke-GuiUpdateDesktopRoute([string]$TargetSha) {
    ...
    Assert-True ($null -ne $desktopExe) "packaged Athena.exe present before update"
    ...
    try {
        ...
        # `athena update` + desktop rebuild + relaunch. Which updater depends
        # on the installed checkout, and BOTH are production paths:
        #   * checkouts shipping scripts/desktop-update.ps1 -> that script,
        #     which writes .athena-update-result.json on every exit;
        #   * older checkouts -> the staged athena-setup.exe --update flow,
        #     which does NOT write the result JSON.
        # So: poll for COMPLETION = (result JSON) OR (checkout reached the
        # target sha AND the marker is gone). The sha/marker/athena/relaunch
        ...
        $updateLog = Join-Path $AthenaHome "logs\update.log"
        $updateLogPos = 0
        ...
        Test-AthenaRuns "post-update"
        Assert-True ($null -ne (Get-DesktopExe)) "Athena.exe still present after update"
        ...
        Write-Host "  waiting for the relaunched Athena.exe ..."
        $rDeadline = (Get-Date).AddMinutes(5)
        $relaunched = $null
        while ((Get-Date) -lt $rDeadline) {
            $relaunched = Get-Process -Name "Athena" -ErrorAction SilentlyContinue
            if ($relaunched) { break }
        ...
        # Foreground the relaunched Athena window so the proof screenshot
        ...
        try {
            $mainProc = Get-Process -Name "Athena" -ErrorAction SilentlyContinue |
...
'@ -ErrorAction SilentlyContinue
        ...
        Stop-DesktopRecorder $recorder (Join-Path $proof "desktop-frames")
        $handoffLog = Join-Path $AthenaHome "logs\desktop-update-handoff.log"
        ...
        Stop-AthenaAppProcesses "post-update"
    }
}

function Invoke-PhaseInstall {
    # Dispatch on the install axis. Each arm ends with the same contract:
    # checkout at OLD, athena runs, and state carries how OLD landed so any
    ...
    switch ($InstallMethod) {
        ...
        "installer-script" {
            ...
            Test-AthenaRuns "post-install-script"
        }
        "installer-script+desktop" {
            ...
            Assert-True ((Get-InstalledHead) -eq $state.old) "installed checkout is at OLD"
            Test-AthenaRuns "post-install-script-desktop"
...
function Invoke-PhaseUpdate {
    ...
    New-Item -ItemType File -Path (Join-Path $AthenaHome ".skip_upstream_prompt") -Force | Out-Null
    ...
    switch ($Route) {
        ...
        "athena-desktop-app-update" {
            Invoke-AthenaDesktopAppUpdate $state.current
        }
        "athena-update" {
            Invoke-AthenaUpdate
        }
        ...
        "desktop-installer@latest" {
            # A user re-downloading Athena-Setup.exe and clicking Install over
...
function Invoke-CheckedPhaseUpdate {
    ...
    foreach ($oldLog in @((Join-Path $WorkRoot "logs\update.log"), (Join-Path $AthenaHome "logs\desktop.log"))) {
