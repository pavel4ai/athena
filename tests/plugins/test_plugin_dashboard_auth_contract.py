...
plugin SDK (``window.__ATHENA_PLUGIN_SDK__``): ``fetchJSON`` (JSON REST),
...
(``athena_session_at`` cookie / single-use ``?ticket=``).

Plugins that hand-roll ``fetch`` / ``WebSocket`` and read
``window.__ATHENA_SESSION_TOKEN__`` directly send an empty token in gated mode
...
_FORBIDDEN = "__ATHENA_SESSION_TOKEN__"
...
def test_there_are_plugin_bundles_to_check() -> None:
    ...
    # kanban + athena-achievements are bundled today; assert at least one is
