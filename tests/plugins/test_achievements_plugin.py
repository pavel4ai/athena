"""Tests for the bundled athena-achievements dashboard plugin.
...
``plugins/athena-achievements/tests/`` covering the achievement engine
...
def plugin_api(tmp_path, monkeypatch):
    """Load plugin_api with isolated ~/.athena so state/snapshot files don't collide.
    ...
    # swap ``sys.modules['athena_state']`` with auto-restoration. Without
    # this, a raw ``sys.modules[...] = fake`` assignment would leak the
    # fake into later tests in the same xdist worker — breaking every
    # test that does ``from athena_state import SessionDB``.
    module._test_monkeypatch = monkeypatch
    yield module


class _FakeSessionDB:
    """Stand-in for athena_state.SessionDB that records scan calls."""
...
def _install_fake_session_db(plugin_api, fake_db):
    ...
    ``sys.modules['athena_state']`` swap is auto-restored at test teardown
    and cannot leak into unrelated tests in the same xdist worker.
    """
    fake_module = type(sys)("athena_state")
    fake_module.SessionDB = lambda: fake_db
    plugin_api._test_monkeypatch.setitem(sys.modules, "athena_state", fake_module)
