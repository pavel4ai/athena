...
def test_store_and_load_photon_token(tmp_athena_home: Path) -> None:
    photon_auth.store_photon_token("abc123def456")
    assert photon_auth.load_photon_token() == "abc123def456"

    auth_json = json.loads((tmp_athena_home / "auth.json").read_text())
...
def test_save_auth_never_world_readable(tmp_athena_home: Path) -> None:
    """auth.json must be created 0o600 — no window at process umask."""
    photon_auth.store_photon_token("secret-token")
    mode = (tmp_athena_home / "auth.json").stat().st_mode & 0o777
    assert mode == 0o600


def test_store_project_credentials_round_trip(
    tmp_athena_home: Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
...
# cross-process lock athena_cli/auth.py's ~15 other writers all use, so a
