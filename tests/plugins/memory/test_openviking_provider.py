...
_EXPECTED_USER_AGENT = f"openviking-memory-athena/{_ATHENA_VERSION}"
...
def test_openviking_provider_config_loader_uses_readonly_config(monkeypatch):
    ...
    config = openviking_module._load_athena_openviking_config()
...
""",
    ...
    settings = openviking_module._resolve_connection_settings(
        openviking_module._load_athena_openviking_config()
    )
...
def test_start_local_openviking_server_strips_pythonpath_from_child_env(monkeypatch):
    """The spawned server must not inherit Athena's PYTHONPATH (#78153).

    Inheriting it makes openviking-server import packages from the Athena
    venv instead of its own, and on Windows locks Athena venv DLLs so the
    venv cannot be rebuilt during `athena update`.
    """
    popen_calls = []
    ...
    monkeypatch.setattr(openviking_module.shutil, "which", lambda name: "/usr/local/bin/openviking-server")
    monkeypatch.setattr(openviking_module.subprocess, "Popen", fake_popen)
    monkeypatch.setenv("PYTHONPATH", "/opt/athena/.venv/Lib/site-packages")
    monkeypatch.setenv("ATHENA_PROFILE", "test-profile")
    ...
    assert child_env.get("ATHENA_PROFILE") == "test-profile"
...
def test_https_local_endpoint_is_not_runtime_autostart_eligible(monkeypatch):
    ...
    assert warnings == [
        ...
        "OpenViking memory is temporarily unavailable; Athena will retry on a later access or when "
        "the config changes. "
...
def test_runtime_does_not_autostart_when_local_server_reports_unhealthy(monkeypatch):
    ...
    assert warnings == [
        ...
        "OpenViking memory is temporarily unavailable; Athena will retry on a later access "
...
def test_tool_add_resource_rejects_athena_credential_file_upload(tmp_path, monkeypatch):
...
def test_concurrent_providers_claim_unlocked_pending_owner_once(
    ...
    for provider in providers:
        ...
        provider._athena_home = str(tmp_path)
...
def test_is_available_false_without_any_endpoint(monkeypatch):
    _clear_openviking_env(monkeypatch)
    monkeypatch.setattr(
        openviking_module, "_load_athena_openviking_config", lambda: {}
    )
...
class TestOpenVikingEnvWriter:
    ...
    def test_bom_prefixed_env_updates_in_place(self, tmp_path):
        ...
        # The stale value must be gone, not left as a duplicate. Athena and
