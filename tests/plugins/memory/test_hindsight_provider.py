...
from athena_cli.memory_setup import _CANCELLED
...
def _provider_for_mode(tmp_path, monkeypatch, mode: str):
    ...
    monkeypatch.setattr(
        "plugins.memory.hindsight.get_athena_home", lambda: tmp_path
    )

    provider = HindsightMemoryProvider()
    provider.initialize(session_id="test-session", athena_home=str(tmp_path), platform="cli")
    return provider
...
def provider(tmp_path, monkeypatch):
    ...
    p.initialize(session_id="test-session", athena_home=str(tmp_path), platform="cli")
...
class TestConfig:
    ...
    def test_config_from_env_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "plugins.memory.hindsight.get_athena_home",
...
class TestPostSetup:
    def test_setup_cancel_at_mode_picker_writes_nothing(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("plugins.memory.hindsight.get_athena_home", lambda: athena_home)

        save_config = MagicMock()
        which = MagicMock(return_value="/usr/bin/uv")
        run = MagicMock()
        monkeypatch.setattr("athena_cli.memory_setup._curses_select", lambda *args, **kwargs: _CANCELLED)
        ...
        monkeypatch.setattr("athena_cli.config.save_config", save_config)

        provider = HindsightMemoryProvider()
        provider.post_setup(str(athena_home), {"memory": {"provider": "builtin"}})

        save_config.assert_not_called()
        which.assert_not_called()
        run.assert_not_called()
        assert not (athena_home / ".env").exists()
        assert not (athena_home / "hindsight" / "config.json").exists()
        assert not (user_home / ".hindsight" / "profiles" / "athena.env").exists()
    ...
    def test_local_embedded_setup_cancel_at_llm_picker_writes_nothing(self, tmp_path, monkeypatch):
        ...
        which = MagicMock(return_value="/usr/bin/uv")
        run = MagicMock()
        monkeypatch.setattr("athena_cli.memory_setup._curses_select", lambda *args, **kwargs: next(selections))
        monkeypatch.setattr("shutil.which", which)
        monkeypatch.setattr("subprocess.run", run)
        monkeypatch.setattr("builtins.input", MagicMock(side_effect=AssertionError("prompt should not run")))
        monkeypatch.setattr("getpass.getpass", MagicMock(side_effect=AssertionError("prompt should not run")))
    ...
    def test_local_embedded_setup_materializes_profile_env(self, tmp_path, monkeypatch):
        ...
        saved_configs = []
        monkeypatch.setattr("athena_cli.config.save_config", lambda cfg: saved_configs.append(cfg.copy()))

        provider = HindsightMemoryProvider()
        provider.post_setup(str(athena_home), {"memory": {}})
        ...
        profile_env = user_home / ".hindsight" / "profiles" / "athena.env"
    ...
    def test_local_embedded_setup_respects_existing_profile_name(self, tmp_path, monkeypatch):
        ...
        provider = HindsightMemoryProvider()
        provider.save_config({"profile": "coder"}, str(athena_home))
        provider.post_setup(str(athena_home), {"memory": {}})

        coder_env = user_home / ".hindsight" / "profiles" / "coder.env"
        athena_env = user_home / ".hindsight" / "profiles" / "athena.env"
        assert coder_env.exists()
        assert not athena_env.exists()
    ...
    def test_local_embedded_setup_blank_inputs_preserve_existing_config(self, tmp_path, monkeypatch):
        ...
        provider = HindsightMemoryProvider()
        provider.save_config(existing_config, str(athena_home))
        ...
        monkeypatch.setattr("athena_cli.memory_setup._curses_select", lambda *args, **kwargs: kwargs.get("default", 0))
        monkeypatch.setattr("shutil.which", lambda name: None)
        monkeypatch.setattr("builtins.input", lambda prompt="": "")
        monkeypatch.setattr("sys.stdin.isatty", lambda: True)
        monkeypatch.setattr("getpass.getpass", lambda prompt="": "")
        ...
        saved = json.loads((athena_home / "hindsight" / "config.json").read_text())
...
class TestSyncTurn:
    ...
    def test_resume_creates_new_document(self, tmp_path, monkeypatch):
        ...
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        monkeypatch.setattr("plugins.memory.hindsight.get_athena_home", lambda: tmp_path)

        p1 = HindsightMemoryProvider()
        p1.initialize(session_id="resumed-session", athena_home=str(tmp_path), platform="cli")
        ...
        p2.initialize(session_id="resumed-session", athena_home=str(tmp_path), platform="cli")
...
class TestBankIdTemplate:
    ...
    def test_resolve_with_profile(self):
        result = _resolve_bank_id_template(
            "athena-{profile}", fallback="athena",
            profile="coder", workspace="", platform="", user="", session="",
        )
        assert result == "athena-coder"
    ...
    def test_resolve_collapses_empty_placeholders(self):
        # When user is empty, "athena-{user}" becomes "athena-" -> trimmed to "athena"
        result = _resolve_bank_id_template(
            "athena-{user}", fallback="default",
    ...
    def test_resolve_invalid_template_returns_fallback(self):
        ...
        result = _resolve_bank_id_template(
            "athena-{unknown}", fallback="athena",
            profile="", workspace="", platform="", user="", session="",
        )
        assert result == "athena"
    ...
    def test_provider_uses_bank_id_template_from_config(self, tmp_path, monkeypatch):
        config = {
            ...
            "bank_id_template": "athena-{profile}",
        }
        config_path = tmp_path / "hindsight" / "config.json"
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        ...
        assert p._bank_id == "athena-coder"
        assert p._bank_id_template == "athena-{profile}"
    ...
    def test_provider_template_with_missing_profile_falls_back(self, tmp_path, monkeypatch):
        ...
        p = HindsightMemoryProvider()
        # No agent_identity passed — template renders to "athena-" which collapses to "athena"
        p.initialize(session_id="s1", athena_home=str(tmp_path), platform="cli")
