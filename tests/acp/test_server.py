...
from acp_adapter.server import AthenaACPAgent, ATHENA_VERSION
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.version == ATHENA_VERSION
...
class TestSessionOps:
    ...
    async def test_new_session_returns_model_state(self):
        ...
        with patch(
            "athena_cli.models.curated_models_for_provider",
...
class TestSessionConfiguration:
    ...
    async def test_set_session_model_accepts_provider_prefixed_choice(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "athena_cli.models.parse_model_input",
...
class TestPrompt:
    ...
    @pytest.mark.asyncio
    async def test_prompt_propagates_athena_session_id_env(self, agent, monkeypatch):
        """ACP must propagate the originating session id to the agent loop
        via ``ATHENA_SESSION_ID`` so tools that want to stamp side-effects
        ...
        def mock_run(user_message, conversation_history=None, task_id=None, **kwargs):
            ...
            captured["env"] = os.environ.get("ATHENA_SESSION_ID")
        ...
        assert captured["env"] == new_resp.session_id, (
            "ATHENA_SESSION_ID must be set to the originating ACP session id "
        ...
        assert os.environ.get("ATHENA_SESSION_ID") is None, (
            "ATHENA_SESSION_ID must be restored after the agent call so "
            "a re-used executor thread doesn't leak the id into the next "
            "session's tools"
        )

    @pytest.mark.asyncio
    async def test_prompt_restores_prior_athena_session_id(self, agent, monkeypatch):
        """If the env already had ATHENA_SESSION_ID set (e.g. nested
        agent loops), the prior value must be restored after the inner
        prompt completes — not popped, not left at the inner id."""
        monkeypatch.setenv("ATHENA_SESSION_ID", "outer-sess")
        ...
        def mock_run(*args, **kwargs):
            captured["inner"] = os.environ.get("ATHENA_SESSION_ID")
            return {"final_response": "ok", "messages": []}
        ...
        assert os.environ.get("ATHENA_SESSION_ID") == "outer-sess"
...
class TestSlashCommands:
    ...
    def test_version(self, agent, mock_manager):
        ...
        assert ATHENA_VERSION in result
