...
class TestSessionOps:
    ...
    async def test_new_session_returns_authenticated_cross_provider_model_state(self):
        ...
        with (
            patch("athena_cli.inventory.load_picker_context", return_value=picker_context),
            patch("athena_cli.inventory.build_models_payload", return_value=payload) as build_payload,
        ):
...
class TestPrompt:
    ...
    async def test_prompt_binds_session_id_into_subprocess_env(self, agent, mock_manager):
        ...
        leaving the ``ATHENA_SESSION_ID`` ContextVar bound to the explicit ""
        ...
        empty ``ATHENA_SESSION_ID`` instead of the session's own id.
        """
        ...
        def _run(*args, **kwargs):
            ...
            captured["child"] = _make_run_env({}).get("ATHENA_SESSION_ID")
