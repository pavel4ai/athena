...
guard is covered in ``tests/athena_cli/test_picker_prewarm.py``):

- ``main()`` invokes ``athena_cli.model_switch_providers.prewarm_picker_cache_async``
...
import athena_cli.model_switch_providers
...
def _run_main(monkeypatch, events, *, prewarm=None):
    ...
    # entry.main() imports the helper lazily from athena_cli.model_switch,
