...
def athena_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves ATHENA_HOME.
...
def server(athena_home, monkeypatch):
    ...
    # Pin config resolution to the isolated ATHENA_HOME. Sibling test
    ...
    # module-level ``_athena_home = get_athena_home()`` snapshot freezes
...
def test_moa_bare_returns_usage(server, session, athena_home):
    _write_moa_config(athena_home, """
moa:
...
def test_goal_draft_uses_session_profile_without_blocking_rpc_reader(
    ...
    def draft(objective):
        observed.append(get_athena_home())
        started.set()
