    ...
    with patch.dict(
        "sys.modules",
        {
            "athena_constants": MagicMock(
                get_athena_home=MagicMock(return_value="/tmp/athena_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'athena-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'athena-release' patched"
    }
