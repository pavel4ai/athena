...
def test_session_operations_require_resolved_owner(tmp_path, monkeypatch, operation):
    ...
    monkeypatch.setattr(sessions, "_sessions", {
        "owned": {"flow": flow, "server_name": "reports", "athena_home": home, "httpd": None}
    })
    ...
    def invoke():
        ...
        return sessions.cancel_flow("owned", "reports", str(get_athena_home()))

    token = set_athena_home_override(tmp_path / "other")
    try:
        rejected = invoke()
    finally:
