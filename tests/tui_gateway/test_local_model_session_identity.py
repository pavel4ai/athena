...
from athena_cli.local_runtime import endpoint
...
def local_route(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("athena_cli.banner.get_update_result", lambda **kw: None)
    monkeypatch.setattr("athena_cli.banner.get_available_skills", lambda: {})
...
def test_session_info_recovers_identity_from_the_owning_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ATHENA_HOME", str(launch))
    ...
    with server._profile_build_scope(secondary):
        ...
        assert get_athena_home() == secondary
    assert get_athena_home() == launch
