"""
Tests for timezone support (athena_time module + integration points).
...
def _reset_athena_time_cache():
    """Reset the athena_time module cache."""
    athena_time.reset_cache()


# =========================================================================
# athena_time.now() — core helper
# =========================================================================
...
class TestAthenaTimeNow:
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["ATHENA_TIMEZONE"] = "Asia/Kolkata"
        result = athena_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["ATHENA_TIMEZONE"] = "UTC"
        result = athena_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["ATHENA_TIMEZONE"] = "Europe/London"
        tz = athena_time.get_timezone()
        assert isinstance(tz, ZoneInfo)
        assert str(tz) == "Europe/London"

    def test_cache_isolated_by_active_profile_config(self, tmp_path, monkeypatch):
        """Switching ATHENA_HOME must not reuse another profile's timezone."""
        ...
        monkeypatch.setenv("ATHENA_HOME", str(first_home))
        assert str(athena_time.get_timezone()) == "Asia/Tokyo"

        # Multiplexed profile runtime scopes switch ATHENA_HOME in one process.
        monkeypatch.setenv("ATHENA_HOME", str(second_home))
        assert str(athena_time.get_timezone()) == "America/New_York"
    ...
    def test_concurrent_profile_resolution_never_mixes_zones(
        ...
        def worker(key: str) -> None:
            barrier.wait()
            for _ in range(200):
                token = set_athena_home_override(str(homes[key]))
                try:
                    tz = athena_time.get_timezone()
                    if str(tz) != zones[key]:
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When ATHENA_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) ATHENA_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("ATHENA_TIMEZONE=" + os.environ.get("ATHENA_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "ATHENA_TIMEZONE=NOT_SET" in result["output"], (
            "ATHENA_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When ATHENA_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("ATHENA_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=athena_tz) which shifted
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        ...
        # of the naive timestamp exceeds _athena_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["ATHENA_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_athena_time_cache()
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["ATHENA_TIMEZONE"] = "US/Eastern"
        _reset_athena_time_cache()
