...
def _reset_athena_time_cache():
    """Reset the athena_time module cache (replacement for removed reset_cache)."""
    athena_time._cached_tz = None
    athena_time._cached_tz_name = None
    athena_time._cache_resolved = False
...
class TestAthenaTimeNow:
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["ATHENA_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="athena_time"):
            result = athena_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_none_for_invalid(self):
        os.environ["ATHENA_TIMEZONE"] = "Not/A/Timezone"
        tz = athena_time.get_timezone()
        assert tz is None
...
class TestCronTimezone:
    ...
    def test_ensure_aware_normalizes_aware_to_athena_tz(self):
