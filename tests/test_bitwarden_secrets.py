...
binary download are exercised manually by `athena secrets bitwarden
...
def athena_home(tmp_path, monkeypatch):
    ...
    # Some modules cache get_athena_home; clear if needed.
    import athena_constants
    if hasattr(athena_constants, "_ATHENA_HOME_CACHE"):
        athena_constants._ATHENA_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(athena_home, monkeypatch):
...
def test_install_bws_happy_path(athena_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(athena_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(athena_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from athena_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under athena_home/cache/."""
    home = tmp_path / ".athena"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
