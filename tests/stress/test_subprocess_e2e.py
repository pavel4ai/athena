...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves athena_cli.kanban_db on its own
  ...
  - worker logs are captured to ATHENA_HOME/kanban/logs/<task>.log
    ...
    def _spawn(task, workspace):
        ...
        env = {
            **os.environ,
            ...
            "HOME": home,
            "PYTHONPATH": WT,
            "ATHENA_KANBAN_TASK": task.id,
            "ATHENA_KANBAN_WORKSPACE": workspace,
...
def main():
    home = tempfile.mkdtemp(prefix="athena_e2e_")
...
""")
    ...
    print("=" * 60)
    # Scenario A workers wrote to /tmp/athena_e2e_*/worker_*.log
    import glob
