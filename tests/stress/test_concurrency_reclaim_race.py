...
def worker_loop(worker_id: int, athena_home: str, result_file: str) -> None:
...
def reclaimer_loop(athena_home: str, result_file: str) -> None:
    os.environ["ATHENA_HOME"] = athena_home
    os.environ["HOME"] = athena_home
    sys.path.insert(0, WT)
    from athena_cli import kanban_db as kb
...
def main():
    home = tempfile.mkdtemp(prefix="athena_reclaim_race_")
