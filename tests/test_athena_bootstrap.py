"""Tests for athena_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Athena entry point
(athena, athena-agent, athena-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Athena entry point imports athena_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported athena_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("athena_bootstrap", None)
    import athena_bootstrap  # noqa: WPS433
    return athena_bootstrap
...
class TestEntryPointsImportBootstrap:
    """Every Athena entry point must import athena_bootstrap as its
    ...
    # import athena_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "athena_cli/main.py",   # athena CLI (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import athena_bootstrap' and that
        ...
        half-finished ``athena update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``athena_bootstrap`` as a top-level module) leaves athena
        ...
        # at tests/test_athena_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # athena start even when athena_bootstrap hasn't been
        ...
        assert first_import_name == "athena_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'athena_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import athena_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Athena's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("ATHENA_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["ATHENA_PYTHON_SRC_ROOT"] = env
            elif "ATHENA_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["ATHENA_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/athena")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ATHENA_PYTHON_SRC_ROOT", None)
            else:
                os.environ["ATHENA_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/athena", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Athena root.  The guard must relocate Athena to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/athena"])
        assert result[0] == "/opt/athena"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Athena root.
        assert result.index("/opt/athena") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["ATHENA_PYTHON_SRC_ROOT"] = "/env/athena"
            hb.harden_import_path()
            assert sys.path[0] == "/env/athena"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ATHENA_PYTHON_SRC_ROOT", None)
