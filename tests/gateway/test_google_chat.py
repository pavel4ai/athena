...
def adapter(tmp_path):
    ...
    ~/.athena/google_chat_thread_counts.json.
    """
    ...
    # Replace the production store (which would write to ~/.athena/...)
...
class TestSend:
    ...
    async def test_send_clarify_posts_choice_card(self, adapter):
        ...
        assert buttons[0]["onClick"]["action"]["function"] == "athena_clarify"
    ...
    def test_interactive_setup_uses_shared_cli_prompt_helpers(self, monkeypatch):
        ...
        answers = {
            ...
            "Pub/Sub subscription (projects/<proj>/subscriptions/<sub>)": (
                "projects/demo-project/subscriptions/athena-chat"
            ),
        ...
        monkeypatch.setattr("athena_cli.config.get_env_value", fake_get_env_value)
        monkeypatch.setattr("athena_cli.config.save_env_value", fake_save_env_value)
        monkeypatch.setattr("athena_cli.cli_output.prompt", fake_prompt)
        monkeypatch.setattr(
            "athena_cli.cli_output.prompt_yes_no", lambda *_a, **_kw: True
        )
        monkeypatch.setattr(
            "athena_cli.cli_output.print_info", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "athena_cli.cli_output.print_success", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "athena_cli.cli_output.print_warning", lambda *_a, **_kw: None
        )
