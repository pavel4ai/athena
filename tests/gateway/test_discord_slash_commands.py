...
async def test_slash_command_registration_stays_under_discord_limit(adapter):
    ...
    with patch("athena_cli.plugins.get_plugin_commands", return_value=many_plugins):
        adapter._register_slash_commands()
...
@pytest.mark.asyncio
async def test_auto_create_thread_falls_back_to_athena_when_only_mentions(adapter):
...
def test_discord_auto_thread_config_bridge(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("DISCORD_AUTO_THREAD", raising=False)
    monkeypatch.setenv("ATHENA_HOME", str(athena_dir))
...
def test_register_skill_command_is_flat_not_nested(adapter):
    ...
    with patch(
        "athena_cli.commands.discord_skill_commands_by_category",
