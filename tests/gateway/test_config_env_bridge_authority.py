...
def _run_gateway_import(athena_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
    ...
    script = textwrap.dedent(
        ...
        for k in (
            ...
            "ATHENA_TURN_LEASE_TIMEOUT",
            "ATHENA_SESSION_STALL_TIMEOUT",
            "ATHENA_GATEWAY_BUSY_INPUT_MODE",
            "ATHENA_GATEWAY_BUSY_TEXT_MODE",
            "ATHENA_GATEWAY_PLATFORM_CONNECT_TIMEOUT",
            "ATHENA_TIMEZONE",
        ):
            v = os.environ.get(k)
            if v is not None:
    ...
    # otherwise intentionally clean so stale Athena settings cannot leak in.
    for k in (
        "PATH",
        "PYTHONPATH",
        "VIRTUAL_ENV",
...
def test_config_gateway_timeout_wins_over_stale_env(athena_home: Path) -> None:
    ...
    _write_env(athena_home, {
        ...
        "ATHENA_SESSION_STALL_TIMEOUT": "15",
    })
    ...
    assert env.get("ATHENA_SESSION_STALL_TIMEOUT") == "300"


def test_config_turn_lease_timeout_wins_over_stale_env(athena_home: Path) -> None:
    ...
    _write_env(
        athena_home,
        {"ATHENA_TURN_LEASE_TIMEOUT": "60"},
    )

    env = _run_gateway_import(athena_home, initial_env={})

    assert env.get("ATHENA_TURN_LEASE_TIMEOUT") == "600"
...
def test_default_turn_lease_timeout_overrides_stale_env_when_key_is_omitted(
    ...
    assert env.get("ATHENA_TURN_LEASE_TIMEOUT") == "5"
...
def test_config_platform_connect_timeout_supplies_env_when_unset(athena_home: Path) -> None:
    ...
    _write_config(athena_home, gateway_cfg={"platform_connect_timeout": 90})

    env = _run_gateway_import(athena_home, initial_env={})

    assert env.get("ATHENA_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "90"


def test_env_platform_connect_timeout_wins_over_config(athena_home: Path) -> None:
    """Unlike the agent.*/display.*/timezone bridges (config-authoritative),
    ATHENA_GATEWAY_PLATFORM_CONNECT_TIMEOUT is the manual-override escape hatch:
    ...
    env = _run_gateway_import(
        athena_home,
        initial_env={"ATHENA_GATEWAY_PLATFORM_CONNECT_TIMEOUT": "120"},
    )

    assert env.get("ATHENA_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "120"
