...
closure the PR changed, against a real temp ``ATHENA_HOME``.
"""
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "athena_cli.model_switch.list_picker_providers",
    ...
    monkeypatch.setattr(
        "athena_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
    # save_config writes to ``get_athena_home() / config.yaml`` — point it here.
    monkeypatch.setattr("athena_constants.get_athena_home", lambda: athena_home)
    monkeypatch.setattr("athena_cli.config.get_athena_home", lambda: athena_home)
    return cfg_path
