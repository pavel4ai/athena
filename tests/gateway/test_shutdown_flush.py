...
def test_recover_closes_owned_db_when_unexpected_exception_escapes(
    ...
    monkeypatch.setattr(
        "athena_state_registry.release_or_close", lambda _: setattr(db, "released", True)
    )
...
def test_get_flush_dir_uses_get_athena_home(tmp_path, monkeypatch):
    """Flush dir must use get_athena_home(), not hardcoded Path.home()."""
    import gateway.shutdown_flush as mod

    captured = {}

    def fake_get_athena_home():
        from pathlib import Path
        captured["called"] = True
        return tmp_path

    monkeypatch.setattr(
        "athena_constants.get_athena_home", fake_get_athena_home
    )
