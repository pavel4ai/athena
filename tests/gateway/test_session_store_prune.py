...
def test_session_store_default_db_uses_runtime_athena_home(tmp_path, monkeypatch):
    """SessionStore must honor runtime ATHENA_HOME when opening the default DB.

    Regression for the import-time DEFAULT_DB_PATH freeze: importing
    athena_state before a fixture redirected ATHENA_HOME used to pin every
    default SessionDB() at the developer's real ~/.athena/state.db.
    """
    config = GatewayConfig()
    fake_home = tmp_path / "alt_athena_home"
    fake_home.mkdir()
...
class TestReadmeSentinel:
    ...
    def test_save_writes_readme_sentinel_first(self, tmp_path):
        ...
        assert "athena sessions list" in raw["_README"]
