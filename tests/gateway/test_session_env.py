...
def test_set_session_env_sets_contextvars(monkeypatch):
    ...
    monkeypatch.delenv("ATHENA_SESSION_CHAT_TYPE", raising=False)
    ...
    assert get_session_env("ATHENA_SESSION_CHAT_TYPE") == "group"
    ...
    assert os.getenv("ATHENA_SESSION_CHAT_TYPE") is None
...
def test_clear_session_env_restores_previous_state(monkeypatch):
    ...
    assert get_session_env("ATHENA_SESSION_CHAT_TYPE") == ""
...
def test_cron_session_contextvar_preserves_legacy_env_fallback(monkeypatch):
    ...
    assert get_session_env("ATHENA_CRON_SESSION") == "1"
...
def test_cron_session_explicit_blank_masks_leaked_env(monkeypatch):
    ...
    try:
        assert get_session_env("ATHENA_CRON_SESSION") == ""
    finally:
        clear_session_vars(tokens)
