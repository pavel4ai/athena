...
Athena instances: a main gateway (user ``ubuntu``,
ATHENA_HOME=/home/ubuntu/.athena) and a demo gateway (user ``demo``,
ATHENA_HOME=/home/demo/.athena).  The demo gateway runs as another user, so
its ``/proc/<pid>/fd`` table is unreadable from the main instance and the
holder scan falls back to ``/proc/<pid>/cmdline`` + ``_looks_like_athena``.
The demo process's argv matches the Athena patterns exactly, so the fallback
...
rebuild in ``athena_state_schema._recover_stale_fts`` was deferred 42 times
...
athena); a genuine second instance with a DIFFERENT ATHENA_HOME is still
misjudged on current main (issue #92401).

Behavior contract: an uninspectable holder identified only by argv must be
counted unless its own argv proves it is scoped to a *different* Athena
...
# token lives under /home/demo/.athena, the binary name matches the Athena
# patterns, and nothing references the main instance's home or state.db.
DEMO_HOME_ARGV = [
    "/home/demo/.athena/athena-agent/athena",
    "gateway",
    "run",
]

# Same, spelled through the venv interpreter + athena launcher script.
DEMO_VENV_ARGV = [
    "/home/demo/.athena/athena-agent/venv/bin/python",
    "/home/demo/.athena/athena-agent/athena_cli/main.py",
    "gateway",
]

# A Athena-shaped argv with no absolute paths: cannot disprove that this
# process touches our state.db, so it must stay fail-closed.
AMBIGUOUS_ARGV = ["athena", "gateway", "run"]
...
class TestUninspectableHolderInstanceScope:
    def test_other_instance_argv_is_not_a_holder_of_our_db(self, tmp_path, monkeypatch):
        ...
        DIFFERENT Athena home → not a holder of our state.db."""
    ...
    def test_argv_referencing_our_db_stays_flagged(self, tmp_path, monkeypatch):
        ...
        for argv in (
            ["athena", f"--db={db_path}", "gateway"],
            ["athena", "checkpoint", f"{db_path}-wal"],
            ["athena", "--home", our_home, "gateway"],
        ):
            assert athena_state_holders._looks_like_athena(argv) or argv[0] == "athena"
            _install_fake_proc(monkeypatch, tmp_path, unreadable_pids=(222,))
