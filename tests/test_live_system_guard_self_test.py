...
def test_systemctl_status_passes_through():
    ...
    r = subprocess.run(
        ["systemctl", "--user", "status", "athena-gateway", "--no-pager"],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_systemctl_show_passes_through():
    r = subprocess.run(
        ["systemctl", "--user", "show", "athena-gateway", "--no-pager"],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_systemctl_unrelated_unit_passes_through():
    """systemctl restart of a non-athena unit is allowed (we only protect athena)."""
...
def test_subprocess_pkill_with_unrelated_pattern_passes_through():
    """``pkill -f some-unrelated-pattern`` (no athena/python) is fine."""
