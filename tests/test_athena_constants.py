...
class TestGetDefaultAthenaRoot:
    ...
    def test_athena_home_is_native(self, tmp_path, monkeypatch):
        """When ATHENA_HOME = ~/.athena, returns ~/.athena."""
        native = tmp_path / ".athena"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ATHENA_HOME", str(native))
        assert get_default_athena_root() == native

    def test_athena_home_is_profile(self, tmp_path, monkeypatch):
        """When ATHENA_HOME is a profile under ~/.athena, returns ~/.athena."""
        native = tmp_path / ".athena"
    ...
    def test_athena_home_is_docker(self, tmp_path, monkeypatch):
        """When ATHENA_HOME points outside ~/.athena (Docker), returns ATHENA_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ATHENA_HOME", str(docker_home))
        assert get_default_athena_root() == docker_home

    def test_athena_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any ATHENA_HOME outside ~/.athena is treated as the root."""
        custom = tmp_path / "my-athena-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ATHENA_HOME", str(custom))
        assert get_default_athena_root() == custom
    ...
    def test_no_athena_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/athena without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("ATHENA_HOME", raising=False)
        ...
        assert get_default_athena_root() == home / "AppData" / "Local" / "athena"
...
class TestAthenaManagedNode:
    ...
    def test_with_athena_node_path_prepends_existing_managed_dirs(self, tmp_path, monkeypatch):
        home = tmp_path / "athena"
        node_dir = home / "node"
        bin_dir = node_dir / "bin"
        node_dir.mkdir(parents=True)
        ...
        env = with_athena_node_path({"PATH": "system-node"})
