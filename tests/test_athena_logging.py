...
class TestSetupLogging:
    ...
    def test_creates_agent_log_handler(self, athena_home):
        ...
        agent_handlers = [
            h for h in athena_logging._queued_file_handlers
    ...
    def test_profile_routing_follows_context_home(self, athena_home, tmp_path):
        ...
        profile_home.mkdir()
        athena_logging.setup_logging(athena_home=athena_home)
        assert athena_logging.enable_profile_log_routing(
            [athena_home, profile_home]
        ) is True
...
class TestAddRotatingHandler:
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        handlers = [h for h in athena_logging._queued_file_handlers if isinstance(h, RotatingFileHandler)]
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        ...
        assert athena_logging._is_windows_concurrent_log_lock_timeout(
            RuntimeError("Cannot acquire lock after 20 attempts")
        )
        assert not athena_logging._is_windows_concurrent_log_lock_timeout(
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, athena_home):
        athena_logging.setup_logging(athena_home=athena_home)
        root = logging.getLogger()
        ...
        queue_handlers = [
            h for h in root.handlers if getattr(h, "_athena_queue", False)
        ]
        assert len(queue_handlers) == 1
        # The real file handlers are discoverable via the accessor.
        assert any(
            "agent.log" in getattr(h, "baseFilename", "")
            for h in athena_logging._queued_file_handlers
        )
