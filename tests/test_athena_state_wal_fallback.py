...
Without this fallback, users on NFS-mounted ``ATHENA_HOME`` silently lose
...
from athena_state import SessionDB, format_session_db_unavailable, get_last_init_error
from athena_state_wal import WalUnsupportedError, apply_wal_with_fallback
...
def _reset_wal_fallback_warned_paths():
    ...
    athena_state_wal._wal_delete_fallback_failed_paths.clear()
    yield
    athena_state_wal._wal_fallback_warned_paths.clear()
    athena_state_wal._wal_delete_fallback_failed_paths.clear()
...
class TestApplyWalWithFallback:
    ...
    def test_error_deduplicated_per_db_label(self, tmp_path, caplog):
        ...
        connection on every operation — see athena_cli/kanban_db.py).
...
class TestGetLastInitError:
    ...
    def test_captures_cause_on_failed_init(self, tmp_path):
        ...
        with patch("athena_state.sqlite3.connect", side_effect=gated_connect):
            with pytest.raises(sqlite3.OperationalError):
...
class TestFormatSessionDbUnavailable:
    ...
    def test_adds_nfs_hint_for_locking_protocol(self):
        ...
        athena_state._set_last_init_error("OperationalError: locking protocol")
