...
``Invoke-AthenaStep``, which starts the step with ``RedirectStandardOutput`` /
...
last of them to close it. ``athena update`` deliberately runs build steps with
stdout inherited (the tee-stderr runner in ``athena_cli/main.py``), so the
...
closes and ``Invoke-AthenaStep`` blocks for the life of the gateway.

Everything the hand-off owes the Desktop is downstream of that call:
``.athena-update-result.json`` is never written, ``.athena-update-in-progress``
is never cleared, and the Desktop is never relaunched -- so the app sits on
"Updating Athena" until the user kills the gateway by hand, and the stale marker
...
step writing to both pipes took 18.3s vs 0.29s. ``athena update`` is exactly this
...
class TestIdleWatchdogCountsUpdateLogGrowth:
    ...
    Real updates are stdout-silent for 40+ minutes: ``athena update`` captures
    the (very loud) Electron/vite build into ``logs/update.log`` — NOT the
    child's stdout (``athena_cli/update_cmd.py``, the update-log tee) — so the
    ...
    the ``ATHENA_UPDATE_STEP_IDLE_SECONDS`` override each fails a test here.
    """
...
def test_update_step_survives_pipe_leak_flood_and_live_child_stall(
    ...
    *leak* -- a step that spawns a grandchild with ``UseShellExecute = $false``
    ...
    asserts the grandchild was **still alive** when ``Invoke-AthenaStep``
    ...
    *logstall* -- a step that is silent on its pipes but appends to the
    ...
    This is the shape of every real ``athena update`` build: output streams to
    ...
    env = {
        **os.environ,
        ...
        "ATHENA_UPDATE_PIPE_DRAIN_SECONDS": "3",
        "ATHENA_UPDATE_STEP_IDLE_SECONDS": "3",
        "ATHENA_SELFTEST_HOLD_SECONDS": "45",
    }
    ...
    assert "PIPE-DRAIN SELF-TEST: PASS" in result.stdout, (
        ...
        "Athena' forever) or metered a chatty step (backpressure on the running "
