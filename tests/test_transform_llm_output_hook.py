...
from athena_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(athena_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <athena_home>/plugins/<name> and opt it in."""
    plugin_dir = athena_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ATHENA_HOME", str(tmp_path / "athena_empty"))
