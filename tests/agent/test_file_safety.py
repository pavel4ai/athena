...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_athena_env(self):
        """Athena' own .env inside ATHENA_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: athena internal .env is in ~/.athena/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.athena/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.athena/.env"))
        assert error is not None
