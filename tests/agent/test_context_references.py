...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_athena_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("ATHENA_HOME", str(tmp_path / ".athena"))

    athena_env = tmp_path / ".athena" / ".env"
    athena_env.parent.mkdir(parents=True)
    athena_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.athena/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
