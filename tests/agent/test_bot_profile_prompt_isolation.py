...
thread that did not bind the ATHENA_HOME ContextVar.
...
active-profile line resolved the home via the ambient ``get_athena_home()``,
so an unbound build thread fell back to ``~/.athena`` (default) and leaked
default's full skills index + "Active Athena profile: default" into a bot's
...
def test_skills_prompt_scoped_to_override_not_ambient_home(tmp_path, monkeypatch):
    """An explicit skills_dir_override wins over ambient ATHENA_HOME, on a
    ...
    def build():
        # No set_athena_home_override on THIS thread — ambient resolves to
...
def test_profile_name_correct_on_bound_profile_session(tmp_path, monkeypatch):
    ...
    ``get_athena_home()/profiles`` as the root would never match and every
    profile would misreport as \"default\". The name must derive from the
    athena ROOT (get_default_athena_root)."""
    from agent import system_prompt

    bot_home = tmp_path / "profiles" / "mybot"
    bot_home.mkdir(parents=True)

    # Bound session: ATHENA_HOME env points at the profile dir itself.
    monkeypatch.setenv("ATHENA_HOME", str(bot_home))
