...
def test_run_review_consolidate_override_runs_llm(curator_env, monkeypatch):
    ...
    the LLM consolidation pass — mirrors `athena curator run --consolidate`."""
    c = curator_env["curator"]
    u = curator_env["usage"]
    skills_dir = curator_env["home"] / "skills"
    _write_skill(skills_dir, "a")
...
def test_curator_slot_is_canonical_aux_task():
    ...
    from athena_cli.main import _AUX_TASKS
    from athena_cli.web_server import _AUX_TASK_SLOTS
