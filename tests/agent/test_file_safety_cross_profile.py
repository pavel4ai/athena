...
Reference: May 2026 incident — a athena-security profile session
accidentally edited skills under both ~/.athena/profiles/athena-security/skills/
AND ~/.athena/skills/ (the default profile's skills), realizing only
...
def _set_active_home(monkeypatch, athena_home: Path):
    """Point file_safety._athena_home_path at a specific profile dir."""
    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_athena_home_path", lambda: athena_home)
...
class TestResolveActiveProfileName:
    def test_default_when_home_is_root(self, fake_athena, monkeypatch):
        _set_active_home(monkeypatch, fake_athena["default_home"])
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "default"

    def test_named_profile(self, fake_athena, monkeypatch):
        _set_active_home(monkeypatch, fake_athena["security_home"])
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "athena-security"

    def test_falls_back_to_default_on_resolution_failure(self, fake_athena, monkeypatch):
        """If ATHENA_HOME resolution raises, return 'default' rather than crashing the tool."""
        import agent.file_safety as fs

        def _boom():
            raise RuntimeError("simulated")

        monkeypatch.setattr(fs, "_athena_home_path", _boom)
...
class TestClassifyCrossProfileTarget:
    def test_same_profile_write_returns_none(self, fake_athena, monkeypatch):
        _set_active_home(monkeypatch, fake_athena["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_athena["security_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is None

    def test_security_writing_default_skill(self, fake_athena, monkeypatch):
        """The exact incident from May 2026."""
        _set_active_home(monkeypatch, fake_athena["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_athena["default_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["active_profile"] == "athena-security"
        assert result["target_profile"] == "default"
        assert result["area"] == "skills"

    def test_default_writing_security_skill(self, fake_athena, monkeypatch):
        ...
        assert result["target_profile"] == "athena-security"

    def test_named_to_named_cross_profile(self, fake_athena, monkeypatch):
        _set_active_home(monkeypatch, fake_athena["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_athena["coder_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["target_profile"] == "coder"

    @pytest.mark.parametrize("area", ["skills", "plugins", "cron", "memories"])
    def test_all_profile_scoped_areas_classified(self, fake_athena, monkeypatch, area):
        _set_active_home(monkeypatch, fake_athena["security_home"])
        from agent.file_safety import classify_cross_profile_target
        target = fake_athena["default_home"] / area / "foo.txt"
        result = classify_cross_profile_target(str(target))
        assert result is not None
        assert result["area"] == area

    def test_non_athena_path_returns_none(self, fake_athena, monkeypatch, tmp_path):
        _set_active_home(monkeypatch, fake_athena["security_home"])
        from agent.file_safety import classify_cross_profile_target
        # Path outside any Athena root
        assert classify_cross_profile_target(str(tmp_path / "random.txt")) is None

    def test_athena_config_not_classified_as_cross_profile(self, fake_athena, monkeypatch):
        ...
        result = classify_cross_profile_target(
            str(fake_athena["default_home"] / "config.yaml")
        )
        assert result is None
...
class TestGetCrossProfileWarning:
    def test_in_profile_returns_none(self, fake_athena, monkeypatch):
        _set_active_home(monkeypatch, fake_athena["security_home"])
    ...
    def test_cross_profile_warning_names_both_profiles(self, fake_athena, monkeypatch):
        _set_active_home(monkeypatch, fake_athena["security_home"])
        from agent.file_safety import get_cross_profile_warning
        ...
        assert "athena-security" in warn
        # Must name the bypass kwarg.
        assert "cross_profile=True" in warn
        # Must reference the area.
        assert "skills" in warn

    def test_warning_is_defense_in_depth_not_boundary(self, fake_athena, monkeypatch):
        _set_active_home(monkeypatch, fake_athena["security_home"])
        from agent.file_safety import get_cross_profile_warning
        warn = get_cross_profile_warning(
            str(fake_athena["default_home"] / "skills" / "foo" / "SKILL.md")
