...
path via ``athena chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp ATHENA_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point ATHENA_HOME at a temp dir and turn moa.save_traces on."""
    athena_home = tmp_path / ".athena"
    athena_home.mkdir()
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))

    # save_moa_turn reads config via athena_cli.config.load_config; stub it to
