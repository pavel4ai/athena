...
def test_athena_client_tag_includes_current_version():
    """The client tag must reflect athena_cli.__version__ verbatim."""
    from athena_cli import __version__
    from agent.portal_tags import athena_client_tag

    assert athena_client_tag() == f"client=athena-client-v{__version__}"


def test_athena_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import athena_client_tag

    tag = athena_client_tag()
    assert tag.startswith("client=athena-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import athena_client_tag, nous_portal_tags
