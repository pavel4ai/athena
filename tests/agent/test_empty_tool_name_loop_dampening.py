...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated ATHENA_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="athena_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".athena"))
    prev_home = os.environ.get("ATHENA_HOME")
    os.environ["ATHENA_HOME"] = os.path.join(test_home, ".athena")
    ...
    try:
        ...
        # references into athena_cli.*/tools.* and their monkeypatches would otherwise
        ...
        if prev_home is None:
            ...
            os.environ["ATHENA_HOME"] = prev_home
