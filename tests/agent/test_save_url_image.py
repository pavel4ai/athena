...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate ATHENA_HOME under tmp_path."""
    monkeypatch.setenv("ATHENA_HOME", str(tmp_path / ".athena"))
    (tmp_path / ".athena").mkdir()

    # Force the constants/image cache helpers to re-read ATHENA_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("athena_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_athena_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under ATHENA_HOME — gateway cleanup
