...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("ATHENA_HOME", "/users/alice/.athena/profiles/backend-worker")
        monkeypatch.setenv("ATHENA_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "ATHENA_KANBAN_DB",
            "/users/alice/.athena/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.athena/kanban/boards/smoke"]'
            in cmd
        )
...
class TestSpawnEnvSecretStripping:
    """codex app-server routes its spawn env through athena_subprocess_env(
    ...
    credentials to authenticate, but it must NOT inherit Tier-1 Athena secrets
    ...
    def test_tier1_and_internal_secrets_stripped_from_spawn_env(self, monkeypatch):
        ...
        for var in (
            ...
            "ATHENA_DASHBOARD_SESSION_TOKEN", "AUXILIARY_VISION_API_KEY",
