...
manipulation. Athena keeps canonical conversation history in state.db and may
...
def fake_homes(tmp_path, monkeypatch):
    """Point ATHENA_HOME at a temp profile dir.

    Uses the real env-var resolution chain (get_athena_home /
    get_default_athena_root) instead of monkeypatching private helpers —
    ...
    ATHENA_HOME=<root>/profiles/<name> makes get_default_athena_root()
