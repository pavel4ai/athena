...
Companion to ``tests/athena_cli/test_xai_oauth_writethrough.py``. That file
...
from athena_cli import auth as A
...
def profile_and_root(tmp_path, monkeypatch):
    ...
    only refuses the *real* user's ``$HOME/.athena/auth.json``; a tmp_path
...
def test_codex_pool_refresh_holds_auth_store_lock_across_post(monkeypatch, tmp_path):
    ...
    Codex refresh tokens are single-use. If two Athena processes both read the
...
def test_athena_pkce_refresh_writes_back_to_singleton(tmp_path, monkeypatch):
    """A successful athena_pkce refresh must update
    ~/.athena/.anthropic_oauth.json, or ``_seed_from_singletons()`` on the
    ...
    _write_store(athena_home / "auth.json", {"version": 1, "providers": {}})
    ...
    assert on_disk["refreshToken"] == "rt1", (
        "successful athena_pkce refresh must write back to "
        "~/.athena/.anthropic_oauth.json, or _seed_from_singletons() will "
        "revert the pool entry to the pre-refresh (spent) token on next load"
    )

    reloaded = load_pool("anthropic")
    reloaded_entries = [e for e in reloaded.entries() if e.source.endswith("athena_pkce")]
    assert reloaded_entries, "athena_pkce entry should still be present after reload"
...
def test_manual_athena_pkce_refresh_does_not_create_duplicate_singleton(
    tmp_path, monkeypatch
):
    """A pool-owned manual:athena_pkce entry must not create a second source."""
    athena_home = tmp_path / "athena"
    athena_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))
    monkeypatch.setattr("athena_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    ...
    assert not oauth_file.exists(), (
        "manual:athena_pkce is already pool-owned; refreshing it must not "
        "create a second athena_pkce singleton source"
    )

    reloaded = load_pool("anthropic")
    matching = [e for e in reloaded.entries() if e.id == "manual-entry"]
    assert len(matching) == 1
    assert matching[0].source == "manual:athena_pkce"
