...
class TestTruncateContent:
    ...
    def _reset_truncation_state(self, monkeypatch):
        ...
        monkeypatch.setattr("athena_cli.config.load_config_readonly", default_load_config)
...
class TestDynamicContextFileCap:
    ...
    def _no_explicit_config(self, monkeypatch):
        ...
        monkeypatch.setattr("athena_cli.config.load_config_readonly", lambda: {})
...
class TestBuildContextFilesPrompt:
    ...
    def test_athena_md_still_wins_over_agents_override(self, tmp_path):
        (tmp_path / ".athena.md").write_text("Athena-first context.")
        (tmp_path / "AGENTS.override.md").write_text("Override context.")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert "Athena-first context" in result
...
class TestFindAthenaMd:
    ...
    def test_no_git_root_checks_cwd_only(self, tmp_path):
        ...
        the way to / and pick up a .athena.md planted in /tmp, /home, or /
...
class TestEnvironmentHints:
    ...
    def test_probe_remote_backend_ssh_is_probe_only_and_torn_down(self, monkeypatch):
        ...
        ~/.athena and snapshot a session just to run `uname`, and its __del__ would
...
class TestContextFileReadTimeout:
    def test_slow_athena_md_is_skipped_and_agents_md_still_loads(self, tmp_path, monkeypatch, caplog):
        (tmp_path / ".git").mkdir()
        ...
        assert "Athena project rules" not in result
