"""Tests for the top-level `./athena` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./athena` should use `athena_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "athena"
    called = []

    fake_main_module = types.ModuleType("athena_cli.main")

    def fake_main():
        called.append("athena_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "athena_cli.main", fake_main_module)
