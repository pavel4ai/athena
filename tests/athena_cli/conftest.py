"""Fixtures shared across athena_cli tests."""
...
def _suppress_concurrent_athena_gate(request, monkeypatch):
    """Default ``_detect_concurrent_athena_instances`` to ``[]`` for every test.

    The Windows update path now refuses to proceed when another
    ``athena.exe`` is detected (issue #26670). On a developer's Windows
    machine running the test suite via ``athena`` itself, this would
    flag the running agent as a concurrent instance and abort every
    ``cmd_update`` test. Tests that want to exercise the gate explicitly
    re-patch ``_detect_concurrent_athena_instances`` with their own
    ...
    if request.node.get_closest_marker("real_concurrent_gate"):
        return
    try:
        from athena_cli import main as _cli_main
    except Exception:
        return
    # raising=False: under pytest's per-test spawn isolation, a concurrent
    # xdist worker importing a module that transitively touches athena_cli.main
    # can briefly expose a partially-initialized module object here — one where
    # _detect_concurrent_athena_instances isn't defined yet. A bare setattr
...
def isolated_update_runtime(monkeypatch, tmp_path, request):
    ...
    from athena_cli import gateway, main, update_cmd, update_cmd_fleet
    from athena_cli import update_inventory, update_receipt
    ...
    monkeypatch.setattr(main, "_purge_stale_athena_modules", lambda: None)
