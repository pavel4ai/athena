"""ensure_athena_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".athena"
    second = tmp_path / "b" / ".athena"

    monkeypatch.setenv("ATHENA_HOME", str(first))
    cfg.ensure_athena_home()

    # Profile switch: ATHENA_HOME moves → the new path is ensured too.
