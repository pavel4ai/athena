...
def container_env(tmp_path, monkeypatch):
    """Set up a fake ATHENA_HOME with .container-mode file."""
    athena_home = tmp_path / ".athena"
    athena_home.mkdir()
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))
    monkeypatch.delenv("ATHENA_DEV", raising=False)

    container_mode = athena_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=athena-agent\n"
        "exec_user=athena\n"
        "athena_bin=/data/current-package/bin/athena\n"
    )
    return athena_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("athena_constants.is_container", return_value=False):
...
def test_get_container_exec_info_skipped_when_athena_dev(container_env, monkeypatch):
    """Returns None when ATHENA_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("ATHENA_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_athena_dev_zero(container_env, monkeypatch):
    """ATHENA_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("ATHENA_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        athena_home = Path(tmpdir) / ".athena"
        athena_home.mkdir()
        (athena_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("athena_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_athena_home": lambda: athena_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ATHENA_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=athena-custom\n"
        "exec_user=myuser\n"
        "athena_bin=/opt/athena/bin/athena\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "athena-custom"
    assert info["exec_user"] == "myuser"
    assert info["athena_bin"] == "/opt/athena/bin/athena"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from athena_cli.main import _exec_in_container
    ...
    assert "/data/current-package/bin/athena" in cmd
    assert "chat" in cmd
