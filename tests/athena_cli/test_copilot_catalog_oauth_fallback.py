...
``athena auth add copilot`` or by ``_seed_from_env`` when the env var
is set in ``~/.athena/.env`` — the picker was silently dropping back to
...
from athena_cli.models import _resolve_copilot_catalog_api_key
