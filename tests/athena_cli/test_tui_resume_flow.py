...
def test_termux_fast_cli_launch_chat_uses_light_parser(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("ATHENA_TUI", raising=False)
    monkeypatch.setattr(
...
def test_termux_fast_cli_launch_bare_defers_agent_startup(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("ATHENA_TUI", raising=False)
    monkeypatch.delenv("ATHENA_DEFER_AGENT_STARTUP", raising=False)
    monkeypatch.delenv("ATHENA_FAST_STARTUP_BANNER", raising=False)
    ...
    assert os.environ["ATHENA_DEFER_AGENT_STARTUP"] == "1"
    assert os.environ["ATHENA_FAST_STARTUP_BANNER"] == "1"
...
def test_termux_ultrafast_version_runs_before_heavy_startup(
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("ATHENA_TERMUX_DISABLE_FAST_CLI", raising=False)
    monkeypatch.setattr(sys, "argv", ["athena", "--version"])
...
def test_termux_fast_cli_launch_can_be_disabled(monkeypatch, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setenv("ATHENA_TERMUX_DISABLE_FAST_CLI", "1")
    monkeypatch.delenv("ATHENA_TUI", raising=False)
    monkeypatch.setattr(sys, "argv", ["athena", "version"])
...
def test_termux_bundled_skills_stamp_controls_sync(monkeypatch, tmp_path, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setattr(main_mod, "get_athena_home", lambda: tmp_path)
    ...
    monkeypatch.setenv("ATHENA_TERMUX_FORCE_SKILLS_SYNC", "1")
    assert main_mod._termux_bundled_skills_sync_needed() is True
...
def test_oneshot_fails_closed_on_empty_final_response(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    import athena_cli.oneshot as oneshot_mod
...
def test_oneshot_filters_invalid_toolsets_before_redirect(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    from athena_cli.oneshot import _validate_explicit_toolsets
...
def test_oneshot_rejects_disabled_mcp_toolset(monkeypatch, capsys):
    ...
    assert valid is None
    assert error == "athena -z: --toolsets did not contain any valid toolsets.\n"
    err = capsys.readouterr().err
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """athena -z bypasses AthenaCLI, but recall still needs SessionDB."""
    from athena_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "athena_state", mod("athena_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "athena_cli.config",
        mod("athena_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "athena_cli.models",
        mod("athena_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        ...
        mod("athena_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["ATHENA_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/athena-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["ATHENA_MODEL"] == "nous/athena-test"
    assert env["ATHENA_INFERENCE_MODEL"] == "nous/athena-test"
    assert env["ATHENA_TUI_PROVIDER"] == "nous"
    assert env["ATHENA_INFERENCE_PROVIDER"] == "nous"
    assert env["ATHENA_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["ATHENA_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("athena-tui-active-session-")
...
def test_launch_tui_applies_terminal_backend_config(
    monkeypatch, main_mod, _isolate_athena_home
):
    captured = {}
    ...
    assert captured["env"]["TERMINAL_DOCKER_IMAGE"] == "example/athena-tools:latest"
...
def test_launch_tui_exit_code_42_relaunches_update(monkeypatch, main_mod):
    ...
    with patch("athena_cli.relaunch.relaunch") as mock_relaunch:
        with pytest.raises(SystemExit) as exc:
            main_mod._launch_tui()
...
def test_launch_tui_drops_stale_resume_env_without_resume_arg(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("ATHENA_TUI_RESUME", "stale-missing-session")
    monkeypatch.setattr(
        main_mod,
        "_make_tui_argv",
        lambda tui_dir, tui_dev: (["node", "dist/entry.js"], Path(".")),
    ...
    assert "ATHENA_TUI_RESUME" not in captured["env"]
...
def test_launch_tui_sets_resume_env_from_resume_arg(monkeypatch, main_mod):
    ...
    assert captured["env"]["ATHENA_TUI_RESUME"] == "20260518_000000_goodid"


def test_make_tui_argv_dev_prebuilds_athena_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("ATHENA_TUI_DIR", raising=False)
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    ...
    monkeypatch.setitem(
        sys.modules, "athena_state", types.SimpleNamespace(SessionDB=lambda: _FakeDB())
    )
    ...
    assert "athena --tui --resume 20260409_000001_abc123" in out
    assert 'athena --tui -c "demo title"' in out
...
def test_print_tui_exit_summary_prefers_actual_active_session_file(
    ...
    assert "athena --tui --resume actual_session" in out
