...
When ``ATHENA_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.athena/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.athena            (has its own auth.json fixture)
    * Profile     -> tmp_path/.athena/profiles/coder   (active, ATHENA_HOME points here)
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    athena_home = tmp_path / "classic"
    athena_home.mkdir()
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))

    _write(athena_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (ATHENA_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.athena as the "real home" — which is NOT equal
    # to the ATHENA_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    athena_home = tmp_path / "classic"
    ...
    _write(athena_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from athena_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: ATHENA_HOME is set to a custom path that is NOT under
    # ~/.athena/profiles/ — get_default_athena_root() returns ATHENA_HOME
