...
from athena_cli import gateway, main, update_cmd_fleet as fleet
...
def test_pending_marker_requires_complete_systemd_recovery(monkeypatch, tmp_path, failure):
    ...
    def systemctl(cmd, **kw):
        if "list-units" in cmd:
            ...
            return SimpleNamespace(returncode=int(failure == "listing"), stdout=(
                "athena-gateway-one.service loaded active running\n"
                "athena-gateway-two.service loaded failed failed\n"), stderr="")
        bad = cmd[-1] == "athena-gateway-two"
        if "restart" in cmd:
    ...
    if failure not in (None, "running"):
        ...
        assert set(recovered) == {"athena-gateway-one", "athena-gateway-two"}
...
def test_pending_launchd_requires_complete_supervision(monkeypatch, tmp_path, failure):
    ...
    current, sibling = "ai.athena.gateway", "ai.athena.gateway-two"
