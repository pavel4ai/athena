...
from athena_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from athena_cli.auth import _resolve_verify
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("ATHENA_CA_BUNDLE", raising=False)
        result = _resolve_verify(auth_state={"tls": {}})
        assert result is True

    def test_missing_athena_ca_bundle_env_falls_back(self, monkeypatch):
        from athena_cli.auth import _resolve_verify

        monkeypatch.setenv("ATHENA_CA_BUNDLE", "/nonexistent/athena-ca.pem")
...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))
    monkeypatch.setenv("ATHENA_AGENT_USE_LEGACY_SESSION_KEYS", "true")
...
def test_nous_inference_auth_logs_do_not_include_secret_values(
    ...
    caplog.set_level(logging.INFO, logger="athena_cli.auth")
    auth_mod.resolve_nous_runtime_credentials(
        force_refresh=True,
    )
...
def test_get_nous_auth_status_pool_opaque_key_is_not_inference_credential(tmp_path, monkeypatch):
    from athena_cli.auth import get_nous_auth_status, invalidate_nous_auth_status_cache
...
def test_get_nous_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    ...
    athena_home = tmp_path / "athena"
    _setup_nous_auth(athena_home, access_token="at-123")
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))
    monkeypatch.setattr(
...
def test_get_nous_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    ...
    athena_home = tmp_path / "athena"
    _setup_nous_auth(athena_home, access_token="at-fresh")
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))
...
def test_get_nous_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("athena_cli.auth.resolve_nous_runtime_credentials", _boom)
...
def test_refresh_token_persisted_when_refreshed_jwt_lacks_invoke_scope(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("athena_cli.auth._refresh_access_token", _fake_refresh_access_token)
...
def test_managed_access_token_refresh_failure_quarantines_tokens(
    ...
    athena_home = tmp_path / "athena"
    _setup_nous_auth(athena_home, refresh_token="refresh-old")
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))
    from agent.credential_pool import load_pool
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `athena model` → Nous Portal → Skip (keep current) after
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        return athena_home, config_path, auth_path
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from athena_cli.auth import PROVIDER_REGISTRY, _login_nous

        athena_home, config_path, auth_path = self._setup_home_with_openrouter(
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `athena auth add nous`
    ...
    """
    from athena_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `athena auth add nous --type oauth --label <name>` silently
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    """
    from athena_cli.auth import _refresh_access_token
    ...
    assert "athena auth add nous" in message.lower()
...
def shared_store_env(tmp_path, monkeypatch):
    """Redirect ATHENA_SHARED_AUTH_DIR to a tmp_path.
    ...
    monkeypatch.setenv("ATHENA_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without ATHENA_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.athena/shared/`` across CI runs.
    """
    from athena_cli.auth import _nous_shared_store_path

    monkeypatch.delenv("ATHENA_SHARED_AUTH_DIR", raising=False)
...
def test_shared_store_honors_env_override(tmp_path, monkeypatch):
    """ATHENA_SHARED_AUTH_DIR must redirect the path."""
    from athena_cli.auth import _nous_shared_store_path, NOUS_SHARED_STORE_FILENAME

    custom_dir = tmp_path / "custom_shared"
    monkeypatch.setenv("ATHENA_SHARED_AUTH_DIR", str(custom_dir))
...
def test_shared_store_read_missing_returns_none(shared_store_env):
    ...
    from athena_cli.auth import _read_shared_nous_state
...
def test_shared_store_read_malformed_returns_none(shared_store_env):
    ...
    from athena_cli.auth import _nous_shared_store_path, _read_shared_nous_state
...
def test_shared_store_write_skips_when_refresh_token_missing(shared_store_env):
    ...
    from athena_cli.auth import _nous_shared_store_path, _write_shared_nous_state
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    AND the shared store, so a future profile's `athena auth add nous
    ...
    athena_home = tmp_path / "athena"
    athena_home.mkdir(parents=True, exist_ok=True)
    (athena_home / "auth.json").write_text(
...
def test_try_import_shared_returns_none_when_store_missing(shared_store_env):
    ...
    from athena_cli.auth import _try_import_shared_nous_state
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    assert result["client_id"] == "athena-cli"
...
def test_shared_store_survives_across_profile_switch(
    ...
    (different ATHENA_HOME) sees the same shared state and can rehydrate
    ...
    monkeypatch.setenv("ATHENA_HOME", str(profile_a))
    ...
    # Profile B: fresh ATHENA_HOME, no auth yet, but the shared store
    ...
    monkeypatch.setenv("ATHENA_HOME", str(profile_b))
