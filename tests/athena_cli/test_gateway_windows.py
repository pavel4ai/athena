"""Tests for athena_cli.gateway_windows."""
...
import athena_cli.setup as setup
...
def test_build_gateway_argv_uses_base_pythonw_for_uv_venv_launcher(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda athena_home: "")
    monkeypatch.setattr("athena_cli.config.get_athena_home", lambda: str(athena_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(base_pythonw), "-m", "athena_cli.main"]
    assert cwd == str(athena_home.resolve())
...
class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_athena_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".athena"
        home.mkdir()
        monkeypatch.setattr("athena_cli.config.get_athena_home", lambda: home)
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        missing = tmp_path / "missing" / ".athena"
        project = tmp_path / "checkout"
        monkeypatch.setattr("athena_cli.config.get_athena_home", lambda: missing)
        assert gateway_windows._stable_gateway_working_dir(project) == str(project)


def test_write_task_script_anchors_cmd_cd_at_athena_home(monkeypatch, tmp_path):
    project = tmp_path / "project"
    athena_home = tmp_path / "athena-home"
    athena_home.mkdir()
    ...
    assert f"cd /d {gateway_windows._quote_cmd_script_arg(str(athena_home.resolve()))}" in content
...
def test_install_startup_fallback_does_not_auto_spawn_when_gateway_stopped(monkeypatch, tmp_path, capsys):
    ...
    assert "athena --profile alice gateway start" in out
...
# resume_pending=True) never fires when `athena gateway stop` kills the
...
def test_stop_writes_planned_stop_marker_before_killing(monkeypatch):
    ...
    monkeypatch.setattr("athena_cli.gateway.kill_gateway_processes", fake_kill)
    monkeypatch.setattr("athena_cli.gateway._get_restart_drain_timeout", lambda: 5.0)
...
def test_stop_escalates_to_force_kill_when_drain_times_out(monkeypatch):
    ...
    `athena gateway start` fails with "another instance is running".
    """
    pid = 77777
    events = []
    ...
    monkeypatch.setattr("athena_cli.gateway._get_restart_drain_timeout", lambda: 1.0)
