"""Tests for the shared tar.gz writer (``athena_cli.archive_safe.make_targz``).

``make_targz`` backs both ``athena profile export`` and ``athena kanban
...
from athena_cli.archive_safe import make_targz
