"""Tests for the kanban CLI surface (athena_cli.kanban)."""
...
def test_run_slash_session_filter(kanban_home):
    """`athena kanban list --session <id>` filters by the originating
...
def test_kanban_in_autocomplete_table():
    from athena_cli.commands import COMMANDS, SUBCOMMANDS
...
def test_run_slash_board_override_restores_prior_env(kanban_home, monkeypatch):
    kb.create_board("alpha")
    kb.create_board("beta")
    monkeypatch.setenv("ATHENA_KANBAN_BOARD", "beta")

    kc.run_slash("--board alpha list")

    assert os.environ.get("ATHENA_KANBAN_BOARD") == "beta"
