"""Tests for athena_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("athena_cli.skills_config.save_config")
...
class TestIsSkillDisabled:
    @patch("athena_cli.config.load_config")
    ...
    @patch("athena_cli.config.load_config")
    @patch.dict("os.environ", {"ATHENA_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """ATHENA_SESSION_PLATFORM should be used when ATHENA_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_athena_platform_takes_precedence(self, tmp_path, monkeypatch):
        """ATHENA_PLATFORM should win over ATHENA_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from athena_cli.skills_config import _get_categories
        skills = [
