"""Tests for athena backup and import commands."""
...
def _make_athena_tree(root: Path) -> None:
    """Create a realistic ~/.athena directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    (root / "memory_store.db").write_bytes(b"fake-sqlite")
    (root / "athena_state.db").write_bytes(b"fake-state")
...
class TestShouldExclude:
    def test_excludes_athena_agent(self):
        from athena_cli.backup import _should_exclude
    ...
    def test_includes_nested_athena_agent_in_skills(self):
    ...
    def test_excludes_regeneratable_dependency_and_cache_dirs(self, rel):
        """Python dep trees and tool caches under ATHENA_HOME must be skipped —
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        athena_home.mkdir()
        _make_athena_tree(athena_home)

        monkeypatch.setenv("ATHENA_HOME", str(athena_home))
        # get_default_athena_root needs this
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        import athena_cli.backup as backup_mod
        staged_dirs = []
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = athena_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, athena_home)

        assert result is not None
        assert staged_dirs, "no SQLite snapshot was staged"
        assert all(d == str(out_zip.parent) for d in staged_dirs), staged_dirs

    def test_excludes_athena_agent(self, tmp_path, monkeypatch):
    ...
    def test_excludes_dependency_and_cache_trees(self, tmp_path, monkeypatch):
        """A plugin venv / site-packages / pip cache under ATHENA_HOME must be
        ...
        venv_pkg = athena_home / "plugins" / "heavy" / ".venv" / "lib" / "site-packages" / "dep"
        venv_pkg.mkdir(parents=True)
        (venv_pkg / "__init__.py").write_text("# dep\n")
        pip_cache = athena_home / ".cache" / "uv" / "wheels"
    ...
    def test_includes_nested_athena_agent_in_skills(self, tmp_path, monkeypatch):
        ...
        nested = athena_home / "skills" / "autonomous-ai-agents" / "athena-agent"
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/athena-backup-*.zip."""
        athena_home = tmp_path / ".athena"
        athena_home.mkdir()
        (athena_home / "config.yaml").write_text("model: test\n")
        ...
        zips = list(tmp_path.glob("athena-backup-*.zip"))
        assert len(zips) == 1

    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside ATHENA_HOME."""
        athena_home = tmp_path / ".athena"
        athena_home.mkdir()
        _make_athena_tree(athena_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(athena_home / "skills" / "outside-link.txt", outside)
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        ...
        from athena_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only athena_state.db (old wrong name) is rejected."""
        from athena_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["athena_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into athena home."""
        athena_home = tmp_path / ".athena"
        athena_home.mkdir()
        monkeypatch.setenv("ATHENA_HOME", str(athena_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (athena_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (athena_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (athena_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (athena_home / "profiles" / "coder" / "config.yaml").exists()

    def test_strips_athena_prefix(self, tmp_path, monkeypatch):
        """Import strips .athena/ prefix if all entries share it."""
        athena_home = tmp_path / ".athena"
        athena_home.mkdir()
        monkeypatch.setenv("ATHENA_HOME", str(athena_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".athena/config.yaml": "model: test\n",
            ".athena/skills/a/SKILL.md": "# A\n",
        })
        ...
        assert (athena_home / "config.yaml").read_text() == "model: test\n"
        assert (athena_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_athena_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a athena backup."""
        athena_home = tmp_path / ".athena"
        athena_home.mkdir()
        monkeypatch.setenv("ATHENA_HOME", str(athena_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        assert (athena_home / "config.yaml").exists()
        # traversal file should NOT exist outside athena home
    ...
    def test_preserves_live_gateway_state(self, tmp_path, monkeypatch):
        ...
        (athena_home / "gateway_state.json").write_text(live_state)
        ...
        assert (athena_home / "gateway_state.json").read_text() == live_state
    ...
    def test_does_not_seed_gateway_state_when_absent(self, tmp_path, monkeypatch):
        ...
        assert (athena_home / "config.yaml").exists()
        assert not (athena_home / "gateway_state.json").exists()
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        athena_home = tmp_path / ".athena"
        (athena_home / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("ATHENA_HOME", str(athena_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        live_state = '{"gateway_state": "running"}'
        (athena_home / "profiles" / "coder" / "gateway_state.json").write_text(live_state)
        ...
        assert (athena_home / "profiles" / "coder" / "config.yaml").read_text() == "model: anthropic\n"
        assert (
            athena_home / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (athena_home / "gateway.pid").write_text("4242")
        (athena_home / "processes.json").write_text('{"live": true}')
        ...
        assert (athena_home / "gateway.pid").read_text() == "4242"
        assert (athena_home / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (athena_home / "cron.pid").exists()
        assert not (athena_home / "gateway.lock").exists()
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (athena_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("ATHENA_HOME", str(athena_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (athena_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (athena_home / "config.yaml").read_text() == "model: restored\n"
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (athena_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".athena"
        src_home.mkdir(parents=True)
        _make_athena_tree(src_home)

        monkeypatch.setenv("ATHENA_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from athena_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".athena"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("ATHENA_HOME", str(dst_home))
...
class TestFormatSize:
    def test_bytes(self):
        from athena_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without athena markers fails validation."""
        import io
        from athena_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_athena(self):
        """Detects .athena/ prefix wrapping all entries."""
        import io
        from athena_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".athena/config.yaml", "test")
            zf.writestr(".athena/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".athena/"
...
class TestBackupEdgeCases:
    def test_nonexistent_athena_home(self, tmp_path, monkeypatch):
        """Backup exits when athena home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".athena"
        monkeypatch.setenv("ATHENA_HOME", str(fake_home))
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("athena-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_athena_home(self, tmp_path, monkeypatch):
        """Backup handles empty athena home (no files to back up)."""
        athena_home = tmp_path / ".athena"
        athena_home.mkdir()
        # Only excluded dirs, no actual files
        (athena_home / "__pycache__").mkdir()
        (athena_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_permission_error_during_backup(self, tmp_path, monkeypatch):
        ...
        bad_file = athena_home / "secret.db"
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        old_file = athena_home / "ancient.txt"
    ...
    def test_skips_output_zip_inside_athena(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside athena root."""
        athena_home = tmp_path / ".athena"
        athena_home.mkdir()
        (athena_home / "config.yaml").write_text("model: test\n")
        ...
        # Output inside athena home
        out_zip = athena_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        athena_home = tmp_path / ".athena"
        athena_home.mkdir()
        (athena_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("ATHENA_HOME", str(athena_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        athena_home = tmp_path / ".athena"
        athena_home.mkdir()
        (athena_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("ATHENA_HOME", str(athena_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (athena_home / "config.yaml").exists()
        assert (athena_home / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (athena_home / "profiles" / "coder" / "config.yaml").exists()
        assert (athena_home / "profiles" / "researcher" / "config.yaml").exists()
        ...
        assert "athena -p coder" in coder_wrapper
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from athena_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    @pytest.fixture
    def athena_home(self, tmp_path):
        """Create a fake ATHENA_HOME with critical state files."""
    ...
    def test_creates_snapshot(self, athena_home):
        from athena_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(athena_home=athena_home)
        assert snap_id is not None
        snap_dir = athena_home / "state-snapshots" / snap_id
        assert snap_dir.is_dir()
        assert (snap_dir / "manifest.json").exists()

    def test_label_in_id(self, athena_home):
        from athena_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", athena_home=athena_home)
        assert "before-upgrade" in snap_id

    def test_state_db_safely_copied(self, athena_home):
        from athena_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(athena_home=athena_home)
        db_copy = athena_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_copies_nested_files(self, athena_home):
        from athena_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(athena_home=athena_home)
        assert (athena_home / "state-snapshots" / snap_id / "cron" / "jobs.json").exists()

    def test_copies_channel_aliases(self, athena_home):
        from athena_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(athena_home=athena_home)
        copied = athena_home / "state-snapshots" / snap_id / "channel_aliases.json"
        assert copied.exists()
        assert "120363408391911677@g.us" in copied.read_text()

    def test_missing_files_skipped(self, athena_home):
        from athena_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(athena_home=athena_home)
        with open(athena_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(athena_home=empty) is None

    def test_list_snapshots(self, athena_home):
        from athena_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", athena_home=athena_home)
        id2 = create_quick_snapshot(label="second", athena_home=athena_home)

        snaps = list_quick_snapshots(athena_home=athena_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, athena_home):
        from athena_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", athena_home=athena_home)
        snaps = list_quick_snapshots(limit=3, athena_home=athena_home)
        assert len(snaps) == 3

    def test_restore_config(self, athena_home):
        from athena_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(athena_home=athena_home)

        (athena_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (athena_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, athena_home=athena_home)
        assert result is True
        assert "openrouter" in (athena_home / "config.yaml").read_text()

    def test_restore_state_db(self, athena_home):
        from athena_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(athena_home=athena_home)

        conn = sqlite3.connect(str(athena_home / "state.db"))
        conn.execute("INSERT INTO sessions VALUES ('s2', 'new')")
        conn.commit()
        conn.close()

        restore_quick_snapshot(snap_id, athena_home=athena_home)
    ...
    def test_restore_nonexistent(self, athena_home):
        from athena_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", athena_home=athena_home) is False

    def test_auto_prune(self, athena_home):
        from athena_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", athena_home=athena_home)
        snaps = list_quick_snapshots(limit=100, athena_home=athena_home)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, athena_home):
        from athena_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", athena_home=athena_home)
        deleted = prune_quick_snapshots(keep=3, athena_home=athena_home)
        assert deleted == 7
        assert len(list_quick_snapshots(athena_home=athena_home)) == 3

    def test_snapshot_includes_pairing_directories(self, athena_home):
        ...
        (athena_home / "platforms" / "pairing").mkdir(parents=True)
        (athena_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (athena_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (athena_home / "pairing").mkdir()
        (athena_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (athena_home / "feishu_comment_pairing.json").write_text(
    ...
    def test_restore_recovers_pairing_data(self, athena_home):
        ...
        assert restore_quick_snapshot(snap_id, athena_home=athena_home) is True
        assert approved.exists()
        assert '"alice"' in approved.read_text()
        assert feishu.exists()
        assert '"user_xyz"' in feishu.read_text()

    def test_empty_pairing_dir_does_not_fail(self, athena_home):
...
# ---------------------------------------------------------------------------
# Pre-update backup (athena update safety net)
# ---------------------------------------------------------------------------
    ...
    def test_restore_rejects_snapshot_id_traversal(self, athena_home):
    ...
    def test_restore_rejects_manifest_rel_traversal(self, athena_home):
        ...
        escape_dst = athena_home.parent.parent / "outside.txt"
        ...
        assert not escape_dst.exists(), (
            f"manifest rel traversal escaped ATHENA_HOME: {escape_dst} exists"
        )
...
class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``athena update``
    ...
    def athena_home(self, tmp_path):
        ...
        root.mkdir()
        _make_athena_tree(root)
        return root

    def test_creates_backup_under_backups_dir(self, athena_home):
        from athena_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(athena_home=athena_home)
        assert out is not None
        assert out.exists()
        assert out.parent == athena_home / "backups"
        assert out.name.startswith("pre-update-")
        assert out.suffix == ".zip"

    def test_backup_contents_match_full_backup(self, athena_home):
        """Pre-update backup should include the same user data that
        ``athena backup`` would, and should exclude the same directories."""
        from athena_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(athena_home=athena_home)
        assert out is not None
    ...
    def test_does_not_recurse_into_prior_backups(self, athena_home):
        """The ``backups/`` directory must be excluded so that each backup
        doesn't grow exponentially by including all prior backups."""
        from athena_cli.backup import create_pre_update_backup
        # First backup
        out1 = create_pre_update_backup(athena_home=athena_home)
        assert out1 is not None
        # Second backup — must not include the first
        out2 = create_pre_update_backup(athena_home=athena_home)
    ...
    def test_rotation_keeps_only_n(self, athena_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(athena_home=athena_home, keep=3)
            created.append(out)
            _t.sleep(1.05)  # ensure distinct seconds in timestamp

        remaining = sorted(
            p.name for p in (athena_home / "backups").iterdir()
    ...
    def test_rotation_preserves_manual_files(self, athena_home):
        """Hand-dropped zips in ``backups/`` must not be touched by
        rotation — it only prunes files matching ``pre-update-*.zip``."""
        import time as _t
        from athena_cli.backup import create_pre_update_backup

        (athena_home / "backups").mkdir(exist_ok=True)
        manual = athena_home / "backups" / "my-manual.zip"
        manual.write_bytes(b"manual backup")

        for _ in range(5):
            create_pre_update_backup(athena_home=athena_home, keep=2)
            _t.sleep(1.05)
    ...
    def test_returns_none_if_root_missing(self, tmp_path):
        from athena_cli.backup import create_pre_update_backup
        assert create_pre_update_backup(athena_home=tmp_path / "does-not-exist") is None

    def test_keep_zero_does_not_delete_freshly_created_backup(self, athena_home):
        ...
        """
        from athena_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(athena_home=athena_home, keep=0)
        assert out is not None
        assert out.exists(), (
    ...
    def test_keep_negative_does_not_delete_freshly_created_backup(self, athena_home):
    ...
    def test_keep_zero_still_prunes_older_backups(self, athena_home):
        ...
        first = create_pre_update_backup(athena_home=athena_home, keep=5)
        _t.sleep(1.05)
        second = create_pre_update_backup(athena_home=athena_home, keep=5)
        _t.sleep(1.05)
        third = create_pre_update_backup(athena_home=athena_home, keep=0)
    ...
    def test_skips_symlinked_files(self, athena_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside ATHENA_HOME."""
        from athena_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def athena_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".athena"
        root.mkdir()
        _make_athena_tree(root)
        # Point ATHENA_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("ATHENA_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for athena_cli.config + athena_constants so they pick up ATHENA_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("athena_cli.config") or mod == "athena_constants":
                del __import__("sys").modules[mod]
        return root

    def test_backup_flag_creates_backup(self, athena_home, capsys):
        """--backup forces the pre-update backup for one run even when config is off."""
        from athena_cli.main import _run_pre_update_backup
        _run_pre_update_backup(Namespace(no_backup=False, backup=True))
        out = capsys.readouterr().out
        assert "Creating pre-update backup" in out
        assert "Saved:" in out
        assert "Restore:" in out
        assert "athena import" in out
        assert "Disable:" in out
        # Actual backup was created
        backups = list((athena_home / "backups").glob("pre-update-*.zip"))
        assert len(backups) == 1

    def test_default_enabled_creates_backup(self, athena_home, capsys):
        """With the new safe default (``pre_update_backup: true``), every
        ``athena update`` creates a backup before any destructive step
        runs — the cost is a few minutes of zip time vs. the alternative
        of silent total data loss of ``~/.athena/`` observed in #48200
    ...
    def test_no_backup_flag_skips(self, athena_home, capsys):
        from athena_cli.main import _run_pre_update_backup
        ...
        # No backup written
        assert not (athena_home / "backups").exists() or not list(
            (athena_home / "backups").glob("pre-update-*.zip")
        )

    def test_config_enabled_creates_backup(self, athena_home, capsys):
        ...
        import yaml
        (athena_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": True},
        }))
        import sys as _sys
        for mod in list(_sys.modules.keys()):
            if mod.startswith("athena_cli.config"):
    ...
    def test_config_disabled_is_silent(self, athena_home, capsys):
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not list((athena_home / "backups").glob("pre-update-*.zip")) \
            if (athena_home / "backups").exists() else True

    def test_cli_flag_overrides_enabled_config(self, athena_home, capsys):
...
# ---------------------------------------------------------------------------
# Pre-migration backup (athena claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``athena claw migrate`` runs before mutating ~/.athena/."""
    ...
    def test_creates_backup_under_backups_dir(self, athena_home):
        from athena_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(athena_home=athena_home)
        assert out is not None
        assert out.exists()
        # Shares the backups/ directory with pre-update backups so `athena
        # import` and the update-backup listing both pick them up.
        assert out.parent == athena_home / "backups"
        assert out.name.startswith("pre-migration-")
        assert out.suffix == ".zip"

    def test_backup_uses_shared_exclusion_rules(self, athena_home):
        """Pre-migration backup reuses the same exclusion rules as
        ``athena backup`` / ``create_pre_update_backup`` — no drift."""
        from athena_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(athena_home=athena_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_restorable_with_athena_import(self, athena_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Athena
        backup — `athena import` should accept it."""
        from athena_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(athena_home=athena_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_recurse_into_prior_backups(self, athena_home):
        from athena_cli.backup import create_pre_migration_backup
        out1 = create_pre_migration_backup(athena_home=athena_home)
        assert out1 is not None
        out2 = create_pre_migration_backup(athena_home=athena_home)
        assert out2 is not None
        with zipfile.ZipFile(out2) as zf:
            names = zf.namelist()
        assert not any(n.startswith("backups/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, athena_home):
        ...
        created = []
        for _ in range(7):
            out = create_pre_migration_backup(athena_home=athena_home, keep=3)
            if out is not None:
                created.append(out)
            _t.sleep(1.05)  # timestamp resolution

        remaining = sorted((athena_home / "backups").glob("pre-migration-*.zip"))
        assert len(remaining) <= 3, f"expected <=3 backups retained, got {len(remaining)}"

    def test_missing_athena_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.athena yet — nothing to back up."""
        from athena_cli.backup import create_pre_migration_backup
        missing = tmp_path / "does-not-exist"
        out = create_pre_migration_backup(athena_home=missing)
        assert out is None

    def test_does_not_touch_pre_update_backups(self, athena_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from athena_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(athena_home=athena_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        import time as _t
        for _ in range(3):
            out = create_pre_migration_backup(athena_home=athena_home, keep=1)
            assert out is not None
            _t.sleep(1.05)
...
class TestRestoreCronJobsIfEmptied:
    """`athena update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, athena_home: Path, label="pre-update"):
        from athena_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, athena_home=athena_home, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from athena_cli.backup import restore_cron_jobs_if_emptied
        athena_home = tmp_path / ".athena"
        jobs_path = athena_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(athena_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, athena_home=athena_home)
        assert result is not None
    ...
    def test_noop_when_snapshot_id_missing(self, tmp_path):
        ...
        jobs_path = athena_home / "cron" / "jobs.json"
        self._seed_jobs(jobs_path, [])
        assert restore_cron_jobs_if_emptied(None, athena_home=athena_home) is None
        assert restore_cron_jobs_if_emptied("", athena_home=athena_home) is None
...
# location, NOT under ATHENA_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------

class TestMemoryProviderExternalPaths:
    def _make_min_tree(self, athena_home: Path) -> None:
        athena_home.mkdir(parents=True, exist_ok=True)
        (athena_home / "config.yaml").write_text("model:\n  provider: openrouter\n")
        (athena_home / ".env").write_text("OPENROUTER_API_KEY=sk-test\n")
        (athena_home / "state.db").write_bytes(b"x")
    ...
    def test_backup_captures_external_paths_under_external_prefix(self, tmp_path, monkeypatch):
        ...
        athena_home = tmp_path / ".athena"
        self._make_min_tree(athena_home)
        # External provider state living OUTSIDE ATHENA_HOME.
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under ATHENA_HOME,
        and credential-shaped files get 0600."""
        dst_home = tmp_path / "dst"
        dst_home.mkdir()
        athena_home = dst_home / ".athena"
        athena_home.mkdir()
        ...
        # External state did NOT leak into ATHENA_HOME.
        assert not (athena_home / "_external").exists()
