...
def _advance_backup_clock(seconds: float = 1.1) -> None:
    """Skew athena_cli.backup's datetime forward instead of sleeping.
    ...
    import athena_cli.backup as _backup

    shim = getattr(_backup.datetime, "_athena_test_shim", None)
    if shim is None:
        class _ShimDatetime(_dt.datetime):
            _athena_test_shim = True
...
def _make_athena_tree(root: Path) -> None:
    ...
    for db_name in ("memory_store.db", "athena_state.db"):
...
class TestShouldExclude:
    ...
    def test_excludes_state_snapshots_dir(self):
        ...
        from athena_cli.backup import _EXCLUDED_DIRS, _QUICK_SNAPSHOTS_DIR, _should_exclude
    ...
    def test_excludes_desktop_emergency_state_db_baks(self):
        ...
        state.db.pre-update-emergency-*.bak files at the ATHENA_HOME root —
...
class TestIterBackupFiles:
    def test_manual_and_automatic_paths_share_one_walk(self, tmp_path):
        ...
        ``athena-agent`` at ANY depth, silently dropping nested skill dirs
        like ``skills/autonomous-ai-agents/athena-agent/`` that the manual
        path preserved. One shared iterator makes that drift impossible;
        this test pins the contract."""
        from athena_cli.backup import _iter_backup_files
        ...
        # athena-agent holding real skill content.
        nested = root / "skills" / "autonomous-ai-agents" / "athena-agent"
        ...
        rel_nested = str(Path("skills/autonomous-ai-agents/athena-agent/SKILL.md"))
        assert rel_nested in selected
        assert str(Path("models/big.gguf")) not in selected
        assert not any(s.startswith("athena-agent") for s in selected)
    ...
    def test_skipped_dirs_collected_for_summary(self, tmp_path):
        ...
        assert "athena-agent" in skipped
...
class TestBackup:
    ...
    def test_state_snapshots_not_nested_into_backup(self, tmp_path, monkeypatch):
        ...
        athena_home.mkdir()
        _make_athena_tree(athena_home)
        with sqlite3.connect(athena_home / "state.db") as conn:
            conn.execute("CREATE TABLE sessions (id TEXT PRIMARY KEY)")
            conn.execute("INSERT INTO sessions VALUES ('s1')")

        from athena_cli.backup import _QUICK_SNAPSHOTS_DIR, create_quick_snapshot, run_backup
        ...
        assert snap_id and (athena_home / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
...
class TestImport:
    ...
    def test_import_survives_service_layer_import_failure(self, tmp_path, monkeypatch, capsys):
        ...
        assert "Done. Your Athena configuration has been restored." in out
        assert "athena gateway install" in out
...
class TestFormatSize:
    def test_bytes(self):
        from athena_cli.sizefmt import format_bytes as _format_size
...
class TestImportAtomicWrites:
    """`athena import` must never leave a user's file truncated.
    ...
    tests pin the invariant for both restore branches: the ATHENA_HOME branch
    ...
    def test_failed_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        ...
        (athena_home / "config.yaml").write_text(original)
        monkeypatch.setenv("ATHENA_HOME", str(athena_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (athena_home / "config.yaml").read_text() == original
        # And the aborted write must not litter the directory it staged in.
        assert list(athena_home.glob(".config.yaml.*")) == []

    def test_failed_external_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        """Same invariant on the `_external/` branch, which writes outside ATHENA_HOME."""
    ...
    def test_restore_preserves_existing_file_mode(self, tmp_path, monkeypatch):
        ...
        athena_home = tmp_path / ".athena"
        athena_home.mkdir()
        target = athena_home / "config.yaml"
    ...
    def test_restore_preserves_existing_file_owner(self, tmp_path, monkeypatch):
        ...
        ``sudo athena import`` onto a user-owned (or Docker/NAS volume-owned)
        ATHENA_HOME would hand every restored file to root. The uid/gid is
        ...
        monkeypatch.setattr(
            "athena_cli.backup._preserve_file_owner",
    ...
    def test_restore_does_not_carry_setuid_onto_archive_content(
        ...
        re-serialize content this process produced; ``athena import`` is the
...
class TestSafeCopyDb:
    ...
    def test_aborts_when_source_remains_busy_past_deadline(
        ...
        from athena_cli import backup as backup_mod
    ...
    def test_is_zeroed_sqlite_file_detects_nul_header(self, tmp_path):
        from athena_cli.backup import is_zeroed_sqlite_file
        p = tmp_path / "state.db"
...
class TestQuickSnapshot:
    ...
    def test_failed_state_db_copy_is_loud(self, athena_home, monkeypatch, capsys):
    ...
    def test_restore_refused_db_is_not_counted(self, athena_home, monkeypatch):
        ...
        manifest = json.loads(
            (backup_mod._quick_snapshot_root(athena_home) / snap_id / "manifest.json").read_text()
        )
    ...
    def test_restore_state_db_live_connection(self, athena_home):
        ...
        live_conn = sqlite3.connect(str(athena_home / "state.db"))
...
# ---------------------------------------------------------------------------
    ...
    def test_oversized_db_suppresses_pruning(self, athena_home, capsys):
        ...
        first_id = create_quick_snapshot(label="complete", athena_home=athena_home)
        assert first_id is not None
        first_dir = athena_home / "state-snapshots" / first_id
        ...
        second_id = create_quick_snapshot(
            label="oversized", athena_home=athena_home, max_file_size=1024, keep=1
        )
        assert second_id is not None
        second_dir = athena_home / "state-snapshots" / second_id
        ...
        all_snaps = list_quick_snapshots(limit=100, athena_home=athena_home)
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_non_default_kanban_board_snapshotted(self, athena_home):
        ...
        board_dir = athena_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(athena_home=athena_home)
        copy = (
            athena_home / "state-snapshots" / snap_id
    ...
    def test_board_db_copied_wal_safely(self, athena_home, monkeypatch):
        ...
        import athena_cli.backup as bk
        from athena_cli.backup import create_quick_snapshot

        board = athena_home / "kanban" / "boards" / "work"
        ...
        copy = athena_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work" / "kanban.db"
...
class TestPreUpdateBackup:
    ...
    def test_pre_update_zip_does_not_nest_the_pre_update_snapshot(self, athena_home):
        """``athena update`` in ``full`` mode takes the quick snapshot *before*
        ...
        snap_id = create_quick_snapshot(label="pre-update", athena_home=athena_home)
        assert snap_id and (athena_home / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
...
class TestRunPreUpdateBackup:
    ...
    def athena_home(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Config reads resolve ATHENA_HOME dynamically and their caches are
    ...
    @staticmethod
    def _set_mode(athena_home, value):
        import yaml
    ...
    @staticmethod
    def _zips(athena_home):
        d = athena_home / "backups"
        return list(d.glob("pre-update-*.zip")) if d.exists() else []

    @staticmethod
    def _snaps(athena_home):
        d = athena_home / "state-snapshots"
        return [p for p in d.iterdir() if p.is_dir()] if d.exists() else []




    def test_config_off_disables_everything_silently(self, athena_home, capsys):
        ...
        self._set_mode(athena_home, "off")
        from athena_cli.update_cmd import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is None
        assert out == ""
        assert not self._snaps(athena_home)
        assert not self._zips(athena_home)



    def test_config_full_mode(self, athena_home, capsys):
        self._set_mode(athena_home, "full")
        from athena_cli.update_cmd import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
        ...
        assert len(self._zips(athena_home)) == 1
...
class TestRestoreConfigModelSettingsIfRewritten:
    ...
    def _seed(self, athena_home: Path) -> Path:
        athena_home.mkdir(parents=True, exist_ok=True)
        cfg = athena_home / "config.yaml"
        cfg.write_text(self.USER_CONFIG, encoding="utf-8")
        return cfg

    def test_restores_rewritten_provider_and_dropped_moa(self, tmp_path):
        import yaml
        from athena_cli.backup import restore_config_model_settings_if_rewritten

        athena_home = tmp_path / ".athena"
        cfg = self._seed(athena_home)
        snap_id = self._make_snapshot(athena_home)
        assert snap_id
        ...
        result = restore_config_model_settings_if_rewritten(
            snap_id, athena_home=athena_home
        )
        assert result is not None
        assert result["restored"] is True
        assert result["snapshot_id"] == snap_id
...
# ---------------------------------------------------------------------------
# run_import: ATHENA_HOME override handling (issue #99839)
# ---------------------------------------------------------------------------


class TestImportHonorsAthenaHomeOverride:
    """`athena import` must restore into the home the command runs under.

    Resolving the target through get_default_athena_root() maps a profile
    ...
    def test_import_targets_named_profile_home(self, tmp_path, monkeypatch):
        """ATHENA_HOME=<root>/profiles/<name> must restore INTO the profile,
        ...
        monkeypatch.setenv("ATHENA_HOME", str(profile))
        from athena_constants import get_athena_home

        assert get_athena_home() == profile
    ...
    def test_import_skips_gateway_install_for_non_default_home(
        ...
        monkeypatch.setenv("ATHENA_HOME", str(sandbox))
        ...
        monkeypatch.setattr(
            backup_mod,
            "_get_platform_default_athena_home",
            lambda: native_default,
        )

        calls = []
        monkeypatch.setattr(
            "athena_cli.gateway.ensure_gateway_service",
            lambda *a, **kw: calls.append(kw),
        )
        monkeypatch.setattr(
            "athena_cli.gateway._is_service_running",
            lambda: False,
        )
    ...
    def test_import_installs_gateway_when_default_home_is_target(
        ...
        native_default = tmp_path / "native-default"
        native_default.mkdir()
        monkeypatch.setenv("ATHENA_HOME", str(native_default))
...
def _write_session_db(path: Path, sessions: int, messages_per_session: int) -> None:
    """Create a minimal Athena-shaped session database at *path*."""
...
class TestImportLiveSessionDatabase:
    """`athena import` must not swap the inode of a database Athena holds open.
...
def test_run_backup_prunes_older_default_named_zips_but_not_others(tmp_path, monkeypatch):
    """Hourly `athena backup` callers accumulated 150+ zips; --keep bounds the default-named
    ...
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    for i in range(4):
        (tmp_path / f"athena-backup-2026-01-0{i + 1}-000000.zip").write_bytes(b"old")
    (tmp_path / "my-archive.zip").write_bytes(b"mine")

    backup_mod.run_backup(Namespace(output=None, keep=2))

    kept = sorted(p.name for p in tmp_path.glob("athena-backup-*.zip"))
    assert len(kept) == 2 and kept[0] == "athena-backup-2026-01-04-000000.zip"
