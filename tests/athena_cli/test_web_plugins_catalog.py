...
``.athena-catalog.json`` sidecar) and the install endpoint has NO kill-list bypass."""
...
from athena_cli import plugin_catalog as pc_cat
...
@pytest.fixture
def client(monkeypatch, tmp_path, _isolate_athena_home):
    try:
...
def _install(name: str, sidecar: dict | None):
    from athena_constants import get_athena_home
    d = get_athena_home() / "plugins" / name
    d.mkdir(parents=True)
    (d / "plugin.yaml").write_text(yaml.safe_dump({"name": name, "version": "1.0", "description": "x"}))
    if sidecar:
        (d / ".athena-catalog.json").write_text(json.dumps(sidecar))
...
def test_install_endpoint_refuses_removed_plugins_with_no_bypass(client, monkeypatch):
    from athena_cli import plugins_cmd
    ...
    for body in ({"identifier": "https://github.com/evil/bad-plugin.git"},
        ...
        assert "removed" in resp.json()["detail"] or "not in the Athena plugin catalog" in resp.json()["detail"]
