...
def git_repo(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("athena_cli.config.get_project_root", lambda: repo_dir)
    return repo_dir


def _stub_git(monkeypatch, *, head=SHA_A, origin="https://github.com/pavel4ai/athena.git"):
    calls = []
...
def test_passive_check_uses_the_api_and_never_fetches(git_repo, monkeypatch):
    ...
    tip.assert_called_once_with("nousresearch/athena-agent", "main")
