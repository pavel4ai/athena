"""Regression test: `athena dashboard --tui` must not hard-crash.

Older Athena desktop app shells (<= 0.15.x) spawn the backend as::

    athena dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
def _run_cli(args, timeout=60):
    """Invoke the real athena_cli.main parser in a subprocess.
