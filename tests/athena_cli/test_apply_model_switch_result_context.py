...
def _run_display(monkeypatch, result):
    ...
    # Avoid writing to ~/.athena/config.yaml during the test.
...
def test_global_switch_clears_context_pin_owned_by_previous_route(monkeypatch):
    ...
    with (
        ...
        patch("athena_cli.config.load_config_readonly", return_value=configured),
    ):
        cli_mod.AthenaCLI._apply_model_switch_result(cli, result, True)
