...
def athena_home(tmp_path, monkeypatch):
    ...
    # /etc/athena on the dev/CI box can't influence the test.
    monkeypatch.setenv("ATHENA_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(athena_home, monkeypatch):
...
def test_env_expansion_in_user_config(athena_home, monkeypatch):
    from athena_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(athena_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_athena_dotenv(athena_home=str(home))
