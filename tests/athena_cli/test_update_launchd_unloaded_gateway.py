"""Regression for #74973 — `athena update` must not leave the gateway down.
...
cron stayed dark until someone manually ran ``athena gateway restart``.
...
def launchd(monkeypatch):
    """Stub athena_cli.gateway so no real launchctl call is made."""
    calls: list[str] = []
    ...
    monkeypatch.setattr(gateway_mod, "get_launchd_label", lambda: "ai.athena.gateway", raising=False)
...
class TestLaunchdRestartAfterUpdate:
    def test_plist_present_always_restarts_without_classifying(self, launchd, capsys):
        ...
        assert update_cmd._restart_launchd_gateway_after_update(supervision_verify=False) == (["ai.athena.gateway"], [])
...
ai.athena.gateway = {
\tactive count = 1
\tpath = /Users/u/Library/LaunchAgents/ai.athena.gateway.plist
\tstate = running
\tpid = 59038
\tprogram = /Users/u/.athena/bin/athena
}
"""
...
class TestServicePidSweepExclusion:
    ...
    def macos_launchd(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_mod, "get_launchd_label", lambda: "ai.athena.gateway")
...
class TestParseLaunchdPidFromPrintOutput:
    def test_running_service(self):
        from athena_cli.gateway import _parse_launchd_pid_from_print_output
