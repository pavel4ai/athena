...
class TestServiceIdentityForForeignHome:
    """A ATHENA_HOME that is neither ``~/.athena`` nor ``~/.athena/profiles/<name>`` must never resolve to
    the default profile's ``athena-gateway`` unit (a temp-home harness uninstalled the production gateway)."""
    ...
    def test_foreign_home_gets_its_own_unit(self, machine_home, tmp_path, monkeypatch):
        ...
        foreign.mkdir()
        monkeypatch.setenv("ATHENA_HOME", str(foreign))

        default_unit = machine_home / ".config" / "systemd" / "user" / "athena-gateway.service"
        assert gateway_cli.get_service_name() != "athena-gateway"
    ...
    def test_default_and_named_profile_homes_keep_their_names(self, machine_home, monkeypatch):
        default_home = machine_home / ".athena"
        (default_home / "profiles" / "alpha").mkdir(parents=True)

        monkeypatch.setenv("ATHENA_HOME", str(default_home))
        assert gateway_cli.get_service_name() == "athena-gateway"

        monkeypatch.setenv("ATHENA_HOME", str(default_home / "profiles" / "alpha"))
        assert gateway_cli.get_service_name() == "athena-gateway-alpha"


class TestUninstallRefusesForeignUnit:
    """systemd_uninstall must not stop/disable/unlink a unit pinned to another ATHENA_HOME."""

    def test_unit_for_other_home_is_left_alone(self, tmp_path, monkeypatch, capsys):
        unit_path = tmp_path / "athena-gateway.service"
        unit_path.write_text('[Service]\nEnvironment="ATHENA_HOME=/somewhere/else"\n', encoding="utf-8")
        monkeypatch.setenv("ATHENA_HOME", str(tmp_path / "mine"))
        monkeypatch.setattr(gateway_cli, "get_systemd_unit_path", lambda system=False: unit_path)
...
class TestGetCronDrainTimeout:
    def test_missing_config_falls_back_to_default(self, monkeypatch):
        monkeypatch.delenv("ATHENA_CRON_DRAIN_TIMEOUT", raising=False)
    ...
    def test_env_overrides_config(self, monkeypatch):
        monkeypatch.setenv("ATHENA_CRON_DRAIN_TIMEOUT", "45")
        monkeypatch.setattr(
            gateway_cli,
            "read_raw_config",
...
class TestGeneratedSystemdUnits:
    ...
    def test_restart_exit_code_is_also_declared_a_success_status(self):
        ...
        and any OnFailure= alert unit fires on every routine restart (athena update,
        athena gateway restart, the in-app restart). SuccessExitStatus=75 keeps the same
    ...
    def test_user_unit_does_not_leak_profile_node_symlink_target(self, tmp_path, monkeypatch):
        ...
        profile_node_bin = tmp_path / ".athena" / "profiles" / "jarvis" / "node" / "bin"
    ...
    def test_launchd_plist_persists_configured_nofile_soft_limit(self, monkeypatch):
        ...
        plist rewrite by `athena gateway start` cannot strip the FD floor and
        reintroduce EMFILE crashes (launchd default soft limit is 256)."""
        import athena_cli.resource_limits as resource_limits
...
class TestLaunchdServiceRecovery:
    ...
    def test_launchd_status_reports_fallback_when_unsupported_and_pid_running(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "get_athena_home", lambda: tmp_path)
...
class TestGatewaySystemServiceRouting:
    ...
    def test_launchd_restart_uses_sigusr1_and_exit_wait_budget(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.athena.gateway")
        ...
        assert ("observe", "ai.athena.gateway", 654, "gui/501") in calls
        out = capsys.readouterr().out
...
class TestSystemUnitAthenaHome:
    ...
    def test_empty_managed_node_dir_uses_only_ambient_fallback(
        ...
        managed_bin = tmp_path / ".athena" / "node" / "bin"
        ...
        gateway_cli._append_node_dir_for_service(entries, tmp_path / ".athena")
    ...
    def test_managed_node_makes_system_unit_independent_of_callers_path(
        ...
        target_athena = target_home / ".athena"
        root_home = tmp_path / "root"
        root_athena = root_home / ".athena"
        managed_bin = target_athena / "node" / "bin"
        managed_bin.mkdir(parents=True)
        node = managed_bin / "node"
        node.write_text("#!/bin/sh\n")
        node.chmod(0o755)
        root_athena.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", staticmethod(lambda: root_home))
        monkeypatch.setenv("ATHENA_HOME", str(root_athena))
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", str(target_home), 1001),
        )
        monkeypatch.setattr(gateway_cli, "get_athena_home", lambda: root_athena)
        ...
        monkeypatch.setattr(
            gateway_cli.shutil,
            "which",
            lambda name: "/home/alice/.local/bin/node"
            if name == "node"
            else None,
        )
    ...
    def test_node_path_lookup_remains_fallback_without_managed_node(
        ...
        monkeypatch.setattr(
            "athena_constants.iter_athena_node_dirs", lambda root=None: []
        )
        monkeypatch.setattr(
            "athena_constants.athena_managed_node_tree_present",
...
class TestSystemUnitRefreshSyncsAthenaHome:
    """sudo system refresh must not flip TimeoutStopSec via /root/.athena."""

    def test_refresh_adopts_unit_athena_home_before_rewriting(self, tmp_path, monkeypatch):
        root_home = tmp_path / "root"
        alice_home = tmp_path / "alice"
        root_athena = root_home / ".athena"
        alice_athena = alice_home / ".athena"
        root_athena.mkdir(parents=True)
        alice_athena.mkdir(parents=True)
        (root_athena / "config.yaml").write_text(
            "agent:\n  restart_drain_timeout: 60\n", encoding="utf-8"
        )
        (alice_athena / "config.yaml").write_text(
        ...
        # Correct installed unit (operator's ATHENA_HOME + drain timeout).
        monkeypatch.setenv("ATHENA_HOME", str(alice_athena))
        ...
        # Simulate sudo without inherited ATHENA_HOME (falls back to root).
        monkeypatch.setenv("ATHENA_HOME", str(root_athena))
        ...
        assert os.environ["ATHENA_HOME"] == str(alice_athena)
    ...
    def test_is_current_syncs_before_reading_unit(self, tmp_path, monkeypatch):
        ...
        unit's pinned ATHENA_HOME *before* it reads/compares the unit.
        ...
        monkeypatch.setattr(gateway_cli, "_sync_athena_home_from_systemd_unit", tracking_sync)
    ...
    def test_start_and_restart_delegate_sync_to_chokepoint(self, monkeypatch):
        ...
        for entry in ("systemd_start", "systemd_restart"):
            ...
            monkeypatch.setattr(
                gateway_cli,
                "_sync_athena_home_from_systemd_unit",
...
class TestLegacyAthenaUnitDetection:
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        for i, execstart in enumerate(variants):
            name = "athena.service" if i == 0 else "athena.service"  # same name
...
class TestLaunchctlBootstrapEioRetry:
    ...
    PLIST = "/tmp/ai.athena.gateway.plist"
    DOMAIN = "gui/501"
    LABEL = "ai.athena.gateway"
...
class TestLaunchdUnloadedJobStderrStaysOffTerminal:
    ...
    def fake_launchctl(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.athena.gateway")
        monkeypatch.setattr(gateway_cli, "_launchd_domain", lambda: "gui/501")
        monkeypatch.setattr(gateway_cli, "get_launchd_plist_path", lambda: tmp_path / "ai.athena.gateway.plist")
        monkeypatch.setattr(gateway_cli, "_clear_launchd_unsupported_marker", lambda: None)
...
class TestTimeoutStopSecCoversCronFloor:
    ...
    def _unit_with_config(self, tmp_path, monkeypatch, config_yaml, env=None):
        athena = tmp_path / "home" / ".athena"
        athena.mkdir(parents=True)
        (athena / "config.yaml").write_text(config_yaml, encoding="utf-8")
        monkeypatch.setenv("ATHENA_HOME", str(athena))
        monkeypatch.delenv("ATHENA_RESTART_DRAIN_TIMEOUT", raising=False)
        monkeypatch.delenv("ATHENA_CRON_DRAIN_TIMEOUT", raising=False)
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
        monkeypatch.setattr(
    ...
    def test_env_override_extends_the_leash(self, tmp_path, monkeypatch):
        unit = self._unit_with_config(
            ...
            monkeypatch,
            "agent:\n  restart_drain_timeout: 0\n",
            env={"ATHENA_CRON_DRAIN_TIMEOUT": "200"},
        )
