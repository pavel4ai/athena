...
from athena_cli.main import cmd_update, PROJECT_ROOT
...
def _patch_managed_uv(request):
    ...
    with patch("athena_cli.managed_uv.resolve_uv", side_effect=_fake_resolve_uv), \
         patch("athena_cli.managed_uv.ensure_uv", side_effect=_fake_ensure_uv), \
         patch("athena_cli.managed_uv.update_managed_uv", side_effect=_fake_update_managed_uv), \
         patch(
             "athena_cli.update_cmd._post_update_sqlite_runtime_status",
...
class TestCmdUpdateNpmLockfileCache:
    ...
    def test_package_json_only_edit_defeats_skip(self, tmp_path, monkeypatch):
        ...
        npm — lockfile unchanged. `athena update` must still install (the
    ...
    def test_update_uses_one_shared_npm_cache_across_profiles(
        ...
        shared_root = tmp_path / ".athena"
        named_profile = shared_root / "profiles" / "work"
        named_profile.mkdir(parents=True)

        monkeypatch.setattr(hm, "PROJECT_ROOT", checkout)
        monkeypatch.setattr(athena_constants.Path, "home", lambda: tmp_path)
        monkeypatch.setattr(
            athena_constants, "find_node_executable", lambda _name: "/usr/bin/npm"
        )
        ...
        with patch.object(
            ...
            monkeypatch.setenv("ATHENA_HOME", str(shared_root))
            update_cmd._update_node_dependencies()

            monkeypatch.setenv("ATHENA_HOME", str(named_profile))
            update_cmd._update_node_dependencies()
...
class TestCmdUpdateTermuxUvBootstrap:
    ...
    def test_termux_reuses_existing_path_uv_without_pip(self, mock_run, monkeypatch):
        ...
        monkeypatch.setattr(hm, "_is_termux_env", lambda env=None: True)
        # Production resolve_uv only checks $ATHENA_HOME/bin/uv; model an empty
        # managed dir so the PATH probe is what surfaces the packaged uv.
        monkeypatch.setattr("athena_cli.managed_uv.resolve_uv", lambda: None)
...
class TestUpdateManagedPythonEnvIsolation:
    ...
    forces UV_PYTHON_INSTALL_DIR to .athena-runtime/python), then re-points
    ...
    def test_update_uv_env_points_venv_and_runtime_store(self):
        ...
        assert ".athena-runtime" in uv_env.get("UV_PYTHON_INSTALL_DIR", "")
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_on_fork_checks_upstream_when_origin_up_to_date(
        ...
        origin but behind pavel4ai/athena silently misses updates.
        """
        from athena_cli import main as hm
    ...
    def test_current_checkout_python_repair_failure_is_durable(
        ...
        with patch.object(
            ...
            "athena_cli.update_receipt.finalize_update_receipt"
        ) as finalize_receipt, patch(
            "athena_cli.update_receipt.finalize_pending_update_receipt"
        ):
    ...
    def test_fork_upstream_sync_that_moves_head_runs_post_update_steps(
        ...
        with patch.object(
            ...
            "athena_cli.update_receipt.collect_fleet_versions",
            return_value=[],
        ), patch(
            ...
            "athena_cli.gateway.find_gateway_pids",
            return_value=[],
        ), patch(
            "athena_cli.gateway.find_profile_gateway_processes",
            return_value=[],
        ), patch(
            "athena_cli.gateway._get_service_pids",
            return_value=set(),
        ), patch.object(
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "athena_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "athena_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch(
            "athena_cli.update_cmd._reload_config_modules"
        ), patch(
            "athena_cli.update_cmd._run_config_check_fresh", return_value=(1, 2)
        ), patch(
            "athena_cli.update_cmd._run_migrate_config_fresh",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ) as migrate_config, patch("athena_cli.main.sys") as mock_sys:
...
class TestCmdUpdateMigrationPrompt:
    ...
    def test_version_bump_only_applies_silently_without_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "athena_cli.config.get_missing_env_vars", return_value=[]
        ), patch(
            "athena_cli.config.get_missing_config_fields", return_value=[]
        ), patch(
            "athena_cli.update_cmd._reload_config_modules"
        ), patch(
            "athena_cli.update_cmd._run_config_check_fresh", return_value=(5, 24)
        ), patch(
            "athena_cli.update_cmd._run_migrate_config_fresh",
    ...
    def test_version_bump_only_surfaces_migration_resets(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "athena_cli.update_cmd._run_config_check_fresh", return_value=(33, 34)
        ), patch(
            "athena_cli.update_cmd._run_migrate_config_fresh",
            return_value={
                "env_added": [],
    ...
    def test_new_options_are_listed_by_name_before_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "athena_cli.config.get_missing_env_vars", return_value=env_items
        ), patch(
            "athena_cli.config.get_missing_config_fields", return_value=cfg_items
        ), patch(
            "athena_cli.update_cmd._reload_config_modules"
        ), patch(
            "athena_cli.update_cmd._run_config_check_fresh", return_value=(1, 24)
        ), patch(
            "athena_cli.update_cmd._run_migrate_config_fresh",
            return_value={"env_added": [], "config_added": [], "warnings": []},
        ), patch("athena_cli.main.sys") as mock_sys:
...
class TestConfigVersionCheckUsesFreshModules:
    ...
    Before the fix, ``athena update`` ran in the PRE-pull Python process.
    After ``git pull`` updated the source on disk, function-level imports
    returned the OLD cached ``athena_cli.config`` module — so
    ...
    def test_run_config_check_fresh_reloads_modules(self):
        ...
        sys.modules held the OLD athena_cli.config with the OLD
        DEFAULT_CONFIG["_config_version"] after git pull.
        """
        from unittest.mock import patch

        import athena_cli.update_cmd as update_cmd
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        default_p = SimpleNamespace(name="default", path=Path("/fake/.athena"))
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.athena/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.athena/profiles/work"))
        ...
        with (
            patch("athena_cli.profiles.list_profiles", return_value=all_profiles),
            patch("athena_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("athena_cli.profiles.list_profiles", return_value=[default_p]),
            patch("athena_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
...
class TestCmdUpdateBranchFlag:
    """``athena update --branch <name>`` targets the requested branch.
...
class TestCmdUpdateCheckBranchFlag:
    """``athena update --check --branch <name>`` honors the branch override.
...
class TestCmdUpdateZipBranchRefusal:
    """``athena update --branch=<non-main>`` must refuse on the ZIP fallback path.
    ...
    def test_zip_fallback_refuses_non_main_branch(self, capsys):
        from athena_cli.update_cmd import _update_via_zip
...
class TestNodeRuntimeNpmResolution:
    ...
    def test_update_rebuilds_desktop_that_disappears_mid_update(self):
        ...
        packaged_exe = desktop_dir / "release" / "win-unpacked" / "Athena.exe"
        ...
        desktop_build.assert_called_once_with(
            [hm.sys.executable, "-m", "athena_cli.main", "desktop", "--build-only"],
            cwd=PROJECT_ROOT,
            env=ANY,
        )
    ...
    def test_git_failure_zip_fallback_rebuilds_missing_desktop(self, tmp_path, monkeypatch):
        ...
        desktop app (release/win-unpacked/Athena.exe) is preserved THROUGH
        ...
        project_root = tmp_path / "athena-agent"
        ...
        def write_source_zip(_url, destination):
            with zipfile.ZipFile(destination, "w") as archive:
                archive.writestr("athena-agent-main/apps/desktop/package.json", "{}")
        ...
        monkeypatch.setattr(update_cmd, "get_athena_home", lambda: tmp_path / "athena-home")
        ...
        with (
            ...
            patch("athena_cli.managed_uv.ensure_uv", return_value="uv"),
            patch("athena_cli.managed_uv.update_managed_uv"),
            patch(
            ...
            patch("athena_cli.model_catalog.seed_cache_from_checkout", return_value=False),
        ):
...
class TestUpdateNodeDependencies:
    ...
    def test_records_lockfile_hash_only_on_success(self, _which, mock_popen, tmp_path, monkeypatch):
        ...
        from athena_cli import update_cmd_deps
...
class TestGitTrampolineSelfHeal:
    ...
    def test_healthy_git_command_unchanged(self):
        ...
        with (
            ...
            patch(
                "athena_cli.update_cmd.subprocess.run",
                side_effect=self._fake_run_healthy,
            ),
            patch("athena_cli.update_cmd._locate_real_git") as locate,
        ):
    ...
    def test_trampoline_swaps_to_real_git(self, capsys):
        ...
        with (
            ...
            patch(
                "athena_cli.update_cmd._locate_real_git", return_value=real
            ),
        ):
            result = update_cmd._ensure_non_trampoline_git(git_cmd)
    ...
    def test_trampoline_no_real_git_keeps_command(self, capsys):
        ...
        with (
            ...
            patch("athena_cli.update_cmd._locate_real_git", return_value=None),
        ):
            result = update_cmd._ensure_non_trampoline_git(git_cmd)
        assert result == git_cmd
        out = capsys.readouterr().out
    ...
    def test_off_windows_noop(self):
        ...
        with (
            ...
            patch("athena_cli.update_cmd.subprocess.run") as run,
        ):
            result = update_cmd._ensure_non_trampoline_git(git_cmd)
        assert result == git_cmd
    ...
    def test_portable_git_candidates_check_shared_root_first(self, tmp_path, monkeypatch):
        # Profile-scoped layout: ATHENA_HOME = <root>/profiles/foo, but the
        # PortableGit tree lives under the SHARED root (monerostar review on
        # #88136). The candidate list must check get_default_athena_root()
        ...
        monkeypatch.setattr(update_cmd, "get_default_athena_root", lambda: root)
        monkeypatch.setattr(update_cmd, "get_athena_home", lambda: profile_home)
