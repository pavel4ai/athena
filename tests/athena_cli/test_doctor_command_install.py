"""Tests for the Command Installation check in athena doctor."""
...
import athena_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal ATHENA_HOME + PROJECT_ROOT for doctor tests."""
    ...
    athena_bin = venv_bin_dir / "athena"
    athena_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    ...
    return home, project, athena_bin
...
class TestDoctorCommandInstallation:
    ...
    def test_correct_symlink_shows_ok(self, monkeypatch, tmp_path):
        home, project, athena_bin = _setup_doctor_env(monkeypatch, tmp_path)
        ...
        cmd_link.symlink_to(athena_bin)
    ...
    def test_missing_symlink_shows_fail(self, monkeypatch, tmp_path):
        ...
        assert "athena doctor --fix" in out
    ...
    def test_fix_creates_missing_symlink(self, monkeypatch, tmp_path):
        ...
        assert cmd_link.resolve() == athena_bin.resolve()
    ...
    def test_wrong_target_symlink_shows_warn(self, monkeypatch, tmp_path):
        ...
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "athena"
        wrong_target = tmp_path / "wrong_athena"
    ...
    def test_dot_venv_dir_is_found(self, monkeypatch, tmp_path):
        ...
        athena_bin = project / ".venv" / "bin" / "athena"
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "athena"
        cmd_link.symlink_to(athena_bin)
        ...
        out = _run_doctor(fix=False)
        assert "Venv entry point exists" in out
        assert ".venv/bin/athena" in out

    @pytest.mark.skipif(sys.platform == "win32", reason="Symlink check is Unix-only")
    def test_non_symlink_regular_file_shows_ok(self, monkeypatch, tmp_path):
        """If ~/.local/bin/athena is a regular file (not symlink), accept it."""
        home, project, athena_bin = _setup_doctor_env(monkeypatch, tmp_path)

        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "athena"
        cmd_link.write_text("#!/bin/sh\nexec python -m athena_cli.main \"$@\"\n")
