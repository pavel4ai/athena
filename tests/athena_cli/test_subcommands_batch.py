...
# ── deprecated `athena login` fails gracefully, not with argparse error ────
#
# `athena login` is a removed command; its handler (`login_command` in
# `athena_cli/auth.py`) prints a deprecation notice pointing at `athena auth` /
# `athena model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `athena --help`.
