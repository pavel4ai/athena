...
- Bare `athena setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `athena setup --quick` runs the narrower "fill in missing items" flow.
- `athena setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("athena_cli.setup.ensure_athena_home", {}),
        ("athena_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("athena_cli.config.is_managed", {"return_value": False}),
        ("athena_cli.setup.load_config", {"return_value": {}}),
        ("athena_cli.setup.save_config", {}),
        ("athena_cli.setup.get_env_value", {"return_value": None}),
        ("athena_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("athena_cli.setup._print_setup_summary", {}),
        ("athena_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("athena_cli.setup.load_config", {"return_value": {}}),
        ("athena_cli.setup.save_config", {}),
        ("athena_cli.auth.get_active_provider", {"return_value": None}),
        ("athena_cli.setup.get_env_value", {"return_value": None}),
        ("athena_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `athena setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="athena_cli.setup.prompt_choice",
                quick="athena_cli.setup._run_quick_setup",
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "athena_cli.setup.run_setup_wizard",
