...
def test_gui_installs_packages_and_launches_desktop_app(tmp_path, monkeypatch):
    ...
    with patch("athena_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("athena_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("athena_cli.main._desktop_build_needed", return_value=True), \
         patch("athena_cli.main._write_desktop_build_stamp"), \
         patch("athena_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("athena_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
...
def test_gui_forwards_desktop_environment_overrides(tmp_path, monkeypatch):
    root = _make_desktop_tree(tmp_path)
    athena_root = tmp_path / "custom-athena"
    ...
    with patch("athena_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("athena_cli.main._run_npm_install_deterministic", return_value=ok), \
         patch("athena_cli.main._desktop_build_needed", return_value=True), \
         patch("athena_cli.main._write_desktop_build_stamp"), \
         patch("athena_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("athena_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
    ...
    assert launch_env["ATHENA_DESKTOP_BOOT_FAKE"] == "1"
    assert launch_env["ATHENA_DESKTOP_IGNORE_EXISTING"] == "1"
    assert launch_env["ATHENA_DESKTOP_ATHENA_ROOT"] == str(athena_root)
    assert launch_env["ATHENA_DESKTOP_CWD"] == str(cwd)
...
def test_gui_exits_when_npm_missing(tmp_path, monkeypatch, capsys):
    ...
    with patch("athena_cli.main.shutil.which", return_value=None), \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_linux_configures_sandbox_before_launch(tmp_path, monkeypatch):
    ...
    with patch("athena_cli.main.shutil.which", return_value="/usr/bin/sudo"), \
         patch("athena_cli.main.subprocess.run", return_value=ok) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_linux_rejects_symlink_sandbox(tmp_path, monkeypatch):
    ...
    with patch("athena_cli.main.shutil.which", return_value="/usr/bin/sudo"), \
         patch("athena_cli.main.subprocess.run") as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_source_mode_uses_renderer_build_and_electron(tmp_path, monkeypatch):
    ...
    with patch("athena_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("athena_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("athena_cli.main._desktop_build_needed", return_value=True), \
         patch("athena_cli.main._write_desktop_build_stamp"), \
         patch("athena_cli.main.subprocess.run", side_effect=[build_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
...
def test_desktop_build_stamp_skips_build_when_up_to_date(tmp_path, monkeypatch):
    ...
    with patch("athena_cli.main._desktop_build_needed", return_value=False), \
         patch("athena_cli.main._run_npm_install_deterministic") as mock_install, \
         patch("athena_cli.main.subprocess.run", return_value=launch_ok) as mock_run, \
         patch("athena_cli.main._desktop_macos_relaunchable_fixup"), \
         pytest.raises(SystemExit) as exc:
...
def test_desktop_force_build_overrides_stamp(tmp_path, monkeypatch):
    ...
    with patch("athena_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("athena_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("athena_cli.main._desktop_build_needed", return_value=False), \
         patch("athena_cli.main._write_desktop_build_stamp") as mock_stamp, \
         patch("athena_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("athena_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
...
def test_gui_retries_pack_once_after_purging_build_cache(tmp_path, monkeypatch):
    ...
    with patch("athena_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("athena_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("athena_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("athena_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("athena_cli.main._write_desktop_build_stamp"), \
         patch("athena_cli.main._purge_electron_build_cache", return_value=[Path("/c/electron.zip")]) as mock_purge, \
         patch("athena_cli.main._electron_dist_ok", return_value=False), \
         patch("athena_cli.main._redownload_electron_dist", return_value=True), \
         patch("athena_cli.main.subprocess.run", side_effect=[pack_fail, pack_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_redownloads_electron_via_mirror_then_repacks(tmp_path, monkeypatch, capsys):
    ...
    with patch("athena_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("athena_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("athena_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("athena_cli.main._purge_electron_build_cache", return_value=[]), \
         patch("athena_cli.main._electron_dist_ok", return_value=False), \
         patch("athena_cli.main._redownload_electron_dist", side_effect=[False, True]) as mock_dl, \
         patch("athena_cli.main.subprocess.run", side_effect=[pack_fail, pack_fail]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_retries_pack_under_mirror_even_when_prefetch_blocked(tmp_path, monkeypatch, capsys):
    ...
    with patch("athena_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("athena_cli.main._purge_electron_build_cache", return_value=[]), \
         patch("athena_cli.main._electron_dist_ok", return_value=False), \
         patch("athena_cli.main._redownload_electron_dist", return_value=False), \
         patch("athena_cli.main.subprocess.run", side_effect=[pack_fail, pack_fail]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_install_failure_self_heals_electron_and_continues(tmp_path, monkeypatch, capsys):
    ...
    with patch("athena_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("athena_cli.main._run_npm_install_deterministic", return_value=install_fail), \
         patch("athena_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("athena_cli.main._write_desktop_build_stamp"), \
         patch("athena_cli.main._electron_dist_ok", return_value=False), \
         patch("athena_cli.main._try_redownload_electron_dist", return_value=True) as mock_dl, \
         patch("athena_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_install_failure_hard_fails_when_electron_dist_exists(tmp_path, monkeypatch, capsys):
    ...
    with patch("athena_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("athena_cli.main._run_npm_install_deterministic", return_value=install_fail), \
         patch("athena_cli.main._electron_dist_ok", return_value=True), \
         patch("athena_cli.main.subprocess.run") as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_does_not_override_user_electron_mirror(tmp_path, monkeypatch, capsys):
    ...
    with patch("athena_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("athena_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("athena_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("athena_cli.main._purge_electron_build_cache", return_value=[]) as mock_purge, \
...
def test_redownload_electron_dist_missing_installer(tmp_path, monkeypatch):
    ...
    with patch("athena_cli.main.shutil.which", return_value="/usr/bin/node"), \
         patch("athena_cli.main.subprocess.run") as mock_run:
...
def test_redownload_electron_dist_runs_installer_with_mirror(tmp_path, monkeypatch):
    ...
    with patch("athena_cli.main.shutil.which", return_value="/usr/bin/node"), \
         patch("athena_cli.main.subprocess.run", side_effect=fake_run):
...
def test_redownload_electron_dist_returns_false_when_download_fails(tmp_path, monkeypatch):
    ...
    with patch("athena_cli.main.shutil.which", return_value="/usr/bin/node"), \
         patch("athena_cli.main.subprocess.run",
