...
def test_tui_finds_bundled_entry_js(tmp_path):
    ...
    tui_dist = tmp_path / "athena_cli" / "tui_dist"
    tui_dist.mkdir(parents=True)
    entry = tui_dist / "entry.js"
    entry.write_text("// bundled TUI", encoding="utf-8")

    from athena_cli.main_tui_launch import _find_bundled_tui
    result = _find_bundled_tui(athena_cli_dir=tmp_path / "athena_cli")
