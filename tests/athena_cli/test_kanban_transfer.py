"""Tests for kanban board export / import (``athena_cli.kanban_transfer``).
...
from athena_cli import kanban_transfer as kt
from athena_cli.archive_safe import normalize_archive_parts, safe_extract_targz
...
def kanban_root(tmp_path, monkeypatch):
    ...
    def _use(name: str) -> Path:
        ...
        monkeypatch.setenv("ATHENA_KANBAN_HOME", str(root))
        for var in ("ATHENA_KANBAN_DB", "ATHENA_KANBAN_WORKSPACES_ROOT",
                    "ATHENA_KANBAN_ATTACHMENTS_ROOT", "ATHENA_KANBAN_BOARD"):
...
def test_import_rejects_a_future_format_version(kanban_root, tmp_path):
    ...
    kanban_root("target")
    with pytest.raises(ValueError, match="newer than this Athena"):
