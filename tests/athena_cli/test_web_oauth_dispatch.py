...
Athena must not mint subscription OAuth tokens from an unattended HTTP
...
import athena_cli.web_routers.oauth as _rt_oauth
import athena_cli.web_server_oauth as _web_server_oauth
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        ...
        "athena_cli.web_server_oauth._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_codex_dashboard_start_rewords_device_authorization_error(monkeypatch):
    ...
    try:
        ...
        assert "athena auth" not in detail
    finally:
...
def test_anthropic_dashboard_oauth_is_removed_and_external():
    ...
    assert providers["anthropic"]["cli_command"] == "athena auth add anthropic"
...
def test_xai_dashboard_poller_seeds_single_entry_and_clears_suppression(tmp_path, monkeypatch):
    ...
    ``device_code`` suppression left by a prior ``athena auth remove
    xai-oauth``.
    """
    from athena_cli import auth as auth_mod
    from athena_cli import web_server as ws
    ...
    # Prior `athena auth remove xai-oauth` left the source suppressed.
