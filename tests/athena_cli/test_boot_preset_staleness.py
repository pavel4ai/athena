...
def test_presets_stale_when_a_staged_model_has_no_section(athena_home):
    from athena_cli.local_runtime.bootstrap import _presets_stale

    _stage(athena_home, "model-a")
    _stage(athena_home, "model-b")
    _write_presets(athena_home, "model-a")
    assert _presets_stale() is True


def test_presets_current_when_every_staged_model_is_covered(athena_home):
    from athena_cli.local_runtime.bootstrap import _presets_stale
...
def test_legacy_presets_without_model_paths_are_regenerated(athena_home):
    from athena_cli.local_runtime.bootstrap import _presets_stale

    _stage(athena_home, "model-a")
    _write_presets(athena_home, "model-a")
    ini = athena_home / "runtimes/llamacpp/presets.ini"
    ini.write_text("[model-a]\nctx-size = 65536\n")
    assert _presets_stale()


def test_no_models_is_never_stale(athena_home):
    from athena_cli.local_runtime.bootstrap import _presets_stale
...
def test_boot_replaces_incumbent_with_stale_presets(athena_home, monkeypatch):
    ...
    import athena_cli.local_runtime.bootstrap as boot
    ...
    stopped = {}
    monkeypatch.setattr(
        "athena_cli.local_runtime.endpoint._state_endpoint",
    ...
    monkeypatch.setattr(
        "athena_cli.local_runtime.binaries.ensure_runtime_installed", fake_boot)
...
def test_refresh_bounces_an_adopted_server(athena_home, monkeypatch):
...
def test_refresh_no_server_anywhere_is_a_noop(athena_home, monkeypatch):
    import athena_cli.local_runtime.bootstrap as boot

    monkeypatch.setattr(boot, "_SUPERVISOR", None)
    monkeypatch.setattr(
        "athena_cli.local_runtime.endpoint._state_endpoint", lambda: None)
