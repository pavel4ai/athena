...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (ATHENA_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via ATHENA_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in ATHENA_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from athena_cli.providers import ATHENA_OVERLAYS
        assert "bedrock" in ATHENA_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from athena_cli.providers import ATHENA_OVERLAYS
        assert ATHENA_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from athena_cli.providers import ATHENA_OVERLAYS
        assert ATHENA_OVERLAYS["bedrock"].auth_type == "aws_sdk"
