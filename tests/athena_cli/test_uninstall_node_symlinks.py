"""Tests for athena_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $ATHENA_HOME/node and prepends ~/.local/bin to
...
def _make_athena_node(athena_home: Path) -> Path:
    """Create a fake $ATHENA_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = athena_home / "node" / "bin"
...
def test_removes_symlinks_pointing_into_athena_node(fake_home):
    athena_home = fake_home / ".athena"
    node_bin = _make_athena_node(athena_home)
    local_bin = fake_home / ".local" / "bin"

    for name in ("node", "npm", "npx"):
        (local_bin / name).symlink_to(node_bin / name)

    removed = uninstall.remove_node_symlinks(athena_home)
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(athena_home) == []


def test_removes_dangling_symlink_into_athena_node(fake_home):
