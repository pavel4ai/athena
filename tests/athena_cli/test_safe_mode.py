"""Tests for `athena chat --safe-mode` — pristine troubleshooting runs.
...
* skips plugin discovery entirely (``athena_cli.plugins``)
...
_VARS = ("ATHENA_SAFE_MODE", "ATHENA_IGNORE_USER_CONFIG", "ATHENA_IGNORE_RULES")
...
class TestSafeModeEnvWiring:
    ...
    def test_safe_mode_sets_all_gates(self):
        # Mirrors the cmd_chat logic in athena_cli/main.py.
        class Args:
            safe_mode = True
        ...
        assert os.environ.get("ATHENA_SAFE_MODE") == "1"
...
class TestSafeModePluginDiscovery:
    """Plugin discovery must be a no-op under ATHENA_SAFE_MODE=1."""
    ...
    def test_discovery_runs_without_safe_mode(self, monkeypatch):
        monkeypatch.delenv("ATHENA_SAFE_MODE", raising=False)
        from athena_cli.plugins import PluginManager
...
class TestSafeModeMCP:
    """_load_mcp_config must return no servers under ATHENA_SAFE_MODE=1."""
...
class TestSafeModeParser:
    """--safe-mode must parse on both the root parser and `athena chat`."""
