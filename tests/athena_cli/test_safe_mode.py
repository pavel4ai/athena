"""Tests for `athena chat --safe-mode` isolation."""
...
def test_cmd_chat_safe_mode_sets_env_before_startup(monkeypatch):
    ...
    def fake_has_provider() -> bool:
        assert os.environ["ATHENA_SAFE_MODE"] == "1"
        assert os.environ["ATHENA_IGNORE_USER_CONFIG"] == "1"
        assert os.environ["ATHENA_IGNORE_RULES"] == "1"
        return True
