...
class TestMissingLogNote:
    ...
    `athena debug share` runs on the backend, so a desktop connected to a
    ...
    def test_backend_written_log_reports_plain_absence(self, athena_home):
        from athena_cli.debug import _capture_log_snapshot

        (athena_home / "logs" / "agent.log").unlink()

        snap = _capture_log_snapshot("agent", tail_lines=10)
        assert snap.full_text is None
        assert snap.tail_text == "(file not found)"

    def test_client_written_log_names_its_writer_and_path(self, athena_home):
        from athena_cli.debug import _capture_log_snapshot
        ...
        assert "Athena Desktop" in snap.tail_text
        # The reader needs the path to collect by hand on the client machine.
        assert str(athena_home / "logs" / "desktop.log") in snap.tail_text

    def test_present_client_log_is_captured_normally(self, athena_home):
    ...
    def test_empty_client_log_is_empty_not_absent(self, athena_home):
        """An empty file means the app ran and logged nothing — a different fact."""
        from athena_cli.debug import _capture_log_snapshot

        (athena_home / "logs" / "desktop.log").write_text("")

        snap = _capture_log_snapshot("desktop", tail_lines=10)
        assert snap.tail_text == "(file empty)"

    def test_report_carries_the_note_for_a_remote_backend(self, athena_home):
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_output_warns_on_dpaste_fallback(self, athena_home, capsys):
        """With dpaste.com URLs the output must not promise 6-hour auto-delete
        or a working `athena debug delete` (#106164)."""
        from athena_cli.debug import run_debug_share
...
class TestCollectShareBundle:
    ...
    def test_no_redact_omits_banner(self, athena_home):
        from athena_cli.debug import collect_share_bundle
    ...
    def test_redaction_keeps_secrets_out(self, athena_home):
        from athena_cli.debug import collect_share_bundle
...
class TestBuildNousBundle:
    def test_envelope_shape_and_gzip(self, athena_home):
        import gzip
        import json as _json

        from athena_cli.debug import build_nous_bundle
        ...
        assert envelope["format"] == "athena-debug-share/1"
...
class TestRunDebugShareNous:
    ...
    def test_nous_success_prints_view_url(self, athena_home, capsys):
        from athena_cli.debug import run_debug_share
        ...
        with patch("athena_cli.dump.run_dump"), patch(
            "athena_cli.diagnostics_upload.share_to_nous", return_value=res
        ) as share:
    ...
    def test_nous_failure_suggests_local(self, athena_home, capsys):
        from athena_cli.debug import run_debug_share

        with patch("athena_cli.dump.run_dump"), patch(
            "athena_cli.diagnostics_upload.share_to_nous",
    ...
    def test_nous_does_not_touch_pastebin(self, athena_home):
        from athena_cli.debug import run_debug_share

        res = {"id": "id-1", "viewUrl": "https://v"}
        with patch("athena_cli.dump.run_dump"), patch(
            "athena_cli.diagnostics_upload.share_to_nous", return_value=res
        ), patch("athena_cli.debug.upload_to_pastebin") as paste:
            run_debug_share(self._args())
...
class TestDebugSlashCommand:
    ...
    ``AthenaCLI.process_command`` → ``_handle_debug_command(cmd_original)``,
    ...
    def _captured(self, cmd_original):
        ...
        with patch("athena_cli.debug.run_debug_share", _fake_run):
...
class TestShareConsentGate:
    """`athena debug share` requires explicit consent before uploading.
    ...
    def test_non_interactive_requires_yes(self, athena_home, capsys, monkeypatch):
        ...
        with patch("athena_cli.dump.run_dump"), \
             patch("athena_cli.debug.upload_to_pastebin") as mock_upload:
            with pytest.raises(SystemExit) as exc:
                run_debug_share(self._args())
    ...
    def test_local_never_prompts(self, athena_home, capsys, monkeypatch):
