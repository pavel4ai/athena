...
Before the fix, ``athena whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``athena gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def _env_value(athena_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from athena_cli.main_platform_setup import cmd_whatsapp
    ...
    monkeypatch.setattr("athena_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``athena whatsapp`` with an existing paired session and
