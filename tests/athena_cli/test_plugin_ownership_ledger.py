...
def _write_plugin(athena_home: Path) -> None:
    plugin_dir = athena_home / "plugins" / "ledger_probe"
...
def _write_profile_probe(athena_home: Path, marker: str) -> None:
    plugin_dir = athena_home / "plugins" / "profile_probe"
    plugin_dir.mkdir(parents=True)
    (plugin_dir / "plugin.yaml").write_text(
        yaml.safe_dump(
            {
...
def test_scoped_plugin_cannot_deregister_a_process_global_tool():
    ...
    module_name = "athena_plugins.scoped_cleanup"
...
def test_shared_entrypoint_module_uses_the_active_profile_scope(tmp_path):
    ...
    registry = ToolRegistry()
    module_name = "third_party.shared_athena_plugin"
...
def test_entrypoint_policy_uses_the_most_specific_module_prefix(tmp_path):
    ...
    token = set_athena_home_override(narrow_scope)
    try:
        registry.register(
...
def test_direct_plugin_platform_registration_infers_immutable_scope(tmp_path):
    ...
    module_name = "company.athena.direct_platform_probe"
...
def test_same_name_tool_and_platform_are_isolated_by_athena_home(
    tmp_path,
    monkeypatch,
):
    ...
    token_a = set_athena_home_override(home_a)
    try:
    ...
    token_b = set_athena_home_override(home_b)
    try:
...
def test_manager_discovery_uses_its_home_not_the_ambient_profile(
    ...
    ambient = set_athena_home_override(home_b)
    try:
        manager_a.discover_and_load()
    finally:
        reset_athena_home_override(ambient)
