...
def _fresh_memo(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("ATHENA_PORTAL_BASE_URL", raising=False)
...
def test_memo_expires_after_ttl(monkeypatch, tmp_path):
    ...
    cache_key = auth.athena_home_key()
...
def test_memo_does_not_leak_across_multiplex_profile_contexts(tmp_path):
    ...
    athena_constants.set_athena_home_override (gateway/run.py,
    tui_gateway/server.py), not the ATHENA_HOME env var — the memo must key
    ...
    reset_token = athena_constants.set_athena_home_override(str(profile_a))
    try:
        token_a = auth.resolve_nous_access_token()
    finally:
        athena_constants.reset_athena_home_override(reset_token)
    ...
    reset_token = athena_constants.set_athena_home_override(str(profile_b))
    try:
