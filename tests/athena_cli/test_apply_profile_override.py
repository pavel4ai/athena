...
class TestApplyProfileOverrideAthenaHomeGuard:
    ...
    def test_athena_home_already_profile_dir_is_trusted(self, tmp_path, monkeypatch):
        """ATHENA_HOME=.../profiles/coder must not be overridden even when
        active_profile says something different.

        Preserves the child-process inheritance contract: a subprocess spawned
        with ATHENA_HOME already set to a specific profile must stay in that
        profile.
        """
        athena_root = tmp_path / ".athena"
        profile_dir = athena_root / "profiles" / "coder"
        profile_dir.mkdir(parents=True, exist_ok=True)

        (athena_root / "active_profile").write_text("other")
        ...
        assert os.environ.get("ATHENA_HOME") == str(profile_dir), (
            "ATHENA_HOME must remain unchanged when already pointing to a profile dir"
        )

    def test_athena_home_unset_reads_active_profile(self, tmp_path, monkeypatch):
        """Classic case: ATHENA_HOME unset + active_profile=coder must set
        ATHENA_HOME to the profile directory (existing behaviour must not regress).
        """
        result = _run_apply_profile_override(
            tmp_path,
            monkeypatch,
    ...
    def test_athena_home_unset_default_profile_no_redirect(self, tmp_path, monkeypatch):
        """active_profile=default must not redirect ATHENA_HOME."""
        athena_root = tmp_path / ".athena"
        athena_root.mkdir(parents=True, exist_ok=True)
        ...
        monkeypatch.delenv("ATHENA_HOME", raising=False)
        monkeypatch.setattr(sys, "argv", ["athena", "gateway", "start"])
        (athena_root / "active_profile").write_text("default")

        from athena_cli.main import _apply_profile_override
        _apply_profile_override()

        assert os.environ.get("ATHENA_HOME") is None
    ...
    def test_subcommand_profile_flag_is_not_consumed(self, tmp_path, monkeypatch):
        ...
        When that argv is passed through `athena mcp add --args`, the early
...
class TestSupervisedChildIgnoresStickyProfile:
    ...
    def test_supervised_child_does_not_follow_active_profile(
        ...
        """ATHENA_S6_SUPERVISED_CHILD + active_profile=briefer must NOT redirect.

        Reproduces the Docker/profile scoping bug: the supervised default
        gateway is launched as bare ``athena gateway run`` with
        ATHENA_HOME=/opt/data (the container root, whose parent is NOT
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Container root ATHENA_HOME: parent dir is NOT "profiles", so the
        # #22502 guard does not short-circuit — step 2 (active_profile) runs.
        monkeypatch.setenv("ATHENA_HOME", str(athena_root))
        monkeypatch.setenv("ATHENA_S6_SUPERVISED_CHILD", "1")
        monkeypatch.setattr(sys, "argv", ["athena", "gateway", "run"])

        from athena_cli.main import _apply_profile_override
        _apply_profile_override()

        assert os.environ.get("ATHENA_HOME") == str(athena_root), (
            "Supervised default gateway must stay on the root profile, not be "
            f"hijacked by active_profile; got {os.environ.get('ATHENA_HOME')!r}"
        )
