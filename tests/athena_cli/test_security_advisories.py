"""Tests for athena_cli.security_advisories.
...
import athena_cli.security_advisories as adv
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect ATHENA_HOME so banner cache and config writes are sandboxed."""
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full athena_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "athena_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
...
class TestRendering:
    def test_short_banner_lines_includes_id_and_version(self, fake_advisory):
        ...
        assert "athena doctor" in joined
