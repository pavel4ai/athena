"""Tests for athena_cli.copilot_auth — Copilot token validation and resolution."""
...
class TestResolveToken:
    ...
    def test_gh_cli_classic_pat_raises(self, monkeypatch):
        ...
        with patch("athena_cli.copilot_auth._try_gh_cli_token", return_value="ghp_classic"):
    ...
    def test_invalid_env_var_skips_gh_cli_fallback(self, monkeypatch):
        ...
        with patch("athena_cli.copilot_auth._try_gh_cli_token") as mock_cli:
            token, source = resolve_copilot_token()
        assert token == ""
        assert source == ""
...
class TestGhCliTokenCache:
    ...
    def test_miss_is_cached_and_probe_runs_once(self):
        from athena_cli import copilot_auth
        self._reset()
