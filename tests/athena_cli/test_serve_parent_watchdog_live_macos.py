...
calls ``os._exit(0)`` ~5ms after ``ATHENA_BACKEND_READY``, which no in-process
...
def test_live_backend_survives_timezone_drifted_parent_marker(tmp_path):
    ...
    try:
        ...
        env.update(
            ...
            PYTHONUNBUFFERED="1",
            ATHENA_PARENT_PID=str(parent.pid),
            ATHENA_PARENT_START_MARKER=drifted_marker,
            ATHENA_PARENT_NONCE="nonce-95693",
            ATHENA_SERVE_WATCHDOG_POLL_S="0.5",
        )
        env.pop("ATHENA_DESKTOP", None)
        code = (
        ...
        assert _read_until(serve, "ATHENA_BACKEND_READY"), "backend never announced READY"
