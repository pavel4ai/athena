"""Regression tests: `athena dashboard` validates ATHENA_WEB_DIST before serving.

A custom ATHENA_WEB_DIST without --skip-build previously skipped BOTH the
...
@pytest.fixture()
def main_mod():
    import athena_cli.main as main
    return main
...
def test_env_dist_without_index_exits(main_mod, monkeypatch, tmp_path, capsys):
    """ATHENA_WEB_DIST pointing at a dist with no index.html must exit 1,
    not start a server that 404s."""
    _wire_common(main_mod, monkeypatch)
    empty_dist = tmp_path / "empty_dist"
    empty_dist.mkdir()
    monkeypatch.setenv("ATHENA_WEB_DIST", str(empty_dist))
    ...
    builds = []
    monkeypatch.setattr(
        "athena_cli.main_web_build._build_web_ui", lambda *a, **k: builds.append(a) or True
    )
    ...
    assert "ATHENA_WEB_DIST" in out and str(empty_dist) in out
...
def test_skip_build_missing_dist_attempts_one_recovery_build(
    ...
    dist = project_root / "athena_cli" / "web_dist"
    ...
    monkeypatch.setattr("athena_cli.main_web_build._build_web_ui", fake_build)
