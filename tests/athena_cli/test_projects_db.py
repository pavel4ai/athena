"""Tests for the per-profile Projects store (athena_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Athena Agent", folders=["/tmp/athena"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "athena-agent"
    assert proj.name == "Athena Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/athena"
    assert [f.path for f in proj.folders] == ["/tmp/athena"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "athena-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' ATHENA_HOME.
