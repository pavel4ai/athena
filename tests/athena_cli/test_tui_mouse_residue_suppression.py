...
The Python launcher (`athena --tui …`) has a ~100–300ms cold-start window
...
from athena_cli.main import _suppress_mouse_residue_early
...
class TestEarlyMouseDisable:
    def test_writes_disable_sequence_when_tui_flag_in_argv(self, monkeypatch):
        ...
        monkeypatch.delenv("ATHENA_TUI_NO_EARLY_DISABLE", raising=False)
    ...
    def test_writes_disable_sequence_when_athena_tui_env_set(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["athena"])
        monkeypatch.setenv("ATHENA_TUI", "1")
        monkeypatch.delenv("ATHENA_TUI_NO_EARLY_DISABLE", raising=False)
    ...
    def test_respects_diagnostic_escape_hatch(self, monkeypatch):
        ...
        monkeypatch.delenv("ATHENA_TUI", raising=False)
        monkeypatch.setenv("ATHENA_TUI_NO_EARLY_DISABLE", "1")
    ...
    def test_skips_when_stdout_is_not_a_tty(self, monkeypatch):
        # `athena --tui … >log` or CI capture: pipe is fd 1, not a TTY. The
