...
against a temp ``ATHENA_HOME`` with the network catalog fetch stubbed.
"""
...
from athena_cli.auth import _save_auth_store
...
def guest_home(monkeypatch, tmp_path):
    ...
    from athena_cli import model_switch_providers as msp
    ...
    return Path(os.environ["ATHENA_HOME"])
...
def _cli_nous_rows(config):
    from athena_cli.main_provider_setup import _build_provider_picker_rows
