"""Tests for athena_cli configuration management."""
    ...
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            if sys.platform == "win32":
                # Windows default is %LOCALAPPDATA%\athena — see
                # athena_constants._get_platform_default_athena_home.
                ...
                assert home == base / "athena"
            else:
                assert home == Path.home() / ".athena"
...
class TestEnsureAthenaHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from athena_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
    ...
    _PRE_REWRITE_DEFAULT_SOUL = (
        "You are Athena Agent, an intelligent AI assistant created by Nous "
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``athena logs`` surfaces it)
...
class TestSaveAndLoadRoundtrip:
    ...
    def test_atomic_config_write_refuses_unparseable_existing_config(self, tmp_path):
        ...
        from athena_cli.config import atomic_config_write
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_quotes_values_containing_hash(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ATHENA_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ATHENA_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ATHENA_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ATHENA_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestSanitizeEnvLines:
    ...
    def test_migrate_reports_normalized_line_formatting(self, capsys):
        ...
        with (
            patch("athena_cli.config.sanitize_env_file", return_value=2),
            patch(
                "athena_cli.config.check_config_version",
                return_value=(latest_version, latest_version),
            ),
            patch("athena_cli.config.read_raw_config", return_value={}),
            patch("athena_cli.config.get_missing_env_vars", return_value=[]),
            patch("athena_cli.config.get_missing_config_fields", return_value=[]),
            patch("athena_cli.config.get_missing_skill_config_vars", return_value=[]),
        ):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from athena_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """ATHENA_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `athena setup`) lets a
        ...
        via config.yaml; ATHENA_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from athena_cli.config import OPTIONAL_ENV_VARS
        assert "ATHENA_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `athena setup` surface it (previously only Honcho was listed, leaving
...
class TestConfigSupportFloor:
    ...
    def test_v11_config_is_refused_and_untouched(self, tmp_path, capsys):
        ...
        assert "run `athena setup` to regenerate" in captured.out
    ...
    def test_floor_message_uses_display_athena_home(self):
        from athena_cli.config_migrations import support_floor_message
        from athena_constants import display_athena_home

        msg = support_floor_message()
        assert f"{display_athena_home()}/config.yaml" in msg
...
class TestCustomProviderCompatibility:
    ...
    def _run_ladder(current_ver: int):
        from athena_cli.config_migrations import run_migrations
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or selected Athena runtime/security controls.
    ...
    def _athena_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "ATHENA_SPOTIFY_CLIENT_ID",
            "ATHENA_QWEN_BASE_URL",
            "ATHENA_MAX_ITERATIONS",
        ],
    )
    def test_athena_integration_keys_still_writable(self, allowed_key):
        """``ATHENA_*`` overall is NOT blocked.
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "ATHENA_COPILOT_ACP_COMMAND",
            "ATHENA_COPILOT_ACP_ARGS",
            "ATHENA_YOLO_MODE",
            "ATHENA_ACCEPT_HOOKS",
            ...
            "ATHENA_CRON_SESSION",
            "ATHENA_SINGLE_QUERY_SESSION",
            "ATHENA_SESSION_KEY",
            "ATHENA_SESSION_PLATFORM",
        ],
    )
    def test_athena_security_control_keys_are_not_writable(self, protected_key):
    ...
    def test_preexisting_optional_mcps_override_still_loads(self, tmp_path):
        ...
        from athena_cli.config import invalidate_env_cache

        catalog = tmp_path / "custom-mcp-catalog"
        (tmp_path / ".env").write_text(
            f"ATHENA_OPTIONAL_MCPS={catalog}\n",
            encoding="utf-8",
        )
        invalidate_env_cache()

        assert load_env()["ATHENA_OPTIONAL_MCPS"] == str(catalog)

    @pytest.mark.parametrize(
        ("key", "expected"),
        [
            ("Path", "PATH"),
            ("Athena_Yolo_Mode", "ATHENA_YOLO_MODE"),
            ("Athena_Optional_Mcps", "ATHENA_OPTIONAL_MCPS"),
            ("Athena_Copilot_Acp_Command", "ATHENA_COPILOT_ACP_COMMAND"),
            ("Athena_Copilot_Acp_Args", "ATHENA_COPILOT_ACP_ARGS"),
        ],
    )
    def test_windows_policy_names_are_case_insensitive(self, key, expected):
        from athena_cli.config import _env_var_policy_name
    ...
    def test_windows_env_assignment_matching_is_case_insensitive(self, prefix):
        from athena_cli.config import _env_line_defines_key
...
"""
    ...
    def test_persist_migration_writes_full_read_raw_config(self, tmp_path):
        from athena_cli.config import _persist_migration, read_raw_config
    ...
    def test_preserves_existing_codex_app_server_auto_value(self, tmp_path):
        with patch.dict(os.environ, {"ATHENA_HOME": str(tmp_path)}):
            self._write(
                ...
                "_config_version: 31\n"
                "compression:\n"
                "  codex_app_server_auto: athena\n",
            )

            migrate_config(interactive=False, quiet=True)

            raw = yaml.safe_load((tmp_path / "config.yaml").read_text())
            assert raw["compression"]["codex_app_server_auto"] == "athena"
...
class TestConfigCommandFailClosedSurface:
    """`athena config set/unset` must exit cleanly (no traceback) when the
