...
class TestGetAthenaHome:
    ...
    def test_env_override(self):
        with patch.dict(os.environ, {"ATHENA_HOME": "/custom/path"}):
            home = get_athena_home()
...
class TestLoadConfigParseFailure:
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        with patch.dict(os.environ, {"ATHENA_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="athena_cli.config"):
                config = load_config()
            ...
            # stands out at athena startup before logging is configured)
            captured = capsys.readouterr()
            assert "athena config:" in captured.err
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"ATHENA_HOME": str(tmp_path)}):
            ...
            assert "athena config:" in after_edit, "edited file should re-warn"
...
class TestRemoveEnvValue:
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ATHENA_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"ATHENA_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or any ``ATHENA_*`` runtime flag.
    ...
    def test_athena_integration_keys_still_writable(self, allowed_key):
        """``ATHENA_*`` overall is NOT blocked — only the four runtime
        location names (HOME/PROFILE/CONFIG/ENV) are. Integration
        credentials following the ``ATHENA_*`` convention must keep
    ...
    def test_arbitrary_user_key_still_works(self):
        ...
        denylist and outside ``ATHENA_*``) keep working. The denylist
