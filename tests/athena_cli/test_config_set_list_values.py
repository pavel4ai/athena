"""``athena config set`` must parse list/mapping literals, not store them as strings.

Before this fix, ``athena config set platform_toolsets.discord '["file","web"]'``
...
def test_round_trip_through_load_config(user_home):
    ...
    from athena_cli.config import set_config_value, load_config
