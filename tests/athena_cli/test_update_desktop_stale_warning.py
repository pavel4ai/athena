...
#88251: ``athena update`` treated a failed desktop pack as non-fatal, printed
...
import athena_cli.update_cmd_maint as update_cmd_maint
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "athena_constants.with_athena_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "athena_constants.display_athena_home", lambda: str(tmp_path), raising=False
    )
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_athena_home", lambda: tmp_path)
