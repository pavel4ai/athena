...
board into `ATHENA_KANBAN_BOARD` at boot so subprocess shell-outs (e.g.
`athena kanban …`) inherit the same board the in-process kanban tools resolve.
Without this, a concurrent `athena kanban boards switch` from another session
...
def _isolate_kanban_board_env():
    """Snapshot `ATHENA_KANBAN_BOARD` and restore it after the test.
    ...
    """
    prev = os.environ.get("ATHENA_KANBAN_BOARD")
    os.environ.pop("ATHENA_KANBAN_BOARD", None)
    try:
        yield
    finally:
        if prev is None:
...
def test_pin_does_not_overwrite_existing_env(monkeypatch):
    ...
    assert main_mod.os.environ.get("ATHENA_KANBAN_BOARD") == "preset"
...
def test_pin_swallows_resolution_failures(monkeypatch):
    ...
    assert "ATHENA_KANBAN_BOARD" not in main_mod.os.environ
