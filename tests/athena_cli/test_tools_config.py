...
from athena_cli.nous_account import NousPortalAccountInfo, NousToolAccessInfo
...
def test_all_invalid_platform_toolsets_logs_runtime_warning(caplog):
    ...
    (e.g. 'athena' instead of 'athena-cli') must warn at resolve time so an
    already-corrupted config is caught at runtime, not just during migration."""
    import athena_cli.tools_config as _tc
    ...
    config = {"platform_toolsets": {"cli": ["athena"]}}

    with caplog.at_level(logging.WARNING, logger="athena_cli.tools_config"):
        _get_platform_tools(config, "cli")

    warnings = [r.getMessage() for r in caplog.records if r.levelno >= logging.WARNING]
    assert any("#38798" in m and "athena" in m for m in warnings), warnings
...
def test_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["athena-cli"]}}
...
def test_partially_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["athena-cli", "bogus"]}}
...
# via the platform's native composite. The composite ``athena-discord``
...
def test_discord_toolsets_do_not_leak_to_other_platforms():
    ...
    config = {"platform_toolsets": {"telegram": ["athena-telegram", "discord"]}}
...
class TestAgentBrowserPostSetup:
    ...
    tools.browser_tool_install._find_agent_browser (PATH -> Homebrew/Athena-managed
    ...
    def _stub_browser_use_install(self):
        ...
        logic under test."""
        with patch("athena_cli.tools_config_post_setup._ensure_browser_use_cli") as stub:
            yield stub

    def test_warns_when_neither_npx_nor_agent_browser_on_path(self):
        with patch("shutil.which", return_value=None), patch(
            "subprocess.run"
        ) as run, patch("athena_cli.tools_config_post_setup._print_warning") as warn:
    ...
    def test_chromium_already_installed_skips_subprocess(self):
        with patch("shutil.which", return_value="/usr/bin/npx"), patch(
            ...
            "athena_cli.tools_config_post_setup._print_success"
        ) as success:
            _run_post_setup("agent_browser")
    ...
    def test_docker_with_missing_chromium_warns_instead_of_installing(self):
        with patch("shutil.which", return_value="/usr/bin/npx"), patch(
            ...
            "athena_cli.tools_config_post_setup._print_warning"
        ) as warn:
            _run_post_setup("agent_browser")
    ...
    def test_installs_chromium_via_npx_resolved_only_through_extended_path(self):
        """Athena-managed-Node-only setups: npx resolves via
        ...
        athena_npx = "/home/user/.athena/node/bin/npx"
        with patch("shutil.which", return_value=None), patch(
            "subprocess.run"
        ) as run, patch(
            "tools.browser_tool_install._chromium_installed", return_value=False
            ...
            "tools.browser_tool_install._resolve_npx_bin", return_value=athena_npx
        ), patch(
            "athena_cli.tools_config_post_setup._print_success"
        ):
            run.return_value = SimpleNamespace(returncode=0, stdout="", stderr="")
        ...
        run.assert_called_once()
        assert run.call_args.args[0] == [
            athena_npx, "--ignore-scripts", "-y", AGENT_BROWSER_NPX_SPEC, "install", "--with-deps",
        ]
    ...
    def test_install_failure_prints_stderr_tail_and_does_not_invalidate_cache(self):
        ...
        with patch("shutil.which", return_value="/usr/bin/npx"), patch(
            ...
            "athena_cli.tools_config_post_setup._print_info"
        ) as info:
...
class TestBrowserUseCliInstalledForAllNonCamofoxBackends:
    ...
    def test_browser_post_setup_attempts_cli_install(self, key):
        with patch("athena_cli.tools_config_post_setup._ensure_browser_use_cli") as ensure, patch(
    ...
    def test_camofox_post_setup_never_touches_browser_use(self):
        ...
        the CLI in."""
        with patch("athena_cli.tools_config_post_setup._ensure_browser_use_cli") as ensure, patch(
            "athena_constants.find_node_executable", return_value=None
        ), patch("subprocess.run"):
    ...
    def test_ensure_helper_always_delegates_to_install_cli(self):
        ...
        $ATHENA_HOME/bin when only side installs exist."""
        with patch(
            "athena_cli.tools_config_post_setup.shutil.which", return_value="/usr/bin/browser-use"
        ), patch(
            "tools.browser_use_cli.install_cli",
            return_value=(True, "browser-use CLI already installed (/managed/bin/browser-use)"),
        ) as install:
            from athena_cli.tools_config import _ensure_browser_use_cli
    ...
    def test_ensure_helper_install_failure_is_non_fatal(self):
        ...
        with patch(
            "athena_cli.tools_config_post_setup.shutil.which", return_value=None
        ), patch(
            "tools.browser_use_cli.install_cli",
            return_value=(False, "`uv tool install browser-use` failed:\nboom"),
        ), patch("athena_cli.tools_config_post_setup._print_warning") as warn:
...
class TestImagegenModelPicker:
    ...
    def test_plugin_picker_falls_back_when_default_is_missing_from_catalog(self):
        ...
        from athena_cli.tools_config import _configure_imagegen_model_for_plugin
        ...
        with (
            patch(
                "athena_cli.tools_config._plugin_image_gen_catalog",
                return_value=(catalog, "also-missing"),
            ),
            patch("athena_cli.tools_config._prompt_choice", return_value=0),
        ):
...
def test_vision_picker_custom_endpoint(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ATHENA_HOME", str(tmp_path))
    import athena_cli.tools_config as tc
    import athena_cli.tools_config_providers as tcp
...
# `athena tools post-setup <key>` child. On Windows each console child (npm,
...
# ── Toolsets that shipped after a platform's last `athena tools` save ────────
#
# Saving the picker (or one toggle in the desktop Toolsets UI) replaces a
# platform's composite (``[athena-cli]``) with a frozen explicit list, and
...
def test_agent_disabled_toolsets_json_array_string_form_still_wins():
    ...
    `athena config set agent.disabled_toolsets '["memory"]'`). It must be
...
def test_disabled_composite_debugging_prunes_constituent_platform_toolsets():
    ...
    toolsets on ``athena tools --summary``, not only strip them at runtime."""
    config = {
        "platform_toolsets": {"cli": ["athena-cli"]},
...
def test_platforms_whose_composite_excludes_it_are_left_narrow():
    ...
    composite (athena-acp, athena-webhook) that never carried the toolset."""
    from toolsets import TOOLSETS, resolve_toolset

    narrow = [
        platform
        for platform in ("acp", "webhook")
        if f"athena-{platform}" in TOOLSETS
        and not any(
            set(resolve_toolset(ts, include_registry=False))
            <= set(resolve_toolset(f"athena-{platform}"))
...
def test_explicit_plugin_toolset_admitted_in_platform_toolsets(monkeypatch):
    ...
    ``athena-cli``), it MUST be admitted as a configurable key instead of
    ...
    import athena_cli.tools_config as _tc_mod
    ...
    # standard composite — exactly the "I want athena-cli AND a2a in my CLI
    # session" config the issue's user was trying to write.
    config = {"platform_toolsets": {"cli": ["athena-cli", "dplat_client"]}}
...
def test_explicit_plugin_toolset_admitted_against_real_a2a_plugin(monkeypatch):
    ...
    a real config like ``platform_toolsets.cli: [athena-cli, a2a]``, ``a2a``
    ...
    from athena_cli.plugins import discover_plugins, get_plugin_toolsets
    discover_plugins()
    ...
    config = {"platform_toolsets": {"cli": ["athena-cli", "a2a"]}}
    enabled = _get_platform_tools(config, "cli")
...
class TestLightpandaPostSetup:
    ...
    def test_reports_binary_when_found(self, _stub_browser_use_install):
        from athena_cli.tools_config import _run_post_setup

        with patch("tools.browser_lightpanda.find_lightpanda_binary", return_value="/opt/lightpanda"), \
             patch("athena_cli.tools_config_post_setup._print_success") as ok, \
             patch("athena_cli.tools_config_post_setup._print_warning") as warn:
    ...
    def test_prints_install_hint_when_missing(self):
        ...
        with patch("tools.browser_lightpanda.find_lightpanda_binary", return_value=None), \
             patch("athena_cli.tools_config_post_setup._print_warning") as warn, \
             patch("athena_cli.tools_config_post_setup._print_info") as info:
            _run_post_setup("lightpanda")
