"""Surfacing tests — managed scope shown in `config show` and `athena doctor`."""
import pytest
...
def test_config_show_no_managed_scope_silent(tmp_path, monkeypatch, capsys):
    ...
    cfg._RAW_CONFIG_CACHE.clear()
    managed_scope.invalidate_managed_cache()
    from athena_cli.config import show_config
...
def test_doctor_silent_with_no_managed_scope(tmp_path, monkeypatch, capsys):
    monkeypatch.setenv("ATHENA_MANAGED_DIR", str(tmp_path / "nope"))
    from athena_cli import managed_scope, doctor
