...
existing Codex CLI tokens via `athena auth openai-codex`. The old
...
@pytest.fixture()
def athena_auth_only_env(tmp_path, monkeypatch):
...
def test_normal_path_still_works(athena_auth_only_env):
...
def test_codex_picker_uses_live_codex_catalog(athena_auth_only_env, tmp_path, monkeypatch):
