...
class TestGetProfileDir:
    ...
    def test_named_profile_returns_profiles_subdir(self, profile_env):
        ...
        assert result == tmp_path / ".athena" / "profiles" / "coder"

    def test_named_profile_matching_is_case_insensitive(self, profile_env):
        tmp_path = profile_env
        assert get_profile_dir("Coder") == tmp_path / ".athena" / "profiles" / "coder"
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.athena must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".athena"
...
class TestNoSkillsOptOut:
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `athena update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestBackfillProfileEnvs:
    ...
    def test_never_overwrites_existing_profile_env(self, profile_env):
        tmp_path = profile_env
        (tmp_path / ".athena" / ".env").write_text("KEY=root\n")
...
class TestGetActiveProfileName:
    ...
    def test_default_athena_home_returns_default(self, profile_env):
        # ATHENA_HOME points to tmp_path/.athena which is the default
...
class TestWrapperScript:
    ...
    def test_creates_bat_on_windows(self, profile_env, monkeypatch):
        ...
        assert "athena -p mybot" in content
        assert "%*" in content
    ...
    def test_remove_returns_false_when_absent(self, profile_env):
        from athena_cli.profiles import remove_wrapper_script
    ...
    def test_custom_alias_target_on_posix(self, profile_env, monkeypatch):
        ...
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "athena -p redqueen" in content
...
class TestFindAliasForProfile:
    ...
    def test_no_alias_returns_none(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from athena_cli.profiles import find_alias_for_profile
...
class TestRenameProfile:
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "athena.ssi_health": {"workspace": "athena", "enabled": True}
            }
        }))
        ...
        assert "athena.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["athena_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["athena_heimdall"]["workspace"] == "athena"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "athena.ssi_health": {"aiPeer": "ssi_health"},
                "athena_heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["athena.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["athena_heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".athena_history",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.athena_history",
...
class TestInternalHelpers:
    ...
    def test_profiles_root_under_home(self, profile_env):
        ...
        assert root == tmp_path / ".athena" / "profiles"

    def test_default_athena_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_athena_home()
        assert home == tmp_path / ".athena"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (ATHENA_HOME outside ~/.athena), profiles go under ATHENA_HOME."""
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (ATHENA_HOME under ~/.athena), profiles root is still ~/.athena/profiles."""
        native = tmp_path / ".athena"
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under ATHENA_HOME."""
        from athena_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ATHENA_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".athena" / "profiles" / "mybot"
        assert result == expected
...
class TestProfilesToServe:
    ...
    def test_off_returns_only_active_default(self, profile_env):
        ...
        name, home = serve[0]
        assert name == "default"
        assert home == _get_default_athena_home()
