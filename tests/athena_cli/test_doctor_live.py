"""Tests for ``athena doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from athena_cli import doctor_live
from athena_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `athena doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("athena_cli.doctor.ATHENA_HOME", tmp_path / "home")
        monkeypatch.setattr("athena_cli.doctor.PROJECT_ROOT", tmp_path / "root")
