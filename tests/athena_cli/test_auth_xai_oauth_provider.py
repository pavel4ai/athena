...
def test_refresh_xai_oauth_tokens_preserves_active_provider(tmp_path, monkeypatch):
    ...
    _setup_athena_auth(athena_home, access_token=near, refresh_token="rt-old")
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))
    ...
    monkeypatch.setattr("athena_cli.auth.refresh_xai_oauth_pure", _fake_pure)
...
def test_login_xai_oauth_relogin_clears_suppression_and_reseeds(tmp_path, monkeypatch):
    """remove -> ``athena model`` re-login (``_login_xai_oauth``) must clear the
    ...
    only ``auth_add_command`` cleared it — the ``athena model`` re-login path did
    not. So after remove -> re-login the seed kept skipping and ``athena auth
    ...
    new_access = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    monkeypatch.setattr(
        "athena_cli.auth._xai_oauth_device_code_login",
        lambda **kwargs: {
            "tokens": {
                "access_token": new_access,
    ...
    monkeypatch.setattr(
        "athena_cli.auth._update_config_for_provider",
