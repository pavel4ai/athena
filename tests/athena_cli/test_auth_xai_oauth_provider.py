"""Tests for xAI Grok OAuth — tokens stored in Athena auth store (~/.athena/auth.json)."""
...
def _setup_athena_auth(
    ...
    *,
    ...
    auth_file = athena_home / "auth.json"
...
def _patch_httpx_client(monkeypatch, response):
    ...
    monkeypatch.setattr("athena_cli.auth.httpx.Client", _factory)
    return holder
...
def test_xai_oauth_authorize_url_includes_referrer_athena_agent():
...
def test_xai_callback_handler_returns_400_when_callback_url_lacks_code_and_error():
    ...
    try:
        ...
        assert "athena auth add xai-oauth" in body
...
def test_save_and_read_xai_oauth_tokens_roundtrip(tmp_path, monkeypatch):
    ...
    athena_home.mkdir(parents=True, exist_ok=True)
    (athena_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
...
def test_read_xai_oauth_tokens_missing_access_token(tmp_path, monkeypatch):
    athena_home = tmp_path / "athena"
    _setup_athena_auth(athena_home, access_token="")
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))
...
def test_read_xai_oauth_tokens_missing_refresh_token(tmp_path, monkeypatch):
    athena_home = tmp_path / "athena"
    _setup_athena_auth(athena_home, refresh_token="")
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))
...
def test_resolve_xai_runtime_credentials_returns_singleton_state(tmp_path, monkeypatch):
    athena_home = tmp_path / "athena"
    fresh = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    _setup_athena_auth(athena_home, access_token=fresh)
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))
    monkeypatch.delenv("ATHENA_XAI_BASE_URL", raising=False)
    ...
    assert creds["source"] == "athena-auth-store"
...
def test_resolve_xai_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("athena_cli.auth._refresh_xai_oauth_tokens", _fake_refresh)
...
def test_resolve_xai_runtime_credentials_honours_env_base_url(tmp_path, monkeypatch):
    ...
    _setup_athena_auth(athena_home, access_token=fresh)
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))
    monkeypatch.setenv("ATHENA_XAI_BASE_URL", "https://custom.x.ai/v1/")
...
# ``XAI_BASE_URL`` / ``ATHENA_XAI_BASE_URL`` are a credential-leak vector
...
def _seed_xai_oauth_state(
    athena_home: Path, state: dict, *, active_provider: str = "xai-oauth"
) -> None:
    athena_home.mkdir(parents=True, exist_ok=True)
    auth_store = {
        "version": 1,
    ...
    (athena_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    """
    athena_home = tmp_path / "athena"
    _seed_xai_oauth_state(athena_home, dict(_STALE_XAI_OAUTH_STATE), active_provider="nous")
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))
    ...
    monkeypatch.setattr("athena_cli.auth._refresh_xai_oauth_tokens", _terminal_refresh)
    ...
    raw = json.loads((athena_home / "auth.json").read_text())
...
def test_resolve_credentials_does_not_quarantine_on_transient_refresh_failure(
    ...
    """
    athena_home = tmp_path / "athena"
    _seed_xai_oauth_state(athena_home, dict(_STALE_XAI_OAUTH_STATE))
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))
    ...
    monkeypatch.setattr("athena_cli.auth._refresh_xai_oauth_tokens", _transient_refresh)
...
def test_refresh_xai_oauth_pure_403_marked_tier_denied_not_relogin(monkeypatch):
    ...
    being active. Re-running ``athena model`` won't help in that
...
def test_format_auth_error_tier_denied_does_not_suggest_relogin():
    ...
    Regression for #26847: telling a tier-gated user to ``athena model``
    ...
    assert "athena model" not in rendered.lower()
...
def test_xai_oauth_discovery_raises_typed_error_on_malformed_json(monkeypatch):
    ...
    from athena_cli.auth import _xai_oauth_discovery
...
def test_xai_oauth_discovery_validates_endpoints(monkeypatch):
    ...
    monkeypatch.setattr("athena_cli.auth.httpx.get", _fake_get)
    with pytest.raises(AuthError) as exc:
        _xai_oauth_discovery()
    assert exc.value.code == "xai_discovery_invalid"
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    """After `athena model` -> xai-oauth, the singleton holds tokens.  load_pool
    must surface that as a pool entry so `athena auth list` reflects truth and
    refreshes route through the pool consistently with codex."""
    from agent.credential_pool import load_pool

    athena_home = tmp_path / "athena"
    fresh = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    _setup_athena_auth(athena_home, access_token=fresh, refresh_token="rt-1")
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))
...
def test_credential_pool_seed_respects_suppression(tmp_path, monkeypatch):
    """`athena auth remove xai-oauth <N>` for the seeded entry suppresses
    ...
    # Suppress the source — mimic `athena auth remove`.
    from athena_cli.auth import suppress_credential_source
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    """End-to-end regression: ``athena auth remove xai-oauth 1`` for a
    ...
    # Act: the user runs `athena auth remove xai-oauth 1`.
    ...
    # nothing to import.
    raw_after = json.loads((athena_home / "auth.json").read_text())
...
def test_pool_sync_back_writes_to_singleton(tmp_path, monkeypatch):
    ...
    _setup_athena_auth(athena_home, access_token=expired, refresh_token="rt-old")
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))
    ...
    monkeypatch.setattr("athena_cli.auth.refresh_xai_oauth_pure", _fake_refresh)
    ...
    auth_path = athena_home / "auth.json"
...
def test_pool_entry_needs_refresh_when_jwt_within_skew(tmp_path, monkeypatch):
    ...
    from agent.credential_pool import load_pool, AUTH_TYPE_OAUTH, PooledCredential
    from athena_cli.auth import XAI_ACCESS_TOKEN_REFRESH_SKEW_SECONDS
    import uuid
...
def test_pool_refresh_marks_entry_exhausted_on_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("athena_cli.auth.refresh_xai_oauth_pure", _fake_refresh_fail)
...
def test_pool_seeded_entry_sync_back_after_refresh(tmp_path, monkeypatch):
    ...
    athena_home = tmp_path / "athena"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_athena_auth(athena_home, access_token=near_expiry, refresh_token="rt-singleton")
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))
...
def test_pool_refresh_adopts_singleton_tokens_when_consumed_elsewhere(tmp_path, monkeypatch):
    ...
    _setup_athena_auth(athena_home, access_token=in_memory_at, refresh_token="rt-stale")
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))
    ...
    (athena_home / "auth.json").write_text(json.dumps(raw))
...
def test_pool_refresh_recovers_when_other_process_already_refreshed(tmp_path, monkeypatch):
    ...
    _setup_athena_auth(athena_home, access_token=in_memory_at, refresh_token="rt-shared")
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))
...
def test_pool_exhausted_xai_entry_recovers_after_singleton_refresh(tmp_path, monkeypatch):
    ...
    user runs ``athena model`` -> xAI Grok OAuth (or another process
    ...
    _setup_athena_auth(athena_home, access_token=stale_at, refresh_token="rt-stale")
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))
    ...
    # Simulate the user re-running `athena model` -> xAI Grok OAuth: the
...
def test_pool_manual_xai_entry_not_synced_from_singleton(tmp_path, monkeypatch):
    ...
    ``athena auth add xai-oauth``) own their own refresh-token lifecycle
    ...
    _setup_athena_auth(athena_home, access_token=singleton_at, refresh_token="rt-singleton")
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))
...
def test_pool_manual_entry_does_not_sync_back_to_singleton(tmp_path, monkeypatch):
    """`athena auth add xai-oauth` entries (source='manual:xai_pkce') are
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    provider (visible to ``athena auth status``, ``athena setup``, and the
    ...
    athena_home = tmp_path / "athena"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_athena_auth(athena_home, access_token=near_expiry, refresh_token="rt-xai")
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))
