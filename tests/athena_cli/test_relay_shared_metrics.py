"""Focused tests for the Athena shared-metrics durable store."""
...
from athena_cli.observability import shared_metrics as shared_metrics_module
from athena_cli.observability.shared_metrics import SharedMetricsStore
from athena_cli.observability.shared_metrics_contract import (
...
def _resource(
    athena_version: str = "test-version",
    *,
    os_family: str = "linux",
...
def test_v1_outbox_package_exports_unchanged_after_upgrade(tmp_path):
    ...
    payload = {
        ...
        "resource": {"athena_version": "legacy-version"},
        "metrics": [
            {
                "name": LEGACY_MODEL_CALL_METRIC,
                "type": "counter",
...
def test_client_active_mark_accepts_only_an_empty_allowlisted_payload():
    event = SimpleNamespace(
        ...
        scope_category=None,
        metadata={
            "athena.metrics.schema_version": "athena.metrics.event.v2",
        },
        data={},
    )
    ...
    wrong_schema.metadata["athena.metrics.schema_version"] = "unknown"
...
def test_auxiliary_logical_scope_projects_one_normalized_terminal_route():
    event = SimpleNamespace(
        ...
        metadata={
            ...
            "athena.call_role": "auxiliary:compression",
        },
    )
    ...
    event.metadata["athena.call_role"] = "primary"
...
def test_tool_subscriber_contract_accepts_only_bounded_events():
    terminal = SimpleNamespace(
        ...
        category="tool",
        category_profile={},
        name="athena.tool_call",
        scope_category="end",
        metadata={"athena.metrics.schema_version": "athena.metrics.event.v2"},
        data={
...
def test_skill_subscriber_contract_accepts_only_bounded_marks():
    ...
    assert skill_counter(load) == ("athena.skill.load.count", load.data)
...
def test_store_exports_task_started_and_terminal_counters(tmp_path):
    ...
    store.record_counter("athena.task_run.finished", terminal, _resource())
...
def test_retention_prunes_only_expired_exported_history(tmp_path):
    ...
    with sqlite3.connect(database_path) as connection:
        connection.execute(
            ...
            WHERE athena_version = 'expired-version'
            """
        )
        connection.execute(
            """
    ...
    with sqlite3.connect(database_path) as connection:
        ...
        aggregate_versions = {
            ...
            for row in connection.execute(
                "SELECT athena_version FROM counter_aggregates"
            ).fetchall()
        }
