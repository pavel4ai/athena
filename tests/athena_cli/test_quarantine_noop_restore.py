...
``athena*.exe`` shims aside (``athena.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``athena`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("athena", "athena-agent", "athena-acp", "athena-gateway"):
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "athena.exe" in names, "athena.exe must be restored after a no-op install"
    assert "athena-acp.exe" in names
    assert "athena-gateway.exe" in names
    assert (scripts / "athena.exe").read_bytes() == b"MZ-old-athena"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "athena.exe").read_bytes() == b"MZ-new-athena"
...
# ---------------------------------------------------------------------------
# athena_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "athena.exe" in names, "athena.exe must be restored after a no-op recovery install"
    assert (scripts / "athena.exe").read_bytes() == b"MZ-old-athena"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "athena.exe").write_bytes(b"MZ-new-athena")
