"""Tests for Codex auth — tokens stored in Athena auth store (~/.athena/auth.json)."""
...
def _setup_athena_auth(athena_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_athena_auth(athena_home)
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_athena_auth(athena_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))
    ...
    monkeypatch.setattr("athena_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    athena_home = tmp_path / "athena"
    _setup_athena_auth(athena_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((athena_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``athena auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``athena auth add openai-codex``.
    ...
    (athena_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `athena auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``athena auth add openai-codex`` for accounts B and C
    ...
    (athena_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``athena auth add openai-codex`` (account B and account C).
...
def test_save_codex_tokens_still_refreshes_legacy_manual_alias(tmp_path, monkeypatch):
    ...
    ``athena auth add openai-codex`` as a workaround when there was no
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    ...
    _save_codex_tokens({"access_token": "athena-at", "refresh_token": "athena-rt"})
    ...
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "athena-at"


def test_resolve_returns_athena_auth_store_source(tmp_path, monkeypatch):
    athena_home = tmp_path / "athena"
    _setup_athena_auth(athena_home)
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run athena auth".
    """
    from athena_cli.auth import (
    ...
    assert "athena auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from athena_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from athena_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "athena_cli.auth.resolve_codex_runtime_credentials",
        lambda: {"base_url": DEFAULT_CODEX_BASE_URL},
    )
    monkeypatch.setattr(
        "athena_cli.auth._import_codex_cli_tokens",
    ...
    monkeypatch.setattr("athena_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("athena_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
...
def _patch_httpx_post(monkeypatch, responses):
    """Patch athena_cli.auth.httpx.Client so .post() returns queued responses."""
    seq = iter(responses)
    ...
    monkeypatch.setattr("athena_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())
