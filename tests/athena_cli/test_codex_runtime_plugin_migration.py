...
from athena_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
    MIGRATION_END_MARKER,
    _build_athena_tools_mcp_entry,
    _format_toml_value,
...
class TestMigrate:
    ...
    def test_no_servers_still_writes_permissions_default(self, tmp_path):
        ...
        report = migrate({}, codex_home=tmp_path, discover_plugins=False, expose_athena_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_plugin_discovery_writes_plugin_blocks(self, tmp_path, monkeypatch):
        ...
        from athena_cli import codex_runtime_plugin_migration as crpm
    ...
    def test_plugin_discovery_skips_unavailable_plugins(self):
        ...
        from athena_cli.codex_runtime_plugin_migration import _query_codex_plugins
    ...
    def test_plugin_discovery_failure_non_fatal(self, tmp_path, monkeypatch):
        ...
        report = migrate({"mcp_servers": {"x": {"command": "y"}}},
                         codex_home=tmp_path, discover_plugins=True, expose_athena_tools=False)
        assert report.written
    ...
    def test_discover_plugins_false_skips_query(self, tmp_path, monkeypatch):
        ...
        migrate({"mcp_servers": {"x": {"command": "y"}}},
                codex_home=tmp_path, discover_plugins=False, expose_athena_tools=False)
    ...
    def test_dry_run_skips_plugin_query(self, tmp_path, monkeypatch):
        ...
        migrate({"mcp_servers": {"x": {"command": "y"}}},
                codex_home=tmp_path, dry_run=True, discover_plugins=True, expose_athena_tools=False)
        assert called["yes"] is False
    ...
    def test_expose_athena_tools_writes_callback_mcp_entry(self, tmp_path):
        """When expose_athena_tools=True (production default), an
        [mcp_servers.athena-tools] entry is written so codex calls back
        ...
        should be useable by athena' — quirk #7."""
        report = migrate({}, codex_home=tmp_path,
                         discover_plugins=False,
                         default_permission_profile=None,
                         expose_athena_tools=True)
        text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.athena-tools]" in text
        assert "athena_tools_mcp_server" in text
        ...
        assert "athena-tools" in report.migrated

    def test_expose_athena_tools_disabled_skips_entry(self, tmp_path):
        """expose_athena_tools=False suppresses the callback registration."""
        migrate({}, codex_home=tmp_path,
                discover_plugins=False,
                default_permission_profile=None,
                expose_athena_tools=False)
        text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.athena-tools]" not in text
        assert "athena_tools_mcp_server" not in text

    def test_dry_run_doesnt_write(self, tmp_path):
        report = migrate({"mcp_servers": {"x": {"command": "y"}}},
                         codex_home=tmp_path, dry_run=True, expose_athena_tools=False)
    ...
    def test_full_migration_round_trip(self, tmp_path):
        ...
        report = migrate(athena_cfg, codex_home=tmp_path, expose_athena_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_idempotent_re_run_replaces_managed_block(self, tmp_path):
        # First migration
        migrate({"mcp_servers": {"a": {"command": "x"}}}, codex_home=tmp_path, expose_athena_tools=False)
        first_text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.a]" in first_text
        # Second migration with different servers
        migrate({"mcp_servers": {"b": {"command": "y"}}}, codex_home=tmp_path, expose_athena_tools=False)
    ...
    def test_managed_root_keys_stay_top_level_when_config_ends_in_table(self, tmp_path):
        ...
        migrate({}, codex_home=tmp_path, discover_plugins=False, expose_athena_tools=False)
        new_text = target.read_text()
    ...
    def test_preserves_user_mcp_server_outside_managed_block(self, tmp_path):
        ...
        migrate({"mcp_servers": {"athena-mcp": {"command": "npx"}}},
        ...
        assert "[mcp_servers.athena-mcp]" in final
    ...
    def test_skipped_keys_reported(self, tmp_path):
        ...
        }, codex_home=tmp_path, expose_athena_tools=False)
    ...
    def test_invalid_mcp_servers_value(self, tmp_path):
        report = migrate({"mcp_servers": "notadict"}, codex_home=tmp_path, expose_athena_tools=False)
...
class TestStripUnmanagedPluginTables:
    ...
    ``athena codex-runtime migrate`` would re-emit them inside the managed
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "athena_cli.codex_runtime_plugin_migration._query_codex_plugins",
            fake_query,
        )
        migrate({}, codex_home=tmp_path, discover_plugins=True, expose_athena_tools=False)
        new_text = target.read_text()
...
# ---- Bug C: ATHENA_HOME tempdir leak into ~/.codex/config.toml ----


class TestAthenaHomeLeakGuard:
    """Regression tests for issue #26250 Bug C.

    Previously ``_build_athena_tools_mcp_entry()`` read ``ATHENA_HOME``
    ...
    once codex spawned the athena-tools MCP subprocess.
    """

    def test_tempdir_detector_recognizes_pytest_paths(self):
        assert _looks_like_test_tempdir(
            "/private/var/folders/abc/pytest-of-kshitij/pytest-137/popen-gw2/test_X/athena_test"
        )
        assert _looks_like_test_tempdir(
            "/tmp/pytest-of-user/pytest-12/test_X/athena"
        )
        assert _looks_like_test_tempdir(
            "/private/var/folders/zz/T/pytest-of-bob/pytest-1"
        )

    def test_tempdir_detector_accepts_real_athena_home(self):
        assert not _looks_like_test_tempdir("/Users/alice/.athena")
        assert not _looks_like_test_tempdir("/home/bob/.athena")
        assert not _looks_like_test_tempdir("/opt/athena")
        assert not _looks_like_test_tempdir("")

    def test_pytest_tempdir_not_burned_into_mcp_env(self, monkeypatch):
        """The headline regression: even when ATHENA_HOME points at a pytest
        tempdir, _build_athena_tools_mcp_entry() must NOT propagate it."""
        monkeypatch.setenv(
            "ATHENA_HOME",
            "/private/var/folders/xx/pytest-of-user/pytest-99/test_x/athena_test",
        )
        entry = _build_athena_tools_mcp_entry()
        env = entry.get("env", {})
        assert "ATHENA_HOME" not in env, (
            f"pytest-tempdir ATHENA_HOME leaked into codex MCP entry: "
            f"{env.get('ATHENA_HOME')!r}"
        )

    def test_real_athena_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate ATHENA_HOME (not a tempdir path) DOES propagate so the
        ...
        real_path = "/Users/alice/.athena"
        monkeypatch.setenv("ATHENA_HOME", real_path)
        entry = _build_athena_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("ATHENA_HOME") == real_path

    def test_unset_athena_home_omits_env_key(self, monkeypatch):
        """When ATHENA_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever ATHENA_HOME its launcher (systemd, gateway, shell)
        ...
        env = entry.get("env", {})
        assert "ATHENA_HOME" not in env, (
            f"ATHENA_HOME should not be set when env var is unset, got: "
            f"{env.get('ATHENA_HOME')!r}"
        )
