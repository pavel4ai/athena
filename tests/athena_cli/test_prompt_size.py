"""Tests for the ``athena prompt-size`` diagnostic (issue #34667)."""

import json

import pytest

from athena_cli.prompt_size import (
    _SKILLS_BLOCK_RE,
...
def _seed_memory(athena_home, memory_text="", user_text=""):
    mem_dir = athena_home / "memories"
...
def _seed_skill(athena_home, name, description):
    skill_dir = athena_home / "skills" / "demo" / name
