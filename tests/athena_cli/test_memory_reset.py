"""Tests for the `athena memory reset` CLI command.
...
- Profile-scoped reset (uses ATHENA_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake ATHENA_HOME with memory files."""
    athena_home = tmp_path / ".athena"
    memories = athena_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("ATHENA_HOME", str(athena_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nAthena repo is at ~/.athena/athena-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `athena memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        athena_home = tmp_path / ".athena"
        (athena_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("ATHENA_HOME", str(athena_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's ATHENA_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_athena_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
