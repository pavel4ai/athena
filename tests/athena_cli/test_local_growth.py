...
def test_overrides_roundtrip_and_clear(athena_home):
    from athena_cli.local_runtime.growth import (
...
def test_corrupt_overrides_read_as_empty(athena_home):
    from athena_cli.local_runtime.growth import (
        load_window_overrides,
...
def test_growth_declines_foreign_endpoints(athena_home):
...
def test_occupancy_confirmed_skips_gate_one():
    ...
    from athena_cli.local_runtime.context_policy import growth_decision
    from athena_cli.local_runtime.estimator import (
        HardwareBudget,
        LayerKind,
        ModelProfile,
    )
    ...
    # Athena' threshold (e.g. 80% of window) can sit BELOW the ladder's 85%
...
def _tiny_profile(model_id: str):
    from athena_cli.local_runtime.estimator import LayerKind, ModelProfile
...
def test_preset_generation_for_catalog_model_with_mmproj(athena_home, tmp_path, monkeypatch):
    ...
    import athena_cli.local_runtime.presets as presets_mod

    from athena_cli.local_runtime.catalog import CATALOG
    from athena_cli.local_runtime.estimator import HardwareBudget
...
def test_preset_restores_grown_window_capped_at_native(athena_home, tmp_path, monkeypatch):
    ...
    from athena_cli.local_runtime.estimator import HardwareBudget
    from athena_cli.local_runtime.growth import save_window_override
...
def test_preset_ignores_override_below_launch_window(athena_home, tmp_path, monkeypatch):
...
def test_preset_restores_grown_window_midladder(athena_home, tmp_path, monkeypatch):
    """The real growth shape: launch at a lower rung, override to a middle
    rung -> the preset window follows the override."""
    import athena_cli.local_runtime.presets as presets_mod

    from athena_cli.local_runtime.estimator import HardwareBudget, LayerKind, ModelProfile
    from athena_cli.local_runtime.growth import save_window_override
...
def test_mtp_plan_matches_cost_at_initial_and_restored_windows(athena_home, tmp_path, monkeypatch):
    from dataclasses import replace
    from types import SimpleNamespace

    from athena_cli.local_runtime import presets
    from athena_cli.local_runtime.context_policy import FLOOR, RUNTIME_OVERHEAD_BYTES, ub_logits_bytes
    from athena_cli.local_runtime.estimator import HardwareBudget, LayerKind, ModelProfile, ctx_bytes
    from athena_cli.local_runtime.growth import save_window_override
...
def test_growth_requires_an_admissible_materialized_preset(athena_home, tmp_path, monkeypatch):
    from dataclasses import replace
    from types import SimpleNamespace

    from athena_cli.local_runtime import bootstrap, catalog, growth, hardware, presets
    from athena_cli.local_runtime.context_policy import FLOOR, RUNTIME_OVERHEAD_BYTES, ub_logits_bytes
    from athena_cli.local_runtime.estimator import HardwareBudget, ctx_bytes
    ...
    from athena_cli.local_runtime import gguf, estimator
    ...
    from athena_cli.local_runtime.binaries import runtimes_root
...
def test_sampling_ladder_file_beats_catalog_beats_nothing(athena_home, tmp_path, monkeypatch):
