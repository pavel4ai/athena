...
def client(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ATHENA_DASHBOARD_SESSION_TOKEN", "soul-test-token")
...
def profile_dir(tmp_path, monkeypatch) -> Path:
    """Create a real profile directory under the test ATHENA_HOME."""
    monkeypatch.setenv("ATHENA_HOME", str(tmp_path))
...
class TestSoulWriteDurability:
    ...
    def test_existing_soul_survives_an_interrupted_save(
        ...
        # ATHENA_HOME patch the client/profile_dir fixtures installed.
...
class TestSoulIoIsOffTheEventLoop:
    ...
    ``tests/athena_cli/test_web_profiles_off_loop.py``.
    """
