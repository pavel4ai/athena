...
the SAME provider universe as ``athena model`` (the CLI/TUI picker).
...
def test_every_athena_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `athena model` universe."""
    ...
    assert not missing, (
        "providers shown in `athena model` but not configurable in the desktop "
