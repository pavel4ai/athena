"""Tests for athena_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import athena_cli.gui_uninstall as gu


def _make_agent(athena_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = athena_home / "athena-agent"
    (agent_root / "athena_cli").mkdir(parents=True)
    (agent_root / "athena_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(athena_home: Path) -> None:
    """Create the source-built GUI artifacts a `athena desktop` run produces."""
    desktop = athena_home / "athena-agent" / "apps" / "desktop"
    ...
    (athena_home / "athena-agent" / "node_modules").mkdir(parents=True)
    (athena_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(athena_home: Path) -> None:
    (athena_home / "config.yaml").write_text("x: 1\n")
    (athena_home / ".env").write_text("KEY=secret\n")
    (athena_home / "sessions").mkdir()


def test_agent_is_installed_detects_source_and_venv(tmp_path):
    athena_home = tmp_path / ".athena"
    athena_home.mkdir()
    assert gu.agent_is_installed(athena_home) is False
    _make_agent(athena_home)
    assert gu.agent_is_installed(athena_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    athena_home = tmp_path / ".athena"
    (athena_home / "athena-agent" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(athena_home) is True


def test_source_built_artifacts_lists_known_paths(tmp_path):
    athena_home = tmp_path / ".athena"
    _make_gui_build(athena_home)
    artifacts = gu.source_built_gui_artifacts(athena_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(athena_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(athena_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    athena_home = tmp_path / ".athena"
    agent_root = _make_agent(athena_home)
    _make_gui_build(athena_home)
    _make_user_data(athena_home)
    ...
    removed = gu.uninstall_gui(athena_home)
    ...
    assert not (athena_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "athena_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (athena_home / "config.yaml").exists()
    assert (athena_home / ".env").exists()
    assert (athena_home / "sessions").exists()
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(athena_home, remove_userdata=False)
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(athena_home)
    ...
    assert summary["athena_home"] == str(athena_home)
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    ATHENA_HOME with all the destructive externals stubbed out.
    """
    import athena_cli.uninstall as uninstall
    ...
    monkeypatch.setattr(uninstall, "get_athena_home", lambda: athena_home)
    ...
    from athena_cli import gui_uninstall as gu_mod
    ...
    assert not (desktop / "release").exists()
    assert (athena_home / "config.yaml").exists()
    assert athena_home.exists()


def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    """``--yes --full`` removes the whole ATHENA_HOME non-interactively."""
    import athena_cli.uninstall as uninstall

    athena_home = tmp_path / ".athena"
    (athena_home / "athena-agent" / "athena_cli").mkdir(parents=True)
    (athena_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
    ...
    assert not athena_home.exists()


def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m athena_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_athena_home", lambda: athena_home)
    ...
    assert not (desktop / "release").exists()
    assert not (athena_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "athena_cli").exists()
    assert (athena_home / "config.yaml").exists()
