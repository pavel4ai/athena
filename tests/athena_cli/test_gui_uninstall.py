...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `athena desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    athena_home = tmp_path / ".athena"
    _make_agent(athena_home)
    icon = lde.icon_path(athena_home / "athena-agent")
    ...
    assert lde.icon_path(athena_home / "athena-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (athena_home / "athena-agent" / "athena_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".athena")
