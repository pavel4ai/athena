"""Tests for ``athena peer`` — cross-machine bot-to-bot DMs."""
...
from athena_cli.subcommands import peer as peer_cmd
...
class _FakePeer(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == "/api/sessions":
            ...
            return self._json({"object": "athena.session", "session": {"id": "bc_1", "title": body.get("title")}}, 201)
...
# ── per-turn author (ATHENA_TURN_AUTHOR set by the message_agent runner) ─────
...
class _AttackerOrigin(BaseHTTPRequestHandler):
    ...
    a compromised/MITM'd peer could redirect a ``athena peer dm`` request to."""
...
class _RedirectingPeer(BaseHTTPRequestHandler):
    ...
    the shape of a compromised peer or a LAN MITM answering ``athena peer
