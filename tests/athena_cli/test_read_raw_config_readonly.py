...
@pytest.fixture()
def isolated_athena_home():
    """Per-test ATHENA_HOME dir (already redirected by the autouse conftest
...
def test_freshness_after_config_edit(isolated_athena_home):
    from athena_cli.config import read_raw_config_readonly

    cfg = _write_config(isolated_athena_home, {"display": {"ephemeral_system_ttl": 1}})
    first = read_raw_config_readonly()
    assert first["display"]["ephemeral_system_ttl"] == 1

    _write_config(isolated_athena_home, {"display": {"ephemeral_system_ttl": 7}})
...
def test_missing_config_returns_empty(isolated_athena_home):
    from athena_cli.config import read_raw_config_readonly

    cfg = isolated_athena_home / "config.yaml"
    if cfg.exists():
        cfg.unlink()
