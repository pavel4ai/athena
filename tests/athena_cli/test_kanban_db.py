...
class TestSharedBoardPaths:
    ...
    def test_dispatcher_spawn_injects_kanban_paths_without_stale_session(
        ...
        # ATHENA_SESSION_* identity inherited from the long-lived gateway.
        # The one exception is ATHENA_SESSION_SOURCE, which the dispatcher
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see athena_state_wal.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    import athena_state_wal as _athena_state_wal
    monkeypatch.setattr(
        _athena_state_wal, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    _athena_state_wal._wal_fallback_warned_paths.clear()

    # Clear module cache so a fresh connect() is attempted
    kb._INITIALIZED_PATHS.clear()
    athena_state_wal._wal_fallback_warned_paths.clear()
    ...
    with _patch("athena_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("ERROR", logger="athena_state"):
            conn = kbc.connect()
...
def test_connect_works_when_wal_is_silently_refused(tmp_path, monkeypatch, caplog):
    ...
    with _patch(
        "athena_cli.kanban_db.sqlite3.connect",
...
def test_add_column_if_missing_is_idempotent_on_race(kanban_home):
    ...
    from athena_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    from athena_cli.sqlite_safe_read import file_length_matches_header
