"""Tests for ``athena update`` / ``--check`` inside the Docker container.
...
fix, ``athena update`` would fall through to ``"✗ Not a git repository.
...
from athena_cli.main import _cmd_update_check, cmd_update
...
@patch("athena_cli.config.detect_install_method", return_value="docker")
@patch("subprocess.run")
def test_cmd_update_in_docker_prints_guidance_and_exits(
    mock_run, _mock_method, _mock_managed, capsys
):
    """``athena update`` inside Docker → friendly message + exit 1, no git calls."""
...
def test_cmd_update_check_in_docker_prints_guidance_and_exits(
    ...
    """``athena update --check`` inside Docker → same message + exit 1, no fetch."""
    with pytest.raises(SystemExit) as excinfo:
...
def test_format_docker_update_message_contents():
    ...
    assert "ATHENA_HOME" in msg or "/opt/data" in msg, (
