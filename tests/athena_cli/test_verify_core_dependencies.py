...
class TestVerifyCoreDependencies:
    ...
    def test_skips_deps_excluded_by_environment_markers(self, temp_pyproject, fake_venv_python):
        ...
        with patch("athena_cli.main_install_repair._resolve_install_target_python", return_value=py), \
             patch("athena_cli.main_install_repair.subprocess.run", side_effect=fake_subprocess_run), \
             patch("athena_cli.main_install_repair._run_install_with_heartbeat"):

            from athena_cli.main_install_repair import _verify_core_dependencies_installed
    ...
    def test_no_pyproject_is_noop(self, tmp_path, monkeypatch):
        ...
        with patch("athena_cli.main_install_repair._resolve_install_target_python") as mock_resolve, \
             patch("athena_cli.main_install_repair._run_install_with_heartbeat") as mock_install:
            from athena_cli.main_install_repair import _verify_core_dependencies_installed
...
class TestResolveInstallTargetPython:
    def test_uses_virtual_env_from_environment(self, tmp_path):
        ...
        with patch("athena_cli.main_install_repair._is_windows", return_value=True):
            from athena_cli.main_install_repair import _resolve_install_target_python
