...
athena_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `athena config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_athena_home = get_athena_home()`` at import time (module
    singleton), so monkeypatching ATHENA_HOME after import doesn't move it.
    ...
    this only matters for tests that swap ATHENA_HOME.)
    """
    import cli

    cli._athena_home = home
