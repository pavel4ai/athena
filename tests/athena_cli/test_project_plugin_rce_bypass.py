...
Execution via the ``ATHENA_ENABLE_PROJECT_PLUGINS`` bypass in the web
server's dashboard plugin loader.

Two primitives combined into the original advisory chain:

1. ``athena_cli.web_server._discover_dashboard_plugins`` opted into
   the untrusted ``./.athena/plugins/`` source via
   ``os.environ.get("ATHENA_ENABLE_PROJECT_PLUGINS")`` — truthy for
...
2. ``athena_cli.web_server._mount_plugin_api_routes`` then imported
...
# ---------------------------------------------------------------------------
# Layer 1 — ATHENA_ENABLE_PROJECT_PLUGINS env gate uses truthy semantics.
# ---------------------------------------------------------------------------
...
class TestProjectPluginsEnvGate:
    ...
    def project_plugin(self, tmp_path, monkeypatch):
        """Plant a project-source plugin under CWD's ``.athena/plugins``
    ...
    def test_falsy_values_keep_project_plugins_disabled(
        ...
        if value == "":
            ...
            monkeypatch.setenv("ATHENA_ENABLE_PROJECT_PLUGINS", value)

        plugins = web_server._get_dashboard_plugins(force_rescan=True)
        names = {p["name"] for p in plugins}
        assert "evil" not in names, (
            f"ATHENA_ENABLE_PROJECT_PLUGINS={value!r} must NOT enable the "
...
class TestDiscoveryScrubsApiField:
    ...
    def test_bundled_safe_api_path_survives(self, tmp_path, monkeypatch):
        ...
        athena_home.mkdir()
        monkeypatch.setenv("ATHENA_BUNDLED_PLUGINS", str(tmp_path / "bundled"))
        dashboard = _write_plugin_manifest(
            tmp_path / "bundled",
            "safe-bundled",
            {
...
class TestEndToEndPocBlocked:
    ...
    ``ATHENA_ENABLE_PROJECT_PLUGINS=0`` (so the operator believed the
    ...
    def test_full_chain_blocked(self, tmp_path, monkeypatch):
        ...
        for call in spec.call_args_list:
            ...
            assert module_name != "athena_dashboard_plugin_evil"
            assert target != payload_py
            assert "evil-repo" not in target.parts
        assert "athena_dashboard_plugin_evil" not in sys.modules
