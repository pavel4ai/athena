...
from athena_cli.auth import AuthError, _refresh_codex_auth_tokens, resolve_codex_runtime_credentials
...
def test_self_heals_missing_singleton_access_token_from_codex_cli(tmp_path, monkeypatch):
    ...
    assert resolved["source"] == "athena-auth-store"
    stored = json.loads((athena_home / "auth.json").read_text())
