...
from athena_cli.web_routers import _common, analytics
...
def test_corrupt_store_polls_return_status_and_warn_once_per_interval(tmp_path, monkeypatch, caplog):
    ...
    with caplog.at_level(logging.DEBUG, logger="athena_cli.web_server"):
    ...
    for resp in (first, second, third):
        ...
        assert "athena doctor" in resp.json()["detail"]["message"]
    # Only the dashboard's own warning counts: athena_state logs an unrelated
    # once-per-process SQLite-version advisory on some interpreters (CI's 3.50.4).
    warnings = [
        r for r in caplog.records
        if r.levelno >= logging.WARNING and r.name.startswith("athena_cli.web_server")
    ]
    ...
    assert sum(
        r.levelno >= logging.WARNING and r.name.startswith("athena_cli.web_server")
