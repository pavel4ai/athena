...
process profile, never a per-task ``set_athena_home_override``.
...
a secondary profile (via ``set_athena_home_override``) must NOT cause that
...
``get_process_athena_home()`` so the comparison reflects the launch scope,
...
def test_process_athena_home_ignores_task_override(tmp_path, monkeypatch):
    ...
    try:
        assert (
            env_loader._process_athena_home().resolve() == launch_home.resolve()
