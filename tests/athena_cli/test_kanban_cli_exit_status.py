...
def _run_athena(home: Path, *args: str, marker: bool = False) -> subprocess.CompletedProcess[str]:
    env = os.environ.copy()
    env["ATHENA_HOME"] = str(home)
    env["ATHENA_KANBAN_HOME"] = str(home)
    for name in (
        "ATHENA_KANBAN_BOARD",
        "ATHENA_KANBAN_DB",
    ...
    if marker:
        env["ATHENA_DELEGATED_CHILD_CONTEXT"] = "1"
    else:
        env.pop("ATHENA_DELEGATED_CHILD_CONTEXT", None)
...
def test_delegated_child_kanban_cli_refusal_returns_nonzero_exit_status(tmp_path):
    ...
    created = _run_athena(home, "kanban", "create", "exit status probe", "--json")
