...
profile's ATHENA_HOME, and the dashboard's own profile stays untouched.
"""
import pytest
import yaml
...
class TestProfileScopedMessagingReads:
    ...
    def test_scoped_read_returns_profile_path_command_and_startup_failure(
        ...
        monkeypatch.setattr(
            ...
            # relying on process-level ATHENA_HOME resolution (#71211).
            lambda *a, **k: {
        ...
        assert payload["gateway_start_command"] == (
            "athena -p worker_alpha gateway start"
        )
...
def test_named_current_home_matches_unscoped(client, isolated_profiles, monkeypatch):
    from athena_cli.web_server_profiles import _config_profile_scope, _athena_home_scope
    from athena_constants import get_athena_home
    ...
    with _athena_home_scope(isolated_profiles["worker_alpha"]):
        with _config_profile_scope("default") as scoped:
            assert scoped is None
            assert get_athena_home() == isolated_profiles["default"]
...
def test_scoped_enablement_uses_only_own_credentials(client, isolated_profiles, monkeypatch):
    ...
    from athena_cli.web_server_messaging import _messaging_platform_catalog
