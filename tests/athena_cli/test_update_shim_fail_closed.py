...
The bug class: on Windows, when `athena.exe`/sibling shims could not be
...
import athena_cli._install_repair as ir
import athena_cli.update_cmd as update_cmd


def _make_shims(scripts_dir: Path, names=("athena", "athena-gateway")) -> list[Path]:
...
def test_strict_quarantine_refuses_before_install(windows, tmp_path, monkeypatch):
    ...
    # athena.exe cannot be renamed; athena-gateway.exe can
    real_rename = Path.rename

    def deny_athena(self, target):
        if self.name == "athena.exe":
            raise PermissionError(13, "held open")
        return real_rename(self, target)

    monkeypatch.setattr(Path, "rename", deny_athena)
    monkeypatch.setattr(main_install_repair, "_athena_exe_shims", lambda d: shims)
    ...
    assert install_ran == []
    assert "athena.exe" in exc_info.value.failed_shims
    # The successful rename (athena-gateway.exe) was rolled back.
    assert (scripts / "athena-gateway.exe").exists()
    assert not list(scripts.glob("athena-gateway.exe.old.*"))


def test_non_strict_keeps_warn_and_try(windows, tmp_path, monkeypatch):
    scripts = tmp_path / "venv" / "Scripts"
    shims = _make_shims(scripts, names=("athena",))
    monkeypatch.setattr(
        Path, "rename",
...
def test_recovery_install_cmd_fail_closed(windows, tmp_path, monkeypatch):
    ...
    _make_shims(scripts, names=("athena",))
...
def test_refusal_writes_marker_and_exits_2(monkeypatch, capsys):
    ...
    exc = main_install_repair.ShimQuarantineError(["athena.exe"])
    with pytest.raises(SystemExit) as exit_info:
        update_cmd._refuse_update_for_contended_shims(exc)
    assert exit_info.value.code == 2
    assert wrote == [1]
    out = capsys.readouterr().out
    assert "athena.exe" in out
