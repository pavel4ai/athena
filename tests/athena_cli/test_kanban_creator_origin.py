...
def test_creator_origin_survives_without_dependency_parent(tmp_path, monkeypatch, capsys, surface):
    from athena_cli import kanban_db as kb, kanban_db_connect as kbc, kanban_db_notify as kn
    from athena_cli.kanban_db_graph import decompose_triage_task
    ...
    with kbc.connect_closing() as conn:
        ...
        if surface == "builtin":
            ...
            from athena_cli.kanban_parser import build_parser
            parser = argparse.ArgumentParser()
            build_parser(parser.add_subparsers())
            monkeypatch.setenv("ATHENA_KANBAN_TASK", owner)
