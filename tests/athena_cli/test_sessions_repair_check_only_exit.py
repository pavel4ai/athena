"""`athena sessions repair --check-only` must fail (non-zero) when the store is unhealthy.
...
import athena_state
from athena_cli import sessions_cmd
...
def test_check_only_exit_status_tracks_probe_verdict(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(athena_state, "DEFAULT_DB_PATH", db_path)
