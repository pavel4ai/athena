...
def test_update_notice_offers_v1_trigram_tool_calls_rebuild(tmp_path, monkeypatch, capsys):
    ...
    class FakeSessionDB:
        ...
        _db_needs_fts_storage_upgrade = staticmethod(
            athena_state.SessionDB._db_needs_fts_storage_upgrade
        )

    monkeypatch.setattr(athena_constants, "get_athena_home", lambda: tmp_path)
    monkeypatch.setattr(athena_state, "SessionDB", FakeSessionDB)
    ...
    assert "athena sessions optimize-storage" in capsys.readouterr().out
    conn.close()
