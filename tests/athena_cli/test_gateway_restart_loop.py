...
Covers:
- Defense 1: gateway stop/restart refuse when _ATHENA_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.athena.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.athena/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l athena-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.athena.gateway.restart-once.plist",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.athena.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.athena/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            ...
            "'ai.athena.gateway-cronus:/c.plist' 'ai.athena.gateway-plutus:/p.plist' "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated athena note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.athena.gateway` executes exactly as
        ...
        "launchctl dis\\able gui/501/ai.athena.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.athena."gateway"',
        # Same class on the systemctl and athena-CLI branches.
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.athena.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'systemctl re"start" athena-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `athena gateway start` is benign — starting a gateway from inside a
        ...
        "athena gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway athena
        # services must not be falsely blocked (the old `.*athena` matched any
        # athena token).
        "launchctl unload ai.athena.update-checker.plist",
        "launchctl restart ai.athena.daemon",
        ...
        "systemctl restart athena-meta.service",
        "systemctl restart athena-cron-helper",
        ...
        "echo after the athena gateway restarted cleanly",
        "the athena gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "athena skill view gateway-notes && echo athena gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `athena` is a path component
        # there, not a command.
        "cat '/docs/athena gateway restart-notes.md'",
        "less /home/user/notes/athena gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "athena gateway restart",
        "athena gateway restart; echo done",
        "athena gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;athena gateway restart",
        "true && athena gateway stop",
        "echo $(athena gateway restart)",
        "echo `athena gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.athena.gateway"])',
        'subprocess.run(["athena", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.athena.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["athena", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: athena gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `athena -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry ATHENA_HOME/ATHENA_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("ATHENA_PROFILE", "zeus")
        monkeypatch.delenv("ATHENA_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "athena -p zeus gateway stop",
        "athena -p zeus gateway restart",
        "athena --profile zeus gateway restart",
        "athena --profile zeus gateway stop",
        "athena --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "athena -v -p zeus gateway restart",
        "athena -p zeus -v gateway restart",
        "athena --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "athena -p venus gateway stop",
        "athena -p venus gateway restart",
        "athena --profile venus gateway restart",
        "athena --profile=venus gateway stop",
        "athena -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "athena -p zeus gateway start",
        "athena -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("athena gateway restart")
        assert _contains_gateway_lifecycle_command("athena gateway stop")

    def test_athena_home_derived_profile(self, monkeypatch):
        # Without ATHENA_PROFILE the guard falls back to the ATHENA_HOME-
        ...
        assert _contains_gateway_lifecycle_command("athena -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("athena -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("athena -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("athena gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_athena_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade athena then run athena gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_athena_gateway_once.sh). The script must live under
        # ATHENA_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("ATHENA_HOME", str(tmp_path / ".athena"))
        scripts_dir = tmp_path / ".athena" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_ATHENA_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _ATHENA_GATEWAY=1.

    Issue #37453: systemctl --user restart athena-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        ...
        "systemctl stop athena-gateway.service",
        "athena gateway restart",
        "athena gateway uninstall",
        "launchctl kickstart gui/501/ai.athena.gateway",
        "launchctl bootout gui/501/ai.athena.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.athena.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.athena/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.athena.gateway.restart-once.plist",
        "pkill -f athena.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart athena-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.athena.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-athena label: label-independent detection is the point
        # (#62891 second reproduction used `ai.athena.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.athena.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _ATHENA_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _ATHENA_GATEWAY=1 is in the
        ...
        self._patch_env(monkeypatch, _FakeEnv(), inside_gateway=False)
        monkeypatch.setenv("_ATHENA_GATEWAY", "1")
        monkeypatch.setattr(
            tt, "_check_all_guards", lambda cmd, env, **kwargs: {"approved": True}
        )

        result = json.loads(tt.terminal_tool(command="athena gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["athena gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.athena.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.athena.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-athena systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run athena gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("athena gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfeathena gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under ATHENA_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("athena gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/athena\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 athena gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_athena_home()` falls back to Path.home(), which raises
        RuntimeError when neither ATHENA_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart athena-gateway' .",
        "grep -rn 'athena gateway restart' ./logs",
        "rg 'athena gateway restart' ../archive",
        "grep -c 'systemctl stop athena-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM athena_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell athena gateway restart"',
        'sqlite3 ./db ".system systemctl restart athena-gateway"',
        'psql ./x -c "\\! systemctl restart athena-gateway"',
        "grep -r 'athena gateway restart' . | sh",
        "grep -r 'athena gateway restart' ./logs | bash",
        "grep -r 'athena gateway restart' . | sudo sh",
        "grep -r 'x' . ; athena gateway restart",
        "grep -r 'x' . && systemctl restart athena-gateway",
        'grep -r "$(athena gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart athena-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell athena gateway restart"',
        'sqlite3 db ".system systemctl restart athena-gateway"',
        'psql -c "\\! systemctl restart athena-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    athena_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run athena gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        result = json.loads(cronjob(
            ...
            prompt="please run athena gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "grep -c 'systemctl restart athena-gateway' /var/log/syslog",
        "rg 'athena gateway restart' /home/user/.athena/logs/",
        "journalctl -u athena-gateway --grep 'systemctl restart athena-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'athena_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell athena gateway restart"',
        'psql -c "\\! systemctl restart athena-gateway"',
        "grep 'systemctl restart athena-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart athena-gateway",
        'grep "$(systemctl restart athena-gateway)" f',
        "grep 'restart' log; systemctl restart athena-gateway",
        'sqlite3 db "SELECT 1"; athena gateway stop',
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart athena-gateway%'\""
        )
