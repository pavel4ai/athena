...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "Athena Gateway Restart".lower(),   # case handled
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.athena.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
...
class TestGatewaySelfTargetingGuard:
    """Verify athena gateway stop/restart refuse when _ATHENA_GATEWAY=1."""
...
class TestTerminalToolGatewayLifecycleGuard:
    ...
    def test_guard_inactive_outside_gateway(self, monkeypatch):
        """Without _ATHENA_GATEWAY=1 the lifecycle guard must not fire."""
        import tools.terminal_tool as tt
        ...
        result = json.loads(tt.terminal_tool(command="systemctl restart athena-gateway"))

        # Outside the gateway the lifecycle guard doesn't block — the normal
        # approval flow handles it (here mocked as approved).
        assert result["exit_code"] == 0
        assert calls == ["systemctl restart athena-gateway"]
