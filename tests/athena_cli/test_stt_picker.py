"""Tests for the Speech-to-Text category in `athena tools` (tools_config).
...
from athena_cli.tools_config import (  # noqa: E402
...
class TestSttCategory:
    ...
    def test_managed_row_shares_tts_coverage_category(self):
        from athena_cli.nous_subscription import MANAGED_FEATURE_COVERAGE_CATEGORY
...
class TestModelPicker:
    ...
    def test_configure_stt_model_defaults_to_current(self):
        ...
        with patch(
            "athena_cli.tools_config._prompt_choice", return_value=0
        ) as pc:
...
class TestPostSetup:
    def test_faster_whisper_in_post_setup_ready(self):
        from athena_cli.tools_config import _POST_SETUP_READY
