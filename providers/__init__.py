...
2. User plugins: ``$ATHENA_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``athena_agent.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$ATHENA_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$ATHENA_HOME/plugins/model-providers/`` if it exists."""
    try:
        from athena_constants import get_athena_home

        d = get_athena_home() / "plugins" / "model-providers"
...
def _installed_plugins_dir() -> Path | None:
    """Return ``$ATHENA_HOME/plugins/`` if it exists.

    This is where ``athena plugins install`` clones a plugin — flat, one
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple ATHENA_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_athena_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``athena_agent.plugins``
    ...
    * **Provider targets only.** The ``athena_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$ATHENA_HOME``) keep their
    ...
    try:
        from athena_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$ATHENA_HOME/plugins/model-providers/<name>/``
      2b. Plugins installed by ``athena plugins install`` at
          ``$ATHENA_HOME/plugins/<name>/`` that declare ``kind: model-provider``
    ...
    # 0. Pip-installed plugins — entry points in the ``athena_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$ATHENA_HOME`` profile of the
    ...
    # 2. User plugins — under $ATHENA_HOME/plugins/model-providers/<name>/.
    ...
    # 2b. Plugins installed by ``athena plugins install`` / the plugin index.
    #     Those clone into $ATHENA_HOME/plugins/<name>/ — flat, NOT under
