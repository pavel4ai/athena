"""Windows UTF-8 bootstrap for Athena entry points (no-op on POSIX).
...
point (``athena``, ``athena-agent``, ``athena-acp``, ``gateway.run``, ``batch_runner``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``athena_cli._subprocess_compat.suppress_platform_ver_console`` for callers
    that never import ``athena_cli.main``; double application is harmless.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Athena modules.

    Athena ships top-level modules with common names (``utils``, ``proxy``, ``ui``); a
    ...
    dropped and the Athena root is *relocated* to the front, not merely inserted when
    ...
    """
    root = src_root or os.environ.get("ATHENA_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    """Put the durable lazy-install dir (``ATHENA_LAZY_INSTALL_TARGET``) on ``sys.path``.
    ...
    """
    if not os.environ.get("ATHENA_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points only need ``import athena_bootstrap`` first.
