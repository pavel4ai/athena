...
> **Heads up:** Native Windows runs Athena without WSL — CLI, gateway, TUI, and tools all work natively. If you'd rather use WSL2, the Linux/macOS one-liner above works there too. Found a bug? Please [file issues](https://github.com/pavel4ai/athena/issues).
...
If your antivirus (Bitdefender, Windows Defender, etc.) quarantines `uv.exe` from the Athena `bin` folder (`%LOCALAPPDATA%\athena\bin\uv.exe`), this is a **false positive**. The file is Astral's `uv` — the Rust Python package manager Athena bundles to manage its Python environment. ML-based antivirus engines commonly flag unsigned Rust binaries that download and install packages.
...
# Run verification
$uv = "$env:LOCALAPPDATA\athena\bin\uv.exe"
...
**To whitelist Athena:**
- **Windows Defender:** Run PowerShell as Admin → `Add-MpPreference -ExclusionPath "$env:LOCALAPPDATA\athena\bin"`
- **Bitdefender:** Add an exception in the Bitdefender console (Protection > Antivirus > Settings > Manage Exceptions)
- Whitelist the **folder**, not the file hash — Athena updates `uv` and the hash changes every version
...
```bash
athena              # Interactive CLI — start a conversation
...
athena config get   # Print individual config values
...
| ------------------------------ | --------------------------------------------- | -------------------------------------------------------------------------------- |
| Start chatting                 | `athena`                                      | Run `athena gateway setup` + `athena gateway start`, then send the bot a message |
...
**During first-time setup:** The setup wizard (`athena setup`) automatically detects `~/.openclaw` and offers to migrate before configuration begins.

**Anytime after install:**

```bash
athena claw migrate              # Interactive migration (full preset)
...
- **Skills** — user-created skills → `~/.athena/skills/openclaw-imports/`
...
See `athena claw migrate --help` for all options, or use the `openclaw-migration` skill for an interactive agent-guided migration with dry-run previews.
...
`~/.athena/athena-agent`). This matches the layout used by `athena update`, the
...
source ~/.athena/venvs/athena-dev/bin/activate
uv pip install -e ".[all,dev]"
scripts/run_tests.sh
```
...
- 🔌 [AthenaClaw](https://github.com/AaronWong1999/athenaclaw) — Community WeChat bridge: Run Athena Agent and OpenClaw on the same WeChat account.
