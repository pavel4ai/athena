...
# IMPORTANT: athena_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See athena_bootstrap.py for full rationale.
try:
    import athena_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Graceful fallback when athena_bootstrap isn't registered in the venv
...
os.environ["ATHENA_QUIET"] = "1"  # Our own modules
...
from athena_cli.banner import _format_context_length, format_banner_version_label
...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    ...
    Relative paths are resolved from ~/.athena/.
...
def _resolve_prefill_messages_file(config: Dict[str, Any]) -> str:
    ...
    """
    env_path = os.getenv("ATHENA_PREFILL_MESSAGES_FILE", "").strip()
    if env_path:
        return env_path
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.athena/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If ATHENA_IGNORE_USER_CONFIG=1 is set (via ``athena chat --ignore-user-config``),
    the user config at ``~/.athena/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({ATHENA_HOME}/config.yaml)
    user_config_path = _athena_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("ATHENA_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see athena_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
    ...
    # athena_cli.config._load_config_impl (which has its own managed merge), so
    ...
    # `athena config`/`doctor`/guards (which use load_config) honor it. The
    ...
    # Normalize config key: the new config system (athena_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && athena` to control it.
    ...
    # UNLESS we're inside a gateway process (detected by _ATHENA_GATEWAY marker)
...
# Initialize centralized logging early — agent.log + errors.log in ~/.athena/logs/.
...
try:
    from athena_cli.skin_engine import init_skin_from_config
...
from athena_cli.banner import build_welcome_banner
from athena_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from athena_cli.callbacks import prompt_for_secret
...
def _prepare_deferred_agent_startup() -> None:
    ...
    _accept_hooks = os.environ.get("ATHENA_ACCEPT_HOOKS", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _notify_session_finalize(
    *,
    ...
    try:
        from athena_cli.plugins import invoke_hook as _invoke_hook
        _invoke_hook(
            "on_session_finalize",
...
def _resolve_worktree_base(repo_root: str) -> tuple:
    ...
    (the ``~/.athena/athena-agent`` clone is updated only by ``athena update``,
...
def _setup_worktree(repo_root: str = None, sync_base: bool = True) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run athena -w")
        return None
    ...
    try:
        subprocess.run(
            ["git", "worktree", "lock", "--reason", f"athena pid={os.getpid()}", str(wt_path)],
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`athena_cli.config.load_config` (the authoritative loader that
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`athena_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``athena/*`` and ``pr-*`` local branches that
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``athena/athena-*`` and ``pr-*`` branches with no worktree.
...
#
# Detection priority:
#   1. ATHENA_LIGHT / ATHENA_TUI_LIGHT env (true/false) — explicit override
#   2. ATHENA_TUI_THEME=light|dark — explicit theme
#   3. ATHENA_TUI_BACKGROUND=#RRGGBB — explicit bg hint
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("ATHENA_TUI_BACKGROUND") or ""
...
def _apply_bracketed_paste_timeout_patch() -> None:
    ...
    ``_athena_bp_timeout_patched`` sentinel on the module.
    """
    try:
...
# ASCII Art - ATHENA AGENT logo (full width, single line - requires ~95 char terminal)
ATHENA_AGENT_LOGO = """[bold #FFD700] █████╗ ████████╗██╗  ██╗███████╗███╗   ██╗ █████╗        █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #FFD700]██╔══██╗╚══██╔══╝██║  ██║██╔════╝████╗  ██║██╔══██╗      ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#FFBF00]███████║   ██║   ███████║█████╗  ██╔██╗ ██║███████║      ███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║[/]
[#FFBF00]██╔══██║   ██║   ██╔══██║██╔══╝  ██║╚██╗██║██╔══██║      ██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║[/]
[#CD7F32]██║  ██║   ██║   ██║  ██║███████╗██║ ╚████║██║  ██║      ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║[/]
...
# ASCII Art - Athena profile (compact, fits in left panel)
ATHENA_CADUCEUS = """[#CD7F32]⠀⠀⠀⠀⠀⠀⠀⠀⠀⣠⣴⣶⣶⣶⣶⣦⣄⠀⠀⠀⠀⠀⠀⠀⠀[/]
[#CD7F32]⠀⠀⠀⠀⠀⠀⠀⢀⣾⣿⣿⠿⠛⠛⠿⣿⣿⣷⡀⠀⠀⠀⠀⠀⠀[/]
[#FFBF00]⠀⠀⠀⠀⠀⠀⢠⣿⡿⠋⠀⣠⣴⣶⣦⡈⠻⣿⣿⡄⠀⠀⠀⠀⠀[/]
[#FFBF00]⠀⠀⠀⠀⠀⠀⣿⣿⠁⠀⢰⣿⣿⣿⣿⣿⠀⠈⣿⣿⠀⠀⠀⠀⠀[/]
[#FFD700]⠀⠀⠀⠀⠀⢰⣿⡇⠀⠀⠈⠻⣿⣿⠟⠁⠀⠀⢸⣿⡆⠀⠀⠀⠀[/]
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "⚕ NOUS ATHENA - AI Agent Framework"
    ...
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- Futurebound Corp.[/]\n"
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.athena/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.athena/config.yaml on first use)
...
class AthenaCLI(CLIAgentSetupMixin, CLICommandsMixin):
    ...
    def __init__(
        ...
        # Read max_tokens from config (env var override: ATHENA_MAX_TOKENS)
        _env_mt = os.environ.get("ATHENA_MAX_TOKENS")
        if _env_mt:
            try:
        ...
        self.requested_provider = (
            ...
            or os.getenv("ATHENA_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("ATHENA_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("ATHENA_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        # by `athena chat --ignore-rules` in athena_cli/main.py. When true we
        ...
        self.system_prompt = (
            os.getenv("ATHENA_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        # it's shared across all Athena processes for this ATHENA_HOME.
        # Never blocks startup on failure.
        _run_state_db_auto_maintenance(self._session_db)

        # Opportunistic shadow-repo cleanup — deletes orphan/stale
        # checkpoint repos under ~/.athena/checkpoints/.  Opt-in via
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from athena_cli.voice import format_voice_record_key_for_status
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        if resolved_provider == "copilot":
            try:
                from athena_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from athena_cli.models import normalize_opencode_model_id, opencode_model_api_mode
        ...
        if self._model_is_default:
            ...
            try:
                from athena_cli.codex_models import get_codex_model_ids
    ...
    def _ensure_tirith_security(self) -> None:
        ...
        try:
            ...
            if (
                ...
                and not is_install_in_progress()
            ):
    ...
    def _show_security_advisories(self):
        ...
        """
        try:
            from athena_cli.security_advisories import (
                detect_compromised,
                startup_banner,
            )
    ...
    def show_banner(self):
        ...
        from athena_cli.model_switch import is_nous_athena_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_athena_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Futurebound Corp. Athena 3 & 4 models are NOT agentic and are not "
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.athena/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from athena_cli.clipboard import save_clipboard_image
    ...
    def show_help(self):
        ...
        try:
            from athena_cli.skin_engine import get_active_help_header
    ...
    def _list_recent_sessions(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        if not self._session_db:
            return []
        try:
            from athena_cli.session_listing import query_session_listing
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from athena_cli.main import _relative_time
    ...
    def show_history(self):
        ...
        for msg in self.conversation_history:
            ...
            print(f"\n  [Athena #{visible_index}]{_ts_suffix(msg)}")
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("ATHENA_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.athena/sessions/saved/.
        ...
        try:
            ...
            return
        path = saved_dir / f"athena_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: athena --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from athena_cli.curses_ui import curses_single_select
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        try:
            from athena_cli.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            if not model_list:
                try:
                    from athena_cli.models import provider_model_ids
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_athena_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_athena_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in athena_cli/commands.py instead of touching every dispatch site.
        ...
        if canonical in {"quit", "exit"}:
            ...
            from athena_cli.main import _print_version_info
            ...
            from athena_cli.config import reload_env
            count = reload_env()
            ...
            try:
                ...
                if not user_entries:
                    ...
                    print("  Install one: athena plugins install owner/repo")
                    print(f"  Or drop a plugin directory into {display_athena_home()}/plugins/")
                    if bundled_count:
                        print(f"  ({bundled_count} bundled plugins available — see: athena plugins list)")
                else:
                    ...
                    if bundled_count:
                        print(f"  (+{bundled_count} bundled — see: athena plugins list)")
                    print("  Enable/disable: athena plugins enable/disable <name>")
            except Exception as e:
            ...
            # Check for plugin-registered slash commands
                ...
                from athena_cli.plugins import (
    ...
    def _maybe_continue_goal_after_turn(self) -> None:
        ...
        try:
            from athena_cli.goals import gather_background_processes as _gather_bg
    ...
    def _toggle_verbose(self):
        ...
        from athena_cli.colors import Colors as _Colors
        labels = {
    ...
    def _is_session_yolo_active(self) -> bool:
        ...
        ``ATHENA_YOLO_MODE`` into ``_YOLO_MODE_FROZEN`` before tool imports
        happen.
        """
        try:
            from tools.approval import (
    ...
    def _toggle_yolo(self):
        ...
        ``ATHENA_YOLO_MODE`` here — that env var is read once at module import
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by athena_logging not
    ...
    def _print_nous_credits_block(self) -> bool:
        ...
        wall-clock-bounded inside the helper; also honors ATHENA_DEV_CREDITS_FIXTURE
    ...
    def _show_billing(self, command: str = "/billing"):
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Nous Portal.')}")
                print("  Run `athena portal` to log in, then /billing.")
            return
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        print(f"  {'─' * 41}")
        _consent = (
            "By confirming, you allow Futurebound Corp. to charge your card."
        )
        ...
        from athena_cli.nous_billing import (
            BillingError,
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        from athena_cli.nous_billing import BillingRateLimited
    ...
    def _billing_handle_scope_required(self, state):
        ...
        if not getattr(self, "_app", None):
            print("  Run `athena portal` and approve terminal billing, then retry.")
            return
        confirm_choices = [
    ...
    def _billing_auto_reload_flow(self, state):
        ...
        print()
        _ar_consent = (
            f"By confirming, you authorize Futurebound Corp. to charge {card.masked} "
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from athena_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.athena/skills/ and queue a note for the
        next user turn.
                ...
                try:
                    if (
                        ...
                        if not is_seen(CLI_CONFIG, TOOL_PROGRESS_FLAG):
                            ...
                            mark_seen(_athena_home / "config.yaml", TOOL_PROGRESS_FLAG)
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "athena_voice"), exist_ok=True)
            mp3_path = os.path.join(
                tempfile.gettempdir(), "athena_voice",
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _athena_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard ATHENA_HOME, so no -p needed.
            try:
            ...
            print(f"  athena --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  athena -c \"{session_title}\"{profile_flag}")
            print()
            ...
            try:
                from athena_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from athena_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from athena_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            from athena_cli.model_switch import prewarm_picker_cache_async
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in athena_cli.pt_input_extras
        ...
        def handle_enter(event):
            ...
            if self._model_picker_state:
                try:
                    ...
                    from athena_cli.model_switch import resolve_persist_behavior
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``ATHENA_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("ATHENA_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the athena process when
...
def _run_kanban_goal_loop_q(cli: "AthenaCLI", first_response: str) -> None:
    ...
    only when ``ATHENA_KANBAN_GOAL_MODE`` is set (dispatcher-spawned
    ...
    task_id = (_os.environ.get("ATHENA_KANBAN_TASK") or "").strip()
    if not task_id:
        return
...
def main(
    ...
    os.environ["ATHENA_INTERACTIVE"] = "1"
    ...
    # Default to athena-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
    ...
    # so main unwinds normally.  ATHENA_SIGTERM_GRACE overrides the 1.5 s
                ...
                if cli._ensure_runtime_credentials():
                    ...
                    if cli._init_agent(
                        ...
                        if isinstance(result, dict) and result.get("failed"):
                            _exit_code = 1
                            if os.environ.get("ATHENA_KANBAN_TASK") and result.get(
                                "failure_reason"
                            ) in ("rate_limit", "billing"):
                                try:
                                    from athena_cli.kanban_db import (
