...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/pavel4ai/athena/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    athena_meta = (fm.get("metadata") or {}).get("athena") or {}
    tags = athena_meta.get("tags") or []
    related = athena_meta.get("related_skills") or []
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        "",
        "Athena ships with a large built-in skill library copied into `~/.athena/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Athena also syncs bundled skills on `athena update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.athena/skills/` tree, it is still shipped with Athena; restore it with `athena skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        "",
        "```bash",
        "athena skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
