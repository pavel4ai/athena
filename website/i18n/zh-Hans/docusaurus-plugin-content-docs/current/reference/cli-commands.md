...
| `athena update` | 拉取最新代码并重新安装依赖（git 安装），或检查 PyPI 并执行 `pip install --upgrade`（pip 安装）。`--check` 预览而不安装；`--backup` 在拉取前对 `ATHENA_HOME` 进行快照。 |
...
athena skills install https://sharethis.chat/SKILL.md                     # 直接 URL（单文件 SKILL.md）
...
参见 [Plugins](../user-guide/features/plugins.md) 和 [构建 Athena Plugin](../guides/build-a-athena-plugin.md)。
...
**pip 安装：** `athena update` 自动检测基于 pip 的安装——查询 PyPI 获取最新版本并运行 `pip install --upgrade athena-agent`，而非 `git pull`。PyPI 发布跟踪标记版本（主要/次要版本），而非 `main` 上的每个 commit。使用 `--check` 查看是否有更新的 PyPI 版本可用，而不安装。
