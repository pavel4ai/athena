...
Athena 附带一个大型内置技能库，安装时会复制到 `~/.athena/skills/`。下方每个技能均链接至专属页面，包含完整定义、配置和用法说明。

Athena 在执行 `athena update` 时也会同步内置技能，但同步清单会尊重本地删除和用户编辑。如果此处列出的某个技能在你的 `~/.athena/skills/` 目录树中缺失，它仍随 Athena 一同发布；可通过 `athena skills reset <name> --restore` 恢复。
...
| [`plan`](/user-guide/skills/bundled/software-development/software-development-plan) | 计划模式：将 Markdown 计划写入 `.athena/plans/`，不执行。 | `software-development/plan` |
