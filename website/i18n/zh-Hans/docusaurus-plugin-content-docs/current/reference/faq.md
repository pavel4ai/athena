...
- **Nous Portal** — Futurebound Corp. 自有推理端点
- **OpenAI** — GPT-5.4、GPT-5-codex、GPT-4.1、GPT-4o 等
- **Anthropic** — Claude 模型（直接 API、通过 `athena auth add anthropic` 进行 OAuth、OpenRouter 或任何兼容代理）
...
使用 `athena model` 设置提供商，或直接编辑 `~/.athena/.env`。所有提供商 key 请参阅[环境变量](./environment-variables.md)参考文档。
...
- [在 Athena 中使用 MCP](../guides/use-mcp-with-athena.md#wsl2-bridge-athena-in-wsl-to-windows-chrome)
...
API 调用**仅发送至您配置的 LLM 提供商**（例如 OpenRouter、您本地的 Ollama 实例）。Athena Agent 不收集遥测数据、使用数据或分析数据。您的对话、记忆和技能均存储在本地 `~/.athena/` 目录中。

### 可以离线使用 / 使用本地模型吗？

可以。运行 `athena model`，选择**自定义端点**，然后输入您服务器的 URL：
...
:::tip 本地模型超时问题
Athena 会自动检测本地端点并放宽流式传输超时（读取超时从 120s 提升至 1800s，禁用停滞流检测）。如果在非常大的上下文下仍然超时，请在 `.env` 中设置 `ATHENA_STREAM_READ_TIMEOUT=1800`。详情请参阅[本地 LLM 指南](../guides/local-llm-on-mac.md#timeouts)。
:::
...
#### 安装后出现 `athena: command not found`
...
**解决方案：** Athena 默认自动 source `~/.bashrc`。如果这还不够 — 例如您是 zsh 用户，PATH 在 `~/.zshrc` 中，或者您从独立文件初始化 `nvm` — 请在 `~/.athena/config.yaml` 中列出需要额外 source 的文件：
...
**解决方案：** 退出当前会话，在终端中使用 `athena model` 添加新提供商：
...
通过 `athena model` 添加新提供商后，启动新的聊天会话 — `/model` 将显示所有已配置的提供商。

:::tip 快速参考
| 目标 | 使用方式 |
|-----------|-----|
| 添加新提供商 | `athena model`（从终端） |
| 输入/更改 API key | `athena model`（从终端） |
...
:::warning
请确保 key 与提供商匹配。OpenAI 的 key 无法用于 OpenRouter，反之亦然。检查 `~/.athena/.env` 中是否有冲突条目。
:::
...
- 升级您的提供商套餐
- 切换到其他模型或提供商
- 使用 `athena chat --provider <alternative>` 路由到其他后端
...
**解决方案：**
- 使用 `athena gateway setup` 验证您的 bot token 是否有效
- 检查网关日志：`cat ~/.athena/logs/gateway.log | tail -50`
...
在 `~/.athena/config.yaml` 中您的网关设置下进行配置。请参阅[消息文档](../user-guide/messaging/index.md)。
...
#### WSL：网关持续断开连接或 `athena gateway start` 失败
...
1. 创建一个任务，运行 `wsl -d Ubuntu -- bash -lc 'athena gateway run'`
2. 设置在用户登录时触发
:::
...
**解决方案：** 网关在您运行 `athena gateway install` 时会捕获您的 shell PATH。如果您在设置网关后安装了新工具，请重新运行 install 以捕获更新后的 PATH：

```bash
athena gateway install    # 重新快照当前 PATH
athena gateway start      # 检测到更新的 plist 并重新加载
```
...
**解决方案：**
- 尝试更快/更小的模型：`athena chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- 减少激活的工具集：`athena chat -t "terminal"`
- 检查到提供商的网络延迟
...
验证您的 `~/.athena/config.yaml` 中的 MCP 配置：
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
- [在 Athena 中使用 MCP](/guides/use-mcp-with-athena)
...
### Profiles 与直接设置 ATHENA_HOME 有何不同？

Profiles 是构建在 `ATHENA_HOME` 之上的托管层。您*可以*在每次命令前手动设置 `ATHENA_HOME=/some/path`，但 profiles 会为您处理所有底层工作：创建目录结构、生成 shell 别名（`athena-work`）、在 `~/.athena/active_profile` 中跟踪活动 profile，以及自动跨所有 profiles 同步技能更新。它们还与 tab 补全集成，让您无需记忆路径。
...
不共享。每个 profile 都有自己独立的记忆存储、会话数据库和技能目录，完全隔离。如果您想用现有的记忆和会话创建新 profile，请使用 `athena profile create newname --clone-all` 从当前 profile 复制所有内容，或添加 `--clone-from <profile>` 从指定源 profile 复制。

### 运行 `athena update` 时会发生什么？

`athena update` 拉取最新代码并重新安装依赖项**一次**（不是每个 profile 各一次）。然后自动将更新的技能同步到所有 profiles。您只需运行一次 `athena update` — 它覆盖机器上的每个 profile。

### 可以运行多少个 profiles？

没有硬性限制。每个 profile 只是 `~/.athena/profiles/` 下的一个目录。实际限制取决于您的磁盘空间以及系统能处理多少个并发网关（每个网关是一个轻量级 Python 进程）。运行数十个 profiles 完全没问题；每个空闲的 profile 不占用任何资源。
...
**解决方案：委托配置。** Athena 可以自动将子智能体路由到不同的模型。在 `~/.athena/config.yaml` 中设置：
...
**场景：** Telegram 有 100 个 slash 命令的限制，您的技能数量已超过此限制。您想禁用 Telegram 上不需要的技能，但 `athena skills config` 设置似乎没有生效。

**解决方案：** 使用 `athena skills config` 按平台禁用技能。这会写入 `config.yaml`：
...
更改后，**重启网关**（`athena gateway restart` 或终止并重新启动）。Telegram bot 命令菜单在启动时重建。
...
2. 在**源机器**上创建完整备份：
   ...
   athena backup
   ```
   这会将您整个 `~/.athena/` 目录（配置、API key、记忆、技能、会话和 profiles）打包为 zip 文件，保存到主目录 `~/athena-backup-<timestamp>.zip`。
...
4. 在新机器上运行 `athena setup` 以验证 API key 和提供商配置是否正常工作。
...
### `athena backup` 与 `athena profile export` 的对比

| 功能 | `athena backup` | `athena profile export` |
| :--- | :--- | :--- |
| **使用场景** | **整机迁移** | **移植/共享特定 profile** |
| **范围** | 全局（整个 `~/.athena` 目录） | 局部（单个 profile 目录） |
...
:::tip
`athena backup` 即使在 Athena 正在运行时也能生成一致的快照。还原的归档文件不包含机器本地的运行时文件，如 `gateway.pid` 和 `cron.pid`。
:::
...
1. **搜索现有 issue：** [GitHub Issues](https://github.com/pavel4ai/athena/issues)
2. **向社区提问：** [Futurebound Corp. Discord](https://discord.gg/nousresearch)
3. **提交 bug 报告：** 请包含您的操作系统、Python 版本（`python3 --version`）、Athena 版本（`athena --version`）以及完整的错误信息