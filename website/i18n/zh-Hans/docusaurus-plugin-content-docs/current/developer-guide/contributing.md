...
- 构建自定义/本地工具而不修改 Athena 核心？从 [构建 Athena 插件](../guides/build-a-athena-plugin.md) 开始
...
对大多数贡献者来说，最好的开发启动方式和用户安装方式相同：运行标准安装器，然后在它克隆出的仓库里开发。安装器会创建 Athena venv、配置 `athena` 命令、为 `athena update` 写入安装方式标记，并把完整 git 项目克隆到 `$ATHENA_HOME/athena-agent`（通常是 `~/.athena/athena-agent`）。这样你的开发环境会和 CLI、updater、lazy dependency installer、gateway、docs 默认假设的布局一致。
...
只有在你明确不想使用 Athena managed install layout 时才使用这种方式（例如容器或 CI job 里的临时 clone）。如果这样安装，请确保运行的是这个 venv 里的 `athena` entrypoint；运行系统 `python3 -m athena_cli.main` 可能会加载无关的系统 Python 包。
...
```bash
# 标准安装器已经把 `athena` 放到了 PATH 上。
athena doctor
athena chat -q "Hello"
```

如果你使用了手动克隆备用路径，可以在 checkout 中运行 `./athena`，或显式把这个 clone 的 venv 链接到 PATH：
...
- **Profile 安全路径**：不得硬编码 `~/.athena` — 代码路径使用 `athena_constants` 中的 `get_athena_home()`，面向用户的消息使用 `display_athena_home()`。完整规则参见 [AGENTS.md](https://github.com/pavel4ai/athena/blob/main/AGENTS.md#profiles-multi-instance-support)。
...
- 使用 [GitHub Issues](https://github.com/pavel4ai/athena/issues)
- 请包含：操作系统、Python 版本、Athena 版本（`athena version`）、完整错误堆栈
- 包含复现步骤
- 创建前请检查是否已有重复 issue
- 安全漏洞请私下报告
...
- **Discord**：[discord.gg/FutureboundCorp](https://futurebnd.com)
- **GitHub Discussions**：用于设计提案和架构讨论
- **Skills Hub**：上传专业 skill 并与社区共享

## 许可证

提交贡献即表示您同意您的贡献将以 [MIT 许可证](https://github.com/pavel4ai/athena/blob/main/LICENSE) 授权。