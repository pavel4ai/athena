...
- [构建 Athena 插件](/guides/build-a-athena-plugin)
...
2. **`toolsets.py`** — 将工具名称添加到 `_ATHENA_CORE_TOOLS`（或特定 toolset）
...
如果你的工具需要 API 密钥，将其添加到 `athena_cli/config.py`：
...
- [ ] 可选：已将 API 密钥添加到 `athena_cli/config.py` 的 `OPTIONAL_ENV_VARS`
- [ ] 可选：已添加到 `toolset_distributions.py` 以支持批量处理
- [ ] 已通过 `athena chat -q "Use the weather tool for London"` 测试