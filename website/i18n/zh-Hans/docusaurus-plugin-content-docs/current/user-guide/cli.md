...
:::tip
Athena 还提供了一个现代 TUI，支持模态覆盖层、鼠标选择和非阻塞输入。使用 `athena --tui` 启动——参见 [TUI](tui.md) 指南。
:::
...
# 使用指定提供商
athena chat --provider nous        # 使用 Nous Portal
athena chat --provider openrouter  # 强制使用 OpenRouter
...
# 恢复之前的会话
athena --continue             # 恢复最近的 CLI 会话（-c）
athena --resume <session_id>  # 通过 ID 恢复指定会话（-r）

# 详细模式（调试输出）
athena chat --verbose

# 隔离的 git worktree（用于并行运行多个 agent）
athena -w                         # 在 worktree 中以交互模式运行
athena -w -z "Fix issue #123"     # 在 worktree 中以单次查询模式运行
```
...
| 时长 | 会话已用时间 |
| ⚠ YOLO | **YOLO 模式警告**——当 `ATHENA_YOLO_MODE` 开启时显示（通过启动时的 `athena --yolo` 或会话中的 `/yolo` 切换）。与横幅行警告保持同步，确保你不会忘记自己处于自动批准模式。 |
...
恢复之前的会话时（`athena -c` 或 `athena --resume <id>`），横幅与输入提示符之间会出现一个"Previous Conversation"面板，显示对话历史的简洁摘要。详情及配置说明参见[会话——恢复时的对话摘要](sessions.md#conversation-recap-on-resume)。
...
`~/.athena/skills/` 中每个已安装的 skill 都会自动注册为斜杠命令。skill 名称即为命令名：
...
你也可以在 `~/.athena/config.yaml` 中定义自定义人格：
...
```bash
athena --continue                          # 恢复最近的 CLI 会话
athena -c                                  # 简写形式
athena -c "my project"                     # 恢复命名会话（谱系中最新的）
athena --resume 20260225_143052_a1b2c3     # 通过 ID 恢复指定会话
athena --resume "refactoring auth"         # 通过标题恢复
athena -r 20260225_143052_a1b2c3           # 简写形式
```

恢复会从 SQLite 中还原完整的对话历史。agent 能看到所有之前的消息、工具调用和响应——就像从未离开一样。

在聊天中使用 `/title My Session Name` 为当前会话命名，或从命令行使用 `athena sessions rename <id> <title>`。使用 `athena sessions list` 浏览历史会话。

### 会话存储

CLI 会话存储在 Athena 的 SQLite 状态数据库 `~/.athena/state.db` 中。数据库保存：
