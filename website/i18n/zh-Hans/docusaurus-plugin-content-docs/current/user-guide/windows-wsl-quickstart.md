...
- 你的 Athena 安装目录（`~/.athena/`）—— Linux 侧。安装程序已自动处理。
...
在 WSL 内 clone。始终如此，除非有特殊原因。典型的 Athena 工作流（`athena chat`、调用 `rg`/`ripgrep` 搜索仓库的工具、文件监听器、后台 gateway）在 `~/code/myrepo` 下会比在 `/mnt/c/Users/you/myrepo` 下快得多，也更可靠。

一个例外：**启动 Windows 二进制文件的 MCP bridge。** 如果你通过 `cmd.exe` 使用 `chrome-devtools-mcp`（参见 [MCP 指南：WSL → Windows Chrome](/guides/use-mcp-with-athena#wsl2-bridge-athena-in-wsl-to-windows-chrome)），当 Athena 的当前工作目录是 `~` 时，Windows 可能会报 `UNC` 警告。此时请从 `/mnt/c/` 下的某个目录启动 Athena，以便 Windows 进程拥有一个带盘符的工作目录。
...
- 从 Windows 侧工具使用 **OpenAI 兼容 API 服务器**（当 `API_SERVER_ENABLED=true` 时由 `athena gateway` 暴露）。参见 [API Server 功能页](/user-guide/features/api-server)。
...
如果你按照上面的安装步骤启用了 systemd，`athena gateway` 和 API 服务器的使用方式与任何 Linux 机器上完全相同。使用 gateway 设置向导：
...
- **[MCP 指南 → WSL → Windows Chrome](/guides/use-mcp-with-athena#wsl2-bridge-athena-in-wsl-to-windows-chrome)** —— 从 WSL 中的 Athena 控制你已登录的 Windows Chrome。
