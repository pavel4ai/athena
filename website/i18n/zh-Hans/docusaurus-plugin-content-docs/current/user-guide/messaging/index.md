...
完整的语音功能集——包括 CLI 麦克风模式、消息中的语音回复以及 Discord 语音频道对话——请参阅 [Voice Mode](/user-guide/features/voice-mode) 和 [Use Voice Mode with Athena](/guides/use-voice-mode-with-athena)。
...
```bash
athena gateway setup        # 交互式配置所有消息平台
```
...
```bash
athena gateway              # 在前台运行
athena gateway setup        # 交互式配置消息平台
athena gateway install      # 安装为用户服务（Linux）/ launchd 服务（macOS）
sudo athena gateway install --system   # 仅 Linux：安装开机启动的系统服务
athena gateway start        # 启动默认服务
athena gateway stop         # 停止默认服务
athena gateway status       # 检查默认服务状态
athena gateway status --system         # 仅 Linux：显式检查系统服务
```
...
在 `~/.athena/gateway.json` 中配置各平台的覆盖设置：
...
# 其他配对命令：
athena pairing list          # 查看待审核和已批准的用户
athena pairing revoke telegram 123456789  # 撤销访问权限
```
...
在 `~/.athena/config.yaml` 中控制显示多少工具活动信息：
...
当运行后台会话的 agent 使用 `terminal(background=true)` 启动长时间运行的进程（服务器、构建等）时，网关可以向你的聊天推送状态更新。通过 `~/.athena/config.yaml` 中的 `display.background_process_notifications` 控制：
...
```bash
athena gateway install               # 安装为用户服务
athena gateway start                 # 启动服务
athena gateway stop                  # 停止服务
athena gateway status                # 检查状态
journalctl --user -u athena-gateway -f  # 查看日志
...
:::info 多个安装
如果你在同一台机器上运行多个 Athena 安装（使用不同的 `ATHENA_HOME` 目录），每个安装都有自己的 systemd 服务名称。默认的 `~/.athena` 使用 `athena-gateway`；其他安装使用 `athena-gateway-<hash>`。`athena gateway` 命令会自动针对当前 `ATHENA_HOME` 对应的正确服务。
:::

### macOS（launchd）

```bash
athena gateway install               # 安装为 launchd agent
athena gateway start                 # 启动服务
athena gateway stop                  # 停止服务
athena gateway status                # 检查状态
tail -f ~/.athena/logs/gateway.log   # 查看日志
```

生成的 plist 文件位于 `~/Library/LaunchAgents/ai.athena.gateway.plist`。它包含三个环境变量：
...
- **ATHENA_HOME** — 将网关限定到你的 Athena 安装。

:::tip 安装后 PATH 变更
launchd plist 是静态的——如果你在配置网关后安装了新工具（例如通过 nvm 安装新版 Node.js，或通过 Homebrew 安装 ffmpeg），请重新运行 `athena gateway install` 以捕获更新后的 PATH。网关会检测到过时的 plist 并自动重新加载。
:::

:::info 多个安装
与 Linux systemd 服务类似，每个 `ATHENA_HOME` 目录都有自己的 launchd 标签。默认的 `~/.athena` 使用 `ai.athena.gateway`；其他安装使用 `ai.athena.gateway-<suffix>`。
:::
...
|----------|---------|--------------|
| CLI | `athena-cli` | 完全访问 |
| Telegram | `athena-telegram` | 完整工具，包括终端 |
| Discord | `athena-discord` | 完整工具，包括终端 |
| WhatsApp | `athena-whatsapp` | 完整工具，包括终端 |
| Slack | `athena-slack` | 完整工具，包括终端 |
| Google Chat | `athena-google_chat` | 完整工具，包括终端 |
| Signal | `athena-signal` | 完整工具，包括终端 |
| SMS | `athena-sms` | 完整工具，包括终端 |
| Email | `athena-email` | 完整工具，包括终端 |
| Home Assistant | `athena-homeassistant` | 完整工具 + HA 设备控制（ha_list_entities、ha_get_state、ha_call_service、ha_list_services） |
| Mattermost | `athena-mattermost` | 完整工具，包括终端 |
| Matrix | `athena-matrix` | 完整工具，包括终端 |
| DingTalk | `athena-dingtalk` | 完整工具，包括终端 |
| Feishu/Lark | `athena-feishu` | 完整工具，包括终端 |
| WeCom | `athena-wecom` | 完整工具，包括终端 |
| WeCom Callback | `athena-wecom-callback` | 完整工具，包括终端 |
| Weixin | `athena-weixin` | 完整工具，包括终端 |
| BlueBubbles | `athena-bluebubbles` | 完整工具，包括终端 |
| QQBot | `athena-qqbot` | 完整工具，包括终端 |
| Yuanbao | `athena-yuanbao` | 完整工具，包括终端 |
| Microsoft Teams | `athena-teams` | 完整工具，包括终端 |
| API Server | `athena-api-server` | 完整工具（去除 `clarify`、`send_message`、`text_to_speech`——程序化访问没有交互用户） |
| Webhooks | `athena-webhook` | 完整工具，包括终端 |
...
1. **网关日志**（`~/.athena/logs/gateway.log` 或 systemd / launchd 单元日志）。搜索平台名称以及 `circuit breaker`、`paused` 或 `disabled`。触发事件包含失败次数和最后一个错误。
