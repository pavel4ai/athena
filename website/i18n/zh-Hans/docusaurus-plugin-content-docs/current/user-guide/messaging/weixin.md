...
3. 等待你用微信手机端扫描二维码
4. 提示你在手机上确认登录
5. 自动将账号凭据保存至 `~/.athena/weixin/accounts/`
...
完成首次扫码登录后，在 `~/.athena/.env` 中至少设置账号 ID：
...
- **扫码登录** — 通过 `athena gateway setup` 扫码连接
...
- Token 按账号+对话方保存至 `~/.athena/weixin/accounts/<account_id>.context-tokens.json`
- 启动时恢复之前保存的 token
...
| `Weixin startup failed: WEIXIN_TOKEN is required` | 运行 `athena gateway setup` 完成扫码登录，或手动设置 `WEIXIN_TOKEN` |
| `Weixin startup failed: WEIXIN_ACCOUNT_ID is required` | 在 `.env` 中设置 `WEIXIN_ACCOUNT_ID`，或运行 `athena gateway setup` |
| `Another local Athena gateway is already using this Weixin token` | 先停止另一个网关实例——每个 token 只允许一个轮询器 |
| 会话过期（`errcode=-14`） | 登录会话已过期。重新运行 `athena gateway setup` 扫描新二维码 |
...
| 终端二维码无法渲染 | 使用 messaging 扩展重新安装：`cd ~/.athena/athena-agent && uv pip install -e ".[messaging]"`。或者，打开二维码上方打印的 URL |