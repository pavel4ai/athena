...
- 一个 topic 名称（任意唯一字符串——`athena-myname-2026` 即可）
...
- 或**选择一个长且难以猜测的 topic 名称**（`athena-7d4f9c8b-2026`），将其视为共享密钥。这是最轻量的方案，但 topic 名称可能通过日志或截图泄露。

在任何情况下，除非底层 topic 已启用访问控制，否则不要通过 ntfy 传输敏感数据。

## 快速开始——从手机与 agent 对话

1. 选择一个 topic 名称：`athena-myname-2026`
2. 在手机上：安装 [ntfy 应用](https://ntfy.sh/docs/subscribe/phone/)，点击 **+**，输入 `athena-myname-2026`
3. 在主机上：
   ```bash
