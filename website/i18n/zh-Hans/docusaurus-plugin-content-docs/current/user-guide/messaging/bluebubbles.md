...
```
使用 `athena pairing list` 查看待处理的配对码和已授权用户。

**预授权特定用户**（在 `~/.athena/.env` 中）：
```bash
...
自动将消息标记为已读由 `~/.athena/config.yaml` 中 `platforms.bluebubbles.extra` 下的 `send_read_receipts` 键控制（默认值：`true`）。该选项没有对应的环境变量。
...
- 查看 `athena logs gateway` 中的 webhook 错误（或使用 `athena logs -f` 实时跟踪）
