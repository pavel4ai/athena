...
sudo athena gateway install --system   # 仅 Linux：开机自启系统服务
```
...
2. **未设置白名单** → 未知用户会收到私信配对码（通过 `athena pairing approve signal CODE` 审批）
...
Signal 单条消息上限为 **8,000 字符**。Athena 会自动将更长的回复拆分为带编号的分段（`(1/3)`、`(2/3)`……）发送，而不是截断内容。此行为覆盖所有投递路径——实时对话回复、cron 任务投递、`athena send` 以及 MCP `send_message` 调用——且原生格式（加粗、斜体、代码、剧透）在分段边界处保持不变。
