...
2. 用新号码注册 WhatsApp
3. 运行 `athena whatsapp` 并从该 WhatsApp 账号扫描二维码

---

## 第三步：配置 Athena

在 `~/.athena/.env` 文件中添加以下内容：
...
Baileys 桥接将会话保存在 `~/.athena/platforms/whatsapp/session` 目录下。这意味着：
...
| **二维码过期** | 二维码约每 20 秒刷新一次。如果超时，重新运行 `athena whatsapp`。 |
| **会话未持久化** | 检查 `~/.athena/platforms/whatsapp/session` 是否存在且可写。如在容器中运行，请将其挂载为持久卷。 |
| **意外退出登录** | WhatsApp 会在长时间不活跃后取消关联设备。保持手机开机并连接网络，如有需要使用 `athena whatsapp` 重新配对。 |
| **桥接崩溃或重连循环** | 重启 gateway，更新 Athena，如会话因 WhatsApp 协议变更而失效则重新配对。 |
| **WhatsApp 更新后机器人停止工作** | 更新 Athena 以获取最新桥接版本，然后重新配对。 |
| **macOS："Node.js not installed"但终端中 node 可用** | launchd 服务不继承你的 shell PATH。运行 `athena gateway install` 将当前 PATH 重新快照到 plist 中，然后运行 `athena gateway start`。详见 [Gateway 服务文档](./index.md#macos-launchd)。 |
...
| **机器人向陌生人回复配对码** | 如需对未授权私信静默处理，在 `~/.athena/config.yaml` 中设置 `whatsapp.unauthorized_dm_behavior: ignore`。 |
...
- `~/.athena/platforms/whatsapp/session` 目录包含完整会话凭证——请像保护密码一样保护它
- 设置文件权限：`chmod 700 ~/.athena/platforms/whatsapp/session`
