...
审批系统支持三种模式，通过 `~/.athena/config.yaml` 中的 `approvals.mode` 配置：
...
1. **CLI 标志**：使用 `athena --yolo` 或 `athena chat --yolo` 启动会话
2. **斜杠命令**：在会话中输入 `/yolo` 以切换开/关
3. **环境变量**：设置 `ATHENA_YOLO_MODE=1`
...
YOLO 模式在 CLI 和 gateway 会话中均可使用。在内部，它会设置 `ATHENA_YOLO_MODE` 环境变量，该变量在每次命令执行前都会被检查。
...
| `tee` 写入 `/etc/`、`~/.ssh/`、`~/.athena/.env` | 通过 tee 覆盖敏感文件 |
| `>` / `>>` 写入 `/etc/`、`~/.ssh/`、`~/.athena/.env` | 通过重定向覆盖敏感文件 |
...
| `pkill`/`killall` athena/gateway | 防止自我终止 |
...
运行 gateway 时，`ATHENA_EXEC_ASK=1` 环境变量会自动设置。

### 永久允许列表

通过"always"批准的命令会保存到 `~/.athena/config.yaml`：
...
:::tip
使用 `athena config edit` 查看或删除永久允许列表中的模式。
:::
...
在 `~/.athena/.env` 中以逗号分隔的值设置允许的用户 ID：
...
1. 未知用户向机器人发送 DM
2. 机器人回复一个 8 位配对码
3. 机器人所有者在 CLI 上运行 `athena pairing approve <platform> <code>`
4. 该用户在该平台上获得永久批准

在 `~/.athena/config.yaml` 中控制未授权私信的处理方式：
...
**存储：** 配对数据存储于 `~/.athena/pairing/`，按平台分为独立的 JSON 文件：
...
- **持久模式**（`container_persistent: true`）：从 `~/.athena/sandboxes/docker/<task_id>/` 绑定挂载 `/workspace` 和 `/root`
...
某些技能需要在沙箱中访问**文件**（而非仅环境变量）——例如，Google Workspace 将 OAuth token 存储为活跃 profile 的 `ATHENA_HOME` 下的 `google_token.json`。技能在 frontmatter 中声明这些文件：
...
加载后，Athena 会检查这些文件是否存在于活跃 profile 的 `ATHENA_HOME` 中，并将其注册为挂载：
...
路径相对于 `~/.athena/`。文件在容器内挂载到 `/root/.athena/`。
...
4. **安全存储密钥** — 将 API 密钥保存在具有适当文件权限的 `~/.athena/.env` 中
...
9. **监控日志** — 检查 `~/.athena/logs/` 中的未授权访问尝试
10. **保持更新** — 定期运行 `athena update` 以获取安全补丁
...
为获得最高安全性，请在独立的机器或虚拟机上运行 gateway。在 `config.yaml` 中设置 `terminal.backend: ssh`，然后通过 `~/.athena/.env` 中的环境变量提供主机详情：
...
Athena 内置了一个公告扫描器，用于标记活跃 venv 中与已知受损版本目录匹配的 Python 包（例如 2026 年 5 月的 `mistralai 2.4.6` 供应链投毒事件）。实现位于 `athena_cli/security_advisories.py`。

运行方式：

- **CLI 启动横幅。** 若有任何公告匹配，会打印一行警告，并指向 `athena doctor` 获取完整修复方案。
- **`athena doctor`。** 显示所有活跃公告的版本详情和 2-4 步修复说明。
...
3. 若安装失败或用户已禁用懒加载安装，调用会抛出 `FeatureUnavailable`，附带实际的 pip stderr 和指向 `athena tools` 的提示。
...
禁用后，需要可选依赖的后端会提示用户手动运行安装（`pip install …`）或通过 `athena tools` 选择其他后端。