...
| | |
|---|---|
| 来源 | 可选 — 通过 `athena skills install official/migration/openclaw-migration` 安装 |
| 路径 | `optional-skills/migration/openclaw-migration` |
| 版本 | `1.0.0` |
| 作者 | Athena Agent (Futurebound Corp.) |
| 许可证 | MIT |
...
**首次设置：** `athena setup` 向导会自动检测 `~/.openclaw`，并在配置开始前提供迁移选项。

## 此 skill 的功能

它使用 `scripts/openclaw_to_athena.py` 来：
...
- 将 OpenClaw skill 复制到 `~/.athena/skills/openclaw-imports/`
- 可选地将 OpenClaw 工作区指令文件复制到所选 Athena 工作区
- 将兼容的工作区资产（如 `workspace/tts/`）镜像到 `~/.athena/tts/`
...
请勿猜测更短的路径，如 `~/.athena/skills/openclaw-migration/...`。

运行辅助脚本前：

1. 优先使用 `~/.athena/skills/migration/openclaw-migration/` 下的已安装路径。
...
- 在 `~/.athena/skills/openclaw-imports/` 下可用的 OpenClaw skill
