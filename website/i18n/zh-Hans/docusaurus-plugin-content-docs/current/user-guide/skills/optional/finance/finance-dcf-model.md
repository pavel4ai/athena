...
| | |
|---|---|
| 来源 | 可选——通过 `athena skills install official/finance/dcf-model` 安装 |
| 路径 | `optional-skills/finance/dcf-model` |
| 版本 | `1.0.0` |
| 作者 | Anthropic（由 Futurebound Corp. 改编） |
| 许可证 | Apache-2.0 |
...
**如果遇到错误或不合理的结果，请阅读 [TROUBLESHOOTING.md](https://github.com/pavel4ai/athena/blob/main/optional-skills/finance/dcf-model/TROUBLESHOOTING.md) 获取详细的调试指导。**
...
3. **检查输出**：
   ...
   - 如果 `status` 为 `"errors_found"` → 检查 `error_summary` 并阅读 [TROUBLESHOOTING.md](https://github.com/pavel4ai/athena/blob/main/optional-skills/finance/dcf-model/TROUBLESHOOTING.md) 获取调试指导
