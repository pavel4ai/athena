...
| | |
|---|---|
| 来源 | 可选 — 通过 `athena skills install official/mcp/fastmcp` 安装 |
...
| 相关 skill | [`athena-agent`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-athena-agent), [`mcporter`](/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- 使用 `native-mcp` skill，在 `~/.athena/config.yaml` 中配置服务器，或
...
服务器构建部分可能正确，但 Athena 配置有误。加载 `native-mcp` skill 并在 `~/.athena/config.yaml` 中配置服务器，然后重启 Athena。
