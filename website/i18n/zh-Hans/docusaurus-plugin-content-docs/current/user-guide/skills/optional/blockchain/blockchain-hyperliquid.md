...
| | |
|---|---|
| 来源 | 可选 — 通过 `athena skills install official/blockchain/hyperliquid` 安装 |
...
辅助脚本：`~/.athena/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
对于 `state`、`spot-balances`、`fills`、`orders` 和 `review`，当 `~/.athena/.env` 中设置了 `HYPERLIQUID_USER_ADDRESS` 时，地址参数为可选。
