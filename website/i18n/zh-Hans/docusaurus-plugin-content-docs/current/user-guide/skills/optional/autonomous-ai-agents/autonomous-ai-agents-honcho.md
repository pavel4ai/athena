...
| | |
|---|---|
| 来源 | 可选 — 通过 `athena skills install official/autonomous-ai-agents/honcho` 安装 |
...
参见：https://docs.honcho.dev/v3/guides/integrations/athena#running-honcho-locally-with-athena
...
手动覆盖：`athena honcho map my-project-name`
...
`--clone` 对 Honcho 的作用：
1. 在 `honcho.json` 中创建 `athena.coder` host 块
...
配置文件：`$ATHENA_HOME/honcho.json`（配置文件本地）或 `~/.honcho/config.json`（全局）。
...
运行 `athena honcho setup`。确保 `~/.athena/config.yaml` 中包含 `memory.provider: honcho`。

### 记忆未跨会话持久化
检查 `athena honcho status` -- 验证 `saveMessages: true` 且 `writeFrequency` 不是 `session`（该选项仅在退出时写入）。

### 配置文件未获得自己的 peer
创建时使用 `--clone`：`athena profile create <name> --clone`。对于现有配置文件：`athena honcho sync`。
...
| 命令 | 描述 |
|---------|-------------|
| `athena honcho setup` | 交互式设置向导（云端/本地、身份、观察、召回、会话） |
| `athena honcho status` | 显示当前配置文件的已解析配置、连接测试、peer 信息 |
| `athena honcho enable` | 为当前配置文件启用 Honcho（如需则创建 host 块） |
| `athena honcho disable` | 为当前配置文件禁用 Honcho |
| `athena honcho peer` | 显示或更新 peer 名称（`--user <name>`、`--ai <name>`、`--reasoning <level>`） |
| `athena honcho peers` | 显示所有配置文件的 peer 身份 |
| `athena honcho mode` | 显示或设置召回模式（`hybrid`、`context`、`tools`） |
| `athena honcho tokens` | 显示或设置 token 预算（`--context <N>`、`--dialectic <N>`） |
| `athena honcho sessions` | 列出已知的目录到会话名称映射 |
| `athena honcho map <name>` | 将当前工作目录映射到 Honcho 会话名称 |
| `athena honcho identity` | 为 AI peer 身份播种，或显示两个 peer 的表示 |
| `athena honcho sync` | 为所有尚未拥有 host 块的 Athena 配置文件创建 host 块 |
| `athena honcho migrate` | 从 OpenClaw 原生记忆迁移到 Athena + Honcho 的分步指南 |
| `athena memory setup` | 通用记忆提供商选择器（选择 "honcho" 运行相同向导） |
| `athena memory status` | 显示当前活跃的记忆提供商及配置 |
| `athena memory off` | 禁用外部记忆提供商 |