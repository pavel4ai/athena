...
| | |
|---|---|
| 来源 | 可选 — 通过 `athena skills install official/research/darwinian-evolver` 安装 |
...
```bash
mkdir -p ~/.athena/cache/darwinian-evolver && cd ~/.athena/cache/darwinian-evolver
...
```bash
cd ~/.athena/cache/darwinian-evolver/darwinian_evolver \
...
在浏览器中打开 `~/.athena/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
并加载 JSON 日志，即可查看进化树。
...
SKILL_DIR=~/.athena/skills/research/darwinian-evolver
DE_DIR=~/.athena/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL 协议。** 永远不要在 Athena 核心中使用 `from darwinian_evolver import ...`。`~/.athena/skills/...` 下的自定义驱动脚本属于用户侧，没有问题。
