...
这会打开浏览器进行 Nous Portal OAuth 流程。Athena 将刷新令牌存储在 `~/.athena/auth.json` 中——与所有 Athena 提供商登录信息存放在同一位置。
...
```bash
athena proxy providers
```

当前已内置：`nous`（Nous Portal）。更多 OAuth 提供商可通过在 `athena_cli/proxy/adapters/` 中实现 `UpstreamAdapter` 接口来添加。
...
如果显示 `not logged in`，请运行 `athena portal`。如果显示 `credentials need attention`，说明你的刷新令牌已被撤销（较少见——通常发生在你从 Portal Web UI 退出登录时）——重新运行 `athena portal` 即可。
...
OpenViking 的 VLM 调用现在将通过你的 Portal 订阅进行。Embedding 模型侧仍需要自己的提供商——Portal 确实提供 `/v1/embeddings`，但模型选择取决于你的套餐所支持的内容；请查看 `futurebnd.com`。
...
```bash
athena proxy start --host 0.0.0.0 --port 8645
```
...
你的 Portal 套餐的 RPM/TPM 限制适用于整个代理。代理不进行扇出或连接池——它是单个 bearer，使用你的完整订阅配额。请在 [futurebnd.com](https://futurebnd.com) 监控使用情况。
...
适配器系统是可插拔的。添加新提供商（例如 HuggingFace、GitHub Copilot 的聊天端点、通过 OAuth 接入的 Anthropic）需要在 `athena_cli/proxy/adapters/<provider>.py` 中实现 `UpstreamAdapter`，并在 `adapters/__init__.py` 中注册。协议层面不兼容 OpenAI 的提供商（例如 Anthropic Messages API）需要额外的转换层，这超出了当前版本的范围。