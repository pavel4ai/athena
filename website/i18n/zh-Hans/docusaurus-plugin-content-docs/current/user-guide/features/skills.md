...
所有 skills 存放在 **`~/.athena/skills/`** 中——这是主目录和唯一可信来源。全新安装时，捆绑的 skills 会从仓库复制过来。通过 Hub 安装和 agent 创建的 skills 也存放在此处。agent 可以修改或删除任何 skill。
...
（计划模式的工作方式相同，但现在是内置命令：`/plan [request]` 告知 Athena 在需要时检查上下文、编写 markdown 实现计划而非直接执行任务，并将结果保存在相对于当前工作区/后端工作目录的 `.athena/plans/` 下。）
...
当遇到缺失的值时，Athena 仅在本地 CLI 中实际加载 skill 时才会安全地请求输入。你可以跳过设置并继续使用该 skill。消息平台不会在聊天中请求密钥——它们会告诉你改用本地的 `athena setup` 或 `~/.athena/.env`。
...
设置存储在 config.yaml 的 `skills.config` 下。`athena config migrate` 会提示配置未设置的项，`athena config show` 会显示它们。当 skill 加载时，其解析后的配置值会注入到上下文中，agent 会自动知晓已配置的值。
...
通过第三方 URL 或 GitHub 安装时，Athena 会安装 `SKILL.md`，以及其中明确引用且位于 `references/`、`templates/`、`scripts/`、`assets/` 和 `examples/` 下的文件。未引用的仓库文件不会被复制。Athena 会扫描完整的隔离捆绑包，并在 `skills/.hub/lock.json` 中记录来源 URL、精确内容哈希、扫描器版本、发现项、时间戳，以及本次结果是新扫描还是缓存复用。

## 外部 Skill 目录

如果你在 Athena 之外维护 skills——例如，供多个 AI 工具使用的共享 `~/.agents/skills/` 目录——你可以告诉 Athena 也扫描这些目录。

在 `~/.athena/config.yaml` 的 `skills` 部分下添加 `external_dirs`：
...
- **本地创建，就地更新**：新的 agent 创建的 skills 写入 `~/.athena/skills/`。现有 skills 在找到的位置被修改，包括 `external_dirs` 下的 skills，当 agent 使用 `skill_manage` 操作（如 `patch`、`edit`、`write_file`、`remove_file` 或 `delete`）时。
...
捆绑包存放在 **`~/.athena/skill-bundles/<slug>.yaml`** 中，格式如下：
...
- `description`（可选）——在 `/bundles` 和 `athena bundles list` 中显示的简短文本。
...
# 重新扫描 ~/.athena/skill-bundles/ 并报告变更
...
- 你想通过将捆绑包 YAML 提交到共享 dotfiles 仓库并符号链接到 `~/.athena/skill-bundles/` 来发布团队范围的"任务配置文件"。

捆绑包只是一个 YAML 别名——它不会为你安装 skills。Skills 本身必须已经存在（在 `~/.athena/skills/` 或外部 skill 目录中）。否则捆绑包调用只会跳过缺失的 skills。
...
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | 可通过 `athena skills search <query> --source skills-sh` 搜索。当 skills.sh slug 与仓库文件夹不同时，Athena 会解析别名式 skills。 |
...
直接从任何 HTTP(S) URL 安装 `SKILL.md`——当作者在自己的站点上托管 skill 时非常有用（无 hub 列表，无需输入 GitHub 路径）。Athena 还会获取其中明确引用且位于 `references/`、`templates/`、`scripts/`、`assets/` 和 `examples/` 下的文件，然后扫描并安装完整捆绑包。

- Athena 来源 id：`url`
- 标识符：URL 本身（无需前缀）
- 范围：`SKILL.md` 加上允许目录中明确引用的支持文件。Athena 不会枚举或复制托管站点上的其他文件。
...
信任级别始终为 `community`——与所有其他来源一样运行相同的安全扫描。URL 作为安装标识符存储，因此当你想刷新时，`athena skills update` 会自动从同一 URL 重新获取。

### 安全扫描与 `--force`

所有通过 hub 安装的 skills 都经过**安全扫描器**检查，检测数据泄露、prompt 注入、破坏性命令、供应链信号及其他威胁。

`athena skills inspect ...` 现在还会在可用时显示上游元数据：
- 仓库 URL
- skills.sh 详情页 URL
- 安装命令
- 每周安装量
...
如果你想分享一组精选的 skills——为你的团队、组织或公开分享——你可以将它们发布为 **tap**：其他 Athena 用户通过 `athena skills tap add <owner/repo>` 添加的 GitHub 仓库。无需服务器，无需注册表注册，无需发布流水线。只需一个包含 `SKILL.md` 文件的目录。
...
- 每个 skill 目录必须包含一个带有标准 [SKILL.md frontmatter](#skillmd-format) 的 `SKILL.md`（`name`、`description`，以及可选的 `metadata.athena.tags`、`version`、`author`、`platforms`、`metadata.athena.config`）。
...
如果你的 skills 不在 `skills/` 下（当你向现有项目添加 `skills/` 子树时很常见），请编辑 `~/.athena/.hub/taps.json` 中的 tap 条目：
...
`athena skills tap add` CLI 默认将新 tap 的 `path` 设为 `"skills/"`；如果需要不同路径，请直接编辑该文件。`athena skills tap list` 显示每个 tap 的有效路径。
...
Tap 存储在 `~/.athena/.hub/taps.json` 中（按需创建）。

## 捆绑 skill 更新（`athena skills reset`）

Athena 在仓库的 `skills/` 中附带一组捆绑 skills。在安装时以及每次 `athena update` 时，同步过程会将这些 skills 复制到 `~/.athena/skills/` 中，并在 `~/.athena/skills/.bundled_manifest` 记录一个清单，将每个 skill 名称映射到同步时的内容哈希（**origin hash**）。
...
这种保护机制很好，但有一个棘手的边缘情况。如果你编辑了一个捆绑 skill，后来想通过从 `~/.athena/athena-agent/skills/` 复制粘贴来放弃更改并回到捆绑版本，清单仍然保存着上次成功同步时的*旧* origin hash。你新复制粘贴的内容（当前捆绑哈希）与那个过时的 origin hash 不匹配，因此同步继续将其标记为用户修改。

`athena skills reset` 是解决此问题的方法：
...
:::note Profiles
每个 profile 在其自己的 `ATHENA_HOME` 下有自己的 `.bundled_manifest`，因此 `athena -p coder skills reset <name>` 只影响该 profile。
:::
