...
| **X 搜索** | `x_search` | 通过 xAI 内置的 `x_search` Responses 工具搜索 X（Twitter）帖子和话题——需要 xAI 凭据（SuperGrok OAuth 或 `XAI_API_KEY`）；默认关闭，可通过 `athena tools` → 🐦 X (Twitter) Search 启用。 |
| **终端与文件** | `terminal`, `process`, `read_file`, `patch` | 执行命令并操作文件。 |
| **浏览器** | `browser_navigate`, `browser_snapshot`, `browser_vision` | 支持文本和视觉的交互式浏览器自动化。 |
| **媒体** | `vision_analyze`, `image_generate`, `video_generate`, `video_analyze`, `text_to_speech` | 多模态分析与生成。`video_generate` 和 `video_analyze` 需手动启用（通过 `athena tools` 或 `--toolsets` 添加 `video_gen` / `video` 工具集）。 |
...
付费 [Nous Portal](https://portal.nousresearch.com) 订阅者可通过 **[Tool Gateway](tool-gateway.md)** 使用网页搜索、图像生成、TTS 和浏览器自动化——无需单独配置 API 密钥。运行 `athena model` 启用，或通过 `athena tools` 配置各工具。
:::
...
完整列表（包括 `athena-cli`、`athena-telegram` 等平台预设以及 `mcp-<server>` 等动态 MCP 工具集）请参阅 [工具集参考](/reference/toolsets-reference)。
...
需同时配置 `VERCEL_TOKEN`、`VERCEL_PROJECT_ID` 和 `VERCEL_TEAM_ID` 三个凭据。此访问令牌配置方式是在 Render、Railway、Docker 及类似平台上进行部署和正常长期运行 Athena 进程的推荐路径。支持的运行时为 `node24`、`node22` 和 `python3.13`；Athena 默认使用 `/vercel/sandbox` 作为远程工作区根目录。

对于本地一次性开发，Athena 也接受短期 Vercel OIDC token：
...
启用 `container_persistent: true` 后，Athena 使用 Vercel 快照在同一任务的沙箱重建时保留文件系统状态，其中可包含沙箱内 Athena 同步的凭据、技能和缓存文件。快照不保留活跃进程、PID 空间或相同的活跃沙箱标识。

后台终端命令使用 Athena 通用的非本地进程流程：在沙箱存活期间，spawn、poll、wait、log 和 kill 均通过标准 process 工具运行，但 Athena 不提供清理或重启后的原生 Vercel 后台进程恢复能力。
...
无头父进程退出或 Athena 重启后，请恢复启动该命令的会话（或其上下文压缩后的
...
如果命令需要 sudo，系统会提示你输入密码（在本次会话内缓存）。也可在 `~/.athena/.env` 中设置 `SUDO_PASSWORD`。

:::warning
在消息平台上，如果 sudo 失败，输出中会提示将 `SUDO_PASSWORD` 添加到 `~/.athena/.env`。
:::