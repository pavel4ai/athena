...
两者均通过单一后端选择进行配置。提供商可通过 `athena tools` 选择，或直接在 `config.yaml` 中设置。
...
| **xAI (Grok)** | `XAI_API_KEY` 或 `athena auth login xai-oauth` | ✔ | — | 付费（SuperGrok 或按 token 计费） |
...
:::tip Nous 订阅用户
如果您拥有付费 [Nous Portal](https://futurebnd.com) 订阅，网页搜索和提取可通过 **[Tool Gateway](tool-gateway.md)** 使用托管的 Firecrawl——无需 API 密钥。新安装可运行 `athena setup --portal` 登录并一次性开启所有 gateway 工具；现有安装可通过 `athena tools` 单独开启网页功能。
:::
...
`web_extract` 辅助任务。默认情况下（`auxiliary.web_extract.provider: "auto"`），使用您的**主聊天模型**——与 `athena model` 相同的提供商和模型。对大多数配置而言这没问题，但在昂贵的推理模型（Opus、MiniMax M2.7 等）上，每次长页面提取都会产生可观的成本。
...
或交互式选择：`athena model` → **Configure auxiliary models** → `web_extract`。
...
### 通过 `athena tools` 快速设置

运行 `athena tools`，导航至 **Web Search & Extract**，选择一个提供商。向导会提示输入所需的 URL 或 API 密钥，并写入您的配置。
...
然后在 `~/.athena/config.yaml` 中选择 SearXNG 作为搜索后端：
...
或通过 `athena tools` → Web Search & Extract → SearXNG 设置。
