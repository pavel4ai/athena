...
`athena fallback` 复用 `athena model` 的提供商选择器——相同的提供商列表、相同的凭据提示、相同的验证流程。使用子命令 `add`、`list`（别名 `ls`）、`remove`（别名 `rm`）和 `clear` 来管理备用链。更改会持久化到 `config.yaml` 顶层的 `fallback_providers:` 列表中。

如果你更倾向于直接编辑 YAML，可在 `~/.athena/config.yaml` 中添加 `fallback_model` 部分：
...
`fallback_model`（单数）是旧版单备用键——Athena 仍支持以保持向后兼容。`fallback_providers`（复数，列表）支持按顺序尝试多个备用；`athena fallback` 写入此键。当两者同时设置时，Athena 会合并它们，`fallback_providers` 优先。
:::
...
| Nous Portal | `nous` | `athena setup --portal`（全新安装）或 `athena auth add nous`（OAuth） |
| OpenAI Codex | `openai-codex` | `athena model`（ChatGPT OAuth） |
...
| xAI Grok OAuth（SuperGrok） | `xai-oauth`（别名 `grok-oauth`） | `athena model` → xAI Grok OAuth（浏览器登录；需 SuperGrok 订阅） |
| AWS Bedrock | `bedrock` | 标准 boto3 认证（`AWS_REGION` + `AWS_PROFILE` 或 `AWS_ACCESS_KEY_ID`） |
| Qwen Portal（OAuth） | `qwen-oauth` | `athena model`（Qwen Portal OAuth；可选：`ATHENA_QWEN_BASE_URL`） |
| MiniMax（OAuth） | `minimax-oauth` | `athena model`（MiniMax 门户 OAuth） |
...
:::tip
没有针对主备用链的环境变量——只能通过 `config.yaml` 或 `athena fallback` 进行配置。这是有意为之：备用配置是一个经过深思熟虑的选择，不应被过期的 shell 导出变量覆盖。
:::
...
| Triage Specifier | `athena kanban specify` / 看板（kanban）✨ 按钮——将单行 triage 任务扩展为完整规格 | `auxiliary.triage_specifier` |
...
| `"nous"` | 强制使用 Nous Portal | `athena auth` |
| `"codex"` | 强制使用 Codex OAuth | `athena model` → Codex |
