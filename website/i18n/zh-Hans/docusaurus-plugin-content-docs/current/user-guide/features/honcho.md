...
```bash
athena memory setup    # 从 provider 列表中选择 "honcho"
```
...
Honcho 在 `~/.honcho/config.json`（全局）或 `$ATHENA_HOME/honcho.json`（profile 本地）中配置。设置向导会自动处理。
...
- `per-session` — 每次 `athena` 运行获得一个新会话。干净启动，通过 tools 访问记忆。推荐新用户使用。
...
`athena honcho` 子命令**仅在 Honcho 为当前活跃 memory provider 时注册**（`config.yaml` 中 `memory.provider: honcho`）。先运行 `athena memory setup` 并选择 Honcho，子命令将在下次调用时出现。

```bash
athena honcho status          # 连接状态、配置及关键设置
athena honcho setup           # 重定向到 `athena memory setup`
athena honcho strategy        # 查看或设置会话策略（per-session/per-directory/per-repo/global）
athena honcho peer            # 查看或更新 peer 名称及辩证推理级别
athena honcho mode            # 查看或设置 recall 模式（hybrid/context/tools）
athena honcho tokens          # 查看或设置上下文和辩证的 token 预算
athena honcho identity        # 初始化或查看 AI peer 的 Honcho 身份
athena honcho sync            # 将 Honcho 配置同步到所有现有 profile
athena honcho peers           # 查看所有 profile 中的 peer 身份
athena honcho sessions        # 列出已知的 Honcho 会话映射
athena honcho map             # 将当前目录映射到 Honcho 会话名称
athena honcho enable          # 为当前 profile 启用 Honcho
athena honcho disable         # 为当前 profile 禁用 Honcho
athena honcho migrate         # 从 openclaw-honcho 迁移的分步指南
```

## 从 `athena honcho` 迁移

如果你之前使用了独立的 `athena honcho setup`：

1. 你的现有配置（`honcho.json` 或 `~/.honcho/config.json`）已保留
2. 你的服务端数据（记忆、结论、用户画像）完好无损
3. 在 config.yaml 中设置 `memory.provider: honcho` 即可重新激活

无需重新登录或重新设置。运行 `athena memory setup` 并选择"honcho"——向导会自动检测你的现有配置。
