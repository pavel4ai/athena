...
这将进入设置向导，向导会提示你输入 API 密钥并将其写入 `~/.athena/.env`。你只需执行一次。强烈建议此时为 gateway 配置一个聊天系统。
...
内置 Web dashboard 在同一容器内作为受 s6-rc 监管的服务与 gateway 并行运行。设置 `ATHENA_DASHBOARD=1` 即可拉起它：
...
| 环境变量 | 描述 | 默认值 |
|---------------------|-------------|---------|
| `ATHENA_DASHBOARD` | 设为 `1`（或 `true` / `yes`）以启用受监管的 dashboard 服务 | *（未设置——服务已注册但保持关闭）* |
| `ATHENA_DASHBOARD_HOST` | dashboard HTTP 服务器的绑定地址 | `0.0.0.0` |
| `ATHENA_DASHBOARD_PORT` | dashboard HTTP 服务器的端口 | `9119` |
| `ATHENA_DASHBOARD_INSECURE` | **已弃用 / 空操作。** 以前用于绕过鉴权门控；自 2026 年 6 月的安全加固起，它不再禁用鉴权。任何非回环绑定都必须配置鉴权提供方 | *（被忽略——请改为配置提供方）* |

容器内的 dashboard 默认绑定 `0.0.0.0`，否则发布的 `-p 9119:9119` 端口将无法从宿主机访问。若你要把它限制在容器回环地址（例如 sidecar / 反向代理拓扑），请显式设置 `ATHENA_DASHBOARD_HOST=127.0.0.1`。
...
- **用户名/密码** —— 最简单的自托管 / 局域网 / VPN 内部署方式：设置 `ATHENA_DASHBOARD_BASIC_AUTH_USERNAME` + `ATHENA_DASHBOARD_BASIC_AUTH_PASSWORD`（以及用于跨重启稳定 session 的 `ATHENA_DASHBOARD_BASIC_AUTH_SECRET`）。不适合直接暴露到公网上。
- **OAuth（Nous Portal）** —— 适合托管/公网部署：设置 `ATHENA_DASHBOARD_OAUTH_CLIENT_ID` 后，`dashboard_auth/nous` 提供者会自动激活。
- **自托管 OIDC** —— 通过标准 OpenID Connect 接入你自己的身份提供商：设置 `ATHENA_DASHBOARD_OIDC_ISSUER` + `ATHENA_DASHBOARD_OIDC_CLIENT_ID` 后，`dashboard_auth/self_hosted` 提供者会激活。
...
`ATHENA_DASHBOARD_INSECURE=1` 现在是一个已弃用的空操作（它会
打印告警并被忽略）。请改为配置鉴权提供方，或设置
`ATHENA_DASHBOARD_HOST=127.0.0.1` 并通过 SSH 隧道 / Tailscale 访问。

:::warning 为什么移除了 `--insecure`
无鉴权的公网 dashboard 是 2026 年 6 月 MCP 配置持久化攻击活动的入口：互联网扫描器访问到暴露的 dashboard（以及 OpenAI API 服务器），诱导 agent 植入 SSH 密钥后门。现在每个非回环绑定都强制启用鉴权门控。对于可信局域网 / homelab 主机，内置的用户名/密码提供方（`ATHENA_DASHBOARD_BASIC_AUTH_USERNAME` + `_PASSWORD`）是满足该要求的零基础设施方式。
:::
...
`/opt/data` 卷是所有 Athena 状态的唯一数据来源。它映射到宿主机的 `~/.athena/` 目录，包含：
...
| `memories/` | 持久化记忆存储 |
| `skills/` | 已安装的技能 |
| `home/` | Athena 工具子进程（`git`、`ssh`、`gh`、`npm` 及 skill CLI）的 per-profile HOME |
| `cron/` | 定时任务定义 |
| `hooks/` | 事件 hook |
| `logs/` | 运行时日志 |
| `skins/` | 自定义 CLI 皮肤 |
...
在托管/发布的 Docker 镜像中，`/opt/athena` 是安装好的应用树。它由 root 拥有，并且对运行时的 `athena` 用户只读，因此 agent 回合、gateway 会话、dashboard 操作以及普通的 `docker exec athena athena ...` 命令都不能原地修改核心源码、打包的 `.venv`、`node_modules` 或 TUI bundle。

所有可变的 Athena 状态都应位于 `/opt/data` 下：配置、`.env`、profiles、skills、memories、sessions、logs、dashboard 上传、plugins 以及其他用户管理的文件。官方镜像还会阻止在运行时向不可变的 `/opt/athena` 树写入 `.pyc` 或执行 Athena 的懒安装依赖流程。

如果运维人员确实需要修复或检查 `/opt/data` 之外的文件，请有意识地使用 root shell。`athena` shim 默认会把 `docker exec athena athena ...` 降回运行时用户；只有在你明确需要 root 语义时，才临时设置 `ATHENA_DOCKER_EXEC_AS_ROOT=1`。
...
Athena 支持[多个 profile](../reference/profile-commands.md)——独立的 `~/.athena/` 子目录，让你可以从单个安装运行独立的 agent（不同的 SOUL、skills、memory、sessions、credentials）。**在官方 Docker 镜像内，s6 监管树把每个 profile 当作一等受监管服务**，因此推荐部署方式是：**一个容器承载多个 profile**。

每个通过 `athena profile create <name>` 创建的 profile 都会获得：

- 一个专用的 s6 服务槽位 `/run/service/gateway-<name>/`，运行时动态注册，无需重建镜像。
- 崩溃后的自动重启，由 `s6-supervise` 管理退避。
- 每个 profile 独立的轮转日志：`${ATHENA_HOME}/logs/gateways/<name>/current`。
...
services:
  athena:
    ...
    ports:
      ...
      - "9119:9119"   # dashboard（仅在 ATHENA_DASHBOARD=1 时生效）
    volumes:
      - ~/.athena:/opt/data
    environment:
      - ATHENA_DASHBOARD=1
...
1. 以 root 身份运行 `/etc/cont-init.d/01-athena-setup`（即 `docker/stage2-hook.sh`）：可选的 UID/GID 重映射、修复卷所有权、首次启动时初始化 `.env` / `config.yaml` / `SOUL.md`、同步内置技能。
2. 运行 `/etc/cont-init.d/02-reconcile-profiles`（即 `athena_cli.container_boot`）：遍历 `$ATHENA_HOME/profiles/<name>/`，在 `/run/service/gateway-<profile>/` 下重建各 profile 的 gateway s6 服务槽，并仅自动启动上次记录状态为 `running` 的 profile（参见 [Per-profile gateway 监管](#per-profile-gateway-supervision)）。
3. 启动静态的 `main-athena` 和 `dashboard` s6-rc 服务。
4. 将容器的 CMD 作为主程序 exec（`/opt/athena/docker/main-wrapper.sh`），根据用户传给 `docker run` 的参数进行路由：
   - 无参数 → `athena`（默认）
   - 第一个参数是 PATH 上的可执行文件（如 `sleep`、`bash`）→ 直接 exec
   - 其他情况 → `athena <args>`（子命令透传）
   主程序退出时容器退出，并使用其退出码。
...
:::warning 权限模型
除非你在命令链中保留 `/init`（或等效的旧版 `docker/entrypoint.sh` shim，它会转发到 stage2 hook），否则不要覆盖镜像入口点。s6-overlay 的 `/init` 以 root 运行，以便在首次启动时对卷执行 chown，然后通过 `s6-setuidgid` 为每个受监管的服务**以及**主程序降权至 `athena` 用户。在官方镜像内以 root 启动 `athena gateway run` 默认会被拒绝，因为这可能在 `/opt/data` 中留下 root 所有的文件，导致后续 dashboard 或 gateway 启动失败。仅在你有意接受该风险时才设置 `ATHENA_ALLOW_ROOT_GATEWAY=1`。
:::

### Per-profile gateway 监管

在容器内，每个通过 `athena profile create <name>` 创建的 profile 都会自动在 `/run/service/gateway-<name>/` 注册一个受 s6 监管的 gateway 服务。你在宿主机上运行的生命周期命令在此同样适用：

```sh
athena profile create coder            # 注册 gateway-coder s6 槽
athena -p coder gateway start          # s6-svc -u  → 受监管的 gateway
athena -p coder gateway stop           # s6-svc -d  → 服务停止
athena -p coder gateway restart        # s6-svc -t  → 向 supervisor 发送 SIGTERM
athena profile delete coder            # 拆除 s6 槽
```

**相比 pre-s6 镜像的监管优势：**

- Gateway 崩溃后由 `s6-supervise` 在约 1 秒退避后自动重启。
- Dashboard 崩溃后自动重启（设置 `ATHENA_DASHBOARD=1` 以启动）。
- `docker restart` 保留运行中的 gateway：cont-init 协调器读取 `$ATHENA_HOME/profiles/<name>/gateway_state.json`，若上次记录状态为 `running` 则恢复该槽。已停止的 gateway 保持停止状态。
- 各 profile 的 gateway 日志持久化于 `$ATHENA_HOME/logs/gateways/<profile>/current`（由 `s6-log` 轮转），协调器的操作记录在每次启动时追加到 `$ATHENA_HOME/logs/container-boot.log`。

在容器内执行 `athena status` 会显示 `Manager: s6 (container supervisor)`。使用 `/command/s6-svstat /run/service/gateway-<name>` 查看原始 supervisor 状态（注意 `/command/` 仅在监管树进程的 PATH 中；从 `docker exec` 调用时请传入绝对路径）。
...
当使用 Docker 作为执行环境时（不是上述方法，而是 agent 在 Docker 沙箱内运行命令——参见 [配置 → Docker 后端](./configuration.md#docker-backend)），Athena 为所有工具调用复用单个长期运行的容器，并自动将技能目录（`~/.athena/skills/`）和技能声明的所有凭据文件以只读卷的形式绑定挂载到该容器中。技能脚本、模板和引用在沙箱内无需手动配置即可使用，由于容器在 Athena 进程的整个生命周期内持续存在，你安装的任何依赖或写入的文件都会在下次工具调用时保留。
...
如果某个工具可能对大多数 Athena Agent 用户有用，考虑将其贡献到上游，而不是在私有派生镜像中维护。在 [athena-agent 仓库](https://github.com/pavel4ai/athena)提交 issue 或 pull request，描述该工具及其使用场景。被纳入官方镜像的工具惠及所有用户，并避免了维护下游 fork 的开销。
...
然后在 `~/.athena/config.yaml` 中，使用**容器名称**作为主机名：
...
1. 两个容器是否在同一 Docker 网络上（`docker network inspect athena-net`）
...
检查日志：`docker logs athena`。常见原因：
- `.env` 文件缺失或无效——先以交互方式运行以完成设置
- 开放端口时存在端口冲突

### "Permission denied" 错误

容器的 stage2 hook 通过 `s6-setuidgid` 在每个受监管的服务内将权限降至非 root 用户 `athena`（UID 10000）。如果宿主机的 `~/.athena/` 由不同 UID 拥有，请设置 `ATHENA_UID`/`ATHENA_GID` 以匹配宿主机用户，或确保数据目录可写：
