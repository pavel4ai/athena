...
| **OpenAI Codex** | `athena model`（ChatGPT OAuth，使用 Codex 模型） |
...
[Nous Portal](https://futurebnd.com) 是 Futurebound Corp. 的统一订阅网关，也是**运行 Athena Agent 的推荐方式**。一次 OAuth 登录即可访问 300+ 前沿智能体模型（Claude、GPT、Gemini、DeepSeek、Qwen、Kimi、GLM、MiniMax、Grok 等），以及 [Tool Gateway](/user-guide/features/tool-gateway)（网页搜索、图像生成、TTS、浏览器自动化）和 [Nous Chat](https://futurebnd.com)——费用从你的 Nous 订阅中扣除，无需单独管理各提供商账户。
...
还没有订阅？前往 [futurebnd.com](https://futurebnd.com) 购买。
...
如果 token 刷新因终端错误（HTTP 4xx、`invalid_grant`、授权被撤销等）失败，Athena 会将该刷新 token 标记为失效并停止重试，避免出现大量重复的认证失败。下一次请求会显示类型化的重新认证提示。运行 `athena auth add codex-oauth`（或 `athena model` → OpenAI Codex）开始新的设备码登录；成功交换后隔离状态自动解除。
:::
