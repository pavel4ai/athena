...
你需要至少一种方式连接到 LLM。使用 `athena model` 交互式切换提供商和模型，或直接配置：

| 提供商 | 配置方式 |
|----------|-------|
| **Nous Portal** | `athena model`（OAuth，订阅制） |
| **OpenAI Codex** | `athena model` → **ChatGPT or Codex Subscription**（ChatGPT OAuth，使用 Codex 模型） |
| **GitHub Copilot** | `athena model`（OAuth 设备码流程，`COPILOT_GITHUB_TOKEN`、`GH_TOKEN` 或 `gh auth token`） |
| **GitHub Copilot ACP** | `athena model`（在本地生成 `copilot --acp --stdio` 子进程） |
| **Anthropic** | `athena model`（Claude Max + 额外用量积分，通过 OAuth；也支持 Anthropic API key 或手动 setup-token——见下方说明） |
| **OpenRouter** | `~/.athena/.env` 中的 `OPENROUTER_API_KEY` |
| **NovitaAI** | `~/.athena/.env` 中的 `NOVITA_API_KEY`（provider: `novita`，200+ 模型，Model API、Agent Sandbox、GPU Cloud） |
| **AI Gateway** | `~/.athena/.env` 中的 `AI_GATEWAY_API_KEY`（provider: `ai-gateway`） |
| **z.ai / GLM** | `~/.athena/.env` 中的 `GLM_API_KEY`（provider: `zai`） |
| **Kimi / Moonshot** | `~/.athena/.env` 中的 `KIMI_API_KEY`（provider: `kimi-coding`） |
| **Kimi / Moonshot（中国）** | `~/.athena/.env` 中的 `KIMI_CN_API_KEY`（provider: `kimi-coding-cn`；别名：`kimi-cn`、`moonshot-cn`） |
| **Arcee AI** | `~/.athena/.env` 中的 `ARCEEAI_API_KEY`（provider: `arcee`；别名：`arcee-ai`、`arceeai`） |
| **GMI Cloud** | `~/.athena/.env` 中的 `GMI_API_KEY`（provider: `gmi`；别名：`gmi-cloud`、`gmicloud`） |
| **MiniMax** | `~/.athena/.env` 中的 `MINIMAX_API_KEY`（provider: `minimax`） |
| **MiniMax 中国** | `~/.athena/.env` 中的 `MINIMAX_CN_API_KEY`（provider: `minimax-cn`） |
| **xAI（Grok）— Responses API** | `~/.athena/.env` 中的 `XAI_API_KEY`（provider: `xai`） |
| **xAI Grok OAuth（SuperGrok）** | `athena model` → "xAI Grok OAuth (SuperGrok / Premium+)"——浏览器登录，无需 API key。参见[指南](../guides/xai-grok-oauth.md) |
| **Qwen Cloud（阿里 DashScope）** | `~/.athena/.env` 中的 `DASHSCOPE_API_KEY`（provider: `alibaba`） |
| **阿里云（Coding Plan）** | `DASHSCOPE_API_KEY`（provider: `alibaba-coding-plan`，别名：`alibaba_coding`）——独立计费 SKU，不同端点 |
| **Kilo Code** | `~/.athena/.env` 中的 `KILOCODE_API_KEY`（provider: `kilocode`） |
| **小米 MiMo** | `~/.athena/.env` 中的 `XIAOMI_API_KEY`（provider: `xiaomi`，别名：`mimo`、`xiaomi-mimo`） |
| **腾讯 TokenHub** | `~/.athena/.env` 中的 `TOKENHUB_API_KEY`（provider: `tencent-tokenhub`，别名：`tencent`、`tokenhub`、`tencentmaas`） |
| **OpenCode Zen** | `~/.athena/.env` 中的 `OPENCODE_ZEN_API_KEY`（provider: `opencode-zen`） |
| **OpenCode Go** | `~/.athena/.env` 中的 `OPENCODE_GO_API_KEY`（provider: `opencode-go`） |
| **DeepSeek** | `~/.athena/.env` 中的 `DEEPSEEK_API_KEY`（provider: `deepseek`） |
| **Hugging Face** | `~/.athena/.env` 中的 `HF_TOKEN`（provider: `huggingface`，别名：`hf`） |
| **Google / Gemini** | `~/.athena/.env` 中的 `GOOGLE_API_KEY`（或 `GEMINI_API_KEY`）（provider: `gemini`） |
| **LM Studio** | `athena model` → "LM Studio"（provider: `lmstudio`，可选 `LM_API_KEY`） |
| **自定义端点** | `athena model` → 选择"Custom endpoint"（保存在 `config.yaml`） |
...
[Nous Portal](https://portal.nousresearch.com) 是 Nous Research 的统一订阅网关，也是**运行 Athena Agent 的推荐方式**。一次 OAuth 登录即可访问 300+ 前沿智能体模型（Claude、GPT、Gemini、DeepSeek、Qwen、Kimi、GLM、MiniMax、Grok 等）以及 [Tool Gateway](/user-guide/features/tool-gateway)（网页搜索、图像生成、TTS、浏览器自动化）——费用从你的 Nous 订阅中扣除，无需单独管理各提供商账户。

```bash
athena setup --portal     # 全新安装——一条命令完成 OAuth + 提供商 + 网关配置
athena model              # 已有安装——从列表中选择"Nous Portal"
athena portal info        # 随时查看登录状态和路由信息
```

还没有订阅？前往 [portal.nousresearch.com/manage-subscription](https://portal.nousresearch.com/manage-subscription) 购买。

**完整详情：** 参见专属的 [Nous Portal 集成页面](/integrations/nous-portal)（订阅内容、模型目录、故障排查）以及分步指南[使用 Nous Portal 运行 Athena Agent](/guides/run-athena-with-nous-portal)。


:::info Codex 说明
OpenAI Codex 提供商通过设备码（device code）认证——打开一个 URL 并输入验证码。Athena 将生成的凭据存储在 `~/.athena/auth.json` 的自有认证存储中，并在存在 `~/.codex/auth.json` 时可导入现有的 Codex CLI 凭据。无需安装 Codex CLI。

如果 token 刷新因终端错误（HTTP 4xx、`invalid_grant`、授权被撤销等）失败，Athena 会将该刷新 token 标记为失效并停止重试，避免出现大量重复的认证失败。下一次请求会显示类型化的重新认证提示。运行 `athena auth add openai-codex`（或 `athena model` → **ChatGPT or Codex Subscription**）开始新的设备码登录；成功交换后隔离状态自动解除。

在 Python/OpenSSL 3.5+ 上，如果网络中间设备拒绝 X25519MLKEM768 等后量子密钥交换组，设备码登录可能报 `[SSL: UNEXPECTED_EOF_WHILE_READING]` 或 TLS 握手超时（此时 curl 仍可能正常）。Athena 不会修改默认 TLS 策略。请在运行 `athena model` 前将 `OPENSSL_CONF` 指向一个把 `Groups` 限制为经典曲线的配置文件，或用 TLS 1.2 进行诊断：
...
:::warning
即使使用 Nous Portal、Codex 或自定义端点，某些工具（视觉、网页摘要、MoA）仍会使用单独的"辅助"模型。默认情况下（`auxiliary.*.provider: "auto"`），Athena 将这些任务路由到你的**主聊天模型**——即你在 `athena model` 中选择的同一模型。你可以单独覆盖每个任务，将其路由到更便宜/更快的模型（例如 OpenRouter 上的 Gemini Flash）——参见[辅助模型](/user-guide/configuration#auxiliary-models)。
:::

:::tip Nous Tool Gateway
付费 Nous Portal 订阅者还可访问 **[Tool Gateway](/user-guide/features/tool-gateway)**——网页搜索、图像生成、TTS 和浏览器自动化，均通过你的订阅路由。无需额外 API key。全新安装时，`athena setup --portal` 一条命令即可完成登录、设置 Nous 为提供商并开启网关。现有用户可通过 `athena model` 或 `athena tools` 按工具启用。随时使用 `athena portal info` 查看路由状态。
:::
...
| 命令 | 运行位置 | 功能 |
|---------|-------------|--------------|
| **`athena model`** | 终端（任何会话之外） | 完整配置向导——添加提供商、运行 OAuth、输入 API key、配置端点 |
| **`/model`** | Athena 聊天会话内部 | 在**已配置的**提供商和模型之间快速切换 |

如果你想切换到尚未配置的提供商（例如你只配置了 OpenRouter，想使用 Anthropic），需要使用 `athena model`，而不是 `/model`。先退出会话（`Ctrl+C` 或 `/quit`），运行 `athena model`，完成提供商配置，然后开启新会话。
...
通过 `athena model` → Anthropic OAuth（或 `athena auth add anthropic --type oauth`）认证时，Athena 以 Claude Code 身份路由到你的 Anthropic 账户。**仅当你订阅了 Claude Max 计划且购买了额外用量积分时才有效。** Claude Max 基础计划的配额（Claude Code 默认包含的用量）不会被 Athena 消耗——只有你额外购买的超额积分才会被使用。Claude Pro 订阅者无法使用此路径。
...
athena chat --provider anthropic  # 自动读取 Claude Code 凭据文件
```

通过 `athena model` 选择 Anthropic OAuth 时，Athena 优先使用 Claude Code 自身的凭据存储，而不是将 token 复制到 `~/.athena/.env`。这样可以保持 Claude 凭据的可刷新性。
...
如果未找到 token，`athena model` 会提供 **OAuth 设备码登录**——与 Copilot CLI 和 opencode 使用的流程相同。
...
| 类型 | 前缀 | 获取方式 |
|------|--------|------------|
| OAuth token | `gho_` | `athena model` → GitHub Copilot → 使用 GitHub 登录 |
| 细粒度 PAT | `github_pat_` | GitHub 设置 → 开发者设置 → 细粒度 token（需要 **Copilot Requests** 权限） |
| GitHub App token | `ghu_` | 通过 GitHub App 安装获取 |

如果你的 `gh auth token` 返回 `ghp_*` token，请使用 `athena model` 通过 OAuth 认证。
:::
...
| `ATHENA_COPILOT_ACP_COMMAND` | 覆盖 Copilot CLI 二进制路径（默认：`copilot`） |
| `ATHENA_COPILOT_ACP_ARGS` | 覆盖 ACP 参数（默认：`--acp --stdio`） |
...
xAI 通过 Responses API（`codex_responses` 传输）接入，自动支持 Grok 4 模型的推理——无需 `reasoning_effort` 参数，服务端默认进行推理。在 `~/.athena/.env` 中设置 `XAI_API_KEY` 并在 `athena model` 中选择 xAI，或直接用 `grok` 作为快捷方式输入 `/model grok-4-1-fast-reasoning`。

SuperGrok 和 X Premium+ 订阅者可以用浏览器 OAuth 登录，无需 API key——在 `athena model` 中选择 **xAI Grok OAuth (SuperGrok / Premium+)**，或运行 `athena auth add xai-oauth`。同一 OAuth bearer token 会被 xAI 直连工具（TTS、图像生成、视频生成、转录）自动复用。完整流程参见 [xAI Grok OAuth 指南](../guides/xai-grok-oauth.md)——如果 Athena 运行在远程主机上，还需参见 [SSH / 远程主机上的 OAuth](../guides/oauth-over-ssh.md) 了解所需的 `ssh -L` 隧道配置。
...
xAI 还提供专属 TTS 端点（`/v1/tts`）。在 `athena tools` → 语音与 TTS 中选择 **xAI TTS**，或参见[语音与 TTS](../user-guide/features/tts.md#text-to-speech) 页面了解配置。
...
[Ollama Cloud](https://ollama.com/cloud) 托管与本地 Ollama 相同的开源模型目录，无需 GPU。在 `athena model` 中选择 **Ollama Cloud**，粘贴来自 [ollama.com/settings/keys](https://ollama.com/settings/keys) 的 API key，Athena 会自动发现可用模型。
...
阿里巴巴 Qwen Portal，支持基于浏览器的 OAuth 登录。在 `athena model` 中选择 **Qwen OAuth (Portal)**，通过浏览器登录，Athena 会持久化刷新 token。

```bash
athena model
# → 选择"Qwen OAuth (Portal)"
# → 浏览器打开；使用阿里巴巴账户登录
# → 确认——凭据保存到 ~/.athena/auth.json

athena chat   # 使用 portal.qwen.ai/v1 端点
```
...
仅在 portal 端点迁移时才需设置 `ATHENA_QWEN_BASE_URL`（默认：`https://portal.qwen.ai/v1`）。
...
通过浏览器 OAuth 登录使用 MiniMax-M2.7——无需 API key。在 `athena model` 中选择 **MiniMax (OAuth)**，通过浏览器登录，Athena 会持久化访问 token 和刷新 token。底层使用 Anthropic Messages 兼容端点（`/anthropic`）。
...
athena chat   # 使用 api.minimax.io/anthropic 端点
```
...
# 需要：~/.athena/.env 中的 NVIDIA_API_KEY
...
```bash
# GMI Cloud
athena chat --provider gmi --model deepseek-ai/DeepSeek-R1
# 需要：~/.athena/.env 中的 GMI_API_KEY
```
...
# 需要：~/.athena/.env 中的 STEPFUN_API_KEY
```
...
```bash
# 使用任意可用模型
athena chat --provider huggingface --model Qwen/Qwen3-235B-A22B-Thinking-2507
# 需要：~/.athena/.env 中的 HF_TOKEN
...
:::warning 旧版环境变量
`.env` 中的 `OPENAI_BASE_URL` 和 `LLM_MODEL` 已**移除**。Athena 的任何部分都不再读取这两个变量——`config.yaml` 是模型和端点配置的唯一来源。如果你的 `.env` 中有过时条目，下次运行 `athena setup` 或配置迁移时会自动清除。请使用 `athena model` 或直接编辑 `config.yaml`。
:::
...
:::warning athena model 与 /model
**`athena model`**（在终端中运行，任何聊天会话之外）是**完整的提供商配置向导**。用于添加新提供商、运行 OAuth 流程、输入 API key 和配置自定义端点。
...
**添加新提供商：** 退出会话（`Ctrl+C` 或 `/quit`），运行 `athena model`，配置新提供商，然后开启新会话。
:::
...
支持的解析器：`athena`（Qwen 2.5、Athena 2/3）、`llama3_json`（Llama 3.x）、`mistral`、`deepseek_v3`、`deepseek_v31`、`xlam`、`pythonic`。没有这些标志，工具调用将无法工作——模型会将工具调用以文本形式输出。
...
| **vLLM** | 添加 `--enable-auto-tool-choice --tool-call-parser athena` |
...
**可能原因：**
1. **服务器上的输出上限（`max_tokens`）过低** — SGLang 默认每次响应 128 tokens。在服务器上设置 `--default-max-tokens`；Athena 不再提供用户输出上限设置。注意：`max_tokens` 只控制响应长度——与对话历史可以有多长无关（那是 `context_length`）。
...
然后通过 `athena model` → 自定义端点 → `http://localhost:4000/v1` 配置 Athena。
...
然后通过 `athena model` → 自定义端点 → `http://localhost:8402/v1` → 模型名称 `blockrun/auto` 配置 Athena。
...
通过 `athena model` → 自定义端点，或在 `config.yaml` 中配置任意上述服务：
...
**输出上限**限制单次响应，而非对话历史。Athena 不再读取 `model.max_tokens`、`ATHENA_MAX_TOKENS` 或提供商及模型的输出上限设置。兼容端点使用服务器默认值；该值不一定是模型最大值。原生 Anthropic Messages 仍要求 `max_tokens`，Athena 会提供内部值。Bedrock Converse 的可选输出限制默认省略。
...
`athena model` 在配置自定义端点时会提示输入上下文长度。留空则自动检测。
...
custom_providers:
  ...
  - name: work
    ...
    api_mode: chat_completions   # 由 `athena model` → 自定义端点向导显式设置；自动检测仍作为回退
...
`athena model` → 自定义端点向导现在会显式提示 `api_mode` 并将你的答案持久化到 `config.yaml`。当字段留空时，基于 URL 的自动检测（例如 `/anthropic` 路径 → `anthropic_messages`）仍作为回退。
...
也可以从交互式 `athena model` 菜单中选择命名自定义提供商。

---

### 实战配置：Together AI、Groq、Perplexity

[其他兼容提供商](#other-compatible-providers) 中列出的云提供商都使用 OpenAI 的 REST 方言，因此在 `custom_providers:` 下的接入方式相同。以下是三个可直接使用的配置示例。每个示例放入 `~/.athena/config.yaml`，对应的 API key 放入 `~/.athena/.env`。
...
Together 的 `/v1/models` 端点可用，因此 `athena model` 可以自动发现可用模型。
...
:::tip 故障排查
- `athena doctor` 对于上述任何名称都不应打印 `Unknown provider` 警告（在 #15083 的 CLI 验证器修复之后）。
- 如果某个提供商的 `/v1/models` 端点不可达（Perplexity 是常见情况），`athena model` 会在警告后持久化模型而不是硬性拒绝——参见 #15136。
...
:::tip
可以随时使用 `athena model` 切换提供商——无需重启。无论使用哪个提供商，你的对话历史、记忆和技能都会保留。
:::
...
使用 OpenRouter 时，可以控制请求如何在提供商之间路由。在 `~/.athena/config.yaml` 中添加 `provider_routing` 节：
...
OpenRouter 提供一个实验性编程模型路由器 `openrouter/pareto-code`，自动将请求路由到满足编程质量标准的最便宜模型（按 [Artificial Analysis](https://artificialanalysis.ai/) 排名）。选择此模型并在 `~/.athena/config.yaml` 中调整 `min_coding_score` 参数：
...
:::tip
故障转移仅通过 `config.yaml` 配置——或通过 `athena fallback` 交互式配置。有关触发时机、链推进方式以及与辅助任务和委托的交互，参见[故障转移提供商](/user-guide/features/fallback-providers)。
:::
