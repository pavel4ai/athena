...
| 目标 | 先做这步 | 再做这步 |
|---|---|---|
| 只想让 Athena 在本机跑起来 | `athena setup` | 运行一次真实对话并验证有响应 |
| 已知道要用哪个 provider | `athena model` | 保存配置，然后开始聊天 |
| 想搭建机器人或长期运行的服务 | CLI 正常后运行 `athena gateway setup` | 接入 Telegram、Discord、Slack 或其他平台 |
| 想使用本地或自托管模型 | `athena model` → 自定义 endpoint | 验证 endpoint、模型名称和上下文长度 |
| 想要多 provider 故障转移 | 先运行 `athena model` | 基础对话正常后再添加路由和故障转移 |
...
### 在 macOS 或 Windows 上使用 Athena Desktop 安装器（推荐）

如需同时安装命令行与桌面应用，请从我们的官网[下载 Athena Desktop 安装器](https://athena-agent.nousresearch.com/)并运行。
...
安装脚本会在 `~/.athena/athena-agent` 创建一个受管理的隔离环境（独立的 uv 托管解释器和 venv），这是唯一受支持的安装方式 —— 包括开发用途。请勿使用 `pip install athena-agent`。
...
这是最重要的配置步骤。使用 `athena model` 以交互方式完成选择：
...
| Provider | 说明 | 配置方式 |
|----------|-----------|---------------|
| **Nous Portal** | 订阅制，零配置 | 通过 `athena model` 进行 OAuth 登录 |
| **OpenAI Codex** | ChatGPT 或 Codex 订阅，使用 Codex 模型 | 通过 `athena model` → **ChatGPT or Codex Subscription** 进行设备码认证 |
| **Anthropic** | 直接使用 Claude 模型——Max 计划 + 额外用量积分（OAuth），或按 token 付费的 API key | `athena model` → OAuth 登录（需要 Max + 额外积分），或 Anthropic API key |
...
| **MiniMax (OAuth)** | 通过浏览器 OAuth 使用 MiniMax-M2.7，无需 API key | `athena model` → MiniMax (OAuth) |
...
| **GitHub Copilot** | GitHub Copilot 订阅（GPT-5.x、Claude、Gemini 等） | 通过 `athena model` 进行 OAuth，或设置 `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent 后端（在本地启动 `copilot` CLI） | `athena model`（需要 `copilot` CLI + `copilot login`） |
...
:::tip
你可以随时通过 `athena model` 切换 provider——没有锁定。所有支持的 provider 完整列表及配置详情，请参阅 [AI Providers](../integrations/providers.md)。
:::
...
- **密钥和 token** → `~/.athena/.env`
- **非密钥配置** → `~/.athena/config.yaml`
...
```bash
athena            # 经典 CLI
athena --tui      # 现代 TUI（推荐）
```

你会看到一个欢迎横幅，显示你的模型、可用工具和 skills。使用一个具体且易于验证的 prompt（提示词）：

:::tip 选择你的界面
Athena 提供两种终端界面：经典的 `prompt_toolkit` CLI，以及更新的 [TUI](../user-guide/tui.md)（支持模态覆盖层、鼠标选择和非阻塞输入）。两者共享相同的会话、斜杠命令和配置——分别用 `athena` 和 `athena --tui` 试试看。
:::
...
- `athena tools` — 按平台调整工具访问权限
- `athena skills` — 浏览并安装可复用的工作流
...
```bash
athena config set terminal.backend docker    # Docker 隔离
athena config set terminal.backend ssh       # 远程服务器
```
...
# ~/.athena/athena-agent，在 Windows 上为 %LOCALAPPDATA%\athena\athena-agent）：
...
```bash
athena skills search kubernetes
...
（如果安装时未包含 `[all]`，请先运行 `cd ~/.athena/athena-agent && uv pip install -e ".[acp]"`。）
...
| 现象 | 可能原因 | 解决方法 |
|---|---|---|
| Athena 启动但回复为空或异常 | Provider 认证或模型选择有误 | 重新运行 `athena model`，确认 provider、模型和认证信息 |
| 自定义 endpoint "可用"但返回乱码 | base URL、模型名称有误，或实际上不兼容 OpenAI | 先用独立客户端验证该 endpoint |
| Gateway 启动但无法收到消息 | Bot token、白名单或平台配置不完整 | 重新运行 `athena gateway setup` 并检查 `athena gateway status` |
| `athena --continue` 找不到旧会话 | 切换了 profile 或会话从未保存 | 检查 `athena sessions list`，确认你在正确的 profile 下 |
| 模型不可用或出现异常的故障转移行为 | Provider 路由或故障转移设置过于激进 | 在基础 provider 稳定之前关闭路由 |
| `athena doctor` 标记配置问题 | 配置值缺失或已过期 | 修复配置，在添加功能前重新测试普通对话 |
...
|---------|-------------|
| `athena` | 开始聊天 |
| `athena model` | 选择 LLM provider 和模型 |
| `athena tools` | 配置每个平台启用的工具 |
| `athena setup` | 完整配置向导（一次性配置所有内容） |
| `athena doctor` | 诊断问题 |
| `athena update` | 更新到最新版本 |
| `athena gateway` | 启动消息 gateway |
| `athena --continue` | 恢复上次会话 |
