...
- 将 `athena` 链接到 `$PREFIX/bin`，使其保留在 Termux PATH 中
...
### 5. 将 `athena` 添加到 Termux PATH

```bash
ln -sf "$PWD/venv/bin/athena" "$PREFIX/bin/athena"
```

`$PREFIX/bin` 在 Termux 中已默认在 PATH 中，因此这样做可以让 `athena` 命令在新 shell 中持续可用，无需每次重新激活虚拟环境。
...
### `athena doctor` 提示缺少 ripgrep 或 Node
