...
- **boto3** — 通过 `cd ~/.athena/athena-agent && uv pip install -e ".[bedrock]"` 安装
- **IAM 权限** — 至少需要：
...
运行 `athena model` 后，你的 `~/.athena/config.yaml` 将包含以下内容：
...
Bedrock 模型使用**推理配置文件 ID** 进行按需调用。`athena model` 选择器会自动显示这些 ID，并将推荐模型置于顶部：
