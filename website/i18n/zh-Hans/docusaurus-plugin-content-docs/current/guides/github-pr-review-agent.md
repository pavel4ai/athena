...
:::tip 没有消息通知？没关系
使用 `deliver: "local"` 将审查结果保存到 `~/.athena/cron/output/`。在接入通知之前用于测试非常方便。
:::
...
创建 `~/.athena/skills/code-review/SKILL.md`：
...
验证是否已加载——启动 `athena`，你应该能在启动时的 skill 列表中看到 `code-review`。
...
- **[构建 Plugin](/guides/build-a-athena-plugin)** — 将审查逻辑封装为可共享的 plugin
