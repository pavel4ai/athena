...
- Athena Agent 已安装并运行（`athena gateway`）
...
在你的 `~/.athena/config.yaml` 中添加以下内容：
...
:::note Payload 中不包含代码
GitHub webhook payload 包含 PR 元数据（标题、描述、分支名、URL），但**不包含 diff**。上方的 prompt 指示 agent 运行 `gh pr diff` 来获取实际变更。`terminal` 工具已包含在默认的 `athena-webhook` 工具集中，无需额外配置。
:::
...
:::note
`athena webhook test <name>` 仅适用于通过 `athena webhook subscribe` 创建的**动态订阅**。它不读取 `config.yaml` 中的路由。
:::
...
- **[构建 Plugin](/guides/build-a-athena-plugin)** —— 将审查逻辑打包为可共享的 plugin
