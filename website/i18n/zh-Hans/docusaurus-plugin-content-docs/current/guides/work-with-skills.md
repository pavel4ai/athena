...
关于如何在自己的插件中捆绑 skills，请参见 [构建 Athena 插件 → 捆绑 skills](/guides/build-a-athena-plugin#bundle-skills)。
...
# 查看所有 skill 配置
athena config get skills.config
```
...
Skill 会自动出现——无需注册。放入 `~/.athena/skills/` 即可立即生效。
...
**使用分类目录。** 将 skills 整理到子目录中（`~/.athena/skills/devops/`、`~/.athena/skills/research/` 等），保持列表整洁，并帮助 agent 更快找到相关 skills。
