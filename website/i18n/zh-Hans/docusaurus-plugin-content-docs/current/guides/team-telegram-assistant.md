...
- **已配置 LLM 提供商**——至少在 `~/.athena/.env` 中配置了 OpenAI、Anthropic 或其他受支持提供商的 API 密钥
...
2. **发送 `/newbot`**——BotFather 会询问两件事：
   ...
   - **用户名**——必须以 `bot` 结尾（例如 `myteam_athena_bot`）
...
sudo athena gateway install --system   # 仅 Linux：开机启动的系统服务
```
...
:::tip macOS PATH
launchd plist 在安装时捕获你的 Shell PATH，以便 gateway 子进程能找到 Node.js 和 ffmpeg 等工具。如果之后安装了新工具，请重新运行 `athena gateway install` 以更新 plist。
:::
...
控制机器人在使用工具时显示的详细程度。在 `~/.athena/config.yaml` 中：
...
通过编辑 `~/.athena/SOUL.md` 自定义机器人的沟通方式：

完整指南请参阅[在 Athena 中使用 SOUL.md](/guides/use-soul-with-athena)。
...
```bash
# 通过 CLI
athena cron list          # 查看所有定时任务
athena cron status        # 检查调度器是否运行
...
| 内容 | 位置 |
|------|----------|
| Gateway 日志 | `journalctl --user -u athena-gateway`（Linux）或 `~/.athena/logs/gateway.log`（macOS） |
| Cron 任务输出 | `~/.athena/cron/output/{job_id}/{timestamp}.md` |
| Cron 任务定义 | `~/.athena/cron/jobs.json` |
| 配对数据 | `~/.athena/pairing/` |
| 会话历史 | `~/.athena/sessions/` |
