...
上次会话有遗漏？运行 `athena -c` 可精确恢复到上次离开的位置，完整对话历史全部还原。也可以按标题恢复：`athena -r "my research project"`。
...
想让 Athena 拥有稳定的默认风格？编辑 `~/.athena/SOUL.md`（如果使用自定义 Athena home，则为 `$ATHENA_HOME/SOUL.md`）。Athena 现在会自动生成一个初始 SOUL 文件，并将该全局文件作为实例级个性来源。

完整说明请参阅 [在 Athena 中使用 SOUL.md](/guides/use-soul-with-athena)。
...
用 `/title auth-refactor` 或 `/title research-llm-quantization` 为会话命名。命名后的会话可通过 `athena sessions list` 轻松找到，并用 `athena -r "auth-refactor"` 恢复。未命名的会话会堆积起来，难以区分。

### DM 配对实现团队访问

不要手动收集用户 ID 来维护白名单，而是启用 DM 配对。当团队成员向 bot 发送私信时，他们会收到一次性配对码。你用 `athena pairing approve telegram XKGH5N7P` 批准即可——简单且安全。
...
:::tip
在消息平台上，会话会在空闲一段时间后自动重置（默认 24 小时），或每天凌晨 4 点重置。如需更长的会话时间，可在 `~/.athena/config.yaml` 中按平台调整。
:::
