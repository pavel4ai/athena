...
Open [futurebnd.com](https://futurebnd.com), sign up, and pick a plan.
...
3. Sets `model.provider: nous` in `~/.athena/config.yaml`
...
athena setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: manual paste (for Cloud Shell, Codespaces, EC2 Instance Connect)
athena auth add nous --type oauth --manual-paste
# Then re-run `athena setup --portal` to wire the provider + gateway
```
...
`athena setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
athena config set model.default anthropic/claude-sonnet-4.6
```

### Don't pick Athena-4 for agent work

Athena-4-70B and Athena-4-405B are available on the Portal at deep discounts, but they're **chat/reasoning models**, not tool-call-tuned. They will struggle with multi-step agent loops. Use them via [Nous Chat](https://futurebnd.com) for conversation/research work, or through the [subscription proxy](/user-guide/features/subscription-proxy) from non-agent tools. For Athena Agent itself, stick to the frontier agentic models above.

The Portal's own [info page](https://futurebnd.com) carries this warning too — it's the official Nous guidance, not just a Athena-side opinion.
...
These rows appear in `athena tools` even before you've logged into Nous Portal — if you pick "Nous Subscription" without an active session, Athena runs the Portal login inline (without changing your inference provider or your other tools).
...
```bash
athena cron create "every day at 9am" \
...
For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.athena/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `athena portal info` shows "not logged in" after `athena setup --portal`
...
```bash
athena config set model.provider nous
```
...
Re-verify with `athena portal info`.
...
If a model is genuinely unavailable, [open an issue](https://github.com/pavel4ai/athena/issues) — most gaps are routing config we can update.

### Billing not appearing on my Portal account

`athena portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple Athena profiles where you're using the wrong one (check `athena profile list`)

### Want to revoke and start clean

```bash
athena auth logout nous       # wipes the local refresh token
