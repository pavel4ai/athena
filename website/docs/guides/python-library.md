...
Clone Athena and create its supported editable development environment:
...
Run your application with `uv run python your_app.py` from that checkout. Athena does not publish a supported wheel or source distribution for `requirements.txt` installs.
...
|-----------|------|---------|-------------|
| `model` | `str` | `""` | Model in OpenRouter format (defaults to empty; resolved from your athena config at runtime) |
