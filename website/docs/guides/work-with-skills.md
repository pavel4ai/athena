...
athena skills install official/research/arxiv
...
What happens:
1. The skill directory is copied to `~/.athena/skills/`
...
For how to ship skills in your own plugin, see [Build a Athena Plugin → Bundle skills](/guides/build-a-athena-plugin#bundle-skills).
...
athena skills config gif-search

# View all skill config
athena config show | grep '^skills\.config'
```
...
```bash
mkdir -p ~/.athena/skills/my-category/my-skill
```

### 2. Write SKILL.md

```markdown title="~/.athena/skills/my-category/my-skill/SKILL.md"
---
name: my-skill
...
```bash
athena chat -q "/my-skill help me with the thing"
```

The skill appears automatically — no registration needed. Drop it in `~/.athena/skills/` and it's live.
...
**Use categories.** Organize skills into subdirectories (`~/.athena/skills/devops/`, `~/.athena/skills/research/`, etc.). This keeps the list manageable and helps the agent find relevant skills faster.
