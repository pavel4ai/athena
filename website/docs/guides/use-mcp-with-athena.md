...
```bash
athena mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
athena mcp test chrome-devtools-win
```
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want Athena to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. Athena remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
athena mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
athena mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `athena mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want Athena to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize Athena to merge, publish, deploy, or spawn runtimes.
