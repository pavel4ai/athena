...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `athena send`."
---

# Pipe Script Output to Messaging Platforms

`athena send` is a small, scriptable CLI that pushes a message to any
...
- Piping any tool's output anywhere (`make | athena send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `athena
...
`athena send` resolves the message body in this order:

1. **Positional argument** — `athena send --to telegram "hi"`
2. **`--file PATH`** — `athena send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | athena send --to telegram`
...
Because `athena send` reuses your Athena config, the same script works on
...
`athena send`. If the Python interpreter can't load because the box is
...
## Does `athena send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `athena send` calls
the platform's REST endpoint directly using credentials from
`~/.athena/.env` and `~/.athena/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `athena gateway start` and retry.
...
The listing is built from `~/.athena/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`athena gateway
...
|----------|----------------|---------------------|---------------|----------|
| `athena send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`athena send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `athena send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `athena send` shares with cron delivery.
