---
sidebar_position: 26
title: "Running Athena on a Personal or Work Machine"
description: "A security-posture walkthrough for running Athena Agent on the machine you live on — what the defaults protect, how to tighten further, and how to undo mistakes"
---

# Running Athena on a Personal or Work Machine
...
**Your data goes only where you point it.** API calls go **only to the LLM provider you configure**. Athena Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.athena/`. See the [FAQ](/reference/faq#is-my-data-sent-anywhere).
...
`ATHENA_WRITE_SAFE_ROOT` restricts `write_file` and `patch` to the directory prefix(es) you list — anything outside is hard-blocked. Multiple roots are separated by `:` on Unix:
...
:::caution
Don't add this to `~/.athena/.env` casually. If you set it to a project directory only, the agent cannot write to `~/.athena/cron/jobs.json`, profile skills, or other Athena state outside that prefix. Include your Athena home as a second root, as above.
:::
...
For `ssh`, set `terminal.backend: ssh` in `config.yaml` and provide host details via `TERMINAL_SSH_HOST`, `TERMINAL_SSH_USER`, and `TERMINAL_SSH_KEY` in `~/.athena/.env`. See [Network Isolation](/user-guide/security#network-isolation).
...
Or use DM pairing instead of hardcoding IDs: unknown users receive a one-time pairing code, and you approve them from the CLI with `athena pairing approve <platform> <code>`. Never set `GATEWAY_ALLOW_ALL_USERS=true` on a machine you care about.

## The Undo Layer: Checkpoints and `/rollback`

Approval gates prevent damage; [checkpoints](/user-guide/checkpoints-and-rollback) reverse it. When enabled, Athena automatically snapshots your project before destructive operations — `write_file`, `patch`, and destructive terminal commands like `rm`, `mv`, `sed -i`, and `git reset` — into a shadow git store under `~/.athena/checkpoints/store/`. Your real project `.git` is never touched.
...
:::tip
Preview with `/rollback diff <N>` before restoring, and combine checkpoints with git worktrees for maximum safety — each Athena session in its own worktree, with checkpoints as an extra layer.
:::
...
Everything above, assembled. Adjust to taste in `~/.athena/config.yaml`:
...
And in `~/.athena/.env`, if you want the write sandbox:

```bash
ATHENA_WRITE_SAFE_ROOT=/path/to/project:/home/you/.athena
```
