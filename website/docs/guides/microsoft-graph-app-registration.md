...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.athena/.env` at the end.
...
3. **Description:** `athena-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
Put the three values you collected into `~/.athena/.env`:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.athena/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `athena gateway restart`.
