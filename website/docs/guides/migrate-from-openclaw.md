...
`athena claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into Athena. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.

:::note
Coming from **Claude Code** or **OpenAI Codex CLI** instead? Use [`athena import-agent`](../user-guide/import-from-other-agents.md).
:::

:::tip
If your OpenClaw setup was multi-provider, `athena setup --portal` collapses it to one OAuth — 300+ models plus the Tool Gateway in a single login. See [Nous Portal](/integrations/nous-portal).
:::
...
athena claw migrate --preset full --migrate-secrets --yes
```
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.athena/` (by default a single restore-point archive is written before apply, under `~/.athena/backups/pre-migration-*.zip`; restorable with `athena import`). |
...
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.athena/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.athena/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.athena/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.athena/memories/MEMORY.md` | All daily files merged into main memory. |
...
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.athena/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.athena/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.athena/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.athena/skills/openclaw-imports/` |
...
| Custom providers | `models.providers.*` | `config.yaml` → `custom_providers` (auto-migrated to the canonical `providers:` dict on the next `athena update` config migration) | Maps `baseUrl`, `apiType`/`api` — handles both short ("openai", "anthropic") and hyphenated ("openai-completions", "anthropic-messages", "google-generative-ai") values |
| Provider API keys | `models.providers.*.apiKey` | `~/.athena/.env` | Requires `--migrate-secrets`. See [API key resolution](#api-key-resolution) below. |
...
Idle and daily reset timers are not imported: Athena conversations persist until an explicit `/new` or `/reset`. Advanced session settings (identity links, thread bindings, maintenance, scope and send policy) remain archived for reference.
...
| TTS assets | `~/.athena/tts/` (file copy) |
...
| Gateway auth token | `gateway.auth.token` | `.env` → `ATHENA_GATEWAY_TOKEN` | Requires `--migrate-secrets` |
...
These are saved to `~/.athena/migration/openclaw/<timestamp>/archive/` for manual review:
...
| Cron jobs | `archive/cron-config.json` | Recreate with `athena cron create` |
| Plugins | `archive/plugins-config.json` | See [plugins guide](/user-guide/features/hooks) |
| Hooks/webhooks | `archive/hooks-config.json` | Use `athena webhook` or gateway hooks |
| Memory backend | `archive/memory-backend-config.json` | Configure via `athena honcho` |
| Skills registry | `archive/skills-registry-config.json` | Use `athena skills config` |
...
2. **Review archived files** — anything in `~/.athena/migration/openclaw/<timestamp>/archive/` needs manual attention.

3. **Start a new session** — imported skills and memory entries take effect in new sessions, not the current one.

4. **Verify API keys** — run `athena status` to check provider authentication.

5. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart athena-gateway`

6. **Check session archives** — review archived advanced settings; idle and daily reset timers are intentionally not imported.

7. **Re-pair WhatsApp** — WhatsApp uses QR code pairing (Baileys), not token migration. Run `athena whatsapp` to pair.

8. **Archive cleanup** — after confirming everything works, run `athena claw cleanup` to rename leftover OpenClaw directories to `.pre-migration/` (prevents state confusion).
...
Keys might be stored in several places depending on your OpenClaw version: inline in `openclaw.json` under `models.providers.*.apiKey`, in `~/.openclaw/.env`, in the `openclaw.json` `"env"` sub-object, or in `agents/main/agent/auth-profiles.json`. The migration checks all four. If keys use `source: "file"` or `source: "exec"` SecretRefs, they can't be resolved automatically — add them via `athena config set`.

### Skills not appearing after migration

Imported skills land in `~/.athena/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.

### TTS voice not migrated

OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `athena config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.
