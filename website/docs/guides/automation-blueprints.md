...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`athena webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `athena webhook subscribe`) |
...
```bash
athena cron create "0 2 * * *" \
  "You are a project manager triaging the pavel4ai/athena GitHub repo.

1. Run: gh issue list --repo pavel4ai/athena --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
athena webhook subscribe github-pr-review \
  --events "pull_request" \
...
athena cron create "0 9 * * 1" \
  "Scan the pavel4ai/athena repo for documentation drift.

1. Run: gh pr list --repo pavel4ai/athena --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (athena_cli/commands.py, athena_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (athena_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
athena cron create "0 6 * * *" \
  "Run a dependency security audit on the athena-agent project.

1. cd ~/.athena/athena-agent && source .venv/bin/activate
...
```bash
athena webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
athena webhook subscribe alert-triage \
...
```python title="~/.athena/scripts/check-uptime.py"
...
athena cron create "every 30m" \
  ...
  --script ~/.athena/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
athena webhook subscribe auto-port \
  --events "pull_request" \
