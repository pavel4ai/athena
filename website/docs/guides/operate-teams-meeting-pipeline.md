...
```bash
athena teams-pipeline validate
```
...
```bash
athena teams-pipeline token-health
athena teams-pipeline token-health --force-refresh
```
...
```bash
athena teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
athena teams-pipeline maintain-subscriptions
athena teams-pipeline maintain-subscriptions --dry-run
```
...
Athena ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.athena/scripts/`. First create the script:

```bash
mkdir -p ~/.athena/scripts
cat > ~/.athena/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec athena teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.athena/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
athena cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/athena-teams-pipeline-maintain.service`:
...
Type=oneshot
User=athena
EnvironmentFile=/etc/athena/env
ExecStart=/usr/local/bin/athena teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/athena-teams-pipeline-maintain.timer`:
...
sudo systemctl enable --now athena-teams-pipeline-maintain.timer
systemctl list-timers athena-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/athena teams-pipeline maintain-subscriptions >> /var/log/athena/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.athena/.env` at the top of a wrapper script that crontab calls.
...
```bash
athena teams-pipeline subscriptions   # should show expirationDateTime advanced
athena teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
athena teams-pipeline list
athena teams-pipeline list --status failed
athena teams-pipeline show <job-id>
```

### Replay a stored job

```bash
athena teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
athena teams-pipeline fetch --meeting-id <meeting-id>
athena teams-pipeline fetch --join-web-url "<join-url>"
athena teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `athena teams-pipeline maintain-subscriptions --dry-run`
- inspect `athena teams-pipeline list --status failed`
...
- run `athena teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `athena teams-pipeline run`
...
- [ ] `athena teams-pipeline validate` returns an OK snapshot
- [ ] `athena teams-pipeline token-health --force-refresh` succeeds
