...
After the first login, credentials are stored under `~/.athena/auth.json` and are refreshed automatically before each session.
...
```bash
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/.athena/.env
```
...
```bash
athena auth add minimax-oauth --no-browser
```
...
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.athena/auth.json` under the `minimax-oauth` key.
...
```bash
athena config set model.default MiniMax-M2.7
athena config set model.provider minimax-oauth
```

## Configuration Reference

After login, `~/.athena/config.yaml` will contain entries similar to:
...
```bash
athena --provider minimax-oauth    # canonical
athena --provider minimax-portal   # alias
athena --provider minimax-global   # alias
athena --provider minimax_oauth    # alias (underscore form)
```
...
To use `minimax-oauth` as the active provider, set `model.provider: minimax-oauth` in `config.yaml` (use `athena setup` for the guided flow), or pass `--provider minimax-oauth` for a single invocation:

```bash
athena --provider minimax-oauth
```
...
**Fix:** run `athena auth add minimax-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, Athena raises a timeout error.

**Fix:** re-run `athena auth add minimax-oauth` (or `athena model`). The flow starts fresh.
...
If `athena` cannot open a browser window, use `--no-browser`:
...
**Fix:** run `athena model` and select MiniMax (OAuth), or run `athena auth add minimax-oauth`.
...
```bash
athena auth logout minimax-oauth
```
...
- [athena doctor](../reference/cli-commands.md)
