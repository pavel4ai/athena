...
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.athena/.env`
...
2. **Send `/newbot`** — BotFather will ask you two things:
   ...
   - **Username** — must end in `bot` (e.g., `myteam_athena_bot`)
...
# View live logs
journalctl --user -u athena-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `athena gateway install` to update the plist.
:::
...
```bash
athena gateway stop && athena gateway start
```
...
athena pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.athena/.env`:
...
Control how much detail the bot shows when using tools. In `~/.athena/config.yaml`:
...
Customize how the bot communicates by editing `~/.athena/SOUL.md`:

For a full guide, see [Use SOUL.md with Athena](/guides/use-soul-with-athena).
...
```bash
# From the CLI
athena cron list          # View all scheduled jobs
athena cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u athena-gateway` (Linux) or `~/.athena/logs/gateway.log` (macOS) |
| Cron job output | `~/.athena/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.athena/cron/jobs.json` |
| Pairing data | `~/.athena/pairing/` |
| Session history | `~/.athena/sessions/` |
