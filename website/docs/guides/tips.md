...
Run `athena setup --portal` — you get 300+ models including Claude, GPT-5, and Gemini under one subscription. See [Nous Portal](/integrations/nous-portal).
:::
...
Forgot something from your last session? Run `athena -c` to resume exactly where you left off, with full conversation history restored. You can also resume by title: `athena -r "my research project"`.
...
Want Athena to have a stable default voice? Edit `~/.athena/SOUL.md` (or `$ATHENA_HOME/SOUL.md` if you use a custom Athena home). Athena now seeds a starter SOUL automatically and uses that global file as the instance-wide personality source.

For a full walkthrough, see [Use SOUL.md with Athena](/guides/use-soul-with-athena).
...
:::tip
Run `/usage` periodically to see your token consumption. Run `/insights` for a broader view of usage patterns over the last 30 days. To see what your *fixed* per-message cost is before any conversation — system prompt, skills index, memory, tool schemas — run [`athena prompt-size`](/reference/cli-commands#athena-prompt-size) (works offline).
:::
...
Name your sessions with `/title auth-refactor` or `/title research-llm-quantization`. Named sessions are easy to find with `athena sessions list` and resume with `athena -r "auth-refactor"`. Unnamed sessions pile up and become impossible to distinguish.

### DM Pairing for Team Access

Instead of manually collecting user IDs for allowlists, enable DM pairing. When a teammate DMs the bot, they get a one-time pairing code. You approve it with `athena pairing approve telegram XKGH5N7P` — simple and secure.
...
# In your .env:
TERMINAL_ENV=docker
TERMINAL_DOCKER_IMAGE=athena-sandbox:latest
```
