...
- **Gateway running** for cron jobs:
  ```bash
  athena gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.athena/cron/output/`. Great for testing before wiring up notifications.
:::
...
```
Run: gh pr list --repo pavel4ai/athena --state open --limit 3
```
...
Run: gh pr diff 3888 --repo pavel4ai/athena
```
...
```bash
mkdir -p ~/.athena/skills/code-review
```

Create `~/.athena/skills/code-review/SKILL.md`:
...
Verify it loaded — start `athena` and you should see `code-review` in the skills list at startup.
...
```bash
athena gateway status    # Is the gateway running?
athena cron list         # Is the job enabled?
```
...
- **[Build a Plugin](/guides/build-a-athena-plugin)** — wrap the review logic into a shareable plugin
