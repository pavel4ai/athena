...
The GitHub webhook payload includes PR metadata (title, description, branch names, URLs) but **not the diff**. The prompt above instructs the agent to run `gh pr diff` to fetch the actual changes. The `terminal` tool is included in the default `athena-webhook` toolset, so no extra configuration is needed.
:::
...
- **[Build a Plugin](/guides/build-a-athena-plugin)** — package review logic into a shareable plugin
