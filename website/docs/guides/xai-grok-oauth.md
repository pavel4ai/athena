...
```bash
athena auth add xai-oauth --manual-paste
# Or via the model picker:
athena model --manual-paste
```

See [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md#browser-only-remote-cloud-shell--codespaces--ec2-instance-connect) for the full walkthrough. Regression fix for [#26923](https://github.com/pavel4ai/athena/issues/26923).
...
3. xAI redirects back to Athena and the tokens are saved to `~/.athena/auth.json`.
...
```bash
athena config set model.default grok-build-0.1
...
This is **not** a stale-token problem — re-running `athena model` won't change it. xAI's backend has been seen to restrict OAuth API access to specific SuperGrok tiers despite the in-app subscription being active (issue [#26847](https://github.com/pavel4ai/athena/issues/26847)).
