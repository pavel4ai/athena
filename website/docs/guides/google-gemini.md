...
sidebar_position: 16
title: "Google Gemini"
description: "Use Athena Agent with Google Gemini — native AI Studio API, API-key setup, tool calling, streaming, and quota guidance"
---
...
echo "GOOGLE_API_KEY=..." >> ~/.athena/.env
...
After running `athena model`, your `~/.athena/config.yaml` will contain:
...
The `athena model` picker shows Gemini models maintained in Athena' provider registry. Common choices include:
...
Model availability changes over time. If a model disappears or is not enabled for your key, run `athena model` again and pick one from the current list.
...
Google publishes moving aliases for the Pro and Flash Gemini families. `gemini-pro-latest` and `gemini-flash-latest` are useful when you want Google to advance the model automatically without changing your Athena config. Note that your usage charges may be affected if newer models introduce different rates.
...
If you have not configured Gemini yet, exit the session and run `athena model` first. `/model` switches among already-configured providers and models; it does not collect new API keys.
...
Athena could not find a usable API key. Add one of these to `~/.athena/.env`:
...
The selected model is not available for your account, region, or key. Run `athena model` again and pick another Gemini model from the current list.

### Gemma model is not shown in `athena model`

Athena may hide low-throughput Gemma models from the picker by default. If you intentionally want to evaluate one, set the model ID directly in `~/.athena/config.yaml`.
...
Upgrade Athena and rerun `athena model`. The native Gemini adapter sanitizes tool schemas for Gemini's stricter function-declaration format; older builds or custom endpoints may not.
