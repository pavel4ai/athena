...
```bash
# ~/.athena/.env
ATHENA_API_TIMEOUT=1800   # 30 minutes — generous for slow local models
```
:::
...
Alternatively, edit `~/.athena/config.yaml` directly:
...
2. Add to your `~/.athena/config.yaml`:
...
Athena sends a fixed payload on every API call — the system prompt plus the tool schemas for all enabled tools — before any of your conversation content. On CPU-only or low-VRAM setups, processing that prompt (the *prefill* phase) dominates the first turn: the model can sit silent for minutes while it works through the prompt, then generate at its normal pace. This is expected behaviour, not a hang. The [Mac local-LLM guide](./local-llm-on-mac.md#timeouts) documents the same effect — during prefill on large contexts, local models may produce no output for minutes while processing the prompt — and Athena automatically raises its stream read timeout from 120s to 1800s for local endpoints (`ATHENA_STREAM_READ_TIMEOUT`).
...
- **Widen the API timeout** — set `ATHENA_API_TIMEOUT=1800` in `~/.athena/.env` (see [What You Need](#what-you-need)).
- **Measure and trim the fixed prompt** — run `athena prompt-size` for a byte breakdown of the system prompt and tool schemas, then disable unused toolsets with `athena tools` and uninstall skills you don't need with `athena skills`.
