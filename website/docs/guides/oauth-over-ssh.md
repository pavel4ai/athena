...
description: "How to complete browser-based OAuth (Spotify, MCP servers) when Athena runs on a remote machine, container, or behind a jump box"
---

# OAuth over SSH / Remote Hosts

Some Athena providers — **Spotify** and **remote MCP servers** (Linear, Sentry, Atlassian, Asana, Figma, …) — use a *loopback redirect* OAuth flow. The auth server redirects your browser to `http://127.0.0.1:<port>/callback` so a tiny HTTP listener started by Athena can grab the authorization code.
...
**xAI Grok OAuth (`xai-oauth`) uses OAuth device code**, not a loopback callback — open the printed verification URL in any browser and Athena polls until approval. No SSH tunnel is required. See [xAI Grok OAuth](./xai-grok-oauth.md).
...
athena auth spotify --no-browser
...
Athena prints the exact port it bound to on the `Waiting for callback on ...` line — copy it from there. Spotify defaults to port `43827`.

## Which Providers Need This

| Provider | Loopback port | Tunnel needed? |
|----------|---------------|----------------|
| Spotify | `43827` (default) | Yes, when Athena is remote |
| MCP servers (`auth: oauth`) | auto-picked per server | Yes, when Athena is remote (or paste redirect URL) |
...
Remote MCP servers (Linear, Sentry, Atlassian, Asana, Figma, etc.) use the same loopback redirect flow. Athena auto-picks a free port per server and prints the authorize URL when the OAuth flow kicks off — either at startup (when a new server appears in `mcp_servers:`) or when you run `athena mcp login <server>`.
...
**Option 2 — SSH port forward (same as Spotify).** Athena prints the exact port it bound to in the SSH-session hint. Open a separate terminal on your laptop:
...
**Pitfall — the 30s config-reload race.** If you edit `~/.athena/config.yaml` to add an OAuth MCP server from inside a running Athena session, the CLI auto-reloads MCP connections with a 30s timeout. That's not enough time to complete an interactive OAuth flow, and the reload will give up. Use `athena mcp login <server>` from a fresh terminal instead — it has no such cap and waits the full 5 min for you to paste back.
...
The redirect never made it back to the remote listener. Check the tunnel is still alive (`ssh -N` doesn't show output, so look at the terminal you started it from), confirm you used the port from the latest `Waiting for callback on ...` line (Athena may auto-bump if the preferred port is busy), restart the tunnel if needed, and re-run the auth command.

### Tokens land in the wrong `~/.athena`

The tokens are written under the Linux user that ran `athena auth add ...`. If your gateway / systemd service runs as a different user (e.g. `root` or a dedicated `athena` user), authenticate as **that** user so the tokens land in their `~/.athena/auth.json`. `sudo -u athena -i` or equivalent.
