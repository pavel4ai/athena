...
If you're expecting jobs to fire automatically, you need a running gateway (`athena gateway` for foreground, or `athena gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `athena cron tick`.
...
```bash
date
athena cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.athena/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.athena/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.athena/.env` |
...
| `local` | Write access to `~/.athena/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `athena cron list` for updated `last_error` field (if available).
...
### Check 5: Relay-fronted platforms (Athena Cloud / Team Gateway)
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `athena skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `athena skills list`.
...
2. `~/.athena/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `athena cron list`
...
```bash
ls ~/.athena/scripts/your-script.py   # Must exist
athena cron edit <job_id> --script ~/.athena/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `athena skills install <skill-name>`.
...
Jobs are stored in `~/.athena/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.athena/cron/jobs.json
chmod 600 ~/.athena/cron/jobs.json   # Your user should own it
```
...
```bash
athena cron list                    # Show all jobs, states, next_run times
athena cron run <job_id>            # Schedule for next tick (for testing)
athena cron edit <job_id>           # Fix configuration issues
athena logs                         # View recent Athena logs
athena skills list                  # Verify installed skills
```
...
1. Run the job with `athena cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.athena/logs/agent.log` for scheduler messages and `~/.athena/logs/errors.log` for warnings
3. Open an issue at [github.com/pavel4ai/athena](https://github.com/pavel4ai/athena) with:
