...
Athena has two sharing paths, and they answer different questions. Distributions are the durable one; export files are the quick one.

| | **Distribution** (git repo) | **Export file** (`.tar.gz`) |
|---|---|---|
| Ship it by | `athena profile install <repo>` | Send a file — chat, AirDrop, USB, email |
| Recipient needs | git, and access to the repo | The file |
| Updates | `athena profile update` pulls new versions | Re-send the file |
...
| Made with | `athena profile install` / `update` | `/export` and `/import`, or `athena profile export` / `import` |
...
```bash
athena profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `athena profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
:::caution
**Athena does not control git.** The file exclusions described on this page are applied by the **installer** when someone runs `athena profile install` or `athena profile update`. They are **not** applied when you run `git add` or `git commit`.
:::
...
```bash
athena profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.athena/profiles/research-bot/SOUL.md
...
Create `~/.athena/profiles/research-bot/distribution.yaml`:

```yaml
name: research-bot
version: 1.0.0
description: "Autonomous research assistant with arXiv and web tools"
athena_requires: ">=0.12.0"
author: "Your Name"
license: "MIT"
...
Create `~/.athena/profiles/research-bot/.gitignore` with at minimum:
...
```bash
cd ~/.athena/profiles/research-bot
git init
git add .
git commit -m "v1.0.0"
...
Recipients who run `athena profile update research-bot` will pull the latest.
...
```bash
athena profile install github.com/you/research-bot --alias
```
...
5. Copies distribution-owned files into `~/.athena/profiles/research-bot/` (or wherever the manifest's `name` resolves). The [hard-excluded paths](#whats-not-in-a-distribution-ever) are stripped during this copy, even if the author accidentally left them in the repo.
...
```bash
# GitHub shorthand
athena profile install github.com/you/research-bot

# Full HTTPS
athena profile install https://github.com/you/research-bot.git

# SSH
athena profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
athena profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
athena profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
athena profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
athena profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
athena profile install github.com/acme/support-bot --name support-eu --alias
```
...
```bash
cp ~/.athena/profiles/research-bot/.env.EXAMPLE ~/.athena/profiles/research-bot/.env
...
```bash
athena profile info research-bot
```
...
`athena profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
athena profile update research-bot
```
...
```bash
athena profile delete research-bot
```
...
Path:    ~/.athena/profiles/research-bot
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `athena profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
cd ~/.athena/profiles/pr-reviewer
# ... build and tune ...
git init && git add . && git status   # confirm no secrets staged
...
# Each engineer
athena profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `athena profile update pr-reviewer` and everyone's on the new version within minutes.
...
cd ~/.athena/profiles/polymarket-trader
# Write a solid README.md at the repo root — GitHub shows it on the repo page
git init && git add . && git status   # confirm no secrets staged
git commit -m "v1.0"
git tag v1.0.0
# Publish to a public GitHub repo
git remote add origin https://github.com/you/athena-polymarket-trader.git
git push -u origin main --tags

# Anyone
athena profile install github.com/you/athena-polymarket-trader --alias
```
...
# Each on-call
athena profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
athena profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `athena profile info <name>` and hold off on updates until you're ready.
:::
...
athena profile info research-bot | grep Version
...
```yaml
# ~/.athena/profiles/research-bot/local/my-overrides.yaml
...
athena profile delete research-bot --yes
athena profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
athena profile update research-bot --force-config --yes
```
...
athena profile install github.com/yourname/forked-research-bot --alias

# Iterate locally in ~/.athena/profiles/forked-research-bot/
...
athena profile install ~/.athena/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
athena profile delete research-bot-test --yes
athena profile install ~/.athena/profiles/research-bot --name research-bot-test
```
...
Without `-o`, the CLI and TUI place the archive in Athena's managed
`profile-exports/` directory under the default Athena home, not in the current
...
source file. If the Athena home itself lives inside a Git checkout (some
Docker/custom deployments), the archive goes to `~/.athena-profile-exports/`
...
```bash
athena profile export research-bot
athena profile export research-bot -o ./research-bot.tar.gz
```
...
```bash
athena profile import ./research-bot.tar.gz
athena profile import ./research-bot.tar.gz --name research-bot-2
```

The profile name is inferred from the archive unless you pass `--name`. Importing over an existing profile is refused — rename or delete the old one first. A shell wrapper (`research-bot` → `athena -p research-bot`) is created when the name doesn't collide with an existing command.
...
:::note
You cannot import as `default` — that name is the built-in root profile (`~/.athena`). Pass `--name something-else`.
:::
...
**The default profile** (`~/.athena`) is exported through an allow-list — only known Athena artifacts, so an unrelated file sitting in your home directory can't get swept in:
...
**A named profile** (`~/.athena/profiles/<name>`) copies the whole directory minus `auth.json` / `.env`. That's broader — if the profile has `state.db`, logs, or caches, they go in the archive too, and the file gets big.
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `athena -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `athena profile` — not a parallel command tree.
...
- Reserved profile names (`athena`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`athena profile export` / `import`](../reference/profile-commands.md#athena-profile-export) — the CLI form of [export files](#export-and-import-a-profile-file)
- [Slash Commands reference](../reference/slash-commands.md) — `/export`, `/import`, and every other in-chat command
- [Using SOUL with Athena](../guides/use-soul-with-athena.md) — authoring personalities
