...
```bash
athena chat --checkpoints
```

Or enable globally in `~/.athena/config.yaml`:
...
This safety net is powered by an internal **Checkpoint Manager** that keeps a single shared shadow git repository under `~/.athena/checkpoints/store/` — your real project `.git` is never touched. Every project the agent works in shares the same store, so git's content-addressable object DB deduplicates across projects and across turns.
...
| Command | Description |
|---------|-------------|
| `athena checkpoints` | Show total size, project count, per-project breakdown |
| `athena checkpoints status` | Same as bare `checkpoints` |
| `athena checkpoints list` | Alias for `status` |
| `athena checkpoints prune` | Force a sweep: delete orphans/stale, GC, enforce size cap |
| `athena checkpoints clear` | Nuke the entire checkpoint base (asks first) |
| `athena checkpoints clear-legacy` | Delete only the `legacy-*` archives from v1 migration |
...
- Once per conversation turn (per directory), it:
  ...
  - Initialises or reuses the **single shared shadow store** at `~/.athena/checkpoints/store/`.
  - Stages into a per-project index, builds a tree, and commits to a per-project ref (`refs/athena/<project-hash>`).
...
checkpoints:
  ...
  # Auto-maintenance (on by default): sweep ~/.athena/checkpoints/ at startup
  ...
  # `athena checkpoints prune` command below, with a confirmation prompt.
  auto_prune: true
  retention_days: 7
...
When `enabled: false`, the Checkpoint Manager is a no-op and never attempts git operations. When `auto_prune: false`, the store grows until you run `athena checkpoints prune` manually.
...
```text
Checkpoint base: /home/you/.athena/checkpoints
Total size:      142.3 MB
  store/         138.1 MB
  legacy-*       4.2 MB
Projects:        12
...
Clear with: athena checkpoints clear-legacy
```

Force a full sweep (ignores the 24h idempotency marker):

```bash
athena checkpoints prune --retention-days 3 --max-size-mb 200
```
...
`/rollback <N>` restores only the files Athena itself changed. Every successful
...
Athena last wrote (you edited it afterwards, or Athena never touched it) is
...
If the ledger is empty (a store created before this feature, or Athena hasn't
...
~/.athena/checkpoints/
  ...
  │   ├── refs/athena/<hash> # per-project branch tip
...
Each `<hash>` is derived from the absolute path of the working directory. You normally never need to touch these manually — use `athena checkpoints status` / `prune` / `clear` instead.

### Migration from v1

Before the v2 rewrite, each working directory got its own complete shadow git repo directly under `~/.athena/checkpoints/<hash>/`. That layout couldn't dedup objects across projects and had a documented no-op pruner — the store would grow without bound.

On first v2 run, any pre-v2 shadow repos are moved into `~/.athena/checkpoints/legacy-<timestamp>/` so the new single-store layout starts clean. Old `/rollback` history is still reachable by manually inspecting the legacy archive with `git`; once you're confident you don't need it, run:

```bash
athena checkpoints clear-legacy
```
...
- **Enable checkpoints only when you need them** — `athena chat --checkpoints` or per-profile `enabled: true`.
...
- **Check `athena checkpoints status` occasionally** if you use checkpoints regularly — shows which projects are active and what the store costs you.
