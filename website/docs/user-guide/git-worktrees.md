...
- CLI: the directory where you run `athena` or `athena chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.athena/config.yaml`
...
Athena creates `.worktrees/my-experiment/` inside the repo (branch
`athena/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `athena-<id>`
...
git worktree add ../repo-feature feature/athena-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/athena-experiment` checked out in that directory
...
git worktree add ../repo-experiment-a feature/athena-a
git worktree add ../repo-experiment-b feature/athena-b
```
...
- Works on its own branch (`feature/athena-a` vs `feature/athena-b`).
...
- Athena checkpoint data under `~/.athena/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.
...
- **Name branches after the experiment**
  - e.g. `feature/athena-checkpoints-docs`, `feature/athena-refactor-tests`.
- **Commit frequently**
...
## Using `athena -w` (Automatic Worktree Mode)
...
- Check out an isolated branch (e.g. `athena/athena-<hash>`).
...
```bash
athena -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `athena -w` in each — every invocation gets its own worktree and branch automatically.
