...
Pull API keys from [Bitwarden Secrets Manager](https://bitwarden.com/products/secrets-manager/) at process startup instead of storing them in plaintext inside `~/.athena/.env`. One bootstrap secret (a machine-account access token) replaces N per-provider keys, and rotating a credential becomes a single change in the Bitwarden web app.
...
2. Athena stores that single token in `~/.athena/.env` as `BWS_ACCESS_TOKEN`.
3. Every time `athena` (or the gateway, or a cron job) starts, after `~/.athena/.env` has loaded, Athena calls `bws secret list <project_id>` and sets the returned keys into `os.environ`.
...
The `bws` binary is auto-downloaded into `~/.athena/bin/` on first use — no `apt`, no `brew`, no `sudo`.
...
1. Download and verify `bws v2.0.0` into `~/.athena/bin/bws`.
2. Prompt you for the access token (input is hidden). Stored in `~/.athena/.env` as `BWS_ACCESS_TOKEN`.
...
```bash
athena secrets bitwarden status
```

From now on, every `athena` invocation pulls fresh secrets at startup. You'll see a one-line summary in stderr the first time secrets are applied in a process.

## CLI

| Command | What it does |
|---|---|
| `athena secrets bitwarden setup` | Interactive wizard (install binary, prompt for token, pick project, test fetch) |
| `athena secrets bitwarden status` | Show config + binary version + token presence/validation |
| `athena secrets bitwarden token` | Rotate the access token: validate the new token against Bitwarden, then store it in `.env` |
| `athena secrets bitwarden sync` | Dry-run: pull secrets now and show what would be applied |
| `athena secrets bitwarden sync --apply` | Pull and export into the current shell's environment |
| `athena secrets bitwarden install` | Just download the pinned `bws` binary (no auth required) |
| `athena secrets bitwarden disable` | Flip `enabled: false`; leaves token + project id in place |
...
Bitwarden Secrets Manager: → Run `athena secrets bitwarden token` to paste a fresh access token ...
```

Fix it without re-running the whole wizard:

```bash
athena secrets bitwarden token                     # masked prompt
athena secrets bitwarden token --access-token 0.…  # non-interactive
```
...
| `encrypted_cache.enabled` | `false` | Store the last successful fetch in an AES-GCM encrypted cache at `~/.athena/cache/bws_cache.enc.json`. |
...
| `auto_install` | `true` | When true, `bws` is auto-downloaded into `~/.athena/bin/` on first use. |
...
| Symptom | Cause | Fix |
|---|---|---|
| `BWS_ACCESS_TOKEN is not set` | Enabled in config but token cleared from `.env` | Re-run `athena secrets bitwarden setup` |
| `Bitwarden rejected the machine-account access token … invalid_client` | Token revoked, expired, machine account deleted — or the token belongs to another region (e.g. EU token hitting the US identity endpoint) | Run `athena secrets bitwarden token` to paste a fresh token; for region mismatches re-run setup and pick EU/self-hosted (or set `secrets.bitwarden.server_url`) |
| `bws exited 1: invalid access token` | Token revoked or wrong | Run `athena secrets bitwarden token` with a new token |
...
- **Single-machine personal setups** where `~/.athena/.env` is fine. You're trading one credential for another and adding a network dependency at startup.
