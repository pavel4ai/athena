...
- **CLI** (`athena`) and **[TUI](./tui.md)** (`athena --tui`) — terminal interfaces.
- **[Web Dashboard](./features/web-dashboard.md)** (`athena dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
Download the app from the [Athena Desktop product page](https://athena-agent.nousresearch.com/desktop), or follow the [installation instructions for Athena Desktop](../getting-started/installation.md).
...
- Set `repo_scan_enabled: false` to stop the filesystem scan completely. Existing disk-discovery cache rows for that profile are cleared; explicit projects and repositories inferred from intentional Athena sessions remain available.
...
Explore and preview the working directory without leaving the app — useful for following along as the agent reads, writes, and edits files. Set the initial project directory with `athena desktop --cwd <path>` (or the `ATHENA_DESKTOP_CWD` environment variable).
...
- **Review pane** — **Cmd/Ctrl+G** toggles the working-tree review pane: branch and ahead/behind status, changed files (list or tree view), and diffs scoped to **Uncommitted**, **Branch**, or **Last turn** (just what the agent changed in its most recent turn). Stage/unstage files, revert changes, write a commit message (or **Generate commit message**), then **Commit** or **Commit & Push** — and **Create PR** via the GitHub CLI (`gh`), or hand the whole thing to the agent with **Ask Athena to open PR**. You can also create and switch branches from here.
...
When `athena gui` runs inside WSL2 with `/dev/dxg` present and Mesa's `d3d12_dri.so` installed, the launcher sets `GALLIUM_DRIVER=d3d12` for Electron so rendering uses the Windows GPU instead of the llvmpipe software rasterizer; an explicit `GALLIUM_DRIVER`, `MESA_LOADER_DRIVER_OVERRIDE`, `LIBGL_ALWAYS_SOFTWARE`, or `LIBGL_DRIVERS_PATH` in your environment is left untouched (for example `GALLIUM_DRIVER=llvmpipe athena gui` keeps software rendering).
...
- **Every provider and model in the menus** — the GUI surfaces the full provider list and every model that `athena model` knows about, so you pick from the same catalog the CLI sees rather than a curated subset.
...
- **Skills** — browse, install, and manage [skills](./features/skills.md). The Skills tab lists your installed skills with enable/disable toggles, and below them the full built-in optional-skills catalog that ships with Athena — each entry has a one-click **Install** button that flips the row into the installed list once it finishes.
...
- **Cron** — view and manage [scheduled jobs](../reference/cli-commands.md#athena-cron).
- **Profiles** — switch between [Athena profiles](./profiles.md) (isolated config/skills/sessions).
- **Messaging** — set up gateway channels. Telegram has a **Quick setup** card: click **Create with QR**, scan the code (or open the link) in Telegram, and Athena creates the bot, detects your user ID for the allowlist, saves the credentials, and restarts the gateway for you. Any credential save, clear, or enable toggle keeps a **Restart now** banner on the page until the gateway has actually restarted; if a restart fails, the banner stays so you can retry or restart manually.
...
roster where every [Athena profile](./profiles.md) appears as a bot with its
...
(recurring tasks backed by Athena cron). The roster lives in the left
...
each other's Bot Chats directly (`athena -p <bot> chat`). The backend teaches
...
- **Command palette** — press **Cmd+K** or **Cmd+P** (Ctrl+K / Ctrl+P on Windows/Linux) to jump to actions and navigate the app from the keyboard: open any page or settings section, jump to a session by title or id, switch model/theme/color mode, spawn a terminal, restart the gateway, update Athena, and more.
...
The desktop app and the Athena backend it talks to update on separate clocks — the app package on your machine, the backend wherever it runs. When more than one update target exists (a remote gateway, or several registered gateways), the update affordances (**Update now** on the About panel, the ⌘K **Update Athena** row, and the update-ready toast) update **everything**: the connected backend first, then every other eligible registered gateway (Athena Cloud entries are platform-managed and skipped), and the desktop app itself last, since applying the client update relaunches the app. Single-machine installs keep the one-button experience.
...
- **Uninstall Chat GUI only** — removes the desktop app and its data; the Athena agent, your config, and your chats stay. (Same as `athena uninstall --gui`.)
- **Uninstall GUI + agent, keep my data** — removes the app and the agent but keeps config, chats, and secrets for a future reinstall. (Same as `athena uninstall`.)
- **Uninstall everything** — removes the app, the agent, and all user data. (Same as `athena uninstall --full`.)
...
You can do the same from the terminal — `athena uninstall --gui` for the GUI alone, or `athena uninstall` / `athena uninstall --full` for the agent too.

:::note
Running `athena uninstall --gui` from a **source checkout** (a `athena desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `athena desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::

## CLI reference: `athena desktop`

To launch via the CLI, simply run `athena desktop`. By default it installs workspace Node dependencies, builds the current OS's unpacked Electron app, then launches that packaged artifact.

On Linux, launches refresh `$XDG_DATA_HOME/applications/athena.desktop` (by default `~/.local/share/applications/athena.desktop`) so Athena appears in the application menu. To preserve a hand-edited entry, disable refreshes:

```bash
athena config set desktop.manage_launcher_entry false
```

A missing entry is still created; the flag only stops `athena desktop` from rewriting an entry that already exists.
...
| `--build-only`       | Build the desktop app but do not launch it (used by `athena update`)                      |
| `--source`           | Launch via `electron .` against `apps/desktop/dist` instead of the packaged app           |
| `--cwd PATH`         | Initial project directory for desktop chat sessions (sets `ATHENA_DESKTOP_CWD`)           |
| `--athena-root PATH` | Override the Athena source root the app uses (sets `ATHENA_DESKTOP_ATHENA_ROOT`)          |
| `--ignore-existing`  | Force the app to ignore any `athena` CLI already on `PATH` during backend resolution      |
| `--fake-boot`        | Enable deterministic boot delays for validating the startup UI                            |

## How it works

The packaged app ships the Electron shell and a native React chat surface. On first launch it can install the Athena Agent runtime into `ATHENA_HOME` (`~/.athena`, or `%LOCALAPPDATA%\athena` on Windows) — **the same layout a CLI install uses**, which is why the two are interchangeable. Backend resolution first honours `ATHENA_DESKTOP_ATHENA_ROOT`, then a completed managed install, then a probed `athena` on `PATH` (unless `--ignore-existing` / `ATHENA_DESKTOP_IGNORE_EXISTING=1` is set), and finally an explicit `ATHENA_DESKTOP_ATHENA` command override for packagers such as Nix. The React renderer talks to a headless backend the app launches for you — a `athena serve` process that serves the `tui_gateway` JSON-RPC/WebSocket API — and reuses the agent runtime rather than embedding `athena --tui`. The desktop app is **self-contained**: it runs its own `athena serve` backend and never opens or requires the [web dashboard](./features/web-dashboard.md). (Runtimes older than the `serve` command fall back to a headless `dashboard --no-open` automatically, so an app update never outruns its backend.) Install, backend-resolution, and self-update logic live in the Electron main process.
...
Further down the same **Settings → Gateways** page, **Registered gateways** manages a named list of every Athena gateway the app knows about — the local runtime, any number of remote gateways (LAN, Tailscale, internet), Athena Cloud instances, and SSH hosts — all persisted together in one place. You can jump there from the plug button at the right end of the sidebar profile rail (**Connect another Athena gateway…**) or via **⌘K → Gateways**. The full guide, including the union agent roster, `@name-device` handles, fleet-wide updates, and the plugin SDK surface, is at [Connecting Desktop to Many Athena Instances](./multi-connection-desktop.md).
...
- Cloud entries come from the Athena Cloud sign-in/discovery flow above, not from a hand-typed URL.
- Tokens are stored encrypted with the OS keyring (with an explicit plain-text opt-in on keyring-less Linux).

Side-by-side routing is live: each registered gateway dials its own backends and sockets on demand (keyed per connection + profile), the plugin SDK exposes the union agent roster (`host.agents()` / `host.ensureAgent()`), and **Update all instances** on the Gateways page dispatches `athena update` to every eligible gateway at once — Athena Cloud entries are skipped (the platform updates them), and each instance reports its own result.


:::info The remote backend is a running `athena serve` process
"Remote backend" means a **`athena serve`** server running on the remote machine — that is the process the desktop app connects to. Nothing in this section works unless that backend is actually up and reachable. The desktop app does not start it for you; you (or a `systemd` service) keep `athena serve` running on the remote host, and the app attaches to it. If you also use messaging channels (Telegram, Discord, etc.), the **gateway** is a *separate* long-running process you start independently — see the note after the setup steps.
:::
...
Set a username and password, then start the backend bound to a reachable address. The credentials live in `~/.athena/.env` (the secrets file, mode 0600):
...
athena serve --host 0.0.0.0 --port 9119
```

Keep that `athena serve` process running for as long as you want the desktop app to be able to connect — if it stops, the app can no longer reach the backend. Run it under `systemd`, `tmux`, or your process manager of choice so it survives logout and reboots.

Separately, make sure the **gateway is running** on the remote host if you rely on messaging channels — the `athena serve` backend is what the desktop app talks to, but your Telegram/Discord/Slack gateway sessions are a different process that you start and keep running on their own. See [Messaging](./messaging/index.md) for gateway setup.

Prefer not to keep a plaintext password at rest? Set `ATHENA_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` to a scrypt hash instead — compute it with `python -c "from plugins.dashboard_auth.basic import hash_password; print(hash_password('PW'))"`. Full configuration surface (config.yaml keys, every env var, the rate limiter): [Web Dashboard → Username/password provider](./features/web-dashboard.md#usernamepassword-provider-no-oauth-idp).

Running the backend as a systemd service? Give the unit `EnvironmentFile=%h/.athena/.env` so the credentials are in the environment at boot.
...
1. **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/athena` work if you front it with a reverse proxy)
...
3. **Save and reconnect** — switches the desktop shell onto the remote backend. The session refreshes automatically; you stay signed in across restarts when `ATHENA_DASHBOARD_BASIC_AUTH_SECRET` is set.

You can also set the backend URL without the UI via the `ATHENA_DESKTOP_REMOTE_URL` environment variable before launching the app (it overrides the in-app setting); you still sign in from the Gateways settings panel.
...
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `ATHENA_DASHBOARD_BASIC_AUTH_USERNAME` / `ATHENA_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for an unknown user and a wrong password (no enumeration oracle), so double-check both. Confirm the gate is on with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
- **No "Sign in" button — it asks for a session token instead** — the backend's username/password provider isn't active. `/api/status` won't list `"basic"` in `auth_providers`. Make sure both the username and a password (or password hash) are set in `~/.athena/.env` and that the dashboard process actually loaded them.
- **Signed out on every restart** — set `ATHENA_DASHBOARD_BASIC_AUTH_SECRET` to a stable value. Without it the token-signing key is regenerated per boot, invalidating all sessions.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting Athena Desktop to a remote backend](./features/web-dashboard.md#connecting-athena-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & Athena Desktop](../reference/environment-variables.md#web-dashboard--athena-desktop).
...
`$ATHENA_HOME/desktop-plugins/<id>/plugin.js`; the app loads it within seconds and
...
Athena: **one row per plugin**, with two switch columns.
...
- **Desktop column** — the half loaded into this app. It is app-level: the
  ...
  `~/.athena/desktop-plugins/`; the desktop half of a unified agent+desktop
...
- **Open logs** — opens `ATHENA_HOME/logs` in your file manager. On a remote
...
- **Send diagnostics** — uploads a redacted debug bundle to Nous-internal
  ...
  `athena debug share --nous`; secrets are always redacted, the bundle is
...
Boot logs land in `ATHENA_HOME/logs/desktop.log` (it includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure. You can also tail it from the CLI:
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.athena/athena-agent/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```

**Other native downloads (e.g. the `get-windows` prebuilt on Windows) that need a mirror:** put the npm keys in `$ATHENA_HOME/npmrc` (`%LOCALAPPDATA%\athena\npmrc` on Windows, `~/.athena/npmrc` elsewhere) — for example `node_get_windows_binary_host_mirror=https://<mirror>/sindresorhus/get-windows/releases/download/`. Every `npm ci`/`npm run` the updater spawns (desktop, web and TUI builds) points `NPM_CONFIG_USERCONFIG` at that file when it exists, so the config survives `athena update`; the repo-root `.npmrc` is git-tracked and gets autostashed on every update, and `~/.npmrc` may be missed because the desktop hand-off inherits the GUI's environment. An `NPM_CONFIG_USERCONFIG` you set yourself is never overridden.
...
(Desktop, then Downloads, then Documents, ...) as Athena touches each folder.
A single **Full Disk Access** grant covers all of them, permanently — and
with Athena' stable signing identities it survives every update:

1. System Settings → **Privacy & Security → Full Disk Access** (or run
   `open "x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles"`)
2. Enable your terminal app — and **Athena.app** if you use Desktop.
3. Fully quit and relaunch them once.

`athena doctor` reports whether the current terminal context already has the
grant, and `athena setup` shows this tip on macOS when it doesn't.
...
```bash
tccutil reset ScreenCapture com.nousresearch.athena   # repeat per service
```

then toggle the fresh entry ON in System Settings and fully quit & relaunch
Athena. Grants are stable from then on.
...
certificate once and tell Athena to use it. The one-shot command does
...
```bash
athena desktop --setup-tcc-identity
```

Or do it manually:

1. Keychain Access → Certificate Assistant → **Create a Certificate…**
2. Name: `Athena Local Signing`, Identity Type: *Self-Signed Root*,
...
4. `athena config set desktop.macos_signing_identity "Athena Local Signing"`

Use `--identity <name>` with the command to create/use a differently named
certificate (default: `Athena Local Signing`). The command is idempotent —
...
`tccutil reset All com.nousresearch.athena` and re-grant.

## See also

- [CLI Guide](./cli.md) — the terminal interface
- [TUI](./tui.md) — the modern terminal UI used by `athena --tui` and the dashboard chat tab
