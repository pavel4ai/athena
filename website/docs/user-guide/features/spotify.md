...
Athena can control Spotify directly — playback, queue, search, playlists, saved tracks/albums, and listening history — using Spotify's official Web API with PKCE OAuth. Tokens are stored in `~/.athena/auth.json` and refreshed automatically on 401; you only log in once per machine (refresh tokens expire after ~6 months; re-run `athena auth spotify` when they do).

Unlike Athena' built-in OAuth integrations (Google, GitHub Copilot, Codex), Spotify requires every user to register their own lightweight developer app. Spotify does not let third parties ship a public OAuth app that anyone can use. It takes about two minutes and `athena auth spotify` walks you through it.
...
### One-shot: `athena tools` or first-run setup
...
Scroll to `🎵 Spotify`, press space to toggle it on, then `s` to save. The same toggle is also available during the first-run `athena setup` / `athena setup tools` flow. Spotify stays opt-in, so enabling it there runs the same provider-aware configuration as `athena tools`.
...
If no `ATHENA_SPOTIFY_CLIENT_ID` is set, Athena walks you through the app registration inline:
...
4. Saves it to `~/.athena/.env` so future runs skip this step
5. Continues straight into the OAuth consent flow

After you approve, tokens are written under `providers.spotify` in `~/.athena/auth.json`. The active inference provider is NOT changed — Spotify auth is independent of your LLM provider.
...
```bash
athena auth status spotify
```

Shows whether tokens are present and when the access token expires. Refresh is automatic: when any Spotify API call returns 401, the client exchanges the refresh token and retries once. Refresh tokens persist across Athena restarts, so you only re-auth if you revoke the app in your Spotify account settings or run `athena auth logout spotify`.
...
- **The cron agent inherits your active toolsets.** Spotify must be enabled in `athena tools` for the cron session to see the Spotify tools.
...
```bash
athena auth logout spotify
```

Removes tokens from `~/.athena/auth.json`. To also clear the app config, delete `ATHENA_SPOTIFY_CLIENT_ID` (and `ATHENA_SPOTIFY_REDIRECT_URI` if you set it) from `~/.athena/.env`, or run the wizard again.
...
**`INVALID_CLIENT: Invalid redirect URI`** — the redirect URI in your Spotify app settings doesn't match what Athena is using. The default is `http://127.0.0.1:43827/spotify/callback`. Either add that to your app's allowed redirect URIs, or set `ATHENA_SPOTIFY_REDIRECT_URI` in `~/.athena/.env` to whatever you registered.
...
**`401 Unauthorized` keeps coming back** — Your refresh token was revoked (usually because you removed the app from your account, or the app was deleted). Run `athena auth spotify` again.
...
```bash
athena auth spotify --scope "user-read-playback-state user-modify-playback-state playlist-read-private"
```
...
```bash
athena auth spotify --client-id <id> --redirect-uri http://localhost:3000/callback
```

Or set them permanently in `~/.athena/.env`:

```
ATHENA_SPOTIFY_CLIENT_ID=<your_id>
ATHENA_SPOTIFY_REDIRECT_URI=http://localhost:3000/callback
```
...
| File | Contents |
|------|----------|
| `~/.athena/auth.json` → `providers.spotify` | access token, refresh token, expiry, scope, redirect URI |
| `~/.athena/.env` | `ATHENA_SPOTIFY_CLIENT_ID`, optional `ATHENA_SPOTIFY_REDIRECT_URI` |
