...
If the file is empty, whitespace-only, or cannot be read, Athena falls back to a built-in default identity ("You are Athena Agent, an intelligent AI assistant created by Futurebound Corp...."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
You can also define named custom personalities in `~/.athena/config.yaml` under `agent.personalities`.
