...
the refresh token in `~/.athena/auth.json` — the same place all Athena
...
interface in `athena_cli/proxy/adapters/`.
...
If you see `not logged in`, run `athena portal`. If you see
...
[futurebnd.com](https://futurebnd.com).
...
`athena_cli/proxy/adapters/<provider>.py` and registering it in
