...
athena auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
athena auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
athena auth add anthropic --type oauth
...
anthropic (3 credentials):
  #1  athena_pkce          oauth   id=ab12cd34 priority=0 athena_pkce ←
...
`athena auth remove <provider> <target>` when a label is ambiguous, and `priority=` is
the order the pool tries credentials in under the `fill_first` strategy.

## Interactive Management

Run `athena auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `athena auth` | Interactive pool management wizard |
| `athena auth list` | Show all pools and credentials |
| `athena auth list <provider>` | Show a specific provider's pool |
| `athena auth add <provider>` | Add a credential (prompts for type and key) |
| `athena auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `athena auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `athena auth add <provider> --priority 0` | Add a credential and place it first in the `fill_first` order |
| `athena auth priority <provider> <target> <n>` | Move a credential to priority `n` (0 = tried first); the rest are renumbered |
| `athena auth remove <provider> <index>` | Remove credential by 1-based index |
| `athena auth reset <provider>` | Clear all cooldowns/exhaustion status |
| `athena auth reset <provider> <target>` | Clear the cooldown on one credential by index, id, or label |
| `athena auth refresh <provider> [target]` | Refresh one OAuth credential's tokens and return it to rotation (proves the grant is alive; the next request re-checks quota) |
...
cooldowns are preserved. Reauthenticate with `athena auth add nous --type oauth`
...
Configure via `athena auth` → "Set rotation strategy" or in `config.yaml`:
...
| Strategy | Behavior |
|----------|----------|
| `fill_first` (default) | Use the first healthy key until it's exhausted, then move to the next; order is each credential's `priority` (`athena auth priority` changes it) |
...
When you set up a custom endpoint via `athena model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via athena model:
athena auth list
# Shows:
...
athena auth add Together.ai --api-key sk-together-second-key
```
...
| Athena PKCE OAuth | `~/.athena/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `athena auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `athena auth add`) are never auto-pruned.
...
2. **`athena_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`athena_cli/runtime_provider.py`** — Pool-aware credential resolution
4. **`agent/turn_api_error.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.athena/auth.json` under the `credential_pool` key:
