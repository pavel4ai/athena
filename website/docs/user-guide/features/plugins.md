...
**→ [Build a Athena Plugin](/guides/build-a-athena-plugin)** — step-by-step guide with a complete working example.
...
User plugins at `~/.athena/plugins/model-providers/<name>/` and `~/.athena/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.
...
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Athena Plugin](/guides/build-a-athena-plugin) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Athena Plugin](/guides/build-a-athena-plugin) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Athena Plugin](/guides/build-a-athena-plugin) · [Extending the CLI](/developer-guide/extending-the-cli) |
...
athena plugins update my-plugin              # pull latest
...
See the **[full guide](/guides/build-a-athena-plugin)** for handler contracts, schema format, hook behavior, error handling, and common mistakes.
