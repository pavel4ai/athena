...
**→ [Build a Athena Plugin](/developer-guide/plugins)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.athena/plugins/` with a `plugin.yaml` and Python code:

```
~/.athena/plugins/my-plugin/
...
**`~/.athena/plugins/hello-world/plugin.yaml`**
...
**`~/.athena/plugins/hello-world/__init__.py`**
...
Drop both files into `~/.athena/plugins/hello-world/`, restart Athena, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

The model-facing tool description belongs in `schema["description"]`. The optional `ctx.register_tool(description=...)` value is separate `ToolEntry` registry metadata: when omitted, it defaults to the schema description, but Athena does not copy it back into a schema that lacks `description`. Prefer defining the text once in the schema. If you provide both values, keep them synchronized; the model sees the schema value.

Project-local plugins under `./.athena/plugins/` are disabled by default. Enable them only for trusted repositories by setting `ATHENA_ENABLE_PROJECT_PLUGINS=true` before starting Athena.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `athena <plugin> <subcommand>` |
...
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `athena plugins install` |
| Distribute via pip | `[project.entry-points."athena_agent.plugins"]` |
...
| User | `~/.athena/plugins/` | Personal plugins |
| Project | `.athena/plugins/` | Project-specific plugins (requires `ATHENA_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `athena_agent.plugins` entry_points | Distributed packages |
...
User plugins at `~/.athena/plugins/model-providers/<name>/` override bundled model providers of the same name (last-writer-wins in `register_provider()`), so you can replace a built-in provider profile without any repo edits. Memory providers resolve the other way round: for `~/.athena/plugins/memory/<name>/` the **bundled** provider wins on a name collision (bundled, then user, then project, then entry points; first seen wins), so a user memory provider needs its own unique name.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `athena plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.athena/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
athena plugins                    # interactive toggle (space to check/uncheck)
athena plugins enable <name>      # add to allow-list
athena plugins disable <name>     # remove from allow-list + add to disabled
```

After `athena plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

For a reproducible install, pin a full immutable commit (tags, branches, and
abbreviated SHAs are not accepted):

```bash
athena plugins install owner/repo --ref 0123456789abcdef0123456789abcdef01234567
```

Athena checks out the commit detached, verifies that `HEAD` exactly matches the
requested SHA, and records the canonical source, installed revision, and pin
status in the current profile. `athena plugins update` refuses to move a pinned
plugin; choose a new exact commit explicitly with
`athena plugins install <source> --force --ref <new-commit>`. The
profile-local install metadata contains no config values, environment values,
secrets, or capability grants.

The same pin is available in Athena Desktop: **Skills → Plugins → Install from
...
can confirm everyone is running the same commit. `athena plugins list` prints
...
`athena plugins install` clones non-interactively (it never prompts for a
username or password), so a private repo needs a credential Athena can find on
...
before. The same resolution applies to `athena plugins update`, catalog MCP
...
| **User-installed platforms** (under `~/.athena/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.athena/plugins/`.

### Approval transports

An approval transport changes **where a human sees and answers** an existing
Athena tool-approval request. It does not decide whether a command needs
...
`present` may be synchronous or async. Athena runs it on a bounded worker and
...
`transport_fallback: builtin`. Without that exact opt-in, Athena never
materializes the prompt on another surface.

Athena still owns hardline blocks, sudo-stdin protection, user deny rules,
...
When you upgrade to a version of Athena that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.athena/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled standalone plugins are NOT grandfathered — even existing users have to opt in explicitly. (Bundled platform/backend plugins never needed grandfathering because they were never gated.)

## Available hooks

Plugins can register the 26 lifecycle events currently accepted by `athena_cli.plugins.VALID_HOOKS`. The **[Event Hooks catalog](/user-guide/features/hooks#shipped-plugin-hook-catalog)** is canonical for exact timing, return handling, payload fields, and privacy notes.
...
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.athena/plugins/` |
...
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Athena Plugin](/developer-guide/plugins) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Athena Plugin](/developer-guide/plugins) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Athena Plugin](/developer-guide/plugins) · [Extending the CLI](/developer-guide/extending-the-cli) |
| A **subcommand** for `athena <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/developer-guide/extending-the-cli) |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `athena skills tap add <repo>` | [Skills Hub](/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.athena/hooks/<name>/` | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `athena plugins install` needed. See the **[Nix Setup guide](/getting-started/nix-setup#plugins)** for full details.
...
```bash
athena plugins                               # unified interactive UI
athena plugins list                          # table: enabled / disabled / not enabled
athena plugins search <term>                 # search the Athena plugin catalog
athena plugins install <name>                # install a catalog entry (repo @ reviewed pinned SHA)
athena plugins install user/repo             # install from Git, then prompt Enable? [y/N]
athena plugins install user/repo --enable    # install AND enable (no prompt)
athena plugins install user/repo --no-enable # install but leave disabled (no prompt)
athena plugins update my-plugin              # pull latest (local edits are autostashed and re-applied)
athena plugins remove my-plugin              # uninstall
athena plugins enable my-plugin              # add to allow-list
athena plugins disable my-plugin             # remove from allow-list + add to disabled
athena plugins capabilities [my-plugin]      # declared vs granted capabilities
```

### One-click install links (Desktop)

Athena Desktop registers the `athena://` URL scheme, so a website, README, or
chat message can link straight to a plugin install:

```
athena://plugin/install?repo=owner/repo            # main install link
athena://plugin/install?repo=owner/repo&enable=1   # enable the agent plugin after install
athena://plugin/install?repo=owner/repo&force=1    # replace an existing install
```

Clicking one opens Athena and shows a **confirmation dialog** — the repo id,
...
Plugins → Install from Git**. Legacy `athena://plugin-agent/…` and
`athena://plugin-desktop/…` URLs route into the same dialog. In dev builds
(`npm run dev`) the scheme is `athena-dev://`.
...
[Add to Athena link](/reference/mcp-config-reference#add-to-athena-link).
...
When a plugin declares capabilities, `athena plugins install` (and
`athena plugins enable`) shows the list with one-line risk descriptions and
...
granted, `athena plugins update` surfaces the additions and asks again. New
...
`athena plugins enable <id>` interactively to grant them later.

Inspect the state at any time:

```bash
athena plugins capabilities             # all plugins with declared/granted capabilities
athena plugins capabilities my-plugin   # one plugin, declared vs granted
```
...
not a code audit, and Athena has not reviewed the plugin's code. Only install
...
### Discovering plugins — the Athena plugin catalog

`athena plugins search <term>` searches the **Athena plugin catalog** — the
curated, SHA-pinned catalog maintained in the athena-agent repository
(`plugin-catalog/`). Matching covers entry names, descriptions, and declared
tools:

```bash
athena plugins search telegram    # search the catalog
athena plugins browse             # browse every entry
athena plugins info <name>        # full details for one entry
```
...
recorded so `athena plugins update` can re-pin when the catalog moves:

```bash
athena plugins install <catalog-name>
```
...
A **plugin pack** is a declarative, shareable YAML file (`athena-pack.yaml`)
...
version: 1.0.0
plugins:
  - name: athena-telegram-business       # bare plugin-catalog name…
...
```bash
athena plugins pack show ./athena-pack.yaml     # dry-run review
athena plugins pack install ./athena-pack.yaml  # review → confirm → install
athena plugins pack export > athena-pack.yaml   # snapshot the current install
athena plugins pack export --enabled-only       # only plugins.enabled
```
...
same pinned install path as `athena plugins install --ref <sha>` and record
...
capability-consent prompt — identical to a single `athena plugins install`.
...
provenance (installed via `athena plugins install`). Local-only plugins are
...
auto-installed — install those manually for now (`athena skills`). Wiring
skill-hub ids into pack install is a documented follow-up seam.

### Install-time security scanning

Every `athena plugins install` and `athena plugins update` runs a static
...
On `athena plugins update`, a dangerous verdict on the updated tree
...
Running `athena plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `athena plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
- Athena reuses that session's stored platform, chat, thread, profile, and conversation history. Plugins cannot supply a new chat route through this API.
- Athena rechecks the stored route against the gateway's current authorisation rules before dispatch.
- Routes that relied only on an adapter-time or upstream authorisation decision are rejected unless Athena can revalidate them from current core allowlists, pairing, or explicit allow-all configuration.
...
- The route and conversation are pinned while dispatch is pending. Athena drops the request if topic recovery changes the route or the session rotates before handling starts.
...
:::warning
Only grant gateway injection to plugins you trust. Athena checks this host API permission and restricts it to existing session routes, but Python plugins run in-process and this setting is not a sandbox.
:::
...
`ctx.call_mcp()` lets a plugin call a tool on one of the user's configured MCP servers — synchronously, from any hook or tool handler — routing through Athena' existing native MCP client (same connections, trust-tier gates, circuit breaker, and reconnect logic as model-invoked MCP tools; never a parallel client).
