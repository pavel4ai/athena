...
Athena Agent can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended Athena integration. It
...
**Fresh installs already have the driver.** The Athena installer
...
- **`athena tools`** → pick `🖱️  Computer Use` — installs the driver
...
on macOS/Linux, `install.ps1` on Windows. Use `athena computer-use
status` to verify the install.

Already have cua-driver? Athena reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `athena update`, and the first
`computer_use` call of a session, Athena checks the local version and
...
selected with `ATHENA_CUA_DRIVER_CMD` stays
under your control, so Athena reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. Athena autodetection is a
planned cua-driver follow-up, so currently point Athena at that directory or
...
interface. The built-in toolset provides Athena actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `athena computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the Athena host identity. |
...
```
athena -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.athena/config.yaml`.

## Permission modes and logged-in browser profiles

Athena maps its existing approval UX onto cua-driver's immutable runtime
modes. Permission mode and capability manifest approval are launch settings.
They cannot change after the runtime starts:

| Athena session | cua-driver mode | Human intervention |
|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Athena approvals; Cua stops at its protected boundary |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Athena risk acceptance; no runtime Cua prompts |
...
computer_use:
  ...
  capability_manifest: ~/.athena/cua-manifest.yaml
```
...
for the format). Athena launches a private runtime with
...
identity instead of resetting with every Athena build), and Athena verifies
...
resetting or closing the Athena session, cancellation cleanup, or process exit
closes that transport session. Athena also stops private runtimes that it
launched for bounded or unrestricted access. One Athena
conversation cannot change another runtime's mode or grants. Bounded and
unrestricted modes use a private
embedded service under the Athena host identity.
...
## `athena computer-use doctor` — your first triage stop

`athena computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ athena computer-use doctor
...
Athena run declares a public cua-driver **session name** (something like
...
without it. Athena disables it automatically where it is a known failure
...
Athena keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Athena-side `computer_use` workflow and action vocabulary. For
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. Athena
autodetection is a planned cua-driver follow-up, so currently point Athena at
...
These are **platform deep dives, not duplicates of the Athena skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Athena
...
image capture also saves a bounded, shareable copy under Athena' image cache
...
Pair with `approvals.mode: manual` in `~/.athena/config.yaml` if you
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Athena agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Athena agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Athena warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Athena from inside the RDP / console
...
On Linux, native Wayland support remains an explicit opt-in. Athena passes the
...
```
ATHENA_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
ATHENA_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Athena disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Athena sets
...
When it's on, `athena computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Athena at a binary you built from source instead
of the published release. Athena resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`ATHENA_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Athena testing (see notes).
...
### Option B — point Athena straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`ATHENA_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
ATHENA_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
ATHENA_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Athena is using your build

- `athena computer-use status` prints the resolved binary path and
  version.
- `athena computer-use doctor` confirms the binary is reachable and
...
- **Athena spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Athena YOLO, Athena instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Athena
  change needed either way.
- **Local builds skip the version check.** Athena warns when the
...
**First action when anything's off: run `athena computer-use doctor`.**
...
Run `athena computer-use install` to fetch the cua-driver binary, or
run `athena tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `athena computer-use doctor` will
...
- **Athena-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Athena `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. Athena autodetection is a
  planned follow-up; currently point Athena at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Athena among them)
