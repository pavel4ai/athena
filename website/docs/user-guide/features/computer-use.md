...
1. Run `athena tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Athena app). `athena computer-use doctor` will tell you which permission is missing. |
...
agent is acting here." Each Athena run declares its own cua-driver
**session id** (something like `athena-3a7b9c14d2e8`); the cursor's
...
Athena intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the Athena-side `computer_use` action vocabulary — the
...
- **Athena spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Athena-side skill** — `skills/computer-use/SKILL.md` — teaches the
...
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Athena (planned follow-up), this happens
