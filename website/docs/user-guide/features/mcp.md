...
The MCP server reads conversation data directly from Athena's session store (`~/.athena/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same `send_message` infrastructure as the Athena agent itself.
