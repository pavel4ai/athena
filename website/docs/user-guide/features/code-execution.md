...
1. The agent writes a Python script using `from athena_tools import ...`
2. Athena generates a `athena_tools.py` stub module with RPC functions
...
from athena_tools import web_search, web_extract
...
```python
from athena_tools import search_files, read_file
import json
...
```python
from athena_tools import search_files, read_file, patch
...
```python
from athena_tools import terminal, read_file
import json
...
`execute_code` has two execution modes controlled by `code_execution.mode` in `~/.athena/config.yaml`:
...
| **Stdout** | 50 KB | Shown head-and-tail inline; the full output is saved to `~/.athena/cache/exec/` and the path is included in the result |
...
**Remote backends** (Docker, SSH, Modal) run a remote session kernel with the same contract. If the kernel cannot be spawned on the backend, Athena falls back to running each call as a standalone script and says so in the result.

**Large output.** Stdout over 50 KB is shown head-and-tail inline, and the full text is saved under `~/.athena/cache/exec/` with the path included in the result, so the agent can page through it with `read_file` instead of re-running the script.
...
### `ATHENA_*` variables in the child

The child process receives only a small, fixed set of operational `ATHENA_*`
...
(plus `ATHENA_RPC_DIR` / `ATHENA_RPC_SOCKET` / `TZ` / `HOME`, which Athena
injects explicitly so the RPC channel works).

:::note Behavior change
Earlier versions passed **any** variable whose name began with `ATHENA_`
through to the child. That broad prefix was removed for security hardening: it
could leak `ATHENA_*`-named configuration that doesn't match a secret substring
(for example `ATHENA_BASE_URL`, `ATHENA_KANBAN_DB`, or a `ATHENA_*_WEBHOOK`
endpoint) into arbitrary sandboxed code.

If an `execute_code` script — or a repo/plugin module it imports at import time
— relied on a `ATHENA_*` variable outside the four operational names above, it
...
**Diagnosing it.** When the child drops one or more non-allowlisted `ATHENA_*`
variables, Athena emits a one-line `debug` log naming them and pointing at the
`env_passthrough` escape hatch. Run with debug logging (`athena logs --level
DEBUG`, or check `~/.athena/logs/agent.log`) and look for
`execute_code: dropped N non-allowlisted ATHENA_* var(s)` if a script behaves
as though a `ATHENA_*` variable is missing.

Athena always writes the script and the auto-generated `athena_tools.py` RPC stub into a temp staging directory that is cleaned up after execution. In `strict` mode the script also *runs* there; in `project` mode it runs in the session's working directory (the staging directory stays on `PYTHONPATH` so imports still resolve). The child process runs in its own process group so it can be cleanly killed on timeout or interruption.
...
Code execution is available on **Linux, macOS, and Windows**. On Linux and macOS the RPC channel uses a Unix domain socket; on Windows, where `AF_UNIX` is unreliable, Athena automatically falls back to a loopback TCP socket for the sandbox RPC transport. Remote terminal backends (Docker/SSH/Modal/etc.) use a file-based RPC transport instead and additionally require Python 3 inside the backend.
