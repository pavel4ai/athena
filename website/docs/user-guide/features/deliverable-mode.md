...
persona in `~/.athena/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.athena/config.yaml` (switchable per session
via `/personality`).
...
Install MCP servers via `~/.athena/config.yaml` under the `mcp_servers`
