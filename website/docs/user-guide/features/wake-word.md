...
sidebar_position: 11
title: "Wake Word"
description: "Hands-free 'Hey Athena' wake word — start a voice session by speaking, the 'Hey Siri' way"
---

# Wake Word ("Hey Athena")

The wake word turns Athena into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, Athena listens in the background for a spoken
trigger phrase. Say it, and Athena starts a fresh session, opens the microphone,
...
When the desktop app connects to a **remote** Athena backend (for example a
...
Athena supports **client capture** for that case:
...
|--------|------|---------|-------|
| **openWakeWord** (default) | Free | None | Local ONNX models. Ships a bundled **"hey athena"** model (default); also supports `hey_jarvis`, `alexa`, `hey_mycroft`, … and custom models |
...
By default the phrase is **"hey athena"** — a model for it ships with Athena, so
...
```bash
cd ~/.athena/athena-agent && uv pip install -e ".[wake]"
```

## Quick start

```bash
# In an interactive `athena` session:
...
desktop ear button — also writes `wake_word.enabled` to `~/.athena/config.yaml`,
...
wake_word:
  ...
  phrase: "hey athena"        # cosmetic label only — detection is keyed by the model/keyword below
  ...
  openwakeword:
    model: hey_athena         # bundled default; OR a built-in name OR a path to a custom .onnx/.tflite
...
- **`confirmation_frames`** (default `3`, openWakeWord only) — how many
  *consecutive* over-threshold frames are required before the wake fires. A real
  "hey athena" holds a high score across several frames; an ambient blip spikes
...
- **`sensitivity`** (default `0.6`) — the detection threshold, `0.0`–`1.0`.
  ...
  **all** engines — for openWakeWord it's the raw per-frame score threshold, for
  ...
  if real "hey athena" utterances are missed.
...
default) to let Athena choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which Athena installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three Athena surfaces, and `surface` picks which one
...
| `cli` | Only the classic `athena` CLI. |
| `tui` | Only `athena --tui`. |
| `gui` | Only the desktop app. |

The detector is on-device and single-mic, so only one surface listens at a time,
including when Athena surfaces run in separate processes. Ownership is sticky:
the first eligible claimant keeps the listener until it stops, disconnects, or
its process exits. Athena does not silently fail over to another open surface.
...
"Hey Athena" works out of the box — the bundled openWakeWord model
(`model: hey_athena`) is the default. To wake on something else, the easiest
...
command (`athena -p <profile>`) instead of routing.
...
wake_word:
  ...
  phrase: "computer"
  openwakeword:
    model: ~/.athena/wakewords/computer.onnx   # or a built-in name like hey_jarvis
```
...
syllables with an uncommon word ("athena" qualifies) beat common words like
"hello" or "stop".
:::

### Option C — Porcupine (custom keyword in seconds)

Create a "Hey Athena" keyword in the [Picovoice Console](https://console.picovoice.ai/),
...
wake_word:
  ...
  phrase: "hey athena"
  porcupine:
    keyword: ~/.athena/wakewords/hey_athena.ppn
```

Set your access key in `~/.athena/.env`:
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `athena tools` (Voice section) sets them up.
- The wake engine deps (auto-installed, or `athena-agent[wake]`).
...
but the phrase never fires. Athena detects this (`/wake status` shows
"mic delivers only silence"; the desktop ear tooltip carries the same hint).
Fix: System Settings → Privacy & Security → Microphone → enable the Athena
backend (it may appear as your terminal, `python`, or Athena), then toggle the
...
```bash
athena config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
athena config set wake_word.input_device null
```
