...
Athena has four hook systems that run custom code at key lifecycle points:

| System | Registered via | Runs in | Use case |
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.athena/hooks/` | Gateway only | Logging, alerts, webhooks |
...
| **[Outbound webhooks](#outbound-webhooks)** | `hooks.outbound:` list in `~/.athena/config.yaml` | CLI + Gateway | Push signed lifecycle events to external HTTP endpoints — CI, dashboards, other agents |
...
Each hook is a directory under `~/.athena/hooks/` containing two files:
...
LOG_FILE = Path.home() / ".athena" / "hooks" / "my-hook" / "activity.log"
...
```yaml
# ~/.athena/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.athena/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".athena" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.athena/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.athena/hooks/session-webhook/handler.py
import httpx

WEBHOOK_URL = "https://your-service.example.com/athena-events"
...
A popular pattern from the community: drop a Markdown checklist at `~/.athena/BOOT.md`, and have the agent run it once every time the gateway starts. Useful for "on every boot, check overnight cron failures and ping me on Discord if anything failed," or "summarize the last 24h of deploy.log and post it to Slack #ops."
...
1. A file at `~/.athena/BOOT.md` with natural-language startup instructions.
...
Create `~/.athena/BOOT.md`. Write it as if you were giving instructions to a human assistant:

```markdown
# Startup Checklist

1. Run `athena cron list` and check if any scheduled jobs failed overnight.
...
```text
~/.athena/hooks/boot-md/
├── HOOK.yaml
└── handler.py
```

**`~/.athena/hooks/boot-md/HOOK.yaml`**

```yaml
name: boot-md
description: Run ~/.athena/BOOT.md on gateway startup
events:
  - gateway:startup
```

**`~/.athena/hooks/boot-md/handler.py`**

```python
"""Run ~/.athena/BOOT.md on every gateway startup."""
...
BOOT_FILE = Path.home() / ".athena" / "BOOT.md"
...
```bash
athena logs --follow --level INFO | grep boot-md
```
...
Delete `~/.athena/BOOT.md` to disable the checklist — the hook stays loaded but silently skips when the file isn't there.
...
1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.athena/hooks/`
...
- Runtime event-name validity comes from `athena_cli.plugins.VALID_HOOKS`. `athena hooks list` lists configured shell/outbound hooks, not every available event; `athena hooks test <event>` reports the valid set only when an invalid event is supplied.
...
| `session_id` | `str` | Current Athena session id |
...
If a `pre_tool_call` callback exceeds `plugins.hook_callback_timeout` (or is still running from a previous timed-out fire), Athena **fails closed**: the tool is blocked with a timeout message rather than proceeding without a policy decision.
...
The clean user-message `content` remains unchanged. For replay and prompt-cache stability, Athena may persist the exact API-bound message, including plugin-injected context, in the row's `api_content` sidecar.
...
```python
VOCAB = "Athena, Teknium, Nous Research, kanban"
...
Declare shell-script hooks in your profile's `config.yaml` and Athena will run them as subprocesses whenever the corresponding plugin-hook event fires — in CLI, gateway, Desktop, TUI, and dashboard chat sessions. No Python plugin authoring required.
...
Shell hooks are registered by calling `agent.shell_hooks.register_from_config(cfg)` at both CLI startup (`athena_cli/main.py`) and gateway startup (`gateway/run.py`). They compose naturally with Python plugin hooks — both flow through the same dispatcher.

### Comparison at a glance

| Dimension | Shell hooks | [Plugin hooks](#plugin-hooks) | [Gateway hooks](#gateway-event-hooks) |
|-----------|-------------|-------------------------------|---------------------------------------|
| Declared in | `hooks:` block in `~/.athena/config.yaml` | `register()` in a `plugin.yaml` plugin | `HOOK.yaml` + `handler.py` directory |
| Lives under | `~/.athena/agent-hooks/` (by convention) | `~/.athena/plugins/<name>/` | `~/.athena/hooks/<name>/` |
...
{"action": "modify", "args": {"new_string": "fixed content"}}         // Athena-canonical
...
hooks:
  pre_tool_call:
    - matcher: "terminal|write_file|patch"
      command: "~/.athena/agent-hooks/secret-scan.sh"
      timeout: 10
      fail_closed: true
```
...
`fail_closed` only applies to blocking-capable events (`pre_tool_call` today); setting it on any other event logs a warning at config-parse time and is ignored. `athena hooks test` reflects these semantics — the `parsed` line shows exactly the block shape the dispatcher would receive.
...
hooks:
  post_tool_call:
    - matcher: "write_file|patch"
      command: "~/.athena/agent-hooks/auto-format.sh"
```

```bash
#!/usr/bin/env bash
# ~/.athena/agent-hooks/auto-format.sh
payload="$(cat -)"
...
hooks:
  pre_tool_call:
    - matcher: "terminal"
      command: "~/.athena/agent-hooks/block-rm-rf.sh"
      timeout: 5
```

```bash
#!/usr/bin/env bash
# ~/.athena/agent-hooks/block-rm-rf.sh
payload="$(cat -)"
...
hooks:
  pre_llm_call:
    - command: "~/.athena/agent-hooks/inject-cwd-context.sh"
```

```bash
#!/usr/bin/env bash
# ~/.athena/agent-hooks/inject-cwd-context.sh
...
hooks:
  subagent_stop:
    - command: "~/.athena/agent-hooks/log-orchestration.sh"
```

```bash
#!/usr/bin/env bash
# ~/.athena/agent-hooks/log-orchestration.sh
log=~/.athena/logs/orchestration.log
...
Each unique `(event, command)` pair prompts the user for approval the first time Athena sees it, then persists the decision to `~/.athena/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.

Three escape hatches bypass the interactive prompt — any one is sufficient:

1. `--accept-hooks` flag on the CLI (e.g. `athena --accept-hooks chat`)
2. `ATHENA_ACCEPT_HOOKS=1` environment variable
3. `hooks_auto_accept: true` in `~/.athena/config.yaml`
...
**Script edits are silently trusted.** The allowlist keys on the exact command string, not the script's hash, so editing the script on disk does not invalidate consent. `athena hooks doctor` flags mtime drift so you can spot edits and decide whether to re-approve.

#### Manual allowlisting

Manual allowlisting is useful for non-TTY or service-account deployments where an operator cannot answer the first-use prompt interactively. The allowlist file is `~/.athena/shell-hooks-allowlist.json`, and the expected format is an `approvals` array. Each approval records the hook `event` and the exact `command` string:
...
```json
{
  "approvals": [
    {
      "event": "post_llm_call",
      "command": "/home/athena/.athena/hooks/my-hook.py"
    }
  ]
}
```

The command string must match the configured hook command exactly. A path-keyed object with a `sha256` field is not the expected format and will not approve the hook. Verify manual entries with `athena hooks list`.

### The `athena hooks` CLI

| Command | What it does |
|---------|--------------|
| `athena hooks list` | Dump configured hooks with matcher, timeout, and consent status |
| `athena hooks test <event> [--for-tool X] [--payload-file F]` | Fire every matching hook against a synthetic payload and print the parsed response |
| `athena hooks revoke <command>` | Remove every allowlist entry matching `<command>` (takes effect on next restart) |
| `athena hooks doctor` | For every configured hook: check exec bit, allowlist status, mtime drift, JSON output validity, and rough execution time |
...
- Keep scripts inside `~/.athena/agent-hooks/` so the path is easy to audit.
- Re-run `athena hooks doctor` after you pull a shared config to spot newly-added hooks before they register.
...
Outbound webhooks are the push-side mirror of the [inbound webhook platform](/user-guide/messaging/webhooks): inbound webhooks wake Athena when the world changes; outbound webhooks tell the world when Athena does something. Configure a list of HTTP endpoints and the lifecycle events they care about, and Athena POSTs a signed JSON payload to each endpoint whenever a matching event fires — no polling on the receiving end.
...
- Wake *another* Athena instance: point the URL at that instance's inbound webhook

### Configuration

Add a `hooks.outbound:` list to `~/.athena/config.yaml`:

```yaml
hooks:
  outbound:
    - name: ci-notify                       # optional label for logs
      url: https://ci.example.com/athena-events
      events: [on_session_end, subagent_stop]
      secret_env: ATHENA_OUTBOUND_WEBHOOK_SECRET   # env var holding the HMAC secret
      timeout: 10                           # per-attempt seconds (1–60)

    - name: tool-monitor
      url: https://metrics.example.com/hooks/athena
...
Secrets: prefer `secret_env` (the name of an environment variable, typically set in `~/.athena/.env`) over an inline `secret:` literal, so the config file stays free of credentials. Entries without a secret are delivered unsigned (flagged as `UNSIGNED` by `athena hooks list`).

### Wire format

Each firing POSTs a JSON body with the same top-level shape as shell hooks' stdin, plus delivery metadata. `profile` names the Athena profile that emitted the event (`"default"` outside profiles), so receivers behind a multiplexed gateway can tell profiles apart:
...
| `X-Athena-Event` | The hook event name |
| `X-Athena-Delivery` | Unique id per delivery — same value as `delivery_id` in the body |
| `X-Athena-Signature-256` | `sha256=<hex>` — HMAC-SHA256 of the raw body, GitHub-style; only present when a secret is configured |
...
- **Dedupe** on `delivery_id` (or the matching `X-Athena-Delivery` header) — remember recently seen ids and skip duplicates. Athena retries failed deliveries once, so the same id can legitimately arrive twice.
...
- **No consent prompt.** Outbound targets execute no code on your machine — they receive data at a URL you configured. `ATHENA_SAFE_MODE=1` still skips registration, same as plugins and shell hooks. Note that payloads include tool inputs and event metadata, so only point targets at endpoints you trust, and prefer `https://`.

`athena hooks list` shows configured outbound targets alongside shell hooks, including whether each target is signed.
