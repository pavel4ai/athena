...
`/goal` and [Kanban](./kanban) both keep Athena working without you re-prompting, so it's tempting to assume one flows into the other. It doesn't — the boundary is sharp:
...
| Many independent tasks, with dependencies, handoffs, or multiple profiles | [Kanban](./kanban) — `athena kanban create …` |
| One card on the board that should keep iterating until its acceptance criteria are met | A kanban card with `--goal` |

:::note
If you want work on the board, put it there yourself (`athena kanban create …`) — `/goal` won't do it for you. The reverse is also true: pausing, resuming, or clearing a goal in this chat never creates, claims, or moves a kanban card.
:::

## Quick start

```
/goal Fix every failing test in tests/athena_cli/ and make sure scripts/run_tests.sh passes for that directory
```
...
`/goal draft <text>` both creates the goal and starts its first turn, including when drafting is unavailable and Athena falls back to a free-form goal. `draft` is a whole-word subcommand: `/goal drafting docs` keeps `drafting docs` as the literal objective without calling the draft model.
...
| `stop_when` | The condition under which Athena should stop and ask for input. |
...
**1. Let Athena draft it** (recommended — adapted from Codex's "let the agent draft the goal" tip):

```
/goal draft Migrate the auth service from session cookies to JWT
```

Athena expands your one-liner into a full contract via the `goal_judge` auxiliary model, sets it, and shows you the result so you can review or tighten any field. If the aux model is unavailable, it falls back to a plain free-form goal — drafting never blocks setting a goal.
...
/goal gate add scripts/run_tests.sh tests/athena_cli/test_goals.py
```
