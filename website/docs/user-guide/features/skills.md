...
```bash
curl -fsSL https://github.com/pavel4ai/athena/install.sh | bash -s -- --no-skills
```
...
- **Create locally, update in place**: New agent-created skills are written to `~/.athena/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
...
[Skills Hub](https://github.com/pavel4ai/athena/docs) page — instead of a
...
If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.athena/.hub/taps.json`:
...
Taps are stored in `~/.athena/.hub/taps.json` (created on demand).
