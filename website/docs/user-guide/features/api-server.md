...
**Streaming** (`"stream": true`): Returns Server-Sent Events (SSE) with token-by-token response chunks. For **Chat Completions**, the stream uses standard `chat.completion.chunk` events plus Athena' custom `athena.tool.progress` event for tool-start UX. For **Responses**, the stream uses OpenAI Responses event types such as `response.created`, `response.output_text.delta`, `response.output_item.added`, `response.output_item.done`, and `response.completed`.

**Tool progress in streams**:
- **Chat Completions**: Athena emits `event: athena.tool.progress` for tool-start visibility without polluting persisted assistant text.
...
Tool calls in the `output` array were already executed server-side by the Athena agent — they are replayed with `"status": "completed"` for structured tool UI, never as pending calls for the client to execute.
...
enumerate every authenticated provider/model combination Athena can route to,
and it does not do pricing or capability enrichment.

### GET /api/model/options

Athena-aware clients can request the same curated provider/model inventory used
...
Normal opens are intentionally conservative for custom providers: Athena probes
...
authenticated UI needs the richer Athena-specific picker metadata.
...
```json
{
  "object": "athena.api_server.capabilities",
...
Athena can route browser tools through an authenticated extension that controls
the browser session associated with the current Athena session. The feature is
...
register only for an existing server session. Athena derives the controller
...
Athena preserves the existing browser backend. Once the gateway binds a
...
Athena never retries the same action through another backend.
...
2. Athena returns a single-use ticket with a 30-second TTL and the filtered,
   server-bound controller scope.
3. Open `GET /v1/browser-control/ws` with both WebSocket subprotocols:
   `athena-browser-control-v1` and
   `athena-browser-control-ticket.<ticket>`.
...
Athena sends `browser.controller.command` frames containing `command_id`,
...
Authenticated clients can override Athena' default model selection per request
by sending:

- `model` — the target model id for this turn
- `provider` — the Athena provider slug to resolve credentials/runtime for this turn
...
alias, Athena rejects the request with `400` instead of silently remixing route
...
Requests that include an explicit `provider` — and the Athena-native
...
For safely retryable creation, send an `Idempotency-Key` header (1–255 visible ASCII characters). Athena durably reserves the key before starting work. An identical retry returns the original `run_id` with HTTP 202 and `Idempotency-Replayed: true`, including after a gateway restart and after the run has completed, failed, or been cancelled. Reusing the same key with a different JSON payload returns HTTP 409 with code `idempotency_key_conflict`. Keys are isolated by authenticated API profile/credential and retained for 24 hours after their last status update; clients should use unique, unguessable keys and must not reuse them for unrelated operations. Requests without the header retain the legacy behavior and always create a new run.

When `session_id` identifies an existing Athena session and no explicit
...
history: an explicit `X-Athena-Session-Id` on Chat Completions, a native
...
Create a new scheduled job. Body accepts the same shape as `athena cron` — prompt, schedule, skills, provider override, delivery target.
...
- Requests to `/p/<profile>/v1/...` must present that profile's own
  `API_SERVER_KEY` (from `~/.athena/profiles/<profile>/.env`). The default
...
The same settings can live in `~/.athena/config.yaml` under a nested `gateway.api_server:` section:
...
- **`X-Athena-Session-Id`** is an allowed request header, so browsers on an allowlisted origin can request session continuation.
...
athena profile create alice
athena profile create bob
...
cat >> ~/.athena/profiles/alice/.env <<EOF
API_SERVER_ENABLED=true
API_SERVER_PORT=8643
API_SERVER_KEY=alice-secret
EOF

cat >> ~/.athena/profiles/bob/.env <<EOF
API_SERVER_ENABLED=true
...
athena -p alice gateway &
athena -p bob gateway &
```
...
- **Simple OpenAI clients still see an alias** — `/v1/models` advertises the
  stable Athena alias (`athena-agent` or the active profile name). Richer
