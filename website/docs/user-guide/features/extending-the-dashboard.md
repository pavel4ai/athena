...
A reference plugin (`example-dashboard` in [`athena-example-plugins`](https://github.com/FutureboundCorp/athena-example-plugins/tree/main/example-dashboard)) ships a live demo that injects a banner into `sessions:top` — install it to see the pattern end-to-end.
...
| 2 | `~/.athena/plugins/<name>/dashboard/` | `user` |
...
The [`strike-freedom-cockpit`](https://github.com/FutureboundCorp/athena-example-plugins/tree/main/strike-freedom-cockpit) plugin (companion repo `athena-example-plugins`) is a complete reskin demo. It pairs a theme YAML with a slot-only plugin to produce a cockpit-style HUD without forking the dashboard.
...
```bash
git clone https://github.com/FutureboundCorp/athena-example-plugins.git
...
1. Confirm the plugin is bundled with Athena. User-installed and project dashboard plugins can extend the UI, but their Python backend routes are not auto-imported.
2. Confirm the manifest has `"api": "plugin_api.py"` pointing to an existing file inside `dashboard/`.
3. Restart `athena dashboard` — plugin API routes are mounted once at startup, **not** on rescan.
4. Check that `plugin_api.py` exports a module-level `router = APIRouter()`. Other export names are not picked up.
5. Tail `~/.athena/logs/errors.log` for `Failed to load plugin <name> API routes` — import errors are logged there.
