...
:::tip
Not using OpenAI Codex? `athena setup --portal` configures a non-Codex backend with Claude/Gemini/etc. in one step. See [Nous Portal](/integrations/nous-portal).
:::
...
When the model wants one of these, codex spawns the `athena_tools_mcp_server` subprocess via stdio MCP, the call is dispatched through `model_tools.handle_function_call()` (same code path as Athena' default runtime), and the result is returned to codex like any other MCP response.
...
**Works on this runtime, with one subtle dependency.** The kanban dispatcher spawns each worker as a separate `athena chat -q` subprocess that reads the user's config — which means if `model.openai_runtime: codex_app_server` is set globally, workers also come up on the codex runtime.
...
- **`kanban_complete` / `kanban_block` / `kanban_comment` / `kanban_heartbeat`** — the worker handoff tools. These read `ATHENA_KANBAN_TASK` from env (set by the dispatcher), gate access correctly, and write to the per-board SQLite DB pinned by `ATHENA_KANBAN_DB`. Without these in the callback, a worker on this runtime could do its task but couldn't report back, hanging until the dispatcher's timeout.
...
The kanban tools are gated by `ATHENA_KANBAN_TASK` env var the dispatcher sets — that var is propagated to the codex subprocess (codex inherits env) and from there to the spawned `athena-tools` MCP server subprocess. So the tools see the right task id and gate correctly. For Codex app-server workers, Athena also passes narrow app-server sandbox overrides when `ATHENA_KANBAN_TASK` is present: keep `workspace-write` sandboxing, add the **board DB directory plus every Kanban path the dispatcher pinned** as extra writable roots (`ATHENA_KANBAN_WORKSPACES_ROOT`, `ATHENA_KANBAN_WORKSPACE`, legacy `ATHENA_KANBAN_ROOT` — deduplicated, DB-dir first), and keep network disabled by default. This avoids the brittle `:danger-no-sandbox` workaround while letting `kanban_complete` / `kanban_block` update the board DB **and** letting workers write reports/artifacts under workspace mounts that live outside the DB directory (e.g. `/media/.../kanban-workspaces/...` on a separate drive — [issue #27941](https://github.com/pavel4ai/athena/issues/27941)).
...
2. **Codex OAuth login.** The codex subprocess reads `~/.codex/auth.json`. Two ways to populate it:
   ...
   ```
   Athena' own `athena auth login codex` writes to `~/.athena/auth.json` — that's a separate session. **Run `codex login` separately** if you haven't.
...
- Migrates user MCP servers from `~/.athena/config.yaml` to `~/.codex/config.toml`.
...
You can also set it manually in `~/.athena/config.yaml`:
```yaml
model:
...
To route specific aux tasks to a cheaper / different model, set explicit overrides in `~/.athena/config.yaml`:
...
By default, Athena points the codex subprocess at `~/.codex/` regardless of which Athena profile is active. This means `athena -p work` and `athena -p personal` share the same Codex auth, plugins, and config. For most users this is the right behavior — it matches what running `codex` CLI directly would do.

If you want per-profile Codex isolation (separate auth, separate installed plugins, separate config), set `CODEX_HOME` explicitly per profile. The cleanest way is to point at a directory under your `ATHENA_HOME`:

```bash
# Inside the work profile, you might wrap athena:
CODEX_HOME=~/.athena/profiles/work/codex athena chat
```

You'll need to re-run `codex login` once with that `CODEX_HOME` set so the OAuth tokens land in the profile-scoped location. After that, `athena -p work` will operate on isolated Codex state.
...
args = ["-m", "agent.transports.athena_tools_mcp_server"]
env = { ATHENA_HOME = "/your/.athena", PYTHONPATH = "...", ATHENA_QUIET = "1" }
startup_timeout_sec = 30.0
tool_timeout_sec = 600.0
```

When the model calls `web_search` (or another exposed Athena tool), codex spawns the `athena_tools_mcp_server` subprocess via stdio, the request is dispatched through `model_tools.handle_function_call()`, and the result is projected back to codex like any other MCP response.
...
- MCP tool calls (verified against `@modelcontextprotocol/server-filesystem` and the new `athena-tools` callback)
...
If you find a bug, [open an issue](https://github.com/pavel4ai/athena/issues) with the output of `athena logs --since 5m`. Mention `codex-runtime` in the title so it's easy to triage.
...
```
        ...
        └──────────────────────────────────┘           │
                                                        ...
                                                        ▼
        ┌──────────────────────────────────────────────────────────┐
        │  athena_tools_mcp_server.py (subprocess on demand)        │
        │   web_search, web_extract, browser_*, vision_analyze,    │
        │   image_generate, skill_view, skills_list, text_to_speech│
        └──────────────────────────────────────────────────────────┘
```

For implementation details, see [PR #24182](https://github.com/pavel4ai/athena/pull/24182) and the [Codex app-server protocol README](https://github.com/openai/codex/blob/main/codex-rs/app-server/README.md).
