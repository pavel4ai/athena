...
fills the password through Athena. The tool result it sees is
...
- **Authenticator key saved with the login** (the "setup key" or `otpauth://`
  ...
  hold a TOTP seed count too): Athena generates the current code and enters
  it. Nobody is asked. Add the key in **Settings → Passwords & Logins → Add**
  or `athena vault add`; the item shows a *2FA auto* badge.
- **Code sent to your phone or email**: a small prompt appears in your
  surface ("Verification code for github.com"), you type the code, Athena
...
in, Athena picks it up automatically and its website logins become fillable
...
per session, 30 minutes idle). Athena hands the master password to the manager's
...
Prefer not to use a detected manager? `athena vault sources --disable bitwarden`,
...
Passwords & Logins → Add**, or `athena vault add`), bound to the checkout site,
...
- **CLI**: `athena vault list`, `athena vault add`, `athena vault rm <handle>`,
  `athena vault sources`.

Items live encrypted under `~/.athena/vault/` (Fernet key + vault file, both
...
Cron jobs, webhooks, the API server and `athena chat -q` have nobody to answer a
...
**Does:** the password never enters the model's context through Athena: not in
