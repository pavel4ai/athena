...
Athena ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.athena/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Athena Plugin](/developer-guide/plugins) to write your own.
...
2. **User** — `~/.athena/plugins/<name>/`
3. **Project** — `./.athena/plugins/<name>/` (requires `ATHENA_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `athena_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `athena memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `athena plugins list` and the interactive `athena plugins` UI), but none load until you explicitly enable them:

```bash
athena plugins enable disk-cleanup
```

Or via `~/.athena/config.yaml`:
...
```bash
athena plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `athena plugins enable <name>`.
...
| `athena-achievements` | dashboard tab | Steam-style collectible badges generated from your real Athena session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `athena memory` and `athena plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ATHENA_HOME` or `/tmp/athena-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under ATHENA_HOME | always | Never |
...
**State** — everything lives at `$ATHENA_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `ATHENA_HOME` or `/tmp/athena-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `athena plugins enable disk-cleanup` (or check the box in `athena plugins`).

**Disabling again:** `athena plugins disable disk-cleanup`.
...
**Enabling:** `athena plugins enable security-guidance` (or check the box in `athena plugins`).

**Disabling again:** `athena plugins disable security-guidance`.
...
```bash
athena tools          # → Langfuse Observability → Cloud or Self-Hosted
```
...
Then put the credentials in `~/.athena/.env`:
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `ATHENA_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Athena session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `athena chat` session lives under one Langfuse session.

**Verify:**

```bash
athena plugins list                 # observability/langfuse should show "enabled"
athena chat -q "hello"              # check the Langfuse UI for a "Athena turn" trace
```
...
| `ATHENA_LANGFUSE_RELEASE` | — | Release/version tag |
| `ATHENA_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `ATHENA_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `ATHENA_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |
...
**Disabling:** `athena plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.

### NeMo Relay native integration (migration note)

NeMo Relay is no longer a bundled Athena plugin. Do not run `athena plugins enable observability/nemo_relay`; Athena core now owns the Relay session, turn, LLM, and tool lifecycles.

To opt into Relay middleware or exporters, create a standard Relay `plugins.toml`, then set `ATHENA_NEMO_RELAY_PLUGINS_TOML` to that file before starting Athena. The policy is process-wide for every profile hosted by that Athena process. See the [NeMo Relay observability configuration](https://docs.nvidia.com/nemo/relay/configure-plugins/observability/about) for ATOF, ATIF, and OpenTelemetry options.

The old `ATHENA_NEMO_RELAY_ATOF_*` and `ATHENA_NEMO_RELAY_ATIF_*` settings no longer activate exporters. `athena doctor` reports these stale settings when no replacement `plugins.toml` is selected.
...
Both defaults preserve one session scope for the full session. Rotated spans retain the same `session_id` and add `athena.session.segment` plus `athena.session.segment_reason` (`compaction` or `max_turns`).
...
- Post-meeting artifacts (transcript, status) saved under `~/.athena/workspace/meetings/<meeting_id>/`

**Setup:**

```bash
athena plugins enable google_meet
athena meet setup   # preflight: playwright, chromium, auth file
athena meet auth    # opens a browser to sign into Google and saves session state —
...
**Disabling:** `athena plugins disable google_meet`. Any saved transcripts stay in `~/.athena/workspace/meetings/` until you remove them.
...
- Scans your entire `~/.athena/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$ATHENA_HOME/plugins/athena-achievements/state.json`
...
**API** — routes mount under `/api/plugins/athena-achievements/`:
...
**State files** — live under `$ATHENA_HOME/plugins/athena-achievements/`:
...
**Enabling:** Nothing to enable — `athena-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `athena dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/athena-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.athena/plugins/athena-achievements/` that ships no dashboard. The plugin's state files under `$ATHENA_HOME/plugins/athena-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Athena plugin — see [Build a Athena Plugin](/developer-guide/plugins). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.athena/plugins/<name>/`
- Manifest source is reported as `bundled` in `athena plugins list`
