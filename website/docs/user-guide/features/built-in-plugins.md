...
See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Athena Plugin](/guides/build-a-athena-plugin) to write your own.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.athena/cache/google_meet/<meeting_id>/`
...
**Disabling:** `athena plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.athena/cache/google_meet/` until you remove them.
...
Bundled plugins are written exactly like any other Athena plugin — see [Build a Athena Plugin](/guides/build-a-athena-plugin). The only differences are:
