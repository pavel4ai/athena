...
/skin mytheme        # switch to a custom skin from ~/.athena/skins/mytheme.yaml
```

Or set the default skin in `~/.athena/config.yaml`:
...
| `banner_logo` | string | Rich-markup ASCII art logo (replaces the default ATHENA_AGENT banner) | `""` |
| `banner_hero` | string | Rich-markup hero art (replaces the default caduceus art) | `""` |

## Custom skins

Create YAML files under `~/.athena/skins/`. User skins inherit missing values from the built-in `default` skin, so you only need to specify the keys you want to change.

### Full custom skin YAML template

```yaml
# ~/.athena/skins/mytheme.yaml
...
[Athena Mod](https://github.com/cocktailpeanut/athena-mod) is a community-built web UI for creating and managing skins visually. Instead of writing YAML by hand, you get a point-and-click editor with live preview.

![Athena Mod skin editor](https://raw.githubusercontent.com/cocktailpeanut/athena-mod/master/nous.png)
...
- Saves directly to `~/.athena/skins/`
- Activates a skin by updating `~/.athena/config.yaml`
...
```bash
git clone https://github.com/cocktailpeanut/athena-mod.git
cd athena-mod/app
npm install
npm start
```
...
6. Click **Save** to write the skin YAML to `~/.athena/skins/`.
7. Click **Activate** to set it as the current skin (updates `display.skin` in `config.yaml`).

Athena Mod respects the `ATHENA_HOME` environment variable, so it works with [profiles](/user-guide/profiles) too.

## Operational notes

- Built-in skins load from `athena_cli/skin_engine.py`.
- Unknown skins automatically fall back to `default`.
- `/skin` updates the active CLI theme immediately for the current session.
- User skins in `~/.athena/skins/` take precedence over built-in skins with the same name.
