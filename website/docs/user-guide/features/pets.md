...
Athena can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<ATHENA_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Athena renders the sprite at full fidelity when your
...
athena pets list
athena pets list cat

# Install a pet and make it active in one step
athena pets install boba --select
...
## `athena pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `athena pets list [query] [--limit N]` |
| List installed pets | `athena pets list --installed` |
| Install a pet | `athena pets install <slug> [--select] [--force]` |
| Set the active pet | `athena pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `athena pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `athena pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `athena pets off` |
| Remove an installed pet | `athena pets remove <slug>` |
| Diagnose setup | `athena pets doctor` |

`athena pets show` flags:
...
Beyond installing pre-made pets from the gallery, Athena can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `athena pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `athena tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `ATHENA_PET_IMAGE_PROVIDER` env var (e.g. `ATHENA_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Athena is
...
Run `athena pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Athena uses its own
  profile-scoped `<ATHENA_HOME>/pets/` instead — install through `athena pets`.

## See also

- The [`athena-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-athena-agent.md)
