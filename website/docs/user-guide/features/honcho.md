...
```bash
athena memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `athena` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
The `athena honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `athena memory setup honcho` (or run `athena memory setup` and pick it from the list); the `athena honcho` subcommand then appears on the next invocation.

```bash
athena memory setup honcho    # Configure Honcho directly (works before activation)
athena honcho status          # Connection status, config, and key settings
athena honcho setup           # Redirects to `athena memory setup` (post-activation alias)
athena honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
athena honcho peer            # Show or update peer names + dialectic reasoning level
athena honcho mode            # Show or set recall mode (hybrid/context/tools)
athena honcho tokens          # Show or set token budget for context and dialectic
athena honcho identity        # Seed or show the AI peer's Honcho identity
athena honcho sync            # Sync Honcho config to all existing profiles
athena honcho peers           # Show peer identities across all profiles
athena honcho sessions        # List known Honcho session mappings
athena honcho map             # Map current directory to a Honcho session name
athena honcho enable          # Enable Honcho for the active profile
athena honcho disable         # Disable Honcho for the active profile
athena honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `athena honcho`

If you previously used the standalone `athena honcho setup`:
...
No re-login or re-setup needed. Run `athena memory setup` and select "honcho" — the wizard detects your existing config.
