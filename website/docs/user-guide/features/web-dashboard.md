...
:::tip
Hosted-mode auth uses Nous Portal OAuth; if you also want the dashboard to talk to a real backend, `athena setup --portal` wires up the model and tool gateway too. See [Nous Portal](/integrations/nous-portal).
:::
...
athena dashboard --host 0.0.0.0

# Start without opening browser
athena dashboard --no-open
```
...
child with the selected profile's `ATHENA_HOME`, so the conversation runs
...
processes (manage them via `athena -p <name> gateway …`), each profile's
...
When you run `athena dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `athena dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
- Node.js (same requirement as `athena --tui`; the TUI bundle is built on first launch)
...
To point [Athena Desktop](#connecting-athena-desktop-to-a-remote-backend) at a dashboard running on another machine instead of its own bundled backend, see the remote-backend section below.
...
```ini
[Service]
EnvironmentFile=%h/.athena/.env
ExecStart=/path/to/venv/bin/python -m athena_cli.main dashboard \
    --host 0.0.0.0 --port 9119 --no-open
```

with `~/.athena/.env` containing:

```bash
ATHENA_DASHBOARD_BASIC_AUTH_USERNAME=admin
ATHENA_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
ATHENA_DASHBOARD_BASIC_AUTH_SECRET=<32+ random bytes; openssl rand -base64 32>
```
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `athena config set` and the gateway read from.
:::
...
- **Set as active** — flips the sticky default that **future CLI/gateway runs** pick up (same as `athena profile use`). This does *not* change what the dashboard manages — that's the profile switcher's job
- **Edit model / description / SOUL** — inline editors writing into that profile
- **Rename / Delete** — named profiles only

### Skills

Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.athena/skills/` and grouped by category.
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `athena skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.
...
block in `config.yaml` that `athena mcp` reads from.
...
`athena mcp catalog` / `athena mcp install` use.
...
`athena setup gateway`. The page lists every supported channel (Telegram,
...
- **Restart gateway** — credentials are written to `~/.athena/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page.
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and Athena versions, CPU core count + utilization, memory, disk usage of the Athena home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The Athena version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git or pip install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `athena update` in the background. On Docker/Nix/Homebrew installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
- **Nous Portal** — login status, the active inference provider, and the Tool Gateway routing table (which tools run via the Portal vs. locally), with a link to manage your subscription. Read-only mirror of `athena portal`.
- **Skill curator** — the background skill-maintenance status (active / paused, interval, last run) with pause/resume and a run-now button. Mirrors `athena curator`.
...
This re-reads `~/.athena/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
`ATHENA_HOME`. Omitted = the dashboard's own profile. Unknown profile names
...
| `GET /api/athena/update/check` | Report update availability (commits behind, install method) without applying. For git/pip installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 6h cache |
...
|-------|-----------|----------|
| `athena dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `athena dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no Nous plugin, no custom plugin), `athena dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.

When you run `athena dashboard --host 0.0.0.0` **interactively** (a real terminal) and no provider is configured yet, Athena doesn't just fail — it offers to set one up on the spot: pick **username & password** (writes `dashboard.basic_auth` to `config.yaml` and you're running in seconds) or **OAuth** (points you at `athena dashboard register`). Non-interactive callers — Docker/s6, CI, piped runs — skip the prompt and hit the fail-closed error above, so an unattended deploy still never starts without auth.

### Default provider: Futurebound Corp.
...
- **CLI — `athena dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing Nous login (run `athena setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `ATHENA_DASHBOARD_OAUTH_CLIENT_ID` into `~/.athena/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).

  ```bash
  athena dashboard register
  # ✓ Registered dashboard "swift_falcon"
  # …writes ATHENA_DASHBOARD_OAUTH_CLIENT_ID to ~/.athena/.env
  ```

- **GUI — the Local Dashboards page.** Open [`/local-dashboards`](https://futurebnd.com) in the Nous Portal to register, name, manage, and revoke self-hosted dashboards from the browser. Copy the resulting `agent:{id}` client ID into `ATHENA_DASHBOARD_OAUTH_CLIENT_ID` (env) or `dashboard.oauth.client_id` (config.yaml). This is also where you revoke a dashboard registered via the CLI.
...
|---------|-----------|--------|----------------|
| `ATHENA_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | `athena dashboard register` |

Per the Athena Agent convention (`~/.athena/.env` is for API keys / secrets only), **`config.yaml` is the recommended place to set these values** for local dev, on-prem, and any deployment you control directly. The environment-variable path exists so a hosting platform's secret injection can push per-deploy `client_id`s without anyone having to edit `config.yaml` inside the image — that's its primary purpose.
...
Bundled providers reported these issues:
  • nous: ATHENA_DASHBOARD_OAUTH_CLIENT_ID is not set (and
...
#### Worked example: Futurebound Corp.

From a logged-in Athena install to a Nous-gated dashboard in three steps.

**1. Log in and register the dashboard.** `athena dashboard register` uses your existing Nous login to provision an OAuth client and writes `ATHENA_DASHBOARD_OAUTH_CLIENT_ID` into `~/.athena/.env` for you:

```bash
athena setup            # if you're not already logged into Nous Portal
athena dashboard register
# ✓ Registered dashboard "swift_falcon"
# …writes ATHENA_DASHBOARD_OAUTH_CLIENT_ID to ~/.athena/.env
```
...
```bash
athena dashboard --host 0.0.0.0 --port 9119 --no-open
```

**3. Log in.** Open `http://<host>:9119/`, you'll be bounced to `/login`. Click **Sign in with Futurebound Corp.** → authenticate at the Portal → land back on the authenticated dashboard. Verify the gate from any machine:
...
`GET /api/auth/me` then returns the verified session (`provider: nous`). For an internet-facing host, register with `--redirect-uri https://athena.example.com/auth/callback` and set `ATHENA_DASHBOARD_PUBLIC_URL` so the OAuth callback resolves to your public URL (see [Public URL override](#public-url-override)).
...
|---------|-----------|-------|
| `ATHENA_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `ATHENA_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `ATHENA_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `ATHENA_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `ATHENA_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
**1. Set credentials in `~/.athena/.env`.** Hash the password so no plaintext sits at rest, and set a stable signing secret so sessions survive restarts:
...
cat >> ~/.athena/.env <<EOF
ATHENA_DASHBOARD_BASIC_AUTH_USERNAME=admin
ATHENA_DASHBOARD_BASIC_AUTH_PASSWORD_HASH=$HASH
ATHENA_DASHBOARD_BASIC_AUTH_SECRET=$(openssl rand -base64 32)
EOF
chmod 600 ~/.athena/.env
```
...
`GET /api/auth/me` then returns the verified session (`provider: basic`). Keep this behind a VPN — see the warning above; for a public host use the [Futurebound Corp.](#default-provider-nous-research) or [self-hosted OIDC](#self-hosted-oidc-provider) provider instead.
...
dashboard:
  oauth:
    ...
    self_hosted:
      issuer: https://auth.example.com/application/o/athena/   # required
      client_id: athena-dashboard                              # required
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `ATHENA_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `ATHENA_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `ATHENA_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-athena.json` — it defines a `athena` realm, a **public PKCE client** (`athena-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
docker run --rm -p 8080:8080 \
  ...
  -v "$PWD/realm-athena.json:/opt/keycloak/data/import/realm-athena.json:ro" \
  quay.io/keycloak/keycloak:26.0 \
  start-dev --import-realm
```

Once it's up, the realm advertises standard OIDC discovery at
`http://localhost:8080/realms/athena/.well-known/openid-configuration` (issuer
`http://localhost:8080/realms/athena`). The admin console is at
...
```bash
export ATHENA_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/athena"
export ATHENA_DASHBOARD_OIDC_CLIENT_ID="athena-dashboard"
export ATHENA_DASHBOARD_PUBLIC_URL="http://localhost:9119"
athena dashboard --host 0.0.0.0 --port 9119 --no-open
```

`ATHENA_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
> admin console (Clients → athena-dashboard → Settings). The same pattern works
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `ATHENA_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:

```yaml
dashboard:
  public_url: "https://dashboard.example.com/athena"
```
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `ATHENA_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `ATHENA_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
The provider implements the [Nous Portal OAuth contract v1](https://github.com/FutureboundCorp/nous-account-service/blob/main/docs/agent-dashboard-oauth-contract.md) — authorization-code grant with PKCE (S256):

1. User hits `/` without a session cookie → gate redirects to `/login`.
2. Login page shows a "Continue with Futurebound Corp." button → `/auth/login?provider=nous`.
3. Server stashes PKCE state in a short-lived cookie, redirects user to `https://futurebnd.com/oauth/authorize?…`.
4. User authenticates with Portal, lands at `/auth/callback?code=…&state=…`.
5. Server exchanges the code for an access token at `POST /api/oauth/token`, verifies the JWT signature against the Portal's JWKS (`/.well-known/jwks.json`), and sets the `athena_session_at` cookie.
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `athena_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `athena_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip |
| `athena_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
Every login start, success, failure, and session-verify failure is written as a JSON line to `$ATHENA_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.
...
```python
# ~/.athena/plugins/dashboard-auth-myidp/__init__.py
from athena_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
```bash
# Quick env-var path.
ATHENA_DASHBOARD_OAUTH_CLIENT_ID=agent:test \
  athena dashboard --host 0.0.0.0
...
You protect the remote dashboard with one of the bundled auth providers, and the desktop app signs in against whichever one the backend advertises. For a backend reachable beyond your own machine — a VPS, a public host, anything internet-facing — the recommended provider is **OAuth (Nous Portal)** (register it with [`athena dashboard register`](#registering-a-dashboard) and sign in with *Sign in with Futurebound Corp.*). The bundled [username/password provider](#usernamepassword-provider-no-oauth-idp) is the quickest option when the backend is on a trusted LAN or reachable only over a VPN, but is **not suitable for direct public-internet exposure**. Binding the dashboard to a non-loopback address engages its auth gate; once signed in, Desktop reuses the session for the chat WebSocket automatically — there is no token to copy or paste.

The recipe below uses the username/password path because it's the quickest to stand up on a trusted network; for the OAuth path see [Default provider: Futurebound Corp.](#default-provider-nous-research).

### On the backend (the remote machine)

```bash
# 1. Set the dashboard login credentials in ~/.athena/.env (secrets file, 0600).
cat >> ~/.athena/.env <<'EOF'
ATHENA_DASHBOARD_BASIC_AUTH_USERNAME=admin
ATHENA_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
...
athena dashboard --no-open --host 0.0.0.0 --port 9119
```

Prefer no plaintext at rest? Use `ATHENA_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` with a scrypt hash instead — see [Username/password provider](#usernamepassword-provider-no-oauth-idp) for the full surface.

If you run the dashboard as a systemd service, `~/.athena/.env` is picked up automatically when the unit has `EnvironmentFile=%h/.athena/.env`, so the credentials are in the environment at boot.
...
- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/athena` are supported if you front it with a reverse proxy)
...
The session refreshes automatically and survives restarts when `ATHENA_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `ATHENA_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `ATHENA_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |

### Troubleshooting

- **"Remote gateway incomplete"** — you haven't entered a remote URL.
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `ATHENA_DASHBOARD_BASIC_AUTH_USERNAME` / `ATHENA_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for unknown user and wrong password, so check both. Confirm the gate with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
...
- **Signed out on every restart** — set `ATHENA_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `athena_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `athena update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `athena dashboard` will build it on first launch.
