...
```bash
cd ~/.athena/athena-agent && uv pip install -e ".[web,pty]"
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `cd ~/.athena/athena-agent && uv pip install -e ".[all]"` includes both extras and is the easiest path if you also want messaging/voice/etc.
...
- **Disk warnings** — the volume holding `~/.athena` is nearly full
...
- `ptyprocess` — installed by the `pty` extra (`cd ~/.athena/athena-agent && uv pip install -e ".[web,pty]"`, or `[all]` covers both)
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and Athena versions, CPU core count + utilization, memory, disk usage of the Athena home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The Athena version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `athena update` in the background. On Docker/Nix installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
...
- **`disk`** — a live `shutil.disk_usage()` sample of the volume holding
  `~/.athena`. Fields: `pressure`, `free_mb`, `total_mb`, `used_percent`,
...
| `GET /api/athena/update/check` | Report update availability (commits behind, install method) without applying. For git installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 24h cache (the check goes through the GitHub API, never `git fetch`) |
...
Configure an auth provider before exposing the dashboard:
  ...
  • OAuth: run `athena dashboard register` (Nous Portal) or install a
...
first; without one, Athena fails closed at startup. This prevents the local SPA
...
Athena always preserves loopback trust and rejects `*`, `0.0.0.0/0`, and
...
athena dashboard --host 127.0.0.1 --port 9119 --no-open
```
...
| `athena_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip. SameSite=None + Secure over HTTPS (must survive the cross-site IDP redirect chain — Chromium drops SameSite=Lax cookies set on a 302 in a cross-site chain); SameSite=Lax on loopback HTTP |
...
You protect the remote dashboard with one of the bundled auth providers, and the desktop app signs in against whichever one the backend advertises. For a backend reachable beyond your own machine — a VPS, a public host, anything internet-facing — the recommended provider is **OAuth (Nous Portal)** (register it with [`athena dashboard register`](#registering-a-dashboard) and sign in with *Sign in with Nous Research*). The bundled [username/password provider](#usernamepassword-provider-no-oauth-idp) is the quickest option when the backend is on a trusted LAN or reachable only over a VPN, but is **not suitable for direct public-internet exposure**. Binding the dashboard to a non-loopback address engages its auth gate; once signed in, Desktop reuses the session for the chat WebSocket automatically — there is no token to copy or paste.
