...
sidebar_position: 8
title: "Context Files"
description: "Project context files — .athena.md, AGENTS.md, CLAUDE.md, global SOUL.md, and .cursorrules — automatically injected into every conversation"
---

# Context Files

Athena Agent automatically discovers and loads context files that shape how it behaves. Some are project-local and discovered from your working directory. `SOUL.md` is now global to the Athena instance and is loaded from `ATHENA_HOME` only.

## Supported Context Files

| File | Purpose | Discovery |
|------|---------|-----------| 
| **.athena.md** / **ATHENA.md** | Project instructions (highest priority) | Walks to git root |
...
| **SOUL.md** | Global personality and tone customization for this Athena instance | `ATHENA_HOME/SOUL.md` only |
| **.cursorrules** | Cursor IDE coding conventions | CWD only |
| **.cursor/rules/*.mdc** | Cursor IDE rule modules | CWD only |

:::info Priority system
Only **one** project context type is loaded per session (first match wins): `.athena.md` → `AGENTS.override.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. **SOUL.md** is always loaded independently as the agent identity (slot #1).
...
When your working directory sits inside a git repository, Athena loads a **merged chain** of `AGENTS.md` files at session start: the git-root `AGENTS.md` first, then the `AGENTS.md` in every intermediate directory down to your working directory. Deeper files appear later in the prompt, so more specific guidance takes precedence. Each file gets its own provenance header (e.g. `## ../../AGENTS.md`), and identical copies along the chain are deduplicated.
...
- `~/.athena/SOUL.md`
- or `$ATHENA_HOME/SOUL.md` if you run Athena with a custom home directory

Important details:

- Athena seeds a default `SOUL.md` automatically if one does not exist yet
- Athena loads `SOUL.md` only from `ATHENA_HOME`
...
Athena is compatible with Cursor IDE's `.cursorrules` file and `.cursor/rules/*.mdc` rule modules. If these files exist in your project root and no higher-priority context file (`.athena.md`, `AGENTS.md`, or `CLAUDE.md`) is found, they're loaded as the project context.
...
1. **Scan working directory** — checks for `.athena.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules` (first match wins)
