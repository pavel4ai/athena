...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Athena at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<ATHENA_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`athena lsp status` reports `installed` once `pwsh` is found; if the
...
run `athena lsp install typescript` or auto-install fires on first
use.

## CLI

```
athena lsp status          # service state + per-server install status
athena lsp list            # registry, optionally --installed-only
athena lsp install <id>    # eagerly install one server
athena lsp install-all     # try every server with a known recipe
athena lsp restart         # tear down running clients
athena lsp which <id>      # print resolved binary path
```

`athena lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <ATHENA_HOME>/lsp/bin
...
`<ATHENA_HOME>/lsp/bin/`. NPM packages land in
`<ATHENA_HOME>/lsp/node_modules/` with bin symlinks one level up.
...
**single process** per Athena process: the first Python project spawns
...
**`athena lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<ATHENA_HOME>/lsp/bin/`. Run
`athena lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `athena lsp status`**
...
When `athena lsp status` shows a `Backend warnings` section, install
...
`~/.athena/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.athena/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `athena lsp restart` to clear the set;
