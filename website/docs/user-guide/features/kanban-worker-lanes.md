...
If you're writing the worker code itself — the agent that runs *inside* a lane — the kanban lifecycle and reference details are injected into the worker's system prompt automatically (the `KANBAN_GUIDANCE` block in [`agent/prompt_builder.py`](https://github.com/pavel4ai/athena/blob/main/agent/prompt_builder.py)).
...
Athena Kanban owns lifecycle truth — `ready` → `running` → `review` / `blocked` / `done` / `archived`. Worker lanes execute work but never own that truth; everything they do flows back through the kanban kernel via the `kanban_*` tools (or, for non-Athena external workers, via the API). Reviewers gate the transition from "code change written" to "task done."
...
For Athena profile lanes, the dispatcher's `_default_spawn` runs `athena -p <assignee> chat -q <prompt>` (or the equivalent module form when the `athena` shim isn't on `$PATH`) inside the task's pinned workspace, with these env vars set:

| Variable | Carries |
|---|---|
| `ATHENA_KANBAN_TASK` | the task id the worker is operating on |
| `ATHENA_KANBAN_DB` | absolute path to the per-board SQLite file |
| `ATHENA_KANBAN_BOARD` | board slug |
| `ATHENA_KANBAN_WORKSPACES_ROOT` | root of the board's workspace tree |
| `ATHENA_KANBAN_WORKSPACE` | absolute path to *this* task's workspace |
| `ATHENA_KANBAN_RUN_ID` | the current run's id (for the lifecycle gate) |
| `ATHENA_KANBAN_CLAIM_LOCK` | the claim lock string (`<host>:<pid>:<uuid>`) |
| `ATHENA_PROFILE` | the worker's own profile name (for `kanban_comment` author attribution) |
| `ATHENA_TENANT` | tenant namespace, if the task has one |
...
Athena subprocess helpers carry a non-owner fence into shells, execution kernels,
...
Athena-tools MCP endpoint can likewise act for its supervising worker, while the
...
The dashboard renders run history with summaries, metadata blocks, and exit-status badges. CLI users can run `athena kanban tail <task_id>` to follow live, or `athena kanban runs <task_id>` for the historical attempt list.

## Existing lane shapes

### Athena profile lane (default)

The shape every kanban worker takes today: the assignee is a profile name, the dispatcher spawns `athena -p <profile>`, the worker gets the `KANBAN_GUIDANCE` system-prompt block injected automatically, and uses the `kanban_*` tools to terminate the run. No setup beyond defining the profile.
...
- [`KANBAN_GUIDANCE`](https://github.com/pavel4ai/athena/blob/main/agent/prompt_builder.py) — the worker + orchestrator lifecycle injected into every kanban worker's system prompt.
