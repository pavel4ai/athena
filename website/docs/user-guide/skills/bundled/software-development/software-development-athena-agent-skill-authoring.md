---
title: "Athena Agent Skill Authoring — Author in-repo SKILL.md files: frontmatter and structure"
...
1. **User-local:** `~/.athena/skills/<maybe-category>/<name>/SKILL.md` — personal, not shared. Created via `skill_manage(action='create')`.
2. **In-repo (this skill is about this case):** `skills/<category>/<name>/SKILL.md` or `optional-skills/<category>/<name>/SKILL.md` inside the athena-agent repo — committed, shipped with the package. Use `write_file` + `git add`. `skill_manage(action='create')` does NOT target this tree.
...
- Don't use for: personal skills in `~/.athena/skills/` (just use `skill_manage`)
...
- **Optional (`optional-skills/<category>/`)** — niche, vertical-specific (blockchain, gaming, finance, one app), recurring-job/task skills, or anything heavy. Installed via `athena skills install official/<category>/<skill>`.
...
author: Real Name (github-handle), Athena Agent
license: MIT
...
- Credit the **human first**, then "Athena Agent" as secondary collaborator: `Ben Barclay (benbarclay), Athena Agent`.
- Never `author: Athena Agent` alone for contributed skills — credit the human, not the tool, even (especially) when an agent drafted the text.
- Maintainer-authored skills: `Teknium (teknium1), Athena Agent`.

### `related_skills` rules

- Every entry must resolve to an existing **in-repo** skill in the same tree state as your PR. Do not reference skills that were only planned, live in another PR, or exist only in `~/.athena/skills/`.
...
|---|---|
| Athena tools + stdlib Python + cross-platform CLIs | `[linux, macos, windows]` |
...
1. **Using `skill_manage(action='create')` for an in-repo skill.** It writes to `~/.athena/skills/`, not the repo tree. Use `write_file`.
...
3. **`author: Athena Agent` on a contributed skill.** Credit the human first.
...
- [ ] `name`, `description`, `version`, `author`, `license`, `platforms`, `metadata.athena.{tags, related_skills}` all present
...
- [ ] Body follows the modern section order; commands framed through Athena tools
