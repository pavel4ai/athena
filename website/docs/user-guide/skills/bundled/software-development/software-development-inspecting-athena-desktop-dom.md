---
title: "Inspecting Athena Desktop Dom — Read the live Athena desktop DOM/CSS over CDP"
sidebar_label: "Inspecting Athena Desktop Dom"
description: "Read the live Athena desktop DOM/CSS over CDP"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Inspecting Athena Desktop Dom

Read the live Athena desktop DOM/CSS over CDP.
...
| Path | `skills/software-development\inspecting-athena-desktop-dom` |
| Version | `1.0.0` |
| Author | Athena Agent |
| License | MIT |
...
# Inspecting the live Athena desktop DOM
...
`debugging-athena-desktop`), or anything where the real question is "does this
look right".
...
- **no `ATHENA_DESKTOP_DEV_SERVER`** — an unpackaged `electron .` against
  `dist/` is how the packaged app gets smoke tested, so it behaves like one.

`ATHENA_DESKTOP_CDP_PORT` moves the port (`=9333`) or disables it (`=off`).

Check before doing anything else:

```bash
curl -s --max-time 3 http://127.0.0.1:${ATHENA_DESKTOP_CDP_PORT:-9222}/json/version
```
...
```bash
cd apps/desktop
ATHENA_HOME=/tmp/cdp-probe-home \
ATHENA_DESKTOP_DEV_SERVER=http://127.0.0.1:5174 \
ATHENA_DESKTOP_CDP_PORT=9333 \
  npx electron . --user-data-dir=/tmp/cdp-probe-userdata
```

The separate `--user-data-dir` dodges Electron's single-instance lock, so it
cannot collide with a running `hgui`; the separate `ATHENA_HOME` keeps it away
from real sessions. Pick a port other than 9222 for the same reason. Run it in
the background and kill it when done.

`npm run perf:serve` does the same with a temp `ATHENA_HOME` baked in, if you
...
- **A throwaway `ATHENA_HOME` has no backend.** The app logs `ECONNREFUSED` for
  `athena:api` and may exit on its own. The renderer still mounts and the DOM is
