...
| Related skills | [`systematic-debugging`](/docs/user-guide/skills/bundled/software-development/software-development-systematic-debugging), [`node-inspect-debugger`](/docs/user-guide/skills/bundled/software-development/software-development-node-inspect-debugger), `debugging-athena-tui-commands` |
...
- A long-running process (athena gateway, tui_gateway) misbehaves and you can't restart it
...
The athena test runner and pytest both support this:
...
Easiest: add `breakpoint()` near the suspect line, then run `athena` normally. Control returns to your terminal at the pause point.

### `tui_gateway` subprocess (spawned by `athena --tui`)
...
debugpy.wait_for_client()
```
Start `athena --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
