...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _athena_env="${ATHENA_HOME:-$HOME/.athena}/.env"; [ -f "$_athena_env" ] && grep -q "^GITHUB_TOKEN=" "$_athena_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_athena_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
    elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
      GITHUB_TOKEN=$(uv run python3 "${ATHENA_HOME:-$HOME/.athena}/skills/github/github-auth/scripts/git-credential-token.py")
    fi
  fi
fi
...
```bash
source "${ATHENA_HOME:-$HOME/.athena}/skills/github/github-auth/scripts/gh-env.sh"
