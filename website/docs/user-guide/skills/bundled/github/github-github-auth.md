...
uv run python3 "${ATHENA_HOME:-$HOME/.athena}/skills/github/github-auth/scripts/git-credential-token.py"
```
...
elif _athena_env="${ATHENA_HOME:-$HOME/.athena}/.env"; [ -f "$_athena_env" ] && grep -q "^GITHUB_TOKEN=" "$_athena_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_athena_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
  export GITHUB_TOKEN=$(uv run python3 "${ATHENA_HOME:-$HOME/.athena}/skills/github/github-auth/scripts/git-credential-token.py")
  echo "AUTH_METHOD=curl"
else
