...
| List/filter by metadata (age, source, cost, tokens, workspace) | `athena sessions list` / `stats` via terminal |
| Rename | `athena sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `athena sessions archive <filters>` |
| Delete (destructive) | `athena sessions delete` / `athena sessions prune <filters>` |
| Export before deleting anything valuable | `athena sessions export --session-id <id> --format md` |
...
`athena sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `athena sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `athena sessions list`
