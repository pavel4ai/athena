---
title: "Honcho — Configure and troubleshoot Honcho memory for Athena"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Athena"
---
...
Configure and troubleshoot Honcho memory for Athena.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `athena skills install official/autonomous-ai-agents/honcho` |
...
See: https://docs.honcho.dev/v3/guides/integrations/athena#running-honcho-locally-with-athena

### Verify

```bash
athena honcho status    # shows resolved config, connection test, peer info
```
...
Manual override: `athena honcho map my-project-name`
...
# creates host block athena.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `athena.coder` host block in `honcho.json`
...
```bash
athena honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Config file: `$ATHENA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `athena honcho setup`. Ensure `memory.provider: honcho` is in `~/.athena/config.yaml`.

### Memory not persisting across sessions
Check `athena honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `athena profile create <name> --clone`. For existing profiles: `athena honcho sync`.
...
| Command | Description |
|---------|-------------|
| `athena honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `athena honcho status` | Show resolved config, connection test, peer info for active profile |
| `athena honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `athena honcho disable` | Disable Honcho for the active profile |
| `athena honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `athena honcho peers` | Show peer identities across all profiles |
| `athena honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `athena honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `athena honcho sessions` | List known directory-to-session-name mappings |
| `athena honcho map <name>` | Map current working directory to a Honcho session name |
| `athena honcho identity` | Seed AI peer identity or show both peer representations |
| `athena honcho sync` | Create host blocks for all Athena profiles that don't have one yet |
| `athena honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Athena + Honcho |
| `athena memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `athena memory status` | Show active memory provider and config |
| `athena memory off` | Disable external memory provider |
