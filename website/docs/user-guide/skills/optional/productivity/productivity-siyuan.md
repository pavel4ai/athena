...
| | |
|---|---|
| Source | Optional — install with `athena skills install official/productivity/siyuan` |
...
3. Store it in `${ATHENA_HOME:-~/.athena}/.env`:
   ```
...
```yaml
# In ~/.athena/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
