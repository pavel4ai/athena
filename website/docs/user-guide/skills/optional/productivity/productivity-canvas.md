...
| | |
|---|---|
| Source | Optional — install with `athena skills install official/productivity/canvas` |
...
5. Copy the token and add to `${ATHENA_HOME:-~/.athena}/.env`:
...
```bash
CANVAS="python $ATHENA_HOME/skills/productivity/canvas/scripts/canvas_api.py"
