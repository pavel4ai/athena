...
| | |
|---|---|
| Source | Optional — install with `athena skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${ATHENA_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ATHENA_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${ATHENA_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client athena
```
...
```bash
PUBLISH="${ATHENA_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client athena
```
...
```bash
DRIVE="${ATHENA_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `athena`)    |
