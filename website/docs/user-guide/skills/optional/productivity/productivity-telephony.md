...
| | |
|---|---|
| Source | Optional — install with `athena skills install official/productivity/telephony` |
...
- save provider credentials into `${ATHENA_HOME:-~/.athena}/.env`
...
```bash
SCRIPT="$(find ~/.athena/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
athena skills search telephony
athena skills install official/productivity/telephony
```
...
3. Buy it and save it into `${ATHENA_HOME:-~/.athena}/.env` + state:
```bash
python "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Athena calling with your status update." --voice Polly.Joanna
```
...
3. persist that number to `${ATHENA_HOME:-~/.athena}/.env`
