---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into Athena"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into Athena"
---
...
Import an OpenClaw setup (memories, skills) into Athena.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `athena skills install official/migration/openclaw-migration` |
...
```bash
athena claw migrate              # Full interactive migration
athena claw migrate --dry-run    # Preview what would be migrated
athena claw migrate --preset user-data   # Migrate without secrets
athena claw migrate --overwrite  # Overwrite existing conflicts
athena claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `athena setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_athena.py` to:
...
- copy OpenClaw skills into `~/.athena/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Athena workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.athena/tts/`
...
- `scripts/openclaw_to_athena.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.athena/skills/migration/openclaw-migration/scripts/openclaw_to_athena.py`

Do not guess a shorter path like `~/.athena/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.athena/skills/migration/openclaw-migration/`.
...
```bash
python ~/.athena/skills/migration/openclaw-migration/scripts/openclaw_to_athena.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python /home/USER/.athena/skills/migration/openclaw-migration/scripts/openclaw_to_athena.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python ~/.athena/skills/migration/openclaw-migration/scripts/openclaw_to_athena.py --preset user-data
```

Execute a user-data migration:

```bash
python ~/.athena/skills/migration/openclaw-migration/scripts/openclaw_to_athena.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python ~/.athena/skills/migration/openclaw-migration/scripts/openclaw_to_athena.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python ~/.athena/skills/migration/openclaw-migration/scripts/openclaw_to_athena.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
- OpenClaw skills available under `~/.athena/skills/openclaw-imports/`
