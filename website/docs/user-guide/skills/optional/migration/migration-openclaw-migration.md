...
| Version | `1.0.0` |
| Author | Athena Agent (Futurebound Corp.) |
| License | MIT |
...
```bash
python3 ~/.athena/skills/migration/openclaw-migration/scripts/openclaw_to_athena.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.athena/skills/migration/openclaw-migration/scripts/openclaw_to_athena.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.athena/skills/migration/openclaw-migration/scripts/openclaw_to_athena.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.athena/skills/migration/openclaw-migration/scripts/openclaw_to_athena.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.athena/skills/migration/openclaw-migration/scripts/openclaw_to_athena.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.athena/skills/migration/openclaw-migration/scripts/openclaw_to_athena.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
