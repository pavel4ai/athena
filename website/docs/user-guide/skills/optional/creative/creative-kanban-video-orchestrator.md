...
| | |
|---|---|
| Source | Optional — install with `athena skills install official/creative/kanban-video-orchestrator` |
...
3. Creates each Athena profile via `athena profile create --clone`
...
7. Fires the initial `athena kanban create` task assigned to the director

Use `scripts/bootstrap_pipeline.py` to generate setup.sh from a brief +
team-design JSON. See **[references/kanban-setup.md](https://github.com/pavel4ai/athena/blob/main/optional-skills/creative\kanban-video-orchestrator/references/kanban-setup.md)**
...
```bash
athena kanban watch --tenant <project-tenant>     # live events
athena kanban list  --tenant <project-tenant>     # board snapshot
athena dashboard                                   # visual board UI
```
...
playbook, see **[references/monitoring.md](https://github.com/pavel4ai/athena/blob/main/optional-skills/creative\kanban-video-orchestrator/references/monitoring.md)**.
...
task graphs. See **[references/examples.md](https://github.com/pavel4ai/athena/blob/main/optional-skills/creative\kanban-video-orchestrator/references/examples.md)**.
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `athena kanban create` call to a specialist profile. The kanban
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `${ATHENA_HOME:-~/.athena}/.env` or the user's secret store.
