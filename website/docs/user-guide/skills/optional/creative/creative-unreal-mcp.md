...
| | |
|---|---|
| Source | Optional — install with `athena skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Athena MCP catalog. The
...
Two halves, in this order: the editor side must be up before Athena connects.
...
### One-time, Athena side

    athena mcp install unreal-engine
...
`~/.athena/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Athena — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Athena connects
...
2. Start the Athena session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Athena
they appear as:
...
then happens at `athena mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Athena runs batched calls
...
- **Start order matters.** Editor + server up first, then the Athena
...
- **Timeouts on long operations.** Athena' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.athena/config.yaml` for
