---
title: "Athena S6 Container Supervision — Modify or debug s6 services in the Athena Docker image"
sidebar_label: "Athena S6 Container Supervision"
description: "Modify or debug s6 services in the Athena Docker image"
---
...
Modify or debug s6 services in the Athena Docker image.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `athena skills install official/devops/athena-s6-container-supervision` |
| Path | `optional-skills/devops\athena-s6-container-supervision` |
| Version | `1.0.0` |
| Author | Athena Agent |
| License | MIT |
| Platforms | linux |
...
- Understanding why the container's CMD is `/opt/athena/docker/main-wrapper.sh` and how leading-dash args reach the user's program
...
│   ├── 01-athena-setup                ← docker/stage2-hook.sh
│   │   ├── UID/GID remap
│   │   ├── chown /opt/data
│   │   ├── chown /opt/data/profiles (every boot)
│   │   ├── seed .env / config.yaml / SOUL.md
│   │   └── skills_sync.py
│   └── 02-reconcile-profiles          ← athena_cli.container_boot
│       ├── chown /run/service (athena-writable for runtime register)
│       └── walk $ATHENA_HOME/profiles/<name>/gateway_state.json
│           → recreate /run/service/gateway-<name>/
│           → auto-start only those with prior_state == "running"
│
├── s6-rc.d (static services, in /etc/s6-overlay/s6-rc.d/)
│   ├── main-athena/run                ← exec sleep infinity (no-op slot)
│   └── dashboard/run                  ← if ATHENA_DASHBOARD=1, runs `athena dashboard`
│
├── /run/service (s6-svscan watches; tmpfs)
│   ├── gateway-coder/                 ← runtime-registered per-profile
│   │   ├── type        ("longrun")
│   │   ├── run         ("#!/command/with-contenv sh ... exec s6-setuidgid athena athena -p coder gateway run")
│   │   ├── down        (marker — present means "registered but don't auto-start")
│   │   └── log/run     (s6-log → $ATHENA_HOME/logs/gateways/coder/current)
│   └── ...
│
└── CMD ("main program")               ← /opt/athena/docker/main-wrapper.sh
    └── routes user args: bare exec | athena subcommand | athena (no args)
...
| Path | Role |
|---|---|
| `Dockerfile` | s6-overlay install + cont-init.d wiring + `ENTRYPOINT ["/opt/athena/docker/entrypoint-dispatch.sh"]` |
...
| `docker/stage2-hook.sh` | The "old entrypoint logic" — UID remap, chown, seed, skills sync. Runs as cont-init.d/01-athena-setup. |
| `docker/cont-init.d/02-reconcile-profiles` | Calls `athena_cli.container_boot` on every boot to restore profile gateway slots from the persistent volume. |
| `docker/main-wrapper.sh` | The container's CMD. Routes user args, drops to athena via `s6-setuidgid`, exec's the chosen program. |
| `docker/s6-rc.d/main-athena/run` | No-op `sleep infinity` — slot exists so the s6-rc user bundle is valid; main athena runs as the CMD, not as a supervised service. |
| `docker/s6-rc.d/dashboard/run` | Conditional service — `exec sleep infinity` unless `ATHENA_DASHBOARD` is truthy. |
...
| `athena_cli/service_manager.py` | `S6ServiceManager`: `register_profile_gateway`, `unregister_profile_gateway`, `start/stop/restart/is_running`, `list_profile_gateways`. |
| `athena_cli/container_boot.py` | `reconcile_profile_gateways()` — walks persistent profiles, regenerates s6 slots, emits `container-boot.log`. |
| `athena_cli/gateway.py::_dispatch_via_service_manager_if_s6` | Intercepts `athena gateway start/stop/restart` and routes to s6 when running in a container. |

## Why Architecture B (CMD as main program, not s6-supervised)

The original plan (v1–v3) called for main athena to run as a supervised s6-rc service. Two real s6-overlay v3 mechanics blocked that:

1. **cont-init.d scripts receive no CMD args** — so the stage2 hook can't parse `docker run <image> chat -q "hi"` to set `ATHENA_ARGS` for a service `run` script to consume.
...
So we use the s6-overlay-native CMD pattern via the dispatcher: `ENTRYPOINT ["/opt/athena/docker/entrypoint-dispatch.sh"]`, which under PID 1 exec's `/init /opt/athena/docker/main-wrapper.sh "$@"`. The wrapper is prepended to user args automatically — so `docker run <image> --version` becomes `/init main-wrapper.sh --version`, and `--version` doesn't get intercepted by /init's POSIX shell. The wrapper drops to athena via `s6-setuidgid`, then exec's the chosen program. The program's exit code becomes the container exit code, exactly matching the pre-s6 tini contract. When the entrypoint is NOT PID 1 (Fly Machines, `docker run --init`), the dispatcher skips `/init` entirely (it would abort with `can only run as pid 1`), restores the s6 helper PATH, runs stage2-hook.sh, and exec's main-wrapper.sh directly — no supervised services on that path (#38349).

Trade-off: main athena is unsupervised under s6. That exactly matches its behavior under tini (the pre-s6 image). Dashboard supervision is the only **new** guarantee — and per-profile gateways under `/run/service/` get full supervision.
...
2. Drop to athena via `s6-setuidgid athena` at the top of run (unless you specifically need root).
...
Edit `S6ServiceManager._render_run_script` in `athena_cli/service_manager.py`. The function is also called by `athena_cli/container_boot.py::_register_service` during boot reconciliation, so it's the single source of truth. Update the corresponding assertion in `tests/athena_cli/test_service_manager.py::test_s6_register_creates_service_dir_and_triggers_scan`.

### Run the docker test harness

```sh
docker build -t athena-agent-harness:latest .
ATHENA_TEST_IMAGE=athena-agent-harness:latest scripts/run_tests.sh tests/docker/ -v
...
`/command/` (where s6-overlay puts its binaries) is on PATH only for processes spawned by the supervision tree — services, cont-init.d, main-wrapper.sh. `docker exec <c> s6-svstat …` will fail with "command not found"; always use the absolute path `/command/s6-svstat`. The `athena` binary works because the Dockerfile adds `/opt/athena/.venv/bin` to the runtime `ENV PATH`.

### Profile directory ownership

The cont-init reconciler runs as athena (`s6-setuidgid athena` in `02-reconcile-profiles`). If a profile dir ends up root-owned (e.g. because `docker exec <c> athena profile create …` ran as root by default), the reconciler can't read SOUL.md and fails with `PermissionError`. Mitigation: `stage2-hook.sh` chowns `$ATHENA_HOME/profiles` to athena on **every** boot, idempotently. Don't remove that block.

### Files written by `docker exec` are root-owned

`docker exec` defaults to root. Either pass `--user athena` or rely on the stage2 chown sweep next reboot. Don't write files under `$ATHENA_HOME/profiles/<name>/` as root manually — the next reconcile pass will sweep them but in-flight operations may hit perm errors.
...
Most likely the profile has no model or auth configured. The service slot is correct — the gateway itself is unconfigured. Run `athena -p <profile> setup` first. The s6 supervisor will keep restarting it; that's the desired behavior (when you fix the config, the next attempt succeeds and stays up).

### Reconciler skipped a profile

The reconciler keys on the **presence of `SOUL.md`** as the "real profile" marker. `athena profile create` always seeds it. If a profile dir is missing SOUL.md (stray directory, partial restore, backup-in-progress), the reconciler skips it intentionally. Add a `SOUL.md` (even empty) to opt back in.
...
- `athena-tool-quirks`: Specific Athena-tool workarounds (sed/grep/etc.) — load when debugging the s6 stack's interaction with athena built-in tools.
