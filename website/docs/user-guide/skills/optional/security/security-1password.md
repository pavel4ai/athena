...
| | |
|---|---|
| Source | Optional — install with `athena skills install official/security/1password` |
...
Set `OP_SERVICE_ACCOUNT_TOKEN` in `${ATHENA_HOME:-~/.athena}/.env` (the skill will prompt for this on first load).
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/athena-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/athena-op.sock"
