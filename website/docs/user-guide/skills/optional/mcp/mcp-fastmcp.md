...
| | |
|---|---|
| Source | Optional — install with `athena skills install official/mcp/fastmcp` |
...
| Related skills | [`athena-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-athena-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
```bash
python ~/.athena/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.athena/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
- configure the server in `~/.athena/config.yaml` using the `native-mcp` skill, or
...
The server-building part may be correct while the Athena config is not. Load the `native-mcp` skill and configure the server in `~/.athena/config.yaml`, then restart Athena.
