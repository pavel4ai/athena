...
After initial setup, verify these variables in `~/.athena/.env`:
...
4. Review gateway logs: `tail -f ~/.athena/logs/gateway.log`
...
4. Consider enabling verbose logging: `ATHENA_LOG_LEVEL=debug`
...
```bash
ATHENA_LOG_LEVEL=debug athena gateway
```
...
```bash
athena chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
