...
> Run `athena gateway setup` and pick **WhatsApp** for a guided walk-through.
...
3. Run `athena whatsapp` and scan the QR code from that WhatsApp account
...
The Baileys bridge saves its session under `~/.athena/platforms/whatsapp/session`. This means:
...
| **QR code expires** | QR codes refresh every ~20 seconds. If it times out, restart `athena whatsapp`. |
| **Session not persisting** | Check that `~/.athena/platforms/whatsapp/session` exists and is writable. If containerized, mount it as a persistent volume. |
| **Logged out unexpectedly** | WhatsApp unlinks devices after long inactivity. Keep the phone on and connected to the network, then re-pair with `athena whatsapp` if needed. |
...
| **macOS: "Node.js not installed" but node works in terminal** | launchd services don't inherit your shell PATH. Run `athena gateway install` to re-snapshot your current PATH into the plist, then `athena gateway start`. See the [Gateway Service docs](./index.md#macos-launchd) for details. |
...
| **Bot replies to strangers with a pairing code** | Set `whatsapp.unauthorized_dm_behavior: ignore` in `~/.athena/config.yaml` if you want unauthorized DMs to be silently ignored instead. |
...
- The `~/.athena/platforms/whatsapp/session` directory contains full session credentials — protect it like a password
- Set file permissions: `chmod 700 ~/.athena/platforms/whatsapp/session`
