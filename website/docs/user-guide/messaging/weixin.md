...
5. Save the account credentials automatically to `~/.athena/weixin/accounts/`
...
After initial QR login, set at minimum the account ID in `~/.athena/.env`:
...
- **QR code login** — scan-to-connect setup via `athena gateway setup`
...
1. Pair Athena once with `athena gateway setup` and note the connected iLink bot
   account.
...
- Tokens are saved per account+peer to `~/.athena/weixin/accounts/<account_id>.context-tokens.json`
...
| `Weixin startup failed: WEIXIN_TOKEN is required` | Run `athena gateway setup` to complete QR login, or set `WEIXIN_TOKEN` manually |
| `Weixin startup failed: WEIXIN_ACCOUNT_ID is required` | Set `WEIXIN_ACCOUNT_ID` in your `.env` or run `athena gateway setup` |
...
| Session expired (`errcode=-14`) | Your login session has expired. Re-run `athena gateway setup` to scan a new QR code |
...
| Terminal QR code doesn't render | Reinstall with the messaging extra: `cd ~/.athena/athena-agent && uv pip install -e ".[messaging]"`. Alternatively, open the URL printed above the QR |
