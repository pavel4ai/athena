...
description: "Chat with Athena from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---
...
For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Athena](/guides/use-voice-mode-with-athena).
...
saved by `athena gateway setup` can enable a credential-based platform without a
...
For group chats, hooks, and automation flows, Athena supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Athena keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Athena does not hide failures just because the text resembles a silence token.
...
```bash
athena gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
athena gateway              # Run in foreground
athena gateway setup        # Configure messaging platforms interactively
athena gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo athena gateway install --system   # Linux only: install a boot-time system service
athena gateway start        # Start the default service
athena gateway stop         # Stop the default service
athena gateway status       # Check default service status
athena gateway status --system         # Linux only: inspect the system service explicitly
```
...
```bash
athena gateway install --force
```

A positive value makes the generated unit use `Type=notify`,
`NotifyAccess=main`, and the matching `WatchdogSec`. Athena sends heartbeats
...
Different channels can run different models and personas from a **single gateway** — e.g. a cheap fast model in `#daily` and a frontier model with a specialist prompt in `#dev`. Configure `channel_overrides` under the platform in `~/.athena/config.yaml`:
...
athena pairing approve telegram XKGH5N7P

# Other pairing commands:
athena pairing list          # View pending + approved users
athena pairing revoke telegram 123456789  # Remove access
```
...
Control how much tool activity is displayed in `~/.athena/config.yaml`:
...
Setting `display.tool_progress: log` sends **no** progress bubbles to chat. Instead, each tool call is appended as a line to `~/.athena/logs/tool_calls.log` — a rotating audit file (5 MB × 3 backups) run through the same secret-redacting formatter as regular logs, so credentials never land on disk. Use it when you want a full tool-call trail without any chat noise.

### Configurable status phrases

Long-running gateway status lines ("still working…"-style heartbeats) draw from a phrase catalog. Built-in defaults ship in `gateway/assets/status_phrases.yaml`; you can add your own with profile-portable files under `ATHENA_HOME`:

- `~/.athena/status_phrases.yaml` or any `*.yaml` in `~/.athena/status_phrases/` (conventional paths, auto-loaded), or
- point config at a relative path:

```yaml
display:
  status_phrases:
    path: status_phrases/whatsapp.yaml  # relative to ATHENA_HOME
...
Off by default. When enabled, Athena prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.athena/config.yaml`:
...
```bash
ATHENA_BACKGROUND_NOTIFICATIONS=result
```
...
athena gateway start                 # Start the service
athena gateway stop                  # Stop the service
athena gateway status                # Check status
journalctl --user -u athena-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo athena gateway install --system
sudo athena gateway start --system
sudo athena gateway status --system
...
The unit Athena installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit athena-gateway` (or `sudo systemctl edit athena-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `athena update`. When `athena update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart athena-gateway` command (it never blocks on an interactive password prompt).
...
```bash
athena gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `athena update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo athena update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/athena-gateway`:

```
athena ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed athena-gateway*, /usr/bin/systemctl --no-ask-password start athena-gateway*, /usr/bin/systemctl --no-ask-password restart athena-gateway*
```
:::
...
If you run multiple Athena installations on the same machine (with different `ATHENA_HOME` directories), each gets its own systemd service name. The default `~/.athena` uses `athena-gateway`; other installations use `athena-gateway-<hash>`. The `athena gateway` commands automatically target the correct service for your current `ATHENA_HOME`.
:::

### macOS (launchd)

```bash
athena gateway install               # Install as launchd agent
athena gateway start                 # Start the service
athena gateway stop                  # Stop the service
athena gateway status                # Check status
tail -f ~/.athena/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.athena.gateway.plist`. It includes three environment variables:
...
- **ATHENA_HOME** — scopes the gateway to your Athena installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `athena gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `ATHENA_HOME` directory gets its own launchd label. The default `~/.athena` uses `ai.athena.gateway`; other installations use `ai.athena.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `athena-cli` | Full access |
| Telegram | `athena-telegram` | Full tools including terminal |
| Discord | `athena-discord` | Full tools including terminal |
| WhatsApp | `athena-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `athena-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `athena-slack` | Full tools including terminal |
| Google Chat | `athena-google_chat` | Full tools including terminal |
| Signal | `athena-signal` | Full tools including terminal |
| SMS | `athena-sms` | Full tools including terminal |
| Email | `athena-email` | Full tools including terminal |
| Home Assistant | `athena-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `athena-mattermost` | Full tools including terminal |
| Matrix | `athena-matrix` | Full tools including terminal |
| DingTalk | `athena-dingtalk` | Full tools including terminal |
| Feishu/Lark | `athena-feishu` | Full tools including terminal |
| WeCom | `athena-wecom` | Full tools including terminal |
| WeCom Callback | `athena-wecom-callback` | Full tools including terminal |
| Weixin | `athena-weixin` | Full tools including terminal |
| BlueBubbles | `athena-bluebubbles` | Full tools including terminal |
| QQBot | `athena-qqbot` | Full tools including terminal |
| Yuanbao | `athena-yuanbao` | Full tools including terminal |
| Microsoft Teams | `athena-teams` | Full tools including terminal |
| API Server | `athena-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `athena-webhook` | Full tools including terminal |
| Raft | `athena-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
`platforms.<name>.enabled: false` in `~/.athena/config.yaml` is authoritative.
...
1. **Gateway log** (`~/.athena/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
