...
[A2A](https://a2a-protocol.org) is the open Agent2Agent protocol (v1.0, stewarded by the Linux Foundation) for communication between independent AI agents. The Athena A2A plugin works in **both directions**: your agent can call other A2A agents as tools, and other agents can send tasks to your Athena over HTTP.

It interoperates with any A2A-compliant peer — another Athena, LangChain, CrewAI, Google ADK agents, or anything built on the official `a2a-sdk`.

## When to use A2A

- **Athena ↔ Athena across machines** — let your desktop agent hand tasks to a Athena on a server, or vice versa, each with its own memory, tools, and credentials.
...
- **Being a callable service** — expose your Athena so other frameworks' agents can send it tasks.
...
```bash
athena tools enable a2a --platform cli        # CLI/TUI sessions
athena tools enable a2a --platform telegram   # or any messaging platform
athena tools enable a2a --platform a2a        # let inbound A2A tasks call peers (agent chaining)
```
...
With the platform enabled, Athena serves:
...
- **Audit log** — every exchange appends to `~/.athena/a2a_audit.jsonl`.
