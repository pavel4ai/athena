...
Configure which events the agent sees in `~/.athena/config.yaml` under the Home Assistant platform's `extra` section:
...
The adapter reads credentials from `~/.athena/.env` (auto-merged at startup) or
