...
1. Generate the manifest. New Slack apps must use Agent view:
   ```bash
   athena slack manifest --agent-view --write
   ```
   This writes `~/.athena/slack-manifest.json` and prints paste-in
   ...
   ```bash
   athena slack manifest --agent-view \
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `athena-socket` (the name doesn't matter)
...
```bash
athena gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
athena gateway              # Foreground
athena gateway install      # Install as a user service
sudo athena gateway install --system   # Linux only: boot-time system service
```
...
can spend the entire turn in hidden reasoning and never produce visible assistant text; Athena now
...
[`COMMAND_REGISTRY`](https://github.com/pavel4ai/athena/blob/main/athena_cli/commands.py)
...
New Slack apps use Slack's **Agent** messaging experience. Existing Athena
...
to `message.im`, `app_home_opened`, and `app_context_changed`, so Athena can
identify a Messages-tab DM and receive the user's active Slack context with a
turn. Athena only supplies that context as a label; it does not read the viewed
channel's history.

### Refreshing slash commands after updates

When Athena adds new commands (e.g. after `athena update`), regenerate
the manifest and update your Slack app:

```bash
athena slack manifest --write
```
...
3. Paste the new contents of `~/.athena/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/athena <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/athena bg run the tests` — Athena routes it the same way as `/bg
run the tests`. Free-form questions also work: `/athena what's the
...
The bang form also works behind a mention (`@Athena !stop`) and with
...
fails, Athena retries via a second ephemeral API path — a slash reply is never
posted publicly to the channel as a fallback. (Commands typed as regular
messages — `!cmd` in threads, `@Athena /cmd` — reply as normal visible
messages instead.)
...
```bash
athena slack manifest --slashes-only > /tmp/slashes.json
```
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.athena/config.yaml`.
...
platforms:
  slack:
    ...
    extra:
      ...
      # If Slack rejects the native stream, Athena keeps one editable text
      ...
      # that message itself @mentions Athena, and "all" accepts every
      # other bot. Athena always ignores its own bot user to prevent
      # self-echoes.
      allow_bots: "none"
...
| `platforms.slack.extra.allow_bots` | `"none"` | Controls messages from other Slack bots: `"none"` ignores them, `"mentions"` accepts a bot message only when **that message itself** @mentions Athena, and `"all"` accepts all of them. Use `"mentions"` for the safest bot-to-bot collaboration mode. See [Accepting messages from other bots](#accepting-messages-from-other-bots-allow_bots). |
...
name in the thread. By default Athena sets it to `is thinking...`; customize it
...
(`platforms.google_chat.typing_status_text`, default `"Athena is thinking…"`) —
...
`streaming.enabled` is on (transport `auto` or `draft`), Athena uses native
streaming automatically wherever it's available:

- The stream starts on the first frame and appends only deltas (the API is
  append-only). The streamed message **is** the final message — Athena seals
...
- If your Slack app doesn't have the AI features enabled (or lacks the
  `assistant:write` scope), the first failure is cached and Athena falls back
...
- If the native stream can't start or update, Athena falls back to a single
...
By default Athena ignores every message authored by another Slack bot or app (including Workflow Builder posts). For multi-agent workspaces — several Athena instances or peer bots collaborating in one channel — opt in with `allow_bots`:
...
- Athena always ignores its own messages, in every mode, to prevent self-echo loops.

`mentions` is the recommended mode for bot-to-bot collaboration: each agent must explicitly summon the other per turn. Avoid `all` unless every peer bot's own reply policy is loop-safe — two bots that answer everything will answer each other forever. Detection covers labeled bot messages (`bot_id`, `subtype: bot_message`), app-originated events, and unlabeled bot *users* (probed via `users.info`), so peer Athena agents are filtered consistently across workspaces.
...
`client_msg_id` — the same signature Athena uses to recognise app posts — so it
is dropped as bot traffic. This blocks a common pattern: a custom front-end (an
internal dashboard, a mobile shell, a kiosk) that sends messages to Athena *as*
the logged-in user.
...
manifest — regenerate with `athena slack manifest`):
...
Delivery works even when the cron process isn't co-located with the gateway — Athena falls back to a standalone Web API sender using `SLACK_BOT_TOKEN`. `MEDIA:` attachments in the cron output are uploaded as native Slack file shares to the same target.
...
- Tokens should be stored in `~/.athena/.env` (file permissions `600`)
