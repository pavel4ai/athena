...
> Run `athena gateway setup` and pick **ntfy** for a guided walk-through.

## Prerequisites

- A topic name (any unique string — `athena-myname-2026` works fine)
...
```
NTFY_TOPIC=athena-myname-2026
NTFY_ALLOWED_USERS=athena-myname-2026
NTFY_HOME_CHANNEL=athena-myname-2026
```
...
- Or **pick a long, unguessable topic name** (`athena-7d4f9c8b-2026`) and treat it as the shared secret. This is the lightest setup but the topic name leaks via any logs or screenshots.
...
1. Pick a topic name: `athena-myname-2026`
2. On your phone: install the [ntfy app](https://ntfy.sh/docs/subscribe/phone/), tap **+**, enter `athena-myname-2026`
3. On the host:
   ```bash
   echo 'NTFY_TOPIC=athena-myname-2026' >> ~/.athena/.env
   echo 'NTFY_ALLOWED_USERS=athena-myname-2026' >> ~/.athena/.env
...
Or target a specific topic explicitly via the cron job's `deliver:` field, or from a shell script with the [`athena send` CLI](/guides/pipe-script-output):

```bash
athena send ntfy:alerts-channel "Done!"
```
