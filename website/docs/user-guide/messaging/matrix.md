...
2. Choose a username like `athena` — the full user ID will be `@athena:your-server.org`.

### Option B: Use matrix.org or Another Public Homeserver

1. Go to [Element Web](https://app.element.io) and create a new account.
2. Pick a username for your bot (e.g., `athena-bot`).
...
```bash
MATRIX_USER_ID=@athena:your-server.org
...
# MATRIX_USER_ID=@athena:matrix.example.org
...
Optional behavior settings in `~/.athena/config.yaml`:
...
- Stores encryption keys in `~/.athena/platforms/matrix/store/` (legacy installs: `~/.athena/matrix/store/`)
...
ATHENA_MATRIX_SYNAPSE_INTEGRATION=1 \
...
If you delete `~/.athena/platforms/matrix/store/crypto.db`, the bot loses its encryption identity. Simply restarting with the same device ID will **not** fully recover — the homeserver still holds one-time keys signed with the old identity key, and peers cannot establish new Olm sessions.
...
1. Stop Synapse and delete the old device from its database:
   ...
   sudo sqlite3 /var/lib/matrix-synapse/homeserver.db "
     DELETE FROM e2e_device_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@athena:your-server';
     DELETE FROM e2e_one_time_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@athena:your-server';
     DELETE FROM e2e_fallback_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@athena:your-server';
     DELETE FROM devices WHERE device_id = 'DEVICE_ID' AND user_id = '@athena:your-server';
   "
   ...
   curl -X DELETE -H "Authorization: Bearer ADMIN_TOKEN" \
     'https://your-server/_synapse/admin/v2/users/%40athena%3Ayour-server/devices/DEVICE_ID'
   ```
...
2. Delete the local crypto store and restart Athena:
   ```bash
   rm -f ~/.athena/platforms/matrix/store/crypto.db*
   # restart athena
   ```
...
**Cause**: The host's system clock is set ahead of real time. The Matrix adapter applies a 5-second startup-grace filter (`event_ts < startup_ts - 5`) to ignore events replayed from initial sync. When the wall clock is ahead, every incoming event looks "older than startup" and is dropped before reaching the message handler — the bot appears connected but never replies. See [#12614](https://github.com/pavel4ai/athena/issues/12614).
...
1. **Generate a new access token** to get a fresh device ID. The simplest way:
   ...
   curl -X POST https://your-server/_matrix/client/v3/login \
     ...
     -d '{
       "type": "m.login.password",
       "identifier": {"type": "m.id.user", "user": "@athena:your-server.org"},
       "password": "***",
       "initial_device_display_name": "Athena Agent"
     }'
   ```

   Copy the new `access_token` and update `MATRIX_ACCESS_TOKEN` in `~/.athena/.env`.

2. **Delete old encryption state**:

   ```bash
   rm -f ~/.athena/platforms/matrix/store/crypto.db
   rm -f ~/.athena/platforms/matrix/store/crypto_store.*
   ```

3. **Set your recovery key** (if you use cross-signing — most Element users do). Add to `~/.athena/.env`:
...
services:
  athena-matrix:
    ...
    environment:
      ...
      MATRIX_DEVICE_ID: "ATHENA_BOT"

      # Proxy mode — forward to host agent
      GATEWAY_PROXY_URL: "http://192.168.1.100:8642"
      GATEWAY_PROXY_KEY: "your-secret-key-here"
    volumes:
      - ./matrix-store:/root/.athena/platforms/matrix/store
```
...
**Fix**: Check that `athena gateway` is running. Look at the terminal output for error messages. Common issues: wrong homeserver URL, expired access token, homeserver unreachable.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `MATRIX_ALLOWED_USERS`.

**Fix**: Add your User ID to `MATRIX_ALLOWED_USERS` in `~/.athena/.env` and restart the gateway. Use the full `@user:server` format.
