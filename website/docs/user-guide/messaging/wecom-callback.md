...
> Run `athena gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
(Use `athena gateway start` only after `athena gateway install` has registered the systemd/launchd service.)
...
in `~/.athena/.env` values around `=` will also break signature checks. After
fixing, restart `athena gateway run`.
...
Check `athena gateway run` logs for the bound host/port. If the adapter bound to
