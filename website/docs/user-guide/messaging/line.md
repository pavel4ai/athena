...
> Run `athena gateway setup` and pick **LINE** for a guided walk-through.
...
# devtunnel
devtunnel create athena-line --allow-anonymous
devtunnel port create athena-line -p 8646 --protocol https
devtunnel host athena-line
```
...
**Bot receives nothing in groups.** Check `LINE_ALLOWED_GROUPS` includes the `C...` group ID. To find a group ID, send a test message and grep `~/.athena/logs/gateway.log` for `LINE: rejecting unauthorized source` — the rejected source dict has the IDs.
