...
> Run `athena gateway setup` and pick **Google Chat** for a guided walk-through.
...
`~/.athena/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a athena restart)
...
- Principal: `athena-chat-bot@<your-project>.iam.gserviceaccount.com`
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/athena-chat-events`.
...
Add the Google Chat section to `~/.athena/.env`:
...
`~/.athena/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "Athena is thinking…").
...
athena -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Athena home (e.g.
`~/.athena/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.athena/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.athena/google_chat_user_token.json` (if present from
...
3. Check `athena gateway` logs for `[GoogleChat] Connected`. If you see
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.athena/google_chat_user_tokens/<sanitized_email>.json` (filesystem
