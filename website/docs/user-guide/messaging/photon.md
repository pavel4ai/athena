...
athena photon setup --phone +15551234567
```
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.athena/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.athena/.env`
...
(device token, dashboard project id) lives in `~/.athena/auth.json` under
...
```bash
athena pairing approve photon <CODE>
```

Use `athena pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.athena/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.athena/.env`):
...
- **`sidecar deps : ✗ run athena photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `athena photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `athena photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `athena photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Athena sends images, voice
  ...
  media.
- **Native polls are supported.** Athena sends poll content through
...
- **Read receipts are supported.** The sidecar marks an inbound iMessage
  read after forwarding it to Athena, so the sender sees `Read` without
  waiting for a model/tool turn. Inbound receipts for Athena-sent messages
...
- **Message effects are supported.** Athena sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `athena send`, the dashboard) reuse the sidecar the
  gateway spawned — they read its port/token from
  `<athena-home>/runtime/photon-sidecar.json`, written once the sidecar
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Athena is set
