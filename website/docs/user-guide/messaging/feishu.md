...
5. Run `athena gateway setup`, select **Feishu / Lark**, and enter the credentials when prompted.
...
Gateway-driven update prompts use a native Feishu `Yes` / `No` card instead of falling back to plain text replies. When `athena update --gateway` needs confirmation, the adapter records the selected answer in Athena's `.update_response` file and replaces the card inline with a resolved state.
...
Rules live in `~/.athena/feishu_comment_rules.json` (pairing grants in `~/.athena/feishu_comment_pairing.json`) with mtime-cached hot-reload — edits take effect on the next comment event without restarting the gateway.
...
|---------|---------|---------|
| Quiet period | `ATHENA_FEISHU_TEXT_BATCH_DELAY_SECONDS` | 0.6s |
| Max messages per batch | `ATHENA_FEISHU_TEXT_BATCH_MAX_MESSAGES` | 8 |
| Max characters per batch | `ATHENA_FEISHU_TEXT_BATCH_MAX_CHARS` | 4000 |
...
| Setting | Env Var | Default |
|---------|---------|---------|
| Quiet period | `ATHENA_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | 0.8s |
...
Inbound messages are deduplicated using message IDs with a 24-hour TTL. The dedup state is persisted across restarts to `~/.athena/feishu_seen_message_ids.json`.

| Setting | Env Var | Default |
|---------|---------|---------|
| Cache size | `ATHENA_FEISHU_DEDUP_CACHE_SIZE` | 2048 entries |
...
| `ATHENA_FEISHU_DEDUP_CACHE_SIZE` | — | `2048` | Max deduplicated message IDs to track |
| `ATHENA_FEISHU_TEXT_BATCH_DELAY_SECONDS` | — | `0.6` | Text burst debounce quiet period |
| `ATHENA_FEISHU_TEXT_BATCH_MAX_MESSAGES` | — | `8` | Max messages merged per text batch |
| `ATHENA_FEISHU_TEXT_BATCH_MAX_CHARS` | — | `4000` | Max characters merged per text batch |
| `ATHENA_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | — | `0.8` | Media burst debounce quiet period |
...
| `FEISHU_APP_ID or FEISHU_APP_SECRET not set` | Set both env vars or configure via `athena gateway setup` |
...
Feishu / Lark uses the `athena-feishu` platform preset, which includes the same core tools as Telegram and other gateway-based messaging platforms.
