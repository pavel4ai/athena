...
Or set environment variables directly in `~/.athena/.env`:
...
```bash
athena pairing approve bluebubbles <CODE>
```
Use `athena pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.athena/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.athena/.env`):
```bash
...
Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in `~/.athena/config.yaml` (default: `true`). There is no corresponding environment variable.
...
- Check `athena logs gateway` for webhook errors (or `athena logs -f` to follow in real-time)
