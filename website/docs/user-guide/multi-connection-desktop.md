...
# Connecting Desktop to Many Athena Instances

Register every Athena backend you own — the local runtime, remote gateways on
your LAN or VPS, SSH hosts, and Athena Cloud instances — in one desktop app,
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another Athena gateway…"**) deep-links straight to
...
list of Athena gateways. Its intro says it plainly: *"Manage this device and
every Athena gateway it can reach through remote, SSH, or Cloud connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The Athena runtime managed by this app." | automatic |
| **Remote gateway** | "A Athena gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "A Athena install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
| **Athena Cloud** | "A hosted instance discovered through your Athena Cloud account." | portal sign-in |
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  ...
  **SSH** entries are deduplicated on the normalized `user@host:port` plus
  remote profile.
- Cloud entries normally come from the Athena Cloud sign-in/discovery flow at
  the top of the Gateways page — the **Athena Cloud** kind in the add-connection
...
The Athena Cloud panel also lists **Saved Cloud gateways** when portal discovery
...
3. Pick the kind: **Local**, **Athena Cloud**, **Remote gateway**, or **SSH**.
   (**Local** is disabled while the app-managed local entry exists — which is
   almost always; **Athena Cloud** directs you to the cloud sign-in/discovery
   flow above.)
4. Fill the fields:
   - **Name** — required, unique; the "device name" shown everywhere this
     instance appears (placeholder: `Homelab`). Max 64 characters.
   - *Remote gateway only:*
     - **Gateway URL** — the base URL of a running `athena serve` backend,
...
Gateways are machines or hosted backends; profiles are isolated Athena agents
...
connection is registered) dispatches `athena update` to every eligible
...
- **Athena Cloud** instances are skipped with a *"Managed by Athena Cloud"*
...
panel, ⌘K **Update Athena**, the update-ready toast) run the same fan-out
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `athena serve` is running on the remote host, the
...
- **"Update Athena Desktop to chat with agents on other connections"** — the
