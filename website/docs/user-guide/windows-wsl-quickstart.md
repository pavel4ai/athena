...
The athena gateway (and anything else you want to keep running) is easier to manage with systemd. On modern WSL, enable it once inside your distro:
...
- Your Athena install (`~/.athena/`) — Linux side. The installer already does this.
...
Clone inside WSL. Always, unless you have a specific reason not to. A typical Athena workflow (`athena chat`, tool calls that `rg`/`ripgrep` the repo, file watchers, background gateway) will be dramatically faster and more reliable against `~/code/myrepo` than `/mnt/c/Users/you/myrepo`.

One exception: **MCP bridges that launch Windows binaries.** If you're using `chrome-devtools-mcp` through `cmd.exe` (see [MCP guide: WSL → Windows Chrome](/guides/use-mcp-with-athena#wsl2-bridge-athena-in-wsl-to-windows-chrome)), Windows may complain with a `UNC` warning if Athena's current working directory is `~`. In that case, start Athena from somewhere under `/mnt/c/` so the Windows process has a drive-letter cwd.
...
- Using the **OpenAI-compatible API server** (exposed by `athena gateway` when `API_SERVER_ENABLED=true`) from a Windows-side tool. See the [API Server feature page](/user-guide/features/api-server).
...
home directory, and launches Athena. If `athena` is not on PATH yet, open WSL
once manually and run `source ~/.bashrc`, or replace the command with
`uv run athena` inside your project checkout.
...
If you enabled systemd per the setup section above, `athena gateway` and the API server work the way they do on any Linux machine. Use the gateway setup wizard:
...
**Massive slowness on `git status` / `athena chat` in a repo.**
...
**`athena` not found after running the installer.**
...
- **[MCP guide → WSL → Windows Chrome](/guides/use-mcp-with-athena#wsl2-bridge-athena-in-wsl-to-windows-chrome)** — controlling your signed-in Windows Chrome from Athena in WSL.
