...
The approval system supports three modes, configured via `approvals.mode` in `~/.athena/config.yaml`:
...
| `timeout` | `300` | Seconds Athena waits for an approval reply before timing out. |
...
| `destructive_slash_confirm` | `true` | When true, destructive session slash commands (`/clear`, `/new`, `/reset`, `/undo`) prompt before discarding conversation state. Three-option dialog (Approve Once / Always Approve / Cancel) routed through native yes/no buttons on Telegram, Discord, and Slack; text fallback elsewhere. Users who click **Always Approve** flip this key to `false`. The TUI also honors this setting for its `/clear`, `/new`, and `/reset` modal; `ATHENA_TUI_NO_CONFIRM=1` force-skips that modal regardless of the configured value. |
...
1. **CLI flag**: Start a session with `athena --yolo` or `athena chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `ATHENA_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `ATHENA_YOLO_MODE` environment variable which is checked before every command execution.
...
the job uses KeepAlive or controls Athena.
...
Configure the timeout in `~/.athena/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.athena/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.athena/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` athena/gateway | Self-termination prevention |
...
The `ATHENA_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.athena/config.yaml`:
...
re-save it with `athena config edit`. Other malformed values are ignored with
a warning; they never become per-character approvals. Loading does not rewrite
your configuration file.

:::tip
Use `athena config edit` to review or remove patterns from your permanent allowlist.
:::

### Mining Approval History (`athena approvals suggest`)
...
```bash
athena approvals suggest            # dry run — prints a numbered proposal
athena approvals suggest --apply 1,3  # merge picks into command_allowlist
athena approvals suggest --json     # machine-readable output
```

The command scans the session database (`~/.athena/state.db`) for
...
### ATHENA_WRITE_SAFE_ROOT (optional sandbox)
...
- Set automatically in the [official Docker image](https://github.com/pavel4ai/athena) (`ATHENA_WRITE_SAFE_ROOT=/opt/data`)
- Supports multiple roots separated by `:` on Unix or `;` on Windows
- **Do not add to `~/.athena/.env` casually.** If you set it to a project directory, the agent cannot write to `~/.athena/cron/jobs.json`, profile skills, or other Athena state outside that prefix

To allow both a workspace and Athena home:

```bash
export ATHENA_WRITE_SAFE_ROOT=/path/to/project:/home/you/.athena
```

Unset the variable to restore unrestricted writes (subject to the protected-path denylist). Full reference: [ATHENA_WRITE_SAFE_ROOT](../reference/environment-variables.md#athena_write_safe_root).

### Cron and other Athena state

Do not ask the agent to `patch` `~/.athena/cron/jobs.json` directly. Use the `cronjob` tool, [`athena cron`](./features/cron.md), or `/cron` — they update the job store through the supported API. The same applies to other Athena control files when write safety blocks direct edits.
...
Set allowed user IDs as comma-separated values in `~/.athena/.env`:
...
3. The bot owner runs `athena pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.athena/config.yaml`:
...
athena pairing approve telegram ABC12DEF

# Revoke a user's access
athena pairing revoke telegram 123456789

# Clear all pending codes
athena pairing clear-pending
```

:::tip Docker users: run pairing commands as the `athena` user
The official Docker image runs the gateway as the unprivileged `athena` user
...
Always pass `-u athena`:

```bash
docker exec -u athena athena-agent athena pairing approve telegram ABC12DEF
```
...
**Storage:** Pairing data is stored in `~/.athena/pairing/` with per-platform JSON files:
...
Container resources are configurable in `~/.athena/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.athena/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `ATHENA_HOME`. Skills declare these in frontmatter:
...
When loaded, Athena checks if these files exist in the active profile's `ATHENA_HOME` and registers them for mounting:
...
4. **Store secrets securely** — keep API keys in `~/.athena/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.athena/logs/` for unauthorized access attempts
10. **Keep updated** — run `athena update` regularly for security patches
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.athena/.env`:
...
TERMINAL_SSH_USER=athena
TERMINAL_SSH_KEY=~/.ssh/athena_agent_key
```
...
Athena ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `athena_cli/security_advisories.py`.

How it runs:

- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `athena doctor` for the full remediation.
- **`athena doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `athena tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `athena tools`.
