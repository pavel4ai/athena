---
sidebar_position: 2
title: "Athena Agent Configuration"
description: "Configure Athena Agent — config.yaml, providers, models, API keys, and more"
---

# Athena Agent Configuration

All settings are stored in the `~/.athena/` directory for easy access.

:::tip Easiest path to a working `config.yaml`
Run `athena setup --portal` — one OAuth gets you a model provider and all four Tool Gateway tools without hand-editing YAML. Portal subscribers also get 10% off token-billed providers. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
athena config              # View current configuration
athena config edit         # Open config.yaml in your editor
athena config get KEY      # Print a resolved value
athena config set KEY VAL  # Set a specific value
athena config unset KEY    # Remove a user-set value
athena config check        # Check for missing options (after updates)
athena config migrate      # Interactively add missing options

# Examples:
athena config get model
athena config set model anthropic/claude-opus-4
athena config set terminal.backend docker
athena config unset terminal.backend
athena config set OPENROUTER_API_KEY sk-or-...  # Saves to .env
```

:::tip
The `athena config set` command automatically routes values to the right file — API keys are saved to `.env`, everything else to `config.yaml`.
:::
...
1. **CLI arguments** — e.g., `athena chat --model anthropic/claude-sonnet-4` (per-invocation override)
2. **`~/.athena/config.yaml`** — the primary config file for all non-secret settings
3. **`~/.athena/.env`** — fallback for env vars; **required** for secrets (API keys, tokens, passwords)
...
Long-running Athena server surfaces (including the gateway and
`athena serve --isolated`) apply the configured `RLIMIT_NOFILE` soft limit
...
The default is `4096`. Athena clamps the target to the operating system's hard
...
The `database:` section controls how Athena opens its SQLite state database
...
database:
  ...
  # live-downgraded — Athena keeps WAL and logs an error telling you the
...
Athena also warns (once per process per database) when an existing
...
You can set `providers.<id>.request_timeout_seconds` for a provider-wide request timeout, plus `providers.<id>.models.<model>.timeout_seconds` for a model-specific override. Applies to the primary turn client on every transport (OpenAI-wire, native Anthropic, Anthropic-compatible), the fallback chain, rebuilds after credential rotation, and (for OpenAI-wire) the per-request timeout kwarg — so the configured value wins over the legacy `ATHENA_API_TIMEOUT` env var.

You can also set `providers.<id>.stale_timeout_seconds` for the non-streaming stale-call detector, plus `providers.<id>.models.<model>.stale_timeout_seconds` for a model-specific override. This wins over the legacy `ATHENA_API_CALL_STALE_TIMEOUT` env var.

Leaving these unset keeps the legacy defaults (`ATHENA_API_TIMEOUT=1800`s, `ATHENA_API_CALL_STALE_TIMEOUT=90`s, native Anthropic 900s). The non-streaming stale detector is auto-disabled for local endpoints when left implicit and can scale upward for very large contexts. Not currently wired for AWS Bedrock (both `bedrock_converse` and AnthropicBedrock SDK paths use boto3 with its own timeout configuration). See the commented example in [`cli-config.yaml.example`](https://github.com/pavel4ai/athena/blob/main/cli-config.yaml.example).
...
retries after an hour). Applying an update (`athena update`, or the desktop's
Update button) always fetches fresh and invalidates the cached answer. Explicit
checks — `athena update --check`, the desktop's "Check for Updates…" menu item,
...
Athena's isolated internal Git commands default to `ssh -o BatchMode=yes`:
...
`athena update` settings live under `updates` in `config.yaml`:

```yaml
updates:
  pre_update_backup: quick       # quick (state snapshot, default) | full (snapshot + ATHENA_HOME zip) | off
...
`pre_update_backup` is the single pre-update safety knob: `quick` (default) snapshots critical state files (pairing data, cron jobs, config, auth; files over 1 GiB are skipped) into `state-snapshots/`; `full` additionally zips all of `ATHENA_HOME` into `backups/` and can add minutes on large homes; `off` disables both. Legacy booleans are honored (`true` → `full`, `false` → `off`).

Point-in-time copies of `config.yaml` itself (taken before `athena setup` rewrites it, before `athena migrate` edits it, and when the file fails to parse) go to `backups/config/config.yaml.<reason>.<timestamp>`. Identical repeats are skipped and only the newest five per reason are kept, so they never pile up beside `config.yaml`.
...
`terminal.temp_dir` controls where Athena puts session temp artifacts on the
local backend — background-process logs/pid/exit files, code-execution
sandboxes, and spilled tool results. When it's empty (the default), Athena
honors an explicit `TMPDIR`/`TMP`/`TEMP` from the environment and otherwise
uses a managed directory on real storage at `~/.athena/cache/terminal`
instead of `/tmp` — on many distros (Arch-based setups in particular) `/tmp`
is a small RAM-backed tmpfs that Athena session artifacts can fill under
...
`terminal.font_family` controls the embedded terminal in Athena Desktop. It accepts either one locally installed family name (for example, `MesloLGS NF`) or a CSS font stack. Athena appends its bundled JetBrains Mono stack as a fallback, and an empty value keeps the default. You can edit the same profile-scoped setting in **Settings → Appearance → Terminal Font**; no Google Fonts download or system-font permission is required.

For cloud sandboxes such as Modal, Daytona, and Vercel Sandbox, `container_persistent: true` means Athena will try to preserve filesystem state across sandbox recreation. It does not promise that the same live sandbox, PID space, or background processes will still be running later.
...
find the credentials and config they already use in your normal shell. Athena
state is still profile-scoped through `ATHENA_HOME`; `HOME` is not how profiles
select config, memory, sessions, or skills.

Athena does **not** change your system-wide `HOME`, your shell startup files, or
the operating system account home. This setting only controls the environment
passed to subprocesses that Athena launches through tools such as `terminal`,
...
|---|---|---|---|
| `auto` | Keep the real OS-user `HOME` | Use `{ATHENA_HOME}/home` | Recommended default. Host CLIs keep working; container state persists. |
...
| `profile` | Use `{ATHENA_HOME}/home` when it exists | Use `{ATHENA_HOME}/home` when it exists | Strict per-profile CLI config isolation, but normal `~/.ssh`, `~/.gitconfig`, `~/.azure`, `~/.config/gh`, Claude/Codex auth, npm state, etc. will not be visible unless you initialize or link them inside the profile home. |
...
In that mode tool subprocesses use `{ATHENA_HOME}/home` as `HOME`. Athena also
sets `ATHENA_REAL_HOME` so scripts can still locate the actual user home when
they need it. Container backends keep using `{ATHENA_HOME}/home` in `auto` mode
because that directory lives on the persistent Athena data volume.

Scripts that need to distinguish profile state from the real user home should
prefer `ATHENA_HOME` for Athena data and `ATHENA_REAL_HOME` for the account home:

```python
from pathlib import Path
import os

athena_home = Path(os.environ["ATHENA_HOME"])
real_home = Path(os.environ.get("ATHENA_REAL_HOME", os.environ["HOME"]))
```

:::warning
The agent has the same filesystem access as your user account. Use `athena tools` to disable tools you don't want, or switch to Docker for sandboxing.
:::
...
**`docker_env`** vs **`docker_forward_env`**: the former injects literal `KEY=value` pairs you specify in the config (the values live in your `config.yaml` or are passed as a JSON dict via `TERMINAL_DOCKER_ENV='{"DEBUG":"1"}'`). The latter forwards values from your shell or `~/.athena/.env`, so the actual secret never appears in the config file. Use `docker_forward_env` for tokens and `docker_env` for static knobs the container needs.
...
**`terminal.docker_network`** (default `true`; env: `TERMINAL_DOCKER_NETWORK`) — set to `false` to run the sandbox container with `--network=none`, cutting off all network egress from agent commands. This applies to the execution container used by `terminal`, `execute_code`, and the file tools. Because containers persist across Athena processes, flipping this to `false` while an older networked container exists will remove that container and start a fresh air-gapped one (a warning is logged); background processes running inside it are lost. Prefer this key over passing `--network=none` through `docker_extra_args`.

**Requirements:** Docker Desktop or Docker Engine installed and running. Athena probes `$PATH` plus common macOS install locations (`/usr/local/bin/docker`, `/opt/homebrew/bin/docker`, Docker Desktop app bundle). Podman is supported out of the box: set `ATHENA_DOCKER_BINARY=podman` (or the full path) to force it when both are installed.
...
- `athena-task-id=<sanitized task_id>` — keys the per-task reuse probe
- `athena-profile=<sanitized profile name>` — scopes reuse and reaping to the active Athena profile by default; when `docker_shared_container_key` is set, its sanitized value is used instead

On startup, Athena runs `docker ps --filter label=athena-task-id=<id> --filter label=athena-profile=<identity>` and **attaches to the existing container** when it finds one. The identity is the active profile unless `docker_shared_container_key` explicitly opts trusted profiles into a common value. If the container is `exited` (e.g. after a Docker daemon restart), it's `docker start`'d and reused — filesystem state and any installed packages survive, but in-container background processes do not.
...
| Orphan reaper at next startup | Sweeps **Exited** athena-labeled containers older than `2 × lifetime_seconds` (default 600s = 10 min), scoped to the current profile. **Running containers are never touched** — sibling-process safety. Set `docker_orphan_reaper: false` to disable. |
...
**Credential forwarding:** Env vars listed in `docker_forward_env` are resolved from your shell environment first, then `~/.athena/.env`. Skills can also declare `required_environment_variables` which are merged automatically.
...
| `ATHENA_DOCKER_BINARY` | _none_ | Force a specific docker/podman binary path |
...
Without a matching `AcceptEnv`, the server silently drops the variables and the remote shell sees them unset. Athena provider credentials (`OPENAI_API_KEY`, …) are never forwarded even if listed. See [Env Var Passthrough](security.md#environment-variable-passthrough).
...
**Persistence:** When enabled, the sandbox filesystem is snapshotted on cleanup and restored on next session. Snapshots are tracked in `~/.athena/modal_snapshots.json`. This preserves filesystem state, not live processes, PID space, or background jobs.

**Credential files:** Automatically mounted from `~/.athena/` (OAuth tokens, etc.) and synced before each command.
...
**Persistence:** When enabled, sandboxes are stopped (not deleted) on cleanup and resumed on next session. Sandbox names follow the pattern `athena-{task_id}`.

**Disk limit:** Daytona enforces a 10 GiB maximum. Requests above this are capped with a warning.

### Vercel Sandbox Backend

Runs commands in a [Vercel Sandbox](https://vercel.com/docs/vercel-sandbox) cloud microVM. Athena uses the normal terminal and file tool surfaces; there are no Vercel-specific model-facing tools.
...
```bash
pip install 'athena-agent[vercel]'
```

**Required authentication:** Configure access-token auth with all three of `VERCEL_TOKEN`, `VERCEL_PROJECT_ID`, and `VERCEL_TEAM_ID`. This is the supported setup for deployments and normal long-running Athena processes on Render, Railway, Docker, and similar hosts.

For one-off local development, Athena also accepts short-lived Vercel OIDC tokens:

```bash
VERCEL_OIDC_TOKEN="$(vc project token <project-name>)" athena chat
```

From a linked Vercel project directory, you can omit the project name:

```bash
VERCEL_OIDC_TOKEN="$(vc project token)" athena chat
```

OIDC tokens are short-lived and should not be used as the documented deployment path.

**Runtime:** `terminal.vercel_runtime` supports `node24`, `node22`, and `python3.13`. If unset, Athena defaults to `node24`.

**Persistence:** When `container_persistent: true`, Athena snapshots the sandbox filesystem during cleanup and restores a later sandbox for the same task from that snapshot. Snapshot contents can include Athena-synced credentials, skills, and cache files that were copied into the sandbox. This preserves filesystem state only; it does not preserve live sandbox identity, PID space, shell state, or running background processes.

**Background commands:** `terminal(background=true)` uses Athena' generic non-local background process flow. You can spawn, poll, wait, view logs, and kill processes through the normal process tool while the sandbox is alive. Athena does not provide native Vercel detached-process recovery after cleanup or restart.

**Disk sizing:** Vercel Sandbox does not currently support Athena' `container_disk` resource knob. Leave `container_disk` unset or at the shared default `51200`; non-default values fail diagnostics and backend creation instead of being silently ignored.
...
**Scratch directory:** Resolved in order: `TERMINAL_SCRATCH_DIR` → `TERMINAL_SANDBOX_DIR/singularity` → `/scratch/$USER/athena-agent` (HPC convention) → `~/.athena/sandboxes/singularity`.
...
- **Docker** — Run `docker version` to verify Docker is working. If it fails, fix Docker or `athena config set terminal.backend local`.
- **SSH** — Both `TERMINAL_SSH_HOST` and `TERMINAL_SSH_USER` must be set. Athena logs a clear error if either is missing.
- **Modal** — Needs `MODAL_TOKEN_ID` env var or `~/.modal.toml`. Run `athena doctor` to check.
...
- This covers Athena state (`~/.athena/`), **not** arbitrary working-tree files inside the sandbox — have the agent copy important artifacts out explicitly (e.g. `scp`, `modal volume put`) before the sandbox is destroyed.
...
terminal:
  ...
  docker_volumes:
    ...
    - "/home/user/.athena/cache/documents:/output" # Gateway-visible exports
```
...
`/home/user/.athena/cache/documents:/output`.

- Write files inside Docker to `/output/...`
- Emit the **host path** in `MEDIA:`, for example:
  `MEDIA:/home/user/.athena/cache/documents/report.txt`
...
Athena resolves each listed variable from your current shell first, then falls back to `~/.athena/.env` if it was saved with `athena config set`.
...
This is a snapd limitation ([LP#1908448](https://bugs.launchpad.net/snapd/+bug/1908448)), not something Athena can probe around. Either install Docker from Docker's apt repository instead of the snap (preferred — all hardening stays on), or opt in:
...
```bash
athena config set terminal.persistent_shell false
```
...
- `athena config migrate` scans all enabled skills, finds unconfigured settings, and offers to prompt you
- `athena config show` displays all skill settings under "Skill Settings" with the skill they belong to
- When a skill loads, its resolved config values are injected into the skill context automatically

**Setting values manually:**

```bash
athena config set skills.config.myplugin.path ~/myplugin-data
```
...
Controls how much content Athena loads from each automatic context file before applying head/tail truncation. This applies to files injected into the system prompt such as `SOUL.md`, `.athena.md`, `AGENTS.md`, `CLAUDE.md`, and `.cursorrules`. It does **not** affect the `read_file` tool.
...
Athena also flags **provider-side elision**: when an MCP or web tool result embeds its own truncation markers (`...N more items`, `"has_more": true`, "saved to sandbox" notes), a one-line notice is appended to the result warning that the visible data is incomplete and should be paged/fetched before treating any enumeration as complete.
...
`athena tools`), so a toolset listed here is always removed — even if a
...
```yaml
worktree: true    # Always create a worktree (same as athena -w)
...
If another turn still holds the session lease when this budget expires, Athena
fails closed: it does not load the transcript or run the model for the waiting
message. The user receives a rejection notice and must resend. Athena does not
...
- **Needs-attention escalation.** A platform continuously in the retry queue past `agent.reconnect_attention_after` (default `7200` seconds = 2 hours, `0` disables) gets `needs_attention: true` and a `retrying_since` timestamp in gateway runtime status (`athena status`), plus a WARNING log. Retries continue unchanged — this is a signal, not a circuit breaker. The flag clears on successful reconnect.
...
Plugin engines are **never auto-activated** — you must explicitly set `context.engine` to the plugin name. Available engines can be browsed and selected via `athena plugins` → Provider Plugins → Context Engine.

See [Memory Providers](/user-guide/features/memory-providers) for the analogous single-select system for memory plugins.

## Iteration Budget

When the agent is working on a complex task with many tool calls, it can burn through its iteration budget (default: 500 turns). Athena does **not** inject mid-task pressure warnings — earlier builds warned the model at 70%/90% budget, which caused models to abandon complex tasks prematurely and was removed in April 2026.

Instead, when the budget is actually exhausted (500/500), Athena injects one message asking the model to wrap up and allows a single **grace call** so it can deliver a final response. If that grace call still doesn't produce text, the agent is asked to summarise what it accomplished.
...
`verify_on_stop` accepts `true` (on everywhere), `false` (off — the default), or `"auto"` (legacy surface-aware behavior: on for interactive coding surfaces — CLI, TUI, desktop — and programmatic callers; off for messaging surfaces like Telegram/Discord where the verification narrative reads as chat noise). Off is the default everywhere: fresh installs ship `false` and the config migration turned it off on existing installs, so enabling it is an explicit opt-in. The `ATHENA_VERIFY_ON_STOP` env var overrides the config value when set.

The evidence that feeds this guard (which test/lint/build commands ran, which files were edited since) lives in `~/.athena/verification_evidence.db`. That ledger is only written or created while the guard is enabled; with `verify_on_stop: false` nothing is recorded and an existing file can be deleted freely.
...
|---------|---------|----------------|--------------|
| Socket read timeout | 120s | Auto-raised to 1800s | `ATHENA_STREAM_READ_TIMEOUT` |
| Stale stream detection | 180s | Raised to a 900s ceiling (`agent.local_stream_stale_timeout`) | `ATHENA_STREAM_STALE_TIMEOUT` |
| Stale non-stream detection | 90s | Auto-disabled when left implicit | `providers.<id>.stale_timeout_seconds` or `ATHENA_API_CALL_STALE_TIMEOUT` |
| API call (non-streaming) | 1800s | Unchanged | `providers.<id>.request_timeout_seconds` / `timeout_seconds` or `ATHENA_API_TIMEOUT` |

The **socket read timeout** controls how long httpx waits for the next chunk of data from the provider. Local LLMs can take minutes for prefill on large contexts before producing the first token, so Athena raises this to 30 minutes when it detects a local endpoint. If you explicitly set `ATHENA_STREAM_READ_TIMEOUT`, that value is always used regardless of endpoint detection.

The **stale stream detection** kills connections that receive SSE keep-alive pings but no actual content. For local providers (which don't send keep-alive pings during prefill) the default is raised to a finite 900-second ceiling instead of the 180s base — configurable via `agent.local_stream_stale_timeout` or the `ATHENA_LOCAL_STREAM_STALE_TIMEOUT` env var.

The **stale non-stream detection** kills non-streaming calls that produce no response for too long. By default Athena disables this on local endpoints to avoid false positives during long prefills. If you explicitly set `providers.<id>.stale_timeout_seconds`, `providers.<id>.models.<model>.stale_timeout_seconds`, or `ATHENA_API_CALL_STALE_TIMEOUT`, that explicit value is honored even on local endpoints.
...
For Claude on **native Anthropic**, **OpenRouter**, and **Nous Portal**, Athena attaches `cache_control` breakpoints with the 1-hour TTL (`ttl: "1h"`) on the system prompt and skill blocks. The first send within a fresh hour pays full input rates; subsequent sends across any session within the same hour pull from the cache at the discounted cached-read rate. This means the system prompt, loaded skill content, and the early portion of any long-context include get reused across `athena` sessions and across forked subagents for the first hour.
...
The one explicit knob is the cache TTL tier Athena requests on Anthropic-style breakpoints:
...
Instead of hand-editing YAML, run `athena model` and pick **"Configure auxiliary models"** from the menu. You'll get an interactive per-task picker:
...
:::tip MiniMax OAuth
`minimax-oauth` logs in via browser OAuth (no API key needed). Run `athena model` and select **MiniMax (OAuth)** to authenticate. Auxiliary tasks use `MiniMax-M2.7-highspeed` automatically. See the [MiniMax OAuth guide](../guides/minimax-oauth.md).
:::

:::tip xAI Grok OAuth
`xai-oauth` logs in via browser OAuth for SuperGrok and X Premium+ subscribers (no API key needed). Run `athena model` and select **xAI Grok OAuth (SuperGrok / Premium+)** to authenticate. The same OAuth token is reused for every direct-to-xAI surface (chat, auxiliary tasks, TTS, image gen, video gen, transcription). See the [xAI Grok OAuth guide](../guides/xai-grok-oauth.md), and if Athena is on a remote host see [OAuth over SSH / Remote Hosts](../guides/oauth-over-ssh.md).
:::
...
auxiliary:
  ...
  # Kanban triage specifier — `athena kanban specify <id>` (or the
...
Each auxiliary task can optionally define a `fallback_chain` — a list of provider/model entries that Athena tries when the primary auxiliary provider fails due to rate limits, connectivity issues, or payment restrictions:
...
When the primary auxiliary provider (`openrouter` / `openai/gpt-4o-mini`) returns a rate-limit, connection timeout, or payment-required error, Athena walks the `fallback_chain` in order. It skips entries whose provider matches the already-failed provider, and tries each remaining entry until one succeeds or the chain is exhausted. If all fallbacks fail, Athena falls back to the main agent model as a final safety net.
...
Or via environment variable (in `~/.athena/.env`):
...
| `"codex"` | Force Codex OAuth (ChatGPT account). Supports vision (gpt-5.3-codex). | `athena model` → ChatGPT or Codex Subscription |
| `"minimax-oauth"` | Force MiniMax OAuth (browser login, no API key). Uses MiniMax-M2.7-highspeed for auxiliary tasks. | `athena model` → MiniMax (OAuth) |
| `"xai-oauth"` | Force xAI Grok OAuth (browser login for SuperGrok or X Premium+ subscribers, no API key). Same OAuth token covers chat, TTS, image, video, and transcription. | `athena model` → xAI Grok OAuth (SuperGrok / Premium+) |
| `"main"` | Use your active custom/main endpoint. This can come from `OPENAI_BASE_URL` + `OPENAI_API_KEY` or from a custom endpoint saved via `athena model` / `config.yaml`. Works with OpenAI, local models, or any OpenAI-compatible API. **Auxiliary tasks only — not valid for `model.provider`.** | Custom endpoint credentials + base URL |
...
```
Run `athena model` and select **MiniMax (OAuth)** to log in and set this automatically. For the China region, the base URL will be `https://api.minimaxi.com/anthropic`. See the [MiniMax OAuth guide](../guides/minimax-oauth.md) for the full walkthrough.
...
:::tip
Run `athena config` to see your current auxiliary model settings. Overrides only show up when they differ from the defaults.
:::
...
For other models routed through OpenRouter, Athena reads the live model
...
automatically without waiting for a Athena update; when the catalog is
unreachable or a model isn't listed, Athena falls back to its built-in
...
:::note
There is no `athena config set` support for `reasoning_overrides` keys — edit the YAML file directly. This is because model names often contain dots (e.g. `claude-opus-4.5`), which conflict with the CLI's dotted-key syntax.
:::
...
display:
  ...
  # blocking prompts) so Warp's tab status and notification mailbox track Athena. No extra keys needed.
...
- `+X -Y` line deltas appear only when the tool result already reports a diff (currently `patch`). Athena never shells out to git to compute them, so a `write_file` edit is counted without a delta.
...
Set `file_mutation_verifier: false` (or `ATHENA_FILE_MUTATION_VERIFIER=0`) to suppress the footer. The verifier only fires when real failures are outstanding at turn end — a model that retries a failed patch and succeeds within the same turn will not trigger it for that file.
...
- **Write denied** — path is on the credential denylist or outside `ATHENA_WRITE_SAFE_ROOT` (see [File write safety](./security.md#file-write-safety))
...
⚠️ File-mutation verifier: 2 file(s) were NOT modified this turn despite any wording above that may suggest otherwise. Run `git status` or `read_file` to confirm.
  • ~/.athena/cron/jobs.json — [patch] Write denied: '…' is outside ATHENA_WRITE_SAFE_ROOT (/path/to/project)
  • ~/.athena/scripts/monitor.py — [write_file] Write denied: '…' is outside ATHENA_WRITE_SAFE_ROOT (/path/to/project)
```

If writes to Athena state (cron jobs, skills, scripts under `~/.athena/`) are failing, check whether `ATHENA_WRITE_SAFE_ROOT` is set in your environment. For cron changes, use the `cronjob` tool or `athena cron edit` instead of patching `jobs.json` directly.
...
You can also set this per-session with the `ATHENA_LANGUAGE` env var, which overrides the config value.
...
- The same key also filters the **Ink TUI** status rule (`athena tui`), where `cache_hit`, `latency`, and `tps` render as width-budgeted tail segments (◎ / ◷ / ↑) on terminals ≥96/104/110 columns respectively.
...
send `originator: athena-agent` and `User-Agent: AthenaAgent/<version>`.
...
stt:
  ...
  # prompt: "Athena, Teknium, Nous Research, kanban"   # Static vocabulary hint (see below)
  local:
...
Language resolution is the same for **every** STT provider (local, groq, openai, mistral, xai, elevenlabs, deepinfra, command providers, and plugins): `stt.<provider>.language` → `stt.language` → `ATHENA_LOCAL_STT_LANGUAGE` env var → provider auto-detect. **The default is `stt.language: "en"`** — Whisper auto-detection frequently misidentifies short or accented clips, which shows up as voice notes transcribed in the wrong language. Non-English speakers should set `stt.language` to their language code once (e.g. `"es"`, `"zh"`, `"uk"`); set it to `""` to restore auto-detection for multilingual use.
...
- `groq` uses Groq's Whisper-compatible endpoint and reads `GROQ_API_KEY`. Pass `stt.groq.language` (or the global `ATHENA_LOCAL_STT_LANGUAGE` env var) to skip auto-detection and reduce latency.
...
An explicitly selected `stt.provider` is honored strictly — if it's unavailable, transcription errors with guidance to run `athena tools` rather than switching providers. Only when no provider has ever been selected does Athena auto-detect in this order: `local` → `groq` → `openai`.
...
stt:
  provider: "local"
  prompt: "Athena, Teknium, Nous Research, kanban, Ollama"
```
...
When the cap is reached, Athena returns a direct limit message naming which
surfaces hold the slots. Existing active sessions keep their normal behavior.
Run `athena status` to see the current slot usage and every holder.
...
It is intended for a single host/profile runtime, not a shared `$ATHENA_HOME`
...
- `pair` is the default for chat-style DM platforms. Athena denies access, but replies with a one-time pairing code in DMs.
...
quick_commands:
  ...
  update:
    type: exec
    command: cd ~/.athena/athena-agent && git pull && uv pip install -e .
  gpu:
    type: exec
...
The `web_search` and `web_extract` tools support five backend providers. Configure the backend in `config.yaml` or via `athena tools`:
...
web:
  ...
  # Pin Exa/Parallel to a tier (set by the athena tools Free/Paid rows).
...
**Backend selection:** The runtime always uses the stored `web.backend` selection (set via `athena tools`; `nous` routes through the managed Tool Gateway). Only if no web backend has ever been selected is one auto-detected from available API keys: if only `SEARXNG_URL` is set, SearXNG is used; if only `EXA_API_KEY` is set, Exa; if only `TAVILY_API_KEY` is set, Tavily; if only `PERPLEXITY_API_KEY` is set, Perplexity; if only `PARALLEL_API_KEY` is set, Parallel; if only `KEENABLE_API_KEY` is set, Keenable. With **no selection and no credentials at all**, requests rotate round-robin across the keyless free-tier ring (Exa / Parallel / Firecrawl / Keenable) with automatic next-in-line failover on rate limits — see the [Web Search guide](/user-guide/features/web-search) for details. Once a selection exists, adding a key to `.env` does not change the route. Selecting Tavily, Firecrawl, or Keenable in `athena tools` also works without a key.
...
**Exa:** Set `EXA_API_KEY` in `~/.athena/.env`. Supports `category` filtering (`company`, `research paper`, `news`, `people`, `personal site`, `pdf`) and domain/date filters.
...
browser:
  ...
  record_sessions: false         # Auto-record browser sessions as WebM videos to ~/.athena/browser_recordings/
...
| `off` | Skip all approval checks. Equivalent to `ATHENA_YOLO_MODE=true`. **Use with caution.** |
...
```yaml
checkpoints:
  enabled: false                 # Enable automatic checkpoints (also: athena chat --checkpoints). Default: false (opt-in).
...
**Direct endpoint override:** If you want the obvious custom-endpoint path, set `delegation.base_url`, `delegation.api_key`, and `delegation.model`. That sends subagents directly to that OpenAI-compatible endpoint and takes precedence over `delegation.provider`. If `delegation.api_key` is omitted, Athena falls back to `OPENAI_API_KEY` only. When `delegation.provider` is set alongside `delegation.base_url`, the explicit endpoint and key still win, but that provider's request settings (`extra_body` overrides and max output tokens from your `custom_providers` entry) are carried into the subagent.
...
| File | Purpose | Scope |
|------|---------|-------|
| `SOUL.md` | **Primary agent identity** — defines who the agent is (slot #1 in the system prompt) | `~/.athena/SOUL.md` or `$ATHENA_HOME/SOUL.md` |
| `.athena.md` / `ATHENA.md` | Project-specific instructions (highest priority) | Walks to git root |
...
- **Project context files use a priority system** — only ONE type is loaded (first match wins): `.athena.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. SOUL.md is always loaded independently.
...
| Context | Default |
|---------|---------|
| **CLI (`athena`)** | Current directory where you run the command |
| **Messaging gateway** | `terminal.cwd` from `~/.athena/config.yaml`; if unset, home directory `~` |
...
`MESSAGING_CWD` and direct `TERMINAL_CWD` entries in `~/.athena/.env` are legacy compatibility fallbacks. New configurations should use `terminal.cwd`.
...
- `seen` — internal state. Athena latches each shown hint here so it never fires again; the profile-build offer is also recorded here once shown. Don't hand-edit it — wipe the whole `onboarding` section if you want to re-see all hints.
...
dashboard:
  ...
  public_url: ""              # Full public authority for OAuth redirect_uri (env: ATHENA_DASHBOARD_PUBLIC_URL)
...
- `oauth` / `basic_auth` / `drain_auth` — auth provider config read by the bundled dashboard-auth plugins. The drain secret itself is **not** set here; it's provisioned via the `ATHENA_DASHBOARD_DRAIN_SECRET` env var. See [Web Dashboard](/user-guide/features/web-dashboard) for full auth setup.
...
- `startup_orphan_sweep` (default `true`) — the WS-orphan reap timer above is in-process, so a gateway restart (update, crash, systemd) before it fires leaves the session row open forever — phantom "active" work in `/resume` and dashboards. On every gateway boot — both the stdio TUI (`entry.main`) and the desktop/dashboard WebSocket sidecar (`handle_ws`) — rows with source `tui` / `desktop` / `subagent` whose start time **and** newest message are both older than the session TTL (`ATHENA_TUI_SESSION_TTL_S`, default 6 hours) are closed with `end_reason: startup_orphan_reap`. Messaging-platform sessions (Telegram, Discord, …) are never touched, live in-memory sessions (a client that already resumed) are excluded, and swept sessions remain resumable.
