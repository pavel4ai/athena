...
```yaml
updates:
  pre_update_backup: false       # Create a full ATHENA_HOME zip before every update
...
- `athena-profile=<sanitized profile name>` — scopes reuse and reaping to the active Athena profile
...
- The remote sandbox may have been torn down by the time you go looking; the local `~/.athena/cache/remote-syncs/…` copy is the authoritative record of what the agent changed.
...
| Stale stream detection | 180s | Auto-disabled | `ATHENA_STREAM_STALE_TIMEOUT` |
| Stale non-stream detection | 300s | Auto-disabled when left implicit | `providers.<id>.stale_timeout_seconds` or `ATHENA_API_CALL_STALE_TIMEOUT` |
...
| `"codex"` | Force Codex OAuth (ChatGPT account). Supports vision (gpt-5.3-codex). | `athena model` → Codex |
...
quick_commands:
  ...
  update:
    type: exec
    command: cd ~/.athena/athena-agent && git pull && pip install -e .
  gpu:
    type: exec
