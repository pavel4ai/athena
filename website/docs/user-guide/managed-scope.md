...
`~/.athena/config.yaml`, `~/.athena/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/athena`:

```text
/etc/athena/
├── config.yaml     # managed config layer (wins over ~/.athena/config.yaml)
└── .env            # managed env layer (wins over ~/.athena/.env + shell)
```
...
The location can be relocated with the `ATHENA_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `ATHENA_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Athena.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export ATHENA_MANAGED_DIR=/opt/org/athena-policy
```

:::warning
A user who can set `ATHENA_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `athena doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/athena/config.yaml` (managed) | `/etc/athena/.env` (managed) |
| 2 | `~/.athena/config.yaml` (user) | `~/.athena/.env` (user) |
...
```bash
athena config        # shows a header naming the managed source + the pinned keys
athena doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Athena refuses and names the source:

```bash
$ athena config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/athena/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `athena config set` / setup will not write
...
sudo tee /etc/athena/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/athena/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/athena
sudo chmod 0644 /etc/athena/config.yaml /etc/athena/.env
```

Changes take effect on the next Athena start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`athena doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Athena as `root`), managed scope is advisory.
