...
athena --tui -c
athena --tui --continue
athena --tui --resume latest

# Resume a specific session by ID or title
athena --tui -r 20260409_000000_aa11bb
athena --tui --resume "my t0p session"
...
```bash
export ATHENA_TUI=1
athena          # now uses the TUI
athena chat     # same
```

Or make it the persistent default in `~/.athena/config.yaml`:
...
With `display.interface: tui`, a bare `athena` (and `athena chat`) launches the TUI. Explicit flags always win — run `athena --cli` to drop back to the classic REPL for a single invocation, or `athena --tui` / `ATHENA_TUI=1` to force the TUI when the config default is `cli`.
...
- **Node.js** ≥ 20 — the TUI runs as a subprocess launched from the Python CLI. `athena doctor` verifies this.
...
Contributors who run `athena --tui --dev` from many worktrees can share one `node_modules` instead of installing per checkout — see [TUI & Desktop from Worktrees](../developer-guide/worktree-ui-dev.md).
:::
...
```bash
export ATHENA_TUI_DIR=/path/to/prebuilt/ui-tui
athena --tui
```
...
| `/reload` | Re-reads `~/.athena/.env` into the running TUI process so newly added API keys take effect without a restart |
...
Use the live session switcher when you want one terminal to act as a dispatcher for several TUI sessions. It lists only sessions that are currently live in this TUI process; closed sessions remain saved transcripts and can still be reopened with `/resume` or `athena --tui --resume <id-or-title>`.
...
<img alt="Athena TUI Session Orchestrator with one live session and a +new row" src="/docs/img/docs/tui-session-orchestrator/session-orchestrator.png" />

<video controls muted loop playsInline src="/docs/img/docs/tui-session-orchestrator/session-orchestrator-demo.mp4" title="Athena TUI Session Orchestrator demo" style={{maxWidth: '100%'}}></video>
...
1. `ATHENA_TUI_THEME` env var — highest priority. Values: `light`, `dark`, or a raw 6-char background hex (e.g. `ffffff`, `1a1a2e`).
...
By default, `athena --tui` starts a fresh session each launch. To re-attach to the most recent TUI session automatically (useful when your terminal or SSH connection drops unexpectedly), opt in:

```bash
export ATHENA_TUI_RESUME=1          # most-recent TUI session
# or:
export ATHENA_TUI_RESUME=<session-id>   # specific session
```
...
- **`⚠ YOLO`** — visible warning whenever YOLO mode is on (`athena --yolo`, `/yolo`, or `ATHENA_YOLO_MODE=1`). The same badge also appears in the startup banner so you cannot launch an auto-approving session without noticing.

## Configuration

The TUI respects all standard Athena config: `~/.athena/config.yaml`, profiles, personalities, skins, quick commands, credential pools, memory providers, tool/skill enablement. No TUI-specific config file exists.
...
Sessions are shared between the TUI and the classic CLI — both write to the same `~/.athena/state.db`. You can start a session in one, resume in the other. The session picker surfaces sessions from both sources, with a source tag.
...
You may see a `ATHENA_TUI_GATEWAY_URL` env var referenced in the codebase or logs. This is an **internal wiring detail of the web dashboard**, not a user-facing remote-attach knob. When you open the dashboard's "Chat" tab (`athena dashboard` → `/chat`), the dashboard's web server spawns an embedded TUI child process and injects `ATHENA_TUI_GATEWAY_URL` so that child attaches to the dashboard's own in-process `tui_gateway` over a loopback WebSocket (`/api/ws`). The `/api/ws` endpoint exists only inside the dashboard server (`athena_cli/web_server.py`) and is bound to that process's lifetime and auth.

There is no general "point any TUI at any standalone gateway port" mode. In particular, the OpenAI-compatible API server (`athena gateway` / the `api_server` platform) does **not** serve `/api/ws` — it's the model-backend surface (`/v1/chat/completions`, `/v1/models`, …) and deliberately does not expose the TUI's JSON-RPC control channel. Setting `ATHENA_TUI_GATEWAY_URL` to that port will 404.

If you want multiple surfaces to share one set of sessions, use the shared `~/.athena/state.db` (see [Sessions](sessions.md)) or the web dashboard's embedded chat (see [Web Dashboard](features/web-dashboard.md#chat)) — not a hand-set gateway URL.

## Reverting to the classic CLI

Launching `athena` (without `--tui`) stays on the classic CLI by default. To make a machine prefer the TUI, set `display.interface: tui` in `~/.athena/config.yaml` (persistent) or `ATHENA_TUI=1` in your shell profile (per-shell). To go back, set `interface: cli` / unset the env var, or pass `athena --cli` for a one-off.
