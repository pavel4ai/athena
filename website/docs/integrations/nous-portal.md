...
sidebar_position: 1
title: "Nous Portal"
description: "One subscription, 300+ frontier models, and the Tool Gateway — the recommended way to run Athena Agent"
---
...
| **Athena** | Hermes-4-70B, Hermes-4-405B (chat, see [note below](#a-note-on-hermes-4)) |
...
OAuth needs a browser, but the loopback callback runs on the machine where Athena is running. For remote hosts, see [OAuth over SSH / Remote Hosts](/guides/oauth-over-ssh) — the same patterns work for the Portal as for any other OAuth-based provider (`ssh -L` port forwarding).
...
The Tool Gateway settings live under their respective tool sections — each category has a single selection key, and picking **Nous Subscription** in `athena tools` (or `athena setup --portal`) writes the value `nous`:
