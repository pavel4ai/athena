...
Athena supports multiple AI inference providers out of the box. Use `athena model` to configure interactively, or set them in `config.yaml`.

- **[AI Providers](/integrations/providers)** — OpenRouter, Anthropic, OpenAI, Google, and any OpenAI-compatible endpoint. Athena auto-detects capabilities like vision, streaming, and tool use per provider.
...
The `web_search` and `web_extract` tools support eight backend providers, configured via `config.yaml` or `athena tools`:
...
| **Slack** | [api.slack.com/apps?new_app=1](https://api.slack.com/apps?new_app=1) | The **Create New App** dialog — pick *From an app manifest* and paste the manifest `athena slack manifest --agent-view` generates |
...
- **[Buzz](/integrations/buzz)** — Block's Nostr-based human+agent workspace. Three integration paths: Buzz Desktop spawns Athena as a managed ACP runtime, the `buzz-acp` relay bridge hosts a Athena identity server-side, or the native gateway platform joins Buzz channels with full Athena memory/skills/approvals/cron. The overview page compares all three.
...
- **[Plugin System](/user-guide/features/plugins)** — Extend Athena with custom tools, lifecycle hooks, and CLI commands without modifying core code. Plugins are discovered from `~/.athena/plugins/`, project-local `.athena/plugins/`, and pip-installed entry points.
- **[Build a Plugin](/developer-guide/plugins)** — Step-by-step guide for creating Athena plugins with tools, hooks, and CLI commands.
