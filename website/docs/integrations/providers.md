...
You need at least one way to connect to an LLM. Use `athena model` to switch providers and models interactively, or configure directly:

| Provider | Setup |
|----------|-------|
| **Nous Portal** | `athena model` (OAuth, subscription-based) |
| **OpenAI Codex** | `athena model` → **ChatGPT or Codex Subscription** (ChatGPT OAuth, uses Codex models) |
| **GitHub Copilot** | `athena model` (OAuth device code flow, `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, or `gh auth token`) |
| **GitHub Copilot ACP** | `athena model` (spawns local `copilot --acp --stdio`) |
| **Anthropic** | `athena model` (Claude Max + extra usage credits via OAuth; also supports Anthropic API key or manual setup-token — see note below) |
| **OpenRouter** | `OPENROUTER_API_KEY` in `~/.athena/.env` |
| **Ramp Router** | `RAMP_ROUTER_API_KEY` in `~/.athena/.env` (provider: `router`; aliases: `ramp-router`, `ramp`, `router.com`; Responses-native gateway, live account-scoped catalog) |
| **Fireworks AI** | `FIREWORKS_API_KEY` in `~/.athena/.env` (provider: `fireworks`; aliases: `fireworks-ai`, `fw`) |
| **NovitaAI** | `NOVITA_API_KEY` in `~/.athena/.env` (provider: `novita`, 200+ models, Model API, Agent Sandbox, GPU Cloud) |
| **AI Gateway** | `AI_GATEWAY_API_KEY` in `~/.athena/.env` (provider: `ai-gateway`) |
| **z.ai / GLM** | `GLM_API_KEY` in `~/.athena/.env` (provider: `zai`) |
| **Kimi / Moonshot** | `KIMI_API_KEY` in `~/.athena/.env` (provider: `kimi-coding`) |
| **Kimi / Moonshot (China)** | `KIMI_CN_API_KEY` in `~/.athena/.env` (provider: `kimi-coding-cn`; aliases: `kimi-cn`, `moonshot-cn`) |
| **Arcee AI** | `ARCEEAI_API_KEY` in `~/.athena/.env` (provider: `arcee`; aliases: `arcee-ai`, `arceeai`) |
| **GMI Cloud** | `GMI_API_KEY` in `~/.athena/.env` (provider: `gmi`; aliases: `gmi-cloud`, `gmicloud`) |
| **Nebius Token Factory** | `NEBIUS_API_KEY` in `~/.athena/.env` (provider: `nebius-token-factory`; aliases: `nebius`, `nebius-tf`, `tokenfactory`) |
| **Actual Computer** | `ACTUAL_API_KEY` in `~/.athena/.env` for the hosted relay; set `model.base_url` in `config.yaml` for a local daemon (no key needed on loopback). Provider: `actual`; aliases: `actual-computer`, `actualcomputer`, `aci`. |
| **MiniMax** | `MINIMAX_API_KEY` in `~/.athena/.env` (provider: `minimax`) |
| **MiniMax China** | `MINIMAX_CN_API_KEY` in `~/.athena/.env` (provider: `minimax-cn`) |
| **xAI (Grok) — Responses API** | `XAI_API_KEY` in `~/.athena/.env` (provider: `xai`) |
| **xAI Grok OAuth (SuperGrok)** | `athena model` → "xAI Grok OAuth (SuperGrok / Premium+)" — browser login, no API key. See [guide](../guides/xai-grok-oauth.md) |
| **Qwen Cloud (Alibaba DashScope)** | `DASHSCOPE_API_KEY` in `~/.athena/.env` (provider: `alibaba`; mainland-China endpoint: `alibaba-cn`) |
...
| **Alibaba Cloud (Token Plan)** | `ALIBABA_TOKEN_PLAN_API_KEY` in `~/.athena/.env` (provider: `alibaba-token-plan`; mainland-China endpoint: `alibaba-token-plan-cn` with `ALIBABA_TOKEN_PLAN_CN_API_KEY`, falling back to the shared key) — Model Studio flat-token tier |
| **Kilo Code** | `KILOCODE_API_KEY` in `~/.athena/.env` (provider: `kilocode`) |
| **Xiaomi MiMo** | `XIAOMI_API_KEY` in `~/.athena/.env` (provider: `xiaomi`, aliases: `mimo`, `xiaomi-mimo`) |
| **Tencent TokenHub** | `TOKENHUB_API_KEY` in `~/.athena/.env` (provider: `tencent-tokenhub`, aliases: `tencent`, `tokenhub`, `tencentmaas`) |
| **Tencent TokenPlan** | `TOKENPLAN_API_KEY` in `~/.athena/.env` (provider: `tencent-tokenplan`, aliases: `tokenplan`, `tencent-lkeap`; Anthropic Messages endpoint) |
| **OpenCode Zen** | `OPENCODE_ZEN_API_KEY` in `~/.athena/.env` (provider: `opencode-zen`) |
| **CommandCode** | `COMMANDCODE_API_KEY` in `~/.athena/.env` (provider: `commandcode`, alias: `commandcode-chat`; Claude models via `commandcode-anthropic`, alias: `commandcode-claude`). Works with GOAT/Pro/Max/Provider plans (not the $1 Go plan — no API access). |
| **OpenCode Go** | `OPENCODE_GO_API_KEY` in `~/.athena/.env` (provider: `opencode-go`) |
| **OpenCode Free** | Keyless — no API key or account needed (provider: `opencode-free`, aliases: `free`, `opencode_free`). Select via `athena model` or `/model free`; requests are sent anonymously. The model list refreshes automatically from OpenCode's live catalog, so rotating free promotions appear (and delisted ones disappear) without a Athena update |
| **DeepSeek** | `DEEPSEEK_API_KEY` in `~/.athena/.env` (provider: `deepseek`) |
| **Hugging Face** | `HF_TOKEN` in `~/.athena/.env` (provider: `huggingface`, aliases: `hf`) |
| **Google / Gemini** | `GOOGLE_API_KEY` (or `GEMINI_API_KEY`) in `~/.athena/.env` (provider: `gemini`) |
| **Google Vertex AI** | `athena model` → "Google Vertex AI" (provider: `vertex`; OAuth2 via service-account JSON or ADC, GCP billing) |
| **OpenAI API (direct)** | `OPENAI_API_KEY` in `~/.athena/.env` (provider: `openai-api`, optional `OPENAI_BASE_URL`) |
| **Azure AI Foundry** | `athena model` → "Azure AI Foundry" (provider: `azure-foundry`; uses Azure OpenAI / Foundry endpoint and key) |
| **AWS Bedrock** | `athena model` → "AWS Bedrock" (provider: `bedrock`; standard AWS credentials chain via boto3) |
| **NVIDIA Build** | `NVIDIA_API_KEY` in `~/.athena/.env` (provider: `nvidia`; NIM-hosted models on build.nvidia.com) |
| **Ollama Cloud** | `athena model` → "Ollama Cloud" (provider: `ollama-cloud`; cloud-hosted Ollama API) |
| **Qwen OAuth** | `athena model` → "Qwen OAuth" (provider: `qwen-oauth`; browser PKCE login) |
| **MiniMax OAuth** | `athena model` → "MiniMax (OAuth)" (provider: `minimax-oauth`; browser PKCE login) |
| **StepFun** | `STEPFUN_API_KEY` in `~/.athena/.env` (provider: `stepfun`) |
| **LM Studio** | `athena model` → "LM Studio" (provider: `lmstudio`, optional `LM_API_KEY`) |
| **Custom Endpoint** | `athena model` → choose "Custom endpoint" (saved in `config.yaml`) |

All three OpenCode providers send an opaque, per-conversation `x-opencode-session` header on every request (main turns on every transport plus auxiliary calls such as compression and titles). OpenCode uses it to pin a conversation to one backend so its prompt cache stays warm; the value is derived from the Athena session id and carries no personal data.
...
```bash
athena setup --portal     # fresh install — OAuth + provider + gateway in one command
athena model              # existing install — pick "Nous Portal" from the list
athena portal info        # inspect login + routing at any time
```
...
**For full details:** see the dedicated [Nous Portal integration page](/integrations/nous-portal) (what's in the subscription, model catalog, troubleshooting) and the step-by-step [Run Athena Agent with Nous Portal guide](/guides/run-athena-with-nous-portal).

**Client identification.** Every Portal request from Athena Agent carries a `client=athena-client-v<version>` tag (e.g. `client=athena-client-v0.13.0`) auto-aligned to your installed release. This is sent on all Portal pathways — main chat loop, auxiliary calls, compression summarizer, web extraction — and lets Portal-side telemetry distinguish Athena traffic from other clients. No config required; the tag updates automatically when you `athena update`.
...
:::info Codex Note
The OpenAI Codex provider authenticates via device code (open a URL, enter a code). Athena stores the resulting credentials in its own auth store under `~/.athena/auth.json` and can import existing Codex CLI credentials from `~/.codex/auth.json` when present. No Codex CLI installation is required.

If a token refresh fails with a terminal error (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Athena marks the refresh token as dead and stops replaying it so you don't see a flood of identical auth failures. The next request surfaces a typed re-auth message instead. Run `athena auth add openai-codex` (or `athena model` → **ChatGPT or Codex Subscription**) to start a fresh device-code login; the quarantine clears on the next successful exchange.

Device login can fail with `[SSL: UNEXPECTED_EOF_WHILE_READING]` or a TLS handshake timeout on Python/OpenSSL 3.5+ when a middlebox rejects post-quantum groups such as X25519MLKEM768 (curl may still work). Athena does not change default TLS policy. Point `OPENSSL_CONF` at a config that restricts `Groups` to classic curves before running `athena model`, or diagnose with TLS 1.2:
...
|---------|-------------|--------------|
| **`athena model`** | Your terminal (outside any session) | Full setup wizard — add providers, run OAuth, enter API keys, configure endpoints |
| **`/model`** | Inside a Athena chat session | Quick switch between **already-configured** providers and models |

If you're trying to switch to a provider you haven't set up yet (e.g. you only have OpenRouter configured and want to use Anthropic), you need `athena model`, not `/model`. Exit your session first (`Ctrl+C` or `/quit`), run `athena model`, complete the provider setup, then start a new session.


### Subscription plans: what your plan pays for

Several providers let you sign in to Athena with a **consumer subscription** (Claude Max, ChatGPT, SuperGrok / X Premium+, …) instead of an API key. What that subscription actually pays for — and what it doesn't — differs per provider, and it's the single most common source of billing surprises. The table below is the short version; each provider's own section has the details.

> Cells marked *not currently documented* mean exactly that: Athena docs do not yet specify the behavior. Don't assume — check your provider's billing dashboard, and treat these as open questions.

| Plan / path | Can Athena use it? | What gets consumed | What does NOT get consumed | Common surprise |
|---|---|---|---|---|
| **Anthropic — Claude Max + OAuth** | ✅ Yes — `athena model` → Anthropic OAuth. Requires Max **and** purchased extra usage credits | The **extra/overage credits** you've added on top of the Max plan | The **base Max plan allowance** (the usage included in Claude Code by default) | All Athena usage bills as "extra usage" even while your included Max allowance sits untouched |
...
| **OpenAI Codex — ChatGPT plan OAuth** | ✅ Yes — `athena model` → **ChatGPT or Codex Subscription** (ChatGPT OAuth device-code login, uses Codex models) | *Not currently documented* | *Not currently documented* | Docs cover auth and token refresh only; plan-quota semantics are not yet documented |
...
| **Google — Gemini consumer plan (Google AI Pro / Ultra)** | ❌ No documented path — the `gemini` provider is API-key only (`GOOGLE_API_KEY` / `GEMINI_API_KEY`); Vertex AI uses GCP billing | Your **API key's quota** (free tier or billing-enabled Google Cloud project) — *consumer-plan consumption not currently documented* | *Not currently documented* | Free-tier keys can be exhausted after a handful of agent turns, because Athena may make several model calls per user turn |

**Anthropic.** The OAuth path routes as Claude Code against your Anthropic account and **only works on a Claude Max plan with purchased extra usage credits** — the base Max allowance is never consumed by Athena, only the extra/overage credits on top. Claude Pro subscribers cannot use this path; the supported alternative is an `ANTHROPIC_API_KEY`, billed pay-per-token against that key's organization at standard API pricing. See [Anthropic (Native)](#anthropic-native) below.

**OpenAI Codex.** Athena authenticates via ChatGPT device-code OAuth, stores credentials in `~/.athena/auth.json`, and can import existing Codex CLI credentials from `~/.codex/auth.json`. Which ChatGPT plan tiers are eligible, and how Athena usage counts against your plan's Codex limits, are **not currently documented** — the Codex note under [Nous Portal](#nous-portal) covers authentication and token-refresh behavior only.
...
When no explicit environment credential is selected, Athena-owned OAuth grants
...
When you authenticate via `athena model` → Anthropic OAuth (or via `athena auth add anthropic --type oauth`), Athena routes as Claude Code against your Anthropic account. **It only works if you're on a Claude Max plan and have purchased extra usage credits.** The base Max plan allowance (the usage included in Claude Code by default) is not consumed by Athena — only the extra/overage credits you've added on top are. Claude Pro subscribers cannot use this path.
...
athena chat --provider anthropic --model claude-sonnet-4-6

# Preferred: authenticate through `athena model`
# Athena will use Claude Code's credential store directly when available
athena model

# Manual override with a setup-token (fallback / legacy)
export ANTHROPIC_TOKEN=***  # setup-token or manual OAuth token
athena chat --provider anthropic

# Auto-detect Claude Code credentials (if you already use Claude Code)
athena chat --provider anthropic  # reads Claude Code credential files automatically
```

When you choose Anthropic OAuth through `athena model`, Athena prefers Claude Code's own credential store over copying the token into `~/.athena/.env`. That keeps refreshable Claude credentials refreshable.
...
```bash
athena chat --provider copilot --model gpt-5.4
```
...
If no token is found, `athena model` offers an **OAuth device code login** — the same flow used by the Copilot CLI and opencode.
...
|------|--------|------------|
| OAuth token | `gho_` | `athena model` → GitHub Copilot → Login with GitHub |
...
If your `gh auth token` returns a `ghp_*` token, use `athena model` to authenticate via OAuth instead.
:::
...
```bash
athena chat --provider copilot-acp --model copilot-acp
...
| `ATHENA_COPILOT_ACP_COMMAND` | Override the Copilot CLI binary path (default: `copilot`) |
| `ATHENA_COPILOT_ACP_ARGS` | Override ACP args (default: `--acp --stdio`) |
...
```bash
# Fireworks AI
athena chat --provider fireworks --model accounts/fireworks/models/kimi-k2p6
# Requires: FIREWORKS_API_KEY in ~/.athena/.env

# NovitaAI Model API
athena chat --provider novita --model moonshotai/kimi-k2.5
# Requires: NOVITA_API_KEY in ~/.athena/.env

# Ramp Router (model IDs come from your account's live catalog)
athena chat --provider router --model gpt-5.4-mini
# Requires: RAMP_ROUTER_API_KEY in ~/.athena/.env

# z.ai / ZhipuAI GLM
athena chat --provider zai --model glm-5
# Requires: GLM_API_KEY in ~/.athena/.env

# Kimi / Moonshot AI (international: api.moonshot.ai)
athena chat --provider kimi-coding --model kimi-for-coding
# Requires: KIMI_API_KEY in ~/.athena/.env

# Kimi / Moonshot AI (China: api.moonshot.cn)
athena chat --provider kimi-coding-cn --model kimi-k2.5
# Requires: KIMI_CN_API_KEY in ~/.athena/.env

# MiniMax (global endpoint)
athena chat --provider minimax --model MiniMax-M2.7
# Requires: MINIMAX_API_KEY in ~/.athena/.env

# MiniMax (China endpoint)
athena chat --provider minimax-cn --model MiniMax-M2.7
# Requires: MINIMAX_CN_API_KEY in ~/.athena/.env

# Qwen Cloud / DashScope (Qwen models)
athena chat --provider alibaba --model qwen3.5-plus
# Requires: DASHSCOPE_API_KEY in ~/.athena/.env

# Xiaomi MiMo
athena chat --provider xiaomi --model mimo-v2-pro
# Requires: XIAOMI_API_KEY in ~/.athena/.env

# Tencent TokenHub (Hy4 preview)
athena chat --provider tencent-tokenhub --model hy4-preview
# Requires: TOKENHUB_API_KEY in ~/.athena/.env

# Tencent TokenPlan (Hy4 preview via Anthropic Messages endpoint)
athena chat --provider tencent-tokenplan --model hy4-preview
# Requires: TOKENPLAN_API_KEY in ~/.athena/.env

# Arcee AI (Trinity models)
athena chat --provider arcee --model trinity-large-thinking
# Requires: ARCEEAI_API_KEY in ~/.athena/.env

# Meta Model API (Muse Spark family)
athena chat --provider meta-ai --model muse-spark-1.2
# Requires: MODEL_API_KEY in ~/.athena/.env

# GMI Cloud
# Use the exact model ID returned by GMI's /v1/models endpoint.
athena chat --provider gmi --model zai-org/GLM-5.1-FP8
# Requires: GMI_API_KEY in ~/.athena/.env

# Nebius Token Factory
athena chat --provider nebius --model deepseek-ai/DeepSeek-V4-Pro
# Requires: NEBIUS_API_KEY in ~/.athena/.env
```

Fireworks uses its native slash-form catalog IDs, such as `accounts/fireworks/models/kimi-k2p6`. Run `athena model`, choose **Fireworks AI**, and select from the live catalog or enter another Fireworks model ID. The default endpoint is `https://api.fireworks.ai/inference/v1`; configure a different endpoint through `model.base_url` in `config.yaml`, not `.env`.
...
xAI is wired through the Responses API (`codex_responses` transport) for automatic reasoning support on Grok 4 models — no `reasoning_effort` parameter needed, the server reasons by default. Set `XAI_API_KEY` in `~/.athena/.env` and pick xAI in `athena model`, or drop `grok` as a shortcut into `/model grok-4-fast-reasoning`.

SuperGrok and X Premium+ subscribers can sign in with browser OAuth instead of using an API key — pick **xAI Grok OAuth (SuperGrok / Premium+)** in `athena model`, or run `athena auth add xai-oauth`. The same OAuth bearer token is automatically reused by direct-to-xAI tools (TTS, image gen, video gen, transcription). See the [xAI Grok OAuth guide](../guides/xai-grok-oauth.md) for the full flow — and if Athena runs on a remote host, also see [OAuth over SSH / Remote Hosts](../guides/oauth-over-ssh.md) for the required `ssh -L` tunnel.
...
xAI also ships a dedicated TTS endpoint (`/v1/tts`). Select **xAI TTS** in `athena tools` → Voice & TTS, or see the [Voice & TTS](../user-guide/features/tts.md#text-to-speech) page for config.

**Retired xAI model migration (May 15, 2026):** xAI is retiring `grok-4*`, `grok-3`, `grok-code-fast-1`, and `grok-imagine-image-pro` on 2026-05-15. `athena doctor` and `athena chat` startup both detect any config still pointing at a retired ref and print the recommended replacement. Use `athena migrate xai` for a one-shot config rewrite — dry-run by default, add `--apply` to write changes (a timestamped copy of the previous config lands in `backups/config/` first).

```bash
athena migrate xai          # preview replacements
athena migrate xai --apply  # rewrite ~/.athena/config.yaml in place
```
...
# Short alias
athena chat --provider novita-ai --model deepseek/deepseek-v3-0324
```
...
[Ollama Cloud](https://ollama.com/cloud) hosts the same open-weight catalog as local Ollama but without the GPU requirement. Pick it in `athena model` as **Ollama Cloud**, paste your API key from [ollama.com/settings/keys](https://ollama.com/settings/keys), and Athena auto-discovers the available models.
...
athena chat --provider bedrock --model us.anthropic.claude-sonnet-4-6

# Or with explicit env vars
AWS_PROFILE=myprofile AWS_REGION=us-east-1 athena chat --provider bedrock --model us.anthropic.claude-sonnet-4-6
```
...
`VERTEX_PROJECT_ID` / `VERTEX_REGION` env vars override the `config.yaml` values. Athena lazy-installs `google-auth` on first use; run `athena setup` if the managed install needs repair. See the [Google Vertex AI guide](/guides/google-vertex) for the full walkthrough, and the [Google Gemini guide](/guides/google-gemini) for the static-API-key AI Studio path instead.

### Qwen Portal (OAuth)

Alibaba's Qwen Portal with browser-based OAuth login. Pick **Qwen OAuth (Portal)** in `athena model`, sign in through the browser, and Athena persists the refresh token.

```bash
athena model
# → pick "Qwen OAuth (Portal)"
# → browser opens; sign in with your Alibaba account
# → confirm — credentials are saved to ~/.athena/auth.json

athena chat   # uses portal.qwen.ai/v1 endpoint
```
...
Set `ATHENA_QWEN_BASE_URL` only if the portal endpoint relocates (default: `https://portal.qwen.ai/v1`).
...
```bash
athena chat --provider alibaba_coding --model qwen3-coder-plus
```
...
MiniMax-M2.7 via browser OAuth login — no API key needed. Pick **MiniMax (OAuth)** in `athena model`, sign in through the browser, and Athena persists the access + refresh tokens. Uses the Anthropic Messages-compatible endpoint (`/anthropic`) under the hood.
...
athena chat   # uses api.minimax.io/anthropic endpoint
```
...
athena chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
# Requires: NVIDIA_API_KEY in ~/.athena/.env

# Local NIM endpoint — override base URL
NVIDIA_BASE_URL=http://localhost:8000/v1 athena chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
```
...
```bash
# GMI Cloud
athena chat --provider gmi --model deepseek-ai/DeepSeek-V3.2
# Requires: GMI_API_KEY in ~/.athena/.env
```
...
- **Local daemon** — on-device at `http://127.0.0.1:8080`, fully offline. No API key needed: Athena detects the loopback base URL and authenticates with an internal placeholder automatically.

```bash
# Hosted relay (ACTUAL_API_KEY in ~/.athena/.env)
athena chat --provider actual --model <model-id-from-your-cluster>

# Local daemon (model.base_url in ~/.athena/config.yaml, no key)
athena chat --provider actual --model <installed-model-name>
```

Store provider settings in `~/.athena/config.yaml`; only the hosted API key belongs in `.env`:
```yaml
model:
  provider: "actual"
  default: "<model-id>"
...
Notes:
- Model IDs come from your cluster's `GET /v1/models` — discover with `athena model` or `curl -s https://api.actual.inc/v1/models -H "Authorization: Bearer $ACTUAL_API_KEY"`.
...
- Small local models: Athena' full default toolset plus the system prompt can exceed a 32k context window, producing an empty-stream error from llama.cpp-family servers. Restrict the toolset (`-t file,web`) or load the model with a larger context. The optional `actual-setup` skill (`athena skills install official/devops/actual-setup`) covers setup and troubleshooting in detail.
...
```bash
# StepFun
athena chat --provider stepfun --model step-3.5-flash
# Requires: STEPFUN_API_KEY in ~/.athena/.env
```
...
```bash
# Use any available model
athena chat --provider huggingface --model Qwen/Qwen3.5-397B-A17B
# Requires: HF_TOKEN in ~/.athena/.env

# Short alias
athena chat --provider hf --model deepseek-ai/DeepSeek-V3.2
```
...
`LLM_MODEL` in `.env` is **removed** — `config.yaml` is the single source of truth for model and endpoint configuration. `OPENAI_BASE_URL` is still honored, but **only** for the `openai-api` provider (it overrides the OpenAI endpoint for direct API-key access). For other providers and custom endpoints, use `athena model` or set `model.base_url` in `config.yaml` directly. If you have stale entries in your `.env`, they are automatically cleared on the next `athena setup` or config migration.
:::
...
:::warning athena model vs /model
**`athena model`** (run from your terminal, outside any chat session) is the **full provider setup wizard**. Use it to add new providers, run OAuth flows, enter API keys, and configure custom endpoints.
...
**To add a new provider:** Exit your session (`Ctrl+C` or `/quit`), run `athena model`, set up the new provider, then start a new session.
:::
...
Supported parsers: `athena` (Qwen 2.5, Athena 2/3), `llama3_json` (Llama 3.x), `mistral`, `deepseek_v3`, `deepseek_v31`, `xlam`, `pythonic`. Without these flags, tool calls won't work — the model will output tool calls as text.

**Qwen reasoning parsers:** Athena preserves structured reasoning metadata such as `reasoning`, `reasoning_content`, and streamed reasoning deltas when OpenAI-compatible servers return them. That metadata is treated as reasoning/thinking trace data, not as a replacement for the assistant's visible answer. For Qwen reasoning models served by vLLM, make sure the final user-visible response still appears in `content`. If `--reasoning-parser qwen3` leaves `content` empty in your deployment, either disable that parser or pass a server-supported request option such as `chat_template_kwargs.enable_thinking: false` through `extra_body`.
...
If responses seem truncated, check the server's generation default and configure it on the server (for example SGLang's `--default-max-tokens`). Athena does not expose an output-token cap setting.
:::
...
Athena preserves the context of an already-loaded LM Studio instance. For an unloaded model in the default explicit mode, Athena omits `context_length` unless you configured one in Athena, so LM Studio can apply its own model setting. Athena then uses only the context length LM Studio reports after loading.
...
If you use LM Studio's Just-In-Time loading / Auto-Evict feature and want LM Studio to manage model loading and eviction from normal chat requests, skip Athena' explicit preload step:

```bash
athena config set model.lmstudio_load_mode jit
```

Set it back to the default explicit preload behavior with:

```bash
athena config set model.lmstudio_load_mode explicit
```
...
| **vLLM** | Add `--enable-auto-tool-choice --tool-call-parser athena` |
...
**Possible causes:**
1. **Low output limit on the server** — configure the server's generation default (for example SGLang's `--default-max-tokens`). Athena does not expose an output-token cap setting. Response length is distinct from the conversation's context window (`context_length`).
...
Then configure Athena with `athena model` → Custom endpoint → `http://localhost:4000/v1`.
...
Then configure Athena with `athena model` → Custom endpoint → `http://localhost:8402/v1` → model name `blockrun/auto`.
...
Configure any of these with `athena model` → Custom endpoint, or in `config.yaml`:
...
Athena no longer reads `model.max_tokens`, `ATHENA_MAX_TOKENS`, provider output-cap
...
`max_tokens`, so Athena supplies an internal value. Bedrock Converse is a separate
...
`athena model` will prompt for context length when configuring a custom endpoint. Leave it blank for auto-detection.
...
providers:
  ...
  work:
    ...
    transport: chat_completions   # set explicitly by `athena model` → Custom Endpoint wizard; auto-detection still happens as a fallback
  anthropic-proxy:
...
`custom_providers:` entries, including `athena model` setup. Helpers run only when
...
:::note Legacy format
Older configs used a top-level `custom_providers:` list instead. It still works — Athena reads both — and `athena update` auto-migrates it to the `providers:` dict (config v12). Field names differ slightly in the dict format: legacy `model` is `default_model`, and legacy `api_mode` is `transport`.
:::
...
The `athena model` → Custom Endpoint wizard now prompts for the API mode explicitly and persists your answer to `config.yaml` (as `transport` on the provider entry). URL-based auto-detection (e.g. `/anthropic` paths → `anthropic_messages`) still happens as a fallback when the field is left blank.
...
You can also select named custom providers from the interactive `athena model` menu.

---

### Cookbook: Together AI, Groq, Perplexity

The cloud providers listed in [Other Compatible Providers](#other-compatible-providers) all speak OpenAI's REST dialect, so they wire up the same way under the `providers:` dict. Three worked recipes follow. Each drops into `~/.athena/config.yaml` and the matching API key goes in `~/.athena/.env`.
...
Together's `/v1/models` endpoint works, so `athena model` can auto-discover available models.
...
:::tip Troubleshooting
- `athena doctor` should print no `Unknown provider` warnings for any of these names after the CLI validator fixes in #15083.
- If a provider's `/v1/models` endpoint is unreachable (Perplexity is the common one), `athena model` will persist the model with a warning rather than hard-reject — see #15136.
...
:::tip
You can switch between providers at any time with `athena model` — no restart required. Your conversation history, memory, and skills carry over regardless of which provider you use.
:::
...
2. Point Athena at your instance (no API key needed):
   ```bash
   athena config set FIRECRAWL_API_URL http://localhost:3002
   ```
...
When using OpenRouter, you can control how requests are routed across providers. Add a `provider_routing` section to `~/.athena/config.yaml`:
...
OpenRouter ships an experimental coding-model router at `openrouter/pareto-code` that auto-routes requests to the cheapest model meeting a coding-quality bar (ranked by [Artificial Analysis](https://artificialanalysis.ai/)). Pick this model and tune the `min_coding_score` knob in `~/.athena/config.yaml`:
...
:::tip
Fallback is configured exclusively through `config.yaml` — or interactively via `athena fallback`. For full details on when it triggers, how the chain advances, and how it interacts with auxiliary tasks and delegation, see [Fallback Providers](/user-guide/features/fallback-providers).
:::
