...
| **OpenAI Codex** | `athena model` (ChatGPT OAuth, uses Codex models) |
...
| **Qwen Cloud (Alibaba DashScope)** | `DASHSCOPE_API_KEY` in `~/.athena/.env` (provider: `alibaba`) |
...
[Nous Portal](https://futurebnd.com) is Futurebound Corp.'s unified subscription gateway and **the recommended way to run Athena Agent**. One OAuth login covers 300+ frontier agentic models (Claude, GPT, Gemini, DeepSeek, Qwen, Kimi, GLM, MiniMax, Grok, ...) plus the [Tool Gateway](/user-guide/features/tool-gateway) (web search, image generation, TTS, browser automation) plus [Nous Chat](https://futurebnd.com) — billed against your Nous subscription instead of separate per-provider accounts.
...
Don't have a subscription yet? Get one at [futurebnd.com](https://futurebnd.com).
...
If a token refresh fails with a terminal error (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Athena marks the refresh token as dead and stops replaying it so you don't see a flood of identical auth failures. The next request surfaces a typed re-auth message instead. Run `athena auth add codex-oauth` (or `athena model` → OpenAI Codex) to start a fresh device-code login; the quarantine clears on the next successful exchange.
:::
...
athena chat --provider tencent-tokenhub --model hy3-preview
...
custom_providers:
  ...
  - name: work
    ...
    api_mode: chat_completions   # set explicitly by `athena model` → Custom Endpoint wizard; auto-detection still happens as a fallback
...
The `athena model` → Custom Endpoint wizard now prompts for `api_mode` explicitly and persists your answer to `config.yaml`. URL-based auto-detection (e.g. `/anthropic` paths → `anthropic_messages`) still happens as a fallback when the field is left blank.
...
The cloud providers listed in [Other Compatible Providers](#other-compatible-providers) all speak OpenAI's REST dialect, so they wire up the same way under `custom_providers:`. Three worked recipes follow. Each drops into `~/.athena/config.yaml` and the matching API key goes in `~/.athena/.env`.
