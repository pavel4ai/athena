...
Athena Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
APT-managed Athena installs are marked with install method `apt`. Athena therefore does not run its Git self-updater against package-owned files; use the package manager instead:

```bash
pkg update
pkg upgrade athena-agent
```

Packaging/repository/signing problems for this option should be reported to the community packaging repositories above. Athena runtime bugs can still be reported here, keeping in mind that Android/Termux support is best-effort.
...
- links `athena` into `$PREFIX/bin` so it stays on your Termux PATH
...
Athena requires **Python >=3.11,&lt;3.14**. Current Termux ships `python`
...
### 5. Put `athena` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/athena" "$PREFIX/bin/athena"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `athena` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.athena/.env`.
...
### `athena doctor` says ripgrep or Node is missing
