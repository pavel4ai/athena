...
|---|---|---|
| I just want Athena working on my machine | `athena setup` | Run a real chat and verify it responds |
| I already know my provider | `athena model` | Save the config, then start chatting |
| I want a bot or always-on setup | `athena gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `athena model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `athena model` first | Add routing and fallback only after the base chat works |
...
The single most important setup step. Use `athena model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `athena setup` offers three modes:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `athena update`. Re-enable anything later with `athena tools`, seed skills with `athena skills opt-in --sync`, or tune settings with `athena setup agent`.
:::
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `athena model` |
| **OpenAI Codex** | ChatGPT or Codex subscription, uses Codex models | Device code auth via `athena model` → **ChatGPT or Codex Subscription** |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `athena model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `athena_cli/models.py` may change between releases) | `athena model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `athena model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `athena model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `athena model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `athena model` (requires `copilot` CLI + `copilot login`) |
...
:::tip
You can switch providers at any time with `athena model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::
...
- **Secrets and tokens** → `~/.athena/.env`
- **Non-secret settings** → `~/.athena/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
athena config set model anthropic/claude-opus-4.6
...
```bash
athena            # classic CLI
athena --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Athena ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `athena` vs `athena --tui`.
:::
...
```bash
athena --continue    # Resume the most recent session
athena -c            # Short form
```
...
```bash
athena gateway setup    # Interactive platform configuration
```
...
- `athena tools` — tune tool access per platform
- `athena skills` — browse and install reusable workflows
...
```bash
athena config set terminal.backend docker    # Docker isolation
athena config set terminal.backend ssh       # Remote server
```

For Docker sandboxes, you can also enable the **egress credential-injection proxy** so the sandbox never sees your real API keys — only opaque proxy tokens that work exclusively from behind a local TLS-intercepting daemon. See [Egress proxy](../user-guide/egress/iron-proxy.md). Setup is `athena egress setup && athena egress start`; `athena setup terminal` also points Docker users at it. Modal, SSH, Daytona, and Singularity are not wired yet.

### Voice mode

```bash
# From the Athena install directory (the curl installer placed it at
# ~/.athena/athena-agent on Linux/macOS or %LOCALAPPDATA%\athena\athena-agent on Windows):
cd ~/.athena/athena-agent
...
Athena ships with a catalog of bundled skills already installed in `~/.athena/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
athena skills browse                      # list everything available
athena skills search kubernetes           # find skills by keyword
athena skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `athena skills browse` shows the exact slugs to use.
...
(If you installed without `[all]`, run `cd ~/.athena/athena-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Athena opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `athena model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `athena gateway setup` and check `athena gateway status` |
| `athena --continue` can't find old session | Switched profiles or session never saved | Check `athena sessions list` and confirm you're in the right profile |
...
| `athena doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `athena model`
3. `athena setup`
4. `athena sessions list`
5. `athena --continue`
6. `athena gateway status`
...
| `athena tools` | Configure which tools are enabled per platform |
| `athena setup` | Full setup wizard (configures everything at once) |
...
- **[Moving to another machine](/reference/faq#exporting-athena-to-another-machine)** — `athena backup` migrates your whole setup (or [a single profile](/reference/faq#moving-a-single-profile-to-another-machine)); no need to rebuild from scratch
