...
Athena Agent ships a Nix flake, a NixOS module, and a Home Manager module.
...
```bash
# Run the desktop app
nix run github:pavel4ai/athena#desktop

# Or install persistently
nix profile install github:pavel4ai/athena#desktop

# run the tui
nix run github:pavel4ai/athena -- setup
nix run github:pavel4ai/athena -- --tui
...
The default package includes ALL libraries athena-agent might need. if you want a smaller variant, check the other flake outputs.
...
:::note
This module needs NixOS. Athena is an agent for one person. If you want an agent for one person and not a system service, use the [Home Manager module](#home-manager-module). That module runs on NixOS and on each other system that Home Manager supports.
:::
...
{ config, ... }: {
  services.athena-agent = {
    ...
    # ── Documents ──────────────────────────────────────────────────────
    # USER.md is memory, so it goes to ATHENA_HOME. Workspace files use
...
| Give the agent an identity | `athenaHomeFiles."SOUL.md"` | `"You are a terse ops assistant."` |
...
Athena reads files from two directories. Thus there are two options. Use the option for the directory that the file must go into.
...
```nix
{
  services.athena-agent = {
    ...
    workingDirectory = "/var/lib/athena/workspace";
    documents = {
...
`athenaHomeFiles` installs into **`ATHENA_HOME`**. Athena reads the identity file and the memory files of the agent from that directory. `SOUL.md` and `memories/` work only from there. A `SOUL.md` in `documents` makes a workspace file. Athena does not load that file as the identity:

```nix
{
  services.athena-agent.athenaHomeFiles = {
...
`athenaHomeFiles` needs no `workingDirectory`, because the module owns the `ATHENA_HOME` directory. Most users want `athenaHomeFiles`.
...
1. **The `ATHENA_MANAGED` environment variable.** The service sets it, and the gateway process reads it.
2. **The `.managed` marker file** in `ATHENA_HOME`. The activation script writes it, and an interactive shell reads it. Thus the CLI also blocks a command such as `docker exec -it athena-agent athena config set ...`.
...
The flake also exports `homeManagerModules.default`. Athena is an agent for one person. The credentials, the memory, the sessions and the cron jobs all belong to that person. Thus a user service is the correct shape on a personal machine. It runs on each distribution that Home Manager supports, and not only on NixOS.

The option set is the same set that the NixOS module uses. It is `services.athena-agent`, with the same `settings`, `environmentFiles`, `documents`, `mcpServers`, `extraPlugins` and `backend` options. Each example above works here without a change. Only the necessary parts are different:

| | NixOS module | Home Manager module |
|---|---|---|
| Runs as | a system user that you declare, with `user`, `group` and `createUser` | you |
| State directory | `stateDir` and `/.athena` | `athenaHome`, set directly. The default is `~/.athena`. |
| Service | `systemd.services` | `systemd.user.services` on Linux, `launchd.agents` on macOS |
| CLI on the PATH | `addToSystemPackages`, which exports `ATHENA_HOME` for the full system | `programs.athena-agent.enable`, which exports it for your session only |
| Desktop application | not supported, because a system service cannot own a user session | `programs.athena-agent.desktop.enable` |
...
```nix
{
  imports = [ athena-agent.homeManagerModules.default ];
...
`home-manager switch` makes `~/.athena`, writes `config.yaml`, builds `.env` and starts the gateway as a user service.
...
`gateway.enable` runs the messaging gateway for Telegram, Discord, Slack and the other platforms. Athena Desktop and the web dashboard connect to a *different* process, which is `athena serve` or `athena dashboard`. `backend.mode` runs that process with the gateway:
...
`serve` runs without a user interface. It gives the `/api/ws` and `/api/pty` sockets that Athena Desktop connects to, and it does not build the web application. `dashboard` gives all of that, and also serves the browser admin panel. Both processes use one `ATHENA_HOME` with the gateway. Thus the sessions, the skills, the memory and the cron jobs are the same for all of them. `backend.mode` works in the same way on the NixOS module, but not in container mode.
...
```bash
# Linux
systemctl --user status athena-agent
journalctl --user -u athena-agent -f

# macOS
launchctl list | grep athena
tail -f ~/Library/Logs/athena-agent.log

athena --version
athena config     # shows the configuration that Nix wrote
```
...
| `athenaHomeFiles` | `attrsOf (either str path)` | `{}` | Files that go into `ATHENA_HOME`. `SOUL.md` and `memories/` must be here, or Athena does not load them. |
...
### Backend (`athena serve` / `athena dashboard`)

This option runs the process that Athena Desktop and the web dashboard connect to, with the gateway. You cannot use it with `container.enable`.
...
| Option | Type | Default | Description |
|---|---|---|---|
| `athenaHome` | `str` | `"${config.home.homeDirectory}/.athena"` | `ATHENA_HOME` directly. The NixOS module builds it from `stateDir`. |
...
### `programs.athena-agent` (Home Manager only)

Home Manager separates "install this application for me" from "run this
daemon". `services.athena-agent` keeps the state, the configuration and the
daemons. `programs.athena-agent` installs what you use, and reads
`athenaHome` and the backend address from the services.

| Option | Type | Default | Description |
|---|---|---|---|
| `enable` | `bool` | `false` | Add the `athena` CLI to `home.packages`, and export `ATHENA_HOME` for your shells |
| `package` | `package` | `services.athena-agent.package` | The package to install. The default applies `extraPythonPackages` and `extraDependencyGroups` from the services, so both are one build. |
| `desktop.enable` | `bool` | `false` | Add the Athena Desktop application, with a launcher entry on Linux |
| `desktop.package` | `package` | `package.athenaDesktop` | The desktop package. The default follows `package`, so the application and the services run one Athena runtime. |

```nix
programs.athena-agent = {
  enable = true;
  desktop.enable = true;
};
...
services.athena-agent = {
  enable = true;
  backend.mode = "serve";
  backend.sessionTokenFile = config.sops.secrets."athena/desktop-token".path;
};
```

The launcher carries `ATHENA_HOME` itself. A desktop menu reads no shell
profile, so the value that `programs.athena-agent.enable` exports with
`home.sessionVariables` reaches an interactive shell only. Without the
value in the launcher, the application opens `~/.athena` while the
services use `athenaHome`, and you see no sessions and no keys.
...
`services.athena-agent.installPackage` was removed by this split. A
...
├── .athena/                         # ATHENA_HOME
│   ├── SOUL.md                      # from athenaHomeFiles: the agent identity
...
```
~/.athena/                           # athenaHome (ATHENA_HOME), 0700
├── SOUL.md                          # from athenaHomeFiles
...
├── auth.json                        # OAuth credentials: seeded, then Athena owns it
