...
Top-level command for managing profiles. Running `athena profile` without a subcommand shows help.
...
```bash
athena profile use <name>
```

Sets `<name>` as the active profile. All subsequent `athena` commands (without `-p`) will use this profile.
...
athena profile use default
```

## `athena profile create`

```bash
athena profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `athena profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `athena update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.athena`), use `athena skills opt-out` / `athena skills opt-in`. |
...
athena profile create work2 --clone-from work

# Clone everything from a specific profile
athena profile create work2-backup --clone-from work --clone-all
```

## `athena profile describe`

```bash
athena profile describe [<name>] [options]
```
...
athena profile describe researcher

# Set it explicitly
athena profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
athena profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
athena profile describe --all --auto
```

## `athena profile delete`

```bash
athena profile delete <name> [options]
```
...
```bash
athena profile delete mybot
athena profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.athena`) cannot be deleted — use `athena uninstall` to remove everything.
:::

## `athena profile show`

```bash
athena profile show <name>
```
...
```bash
$ athena profile show work
Profile: work
Path:    ~/.athena/profiles/work
...
## `athena profile alias`

```bash
athena profile alias <name> [options]
```
...
```bash
athena profile alias work
# Creates/updates ~/.local/bin/work

athena profile alias work --name mywork
# Creates ~/.local/bin/mywork

athena profile alias work --remove
# Removes the wrapper script
```

## `athena profile rename`

```bash
athena profile rename <old-name> <new-name>
```
...
```bash
athena profile rename mybot assistant
# ~/.athena/profiles/mybot → ~/.athena/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `athena profile export`

```bash
athena profile export <name> [options]
```
...
```bash
athena profile export work
# Creates work.tar.gz in the current directory

athena profile export work -o ./work-2026-03-29.tar.gz
```
...
## `athena profile import`

```bash
athena profile import <archive> [options]
```
...
```bash
athena profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

athena profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `athena -p telemetry`). |
...
athena profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
athena profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
athena profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
athena profile install ./telemetry/
```

### `athena profile update`

```bash
athena profile update <name> [--force-config] [--yes]
```
...
`athena profile list` also shows the distribution name and version in a
`Distribution` column, and `athena profile show <name>` / `delete <name>`
...
athena profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
athena profile install https://github.com/your-org/internal-assistant.git
```
...
`athena_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
...
3. Tell recipients to run `athena profile install <your-repo-url>`.
...
## `athena -p` / `athena --profile`

```bash
athena -p <name> <command> [options]
athena --profile <name> <command> [options]
```
...
```bash
athena -p work chat -q "Check the server status"
athena --profile dev gateway start
athena -p personal skills list
athena -p work config edit
```

## `athena completion`

```bash
athena completion <shell>
```
...
- `athena profile <TAB>` — subcommands (list, use, create, etc.)
- `athena profile use <TAB>` — profile names
- `athena -p <TAB>` — profile names
