...
- **Anthropic** — Claude models (direct API, OAuth via `athena auth add anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `athena model` or by editing `~/.athena/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
- [Use MCP with Athena](../guides/use-mcp-with-athena.md#wsl2-bridge-athena-in-wsl-to-windows-chrome)
- [Browser Automation](../user-guide/features/browser.md#wsl2--windows-chrome-prefer-mcp-over-browser-connect)

### Is my data sent anywhere?

API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Athena Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.athena/`.

### Can I use it offline / with local models?

Yes. Run `athena model`, select **Custom endpoint**, and enter your server's URL:
...
#### `athena: command not found` after installation
...
**Solution:** Athena auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.athena/config.yaml`:
...
#### The agent says "Athena policy" or "Athena guardrails" refused my request

A model cannot reliably identify why it refused a request. If the refusal appears only in the assistant's prose, its claim that a hidden Athena runtime policy caused it may be a hallucinated explanation or a restriction applied by the selected model or provider.

Athena enforcement is explicit: a blocked tool action returns a tool error naming the denied command or path, and an approval-required action shows an approval prompt. Athena does not silently turn those execution controls into a general content-refusal layer. Provider-level controls can still apply when configured, such as Amazon Bedrock Guardrails.

To isolate the source:

1. Run `/status` to confirm the active model and provider.
2. Check whether the refusal includes an actual Athena tool error or approval prompt. If it is prose only, do not treat the model's attribution as runtime evidence.
3. Retry in a fresh session with another configured model or provider. A refusal that changes with the model is model/provider behavior, not a Athena execution control.
4. If an explicit tool error appears, use its exact text when reporting the problem.

See [Security](/user-guide/security) for Athena' documented execution controls and [Providers](/integrations/providers) for provider configuration.
...
**Solution:** Exit your session and use `athena model` from your terminal to add new providers:
...
After adding a new provider via `athena model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `athena model` (from terminal) |
| Enter/change API keys | `athena model` (from terminal) |
...
# Or set directly
athena config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.athena/.env` for conflicting entries.
:::
...
# Set a valid model
athena config set model.default anthropic/claude-opus-4.7

# Or specify per-session
athena chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `athena chat --provider <alternative>` to route to a different backend
...
athena chat --model openrouter/google/gemini-3-flash-preview
```
...
- Or switch to the terminal interface for administrative tasks: `athena chat`
...
cat ~/.athena/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `athena gateway setup`
- Check gateway logs: `cat ~/.athena/logs/gateway.log | tail -50`
...
Configure in `~/.athena/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
cd ~/.athena/athena-agent && uv pip install -e ".[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
#### WSL: Gateway keeps disconnecting or `athena gateway start` fails
...
tmux new -s athena 'athena gateway run'
# Reattach later: tmux attach -t athena

# Option 3: Background via nohup
nohup athena gateway run > ~/.athena/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'athena gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `athena gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
athena gateway install    # Re-snapshots your current PATH
athena gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.athena.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `athena chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `athena chat -t "terminal"`
...
If the baseline looks high before you've typed anything, that's the fixed prompt budget — the system prompt plus tool schemas sent on every call. Run [`athena prompt-size`](/reference/cli-commands#athena-prompt-size) to measure it, then trim: disable toolsets you don't use (`athena tools`) and uninstall or disable skills you don't need (`athena skills`).
...
cd ~/.athena/athena-agent && uv pip install -e ".[mcp]"
...
Verify your `~/.athena/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
athena config show | grep -A 12 mcp_servers
...
- [Use MCP with Athena](/guides/use-mcp-with-athena)
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `athena profile create newname --clone-all` to copy everything from the current profile, or add `--clone-from <profile>` to copy from a specific source profile.

This isolation is also the reason to never run two agents against the *same* profile or Athena home: both write memory automatically and each loads the other's writes at session start, so their stored state degrades with every session. One agent per profile; for genuinely shared memory across agents, use an [external memory provider](/user-guide/features/memory-providers).

### What happens when I run `athena update`?

`athena update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `athena update` once — it covers every profile on the machine.


### How many profiles can I run?

There is no hard limit. Each profile is just a directory under `~/.athena/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** Athena can route subagents to a different model automatically. Set this in `~/.athena/config.yaml`:
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `athena skills config` settings don't seem to take effect.

**Solution:** Use `athena skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`athena gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
2. On the **source machine**, create a full backup:
   ...
   athena backup
   ```
   This creates a zip of your entire `~/.athena/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/athena-backup-<timestamp>.zip`.

3. Copy the zip to the new machine and import it:
   ```bash
   # On the source machine
   scp ~/athena-backup-<timestamp>.zip newmachine:~/

   # On the new machine
   athena import ~/athena-backup-<timestamp>.zip
   ```

4. On the new machine, run `athena setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
athena profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
athena profile import ./work-backup.tar.gz work
```
...
### `athena backup` vs `athena profile export`

| Feature | `athena backup` | `athena profile export` |
| :--- | :--- | :--- |
| **Use Case** | **Full machine migration** | **Porting/sharing a specific profile** |
| **Scope** | Global (entire `~/.athena` directory) | Local (single profile directory) |
...
```bash
rsync -av --exclude='athena-agent' ~/.athena/ newmachine:~/.athena/
```

:::tip
`athena backup` produces a consistent snapshot even while Athena is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::
...
athena config show | head -20

# Re-run model selection
athena model

# Or test with a known-good model
athena chat -q "hello" --model anthropic/claude-opus-4.7
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/pavel4ai/athena/issues)
2. **Ask the community:** [Nous Research Discord](https://discord.gg/nousresearch)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Athena version (`athena --version`), and the full error message
