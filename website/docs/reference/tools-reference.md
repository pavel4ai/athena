...
:::tip MCP Tools
In addition to built-in tools, Athena can load tools dynamically from MCP servers. MCP tools appear with the prefix `mcp__<server>__` (e.g., `mcp__github__create_issue` for the `github` MCP server). See [MCP Integration](/user-guide/features/mcp) for configuration.
:::
...
Scoped to the Feishu document-comment intelligent-reply handler (`gateway/platforms/feishu_comment.py`). Not exposed on `athena-cli` or the regular Feishu chat adapter.
...
| Tool | Description | Requires environment |
|------|-------------|----------------------|
| `computer_use` | Background desktop control via cua-driver — screenshots (SOM / vision / AX), click / drag / scroll / type / key / wait, list_apps, focus_app. Does NOT steal the user's cursor or keyboard focus. Works with any tool-capable model. macOS, Windows, and Linux. | `cua-driver` on `$PATH` (install via `athena tools`). |
...
Registered when the agent is either (a) spawned by the kanban dispatcher (`ATHENA_KANBAN_TASK` env set) or (b) running in a profile that explicitly enables the `kanban` toolset. Task-scoped workers use lifecycle tools for their assigned task; orchestrator profiles additionally get board-routing tools like `kanban_list` and `kanban_unblock`. See [Kanban Multi-Agent](/user-guide/features/kanban) for the full workflow.

| Tool | Description | Requires environment |
|------|-------------|----------------------|
| `kanban_show` | Show the active kanban task assigned to this worker (title, description, comments, dependencies). | `ATHENA_KANBAN_TASK` or `kanban` toolset |
...
| `kanban_complete` | Mark the current task done with a structured handoff payload (results, artifacts, follow-ups). | `ATHENA_KANBAN_TASK` or `kanban` toolset |
| `kanban_block` | Block the current task on a question for the user — the dispatcher pauses, surfaces the question, and resumes once a human replies. | `ATHENA_KANBAN_TASK` or `kanban` toolset |
| `kanban_request_review` | Hand the implementation to a reviewer with `summary`, optional structured `metadata`, and an optional reviewer profile. Moves the same task to `review`; it is not a block and does not affect block-loop accounting. | `ATHENA_KANBAN_TASK` or `kanban` toolset |
| `kanban_request_changes` | Reviewer verdict for an actively claimed review run. Closes the review run, reapplies parent gating, and routes the task back to the original implementer without using a block. | `ATHENA_KANBAN_TASK` or `kanban` toolset |
| `kanban_heartbeat` | Send a progress heartbeat during a long-running operation so the dispatcher knows the worker is still alive. | `ATHENA_KANBAN_TASK` or `kanban` toolset |
| `kanban_comment` | Add a comment to the task thread without changing its state — useful for surfacing intermediate findings. | `ATHENA_KANBAN_TASK` or `kanban` toolset |
| `kanban_create` | Fan out child tasks from the current task. Used by orchestrators and follow-up-spawning workers. | `ATHENA_KANBAN_TASK` or `kanban` toolset |
| `kanban_link` | Link tasks with a parent → child dependency edge. | `ATHENA_KANBAN_TASK` or `kanban` toolset |
...
| `kanban_attach` | Attach a file to a task by passing its bytes inline (base64). Stored as a real attachment under the task's attachments dir, capped at 25 MB. | `ATHENA_KANBAN_TASK` or `kanban` toolset |
| `kanban_attach_url` | Attach a file to a task by URL — Athena downloads it server-side and stores it as a real attachment (capped at 25 MB). Only http/https URLs. | `ATHENA_KANBAN_TASK` or `kanban` toolset |
| `kanban_attachments` | List the files attached to a task: id, filename, content_type, size, uploader, and the absolute on-disk path. | `ATHENA_KANBAN_TASK` or `kanban` toolset |
...
| Tool | Description | Requires environment |
|------|-------------|----------------------|
| `skill_manage` | Manage skills (create, update, delete). Skills are your procedural memory — reusable approaches for recurring task types. New skills go to ~/.athena/skills/; existing skills can be modified wherever they live. Actions: create (full SKILL.m… | — |
...
Enabled for sessions whose source is the Athena desktop app, on any backend it
is connected to (local, SSH, URL, or Athena Cloud). Absent from CLI, TUI,
messaging, and cron sessions.

| Tool | Description | Requires environment |
|------|-------------|----------------------|
| `read_terminal` | Read what's currently shown in the in-app terminal pane of the Athena desktop GUI (the embedded shell beside this chat). | — |
| `close_terminal` | Close the read-only terminal tab for a background process in the Athena desktop GUI. Does NOT kill the process — only drops the tab/view; use process(action='kill') to stop it. | — |
| `open_preview` | Open a web URL, localhost dev-server URL, or file path in the preview pane beside the chat in the Athena desktop app. | — |
...
| `read_preview` | Read what's currently shown in the preview pane of the Athena desktop GUI — the in-app Browser's page text (URL + title + rendered text, pageable with `start`/`count`), or a file/artifact tab's identity. | — |
...
| `read_window_below` | Identify the OS window directly underneath the Athena desktop window — app name, title, bounds (metadata only, never pixels). On macOS, other apps' titles appear only when Screen Recording is already granted; the tool never prompts for it. | — |
| `focus_pane` | Reveal and focus a pane in the Athena desktop app (chat, files, terminal, review, sessions). | — |
...
| `tour` | Give a live guided tour: dim the screen, highlight an element, and attach a narrated popover (driver.js). Works on the Athena app's own UI and on any page open in the preview pane; `targets` discovers what's on screen, `show` narrates step-by-step, `start` hands the user Next/Prev controls. | — |
...
only at a genuinely idle moment. A tip from Athena shares that cooldown, so it
...
(`display.in_app_tips`, `display.in_app_tours`). Off covers Athena as well as
...
Opt-in toolset (not loaded in the default `athena-cli` set). Add via `--toolsets video` or include `video` in your `toolsets:` config.
...
Opt-in toolset (not loaded in the default `athena-cli` set). Add via `--toolsets video_gen` or enable it in `athena tools` → Video Generation, which also walks you through picking a backend.
...
Registered on the `athena-discord` platform toolset (gateway only). Uses the same bot token as the messaging adapter.
...
Registered on the `athena-discord` platform toolset. Moderation actions require the bot to hold the matching Discord permissions.
...
Registered by the bundled `spotify` plugin. Requires an OAuth token — run `athena auth spotify` once to authorize.
...
## `athena-yuanbao` toolset

Registered only on the `athena-yuanbao` platform toolset. Yuanbao is Tencent's chat app; these tools drive its DM/group/sticker APIs.
