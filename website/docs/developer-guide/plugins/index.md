...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/user-guide/features/tts#voice-message-transcription-stt) — set `ATHENA_LOCAL_STT_COMMAND` to an argv-tokenized template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.athena/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `athena skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
Athena validates `plugin.json`, Agent Skills frontmatter, fixed component
...
profile-scoped writable directory managed by Athena.
...
Portable `streamable-http` entries are routed through Athena' existing native
...
same full-trust posture as other installed Athena plugins.
...
records it as Published. Athena keys behavior on the canonical v1.0.0 schema
...
not by one global plugin API number. Athena does not expose a
...
continue to work after a normal Athena upgrade.
...
- **Hook payloads are keyword payloads.** New hook data is added as keyword
  ...
  Athena inspects callback signatures: a legacy callback receives the fields it
...
- **Manifests are open to additions.** Unknown `plugin.yaml` fields are ignored.
  Older Athena releases can therefore load a plugin whose manifest contains
...
Athena enforces this contract with frozen external-plugin fixtures discovered
from an isolated `ATHENA_HOME`. Those tests load and invoke the plugin through
...
Athena's internals were split into `<stem>_<topic>` sibling modules in Sep 2026 (PR #102117). **Internal
...
- **Check your plugin:** `athena plugins compat /path/to/your/plugin` lists every `file:line` with the
  old path and the new one, and exits 1 while any remain. `COMPAT_MANIFEST.md` in the repo is the full map.
- **What users see:** a notice under the CLI banner, in `athena doctor` and after `athena update`, and a
  one-time Desktop dialog naming the plugin. Each resolution through an old path also emits a
  `AthenaPluginCompatWarning` once per process.
- **From 2026-09-14:** plugins that still import old paths are **not loaded** (the reason shows in
  `athena plugins list`). Users can force-load with `plugins.allow_deprecated_imports: true` until the
...
```bash
mkdir -p ~/.athena/plugins/calculator
cd ~/.athena/plugins/calculator
```

### Validate with Plugin Doctor

`athena plugins doctor [path-or-id]` runs the same directory discovery,
manifest parser, namespaced import, `register(ctx)`, hook registry, and tool
registry used by Athena itself. It reports invalid hook names, callbacks that do
...
```bash
athena plugins doctor . --ci
```

Doctor uses a temporary `ATHENA_HOME`, restores plugin registration state after
...
`llm.task_override` (see `athena_cli/plugin_capabilities.py` for the
...
`athena_agent.plugin_capabilities` entry-point group. Each declaration is
named `<plugin-id>.<capability-id>` and points at the same object as your
`athena_agent.plugins` entry point:

```toml
[project.entry-points."athena_agent.plugins"]
calculator = "my_pkg:register"

[project.entry-points."athena_agent.plugin_capabilities"]
"calculator.tools.override" = "my_pkg:register"
```

Athena reads these from installed metadata without importing your code, so
`athena plugins capabilities` and the consent flow stay accurate for pip
installs.
...
this Athena understands still loads with a warning.
...
| `python_dependencies` | list of str | Declared pip requirements (e.g. `"requests>=2.0,<3"`). **Declaration seam only** — Athena validates them, and `athena plugins install` / `athena plugins doctor` surface missing ones with a `pip install` hint, but Athena **never auto-installs** them. Pin upper bounds. |
...
arbitrary packages into Athena' shared venv is a conflict and supply-chain
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `athena my-plugin <subcommand>`)
...
Use plugin-relative config keys for user-visible behavior. Athena resolves them
...
`<ATHENA_HOME>/plugin-data/`. Neither API exposes another plugin's namespace.
...
If your plugin doesn't show up — or shows up but isn't loading — set `ATHENA_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
ATHENA_PLUGINS_DEBUG=1 athena plugins list
```
...
The same logs are always written to `~/.athena/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
athena logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `athena plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout:** Native packages use `~/.athena/plugins/<plugin-name>/plugin.yaml` (flat) or one category level. Portable packages use root `plugin.json` in the same locations. Anything deeper is ignored.
...
tree, and `athena plugins update` / `remove` git-pull or delete it — your
...
# <athena home>/plugin-data/<name>/ — created on first use
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.athena/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.athena/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `athena plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
auditable in `~/.athena/logs/agent.log`. Plugins load after built-in
...
does not ship with Athena core (user, project, or pip source),
...
plugins:
  entries:
    my-plugin:                    # the plugin's registry key from `athena plugins list`
...
loader, the plugin is disabled and Athena continues. The gate exists
...
You normally never edit this key by hand. `athena plugins enable <name>`
...
Per-hook context is capped at `10,000` characters by default. Anything above the cap is written to `$ATHENA_HOME/hook_outputs/<session_id>/<uuid>.txt` and replaced with a head/tail preview plus the saved path. The model can read the full content via `read_file` or `terminal` if it genuinely needs it. This keeps a runaway plugin from inflating every subsequent turn's prompt and blowing out the prompt cache prefix. Tune in `config.yaml`:
...
hooks:
  output_spill:
    ...
    # directory: null      # default: $ATHENA_HOME/hook_outputs
```
...
The canonical list of kinds is `VALID_MIDDLEWARE` in `athena_cli/middleware.py`:
...
| `llm_request` | `request`, `original_request`, context kwargs | Return `{"request": {...}}` to replace the effective provider kwargs before Athena sends them. |
...
- Middleware payloads carry `middleware_schema_version` (`athena.middleware.v1`) alongside the observer telemetry fields.
- Unknown kinds register with a warning instead of failing, so a plugin written against a newer Athena still loads on an older one.

### Register CLI commands

Plugins can add their own `athena <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for athena my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: athena my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for athena my-plugin."""
...
After registration, users can run `athena my-plugin status`, `athena my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `athena name` in a terminal |
...
`ctx._cli_ref` is only populated in an **interactive CLI** session. It is `None` in the gateway, in non-interactive `athena chat -q` runs, and in **kanban-spawned worker sessions** — so any plugin logic that reaches through `_cli_ref` silently no-ops in exactly those contexts. Two stable, session-agnostic APIs cover what hooks actually need:

- **`ctx.profile_name`** — the active profile name (e.g. `"default"`, or the assignee profile in a kanban worker). Derived from `ATHENA_HOME`, so it works everywhere with no `_cli_ref` dependency.
...
For running a full `athena <subcommand>` (e.g. `athena kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "athena kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive Athena from a hook.
...
Athena has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.athena/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `athena gateway status` without SDK instantiation.
...
Model Context Protocol (MCP) servers register their own tools into Athena without any Python plugin. Declare them in `~/.athena/config.yaml`:
...
Drop a manifest + handler into `~/.athena/hooks/<name>/`:

```yaml
# ~/.athena/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.athena/hooks/long-task-alert/handler.py
...
athena skills search my-workflow --source myorg/skills-repo
athena skills install myorg/skills-repo/my-workflow
```
...
```bash
pip install athena-plugin-calculator
# Plugin auto-discovered on next athena startup
```
