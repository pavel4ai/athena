...
So ACP advertises and uses the currently configured Athena provider/credentials. It also always advertises a terminal setup auth method (`athena-setup`, args `--setup`) so first-run ACP clients can open Athena' interactive model/provider configuration before starting a normal ACP session.
