...
Configure via `athena plugins` → Provider Plugins → Context Engine, or edit `config.yaml` directly.
...
value is kept per `model@host` in `~/.athena/cache/image_token_costs.json` and
...
compression:
  ...
  codex_app_server_auto: native  # native|athena|off for Codex app-server thread compaction
...
| `codex_app_server_auto` | `native` | `native`, `athena`, `off` | Thread-compaction mode for Codex app-server sessions (see below) |
...
excluded because they already unlock the wider window), Athena raises the
trigger to **85%** (~231K) and shows a notice with the opt-out command. The
notice is shown once per profile — a marker under `$ATHENA_HOME`
...
```bash
athena config set compression.codex_gpt55_autoraise false
```

To keep the 85% autoraise but hide only the one-time notice:

```bash
athena config set compression.codex_gpt55_autoraise_notice false
```
...
for ChatGPT-subscription accounts (live-verified Aug 2026). Athena keeps the
...
`gpt-5.4-900k`). These are Athena-side aliases: the suffix is stripped before
...
thread context, so Athena' auxiliary summarizer cannot shrink it — rewriting the
...
- Automatic compaction is controlled by `compression.codex_app_server_auto`:
  the default `native` lets the app-server decide when to compact and Athena
  records the resulting compaction events (compression counters, session
  events). Set `athena` to let Athena' compression threshold initiate
  app-server compaction, or `off` to disable Athena-initiated automatic
  compaction entirely (codex may still compact natively).

Athena' local transcript is never rewritten on this runtime — state.db records
the compaction boundary while the visible transcript stays intact. All other
routes (including Codex OAuth chat sessions) keep Athena' summary compressor.
...
encrypted `compaction` output item. Athena captures that item into the
...
5. **Model identity is part of the cache key**: Provider-side caches are scoped
   ...
   Athena can avoid; user-facing docs for `/model`, fallback providers, and
