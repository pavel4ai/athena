...
using Athena built-in auxiliary tasks; it does not permit another plugin's
  ...
  entries:
    my-plugin:
      llm:
        ...
        # `athena model` and config.yaml model.provider).
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.athena/auth.json` / env, including the credential pool when
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/pavel4ai/athena/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/pavel4ai/athena/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/NousResearch/athena-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/NousResearch/athena-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
