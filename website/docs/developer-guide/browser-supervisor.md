...
(`athena-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs before
...
- `toolsets.py` — registers `browser_dialog` in `browser`, `athena-acp`, `athena-api-server`, and core toolsets (gated on CDP reachability)
- `athena_cli/config.py` — `browser.dialog_policy` and `browser.dialog_timeout_s` defaults
