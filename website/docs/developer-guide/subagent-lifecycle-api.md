...
Plugins can launch and supervise fresh Athena child sessions without importing
...
active agent turn fails closed with `No active Athena parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Athena can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Athena's existing unsafe-tool block remains enforced.
