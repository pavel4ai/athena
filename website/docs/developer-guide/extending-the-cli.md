...
Athena exposes protected extension hooks on `AthenaCLI` so wrapper CLIs can add widgets, keybindings, and layout customizations without overriding the `run()` method or the TUI construction in `athena_cli/cli_tui_mixin.py` (where these hooks are defined; `AthenaCLI` in `cli.py` mixes it in). This keeps your extension decoupled from internal changes.
