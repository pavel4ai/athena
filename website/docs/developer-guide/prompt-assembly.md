...
2. **context** — caller-supplied `system_message`, project context files (`.athena.md` / `AGENTS.md` / `CLAUDE.md` / `.cursorrules`), then the worktree-dependent git workspace snapshot, operator instructions and platform hints
...
the first host-info paragraph after the rendered `# Athena runtime environment` boundary, with a
...
```
# Layer 1: Agent Identity (from ~/.athena/SOUL.md)
...
The platform hint (Layer 10 above) is the per-surface guidance Athena
...
`SOUL.md` lives at `~/.athena/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_athena_md(cwd_path)       # 1. .athena.md / ATHENA.md (walks to git root)
    ...
    # SOUL.md from ATHENA_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.athena.md`, `ATHENA.md` | CWD up to git root | Athena-native project config |
...
- **YAML frontmatter stripped** — `.athena.md` frontmatter is removed (reserved for future config overrides)
...
1. `.athena.md` / `ATHENA.md` (walks to git root)
...
- `~/.athena/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.athena/MEMORY.md` and `~/.athena/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `.athena.md`, `ATHENA.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
...
- Ephemeral overlays such as `ATHENA_EPHEMERAL_SYSTEM_PROMPT` or prefill messages — add turn-scoped guidance that should not become part of the cached prompt prefix.
