...
2. **User** — `~/.athena/plugins/web/<name>/` (opt-in via `plugins.enabled` or `athena plugins enable <name>`)
...
When neither key is set, Athena auto-detects the backend from whichever API key/URL is present in the environment. `athena tools` walks users through selection.
...
class MyBackendWebSearchProvider(WebSearchProvider):
    ...
    def display_name(self) -> str:
        # Human label shown in `athena tools`. Defaults to `name`.
        return "My Backend"

    def is_available(self) -> bool:
        # Cheap check — env var present, optional dep importable, etc.
        # MUST NOT make network calls (runs on every `athena tools` paint).
...
| `provides_web_providers` | List of provider `name`s this plugin registers — used by the loader to advertise the plugin in `athena tools` even before `register()` runs |
| `requires_env` | Interactive credential prompt during `athena plugins install` (see [Build a Athena Plugin](/guides/build-a-athena-plugin#gate-on-environment-variables) for the rich format) |
...
| `display_name` | — | `name` | Label shown in `athena tools` |
...
Errors surface as the tool result; the LLM decides how to explain them. If no provider is registered (or every available one fails the capability gate), the tool returns a helpful error pointing at `athena tools`.

## Lazy-installing optional dependencies

If your provider wraps a third-party SDK (like DDGS does with the `ddgs` package), don't `import` it at module top level. Use `tools.lazy_deps.ensure(...)` inside `is_available()` or `search()` — Athena will install the package on first use, gated by `security.allow_lazy_installs`. See [Build a Athena Plugin → Lazy-install](/guides/build-a-athena-plugin#lazy-install-optional-python-dependencies) for the security model.
...
`my_backend_web_package` must expose a top-level `register` function. See [Distribute via pip](/guides/build-a-athena-plugin#distribute-via-pip) in the general plugin guide for the full setup.
