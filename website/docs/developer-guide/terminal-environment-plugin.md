...
Athena runs shell commands through a pluggable set of **terminal backends**.
...
installed under `~/.athena/plugins/`, registering a backend the user selects
...
| `athena setup` backend picker | `display_name`, `description`, `setup_instructions()`, `post_setup()` |
| Dashboard terminal-backend picker (probe status) | `probe()` |
| `athena status` / `athena doctor` | `doctor_checks()` |
...
```python title="~/.athena/plugins/acmebox/__init__.py"
...
class AcmeBoxProvider(TerminalEnvironmentProvider):
    ...
    def cache_path_base(self):
        return "~/.athena"    # where synced cache files land, or None
...
```yaml title="~/.athena/plugins/acmebox/plugin.yaml"
name: acmebox
version: 0.1.0
...
```bash
athena plugins enable acmebox
athena config set terminal.backend acmebox
```
