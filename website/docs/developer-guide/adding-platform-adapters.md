...
- **Plugin** (recommended for community/third-party): Drop a plugin directory into `~/.athena/plugins/` — zero core code changes needed. See [Plugin Path](#plugin-path-recommended) below.
...
Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `athena config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-athena-config) below).
...
With `provides_tools` declared, Athena imports only `tools.py` during plugin
...
`athena tools enable my_platform --platform cli`, or by listing the toolset
...
| `athena config` UI entries | `requires_env` / `optional_env` in `plugin.yaml` auto-populate |
...
| `athena status` | Shows plugin platforms with `(plugin)` tag |
| `athena gateway setup` | Plugin platforms appear in setup menu |
| `athena tools` / `athena skills` | Plugin platforms in per-platform config |
...
direct `athena send --to ...` and cron `deliver=platform:...` by declaring send
...
def register(ctx):
    ctx.register_platform(
        ...
        # May be a regular function or async def. Athena awaits any awaitable
...
Most users set up a platform by dropping env vars into `~/.athena/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `athena gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
`cron_deliver_env_var` makes your platform a recognized `deliver=` target. To make the actual send succeed when the cron job runs in a separate process from the gateway (i.e., `athena cron run` separate from `athena gateway`), register a `standalone_sender_fn`:
...
## Surfacing Env Vars in `athena config`

`athena_cli/config.py` scans `plugins/platforms/*/plugin.yaml` at import time and auto-populates `OPTIONAL_ENV_VARS` from `requires_env` and (optional) `optional_env` blocks. Use the rich-dict form to contribute proper descriptions, prompts, password flags, and URLs — the CLI setup UI picks them up for free.
...
requires_env:
  ...
  - name: MY_PLATFORM_CHANNEL
    description: "Channel to join (e.g. #athena)"
    prompt: "Channel"
    password: false
optional_env:
  - name: MY_PLATFORM_HOME_CHANNEL
...
1. **`athena_cli/config.py`** — Add all `NEWPLAT_*` vars to `_EXTRA_ENV_KEYS`
2. **`athena_cli/gateway.py`** — Add entry to `_PLATFORMS` list with key, label, emoji, token_var, setup_instructions, and vars
3. **`athena_cli/platforms.py`** — Add `PlatformInfo` entry with label and default_toolset (used by `skills_config` and `tools_config` TUIs)
4. **`athena_cli/setup.py`** — Add `_setup_newplat()` function (can delegate to `gateway.py`) and add tuple to the messaging platforms list
5. **`athena_cli/status.py`** — Add platform detection entry: `"NewPlat": ("NEWPLAT_TOKEN", "NEWPLAT_HOME_CHANNEL")`
6. **`athena_cli/dump.py`** — Add `"newplat": "NEWPLAT_TOKEN"` to platform detection dict
...
1. **`toolsets.py`** — Add `"athena-newplat"` toolset definition with `_ATHENA_CORE_TOOLS`
2. **`toolsets.py`** — Add `"athena-newplat"` to the `"athena-gateway"` includes list
...
| `website/docs/reference/toolsets-reference.md` | athena-newplat toolset |
