...
- setup / `athena model` menu entries
...
1. `athena_cli/auth.py` decides how credentials are found.
2. `athena_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `athena_cli/models.py` and `athena_cli/main.py` make the provider show up in the CLI. (`athena_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `athena_cli/auth.py`
2. `athena_cli/models.py`
3. `athena_cli/runtime_provider.py`
4. `athena_cli/main.py`
...
:::tip
`athena_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `athena setup`.
:::
...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$ATHENA_HOME/plugins/model-providers/` both get picked up.
...
7. `athena model` menu includes the provider
8. `athena setup` wizard delegates to `main.py` automatically
...
User plugins at `$ATHENA_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- Provider-specific `athena model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `athena_cli/auth.py`
- `_PROVIDER_LABELS` in `athena_cli/models_catalog_static.py` (re-exported by `athena_cli/models.py`)
- `_PROVIDER_ALIASES` in both `athena_cli/auth.py` and `athena_cli/models_catalog_static.py`
- CLI `--provider` choices in `athena_cli/main.py`
...
## Step 2: Add auth metadata in `athena_cli/auth.py`
...
## Step 3: Add model catalog and aliases in `athena_cli/models.py`
...
## Step 4: Resolve runtime data in `athena_cli/runtime_provider.py`
...
## Step 5: Wire the CLI in `athena_cli/main.py`

A provider is not discoverable until it shows up in the interactive `athena model` flow.

Update these in `athena_cli/main.py`:
...
:::tip
`athena_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `athena model` and `athena setup` automatically.
:::
...
- `tests/athena_cli/test_runtime_provider_resolution.py`
- `tests/cli/test_cli_provider_resolution.py`
- `tests/athena_cli/test_model_switch_custom_providers.py` (and adjacent `tests/athena_cli/test_model_switch_*.py`)
- `tests/athena_cli/test_setup_model_provider.py`
...
python -m pytest tests/athena_cli/test_runtime_provider_resolution.py tests/cli/test_cli_provider_resolution.py tests/athena_cli/test_setup_model_provider.py tests/run_agent/test_provider_parity.py -q
```
...
```bash
source venv/bin/activate
python -m athena_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
```bash
source venv/bin/activate
python -m athena_cli.main model
...
- [ ] `ProviderConfig` added in `athena_cli/auth.py`
- [ ] aliases added in `athena_cli/auth.py` and `athena_cli/models.py`
- [ ] model catalog added in `athena_cli/models.py`
- [ ] runtime branch added in `athena_cli/runtime_provider.py`
- [ ] CLI wiring added in `athena_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `athena model` but not `athena setup`
