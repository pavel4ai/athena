...
| Variable | Meaning |
|----------|---------|
| `ATHENA_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `ATHENA_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `ATHENA_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Athena itself — they're private to these helpers. The variables Athena *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `athena --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_athena_root)" || { echo "htui: not in a Athena checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$ATHENA_MAIN_CHECKOUT/.venv/bin/python" -m athena_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `ATHENA_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_athena_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `ATHENA_TUI_DIR` are mutually exclusive
`ATHENA_TUI_DIR` points Athena at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `athena --tui --dev` exits with an error. Run `unset ATHENA_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_athena_root)" || { echo "hgui: not in a Athena checkout" >&2; return 1; }
  deps="${ATHENA_GUI_DEPS_CHECKOUT:-$ATHENA_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _athena_link_deps "$desktop" "$deps/apps/desktop"
    _athena_link_deps "$root" "$deps"
  else
  ...
  trap '_athena_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    ATHENA_DESKTOP_ATHENA_ROOT="$root" \
    ATHENA_DESKTOP_PYTHON="$ATHENA_MAIN_CHECKOUT/.venv/bin/python" \
    ATHENA_DESKTOP_IGNORE_EXISTING=1 \
    ATHENA_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `ATHENA_DESKTOP_ATHENA_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `athena`. |
| `ATHENA_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `ATHENA_DESKTOP_IGNORE_EXISTING` | Ignores any `athena` on `PATH` so it can't shadow the worktree. |
| `ATHENA_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Athena checkout.
_athena_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/athena_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_athena_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'athena_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `athena --tui --dev` and the `ATHENA_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/pavel4ai/athena/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `ATHENA_*` variable Athena reads
