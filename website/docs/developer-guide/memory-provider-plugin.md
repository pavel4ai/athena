...
:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `athena plugins`.
:::

## Installation Layouts

Athena discovers memory providers from four sources, in this precedence order:

| Source | Location | Notes |
|---|---|---|
| Bundled | `plugins/memory/<name>/` | Ships with Athena. Closed to new providers — see [CONTRIBUTING](https://github.com/pavel4ai/athena/blob/main/CONTRIBUTING.md). |
| User | `$ATHENA_HOME/plugins/<name>/` | Dropped in by the user, per profile. |
| Project | `./.athena/plugins/<name>/` | Opt-in via `ATHENA_ENABLE_PROJECT_PLUGINS=1`. |
| Package | `athena_agent.memory_providers` entry point | `pip install`, nothing to copy. |
...
Athena, in `$ATHENA_HOME/plugins/<name>/` when installed by a user, or in
`./.athena/plugins/<name>/` for a project-local one:
...
`athena_agent.memory_providers` group. The entry-point name is the provider
name users select in `memory.provider`; its value points to the provider's
`register(ctx)` function:

```toml title="pyproject.toml"
[project.entry-points."athena_agent.memory_providers"]
...
package layout. No copy under `$ATHENA_HOME/plugins/` is required.

A package entry point gets everything a directory install does, including the
two files Athena reads from disk rather than importing — `config_schema.py`
(the dashboard config panel) and `cli.py` (your `athena <provider>`
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          athena_home (str): Active ATHENA_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `athena memory setup` | **Yes** |
| `save_config(values, athena_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
The gate binds to every compaction authority, not just the Athena
...
required checkpoint cannot be guaranteed there. The checkpoint-aware Athena
...
`get_config_schema()` returns a list of field descriptors used by `athena memory setup`:
...
Every field in `get_config_schema()` is prompted during `athena memory setup`. Providers with many options should keep the schema minimal — only include fields the user **must** configure (API key, required credentials). Document optional settings in a config file reference (e.g. `$ATHENA_HOME/myprovider.json`) rather than prompting for them all during setup. This keeps the setup wizard fast while still supporting advanced configuration. See the Supermemory provider for an example — it only prompts for the API key; all other options live in `supermemory.json`.
:::

## Save Config

```python
def save_config(self, values: dict, athena_home: str) -> None:
    """Write non-secret config to your native location."""
    import json
    from pathlib import Path
    config_path = Path(athena_home) / "my-provider.json"
...
All storage paths **must** use the `athena_home` kwarg from `initialize()`, not hardcoded `~/.athena`:

```python
# CORRECT — profile-scoped
from athena_constants import get_athena_home
data_dir = get_athena_home() / "my-provider"

# WRONG — shared across all profiles
data_dir = Path("~/.athena/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `athena my-provider status`, `athena my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `athena <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `athena --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: athena my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the athena my-provider argparse tree.
