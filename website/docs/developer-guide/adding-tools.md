...
- [Build a Athena Plugin](/guides/build-a-athena-plugin)
...
2. **`toolsets.py`** — add tool name to `_ATHENA_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `athena_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `athena_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `athena chat -q "Use the weather tool for London"`
