...
Athena is a personal AI agent that runs the same agent core across a CLI, a messaging
...
Read the balance right: Athena ships a **lot**. Most merges are bug fixes to reported
...
- **Expand reach at the edges.** New adapters, channels, providers, models, desktop/TUI/
  ...
  the existing setup/config UX (`athena tools`, `athena setup`, auto-install) rather than
...
- **E2E validation, not just green unit mocks.** Anything touching resolution chains, config
  ...
  real path with real imports against a temp `ATHENA_HOME`. Mocks hide integration bugs.
...
- **New `ATHENA_*` env vars for non-secret config.** `.env` is for secrets only. Behavioral
...
- **Outbound telemetry / usage attribution without opt-in gating.** No analytics,
  ...
  (config gate + setup prompt + `athena tools` toggle) exists. Park behind a label.
...
- **Third-party products integrated into the core tree.** Observability backends, vendor
  ...
  backend we don't own. Ship as a **standalone plugin repo** (`~/.athena/plugins/` or pip
...
2. **CLI command + skill** — config/state/infra expressible as shell commands; the agent runs
   `athena <subcommand>` guided by a skill. Default for subscriptions, scheduled tasks,
   service setup (`athena webhook`, `athena cron`, `athena tools`).
...
4. **Plugin** — third-party/niche/user-specific; lives in `~/.athena/plugins/` or a pip
...
drive a locally spawned backend, one over SSH, one behind URL + token, or Athena Cloud, and
only the first two carry `ATHENA_DESKTOP=1`. An env-keyed gate is a silent no-op on the other
topologies — the tool is stripped from the schema while the platform hint tells the model it
is "inside the Athena desktop app". The pattern:

- **The toolset is the surface gate.** Keep such tools off `_ATHENA_CORE_TOOLS` and in a named
...
- **Ask which identity you mean.** `ATHENA_DESKTOP=1` legitimately means "this backend was
  spawned by the app" (cron ticker, web-dist handling). It does NOT mean "a GUI is watching";
  the embedded terminal pane (`athena --tui` against that backend) is the counterexample.
...
```
`scripts/run_tests.sh` probes `.venv`, then `venv`, then `$HOME/.athena/athena-agent/venv`
...
├── toolsets.py           # TOOLSETS dict, _ATHENA_CORE_TOOLS
├── cli.py                # AthenaCLI (REPL, slash dispatch) + athena_cli/cli_*_mixin.py
├── athena_state.py       # SessionDB facade; athena_state_*.py siblings
├── athena_constants.py   # get_athena_home(), display_athena_home() — profile-aware paths
├── athena_logging.py     # agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # turn_*.py loop phases, providers, memory, compression, prompt builder
├── athena_cli/           # CLI subcommands, setup, config, plugins loader, skins, updater
...
**User state:** `~/.athena/config.yaml` (settings), `~/.athena/.env` (secrets only),
`~/.athena/logs/` (`agent.log` INFO+, `errors.log` WARNING+, `gateway.log`); all
profile-aware via `get_athena_home()`. Browse logs with `athena logs [--follow] [--level] [--session]`.
...
families: `athena_state.py` (21), `gateway/run.py` (15), `tools/mcp_tool.py` (15),
`athena_cli/kanban.py` (14), `athena_cli/web_server.py` (13 + 24 routers), `athena_cli/auth.py`
(12), `tools/browser_tool.py` (11), `cli.py` (12 `athena_cli/cli_*_mixin.py`), `run_agent.py`
...
- **Compat pointers are OFF LIMITS in-tree.** Old import paths kept alive for external plugins
  ...
  `-W error::athena_cli.plugin_compat.AthenaPluginCompatWarning` catches them in the suite.
...
- **Never infer process identity from argv substrings** (`"serve" in cmdline`) — the bug class
  ...
  `athena_cli.update_cmd._athena_holder_subcommand`; flag sets are DERIVED from the parser
  (`_holder_value_flags()`), never hand-written; match FULL cmdlines and truncate only for
  display. Details: `athena_cli/AGENTS.md`.
- **Never hardcode `~/.athena`.** `get_athena_home()` for code paths, `display_athena_home()`
  for user-facing text (both from `athena_constants`). Hardcoding breaks profiles (5 bugs in
  PR #3575). Module-level constants are fine — they cache after `_apply_profile_override()`
  sets `ATHENA_HOME`. Profile operations themselves are HOME-anchored
  (`_get_profiles_root()` = `Path.home()/.athena/profiles`) so `athena -p x profile list`
  sees all profiles — intentional, not a bug.
- **Argparse alias dispatch:** `add_parser("list", aliases=["ls"])` sets `dest` to the literal
  the user typed (`"ls"`). Dispatch must accept both (caught PTY-testing `athena webhook ls`).
- **Don't wire in dead code without E2E validation.** Unshipped code was dead for a reason;
  E2E the real resolution chain with real imports against a temp `ATHENA_HOME` first.
...
vars unset, `TZ=UTC`, `LANG=C.UTF-8`, `ATHENA_HOME` → temp dir, and per-file subprocess
...
- **Flake policy:** a failing FILE is retried once in a fresh subprocess (`--file-retries`;
  `ATHENA_TEST_FILE_RETRIES=0` disables). Pass-on-retry is green but printed under `⚠ FLAKY`
...
- **Tests must not write to `~/.athena/`.** The autouse `_isolate_athena_home` fixture in
  `tests/conftest.py` redirects `ATHENA_HOME`; never hardcode `~/.athena/` in tests. Profile
  tests also mock `Path.home()` so `_get_profiles_root()` / `_get_default_athena_home()` stay
  in the temp dir (pattern: `tests/athena_cli/test_profiles.py`):
  ```python
  @pytest.fixture
  def profile_env(tmp_path, monkeypatch):
      ...
      return home
  ```
  Tests that `patch.object(Path, "home", ...)` must ALSO set `ATHENA_HOME` — code reads the
  env var, not `Path.home()/.athena`.
...
`tests/athena_cli/test_venv_holder_windows_live.py` on a real `windows-latest` runner (real
...
| `cli.py`, `athena_cli/`, `main.py` | `athena_cli/AGENTS.md` | CLI mixins, `_SLASH_DISPATCH`, slash registry, config system + loaders, skins, `athena update` pipeline, profiles / multiplex |
...
| `plugins/`, `athena_cli/plugins*.py` | `plugins/AGENTS.md` | Plugin kinds, native compat contract, in-tree policy, Sep-2026 compat window |
| `tui_gateway/`, `ui-tui/` | `tui_gateway/AGENTS.md` | Process model, JSON-RPC transport, key surfaces, slash flow, dev commands |
| `web/`, `athena_cli/web_routers/` | `web/AGENTS.md` | Dashboard embeds the real TUI; what React may and may not rebuild |
...
| `cron/`, kanban (`athena_cli/kanban*.py`, `tools/kanban_tools.py`, `plugins/kanban/`) | `cron/AGENTS.md` | Scheduler invariants, job fields, kanban board/dispatcher |
...
`athena-agent-dev` skill, not here.
