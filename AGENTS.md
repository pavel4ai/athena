...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`athena tools`, `athena setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `ATHENA_HOME`. Mocks hide integration bugs.
...
- **New `ATHENA_*` env vars for non-secret config.** `.env` is for secrets
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `athena tools`
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `athena <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `athena webhook`, `athena cron`, `athena tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.athena/plugins/` or a pip package, discovered at runtime.
...
`$HOME/.athena/athena-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── toolsets.py           # Toolset definitions, _ATHENA_CORE_TOOLS list
├── cli.py                # AthenaCLI class — interactive CLI orchestrator (~11k LOC)
├── athena_state.py       # SessionDB — SQLite session store (FTS5 search)
├── athena_constants.py   # get_athena_home(), display_athena_home() — profile-aware paths
├── athena_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── athena_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── athena-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `athena --tui`
...
**User config:** `~/.athena/config.yaml` (settings), `~/.athena/.env` (API keys only).
**Logs:** `~/.athena/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_athena_home()`.
Browse with `athena logs [--follow] [--level ...] [--session ...]`.
...
- **Skin engine** (`athena_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `AthenaCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.athena/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`athena_cli/commands.py`)
...
- **Slack** — `slack_subcommand_map()` generates `/athena` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `athena_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `AthenaCLI.process_command()` in `cli.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `athena --tui` or `ATHENA_TUI=1`.
...
### TUI in the Dashboard (`athena dashboard` → `/chat`)

The dashboard embeds the real `athena --tui` — **not** a rewrite.  See `athena_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `athena_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `athena --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `athena --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). It does NOT embed `athena --tui` — it has its own composer, transcript, and slash-command pipeline. Route desktop bugs to the `athena-desktop-app-work` skill, not `athena-dashboard-work`.
...
Use the plugin route instead: create `~/.athena/plugins/<name>/plugin.yaml`
and `~/.athena/plugins/<name>/__init__.py`, then register tools with
...
**2. Add to `toolsets.py`** — either `_ATHENA_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_ATHENA_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_athena_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `ATHENA_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_athena_home()` for the base directory — never `Path.home() / ".athena"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `athena_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `athena_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `athena tools`, `athena setup`, most CLI subcommands | `athena_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`athena_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
athena_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.athena/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
Add to `_BUILTIN_SKINS` dict in `athena_cli/skin_engine.py`:
...
Users create `~/.athena/skins/<name>.yaml`:
...
`~/.athena/plugins/` and pip-installed entry points.

### General plugins (`athena_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.athena/plugins/`, `./.athena/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `athena` at startup so
  `athena <pluginname> <subcmd>` works with no change to `main.py`
...
`post_setup(athena_home, config)` for setup-wizard integration.

**CLI commands via `plugins/memory/<name>/cli.py`:** if a memory plugin
defines `register_cli(subparser)`, `discover_plugin_cli_commands()` finds
it at argparse setup time and wires it into `athena <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `athena --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `athena_cli/main.py`, etc.).
...
into `~/.athena/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `athena memory setup` / `post_setup()` without
...
2. User: `$ATHENA_HOME/plugins/model-providers/<name>/`
...
[`athena-example-plugins`](https://github.com/FutureboundCorp/athena-example-plugins)
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `athena skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.athena.tags`, `metadata.athena.category`,
`metadata.athena.related_skills`, `metadata.athena.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.athena.*` by the loader.
...
`"messaging"`); `_ATHENA_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `athena tools` (the curses UI) or the
...
Athena returns a delegation id immediately and the result re-enters the
...
go to `~/.athena/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `athena_cli/curator.py` wires `athena curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.athena/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `athena cron <verb>`
...
- File lock at `~/.athena/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `athena kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `athena_cli/kanban.py` wires `athena kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`athena-kanban-dispatcher.service` for
  standalone dispatcher deployment).

Isolation model:
- **Board** is the hard boundary — workers are spawned with
  `ATHENA_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
in config.yaml (or `ATHENA_BACKGROUND_NOTIFICATIONS` env var):
...
`ATHENA_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `athena_cli/main.py` sets
`ATHENA_HOME` before any module imports. All `get_athena_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_athena_home()` for all ATHENA_HOME paths.** Import from `athena_constants`.
   NEVER hardcode `~/.athena` or `Path.home() / ".athena"` in code that reads/writes state.
   ```python
   # GOOD
...
2. **Use `display_athena_home()` for user-facing messages.** Import from `athena_constants`.
   This returns `~/.athena` for default or `~/.athena/profiles/<name>` for profiles.
   ```python
   # GOOD
   from athena_constants import display_athena_home
   print(f"Config saved to {display_athena_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.athena/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_athena_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_athena_home()`,
   not `Path.home() / ".athena"`.

4. **Tests that mock `Path.home()` must also set `ATHENA_HOME`** — since code now uses
   `get_athena_home()` (reads env var), not `Path.home() / ".athena"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"ATHENA_HOME": str(tmp_path / ".athena")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not ATHENA_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".athena" / "profiles"`, NOT `get_athena_home() / "profiles"`.
   This is intentional — it lets `athena -p coder profile list` see all profiles regardless
   of which one is active.

## Known Pitfalls

### DO NOT hardcode `~/.athena` paths
Use `get_athena_home()` from `athena_constants` for code paths. Use `display_athena_home()`
for user-facing print/log messages. Hardcoding `~/.athena` breaks profiles — each profile
has its own `ATHENA_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### DO NOT introduce new `simple_term_menu` usage
Existing call sites in `athena_cli/main.py` remain for legacy fallback only;
the preferred UI is curses (stdlib) because `simple_term_menu` has
ghost-duplication rendering bugs in tmux/iTerm2 with arrow keys. New
interactive menus must use `athena_cli/curses_ui.py` — see
`athena_cli/tools_config.py` for the canonical pattern.
...
with actual imports (not mocks) against a temp `ATHENA_HOME`.

### Tests must not write to `~/.athena/`
The `_isolate_athena_home` autouse fixture in `tests/conftest.py` redirects `ATHENA_HOME` to a temp dir. Never hardcode `~/.athena/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_athena_home()` resolve within the temp dir.
Use the pattern from `tests/athena_cli/test_profiles.py`:
```python
@pytest.fixture
...
- The plugin disables itself in child processes (sentinel envvar
  `ATHENA_ISOLATE_CHILD=1`), so there's no fork-bomb risk.
...
| HOME / `~/.athena/` | Your real config+auth.json | Temp dir per test |
