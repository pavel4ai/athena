...
This is an additive Athena extension surfaced under ACP ``_meta.athena`` so
...
def build_session_provenance(
    ...
    *,
    ...
    Args:
        ...
        current_athena_session_id: The live internal Athena DB session id
            (``state.agent.session_id``).
        previous_athena_session_id: The internal id from before the most recent
    ...
    """
    try:
        row = db.get_session(current_athena_session_id)
    except Exception:
        return None
    if not row:
        return None
    ...
    root_id = current_athena_session_id
    compression_depth = 0
    ...
    rotated = bool(
        ...
        and previous_athena_session_id != current_athena_session_id
    )

    provenance: Dict[str, Any] = {
        "acpSessionId": acp_session_id,
        "currentAthenaSessionId": current_athena_session_id,
