...
Loads ``~/.athena/.env``, routes logging to stderr (stdout is reserved for ACP
JSON-RPC), and starts the ACP agent server.

Usage::

    python -m acp_adapter.entry   # or: athena acp / athena-acp
"""
...
try:
    ...
    # Stop a ``utils/``/``proxy/``/``ui/`` package in the launch cwd from shadowing Athena modules.
...
def _load_env() -> None:
    """Load .env from ATHENA_HOME (default ``~/.athena``)."""
    from athena_cli.env_loader import load_athena_dotenv

    athena_home = get_athena_home()
    loaded = load_athena_dotenv(athena_home=athena_home)
    log = logging.getLogger(__name__)
    for env_file in loaded or ():
        log.info("Loaded env from %s", env_file)
    if not loaded:
        log.info("No .env found at %s, using system env", athena_home / ".env")


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="athena-acp", description="Run Athena Agent as an ACP stdio server.")
    ...
    parser.add_argument("--setup-browser", action="store_true",
                        help="Install agent-browser + Playwright Chromium into ~/.athena/node/ "
...
def main(argv: list[str] | None = None) -> None:
    ...
    if os.environ.get("ATHENA_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
