...
class AthenaACPAgent(acp.Agent):
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from athena_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _provenance_meta(
        self,
        acp_session_id: str,
        current_athena_session_id: str,
        previous_athena_session_id: Optional[str] = None,
    ) -> Optional[dict]:
    ...
    async def _send_session_info_update(
        ...
        session_id: str,
        *,
        current_athena_session_id: Optional[str] = None,
        previous_athena_session_id: Optional[str] = None,
    ) -> None:
        ...
        split during a turn), pass ``previous_athena_session_id`` so the
        attached ``_meta.athena.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        # athena_state.py schema — only started_at/ended_at). Use "now" as
        ...
        meta = self._provenance_meta(
            session_id,
            current_athena_session_id or session_id,
            previous_athena_session_id,
        )
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        # not the executor's. Also set ATHENA_INTERACTIVE so approval.py
        # takes the CLI-interactive path (which calls the registered
        # callback via prompt_dangerous_approval) instead of the
        # non-interactive auto-approve branch (GHSA-96vc-wcxf-jjff).
        # ACP's conn.request_permission maps cleanly to the interactive
        # callback shape — not the gateway-queue ATHENA_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind ATHENA_SESSION_KEY for this session so per-session caches
            ...
            previous_interactive = os.environ.get("ATHENA_INTERACTIVE")
            ...
            previous_session_id = os.environ.get("ATHENA_SESSION_ID")
            ...
            try:
                ...
                # Restore ATHENA_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("ATHENA_INTERACTIVE", None)
                else:
                    os.environ["ATHENA_INTERACTIVE"] = previous_interactive
                # Restore ATHENA_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("ATHENA_SESSION_ID", None)
                else:
                    os.environ["ATHENA_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            ...
            # stomp on each other's ContextVar writes (ATHENA_SESSION_KEY in
        ...
        # _meta.athena.sessionProvenance so ACP clients can render the boundary
        # and keep old/new ids in lineage. The ACP session_id is unchanged.
        post_turn_athena_id = getattr(state.agent, "session_id", None)
        if (
            conn
            and post_turn_athena_id
            and pre_turn_athena_id
            and post_turn_athena_id != pre_turn_athena_id
        ):
            try:
                await self._send_session_info_update(
                    session_id,
                    current_athena_session_id=post_turn_athena_id,
